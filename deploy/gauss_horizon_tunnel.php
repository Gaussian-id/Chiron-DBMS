...
/*
 * Gauss Horizon HTTP Script Tunnel
 *
 * Upload this file to a PHP server that can reach the target database.
 *
 * Required configuration:
 *   GAUSS_HORIZON_TUNNEL_TOKEN=change-this-to-a-long-random-string
 *
 * Optional configuration:
 *   GAUSS_HORIZON_TUNNEL_DIR=/tmp/gauss_horizon_tunnel
 *   GAUSS_HORIZON_TUNNEL_ALLOWED_HOSTS=mysql.internal,postgres.internal
 *   GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS=3600
 *   GAUSS_HORIZON_TUNNEL_PHP=/usr/bin/php
 */

$GAUSS_HORIZON_TUNNEL_TOKEN = getenv('GAUSS_HORIZON_TUNNEL_TOKEN') ?: '';
$GAUSS_HORIZON_TUNNEL_DIR = getenv('GAUSS_HORIZON_TUNNEL_DIR') ?: sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'gauss_horizon_tunnel';
$GAUSS_HORIZON_TUNNEL_ALLOWED_HOSTS = array_values(array_filter(array_map('trim', explode(',', getenv('GAUSS_HORIZON_TUNNEL_ALLOWED_HOSTS') ?: ''))));
$GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS = max(30, (int) (getenv('GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS') ?: '3600'));

// Keep sequential protocol exchanges responsive, then restore the original idle cadence.
const GAUSS_HORIZON_WORKER_ACTIVE_POLL_US = 10000;
const GAUSS_HORIZON_WORKER_WARM_POLL_US = 50000;
const GAUSS_HORIZON_WORKER_IDLE_POLL_US = 200000;
const GAUSS_HORIZON_WORKER_ACTIVE_POLL_COUNT = 100;
const GAUSS_HORIZON_WORKER_WARM_POLL_COUNT = 20;

// Standalone tests load the worker helpers without dispatching an HTTP request.
if (defined('GAUSS_HORIZON_TUNNEL_FUNCTIONS_ONLY') && GAUSS_HORIZON_TUNNEL_FUNCTIONS_ONLY) {
    return;
}

if (PHP_SAPI === 'cli' && isset($argv[1]) && $argv[1] === '--gauss-horizon-worker') {
...
try {
    ...
    $action = request_param('gauss_horizon_action');
    switch ($action) {
        case 'open':
            handle_open();
            break;
        ...
        case 'close':
            ...
            break;
        default:
            respond_error(400, 'Unsupported gauss_horizon_action');
    }
} catch (Throwable $e) {
...
function handle_open(): void
{
    ...
    $host = request_param('gauss_horizon_target_host');
    $port = (int) request_param('gauss_horizon_target_port');
...
{
    $dir = existing_session_dir(request_session());
    $waitMs = max(0, min(5000, (int) (request_param('gauss_horizon_wait_ms', '1000'))));
...
{
    global $GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS;
    ...
    $expiresAt = time() + $GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS;
    $lastActivity = time();
    $idlePolls = 0;
    ...
    try {
        while (time() < $expiresAt) {
            ...
            if (time() - $lastActivity > $GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS) {
                break;
            }
        }
    } catch (Throwable $e) {
...
{
    if ($idlePolls < GAUSS_HORIZON_WORKER_ACTIVE_POLL_COUNT) {
        return GAUSS_HORIZON_WORKER_ACTIVE_POLL_US;
    }
    if ($idlePolls < GAUSS_HORIZON_WORKER_ACTIVE_POLL_COUNT + GAUSS_HORIZON_WORKER_WARM_POLL_COUNT) {
        return GAUSS_HORIZON_WORKER_WARM_POLL_US;
    }
    return GAUSS_HORIZON_WORKER_IDLE_POLL_US;
}
...
function next_worker_idle_poll_count(int $idlePolls, bool $activity): int
{
    ...
    return min($idlePolls + 1, GAUSS_HORIZON_WORKER_ACTIVE_POLL_COUNT + GAUSS_HORIZON_WORKER_WARM_POLL_COUNT);
}
...
function spawn_worker(string $dir, string $host, int $port, int $connectTimeout): bool
{
    ...
    $php = getenv('GAUSS_HORIZON_TUNNEL_PHP') ?: PHP_BINARY;
    if ($php === '') {
        return false;
    }
    $cmd = escapeshellarg($php)
        . ' ' . escapeshellarg(__FILE__)
        . ' --gauss-horizon-worker ' . escapeshellarg($dir)
        . ' ' . escapeshellarg($host)
...
{
    global $GAUSS_HORIZON_TUNNEL_TOKEN;

    if ($GAUSS_HORIZON_TUNNEL_TOKEN === '') {
        respond_error(503, 'GAUSS_HORIZON_TUNNEL_TOKEN is not configured');
    }
    $provided = request_token();
    if ($provided === '' || !hash_equals($GAUSS_HORIZON_TUNNEL_TOKEN, $provided)) {
...
{
    if (isset($_SERVER['HTTP_X_GAUSS_HORIZON_TUNNEL_TOKEN'])) {
        return trim((string) $_SERVER['HTTP_X_GAUSS_HORIZON_TUNNEL_TOKEN']);
    }
...
{
    global $GAUSS_HORIZON_TUNNEL_ALLOWED_HOSTS;
    ...
    if ($GAUSS_HORIZON_TUNNEL_ALLOWED_HOSTS !== [] && !in_array($host, $GAUSS_HORIZON_TUNNEL_ALLOWED_HOSTS, true)) {
...
{
    $session = request_param('gauss_horizon_session');
...
{
    $timeout = (int) request_param('gauss_horizon_connect_timeout', '10');
    return max(1, min(300, $timeout));
}

function session_dir(string $session): string
{
    global $GAUSS_HORIZON_TUNNEL_DIR;

    ensure_base_dir();
    return rtrim($GAUSS_HORIZON_TUNNEL_DIR, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . $session;
}
...
function ensure_base_dir(): void
{
    ...
    if (!is_dir($GAUSS_HORIZON_TUNNEL_DIR) && !mkdir($GAUSS_HORIZON_TUNNEL_DIR, 0700, true) && !is_dir($GAUSS_HORIZON_TUNNEL_DIR)) {
...
{
    global $GAUSS_HORIZON_TUNNEL_DIR, $GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS;

    if (!is_dir($GAUSS_HORIZON_TUNNEL_DIR)) {
        return;
    }
    foreach (glob(rtrim($GAUSS_HORIZON_TUNNEL_DIR, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . '*', GLOB_ONLYDIR) ?: [] as $dir) {
        if (filemtime($dir) !== false && time() - filemtime($dir) > ($GAUSS_HORIZON_TUNNEL_MAX_SESSION_SECONDS * 2)) {
            remove_dir($dir);
        }
    }
}
