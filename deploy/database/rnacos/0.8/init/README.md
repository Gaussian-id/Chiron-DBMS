...
The service initializes the `admin` administrator from `RNACOS_INIT_ADMIN_USERNAME` and `RNACOS_INIT_ADMIN_PASSWORD` when its data volume is empty. The Chiron Horizon recipe defaults the password to `123456`; set `DB_PASSWORD` before the first start to choose another password.
