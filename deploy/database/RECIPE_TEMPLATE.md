...
Every recipe must use a pinned image, default its host binding to `${DB_BIND_ADDRESS:-127.0.0.1}`, allocate unique host ports from its product's `101xx`-`115xx` range, set `container_name` to `chiron-horizon-<product>-<version>`, use password `123456`, and initialize database `chiron-horizon`. Redis-like services without named databases use DB 0 and the `chiron-horizon:` key prefix.
