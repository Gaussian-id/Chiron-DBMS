...
/*
 * Chiron Horizon HTTP Script Tunnel
 *
 * Upload this file to a PHP server that can reach the target database.
 *
 * Required configuration:
 *   CHIRON_HORIZON_TUNNEL_TOKEN=change-this-to-a-long-random-string
 *
 * Optional configuration:
 *   CHIRON_HORIZON_TUNNEL_DIR=/tmp/chiron_horizon_tunnel
 *   CHIRON_HORIZON_TUNNEL_ALLOWED_HOSTS=mysql.internal,postgres.internal
 *   CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS=3600
 *   CHIRON_HORIZON_TUNNEL_PHP=/usr/bin/php
 */

$CHIRON_HORIZON_TUNNEL_TOKEN = getenv('CHIRON_HORIZON_TUNNEL_TOKEN') ?: '';
$CHIRON_HORIZON_TUNNEL_DIR = getenv('CHIRON_HORIZON_TUNNEL_DIR') ?: sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'chiron_horizon_tunnel';
$CHIRON_HORIZON_TUNNEL_ALLOWED_HOSTS = array_values(array_filter(array_map('trim', explode(',', getenv('CHIRON_HORIZON_TUNNEL_ALLOWED_HOSTS') ?: ''))));
$CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS = max(30, (int) (getenv('CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS') ?: '3600'));

// Keep sequential protocol exchanges responsive, then restore the original idle cadence.
const CHIRON_HORIZON_WORKER_ACTIVE_POLL_US = 10000;
const CHIRON_HORIZON_WORKER_WARM_POLL_US = 50000;
const CHIRON_HORIZON_WORKER_IDLE_POLL_US = 200000;
const CHIRON_HORIZON_WORKER_ACTIVE_POLL_COUNT = 100;
const CHIRON_HORIZON_WORKER_WARM_POLL_COUNT = 20;

// Standalone tests load the worker helpers without dispatching an HTTP request.
if (defined('CHIRON_HORIZON_TUNNEL_FUNCTIONS_ONLY') && CHIRON_HORIZON_TUNNEL_FUNCTIONS_ONLY) {
    return;
}

if (PHP_SAPI === 'cli' && isset($argv[1]) && $argv[1] === '--chiron-horizon-worker') {
...
try {
    ...
    $action = request_param('chiron_horizon_action');
    switch ($action) {
        case 'open':
            handle_open();
            break;
        ...
        case 'close':
            ...
            break;
        default:
            respond_error(400, 'Unsupported chiron_horizon_action');
    }
} catch (Throwable $e) {
...
function handle_open(): void
{
    ...
    $host = request_param('chiron_horizon_target_host');
    $port = (int) request_param('chiron_horizon_target_port');
...
{
    $dir = existing_session_dir(request_session());
    $waitMs = max(0, min(5000, (int) (request_param('chiron_horizon_wait_ms', '1000'))));
...
{
    global $CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS;
    ...
    $expiresAt = time() + $CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS;
    $lastActivity = time();
    $idlePolls = 0;
    ...
    try {
        while (time() < $expiresAt) {
            ...
            if (time() - $lastActivity > $CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS) {
                break;
            }
        }
    } catch (Throwable $e) {
...
{
    if ($idlePolls < CHIRON_HORIZON_WORKER_ACTIVE_POLL_COUNT) {
        return CHIRON_HORIZON_WORKER_ACTIVE_POLL_US;
    }
    if ($idlePolls < CHIRON_HORIZON_WORKER_ACTIVE_POLL_COUNT + CHIRON_HORIZON_WORKER_WARM_POLL_COUNT) {
        return CHIRON_HORIZON_WORKER_WARM_POLL_US;
    }
    return CHIRON_HORIZON_WORKER_IDLE_POLL_US;
}
...
function next_worker_idle_poll_count(int $idlePolls, bool $activity): int
{
    ...
    return min($idlePolls + 1, CHIRON_HORIZON_WORKER_ACTIVE_POLL_COUNT + CHIRON_HORIZON_WORKER_WARM_POLL_COUNT);
}
...
function spawn_worker(string $dir, string $host, int $port, int $connectTimeout): bool
{
    ...
    $php = getenv('CHIRON_HORIZON_TUNNEL_PHP') ?: PHP_BINARY;
    if ($php === '') {
        return false;
    }
    $cmd = escapeshellarg($php)
        . ' ' . escapeshellarg(__FILE__)
        . ' --chiron-horizon-worker ' . escapeshellarg($dir)
        . ' ' . escapeshellarg($host)
...
{
    global $CHIRON_HORIZON_TUNNEL_TOKEN;

    if ($CHIRON_HORIZON_TUNNEL_TOKEN === '') {
        respond_error(503, 'CHIRON_HORIZON_TUNNEL_TOKEN is not configured');
    }
    $provided = request_token();
    if ($provided === '' || !hash_equals($CHIRON_HORIZON_TUNNEL_TOKEN, $provided)) {
...
{
    if (isset($_SERVER['HTTP_X_CHIRON_HORIZON_TUNNEL_TOKEN'])) {
        return trim((string) $_SERVER['HTTP_X_CHIRON_HORIZON_TUNNEL_TOKEN']);
    }
...
{
    global $CHIRON_HORIZON_TUNNEL_ALLOWED_HOSTS;
    ...
    if ($CHIRON_HORIZON_TUNNEL_ALLOWED_HOSTS !== [] && !in_array($host, $CHIRON_HORIZON_TUNNEL_ALLOWED_HOSTS, true)) {
...
{
    $session = request_param('chiron_horizon_session');
...
{
    $timeout = (int) request_param('chiron_horizon_connect_timeout', '10');
    return max(1, min(300, $timeout));
}

function session_dir(string $session): string
{
    global $CHIRON_HORIZON_TUNNEL_DIR;

    ensure_base_dir();
    return rtrim($CHIRON_HORIZON_TUNNEL_DIR, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . $session;
}
...
function ensure_base_dir(): void
{
    ...
    if (!is_dir($CHIRON_HORIZON_TUNNEL_DIR) && !mkdir($CHIRON_HORIZON_TUNNEL_DIR, 0700, true) && !is_dir($CHIRON_HORIZON_TUNNEL_DIR)) {
...
{
    global $CHIRON_HORIZON_TUNNEL_DIR, $CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS;

    if (!is_dir($CHIRON_HORIZON_TUNNEL_DIR)) {
        return;
    }
    foreach (glob(rtrim($CHIRON_HORIZON_TUNNEL_DIR, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . '*', GLOB_ONLYDIR) ?: [] as $dir) {
        if (filemtime($dir) !== false && time() - filemtime($dir) > ($CHIRON_HORIZON_TUNNEL_MAX_SESSION_SECONDS * 2)) {
            remove_dir($dir);
        }
    }
}
