...
Gauss Horizon uses a hybrid driver architecture: built-in Rust drivers, standalone native Agents, and Java/JDBC Agents.

## Driver Architecture

| Driver Type       | How It Works                         | Best For                                                                                                                                                        |
| ----------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native (built-in) | Rust drivers compiled into Gauss Horizon       | MySQL, PostgreSQL, SQLite, SQL Server, Redis, MongoDB, ClickHouse, and more                                                                                      |
| Native Agent / external driver | Standalone Go/Rust process or sidecar managed by Gauss Horizon | Oracle, KingbaseES, XuguDB, DuckDB, and connections such as RabbitMQ that require a separate runtime |
| JDBC Agent        | Java subprocess managed by Gauss Horizon       | Databases that require JDBC: GaussDB, openGauss, DM, HighGo, Vastbase, UXDB, Trino, Hive, DB2, Informix, Neo4j, TDengine, YashanDB, GoldenDB, Kylin, SunDB, and more |
...
<Steps>
  ...
  <Step>
    ...
    Gauss Horizon downloads the native Agent executable or JDBC Agent JAR and any required runtime. Progress is shown during download.
  </Step>
  <Step>
    ### Create a Connection
...
JDBC agent drivers require a Java Runtime Environment. Gauss Horizon includes JRE management:

- **System JRE**: Gauss Horizon can use an existing Java installation on your system
- **Managed JRE**: Gauss Horizon can download and manage its own JRE, isolated from your system Java
...
Gauss Horizon checks the Gauss Horizon release registry when you refresh Driver Store:
...
For environments without internet access, Gauss Horizon supports offline driver installation:

1. Download the Gauss Horizon offline driver bundle matching the target OS and CPU architecture on another machine
...
Download `gauss-horizon-agent-kingbase-<version>-windows-x64.tar.zst` for Windows x64, then choose **Import offline package** in Driver Manager. Do not import a Linux or macOS package.

The package contains the version metadata and native agent for that platform. Gauss Horizon installs it as `%USERPROFILE%\.gauss-horizon\agents\drivers\kingbase\agent.exe`, without requiring users to unblock or manually copy the executable.

### Importing the JDBC plugin offline

Generic JDBC connections (custom JDBC URL + imported driver JAR) run through a separate **Gauss Horizon JDBC plugin**, released as the immutable `gauss-horizon-jdbc-plugin-0.1.0.zip` asset. It is also included in the complete platform offline agent bundle. Install the standalone ZIP with **Local install** in the **JDBC Drivers** tab, or import the full offline bundle when the machine is air-gapped.
...
Version 0.1.0 uses a fixed, immutable JDBC plugin asset. The desktop updater is disabled; install a later plugin explicitly from a verified Gauss Horizon release when one is available.
