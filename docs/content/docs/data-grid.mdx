---
title: Data Grid
description: Browse, filter, edit, validate, copy, and export query results or table data in Chiron Horizon.
---

The data grid displays query results and table data. It covers both read-only analysis and small, controlled changes: browsing large result sets, filtering and sorting, inspecting complex values, bulk copying, and generating previewable `INSERT`, `UPDATE`, or `DELETE` statements when Chiron Horizon can reliably identify the target rows.
...
Chiron Horizon uses virtual scrolling and renders only the visible area. Common browsing operations include:
...
Results are editable only when Chiron Horizon can identify the target table and has enough key or row-identity information to generate reviewable SQL.
...
Chiron Horizon uses specialized controls for recognizable date/time, enum, JSON, and related types. Before bulk editing, verify that the selected region matches the target column types.

## SQL Preview and Save

Row-level relational mutations remain local until saved. Chiron Horizon then shows the generated `INSERT`, `UPDATE`, and `DELETE` statements.

A MySQL condition-wide bulk update is different because not all matching rows are necessarily loaded into the grid. In the bulk-edit dialog, use the current table filter or build matching conditions by choosing columns, operators, and values; combine multiple rules with `AND` or `OR`. A custom `WHERE` expression remains available as an advanced option. You can preview the estimated affected-row count before continuing; it is a snapshot and the actual count can differ when the update executes. Chiron Horizon shows the exact `UPDATE ... WHERE ...` statement in a danger confirmation and executes it immediately after confirmation. The database evaluates the condition at execution time, and Chiron Horizon cannot generate reliable rollback SQL for rows that were not loaded.
...
When Chiron Horizon can resolve a foreign-key relationship, a cell can navigate to the referenced record. This depends on metadata and target-connection permissions.
