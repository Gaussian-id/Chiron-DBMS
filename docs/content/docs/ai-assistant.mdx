...
Chiron Horizon includes presets for:
...
Compatible and custom configurations can set the endpoint, model, API style, authentication method, proxy, context window, and extra HTTP headers. API styles include OpenAI-style completions, the Responses API, and Anthropic Messages. When supported, Chiron Horizon discovers models and filters out endpoints that are not suitable for assistant or Agent use.
...
You can configure the executable path and additional environment variables. CLI Agents run on the Desktop computer and access the active database through Chiron Horizon-scoped MCP tools. Docker/Web hides these local CLI providers because the server cannot assume the browser computer has the commands installed.
...
<Steps>
  ...
  <Step>
    ...
    API modes accept an API key, auth method, endpoint, proxy, and custom HTTP headers. Use headers for gateway authentication or tenant routing, for example `X-GoModel-User-Path: /chiron-horizon-assistant`; they are sent for model discovery, connection tests, chat, and Agent requests. Supported models expose effort, reasoning, or thinking controls based on provider capabilities rather than one universal setting.
...
Multiple templates can be active at once. Chiron Horizon snapshots the selected templates and global instructions when the request is sent, so later edits do not alter an already running task.
...
5. AI Agents cannot receive write or DDL authorization for production databases; they can only return SQL for manual review in Chiron Horizon
...
Providers expose different mechanisms: enumerated effort, reasoning levels, boolean thinking, integer budgets, or free-text options. Chiron Horizon shows controls from the active model capability and remembers the selection per configuration and model.
...
<Cards>
  ...
  <Card title="MCP Integration" href="/en/docs/mcp">
    Connect external AI coding assistants to databases through Chiron Horizon.
  </Card>
