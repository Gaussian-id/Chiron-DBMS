...
Chiron Horizon can import connections from other database tools, saving you from manually re-creating dozens of connections when switching tools.
...
<Steps>
  ...
  <Step>
    ...
    In Chiron Horizon, go to **Settings → Import Connections** or right-click the sidebar group header and choose **Import Connections**.
  ...
  <Step>
    ...
    Chiron Horizon parses the file and lists all found connections. Review the list, re-enter passwords where needed, and save.
...
<Steps>
  ...
  <Step>
    ...
    In Chiron Horizon, go to **Settings → Import Connections**.
  ...
  <Step>
    ...
    Chiron Horizon extracts connection details from the configuration. Re-enter any passwords that could not be decrypted.
...
<Steps>
  <Step>
    ...
    Choose **Import DataGrip** in Chiron Horizon and select `dataSources.xml` from a DataGrip project or global configuration. A matching local file in the same directory can supply additional fields.

  </Step>
  <Step>
    ### Review Type Detection

    Chiron Horizon combines driver references, database product names, JDBC subprotocols, and driver classes. Databases such as KingbaseES map to their Chiron Horizon profile; unknown drivers remain generic JDBC.

  </Step>
  <Step>
    ### Review Groups

    DataGrip group paths are imported as Chiron Horizon connection groups so project organization is preserved.

  </Step>
  <Step>
    ### Complete Credentials

    On macOS, Chiron Horizon reads DataGrip Keychain passwords when available. Other encrypted or inaccessible credentials must be entered manually.
