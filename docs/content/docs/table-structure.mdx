...
The table structure editor keeps single-table metadata and a change draft in a persistent tab. Chiron Horizon enables only operations that the active database capability can generate reliably and shows the final DDL before save.
...
- Some databases save only the local display order in Chiron Horizon without changing physical structure
...
Edits do not immediately change the database. Before save, Chiron Horizon generates highlighted dialect-specific SQL such as:
...
<Steps>
  ...
  <Step>
    ...
    Execute after review. Production scope enters production-write confirmation again, and Chiron Horizon reloads structure after success.
