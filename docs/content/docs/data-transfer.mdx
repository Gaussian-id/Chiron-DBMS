...
<Callout type="warn">Cross-engine transfer passes through Chiron Horizon field and type conversion. Target privileges, constraints, triggers, defaults, and engine semantics still determine the result. Rehearse important migrations and validate row counts and key fields afterward.</Callout>
...
<Steps>
  <Step>
    ...
    Choose the source connection, catalog when applicable, database, and schema. Chiron Horizon lists transferable base tables; MongoDB paths list collections.
...
For PostgreSQL-to-PostgreSQL transfers with table creation enabled, Chiron Horizon handles supported sequences, indexes, foreign keys, and selected schema-level objects and dependencies. If source owners do not exist on the target instance, Chiron Horizon prompts you to:
...
A multi-table task does not discard the entire queue after the first table failure. Chiron Horizon records the failure, attempts later tables, and finishes either successfully or with a terminal error summary.
...
Chiron Horizon filters source and target connections through the driver capability manifest. The implementation includes SQL-to-SQL, MongoDB-to-MongoDB, and selected MongoDB/SQL conversion paths. Not every connectable database supports Data Transfer.
