...
<Callout type="warn">Cross-engine transfer passes through Gauss Horizon field and type conversion. Target privileges, constraints, triggers, defaults, and engine semantics still determine the result. Rehearse important migrations and validate row counts and key fields afterward.</Callout>
...
<Steps>
  <Step>
    ...
    Choose the source connection, catalog when applicable, database, and schema. Gauss Horizon lists transferable base tables; MongoDB paths list collections.
...
For PostgreSQL-to-PostgreSQL transfers with table creation enabled, Gauss Horizon handles supported sequences, indexes, foreign keys, and selected schema-level objects and dependencies. If source owners do not exist on the target instance, Gauss Horizon prompts you to:
...
A multi-table task does not discard the entire queue after the first table failure. Gauss Horizon records the failure, attempts later tables, and finishes either successfully or with a terminal error summary.
...
Gauss Horizon filters source and target connections through the driver capability manifest. The implementation includes SQL-to-SQL, MongoDB-to-MongoDB, and selected MongoDB/SQL conversion paths. Not every connectable database supports Data Transfer.
