...
Gauss Horizon chooses a formatter dialect from the active connection and writes the result back to the query editor. When text is selected, only that range is formatted; otherwise Gauss Horizon formats the entire editor.
...
Gauss Horizon currently uses dedicated formatter grammars for:
...
**Restore Defaults** restores the formatter settings defined by the current Gauss Horizon build. Click **Apply** in the settings dialog to persist changes.
...
The JSON editor validates the draft continuously. Gauss Horizon blocks apply or import when:
...
**Export Config** downloads `gauss-horizon-sql-formatter.json`. Import replaces only SQL formatter options; it does not change connections, editor shortcuts, or other Gauss Horizon settings. Use [Configuration Cloud Sync](/en/docs/cloud-sync) when you need a broader settings migration.
...
Gauss Horizon always adds recognition for `${name}` and `#{name}` variables so those templates do not cause formatter parse failures. A non-null legacy `params` option is no longer used for text substitution; configure `paramTypes` instead.
