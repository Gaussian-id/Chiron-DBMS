...
description: Internal HTTP APIs used by the Gauss Horizon Web and Docker UI, including authentication, queries, background jobs, uploads, and downloads.
---

<Callout type="warn">This API powers the Gauss Horizon Web UI and internal tooling such as the MCP Web backend. It is not a separate public integration contract. Endpoints and payloads may change between releases. For scripting and agent workflows, prefer [@gauss-horizon/cli](/en/docs/cli) or [@gauss-horizon/mcp-server](/en/docs/mcp) when possible.</Callout>

## Base URL

By default, Gauss Horizon Web listens on port `4224`:

```text
http://localhost:4224
```

When deployed behind a reverse-proxy subpath, set `GAUSS_HORIZON_PUBLIC_BASE_PATH` and prefix every route. For example, with `/gauss-horizon`:

```text
https://example.com/gauss-horizon/api/auth/check
```
...
Protected routes require a session cookie named `gauss_horizon_session`.
...
On success, the response sets `Set-Cookie: gauss_horizon_session=...`. Reuse that cookie on later requests.

Five consecutive login failures trigger an approximately 60-second lockout. Sessions live in the current Web process and require login again after restart. The cookie is `HttpOnly`, uses `SameSite=Lax`, and follows `GAUSS_HORIZON_PUBLIC_BASE_PATH` for its path.

### Logout

```http
POST /api/auth/logout
Cookie: gauss_horizon_session=...
```
...
| Variable | Purpose |
| --- | --- |
| `GAUSS_HORIZON_PASSWORD` | Set an initial password at container startup |
| `GAUSS_HORIZON_DISABLE_PASSWORD=1` | Disable password protection entirely |
| `GAUSS_HORIZON_PORT` | Change the listen port (default `4224`) |
| `GAUSS_HORIZON_DATA_DIR` | Data directory containing `gauss-horizon.db` |
| `GAUSS_HORIZON_PUBLIC_BASE_PATH` | Serve Gauss Horizon under a subpath such as `/gauss-horizon` |
| `GAUSS_HORIZON_MAX_UPLOAD_MB` | Override the general request and uploaded-file limit; default 1024 MB. Multipart routes reserve fixed protocol overhead (set at least `4096` for a 4 GiB file) |
| `GAUSS_HORIZON_AGENT_DIR` | Override the Web Agent/driver directory; defaults to `agents` under the data directory |
| `GAUSS_HORIZON_STATIC_DIR` | Override the Web static asset directory |
...
Only the data-grid extractor currently exposes a tested, scoped OpenAPI document at `/api/query/data-grid-extractor-openapi.json`. It does not cover the complete Gauss Horizon Web API or version the other routes.
...
- MCP: [@gauss-horizon/mcp-server](/en/docs/mcp)
- CLI: [@gauss-horizon/cli](/en/docs/cli)
...
{
  "env": {
    "GAUSS_HORIZON_WEB_URL": "http://localhost:4224",
    "GAUSS_HORIZON_WEB_PASSWORD": "your-password"
  }
}
```
...
CLI supports the same `GAUSS_HORIZON_WEB_URL` and `GAUSS_HORIZON_WEB_PASSWORD` variables. Both integrations reuse server-side connections, drivers, read-only protection, production protection, and database privileges, making them easier to maintain across Gauss Horizon upgrades than copied internal route payloads.
...
Browse `crates/gauss-horizon-web/src/main.rs` in the repository for the full route list.
