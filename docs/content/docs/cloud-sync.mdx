---
title: Chiron Horizon Configuration Cloud Sync
description: Sync Chiron Horizon connections, settings, and saved SQL with WebDAV, GitHub Gist, or Gitee snippets.
---

<Callout type="info">
Chiron Horizon can package connections, application settings, and saved SQL into a snapshot. Desktop can sync through WebDAV or private snippets, while Web/Docker can sync through WebDAV and restore the snapshot in another Chiron Horizon instance.
</Callout>

<Callout type="warn">Cloud Sync backs up Chiron Horizon configuration. It does not read or back up the business databases behind your connections. Use [Database Export](/en/docs/database-export) for schema and data, or [Database Backup](/en/docs/database-backup) for scheduled Desktop MySQL/PostgreSQL backups.</Callout>
...
<Step>
    ...
    Leave **Snippet ID** empty and select **Upload**. Chiron Horizon creates a private Gist and stores the returned ID.
  </Step>
</Steps>

Chiron Horizon stores an encrypted `chiron-horizon-sync.json` file in the Gist. The snippet encryption password protects the complete snapshot, so the URL does not expose connection details, SQL, or settings. Existing plain-text snippets can still be downloaded once for migration.

<Callout type="warn">
Plain-text snippets created by older versions cannot be safely migrated in place: updating the same Gist does not remove its revision history. Use **Migrate legacy snippet** instead. Chiron Horizon encrypts the existing remote snapshot into a new snippet first, stores the new ID locally, and then rereads the old snippet before deleting it. If another device changed the old content, Chiron Horizon stops automatic cleanup and tells you which old snippet must be deleted manually. If the old snapshot contains encrypted credentials, enter its original credentials password so Chiron Horizon can verify them before cleanup. Rotate any credentials the old snippet may have exposed.
</Callout>
...
2. Enter the personal access token. Leave the snippet ID empty on the first upload; Chiron Horizon creates a private snippet and stores its ID.
3. To restore elsewhere, enter the same snippet ID and select **Download**.

Gitee snippets also use a `chiron-horizon-sync.json` file.
...
Chiron Horizon derives the encryption key with **Argon2id** and uses **AES-256-GCM** encryption. Provider access tokens are encrypted with a device-local key and are never written to `chiron-horizon-sync.json`.
...
Enter the WebDAV endpoint, username, application password, and remote snapshot path. The default path is `Chiron Horizon/sync/snapshot.json`.

In Web/Docker mode, the Chiron Horizon server sends WebDAV requests, so browser CORS restrictions do not apply. WebDAV application passwords are encrypted in the current Chiron Horizon instance and are never included in sync snapshots.

WebDAV supports scheduled uploads while the corresponding Chiron Horizon client remains open. Keep at least one Chiron Horizon page open when using Web/Docker. Downloads always require confirmation to prevent accidental replacement of data in the current instance.
...
Chiron Horizon stores it automatically. You can also find it in the URL of the GitHub Gist or Gitee snippet page.
