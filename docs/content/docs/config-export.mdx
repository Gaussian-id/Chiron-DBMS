...
<Callout type="warn">This exports Chiron Horizon connection configuration, not business database schema or data, and it is not a database backup. Use [Database Export](/en/docs/database-export) or [Database Backup](/en/docs/database-backup) for database contents.</Callout>
...
<Step>
    ...
    Select the exported file. Chiron Horizon automatically detects encrypted and plain configuration files. Encrypted files ask for the passphrase first so Chiron Horizon can decrypt and preview them without writing anything yet.
  </Step>
<Step>
    ### Select Connections
...
- Chiron Horizon stores ordinary connection fields separately from secrets such as database passwords, SSH passwords, SSH key passphrases, and full connection strings. Encrypted export protects the combined data; plaintext export writes those secrets as readable JSON.
