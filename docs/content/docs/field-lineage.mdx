...
Field lineage helps answer where a column comes from, where it is used, and what a change might affect. Chiron Horizon combines database metadata with local query history and labels evidence strength instead of treating every matching name as a confirmed dependency.
...
Chiron Horizon uses executed queries as supporting evidence, including:
...
History only represents SQL executed or saved in Chiron Horizon. It does not cover all application code, ETL, BI, or other clients.
...
- Without declared foreign keys, Chiron Horizon can only provide view, history, and same-name clues
