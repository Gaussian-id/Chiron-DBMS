...
Field lineage helps answer where a column comes from, where it is used, and what a change might affect. Gauss Horizon combines database metadata with local query history and labels evidence strength instead of treating every matching name as a confirmed dependency.
...
Gauss Horizon uses executed queries as supporting evidence, including:
...
History only represents SQL executed or saved in Gauss Horizon. It does not cover all application code, ETL, BI, or other clients.
...
- Without declared foreign keys, Gauss Horizon can only provide view, history, and same-name clues
