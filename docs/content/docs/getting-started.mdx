...
Chiron Horizon 0.1.3 is distributed from [GitHub Releases](https://github.com/Gaussian-id/Chiron-Horizon/releases). Download the asset for your platform, compare its SHA-256 with `SHA256SUMS`, then install it locally. Application auto-update and npm distribution are not enabled in this release.

## Install the desktop application

| Platform | Asset | Install |
| --- | --- | --- |
| macOS Apple Silicon | `Chiron-Horizon_0.1.3_macos-arm64.dmg` | Open the DMG and drag **Chiron Horizon** into Applications. |
| macOS Intel | `Chiron-Horizon_0.1.3_macos-x64.dmg` | Open the DMG and drag **Chiron Horizon** into Applications. |
| Windows 10/11 x64 | `Chiron-Horizon_0.1.3_windows-x64-setup.exe` | Run the NSIS installer. SmartScreen may require **More info → Run anyway**. |
| Ubuntu 22.04+ x64 | `Chiron-Horizon_0.1.3_linux-x64.deb` | Run `sudo apt install ./Chiron-Horizon_0.1.3_linux-x64.deb`. |
| Ubuntu 22.04+ x64 | `Chiron-Horizon_0.1.3_linux-x64.AppImage` | Run `chmod +x Chiron-Horizon_0.1.3_linux-x64.AppImage`, then open it. |

The macOS bundles are ad-hoc signed for bundle integrity rather than notarized. Gatekeeper may require an explicit confirmation. The Windows installer is unsigned in 0.1.3, so SmartScreen confirmation is expected.
...
```powershell
# Windows PowerShell
Get-FileHash .\Chiron-Horizon_0.1.3_windows-x64-setup.exe -Algorithm SHA256
```
