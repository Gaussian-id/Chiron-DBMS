...
description: Install Gauss Horizon Desktop, create your first connection, and build from source.
---

# Get Started

Gauss Horizon 0.1.0 is distributed from [GitHub Releases](https://github.com/Gaussian-id/Gauss-Horizon/releases). Download the asset for your platform, compare its SHA-256 with `SHA256SUMS`, then install it locally. Application auto-update and npm distribution are not enabled in this release.

## Install the desktop application

| Platform | Asset | Install |
| --- | --- | --- |
| macOS Apple Silicon | `Gauss-Horizon_0.1.0_macos-arm64.dmg` | Open the DMG and drag **Gauss Horizon** into Applications. |
| macOS Intel | `Gauss-Horizon_0.1.0_macos-x64.dmg` | Open the DMG and drag **Gauss Horizon** into Applications. |
| Windows 10/11 x64 | `Gauss-Horizon_0.1.0_windows-x64-setup.exe` | Run the NSIS installer. SmartScreen may require **More info → Run anyway**. |
| Ubuntu 22.04+ x64 | `Gauss-Horizon_0.1.0_linux-x64.deb` | Run `sudo apt install ./Gauss-Horizon_0.1.0_linux-x64.deb`. |
| Ubuntu 22.04+ x64 | `Gauss-Horizon_0.1.0_linux-x64.AppImage` | Run `chmod +x Gauss-Horizon_0.1.0_linux-x64.AppImage`, then open it. |

The macOS bundles are ad-hoc signed for bundle integrity rather than notarized. Gatekeeper may require an explicit confirmation. The Windows installer is unsigned in 0.1.0, so SmartScreen confirmation is expected.
...
```powershell
# Windows PowerShell
Get-FileHash .\Gauss-Horizon_0.1.0_windows-x64-setup.exe -Algorithm SHA256
```

The release also includes `agent-registry.json`, managed JRE 21 archives, individual Agent packages, and offline Agent bundles for databases that need an Agent. The application downloads Agent material only from the Gauss Horizon GitHub Release; it has no application updater or mirror fallback.
...
<Steps>
  ...
  <Step>
    ...
    Gauss Horizon can parse common connection URLs for engines such as MySQL, PostgreSQL, Redis, MongoDB, ClickHouse, SQL Server, Oracle, Elasticsearch, Easysearch, Meilisearch, DM, GaussDB, openGauss, TDengine, and Access. Review the parsed fields before saving.
...
<Callout type="info">Connection passwords, SSH passwords, SSH key passphrases, and connection strings are stored separately from the regular connection JSON in Gauss Horizon's local storage. Use [Config Export/Import](/en/docs/config-export) when you need to migrate encrypted connection settings.</Callout>
...
- For query-only access, combine a database read-only account with Gauss Horizon **Read-only connection**.
- Enable **Production protection** for the whole connection or selected databases; Gauss Horizon requires a fresh explicit confirmation for writes.
...
Use source mode when contributing or debugging Gauss Horizon locally.
