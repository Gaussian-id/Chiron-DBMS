---
title: Query Editor
description: Write, understand, parameterize, execute, analyze, and reuse SQL in Chiron Horizon.
---

The query editor is the SQL workspace in Chiron Horizon. It combines dialect-aware highlighting and completion, semantic diagnostics, parameter input, execution progress, multiple results, execution plans, query history, the SQL library, and AI assistance in one tab.
...
When text is selected, Chiron Horizon prioritizes the selection. Otherwise, the current-statement and execution-target settings determine the scope. Shortcuts can be changed under [Keyboard Shortcuts](/en/docs/keyboard-shortcuts).
...
For an `UPDATE`, `INSERT`, or `DELETE` statement, the toolbar shows a **Preview changes** button (also available in the editor context menu). Chiron Horizon rewrites the statement into an equivalent read-only `SELECT` and opens it in a new result tab. The preview re-evaluates the expressions being written, so expressions with side effects (advancing a sequence, volatile functions) still take effect during the preview, and values shown for non-deterministic functions (`uuid()`, `rand()`, `now()`) may differ from the actual execution:
...
Statements Chiron Horizon cannot rewrite safely (for example some dialect-specific forms, or `UPDATE ... FROM`) are reported as **Preview is not supported** and can still be executed normally.
...
<Callout type="info">Refresh connection metadata after creating or changing objects. Chiron Horizon caches and lazy-loads table structure to reduce repeated requests in large schemas.</Callout>
...
Before execution, Chiron Horizon can recognize common placeholders and open a parameter-input dialog:
...
| --- | --- | --- |
| SQL Library | Saving and searching reusable queries in folders inside Chiron Horizon | Full local SQL Library management is available on Desktop; cloud-sync scope depends on settings |
