...
Gauss Horizon prefers the native MongoDB path. When a server or authentication protocol requires the older driver, you can select or automatically fall back to the `mongodb-legacy` Agent. The legacy path may expose fewer administration and mutation capabilities.
