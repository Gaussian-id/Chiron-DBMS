---
title: What is Gauss Horizon?
description: Gauss Horizon is an open-source, lightweight workspace for 90+ databases and data systems, with desktop and self-hosted access, AI, CLI, and MCP.
---

Gauss Horizon is an open-source, lightweight workspace for databases and data infrastructure. It brings connection management, object browsing, SQL editing, data changes, schema tools, specialized consoles, and automation into one interface across 90+ databases and data systems.

Gauss Horizon does not force every system into a SQL-shaped interface. Relational databases use the query editor, data grid, and schema tools, while Redis, MongoDB, vector databases, message queues, etcd, ZooKeeper, Nacos, and similar systems use workspaces designed around their own data models.

<Callout type="info">If you are ready to use Gauss Horizon, start with [Getting Started](/en/docs/getting-started). To check a specific engine or feature, use [Database Support and the feature matrix](/en/docs/databases).</Callout>

![Gauss Horizon dark interface screenshot](/screenshot-dark.png)

## What Gauss Horizon Helps You Do
...
| Category | Representative systems | Main Gauss Horizon experience |
| --- | --- | --- |
...
| MCP Server | Claude Code, Cursor, Windsurf, and other AI clients | Lists connections, reads schema, and runs policy-controlled queries or commands against local storage or Gauss Horizon Web |
...
<Steps>
  ...
  <Step>
    ...
    Run SQL in the query editor or use the data grid, document/key-value browsers, and specialized consoles. Gauss Horizon shows SQL or request content before execution where the workflow supports it.
...
| Task | Product capabilities implemented in Gauss Horizon | Documentation |
| --- | --- | --- |
...
Gauss Horizon uses layered controls rather than relying on a single dangerous-operation confirmation switch:
...
<Callout type="warn">
  Gauss Horizon is a database workbench, not a managed database service, and it does not replace database privileges, auditing, backups, or change approval. Database export is useful for lightweight backup or migration, but large production systems should use native backup tools. AI output, generated SQL, and cross-engine transfers still require human review.
</Callout>
...
- The [Web API](/en/docs/web-api) primarily serves Gauss Horizon Web and internal integrations and is not a stable public contract. Prefer the CLI or MCP for scripts and AI integrations.
