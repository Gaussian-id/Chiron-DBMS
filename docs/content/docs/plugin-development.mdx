---
title: Develop and Submit Chiron Horizon Plugins
description: Build Chiron Horizon plugins with the CLI, Manifest, Workbench and Sidecar APIs, then debug, package, automatically submit, and publish them through the official Marketplace.
---

Chiron Horizon plugins are independently installed `.chiron-horizonp` packages that can contribute connection types, workbenches, filesystems, and native sidecars. Plugin source, the Chiron Horizon plugin platform, and the official Marketplace catalog are owned by different repositories.

<Callout type="info">
  **Recommended path:** create a project with `chiron-horizon-plugin create`, develop and publish unsigned candidates from the plugin's own repository, then let `chiron-horizon-store` create or update the candidate PR. A maintainer reviews and signs the candidates before they enter the official catalog.
</Callout>

<Callout type="warn">
  **Marketplace listing pull requests go to [`Gaussian-id/Gauss-DBM-store`](https://github.com/Gaussian-id/Gauss-DBM-store), not `Gaussian-id/Chiron-Horizon`.** A plugin's feature code normally remains in its author's source repository. Open a PR against [`Gaussian-id/Chiron-Horizon`](https://github.com/Gaussian-id/Chiron-Horizon) only when changing the Chiron Horizon host, SDK, CLI, protocol, schemas, documentation, or official examples.
</Callout>

## Which Repository Receives the Change

| Change | Destination | Official Chiron Horizon PR? |
| --- | --- | --- |
| Your plugin frontend, Rust/Go backend, tests, and release scripts | The plugin's source repository | Do not submit it to `chiron-horizon`; develop and release it in its own repository |
| A new Marketplace listing, version update, icon, or description | [`Gaussian-id/Gauss-DBM-store`](https://github.com/Gaussian-id/Gauss-DBM-store) | Yes, target `main` |
| Chiron Horizon plugin host, Manifest/Marketplace schemas, SDK, CLI, or packager | [`Gaussian-id/Chiron-Horizon`](https://github.com/Gaussian-id/Chiron-Horizon) | Yes, target `main` |
| Official examples or plugin-development documentation | [`Gaussian-id/Chiron-Horizon`](https://github.com/Gaussian-id/Chiron-Horizon) | Yes |
| A bug in a specific third-party plugin | That plugin's source repository | Do not submit it to `chiron-horizon-store` |
...
Chiron Horizon-maintained plugins that need an independent release cadence should normally use separate source repositories as well. Only tightly coupled host examples, SDK verification projects, or components explicitly accepted by maintainers belong in `Gaussian-id/Chiron-Horizon`.
...
- `ui/` runs in a sandboxed workbench and talks to Chiron Horizon through Host APIs.
- `backend/` is an optional native sidecar. Rust and Go SDKs are provided, while other languages may implement the Chiron Horizon JSON-RPC protocol directly.
- `.chiron-horizonp` is an installable artifact, not the source repository.
...
`manifest.json` is the runtime contract and must be at the root of the plugin package. Manifest v1 rejects undeclared fields. Reference the repository's [`plugins/manifest.schema.json`](https://github.com/Gaussian-id/Chiron-Horizon/blob/main/plugins/manifest.schema.json) from your editor. A minimal frontend-only Manifest looks like this:

```json
{
  "$schema": "https://raw.githubusercontent.com/Gaussian-id/Chiron-Horizon/main/plugins/manifest.schema.json",
  "manifest_version": 1,
  ...
  "source": "https://github.com/example/chiron-horizon-plugin-files",
  "homepage": "https://github.com/example/chiron-horizon-plugin-files",
  "engines": { "host_api": "1" },
  "permissions": [],
...
```json
{
  "engines": { "chiron-horizon": ">=0.1.0", "host_api": "1" },
  "permissions": [
    "host.workbench",
    "host.events",
    "host.filesystem",
...
- `engines.chiron-horizon` is an optional Chiron Horizon version range; `engines.host_api` is required.
  ...
  "es-ES": {
    ...
    "contributions": {
      "com.example.files.connection": {
        "label": "Servicio de ejemplo",
        "fields": { "token": { "label": "Token de acceso" } }
      }
    }
  }
}
...
Workbench context is a JSON data snapshot across the Chiron Horizon/plugin boundary. The host recursively removes Vue reactivity and sends an independent copy, so plugins must not expect Vue refs, proxies, DOM nodes, functions, component instances, or credentials.
...
The UI runs in an isolated iframe. Chiron Horizon exposes the bridge as `window.chironHorizonPlugin`; do not import Chiron Horizon Vue/Tauri modules or assume that Node.js, local files, or arbitrary network access are available.
...
```js
await window.chironHorizonPlugin.ready;

const context = window.chironHorizonPlugin.context;
const result = await window.chironHorizonPlugin.invoke("objects/list", { prefix: "docs/" }, { timeoutMs: 30000 });
const off = window.chironHorizonPlugin.onContext((nextContext) => render(nextContext));

await window.chironHorizonPlugin.notify("objects/changed", { count: result.items.length });
const assetUrl = await window.chironHorizonPlugin.readAssetUrl("assets/empty-state.svg");
```

| API | Purpose |
| --- | --- |
| `ready` | Waits for Host initialization; start application logic after `await chironHorizonPlugin.ready`. |
| `context` / `onContext(fn)` | Reads or observes the current workbench context. |
| `locale` | Reads the current Chiron Horizon locale; the plugin translates its own UI. |
| `theme` | Reads `appearance` and Chiron Horizon design tokens for light/dark support. |
...
Failed calls reject their Promise; show a useful error and offer retry. Host methods, parameters, and results are versioned API surface—do not call undocumented Chiron Horizon internals.
...
Chiron Horizon injects theme tokens on the document root and updates `data-chiron-horizon-theme`. Custom libraries, including Svelte and shadcn-svelte, should consume these variables; parent-page CSS does not cross the iframe:
...
Listen with `window.addEventListener("chiron-horizon-plugin-env", handler)` for locale/theme changes. Initialize from `chironHorizonPlugin.locale`/`theme`, then refresh your state on events. Chiron Horizon includes lightweight `chiron-horizon-btn` and `chiron-horizon-input` styles, but the development host does not emulate the complete component kit; custom token-based styles work more consistently in both.
...
The example can replace the Rust template's `backend/src/main.rs`; keep its Cargo package version equal to the Manifest. Go templates use `chiron_horizonpluginsdk.NewServer(metadata, handler).Serve()`. The current Go SDK supports JSONL; the Rust SDK also supports framed transport. Changing a Go Manifest's transport alone does not implement binary framing. See the [Rust SDK](https://github.com/Gaussian-id/Chiron-Horizon/tree/main/plugins/sdk/rust/chiron-horizon-plugin-sdk) and [Go SDK](https://github.com/Gaussian-id/Chiron-Horizon/tree/main/plugins/sdk/go/chiron-horizon-plugin-sdk) for complete interfaces.

## Create a Plugin

Install the precompiled CLI without Rust or a Chiron Horizon source checkout:

```bash
npm install --global @chiron-horizon/plugin-cli
chiron-horizon-plugin --help
```

Or run it directly through `npx`:

```bash
npx @chiron-horizon/plugin-cli create my-ui-plugin --template frontend
```
...
chiron-horizon-plugin create my-plugin \
  ...
  --name "My Plugin" \
  --publisher example \
  --description "A Chiron Horizon plugin." \
  --version 0.1.0 \
  --yes
```

The Svelte template writes Vite output to `ui/` and emits `CHIRON_HORIZON_UI_BUILD_SUCCESS` after a successful build:

```bash
cd my-svelte-plugin
npm install
npm run build
chiron-horizon-plugin dev --port 5190
```

Develop, test, and version the generated project in the plugin's own source repository. Do not copy an ordinary plugin source tree into `Gaussian-id/Chiron-Horizon` merely to publish it.
...
```text
dist/<plugin-id>-<version>-<target>.chiron-horizonp
...
```bash
chiron-horizon-plugin package . --target universal --output-dir dist --artifact-url https://downloads.example.com/my-plugin.chiron-horizonp
```

The CLI packages only directories declared by `[package].include` in `chiron-horizon-plugin.toml` and rejects symlinks, traversal paths, `.chiron-horizon-dev`, oversized files, and unsafe output locations. Do not use `dist/` or development data as a nested package input.

Official plugin authors do not create or possess the Chiron Horizon Store private key. The generated GitHub Release workflow builds candidates for each target and merges their metadata into `release-candidates.json`.
...
Run the standalone browser development host from the plugin root when you need live UI and Sidecar diagnostics; it does not start the Chiron Horizon desktop application:
...
```bash
chiron-horizon-plugin dev --path . --data-dir /tmp/my-plugin-dev
```

`chiron-horizon-plugin dev` does not install project dependencies. Configure frontend builds in `chiron-horizon-plugin.toml`:
...
When `ui_watch` is configured, it must print a standalone `CHIRON_HORIZON_UI_BUILD_SUCCESS` line only after a complete successful build has written every output. Do not print it on failure or from an unconditional exit hook.

The debug page can switch locale, theme, auto-reload, and diagnostic views. It keeps only the latest 500 in-memory entries, redacts passwords, tokens, and Manifest secret fields, and omits or truncates binary and oversized values. `.chiron-horizon-dev/` may contain plaintext credentials: keep it in `.gitignore` and never share it.
...
Pass `nextAfter` and `instanceId` from the response to the next request. The development host implements a supported Host API subset; validate installation, signing, the real Secret Store, desktop lifecycle, and production permissions in Chiron Horizon itself.
...
- Test Chiron Horizon light/dark themes and `zh-CN`/`en`; the host does not translate plugin UI text.
...
- one unsigned `.chiron-horizonp` candidate for each supported target;
- the corresponding `.artifact.json` files;
- merged `release-candidates.json`;
- release notes and the matching source tag.

Candidate packages may live in GitHub Releases, a CDN, or object storage. Do not commit `.chiron-horizonp` binaries to Git history.

### Step 2: Let chiron-horizon-store Create the Candidate PR

The `chiron-horizon-store` synchronizer periodically checks public plugin repositories registered in `automation/plugin-sources.json` with `autoUpdate: true`. It reads the newest Release's `release-candidates.json` and creates or updates exactly one:

```text
candidates/<plugin-id>.json
```

For an auto-updated plugin, authors **do not need to create a manual Issue or PR for every version**, and do not configure Marketplace signing secrets in the plugin repository. The source Release, candidate metadata, and an accessible source tag are the important inputs. For a first listing or an intentional Marketplace metadata change, follow the [`chiron-horizon-store` contribution guide](https://github.com/Gaussian-id/Gauss-DBM-store/blob/main/CONTRIBUTING.md).

`.chiron-horizon-store.json` is optional Marketplace metadata. Use it for first registration or deliberate changes to the store name, tags, license, homepage, or similar listing fields. A version-only release does not need a new `.chiron-horizon-store.json`; it is not the runtime Manifest and must never contain a private signing key.
...
Allowed store fields are `name`, `description`, `icon`, `tags`, `permissions`, `source`, `homepage`, `license`, `releaseNotes`, and `localizations`. A package-relative `icon` is converted to an HTTPS URL under the source tag during synchronization. `autoUpdate` belongs to the repository registration in `chiron-horizon-store/automation/plugin-sources.json`, not this file.

Automation only creates or updates a candidate PR; it **never approves, signs, or merges** it. The PR targets [`Gaussian-id/Gauss-DBM-store`](https://github.com/Gaussian-id/Gauss-DBM-store) on `main`, not `Gaussian-id/Chiron-Horizon`.

If the plugin is not registered for automatic synchronization, fork `chiron-horizon-store` and open **one** PR against `main` containing `publishers/<publisher-id>.json` for a first publisher submission and `candidates/<plugin-id>.json`. There is no separate submission Issue. Before signing, CI intentionally stays red with `open candidate(s) awaiting Chiron Horizon Store signing`; the signing workflow writes the finalized catalog back to the PR and makes it green. Authors can submit a PR to register their public repository for synchronization or ask a maintainer to register it; no App private key belongs in the plugin repository.
...
```json
{
  ...
  "version": "0.1.0",
  "name": "Example Files",
  "source": "https://github.com/example/chiron-horizon-plugin-files/tree/v0.1.0",
  "license": "Apache-2.0",
  "targets": [{
    "target": "universal",
    "url": "https://github.com/example/chiron-horizon-plugin-files/releases/download/v0.1.0/com.example.files-0.1.0-universal.chiron-horizonp",
...
### Step 3: Chiron Horizon Store Reviews and Signs

Maintainers review the source, Manifest, permissions, candidate SHA-256 values, sizes, and native behavior. After approval, the protected Chiron Horizon Store workflow:
...
3. adds an Ed25519 signature using the Chiron Horizon Store repository key;
4. publishes the final `.chiron-horizonp`, final artifact metadata, and signing receipt.

Plugin authors never receive the official repository private key.

### Step 4: Merge the Candidate PR

After signing, the workflow writes finalized `plugins/<plugin-id>.json` and generated `catalog/index.json` back to the same candidate PR. It publishes the signed package, artifact metadata, and signing receipt to a `chiron-horizon-store` Release. Once checks pass, a maintainer reviews and merges the PR.

Do not include the following in a submission or update PR:

- `.chiron-horizonp` binaries;
...
If the repository is not registered for automatic synchronization, submit `candidates/<plugin-id>.json` manually according to the `chiron-horizon-store` contribution guide. For an update, submit only the new version's candidate data; do not hand-edit generated `plugins/` or `catalog/` files.

Fix plugin-code problems in the plugin source repository. Only catalog metadata, review status, final URLs, hashes, sizes, and Marketplace copy belong in `chiron-horizon-store`.

## Signatures and Publisher Identity

Chiron Horizon currently uses one repository signature:
...
- Chiron Horizon Store reviews and signs official plugins centrally;
...
- [Contribute to the Chiron Horizon Core Repository](/en/docs/contributing)
- [`Gaussian-id/Chiron-Horizon` plugin platform source](https://github.com/Gaussian-id/Chiron-Horizon/tree/main/plugins)
- [`Gaussian-id/Gauss-DBM-store` official Marketplace repository](https://github.com/Gaussian-id/Gauss-DBM-store)
