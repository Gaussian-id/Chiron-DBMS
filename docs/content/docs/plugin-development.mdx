...
| Chiron Horizon plugin host, Manifest/Marketplace schemas, SDK, CLI, or packager | [`Gaussian-id/Gauss-Horizon`](https://github.com/Gaussian-id/Gauss-Horizon) | Yes, target `main` |
...
Chiron Horizon-maintained plugins that need an independent release cadence should normally use separate source repositories as well. Only tightly coupled host examples, SDK verification projects, or components explicitly accepted by maintainers belong in `Gaussian-id/Gauss-Horizon`.
