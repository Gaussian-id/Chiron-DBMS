...
description: Organize connections, search database objects, open data and structure, and run database-specific actions from the Gauss Horizon sidebar.
---

The object tree is the main navigation surface in Gauss Horizon. It displays more than databases, schemas, tables, and columns: it also manages connection groups, visible scope, object search, pins, copy/paste, schema actions, specialized workspaces, and administration views.
...
Supported connection imports can retain group information. When sidebar layouts move between devices, Gauss Horizon remaps connection IDs.
...
Gauss Horizon does not force every system into a relational tree. For example:
