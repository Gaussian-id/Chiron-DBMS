...
<Callout type="warn">
  Pull requests described on this page target `Gaussian-id/Gauss-Horizon` and cover the Chiron Horizon application, plugin platform, SDK, CLI, documentation, and Agents. Ordinary plugin source remains in the plugin's own repository, while Marketplace listing PRs target `Gaussian-id/Gauss-DBM-store`. See [Develop and Submit Chiron Horizon Plugins](/en/docs/plugin-development) for the complete flow.
</Callout>

## 1. Choose and Claim an Issue

Open [Chiron Horizon Issues](https://github.com/Gaussian-id/Gauss-Horizon/issues) and choose an issue that has no assignee and no active contributor in its comments. Good first contributions have one of these properties:
