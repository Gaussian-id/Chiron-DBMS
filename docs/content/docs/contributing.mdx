...
description: Set up Chiron Horizon from scratch, run the desktop app, make your first change, test it, and open a pull request.
---

This tutorial is for first-time Chiron Horizon contributors. By the end, you will have:

1. Installed Node.js, pnpm, Rust, and native build dependencies
2. Forked, cloned, and run Chiron Horizon Desktop
...
<Callout type="warn">
  Pull requests described on this page target `Gaussian-id/Chiron-Horizon` and cover the Chiron Horizon application, plugin platform, SDK, CLI, documentation, and Agents. Ordinary plugin source remains in the plugin's own repository, while Marketplace listing PRs target `Gaussian-id/Gauss-DBM-store`. See [Develop and Submit Chiron Horizon Plugins](/en/docs/plugin-development) for the complete flow.
</Callout>

## 1. Choose and Claim an Issue

Open [Chiron Horizon Issues](https://github.com/Gaussian-id/Chiron-Horizon/issues) and choose an issue that has no assignee and no active contributor in its comments. Good first contributions have one of these properties:
...
Chiron Horizon Desktop uses Tauri, Vue, and Rust. The repository currently requires:
...
See the [official Tauri prerequisites](https://v2.tauri.app/start/prerequisites/) for complete platform details. Chiron Horizon also uses ODBC development libraries.
...
Open [Gaussian-id/Chiron-Horizon](https://github.com/Gaussian-id/Chiron-Horizon) on GitHub and click **Fork**.

Replace `<your-github-name>` with your account name:

```bash
git clone https://github.com/<your-github-name>/chiron.horizon.git
cd chiron-horizon
git remote add upstream https://github.com/Gaussian-id/Chiron-Horizon.git
git remote -v
```
...
## 4. Run Chiron Horizon for the First Time
...
Development builds can run alongside an installed Chiron Horizon instance. Both use the same local Chiron Horizon data, so connections and history are available in each window. Avoid changing the same connection, saved SQL, or global setting in both windows at once.

When testing the MCP bridge, the instance started last owns the shared `mcp-bridge-port` discovery file and receives MCP requests. Redis PubSub prefers port `4224` (or `CHIRON_HORIZON_PORT` when set) and automatically falls back to an available local port; the frontend uses the actual bound port.
...
A successful run opens the Chiron Horizon desktop window without a compiler error in the terminal. Complete three quick checks:

1. Open Settings
2. Create or open a local test connection
3. Close and start the app again to confirm the setup is repeatable

If port `1420` is already in use, stop the previous Chiron Horizon/Vite development process and restart.
...
| `crates/chiron-horizon-core/` | Connections, queries, metadata, and shared Rust database logic |
| `crates/chiron-horizon-web/` | Docker/Web backend |
...
Shared Rust logic is mainly in `crates/chiron-horizon-core/`; desktop commands are in `src-tauri/`. When DuckDB is unrelated, start with the fast checks:
...
Agents are separate processes that communicate with Chiron Horizon through stdin/stdout JSON-RPC. Java/JDBC agents normally use JDK 21; native agents use Go or Rust. Read these first:

- [`agents/README.md`](https://github.com/Gaussian-id/Chiron-Horizon/blob/main/agents/README.md)
- [`agents/docs/agent-authoring.md`](https://github.com/Gaussian-id/Chiron-Horizon/blob/main/agents/docs/agent-authoring.md)
...
A successful build does not mean the local Chiron Horizon app is using the new code. Chiron Horizon runs `.chiron-horizon/agents/drivers/<db_type>/agent.jar` under the user home directory, so back up and replace the runtime JAR:

<Tabs groupId="agent-runtime-replace" items={["macOS / Linux", "Windows PowerShell"]}>
  <Tab value="macOS / Linux">
    ```bash
    cp ~/.chiron-horizon/agents/drivers/<db_type>/agent.jar \
      ~/.chiron-horizon/agents/drivers/<db_type>/agent.jar.bak
    cp drivers/<driver-module>/build/libs/*-all.jar \
      ~/.chiron-horizon/agents/drivers/<db_type>/agent.jar
    ```
  </Tab>
  <Tab value="Windows PowerShell">
    ```powershell
    Copy-Item "$HOME\.chiron-horizon\agents\drivers\<db_type>\agent.jar" `
      "$HOME\.chiron-horizon\agents\drivers\<db_type>\agent.jar.bak"
    $jar = Get-ChildItem "drivers\<driver-module>\build\libs\*-all.jar" | Select-Object -First 1
    Copy-Item $jar.FullName "$HOME\.chiron-horizon\agents\drivers\<db_type>\agent.jar" -Force
    ```
  </Tab>
</Tabs>

Restart Chiron Horizon or disconnect and reconnect the database so the old agent process exits and the new JAR is loaded. Repeat the issue's reproduction flow against the real database version.
...
Open your fork on GitHub and click **Compare & pull request**. Set the target repository to `Gaussian-id/Chiron-Horizon` and the target branch to `main`.
...
- Read the repository [CONTRIBUTING.md](https://github.com/Gaussian-id/Chiron-Horizon/blob/main/CONTRIBUTING.md)
