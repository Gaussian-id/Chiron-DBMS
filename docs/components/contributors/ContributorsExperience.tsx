...
const copy = {
  en: {
    title: "Your work is part of Gauss Horizon.",
    intro: "Sign in with GitHub to verify your identity and collect a shareable certificate for pull requests merged into Gauss Horizon.",
    ...
    notEligible: "This GitHub account does not have a merged pull request in Gauss Horizon yet.",
    ...
    certificateBody: "For work accepted and merged into Gauss Horizon, helping make database tools better for everyone.",
  ...
  cn: {
    title: "你的代码，已经成为 Gauss Horizon 的一部分。",
    intro: "使用 GitHub 验证身份，领取一张属于你的贡献者证书，记录被 Gauss Horizon 接受并合并的 Pull Request。",
    ...
    signedInAs: "已验证身份",
    claim: "打开我的证书",
    notEligible: "这个 GitHub 账号暂时还没有被 Gauss Horizon 合并的 Pull Request。",
    contributionCallout: "提交一个专注、可合并的 PR，加入这面贡献者墙。",
    viewGuide: "查看贡献指南",
    signOut: "退出验证",
    certificate: "开源项目贡献者",
    awardedTo: "授予",
    certificateBody: "感谢你的代码被 Gauss Horizon 接受并合并，让每个人都能使用更好的数据库工具。",
    download: "下载贡献者证书",
    close: "关闭",
...
async function drawCertificate(contributor: ContributorActivity, lang: Language) {
  ...
  context.fillText("Gauss Horizon · OPEN SOURCE DATABASE TOOL", 276, 216);
  ...
  link.download = `gauss-horizon-contributor-${contributor.login}.png`;
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroCertificate}>
          <div className={styles.certificatePreview}>
            <div className={styles.previewTop}><span><img src="/logo-64.png" alt="" aria-hidden="true" width={28} height={28} />Gauss Horizon</span><ShieldCheck size={18} /></div>
        ...
        <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setCertificateOpen(false); }}>
          <section className={styles.modal} role="dialog" aria-modal="true" aria-label={text.certificate}>
            <div className={styles.fullCertificate}>
              <div className={styles.previewTop}><span><img src="/logo-64.png" alt="" aria-hidden="true" width={30} height={30} />Gauss Horizon · OPEN SOURCE DATABASE TOOL</span><ShieldCheck size={20} /></div>
