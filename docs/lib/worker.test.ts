...
test("signed OAuth payloads round-trip and reject tampering", async () => {
  const signed = await signPayload({ login: "gauss-horizon-user" }, "test-secret");
  assert.deepEqual(await verifySignedPayload<{ login: string }>(signed, "test-secret"), { login: "gauss-horizon-user" });
  assert.equal(await verifySignedPayload(`${signed}x`, "test-secret"), null);
});

test("OAuth return paths stay on the Gauss Horizon origin", () => {
...
test("static assets receive browser cache headers without caching HTML", () => {
  ...
  assert.equal(staticAssetCacheControl("/screenshots/gauss-horizon-light-1280.webp"), "public, max-age=86400, stale-while-revalidate=604800");
