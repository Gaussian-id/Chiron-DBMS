...
test("signed OAuth payloads round-trip and reject tampering", async () => {
  ...
  assert.deepEqual(await verifySignedPayload<{ login: string }>(signed, "test-secret"), { login: "chiron-horizon-user" });
  assert.equal(await verifySignedPayload(`${signed}x`, "test-secret"), null);
});

test("OAuth return paths stay on the Chiron Horizon origin", () => {
