...
const i18n = {
  en: {
    ...
    jdbcPluginDesc: "Install this optional Gauss Horizon sidecar before using custom JDBC connections. Database vendor JDBC driver JARs still need to be imported separately.",
    jdbcPluginFile: "Plugin package",
    jdbcPluginInstallHint: "Import this ZIP in Gauss Horizon from Settings > Driver Manager > JDBC Drivers > Local Install.",
    ...
    downloadHint: "For air-gapped environments: download the bundle for your platform on an internet-connected machine, then transfer it to the offline machine and import it in Gauss Horizon from Settings > Driver Manager. Use the driver and JRE tabs only when you need individual artifacts.",
  },
  cn: {
    title: "离线驱动下载",
    subtitle: "下载数据库驱动和 JRE 离线包。搜索内网环境需要的资源，在有网机器下载后传输。",
    jdbcPlugin: "JDBC 插件",
    jdbcPluginDesc: "使用自定义 JDBC 连接前先安装这个 Gauss Horizon 可选插件。数据库厂商的 JDBC Driver JAR 仍需单独导入。",
    jdbcPluginFile: "插件包",
    jdbcPluginInstallHint: "在 Gauss Horizon 的“设置 > 驱动管理 > JDBC 驱动 > 本地安装”中导入这个 ZIP。",
    bundles: "整包下载",
    ...
    downloadHint: "内网环境使用说明：在有网的电脑上下载对应平台的整包，然后传输到内网机器，在 Gauss Horizon 的“设置 > 驱动管理”中导入。只有需要单个产物时再使用驱动和 JRE 标签页。",
  },
};
