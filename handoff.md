...
- **root**: D:\Developments\jetbrains\workspace\rust\chiron-horizon
- **package_manager**: pnpm

## Current Task
We are finishing the review-driven stabilization of the Schema Diff and synchronization tool. The main active blocker is in `crates/chiron-horizon-core/src/query.rs`, specifically `execute_schema_diff_deploy`, which currently distinguishes `committed`, `rolled_back`, and `mixed`, but still needs a stronger DDL atomicity model based on both target database behavior and SQL type semantics. This affects the Desktop and Web deploy flow exposed through `src-tauri/src/commands/query.rs`, `crates/chiron-horizon-web/src/routes/query.rs`, and the frontend orchestration in `apps/desktop/src/components/diff/SchemaDiffDialog.vue`. In parallel, we documented the overall problem and optimization plan in a new requirements doc and completed a long-term DDL architecture refactor that introduced `DdlDialectProfile` and `type_rewrite` for cross-database DDL generation.
...
- [x] Introduced `crates/chiron-horizon-core/src/sql_dialect/ddl_profile.rs`
- [x] Introduced `crates/chiron-horizon-core/src/sql_dialect/type_rewrite.rs`
...
- `crates/chiron-horizon-core/src/query.rs` — `SchemaDiffDeployResult` and `execute_schema_diff_deploy`; current blocker lives here
- `src-tauri/src/commands/query.rs` — Tauri deploy endpoint now delegates to `execute_schema_diff_deploy`
- `crates/chiron-horizon-web/src/routes/query.rs` — Web deploy route now delegates to `execute_schema_diff_deploy`; includes tests
- `crates/chiron-horizon-core/src/two_phase_commit.rs` — mixed / rolled_back logic no longer probes by re-calling `commit()`
- `crates/chiron-horizon-core/src/schema_diff.rs` — main schema diff DDL generation path; profile-driven create/alter/index/fk/comment/trigger logic
- `crates/chiron-horizon-core/src/script_generator.rs` — idempotent wrapper and lock-timeout behavior now profile-driven
- `crates/chiron-horizon-core/src/sql_dialect.rs` — exports for new ddl_profile and type_rewrite modules
- `crates/chiron-horizon-core/src/sql_dialect/ddl_profile.rs` — target `DatabaseType` profile registry and DDL behavior knobs
- `crates/chiron-horizon-core/src/sql_dialect/type_rewrite.rs` — type rewrite pipeline and auto-increment helpers
...
- `crates/chiron-horizon-web/src/state.rs` — added `WebState::for_tests` helper to avoid missing new fields in scattered test fixtures
- `crates/chiron-horizon-web/src/routes/connection.rs` — updated tests to use `WebState::for_tests`
- `crates/chiron-horizon-web/src/routes/mongo.rs` — updated tests to use `WebState::for_tests`
- `crates/chiron-horizon-core/tests/api_contract_verification.rs` — full options initializer updated with `detect_table_renames`
- `crates/chiron-horizon-core/tests/bidirectional_diff_e2e.rs` — rename detection test updated to enable table rename detection explicitly
...
- Start from `crates/chiron-horizon-core/src/query.rs`; that is the last review blocker that is still not fully solved.
