package com.gauss.horizon.agent.dameng;
...
import com.gauss.horizon.agent.ObjectSource;
import com.gauss.horizon.agent.TableInfo;
import com.gauss.horizon.agent.test.JdbcMetadataSqlFake;
...
class DamengAgentMetadataTest {
    ...
    void legacyDamengUsesJdbcMetadataWithoutQueryingDm8CatalogViews() {
        ...
        List<TableInfo> tables = agent.listTables("GAUSS_HORIZON_TEST");

        Assertions.assertEquals(List.of("GAUSS_HORIZON_TEST", "SYSDBA"), databases.stream().map(DatabaseInfo::getName).toList());
        Assertions.assertEquals(List.of("GAUSS_HORIZON_TEST", "SYSDBA"), schemas);
    ...
    void readsIdentityColumnsFromDdlText() {
        Assertions.assertEquals(
            ...
            DamengAgent.identityColumnsFromDdlText(
                "CREATE TABLE \"GAUSS_HORIZON_TEST\".\"T\"\n(\n\"ID\" INT IDENTITY(1, 1) NOT NULL,\n"
    ...
    void readsFullTableDdlFromCharacterStreamWhenGetStringIsTruncated() {
        ...
        Assertions.assertTrue(ddl.contains("GAUSS_HORIZON_FULL_DDL_END"), ddl);
    }
        ...
        DatabaseMetaData metadata = proxy(DatabaseMetaData.class, (method, args) -> {
            switch (method.getName()) {
                case "getSchemas":
                    ...
                    return metadataResultSet(List.of(List.of("SYSDBA"), List.of("GAUSS_HORIZON_TEST")));
                case "getTableTypes":
