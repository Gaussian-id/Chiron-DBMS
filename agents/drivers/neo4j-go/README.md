...
- One Neo4j `Driver` connection pool is shared by Gauss Horizon Agent sessions with the same connection identity.
- Each Gauss Horizon Agent session is serialized independently, while different sessions can execute concurrently.
...
- The default Go scheduler parallelism is capped at four OS threads on high-core hosts. Set `GOMAXPROCS` or `GAUSS_HORIZON_AGENT_NEO4J_GOMAXPROCS` to override it.
...
```bash
GAUSS_HORIZON_NEO4J_LIVE=1 \
GAUSS_HORIZON_NEO4J_HOST=127.0.0.1 \
GAUSS_HORIZON_NEO4J_USER=neo4j \
GAUSS_HORIZON_NEO4J_PASSWORD=password \
