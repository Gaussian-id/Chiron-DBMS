...
func TestRocketMQIntegration(t *testing.T) {
	...
	topic := "CHIRON_HORIZON_GO_INTEGRATION_" + suffix
	groupID := "GID_CHIRON_HORIZON_GO_" + suffix
	t.Cleanup(func() {
	...
	integrationCall(t, agent, "mq_send_message", map[string]any{
		"topic": topic, "partition": 0, "key": "chiron-horizon-integration-key", "tag": "chiron-horizon-integration",
	...
	queried := waitForIntegrationMessages(t, agent, "mq_query_message_by_key", map[string]any{
		"topic": topic, "key": "chiron-horizon-integration-key", "maxNum": 10,
