...
func TestConnectionAndChannelMappings(t *testing.T) {
	connection := clientConnectionInfoFromJSON(mustObject(t, `{
      "name":"127.0.0.1:1 -> 127.0.0.1:5672","user":"gauss-horizon","peer_host":"127.0.0.1","peer_port":1234,
...
func TestUserPermissionPolicyMappings(t *testing.T) {
	...
	permission := permissionInfoFromJSON(mustObject(t, `{"user":"gauss-horizon","vhost":"/","configure":".*","write":"^orders","read":".*"}`))
