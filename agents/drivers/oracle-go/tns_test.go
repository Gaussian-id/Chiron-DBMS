...
func TestBuildDSNForConnectResolvesTNSAlias(t *testing.T) {
	...
	writeTNSNames(t, tnsAdmin, "GAUSS_HORIZON_FAILOVER =\n  "+descriptor+"\n")

	dsn, err := buildDSNForConnect(connectParams{
		Username:         "scott",
		Password:         "tiger",
		ConnectionString: oracleTNSJDBCURL("GAUSS_HORIZON_FAILOVER", tnsAdmin),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if parsed.Query().Get("PREFETCH_ROWS") != oracleDefaultPrefetchRows {
		t.Fatalf("TNS Oracle DSN should use the Gauss Horizon prefetch default, got: %s", dsn)
	}
}

func TestBuildDSNForConnectPreservesTNSPrefetchRows(t *testing.T) {
	tnsAdmin := t.TempDir()
	writeTNSNames(t, tnsAdmin, "Gauss Horizon = (DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=db.example.com)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=XE)))")
	dsn, err := buildDSNForConnect(connectParams{
		ConnectionString: oracleTNSJDBCURL("Gauss Horizon", tnsAdmin),
		Username:         "scott",
		Password:         "tiger",
...
func TestBuildDSNForConnectRejectsMissingTNSAdmin(t *testing.T) {
	_, err := buildDSNForConnect(connectParams{ConnectionString: "jdbc:oracle:thin:@GAUSS_HORIZON_FAILOVER"})
...
func TestBuildDSNForConnectRejectsInvalidTNSAdmin(t *testing.T) {
	_, err := buildDSNForConnect(connectParams{ConnectionString: oracleTNSJDBCURL("Gauss Horizon", filepath.Join(t.TempDir(), "missing"))})
...
func TestReadOracleTNSAliasesSupportsIFILEAndMultipleAliases(t *testing.T) {
	...
	if err := os.WriteFile(includePath, []byte("GAUSS_HORIZON_A, GAUSS_HORIZON_B = (DESCRIPTION=(ADDRESS=(HOST=db.example.com)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=ORCL)))\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if aliases["GAUSS_HORIZON_A"] == "" || aliases["GAUSS_HORIZON_B"] == "" {
...
`)
	...
	descriptor := aliases["GAUSS_HORIZON_INDENTED"]
