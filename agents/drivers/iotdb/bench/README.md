...
The default fixture contains 10,000 rows in `root.gauss_horizon_bench.d1`. Workloads are:
...
```bash
docker run --rm --name gauss-horizon-iotdb-bench -p 6667:6667 apache/iotdb:2.0.8-standalone
```
...
python3 agents/drivers/iotdb/bench/run.py \
  > /tmp/gauss-horizon-iotdb-driver-benchmark.json
```
