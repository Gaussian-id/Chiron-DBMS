...
const integrationConnectStringEnv = "GAUSS_HORIZON_ZOOKEEPER_TEST_CONNECT_STRING"
...
func TestZooKeeperIntegration(t *testing.T) {
	...
	if authScheme := os.Getenv("GAUSS_HORIZON_ZOOKEEPER_TEST_AUTH_SCHEME"); authScheme != "" {
		connection["auth_scheme"] = authScheme
		connection["username"] = os.Getenv("GAUSS_HORIZON_ZOOKEEPER_TEST_USERNAME")
		connection["password"] = os.Getenv("GAUSS_HORIZON_ZOOKEEPER_TEST_PASSWORD")
	}
	...
	root := fmt.Sprintf("/gauss-horizon-go-integration-%d", time.Now().UnixNano())
...
func TestZooKeeperLargeChildrenIntegration(t *testing.T) {
	...
	t.Cleanup(service.closeClient)
	root := fmt.Sprintf("/gauss-horizon-go-large-children-%d", time.Now().UnixNano())
...
func BenchmarkZooKeeperOperations(b *testing.B) {
	...
	root := fmt.Sprintf("/gauss-horizon-go-benchmark-%d", time.Now().UnixNano())
...
func TestZooKeeperTLSIntegration(t *testing.T) {
	connectString := os.Getenv("GAUSS_HORIZON_ZOOKEEPER_TLS_TEST_CONNECT_STRING")
	if connectString == "" {
		t.Skip("GAUSS_HORIZON_ZOOKEEPER_TLS_TEST_CONNECT_STRING is not set")
	}
	caCertPath := os.Getenv("GAUSS_HORIZON_ZOOKEEPER_TLS_TEST_CA_CERT_PATH")
	if caCertPath == "" {
		t.Skip("GAUSS_HORIZON_ZOOKEEPER_TLS_TEST_CA_CERT_PATH is not set")
	}
	service := &server{statLookupConcurrency: 4}
	...
	t.Cleanup(service.closeClient)
	root := fmt.Sprintf("/gauss-horizon-go-tls-integration-%d", time.Now().UnixNano())
