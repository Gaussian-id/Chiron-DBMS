...
func TestIntersectReadRangesLimitsBroadPrefixToGrantedScopes(t *testing.T) {
	...
	narrow := intersectReadRanges(granted, "/gauss-horizon/config/", "/gauss-horizon/config0")
	if len(narrow) != 1 || narrow[0].start != "/gauss-horizon/config/" || narrow[0].end != "/gauss-horizon/config0" {
	...
	unbounded := intersectReadRanges([]etcdReadRange{{start: "/gauss-horizon", end: unboundedRangeEnd}}, "/gauss-horizon", unboundedRangeEnd)
