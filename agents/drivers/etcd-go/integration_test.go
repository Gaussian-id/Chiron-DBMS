...
// server. Enable with GAUSS_HORIZON_ETCD_LIVE=1; configure via GAUSS_HORIZON_ETCD_ENDPOINTS,
// GAUSS_HORIZON_ETCD_USER, and GAUSS_HORIZON_ETCD_PASSWORD (defaults match the deploy recipes:
// root/123456 on 127.0.0.1:10700 for 3.7).
func TestLiveEtcdAgent(t *testing.T) {
	if os.Getenv("GAUSS_HORIZON_ETCD_LIVE") != "1" {
		t.Skip("set GAUSS_HORIZON_ETCD_LIVE=1 to run the live etcd agent test")
	}
	endpoints := envOrDefault("GAUSS_HORIZON_ETCD_ENDPOINTS", "http://127.0.0.1:10700")
	user := envOrDefault("GAUSS_HORIZON_ETCD_USER", "root")
	password := envOrDefault("GAUSS_HORIZON_ETCD_PASSWORD", "123456")
	...
	// --- auth ----------------------------------------------------------
	userName := fmt.Sprintf("gauss_horizon_live_%d", time.Now().UnixNano()%100000)
