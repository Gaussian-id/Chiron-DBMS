...
the Chiron Horizon native-agent build and release path. A unit test alone does not count as
production parity.

## Baseline

- Chiron Horizon Java baseline: Apache Hive JDBC standalone 4.0.1.
...
- Chiron Horizon protocol baseline: the Go Agent implements the same stdin/stdout JSON-RPC
...
| Native Chiron Horizon install/launch | yes | yes | local artifact smoke | no | Chiron Horizon tests prove native launch without a JRE and replacement of a stale Hive `agent.jar`; packaged desktop upgrade remains pending |
...
native Chiron Horizon delivery verification, not another Java implementation.
...
The current Linux x86-64 native artifact after the Chiron Horizon visible-schema and
