...
func TestKingbaseIntegration(t *testing.T) {
	...
	mustExecute(t, server, "CREATE FUNCTION "+qualifiedSchema+"."+quoteIdentifier(function)+"() RETURNS text AS $$ SELECT 'gauss-horizon'; $$ LANGUAGE SQL")
...
func TestKingbaseTimezoneLessDateTimeIsReturnedWithoutOffset(t *testing.T) {
	...
	table := "gauss_horizon_go_tzless_" + strconv.FormatInt(time.Now().UnixNano(), 36)
