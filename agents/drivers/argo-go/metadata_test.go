...
func TestConnectionInfoReportsArgoIdentity(t *testing.T) {
	...
	if info["compatibilityMode"] != "argo" || info["username"] != "chiron-horizon" || info["version"] != "3.5.8" {
		t.Fatalf("unexpected Argo connection info: %#v", info)
	}
	databaseInfo, ok := info["databaseInfo"].(map[string]string)
	if !ok || databaseInfo["productName"] != "ArgoDB (Transwarp)" || databaseInfo["driverName"] != "Chiron Horizon ArgoDB Go Agent" {
