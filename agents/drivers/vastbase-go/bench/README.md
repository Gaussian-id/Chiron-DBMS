...
This benchmark compares the same Chiron Horizon JSON-RPC workload through:

- Vastbase JDBC `2.11v` (current Chiron Horizon baseline)
- Vastbase JDBC `2.15v`
...
```bash
mkdir -p /tmp/chiron-horizon-vastbase-bench

go build -o /tmp/chiron-horizon-vastbase-bench/vastbase-go ./drivers/vastbase-go
go build -o /tmp/chiron-horizon-vastbase-bench/agent-compare ./drivers/vastbase-go/bench
```
...
```bash
go build -o /tmp/chiron-horizon-vastbase-bench/vastbase-direct ./drivers/vastbase-go/bench/direct

CHIRON_HORIZON_TEST_PASSWORD='secret' \
...
BENCH_SECONDS=4 \
/tmp/chiron-horizon-vastbase-bench/vastbase-direct
```
...
```bash
JDBC_211_AGENT_JAR=/tmp/chiron-horizon-vastbase-bench/vastbase-jdbc-2.11v.jar \
JDBC_215_AGENT_JAR=/tmp/chiron-horizon-vastbase-bench/vastbase-jdbc-2.15v.jar \
GO_AGENT=/tmp/chiron-horizon-vastbase-bench/vastbase-go \
BENCH_PHASES=startup \
/tmp/chiron-horizon-vastbase-bench/agent-compare > /tmp/chiron-horizon-vastbase-bench/startup.ndjson
```
...
/tmp/chiron-horizon-vastbase-bench/agent-compare > /tmp/chiron-horizon-vastbase-bench/live.ndjson
```
