...
func TestRuntimeReconnectReleasesDetachedControlAndAllowsReplacement(t *testing.T) {
	...
	params := connectParams{
		...
		Port:     5138,
		Database: "SHOP_DEMO",
		Username: "GAUSS_HORIZON_LOCAL_TEST",
		Password: "secret",
	}
...
func TestServerDisconnectClearsDegradedControlState(t *testing.T) {
	s := newServer()
	s.params = connectParams{Database: "SHOP_DEMO", Username: "GAUSS_HORIZON_LOCAL_TEST"}
	s.nodeID = 0
...
func TestXuguSessionAppNameIsStableAndDoesNotExposeSessionID(t *testing.T) {
	...
	if strings.Contains(name, "tab-session-secret") || !strings.HasPrefix(name, "GAUSS_HORIZON_") {
...
func TestBuildDSNParsesGaussHorizonURL(t *testing.T) {
	dsn := buildDSN(connectParams{
...
func TestXuguListSchemasExposesPublicScopeWithoutGUESTCollision(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "public with former reserved schema", realReserved: true, public: true, want: []string{"APP_TEST", "__GAUSS_HORIZON_XUGU_PUBLIC_SYNONYMS__", "SYSDBA", xuguPublicSynonymScope, xuguSchedulerJobScope}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestXuguSchedulerJobQueriesRemainInCurrentDatabase(t *testing.T) {
	...
	metadataQuery := xuguSchedulerJobMetadataQuery("GaussHorizonJob")
	if !strings.Contains(metadataQuery, "DB_ID = CURRENT_DB_ID") || !strings.Contains(metadataQuery, "JOB_NAME = 'GaussHorizonJob'") {
	...
	exact := xuguCatalogSchedulerJobNameQuery("GaussHorizonJob", false)
	folded := xuguCatalogSchedulerJobNameQuery("GaussHorizonJob", true)
...
func TestGetSynonymSourceReconstructsPrivateQuotedDDL(t *testing.T) {
	...
	s := newServer()
	s.db = db
	source, err := s.getObjectSource("SYSDBA", "gaussHorizonSynonymReplayCase", "SYNONYM")
	if err != nil {
		t.Fatalf("get synonym source: %v", err)
	}
	if source["schema"] != "SYSDBA" || source["name"] != "gaussHorizonSynonymReplayCase" {
	...
	ddl, _ := source["source"].(string)
	want := "CREATE SYNONYM \"SYSDBA\".\"gaussHorizonSynonymReplayCase\"\nFOR \"AppSchema\".\"tbUserProfile\";"
	if ddl != want {
...
func TestGetSynonymSourceReconstructsPublicDDLWithoutSyntheticSchema(t *testing.T) {
	...
	s := newServer()
	s.db = db
	source, err := s.getObjectSource(xuguPublicSynonymScope, "GaussHorizonPublicMixed", "SYNONYM")
	if err != nil {
		t.Fatalf("get public synonym source: %v", err)
	}
	if source["schema"] != xuguPublicSynonymScope || source["name"] != "GaussHorizonPublicMixed" {
		t.Fatalf("public synonym source must preserve synthetic scope and catalog spelling: %#v", source)
	}
	ddl, _ := source["source"].(string)
	want := "CREATE PUBLIC SYNONYM \"GaussHorizonPublicMixed\"\nFOR \"SYSDBA\".\"SHOP_USERS\";"
	if ddl != want {
...
func TestXuguCatalogSynonymQueryUsesReservedPublicScope(t *testing.T) {
	exact := strings.ToUpper(xuguCatalogSynonymQuery(xuguPublicSynonymScope, "GaussHorizonPublicMixed", false))
	if !strings.Contains(exact, "Y.IS_PUBLIC = TRUE") || !strings.Contains(exact, "Y.SYNO_NAME = 'GaussHorizonPUBLICMIXED'") {
	...
	folded := strings.ToUpper(xuguCatalogSynonymQuery(xuguPublicSynonymScope, "gauss-horizonpublicmixed", true))
	if !strings.Contains(folded, "Y.IS_PUBLIC = TRUE") || !strings.Contains(folded, "UPPER(Y.SYNO_NAME) = 'GaussHorizonPUBLICMIXED'") {
...
func TestXuguCatalogSynonymQueryTreatsRealGuestAsPrivate(t *testing.T) {
	query := strings.ToUpper(xuguCatalogSynonymQuery("GUEST", "GaussHorizonPrivateMixed", false))
...
func TestSelectXuguCatalogTableNamePrefersExactCaseAndRejectsAmbiguity(t *testing.T) {
	candidates := []xuguCatalogTableName{
		{Schema: "SYSDBA", Table: "GAUSS_HORIZON_CASE_TABLE"},
		{Schema: "SYSDBA", Table: "gauss_horizon_case_table"},
	}

	schema, table, err := selectXuguCatalogTableName("SYSDBA", "gauss_horizon_case_table", candidates)
	if err != nil || schema != "SYSDBA" || table != "gauss_horizon_case_table" {
		t.Fatalf("exact-case selection = (%q, %q, %v), want lower-case catalog table", schema, table, err)
	}

	if _, _, err := selectXuguCatalogTableName("SYSDBA", "GaussHorizon_Case_Table", candidates); err == nil || !strings.Contains(err.Error(), "ambiguous") {
		t.Fatalf("mixed-case ambiguous selection error = %v, want ambiguity error", err)
	}

	schema, table, err = selectXuguCatalogTableName("sysdba", "gauss_horizon_plain_table", []xuguCatalogTableName{{Schema: "SYSDBA", Table: "GAUSS_HORIZON_PLAIN_TABLE"}})
	if err != nil || schema != "SYSDBA" || table != "GAUSS_HORIZON_PLAIN_TABLE" {
...
func TestSelectXuguCatalogSynonymPrefersExactCaseAndRejectsAmbiguity(t *testing.T) {
	candidates := []xuguCatalogSynonym{
		{Schema: "SYSDBA", Name: "gaussHorizonSynonym", TargetName: "TB_A"},
		{Schema: "SYSDBA", Name: "GaussHorizonSYNONYM", TargetName: "TB_B"},
	}

	synonym, err := selectXuguCatalogSynonym("SYSDBA", "gaussHorizonSynonym", candidates)
	if err != nil || synonym.Name != "gaussHorizonSynonym" || synonym.TargetName != "TB_A" {
		t.Fatalf("exact-case selection = (%#v, %v), want quoted catalog synonym", synonym, err)
	}

	if _, err := selectXuguCatalogSynonym("SYSDBA", "GaussHorizonSynonym", candidates); err == nil || !strings.Contains(err.Error(), "ambiguous") {
		t.Fatalf("mixed-case ambiguous selection error = %v, want ambiguity error", err)
	}
}
...
func (c *xuguSchemaListingConn) QueryContext(_ context.Context, query string, _ []driver.NamedValue) (driver.Rows, error) {
	...
	case strings.Contains(upper, "FROM ALL_SCHEMAS") && strings.Contains(upper, "FROM ALL_SYNONYMS"):
		...
		if realReserved {
			values = append(values, []driver.Value{"__GAUSS_HORIZON_XUGU_PUBLIC_SYNONYMS__"})
		}
...
func (c *xuguSynonymSourceConn) QueryContext(_ context.Context, query string, _ []driver.NamedValue) (driver.Rows, error) {
	...
	if strings.Contains(upper, "UPPER(") || !strings.Contains(query, "s.SCHEMA_NAME = 'SYSDBA'") || !strings.Contains(query, "y.SYNO_NAME = 'gaussHorizonSynonymReplayCase'") {
	...
	return &xuguStaticRows{
		...
		values:  [][]driver.Value{{"SYSDBA", "gaussHorizonSynonymReplayCase", "AppSchema", "tbUserProfile", false}},
	}, nil
}
...
func (c *xuguPublicSynonymSourceConn) QueryContext(_ context.Context, query string, _ []driver.NamedValue) (driver.Rows, error) {
	...
	if strings.Contains(upper, "S.SCHEMA_NAME =") || strings.Contains(upper, "UPPER(") || !strings.Contains(query, "y.SYNO_NAME = 'GaussHorizonPublicMixed'") {
		return nil, fmt.Errorf("public synonym resolution must use the global exact lookup: %s", query)
	}
	return &xuguStaticRows{
		columns: []string{"SCHEMA_NAME", "SYNO_NAME", "TARGET_SCHEMA", "TARG_NAME", "IS_PUBLIC"},
		values:  [][]driver.Value{{nil, "GaussHorizonPublicMixed", "SYSDBA", "SHOP_USERS", true}},
	}, nil
}
...
func TestXuguWatchdogClassifiesBlockingExecAsTimeout(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		_, err := s.executeQuery(queryOptions{SQL: "UPDATE GAUSS_HORIZON_TIMEOUT_TEST SET VALUE = 1", TimeoutSecs: 1})
		errCh <- err
	}()
...
func TestXuguExplicitCancelClassifiesBlockingQueryAndExec(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			name: "exec",
			run: func(s *server) error {
				_, err := s.executeQuery(queryOptions{SQL: "UPDATE GAUSS_HORIZON_CANCEL_TEST SET VALUE = 1"})
				return err
			},
		},
	} {
