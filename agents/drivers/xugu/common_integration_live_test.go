...
//
// Coverage is deliberately limited to the common Chiron Horizon contract:
...
func TestLiveXuguCommonIntegration(t *testing.T) {
	...
	t.Run("sql_error_recovery", func(t *testing.T) {
		_, err := s.executeQuery(queryOptions{SQL: "SELECT * FROM " + quoteIdentifier("CHIRON_HORIZON_COMMON_MISSING_") + "", MaxRows: 1})
		if err == nil {
...
func newXuguCommonFixture(schema string) xuguCommonFixture {
	...
	return xuguCommonFixture{
		schema:     schema,
		suffix:     suffix,
		parent:     "CHIRON_HORIZON_IT_PARENT_" + suffix,
		child:      "CHIRON_HORIZON_IT_CHILD_" + suffix,
		view:       "CHIRON_HORIZON_IT_VIEW_" + suffix,
		childIndex: "CHIRON_HORIZON_IT_CHILD_I_" + suffix,
		parentPK:   "CHIRON_HORIZON_IT_PARENT_PK_" + suffix,
	}
}
