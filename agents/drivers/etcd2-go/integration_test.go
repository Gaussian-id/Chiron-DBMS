...
// server. Enable with GAUSS_HORIZON_ETCD2_LIVE=1; configure via GAUSS_HORIZON_ETCD2_ENDPOINTS,
// GAUSS_HORIZON_ETCD2_USER, and GAUSS_HORIZON_ETCD2_PASSWORD (defaults match the server
// deployment: root/123456).
func TestLiveEtcd2Agent(t *testing.T) {
	if os.Getenv("GAUSS_HORIZON_ETCD2_LIVE") != "1" {
		t.Skip("set GAUSS_HORIZON_ETCD2_LIVE=1 to run the live etcd v2 agent test")
	}
	endpoints := envOrDefault("GAUSS_HORIZON_ETCD2_ENDPOINTS", "http://172.26.129.83:20041")
	user := envOrDefault("GAUSS_HORIZON_ETCD2_USER", "root")
	password := envOrDefault("GAUSS_HORIZON_ETCD2_PASSWORD", "123456")
	...
	prefix := fmt.Sprintf("/gauss-horizon/live/%d/", time.Now().UnixNano())
	...
	// --- auth ----------------------------------------------------------
	userName := fmt.Sprintf("gauss-horizonlive%d", time.Now().UnixNano()%100000)
	// Idempotent pre-cleanup: an interrupted earlier run may have left the
	// fixed-named role behind.
	_, _ = state.authRoleDelete(paramsWith(map[string]json.RawMessage{"role": jsonString("gauss_horizon_live_role")}))
	...
	if _, err := state.authRoleAdd(paramsWith(map[string]json.RawMessage{"role": jsonString("gauss_horizon_live_role")})); err != nil {
		t.Fatalf("role add failed: %v", err)
	}
	if _, err := state.authRolePermission(paramsWith(map[string]json.RawMessage{
		"role":     jsonString("gauss_horizon_live_role"),
	...
	roleDetail, err := state.authRoleGet(paramsWith(map[string]json.RawMessage{"role": jsonString("gauss_horizon_live_role")}))
	if err != nil {
	...
	if err != nil || !containsAny(roles.(map[string]any)["roles"].([]string), "gauss_horizon_live_role") {
	...
	afterRevoke, err := state.authRoleGet(paramsWith(map[string]json.RawMessage{"role": jsonString("gauss_horizon_live_role")}))
	if err != nil {
	...
	if _, err := state.authRoleDelete(paramsWith(map[string]json.RawMessage{"role": jsonString("gauss_horizon_live_role")})); err != nil {
...
func TestLiveEtcd2ProtocolFlow(t *testing.T) {
	...
	putLine := fmt.Sprintf(`{"id":3,"method":"kv_put","params":{"agentSessionId":"s1","key":"/gauss-horizon:proto","value":{"encoding":"utf8","data":"flow"}}}`)
	put, _ := server.handleLine(putLine)
	if put.Error != nil {
		t.Fatalf("kv_put failed: %#v", put)
	}
	v3Only, _ := server.handleLine(`{"id":4,"method":"kv_history","params":{"agentSessionId":"s1","key":"/gauss-horizon:proto"}}`)
