...
Native Go agent speaking the etcd **v2 HTTP/JSON API** with the standard Gauss Horizon
...
```bash
go test ./...                                  # unit tests
GAUSS_HORIZON_ETCD2_LIVE=1 go test -run Live ./...       # live flow (default endpoint below)
```

Live test environment variables: `GAUSS_HORIZON_ETCD2_ENDPOINTS`, `GAUSS_HORIZON_ETCD2_USER`,
`GAUSS_HORIZON_ETCD2_PASSWORD`. The default endpoint points at the team test server's
