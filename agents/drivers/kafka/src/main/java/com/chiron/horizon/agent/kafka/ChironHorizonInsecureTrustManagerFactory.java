...
public final class ChironHorizonInsecureTrustManagerFactory {
    static final String ALGORITHM = "CHIRON_HORIZON_TRUST_ALL";
    private static final String PROVIDER_NAME = "CHIRON_HORIZON_KAFKA_INSECURE_TLS";

    private ChironHorizonInsecureTrustManagerFactory() {}

    static void ensureRegistered() {
        if (Security.getProvider(PROVIDER_NAME) != null) return;
        synchronized (ChironHorizonInsecureTrustManagerFactory.class) {
            if (Security.getProvider(PROVIDER_NAME) != null) return;
            Provider provider = new Provider(PROVIDER_NAME, "1.0", "Chiron Horizon Kafka trust-all provider") {};
