...
public final class GaussHorizonInsecureTrustManagerFactory {
    static final String ALGORITHM = "GAUSS_HORIZON_TRUST_ALL";
    private static final String PROVIDER_NAME = "GAUSS_HORIZON_KAFKA_INSECURE_TLS";

    private GaussHorizonInsecureTrustManagerFactory() {}

    static void ensureRegistered() {
        if (Security.getProvider(PROVIDER_NAME) != null) return;
        synchronized (GaussHorizonInsecureTrustManagerFactory.class) {
            if (Security.getProvider(PROVIDER_NAME) != null) return;
            Provider provider = new Provider(PROVIDER_NAME, "1.0", "Gauss Horizon Kafka trust-all provider") {};
