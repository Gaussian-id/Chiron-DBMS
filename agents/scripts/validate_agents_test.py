...
class ValidateAgentsTest(unittest.TestCase):
    ...
    def test_versions_accept_crate_native_sqlite_worker(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            root = repo / "agents"
            crate = repo / "crates" / "gauss-horizon-sqlite-worker"
            root.mkdir()
    ...
    def test_source_scan_rejects_old_execute_query_patterns(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = root / "h2/src/main/java/com/gauss/horizon/agent/h2/H2Agent.java"
            ...
            self.assertEqual(
                [
                    "h2/src/main/java/com/gauss/horizon/agent/h2/H2Agent.java:2: forbidden local SQL prefix classifier",
                    "h2/src/main/java/com/gauss/horizon/agent/h2/H2Agent.java:5: forbidden executeUpdate(trimmedSql) in query execution",
                ],
                problems,
            )
    ...
    def test_jdbc_architecture_requires_shared_base_for_new_agents(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            module = root / "example"
            source = module / "src/main/java/com/gauss/horizon/agent/example/ExampleAgent.java"
            source.parent.mkdir(parents=True)
            ...
            source.write_text(
                textwrap.dedent(
                    """
                    package com.gauss.horizon.agent.example;

                    import com.gauss.horizon.agent.BaseDatabaseAgent;
            ...
            self.assertEqual(
                [
                    "example/src/main/java/com/gauss/horizon/agent/example/ExampleAgent.java: JDBC agents must extend AbstractJdbcAgent, ConfiguredJdbcAgent, or PostgresLikeAgent",
                    "example/src/main/java/com/gauss/horizon/agent/example/ExampleAgent.java:9: copied driver loading; use shared JDBC foundation",
                    "example/src/main/java/com/gauss/horizon/agent/example/ExampleAgent.java:10: copied JDBC connection creation; use shared JDBC foundation",
                ],
                problems,
            )
    ...
    def test_authoring_template_must_use_shared_foundation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [
                    "docs/examples/jdbc-agent-template/src/main/java/com/gauss/horizon/agent/template/TemplateAgent.java: template must use shared JDBC foundation",
                    "docs/examples/jdbc-agent-template/src/main/java/com/gauss/horizon/agent/template/TemplateAgent.java: template contains copied JDBC connection creation",
                ],
            ...
            (root / "build.gradle").write_text(
                textwrap.dedent(
                    ...
                    configure(agentProjects) {
                        tasks.named('shadowJar') {
                            archiveBaseName = "gauss-horizon-agent-${project.name}"
                        }
                    }
                    """
                ),
    ...
    def test_manifest_validation_requires_main_class_source(self):
        with tempfile.TemporaryDirectory() as tmp:
                ...
                textwrap.dedent(
                    ...
                    tasks.named('shadowJar') {
                        manifest {
                            attributes(
                                'Agent-Label': 'H2',
                                'Main-Class': 'com.gauss.horizon.agent.h2.H2Agent'
                            )
                        }
                    }
                    """
            ...
            self.assertEqual(
                ["h2/build.gradle: Main-Class source not found: h2/src/main/java/com/gauss/horizon/agent/h2/H2Agent.java"],
                problems,
            )

            source = module / "src/main/java/com/gauss/horizon/agent/h2/H2Agent.java"
            source.parent.mkdir(parents=True)
            source.write_text("package com.gauss.horizon.agent.h2; public final class H2Agent {}", encoding="utf-8")
    ...
    def test_kotlin_residue_scan_rejects_kt_and_kts_files_outside_build_dirs(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            forbidden_source = root / "h2/src/main/kotlin/com/gauss/horizon/agent/h2/H2Agent.kt"
            ...
            self.assertEqual(
                [
                    "h2/src/main/kotlin/com/gauss/horizon/agent/h2/H2Agent.kt: forbidden Kotlin file",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            workflow.write_text(
                textwrap.dedent(
                    ...
                    EOF
                    legacy_jar_url="https://github.com/${REPO}/releases/download/${TAG}/gauss-horizon-agent-${name}-legacy-placeholder.jar"
                    '''
                ),
                encoding="utf-8",
            )
    ...
    def test_agent_jar_validation_requires_main_class_entry(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            jar = root / "h2/build/libs/gauss-horizon-agent-h2.jar"
            ...
            self.assertEqual(
                ["h2/build/libs/gauss-horizon-agent-h2.jar: Main-Class class not found: com/gauss/horizon/agent/h2/H2Agent.class"],
                validate_agent_jars.validate_agent_jars(root),
            )

            with zipfile.ZipFile(jar, "a") as archive:
                archive.writestr("com/gauss/horizon/agent/h2/H2Agent.class", b"class-bytes")
    ...
    def test_agent_jar_validation_supports_drivers_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            jar = root / "drivers/h2/build/libs/gauss-horizon-agent-h2.jar"
            jar.parent.mkdir(parents=True)
            with zipfile.ZipFile(jar, "w") as archive:
                archive.writestr(
                    "META-INF/MANIFEST.MF",
