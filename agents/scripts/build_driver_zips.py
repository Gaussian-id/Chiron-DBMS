...
def build_driver_zips(release_dir: Path) -> list[Path]:
    ...
    for driver_name, driver in registry.get("drivers", {}).items():
        ...
        if jar_artifact and jar_artifact.get("size", 0) > 0:
            ...
            output = release_dir / f"gauss-horizon-agent-{driver_name}-{version}.tar.zst"
        ...
        for platform, artifact in driver.get("native", {}).items():
            ...
            package_registry = {"jres": {}, "drivers": {driver_name: package_driver}}
            output = release_dir / f"gauss-horizon-agent-{driver_name}-{version}-{platform}.tar.zst"
            if not output.exists():
...
def remove_raw_driver_artifacts(release_dir: Path) -> list[Path]:
    removed: list[Path] = []
    for path in sorted(release_dir.glob("gauss-horizon-agent-*")):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build tar.zst packages for individual Gauss Horizon agents")
