...
# The release-dir must contain agent-registry.json, raw driver artifacts, and gauss-horizon-jre-*.tar.zst.
...
for platform in "${PLATFORMS[@]}"; do
  ...
  JRE_COUNT=0
  for jre_file in "$RELEASE_DIR"/gauss-horizon-jre-*-"$platform".tar.zst; do
    [ -f "$jre_file" ] || continue
  ...
  for jar_file in "$RELEASE_DIR"/gauss-horizon-agent-*.jar; do
    [ -f "$jar_file" ] || continue
    cp "$jar_file" "$WORK/drivers/"
  done

  for native_file in "$RELEASE_DIR"/gauss-horizon-agent-*-"$platform" "$RELEASE_DIR"/gauss-horizon-agent-*-"$platform".exe; do
    [ -f "$native_file" ] || continue
    cp "$native_file" "$WORK/drivers/"
  done

  ZIP_NAME="gauss-horizon-agents-offline-${platform}.zip"
