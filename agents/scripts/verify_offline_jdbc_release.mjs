...
async function main() {
  ...
  for (const platform of PLATFORMS) {
    const zip = join(releaseDir, `gauss-horizon-agents-offline-${platform}.zip`);
    if (!existsSync(zip)) fail(`Offline ZIP not found: ${zip}`);
    const extractionRoot = mkdtempSync(join(tmpdir(), `gauss-horizon-offline-verify-${platform}-`));
    try {
    ...
    console.log(`Verified complete jdbc/ payload in gauss-horizon-agents-offline-${platform}.zip`);
  }
}
