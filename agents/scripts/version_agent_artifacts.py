...
def version_agent_artifacts(release_dir: Path, versions: dict[str, str]) -> list[Path]:
    ...
    for driver, version in sorted(versions.items()):
        jar = rename_artifact(
            release_dir / f"chiron-horizon-agent-{driver}.jar",
            release_dir / f"chiron-horizon-agent-{driver}-{version}.jar",
        )
        if jar:
            renamed.append(jar)
    ...
    for driver in NATIVE_DRIVERS:
        ...
        for platform in PLATFORMS:
            ...
            artifact = rename_artifact(
                release_dir / f"chiron-horizon-agent-{driver}-{platform}{extension}",
                release_dir / f"chiron-horizon-agent-{driver}-{version}-{platform}{extension}",
            )
            if artifact:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Add module versions to Chiron Horizon agent release filenames")
