package com.gauss.horizon.agent;
...
final class JdbcConnectionPoolRegistry implements AutoCloseable {
    ...
    private PoolEntry createPoolEntry(String key, String connectionTestQuery, ConnectionFactory connectionFactory) {
        try {
            ...
            config.setPoolName("gauss-horizon-jdbc-" + key.substring(0, 12));
            ...
            config.setThreadFactory(daemonThreadFactory("gauss-horizon-jdbc-pool-" + key.substring(0, 8)));
    ...
    static final class PoolSettings {
        ...
        static PoolSettings fromEnvironment() {
            int maximumPoolSize = intSetting(
                "gauss.horizon.agent.jdbc.pool.maximumPoolSize",
                "GAUSS_HORIZON_AGENT_JDBC_POOL_MAXIMUM_POOL_SIZE",
                DEFAULT_MAXIMUM_POOL_SIZE,
                1,
                32
            );
            int minimumIdle = intSetting(
                "gauss.horizon.agent.jdbc.pool.minimumIdle",
                "GAUSS_HORIZON_AGENT_JDBC_POOL_MINIMUM_IDLE",
                DEFAULT_MINIMUM_IDLE,
                0,
                maximumPoolSize
            );
            long connectionTimeoutMillis = longSetting(
                "gauss.horizon.agent.jdbc.pool.connectionTimeoutMillis",
                "GAUSS_HORIZON_AGENT_JDBC_POOL_CONNECTION_TIMEOUT_MILLIS",
            ...
            long validationTimeoutMillis = Math.min(
                connectionTimeoutMillis,
                longSetting(
                    "gauss.horizon.agent.jdbc.pool.validationTimeoutMillis",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_VALIDATION_TIMEOUT_MILLIS",
            ...
            return new PoolSettings(
                booleanSetting(
                    "gauss.horizon.agent.jdbc.pool.enabled",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_ENABLED",
                    true
                ),
                maximumPoolSize,
                minimumIdle,
                ...
                validationTimeoutMillis,
                longSetting(
                    "gauss.horizon.agent.jdbc.pool.idleTimeoutMillis",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_IDLE_TIMEOUT_MILLIS",
                    DEFAULT_IDLE_TIMEOUT_MILLIS,
                    10_000L
                ),
                longSetting(
                    "gauss.horizon.agent.jdbc.pool.maxLifetimeMillis",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_MAX_LIFETIME_MILLIS",
                    DEFAULT_MAX_LIFETIME_MILLIS,
                    30_000L
                ),
                longSetting(
                    "gauss.horizon.agent.jdbc.pool.retireMillis",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_RETIRE_MILLIS",
                    DEFAULT_POOL_RETIRE_MILLIS,
                    60_000L
                ),
                intSetting(
                    "gauss.horizon.agent.jdbc.pool.metadataReserve",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_METADATA_RESERVE",
                    DEFAULT_METADATA_RESERVE,
                    0,
                    Math.max(0, maximumPoolSize - 1)
                ),
                intSetting(
                    "gauss.horizon.agent.jdbc.pool.globalMaximumPhysicalConnections",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_GLOBAL_MAXIMUM_PHYSICAL_CONNECTIONS",
                    DEFAULT_GLOBAL_MAXIMUM_PHYSICAL_CONNECTIONS,
                    1,
                    256
                ),
                intSetting(
                    "gauss.horizon.agent.jdbc.pool.maxQuarantinedOperations",
                    "GAUSS_HORIZON_AGENT_JDBC_POOL_MAX_QUARANTINED_OPERATIONS",
                    DEFAULT_MAX_QUARANTINED_OPERATIONS,
                    1,
                    64
                )
    ...
    private static final class ConnectionFactoryDataSource implements DataSource {
        ...
        public Logger getParentLogger() throws SQLFeatureNotSupportedException {
            return Logger.getLogger("com.gauss.horizon.agent.jdbc.pool");
        }
    ...
    private static final class JdbcCheckoutExecutor implements AutoCloseable {
        ...
        private JdbcCheckoutExecutor(int maximumConcurrentCheckouts, ConnectionReleaseExecutor releaseExecutor) {
            this.executor = boundedExecutor(maximumConcurrentCheckouts, "gauss-horizon-jdbc-checkout");
    ...
    private static final class ConnectionReleaseExecutor implements AutoCloseable {
        ...
        private ConnectionReleaseExecutor(int maximumConcurrentReleases) {
            executor = boundedExecutor(maximumConcurrentReleases, "gauss-horizon-jdbc-release");
        }
    ...
    private static final class PoolCloseExecutor implements AutoCloseable {
        ...
        private PoolCloseExecutor(int maximumConcurrentCloses, AtomicReference<SQLException> runtimeFailure) {
            this.executor = boundedExecutor(maximumConcurrentCloses, "gauss-horizon-jdbc-pool-close");
    ...
    private static final class PhysicalConnectionCloser implements AutoCloseable {
        ...
        private PhysicalConnectionCloser(int maximumConcurrentCloses) {
            executor = boundedExecutor(maximumConcurrentCloses, "gauss-horizon-jdbc-physical-close");
        }
    ...
    private static final class PhysicalConnectionOpener implements AutoCloseable {
        ...
        private PhysicalConnectionOpener(int maximumConcurrentOpens) {
            executor = boundedExecutor(maximumConcurrentOpens, "gauss-horizon-jdbc-physical-connect");
        }
