...
test("labels tests-only changes", () => {
  const result = evaluatePullRequestLabels({
    ...
    changedFiles: [
      "crates/gauss-horizon-core/tests/transaction_recovery.rs",
...
test("recognizes conventional titles with a full-width colon", () => {
  const result = evaluatePullRequestLabels({
    ...
    changedFiles: ["crates/gauss-horizon-core/src/db/mod.rs"],
    knownDatabaseTypes,
  });
...
test("collapses broad area and database changes", () => {
  const result = evaluatePullRequestLabels({
    ...
    changedFiles: [
      ...
      "crates/gauss-horizon-core/src/db/postgres.rs",
      "crates/gauss-horizon-mcp/src/main.rs",
      "crates/gauss-horizon-web/src/main.rs",
...
test("detects frontend and backend additions in the same pull request", () => {
  const manifests = {
    base: {
      ...
      "crates/gauss-horizon-core/Cargo.toml": "[dependencies]\ntokio = \"1\"\n",
    },
    head: {
  ...
  const result = evaluatePullRequestLabels({
    ...
    changedFiles: ["package.json", "crates/gauss-horizon-core/Cargo.toml", "pnpm-lock.yaml", "Cargo.lock"],
    knownDatabaseTypes,
