...
test("labels tests-only changes", () => {
  const result = evaluatePullRequestLabels({
    ...
    changedFiles: [
      "crates/chiron-horizon-core/tests/transaction_recovery.rs",
...
test("recognizes conventional titles with a full-width colon", () => {
  const result = evaluatePullRequestLabels({
    ...
    changedFiles: ["crates/chiron-horizon-core/src/db/mod.rs"],
    knownDatabaseTypes,
  });
...
test("detects frontend and backend additions in the same pull request", () => {
  const manifests = {
    base: {
      ...
      "crates/chiron-horizon-core/Cargo.toml": "[dependencies]\ntokio = \"1\"\n",
    },
    head: {
  ...
  const result = evaluatePullRequestLabels({
    ...
    changedFiles: ["package.json", "crates/chiron-horizon-core/Cargo.toml", "pnpm-lock.yaml", "Cargo.lock"],
    knownDatabaseTypes,
