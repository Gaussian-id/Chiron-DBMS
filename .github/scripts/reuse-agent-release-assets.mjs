...
function extractRawDriver(packagePath, asset, outputDir = "") {
  const extractDir = mkdtempSync(join(tmpdir(), "chiron-horizon-agent-package-"));
  try {
...
export function extractReusableDriverPackages({ packagesDir, outputDir, versions, modules }) {
  ...
  for (const moduleName of modules) {
    ...
    const javaName = `chiron-horizon-agent-${moduleName}-${releaseVersion}.tar.zst`;
    ...
    for (const platform of PLATFORMS) {
      const nativeName = `chiron-horizon-agent-${moduleName}-${releaseVersion}-${platform}.tar.zst`;
...
function main() {
  ...
  const workDir = mkdtempSync(join(tmpdir(), "chiron-horizon-agent-reuse-"));
