...
test("collects complete reusable Java, native, and JRE assets", () => {
  const access = artifact("chiron-horizon-agent-access-0.1.34.tar.zst", "a");
  const kingbase = Object.fromEntries(
    platforms.map((platform, index) => [platform, artifact(`chiron-horizon-agent-kingbase-0.1.40-${platform}.tar.zst`, String(index + 1))]),
  );
  const jre = Object.fromEntries(
    platforms.map((platform, index) => [platform, artifact(`chiron-horizon-jre-21-${platform}.tar.zst`, String(index + 7))]),
  );
  const registry = {
    drivers: {
...
test("rejects an incomplete reusable native platform set", () => {
  const native = Object.fromEntries(
    platforms.slice(1).map((platform, index) => [platform, artifact(`chiron-horizon-agent-vastbase-0.1.38-${platform}.tar.zst`, String(index + 1))]),
  );
...
test("requires all TDengine native platforms when reusing a release", () => {
  const native = Object.fromEntries(
    platforms.slice(0, -1).map((platform, index) => [platform, artifact(`chiron-horizon-agent-tdengine-0.1.40-${platform}.tar.zst`, String(index + 1))]),
  );
...
test("requires all Neo4j native platforms when reusing a release", () => {
  const native = Object.fromEntries(
    platforms.slice(0, -1).map((platform, index) => [platform, artifact(`chiron-horizon-agent-neo4j-0.1.40-${platform}.tar.zst`, String(index + 1))]),
  );
...
test("requires all IoTDB native platforms when reusing a release", () => {
  const native = Object.fromEntries(
    platforms.slice(0, -1).map((platform, index) => [platform, artifact(`chiron-horizon-agent-iotdb-0.1.30-${platform}.tar.zst`, String(index + 1))]),
  );
...
test("requires all RocketMQ native platforms when reusing a release", () => {
  const native = Object.fromEntries(
    platforms.slice(0, -1).map((platform, index) => [platform, artifact(`chiron-horizon-agent-rocketmq-0.1.40-${platform}.tar.zst`, String(index + 1))]),
  );
...
test("ignores zero-size legacy JAR placeholders for native-only modules", () => {
  const native = Object.fromEntries(
    platforms.map((platform, index) => [platform, artifact(`chiron-horizon-agent-duckdb-0.1.2-${platform}.tar.zst`, String(index + 1))]),
  );
  const registry = {
    drivers: {
      duckdb: {
        version: "0.1.2",
        jar: {
          url: "https://example.invalid/chiron-horizon-agent-duckdb-legacy-placeholder.jar",
          size: 0,
          sha256: "",
        },
        native,
...
test("rejects a registry version that differs from the effective baseline", () => {
  const access = artifact("chiron-horizon-agent-access-0.1.33.tar.zst", "a");
