...
test("bumps the SQLite SSH worker from its crate path", () => {
  const result = evaluateAgentVersionBump({
    ...
    changedFiles: ["crates/gauss-horizon-sqlite-worker/src/runtime.rs"],
    moduleExists: (path) => path === "crates/gauss-horizon-sqlite-worker",
    readModuleFile: () => "",
  });
...
test("builds only common-dependent Java modules for a shared runtime change", () => {
  ...
  const result = evaluateAgentVersionBump({
    ...
    changedFiles: ["agents/common/src/main/java/com/gauss-horizon/Agent.java"],
...
function createRepository(versions) {
  const repository = mkdtempSync(join(tmpdir(), "gauss-horizon-agent-release-"));
  git(repository, ["init", "--initial-branch=main"]);
  git(repository, ["config", "user.name", "Gauss Horizon Test"]);
  git(repository, ["config", "user.email", "gauss-horizon-test@example.com"]);
