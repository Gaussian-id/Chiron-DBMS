...
$session = $null
$remoteRoot = "C:\Windows\Temp\gauss-horizon-server2012-ci-$([Guid]::NewGuid().ToString('N'))"
try {
  ...
  if ($null -ne $session) {
    Invoke-Command -Session $session -ScriptBlock {
      ...
      if (!$resolvedPath.StartsWith("$windowsTemp\gauss-horizon-server2012-ci-", [System.StringComparison]::OrdinalIgnoreCase)) {
