# Gauss Horizon plugin platform

Gauss Horizon plugins are optional, versioned `.gauss-horizonp` packages. They can add native backend behavior, sandboxed workbench UI, saved connection types, and filesystem providers without increasing the base Gauss Horizon installation size. Declarative extension metadata consumed at build time lives here as well.
...
The platform contract is manifest v1 + Host API 1.x + sidecar protocol v1. The source code for a plugin may live in this repository or in a separate repository; Gauss Horizon installs only the built package.

## Repository layout

- `manifest.schema.json` — editor/CI schema for manifest v1.
- `marketplace.schema.json` — editor/CI schema for marketplace catalog v1.
- `sdk/rust/gauss-horizon-plugin-sdk` — Rust sidecar SDK.
- `sdk/go/gauss-horizon-plugin-sdk` — Go sidecar SDK.
- `sdk/cli` — Rust source for the `gauss-horizon-plugin create/package` CLI; npm publication is deferred for 0.1.0.
- `sdk/packager` — deterministic `.gauss-horizonp` packager plus repository-side Ed25519 signing.
...
- `connection-types/` — build-time registry for every Gauss Horizon connection target, including databases, data services, message queues, and service registries.
...
```bash
gauss-horizon-plugin create my-ui-plugin --template frontend
gauss-horizon-plugin create my-rust-plugin --template rust
gauss-horizon-plugin create my-go-plugin --template go
```

All generated projects include sandbox workbench UI, localized manifest metadata, `.gauss-horizonp` packaging configuration, and a GitHub Release workflow. Rust and Go templates additionally include a native sidecar and multi-platform build matrix; frontend-only projects publish one `universal` package.
...
2. **`.gauss-horizonp` artifacts** — plugin CI publishes unsigned review candidates; the repository operator signs approved candidates and publishes the installable assets.
3. **Installed plugin** — Gauss Horizon verifies and extracts the package under its plugin store.

Optional plugins are not bundled into the Gauss Horizon base package. Installing SSH, SFTP, OpenDAL, or another future plugin increases only the local plugin store size.
...
- **Settings** manages local `.gauss-horizonp` installation, custom repositories, development-only unsigned packages, and advanced repository trust.

Repository configuration is persisted under the plugin store in `.repositories.json`. Gauss Horizon always exposes one managed `gauss-horizon-official` repository backed by the official catalog:

```text
https://distribution-disabled.invalid/main/catalog/index.json
```

The official URL is part of the Gauss Horizon client contract and cannot be edited in Plugin Center settings. Official repository signing public keys are built into Gauss Horizon; release builds may append rotation keys with `GAUSS_HORIZON_PLUGIN_MARKETPLACE_TRUSTED_KEYS_JSON`. Custom repositories use user-managed repository keys. The official repository accepts only Gauss Horizon-managed keys, so adding a custom key cannot impersonate an official package.

Catalog v1 is defined by `marketplace.schema.json` and includes repository metadata, localized plugin metadata, searchable tags, permissions, versions, release notes, and target artifacts. Each artifact declares `signingKeyId`. Gauss Horizon first selects the exact current target and then falls back to `universal`. Artifact URLs may be relative to the catalog URL. Gauss Horizon enforces a 4 MiB catalog limit and a 512 MiB package limit, resolves only HTTP(S) URLs, limits redirects, isolates repository failures, verifies the catalog-declared size and SHA-256, then requires the package Manifest ID, version, publisher, permissions, and Ed25519 key ID to match the reviewed catalog before activation.

Run the live official-store smoke test with:

```bash
cargo run -p gauss-horizon-core --no-default-features --example plugin_marketplace_smoke
cargo run -p gauss-horizon-core --no-default-features --example plugin_marketplace_smoke -- <plugin-id> [version]
```
...
```text
example-1.0.0-darwin-arm64.gauss-horizonp
├── manifest.json
├── checksums.json
...
`.gauss-horizonp` is a ZIP container with stricter rules:
...
```json
{
  ...
  "version": "1.0.0",
  "source": "https://github.com/example/gauss-horizon-plugin",
  "homepage": "https://example.com/gauss-horizon-plugin",
  "engines": {
    "gauss-horizon": ">=0.5.68",
    "host_api": "^1.0"
  }
}
```
...
A provider owns validation and connect/disconnect lifecycle for a saved non-SQL connection. Gauss Horizon stores these as:
...
The provider-defined type, such as `ssh`, stays in `plugin_connection_type`; it does not extend Gauss Horizon's exhaustive database enum.
...
Gauss Horizon resolves connection display metadata in this order: provider `label` / `icon`, plugin `name` / `icon`, then provider ID and the built-in generic plugin icon. Declared icon files must remain inside the plugin package and use SVG, PNG, JPEG, GIF, WebP, or ICO. SVG is rendered through an image URL rather than injected into the Gauss Horizon document.
...
Password fields default to `secret` when `binding` is omitted. Gauss Horizon validates required values and value types before calling the plugin. The plugin receives the hydrated connection only in its backend lifecycle request; the workbench UI receives a connection ID and non-secret navigation context.
...
`runtime.host` and `runtime.port` are the final endpoint after Gauss Horizon transport layers. A protocol plugin must connect to this endpoint instead of rebuilding Gauss Horizon tunnels itself.

#### Connection dialog actions

Connection providers may add ordered custom actions before Gauss Horizon-owned lifecycle buttons:
...
- `actions` declares only custom button metadata. Gauss Horizon invokes `connection/action` with the action ID; plugins cannot choose arbitrary RPC method names.
- `test`, `save`, and `save-and-connect` are host-owned actions. Gauss Horizon adds them from provider capabilities and dialog mode, validates the form, persists secrets through its secret store, and invokes the fixed lifecycle methods.
- A custom action may run with an incomplete form only when `requires_valid_form` is `false`; Gauss Horizon still validates declared field types, secret keys, and transport configuration.
...
`fieldValues` may contain only fields declared by that provider and must match their declared types. `null` clears a field. The plugin cannot write arbitrary `ConnectionConfig` keys or bypass Gauss Horizon-owned save, secret persistence, transport, and connection lifecycle logic.

### `workbench`

A workbench opens in a normal persistent Gauss Horizon tab. The iframe is loaded with `sandbox="allow-scripts"`, a restrictive CSP, no Tauri object, no parent DOM access, and no direct network access. The host injects `window.gaussHorizonPlugin`:

- `ready` / `context` / `locale` — `locale` is the current Gauss Horizon locale such as `en` or `zh-CN`
- `theme` — `{ appearance: "light" | "dark", tokens }` with the resolved Gauss Horizon design tokens; theme changes are pushed live through env updates, and the SDK applies them to the plugin document root
...
Every sandbox document ships with a small official component kit built on the Gauss Horizon design tokens, so plugin UI follows light/dark mode and custom palettes automatically:

```html
<button class="gauss-horizon-btn gauss-horizon-btn--primary">Connect</button>
<input class="gauss-horizon-input" placeholder="Endpoint" />
<span class="gauss-horizon-badge">Ready</span>
```

Available classes: `gauss-horizon-card`, `gauss-horizon-section-title`, `gauss-horizon-btn` (`--primary` / `--danger` / `--ghost`), `gauss-horizon-label`, `gauss-horizon-input`, `gauss-horizon-select`, `gauss-horizon-textarea`, `gauss-horizon-hint`, `gauss-horizon-row` (label + field grid), `gauss-horizon-table`, `gauss-horizon-badge`, `gauss-horizon-link`. Custom plugin CSS can use the same `var(--color-*)` tokens; `document.documentElement.dataset.gaussHorizonTheme` reflects the current appearance.

Plugin-authored names, descriptions, contribution labels, form-field text, and select-option labels can be localized through `manifest.json > localizations`. Gauss Horizon selects the exact current locale first, then its base language, and finally falls back to the manifest's default text:
...
A result view contributes a plugin-rendered visualization for query results. Gauss Horizon shows one toolbar button per installed view next to the result grid; clicking it opens the plugin workbench with the current result as context:
...
A context-menu entry is rendered **natively** by Gauss Horizon (no sandbox iframe, native theme and keyboard behavior) in the declared menu surface. v1 supports the saved-connection sidebar menu:
...
A filesystem provider declares URI schemes, an optional icon, `root_uri`, and capabilities (`read`, `write`, `delete`, `rename`, `mkdir`). It is the reusable boundary for OpenDAL-like storage integrations: Gauss Horizon owns the generic file-browser tab, while the plugin owns authentication, remote API calls, and provider-specific state. The provider icon is used for the saved connection in the sidebar and for its Gauss Horizon tab; it falls back to the plugin-level icon when omitted.

A connection provider can set `filesystem_provider` instead of `workbench`. Opening that saved connection connects the plugin lifecycle and opens the Gauss Horizon host file manager. A provider may declare both: Gauss Horizon opens the custom workbench by default, and the sandboxed UI can call `openFilesystem(providerId, context)` with `host.filesystem` permission.
...
Every entry has `name`, canonical `uri`, `kind` (`file`, `directory`, `symlink`, or `other`), and optional `size`, `modifiedAt`, and `contentType`. Gauss Horizon validates schemes, response sizes, base64, cursors, and entry metadata before the frontend sees a result.
...
The backend is a persistent child process with stdin/stdout reserved for the Gauss Horizon protocol. Diagnostics must go to stderr.

### Initialization

Gauss Horizon starts every sidecar with `plugin/initialize`:

```json
{
  "host": {
    "gaussHorizonVersion": "0.5.68",
...
```mermaid
sequenceDiagram
  participant UI as Gauss Horizon UI
...
Sidecars are shared per plugin process, not spawned per tab. Plugins own their internal session registries. Gauss Horizon tears down plugin-owned connection pools before replace, rollback, or uninstall. Uninstall is blocked while saved connections still reference the plugin.
...
- **Secret persistence:** plugin secrets are removed from connection JSON and stored through Gauss Horizon's secret-store path. Ordinary cloud-sync snapshots always contain redacted placeholders. Secrets enter sync data only inside the encrypted payload when the user has configured a sync passphrase; without one, plugin secrets remain local and are not synchronized.
...
- **Permission declarations:** privileged host bridge operations require declared permissions. Plugin UI network egress is fully blocked except for explicitly declared `host.network:` origins. Native process filesystem/network access cannot currently be completely mediated by Gauss Horizon.
...
The complete author and official-store flow is documented in [`RELEASING.md`](RELEASING.md). Plugin authors keep source code in their own repository and publish unsigned candidates. Gauss Horizon Store reviews and signs approved candidates, publishes installable artifacts, and records only catalog metadata in Git.
...
For native plugins, copy `sdk/templates/github/plugin-release.yml` into the plugin repository and pin the reusable workflow to a released Gauss Horizon plugin SDK tag or commit. Frontend-only plugins may replace the build matrix with one `universal` build.

## Compatibility policy

- Increment `manifest_version` only for manifest shape changes that an older Gauss Horizon cannot interpret.
- Increment the sidecar protocol version only for wire-level incompatibilities.
- Use `engines.host_api` for host API compatibility and `engines.gauss-horizon` for product-version constraints.
...
```bash
cargo test -p gauss-horizon-core --no-default-features plugins::
cargo check -p gauss-horizon-web --no-default-features
```

SDK and example checks:

```bash
cargo test --manifest-path plugins/sdk/rust/gauss-horizon-plugin-sdk/Cargo.toml
