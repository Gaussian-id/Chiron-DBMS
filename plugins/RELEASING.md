# Publishing Chiron Horizon plugins

Chiron Horizon separates plugin source, unsigned review candidates, repository-signed installable artifacts, and catalog metadata. These artifacts have different owners and should not be copied into one Git repository.
...
```text
vendor.example-1.2.0-darwin-arm64.chiron-horizonp
vendor.example-1.2.0-darwin-x64.chiron-horizonp
vendor.example-1.2.0-windows-x64.chiron-horizonp
vendor.example-1.2.0-linux-x64.chiron-horizonp
vendor.example-1.2.0-linux-arm64.chiron-horizonp
release-candidates.json
```

Frontend-only or otherwise platform-independent plugins may publish one `vendor.example-1.2.0-universal.chiron-horizonp` candidate. Use `universal` only when the exact same package is valid on every supported Chiron Horizon target.
...
After review, the repository operator signs the accepted candidate bytes and publishes final installable `.chiron-horizonp` assets to repository-controlled release storage. Final artifact metadata contains:

- target and final download URL;
- SHA-256 and byte size of the signed package;
- repository `signingKeyId`.

The official implementation publishes these assets from a protected `chiron-horizon-store` workflow. A custom or private repository operates the same trust boundary with its own key and storage.

### Official store Git repository

`Gaussian-id/Gauss-DBM-store` contains reviewed catalog inputs, publisher identities, repository public keys, revocations, and generated catalog JSON. It does not contain plugin source trees, private keys, or `.chiron-horizonp` binaries.
...
Human review decides whether a plugin is allowed in the official catalog. SHA-256 and Ed25519 repository signing ensure the installed bytes are the reviewed bytes. Chiron Horizon also compares the package Manifest ID, version, publisher, and permissions with the selected catalog entry before activation.

## Author release workflow

Install the precompiled CLI and run `chiron-horizon-plugin create <directory> --template frontend|rust|go` for a complete project, or copy `sdk/templates/github/plugin-release.yml`:

```bash
npm install --global @chiron-horizon/plugin-cli@0.1.2
```

Pin both the reusable workflow reference and `plugin-cli-version`. The npm package contains the matching CLI binary, packager, and Rust/Go SDK sources. Building the CLI from a Chiron Horizon checkout is reserved for SDK development and can be enabled with `install-plugin-cli-from-source: true` plus a reviewed `sdk-ref`.

Before publishing a new CLI version, update the CLI and npm manifests together, merge them to `main`, then run the **Plugin CLI Release** workflow. The workflow creates `plugin-cli-v<version>`, builds six platform packages, publishes `@chiron-horizon/plugin-cli`, and verifies clean npm installation with frontend, Rust, and Go templates.
...
1. installs the pinned precompiled `chiron-horizon-plugin` npm package and builds on the configured target runners;
2. produces one unsigned `.chiron-horizonp` and `.artifact.json` per target;
...
4. Run the protected `chiron-horizon-store` signing workflow with the expected plugin ID, version, target, output filename, and reviewed Chiron Horizon SDK ref.
5. The workflow downloads the candidate, rejects pre-existing signatures, verifies Manifest identity, appends the Chiron Horizon Store signature, and publishes the signed asset plus final metadata.
...
The protected GitHub Environment must require reviewer approval and provide matching `CHIRON_HORIZON_STORE_SIGNING_KEY` secret and `CHIRON_HORIZON_STORE_SIGNING_KEY_ID` variable values. The public key must already exist in `signing-keys.json` and must not be revoked.
...
cargo run --release \
  ...
  -- sign candidate.chiron-horizonp signed.chiron-horizonp \
  --key-id "$CHIRON_HORIZON_PLUGIN_SIGNING_KEY_ID" \
  --artifact-metadata signed.artifact.json \
  --target darwin-arm64 \
  --artifact-url https://plugins.example.com/releases/signed.chiron-horizonp
```
