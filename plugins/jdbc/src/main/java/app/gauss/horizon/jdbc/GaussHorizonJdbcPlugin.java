...
public final class GaussHorizonJdbcPlugin {
    ...
    private static final String JDBCX_HIGH_PRIVILEGE_EXTENSIONS_OPT_IN = "-Dgauss.horizon.jdbcx.allowHighPrivilegeExtensions=";
    ...
    private GaussHorizonJdbcPlugin() {
    }
    ...
    public static void main(String[] args) throws Exception {
        try (
            ...
            while ((line = reader.readLine()) != null) {
                ...
                writer.newLine();
                writer.flush();
                if (response.path("_gauss_horizon_close").asBoolean(false)) {
                    break;
                }
            }
        } finally {
    ...
    private static ObjectNode handleLine(String line) throws Exception {
        ...
        try {
            ...
            if ("close".equals(method)) {
                ...
                response.put("_gauss_horizon_close", true);
                return response;
            }
    ...
    static String enrichDriverHint(JsonNode connection, String message) {
        ...
        String hint = message.contains("不支持的字符集")
            ? "。请在 设置 → JDBC 驱动 中为该 Oracle 驱动一并导入同版本的 orai18n.jar，或改用 Gauss Horizon 内置 Oracle 连接（默认驱动已支持中文多字节字符集）"
            : ". Import orai18n.jar (same version as the ojdbc driver) next to the Oracle driver under Settings -> JDBC Drivers, or use Gauss Horizon's built-in Oracle connection instead, whose default driver supports multibyte Chinese charsets";
    ...
    private static void registerDrivers(JsonNode connection) throws Exception {
        ...
        ClassLoader loader = urls.isEmpty()
            ...
            : new URLClassLoader(urls.toArray(URL[]::new), GaussHorizonJdbcPlugin.class.getClassLoader());
    ...
    private static String getOracleExplainInfo(Connection connection, String sql, int timeoutSecs) throws SQLException {
        String statementId = "GAUSS_HORIZON_" + UUID.randomUUID().toString().replace("-", "").substring(0, 26);
