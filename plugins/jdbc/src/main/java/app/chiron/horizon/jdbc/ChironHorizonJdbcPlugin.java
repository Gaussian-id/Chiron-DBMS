...
public final class ChironHorizonJdbcPlugin {
    ...
    private static final String JDBCX_HIGH_PRIVILEGE_EXTENSIONS_OPT_IN = "-Dchiron.horizon.jdbcx.allowHighPrivilegeExtensions=";
    ...
    private ChironHorizonJdbcPlugin() {
    }
    ...
    private static ObjectNode handleLine(String line) throws Exception {
        ...
        try {
            ...
            if ("close".equals(method)) {
                ...
                response.put("_chiron_horizon_close", true);
                return response;
            }
    ...
    static String enrichDriverHint(JsonNode connection, String message) {
        ...
        String hint = message.contains("不支持的字符集")
            ? "。请在 设置 → JDBC 驱动 中为该 Oracle 驱动一并导入同版本的 orai18n.jar，或改用 Chiron Horizon 内置 Oracle 连接（默认驱动已支持中文多字节字符集）"
            : ". Import orai18n.jar (same version as the ojdbc driver) next to the Oracle driver under Settings -> JDBC Drivers, or use Chiron Horizon's built-in Oracle connection instead, whose default driver supports multibyte Chinese charsets";
    ...
    private static String getOracleExplainInfo(Connection connection, String sql, int timeoutSecs) throws SQLException {
        String statementId = "CHIRON_HORIZON_" + UUID.randomUUID().toString().replace("-", "").substring(0, 26);
