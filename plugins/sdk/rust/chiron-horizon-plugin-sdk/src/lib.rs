...
/// Host API method that relays a question to the Chiron Horizon user interface.
...
/// Client for Host API methods the plugin calls back into Chiron Horizon. Create it by
...
impl HostClient {
    ...
    /// Host API version Chiron Horizon reported at `plugin/initialize`.
    pub fn host_api_version(&self) -> Option<String> {
        self.host.lock().ok().and_then(|host| host.api_version.clone())
    }

    /// Whether Chiron Horizon advertised `method` for this session. Gate optional calls on
    ...
    /// Asks the user a question through the Chiron Horizon UI. Fails when the host has no
    ...
    fn serve_json_lines<R: BufRead>(
        ...
        loop {
            ...
            if let Err(error) = self.dispatch_json(&line, emitter.clone(), workers) {
                eprintln!("[chiron-horizon-plugin-sdk] {error}");
            }
        }
    }
    ...
    fn dispatch_json(&self, payload: &[u8], emitter: PluginEmitter, workers: &WorkerPool) -> Result<(), String> {
        ...
        if request.method == "plugin/initialize" {
            ...
            let result = if supported {
                ...
                Err(PluginError::new(-32001, "Chiron Horizon and plugin do not share a protocol version"))
            };
        ...
        workers.submit(move || {
            ...
            if let Some(id) = request.id {
                if let Err(error) = emitter.respond(id, result) {
                    eprintln!("[chiron-horizon-plugin-sdk] failed to write response: {}", error.message);
                }
            }
        })
    }
    ...
    fn dispatch_binary(&self, payload: Vec<u8>, emitter: PluginEmitter, workers: &WorkerPool) -> Result<(), String> {
        ...
        workers.submit(move || {
            if let Err(error) = handler.handle_binary(&channel, data, &emitter) {
                eprintln!("[chiron-horizon-plugin-sdk] binary handler failed: {}", error.message);
            }
        })
    }
}
...
impl WorkerPool {
    fn new(worker_threads: usize, queue_capacity: usize) -> io::Result<Self> {
        ...
        for index in 0..worker_threads {
            ...
            thread::Builder::new().name(format!("chiron-horizon-plugin-worker-{index}")).spawn(move || loop {
...
mod tests {
    ...
    fn host_client_surfaces_host_errors_cancels_and_timeouts() {
        ...
        assert!(client.deliver_response(&serde_json::json!({
            "jsonrpc": "2.0",
            "id": request["id"],
            "error": { "code": -32001, "message": "The Chiron Horizon host cannot ask the user for input right now" }
        })));
