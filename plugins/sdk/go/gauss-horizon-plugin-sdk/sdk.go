package gauss-horizonpluginsdk
...
func (server *Server) Serve() error {
	...
	for scanner.Scan() {
		...
		if err != nil {
			fmt.Fprintf(server.errors, "[gauss-horizon-plugin-sdk-go] %v\n", err)
			continue
		}
		if request.Method == "plugin/initialize" {
			if len(request.ID) == 0 {
				fmt.Fprintln(server.errors, "[gauss-horizon-plugin-sdk-go] plugin/initialize must be a request")
				continue
			}
		...
		go func(request protocolRequest) {
			...
			if len(request.ID) == 0 {
				if pluginError != nil {
					fmt.Fprintf(server.errors, "[gauss-horizon-plugin-sdk-go] %s\n", pluginError.Message)
				}
				return
			}
			if writeError := emitter.respond(request.ID, result, pluginError); writeError != nil {
				fmt.Fprintf(server.errors, "[gauss-horizon-plugin-sdk-go] failed to write response: %s\n", writeError.Message)
			}
		}(request)
	}
	workers.Wait()
...
func (server *Server) initialize(params json.RawMessage) (any, *PluginError) {
	...
	return nil, NewError(-32001, "Gauss Horizon and plugin do not share a protocol version")
}
