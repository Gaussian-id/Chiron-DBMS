...
test("page reconnects preserve frames, abandoned pages release their last connection", async (t) => {
  ...
  const page = { "X-Chiron Horizon-Page": "browser-page" };
  const events = async () => {
...
test("pages sharing a cookie have independent workbench generations", async (t) => {
  ...
  const a = { "X-Chiron Horizon-Page": "page-a" },
    b = { "X-Chiron Horizon-Page": "page-b" };
...
test("refresh cleanup does not disconnect a connection still used by another page", async (t) => {
  ...
  await reader.read();
  const saved = (await request("connections/save", { providerId: "example.connection", values })).value;
  const old = (await request("connections/connect", { id: saved.id }, { "X-Chiron Horizon-Page": "old" })).value.frame;
  const fresh = (await request("connections/connect", { id: saved.id }, { "X-Chiron Horizon-Page": "fresh" })).value.frame;
  await reader.cancel();
  let status = 200;
  for (let i = 0; i < 100 && status === 200; i++) {
    await new Promise((resolve) => setTimeout(resolve, 20));
    status = (await request("frame-document", { frameId: old.id }, { "X-Chiron Horizon-Page": "old" })).status;
  }
  ...
  const closed = await request("frames/close", { id: fresh.id }, { "X-Chiron Horizon-Page": "fresh" });
...
async function fixture(t, frontend = false, overrides = {}) {
  ...
  const root = await mkdtemp(join(tmpdir(), "chiron-horizon-mock-test-"));
...
test("save, reload, iframe isolation, generic RPC and close lifecycle", async (t) => {
  ...
  assert.match(document.html, /window.chironHorizonPlugin/);
...
test("icon reads reject unsafe paths, missing files, non-images, symlink escapes and oversized files", async (t) => {
  ...
  const outside = await mkdtemp(join(tmpdir(), "chiron-horizon-icon-outside-"));
