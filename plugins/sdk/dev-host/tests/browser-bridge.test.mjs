...
test("bridge delivers host-compatible environment, lifecycle and binary events", async (t) => {
  ...
  const events = [];
  for (const type of ["init", "context", "env", "event", "binary"]) w.addEventListener(`chiron-horizon-plugin-${type}`, (event) => events.push(event));
  let initialized = 0,
    notifications = 0,
    binary;
  w.chironHorizonPlugin.onInit(() => initialized++);
  w.chironHorizonPlugin.onEvent(() => notifications++);
  w.chironHorizonPlugin.onBinary((value) => {
    binary = value;
  });
  send({ type: "init", context: { value: 1 } });
  await w.chironHorizonPlugin.ready;
...
test("sandbox bootstrap handles initialization, generation filtering and context listeners", async () => {
  ...
  await w.chironHorizonPlugin.ready;
  assert.equal(w.chironHorizonPlugin.context.connectionId, "one");
  assert.equal(w.chironHorizonPlugin.locale, "zh-CN");
  send(hostMessage("old", { type: "env", locale: "ja" }));
  assert.equal(w.chironHorizonPlugin.locale, "zh-CN");
  send(hostMessage("generation", { type: "env", locale: "en" }));
  assert.equal(w.chironHorizonPlugin.locale, "en");
  send(hostMessage("generation", { type: "env", theme: { appearance: "dark" } }));
  assert.equal(w.chironHorizonPlugin.locale, "en");
  assert.equal(w.chironHorizonPlugin.context.connectionId, "one");
  let calls = 0;
  const off = w.chironHorizonPlugin.onContext(() => calls++);
