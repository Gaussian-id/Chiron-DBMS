...
test("bridge delivers host-compatible environment, lifecycle and binary events", async (t) => {
  ...
  const events = [];
  for (const type of ["init", "context", "env", "event", "binary"]) w.addEventListener(`gauss-horizon-plugin-${type}`, (event) => events.push(event));
  let initialized = 0,
    notifications = 0,
    binary;
  w.gaussHorizonPlugin.onInit(() => initialized++);
  w.gaussHorizonPlugin.onEvent(() => notifications++);
  w.gaussHorizonPlugin.onBinary((value) => {
    binary = value;
  });
  send({ type: "init", context: { value: 1 } });
  await w.gaussHorizonPlugin.ready;
...
test("sandbox bootstrap handles initialization, generation filtering and context listeners", async () => {
  ...
  await w.gaussHorizonPlugin.ready;
  assert.equal(w.gaussHorizonPlugin.context.connectionId, "one");
  assert.equal(w.gaussHorizonPlugin.locale, "zh-CN");
  send(hostMessage("old", { type: "env", locale: "ja" }));
  assert.equal(w.gaussHorizonPlugin.locale, "zh-CN");
  send(hostMessage("generation", { type: "env", locale: "en" }));
  assert.equal(w.gaussHorizonPlugin.locale, "en");
  send(hostMessage("generation", { type: "env", theme: { appearance: "dark" } }));
  assert.equal(w.gaussHorizonPlugin.locale, "en");
  assert.equal(w.gaussHorizonPlugin.context.connectionId, "one");
  let calls = 0;
  const off = w.gaussHorizonPlugin.onContext(() => calls++);
