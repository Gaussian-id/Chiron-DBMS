...
async function project(t) {
  const root = await mkdtemp(join(tmpdir(), "gauss-horizon-dev-runtime-"));
...
test("watcher success markers trigger reload only after a complete line", async (t) => {
  const options = await project(t);
  const host = await startDevelopment({ ...options, commands: { watch: [process.execPath, "-e", 'process.stdout.write("x".repeat(1100)+"GAUSS_HORIZON_UI_BUILD_SUCCESS\\n"); process.stdout.write("GAUSS_HORIZON_UI_BUILD_"); setTimeout(() => process.stdout.write("SUCCESS\\n"), 100); setInterval(() => {}, 1000)'] } });
  t.after(() => host.close());
  let revision = 0;
