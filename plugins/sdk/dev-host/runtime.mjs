...
export async function startDevelopment(options) {
  ...
  const project = resolve(options.project),
    dataDir = resolve(options.dataDir || resolve(project, ".gauss-horizon-dev"));
    ...
    if (commands.watch?.length) {
      ...
      watcher.stdout.on("data", (chunk) => {
        ...
        for (const character of chunk) {
          if (character === "\n") {
            if (!oversized && line.replace(/\r$/, "") === "GAUSS_HORIZON_UI_BUILD_SUCCESS") {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    if (process.argv[2] !== "--config" || !process.argv[3]) throw new Error("Start this runtime using gauss-horizon-plugin dev");
