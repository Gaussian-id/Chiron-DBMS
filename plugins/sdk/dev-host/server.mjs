...
function localDocumentAssetPath(url, entry) {
  ...
  try {
    const parsed = new URL(url, `http://gauss-horizon-plugin.local/${entry}`);
    if (parsed.origin !== "http://gauss-horizon-plugin.local") return undefined;
...
export async function createMockHost(options) {
  ...
  // Gauss Horizon uses Rust semver requirements, whose comparator separators include commas.
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (path.startsWith("/api/")) {
        ...
        const page = pageId(request.headers["x-gauss-horizon-page"]);
  ...
  cookieName = `gauss_horizon_dev_session_${server.address().port}`;
    ...
    void (async () => {
      try {
        for await (const event of watch(options.backendWatch, { signal: watchStop.signal, recursive: true })) {
          ...
          if (path.split("/").some((p) => ["target", ".git", ".gauss-horizon-dev", "node_modules", "vendor"].includes(p))) continue;
