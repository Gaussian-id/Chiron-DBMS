<script>
  let context = $state({});
  let message = $state("Waiting for Chiron Horizon plugin host…");

  const copy = {
    en: { eyebrow: "Chiron Horizon Svelte plugin", description: "A Svelte-powered sandboxed workbench.", action: "Show host context", ready: "Ready" },
    zh: { eyebrow: "Chiron Horizon Svelte 插件", description: "由 Svelte 驱动的沙箱工作台。", action: "查看宿主上下文", ready: "已就绪" },
  };
  let text = $state(copy.en);

  $effect(() => {
    window.chironHorizonPlugin.ready.then((value) => {
      context = value || {};
      text = window.chironHorizonPlugin.locale.toLowerCase().startsWith("zh") ? copy.zh : copy.en;
      message = text.ready;
    });
  });

  async function showContext() {
    context = await window.chironHorizonPlugin.request("host.getContext");
