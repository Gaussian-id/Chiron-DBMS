...
fn run_package(arguments: Vec<String>) -> Result<(), String> {
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            "--key-id" => {
                return Err(
                    "--key-id is no longer supported by chiron-horizon-plugin package; build an unsigned candidate, then let the repository operator sign it after review"
                        .to_string(),
                );
            }
            "--help" | "-h" => {
...
fn run_keygen(arguments: Vec<String>) -> Result<(), String> {
    let mut key_id = None;
    let mut output = PathBuf::from(".chiron-horizon-repository-signing-key.env");
    let mut force = false;
    let mut index = 0;
    while index < arguments.len() {
        match arguments[index].as_str() {
    ...
    let key_id = match key_id {
        Some(value) => resolve_string(
            ...
            chiron_horizon_plugin_packager::validate_key_id,
            &mut reader,
            &mut writer,
        )?,
    ...
    println!(
        "{} Use CHIRON_HORIZON_PLUGIN_SIGNING_KEY only in repository signing CI; publish only the repository key ID and public key.",
...
pub fn generate_signing_key_file(
    ...
    output: &Path,
    force: bool,
) -> Result<chiron_horizon_plugin_packager::SigningKeyMaterial, String> {
    chiron_horizon_plugin_packager::validate_key_id(key_id)?;
    if let Some(parent) = output.parent().filter(|parent| !parent.as_os_str().is_empty()) {
        fs::create_dir_all(parent).map_err(|error| format!("Failed to create {}: {error}", parent.display()))?;
    }
    let material = chiron_horizon_plugin_packager::generate_signing_key()?;
    ...
    writeln!(file, "export CHIRON_HORIZON_PLUGIN_SIGNING_KEY={}", material.private_seed_base64)
        .map_err(|error| error.to_string())?;
    writeln!(file, "export CHIRON_HORIZON_PLUGIN_SIGNING_KEY_ID={key_id}").map_err(|error| error.to_string())?;
    writeln!(file, "export CHIRON_HORIZON_PLUGIN_SIGNING_PUBLIC_KEY={}", material.public_key_base64)
        .map_err(|error| error.to_string())?;
...
fn prompt_project_directory<R: BufRead, W: Write>(
    ...
    loop {
        let value = prompt_line("Project directory", "my-chiron-horizon-plugin", reader, writer)?;
...
pub fn create_project(options: &CreateOptions) -> Result<(), String> {
    ...
    let binary_name = format!("chiron-horizon-plugin-{slug}");
    ...
    println!(
        ...
        styled_stdout(format!("cd {} && chiron-horizon-plugin package .", options.directory.display()), ANSI_MUTED)
    );
    Ok(())
}
...
pub fn package_project(options: &PackageOptions) -> Result<(PathBuf, PathBuf), String> {
    ...
    let config: ProjectConfig = toml::from_str(
        &fs::read_to_string(project.join("chiron-horizon-plugin.toml"))
            .map_err(|error| format!("Failed to read chiron-horizon-plugin.toml: {error}"))?,
    )
    .map_err(|error| format!("Invalid chiron-horizon-plugin.toml: {error}"))?;
    if config.schema_version != 1 {
        return Err(format!("Unsupported chiron-horizon-plugin.toml schema version {}", config.schema_version));
    }
    ...
    for include in &config.package.include {
        ...
        if include.components().any(|part| part.as_os_str() == ".chiron-horizon-dev") {
            return Err("Package input cannot contain .chiron-horizon-dev development data".to_owned());
        }
    }
    ...
    if manifest.manifest_version != 1 {
        return Err(format!(
            "Unsupported manifest.json version {}; chiron-horizon-plugin packages require version 1",
    ...
    let target = options
        .target
        .clone()
        .or_else(|| std::env::var("CHIRON_HORIZON_PLUGIN_TARGET").ok())
    ...
    chiron_horizon_plugin_packager::run_cli_silent(packager_arguments)?;
...
fn package_manifest(mut manifest: Value, backend: Option<&BackendConfig>, target: &str) -> Result<Vec<u8>, String> {
    ...
    if let Some(backend) = backend {
        ...
        if backend_entrypoint.contains_key("protocol") {
            return Err(
                "manifest.json entrypoints.backend.protocol is obsolete; Chiron Horizon manifest v1 uses the Chiron Horizon JSON-RPC protocol"
                    .to_string(),
            );
        }
...
fn build_rust_backend(
    ...
    if let Some(sdk_root) = sdk_root_from_environment()? {
        let sdk = sdk_root.join("plugins/sdk/rust/chiron-horizon-plugin-sdk");
...
fn build_go_backend(
    ...
    if let Some(sdk_root) = sdk_root_from_environment()? {
        let sdk = sdk_root.join("plugins/sdk/go/chiron-horizon-plugin-sdk");
...
fn copy_path(source: &Path, destination: &Path) -> Result<(), String> {
    if source.file_name().is_some_and(|name| name == ".chiron-horizon-dev") {
        return Err("Package input cannot contain .chiron-horizon-dev development data".to_owned());
    }
...
fn rust_sdk_dependency(sdk_root: Option<&Path>) -> Result<String, String> {
    match sdk_root {
        Some(root) => {
            let path = root.join("plugins/sdk/rust/chiron-horizon-plugin-sdk");
...
fn go_sdk_replace(sdk_root: Option<&Path>) -> Result<String, String> {
    match sdk_root {
        Some(root) => {
            let path = root.join("plugins/sdk/go/chiron-horizon-plugin-sdk");
...
fn sdk_root_from_environment() -> Result<Option<PathBuf>, String> {
    std::env::var_os("CHIRON_HORIZON_PLUGIN_SDK_ROOT")
...
fn default_description(name: &str) -> String {
    if name.to_ascii_lowercase().ends_with(" plugin") {
        format!("{name} for Chiron Horizon.")
    } else {
        format!("{name} plugin for Chiron Horizon.")
    }
}
...
fn print_usage() {
    println!(
        ...
        styled_stdout("Create, sign, and package Chiron Horizon plugins", ANSI_MUTED)
    );
    ...
    println!("  chiron-horizon-plugin create");
    println!("  chiron-horizon-plugin create my-plugin --template frontend --yes");
    println!("  chiron-horizon-plugin keygen example.release");
    println!("  chiron-horizon-plugin package .");
    println!("\nRun chiron-horizon-plugin <command> --help for command options.");
}

fn print_create_help() {
    println!("{}", styled_stdout("Create a Chiron Horizon plugin project", ANSI_ACCENT));
    ...
    println!("      --sdk-root PATH       Local Chiron Horizon SDK checkout for Rust or Go templates");
...
fn print_package_help() {
    println!("{}", styled_stdout("Package a Chiron Horizon plugin", ANSI_ACCENT));
    ...
    println!("\n{}", styled_stdout("Options:", ANSI_PROMPT));
    println!("      --target TARGET       Artifact target or CHIRON_HORIZON_PLUGIN_TARGET");
...
fn create_usage() -> String {
    "chiron-horizon-plugin create [directory] [--template frontend|rust|go] [options]".to_string()
}

fn package_usage() -> String {
    "chiron-horizon-plugin package [project] [--target TARGET] [--output-dir DIR] [--artifact-url URL]".to_string()
}

fn keygen_usage() -> String {
    "chiron-horizon-plugin keygen [KEY_ID] [--output FILE] [--force]".to_string()
}
...
mod tests {
    ...
    fn rejects_development_data_in_package_inputs() {
        ...
        std::fs::create_dir_all(input.join(".chiron-horizon-dev")).unwrap();
        std::fs::write(input.join(".chiron-horizon-dev/connections.json"), "private").unwrap();
    ...
    fn styles_text_without_changing_plain_output() {
        assert_eq!(styled("Chiron Horizon", ANSI_ACCENT, false), "Chiron Horizon");
        assert_eq!(styled("Chiron Horizon", ANSI_ACCENT, true), "\x1b[1;35mChiron Horizon\x1b[0m");
    }
    ...
    fn rejects_obsolete_fixed_entrypoint_fields() {
        ...
        let backend_manifest = serde_json::json!({
            "entrypoints": {
                "backend": {
                    "protocol": "chiron-horizon-jsonrpc",
    ...
    fn package_validation_and_failures_leave_no_temporary_directories() {
        ...
        .unwrap();
        std::fs::write(
            mismatch.join("chiron-horizon-plugin.toml"),
            "schema_version = 1\n\n[backend]\nlanguage = \"rust\"\ndirectory = \"backend\"\nbinary = \"mismatch\"\n\n[package]\ninclude = [\"assets\", \"ui\"]\n",
        )
        .unwrap();
    ...
    fn interactive_wizard_reprompts_invalid_values() {
        ...
        assert_eq!(options.description, "Wizard Plugin for Chiron Horizon.");
    ...
    fn formats_common_plugin_acronyms_and_validates_semver() {
        assert_eq!(title_from_slug("chiron-horizon-ssh-sftp-workbench"), "Chiron Horizon SSH SFTP Workbench");
