...
fn build_config(options: &Options, sdk_root: Option<&Path>) -> Result<Value, String> {
    ...
    let config: ProjectConfig = toml::from_str(
        &fs::read_to_string(project.join("chiron-horizon-plugin.toml"))
            .map_err(|e| format!("Cannot read chiron-horizon-plugin.toml: {e}"))?,
    )
    .map_err(|e| format!("Invalid chiron-horizon-plugin.toml: {e}"))?;
    ...
    let data_dir = match &options.data_dir {
        ...
        None => project.join(".chiron-horizon-dev"),
    };
    ...
    if let Some(backend) = config.backend {
        ...
        let (binary, command) = match backend.language.as_str() {
            "rust" => {
                ...
                let dependency = cargo.get("dependencies").and_then(|d| d.get("chiron-horizon-plugin-sdk"));
                ...
                if let Some(root) = sdk_root.filter(|_| dependency.is_some() && !explicit_source) {
                    let sdk = root.join("plugins/sdk/rust/chiron-horizon-plugin-sdk");
            ...
            "go" => {
                ...
                if let Some(root) = sdk_root {
                    let sdk = root.join("plugins/sdk/go/chiron-horizon-plugin-sdk");
...
pub(super) fn run(arguments: Vec<String>) -> Result<(), String> {
    if arguments.iter().any(|a| a == "--help" || a == "-h") {
        println!("Run a plugin without starting Chiron Horizon (Node.js 22+)\n\nUsage: chiron-horizon-plugin dev [--path DIR] [--port PORT] [--data-dir DIR]\n\n  --path DIR      Plugin project (default: current directory)\n  --port PORT     Loopback port (default: 5190; occupied ports fall back to a free port)\n  --data-dir DIR  Development data (default: <project>/.chiron-horizon-dev)\n\nOptional [dev] ui_build and ui_watch arrays configure UI commands.\nDevelopment credentials are stored locally as plaintext.\nCHIRON_HORIZON_PLUGIN_DEV_RUNTIME overrides the runtime entrypoint; CHIRON_HORIZON_PLUGIN_NODE selects Node.");
        return Ok(());
    }
    let options = parse(&arguments)?;
    let sdk_root = sdk_root_from_environment()?;
    let launch = build_config(&options, sdk_root.as_deref())?;
    let runtime = std::env::var_os("CHIRON_HORIZON_PLUGIN_DEV_RUNTIME")
        .map(PathBuf::from)
        .unwrap_or_else(|| Path::new(env!("CARGO_MANIFEST_DIR")).join("../dev-host/dist/runtime.mjs"));
    if !runtime.is_file() {
        return Err("Development runtime is missing. Install @chiron-horizon/plugin-cli, or run 'npm ci --prefix plugins/sdk/dev-host && npm run build --prefix plugins/sdk/dev-host' in a Chiron Horizon checkout and set CHIRON_HORIZON_PLUGIN_DEV_RUNTIME to dist/runtime.mjs.".into());
    }
    let node = std::env::var_os("CHIRON_HORIZON_PLUGIN_NODE").unwrap_or_else(|| "node".into());
    let version = Command::new(&node)
        .arg("--version")
        .output()
        .map_err(|_| "dev requires Node.js 22+; install Node or set CHIRON_HORIZON_PLUGIN_NODE")?;
...
mod tests {
    ...
    fn frontend_needs_no_backend_and_dev_arrays_are_typed() {
        ...
        fs::write(root.path().join("manifest.json"), r#"{"entrypoints":{"ui":{"root":"ui","entry":"ui/index.html"}}}"#)
            .unwrap();
        fs::write(
            root.path().join("chiron-horizon-plugin.toml"),
