...
const dist = process.env.CHIRON_HORIZON_PLUGIN_OUTPUT_DIR ? path.resolve(process.env.CHIRON_HORIZON_PLUGIN_OUTPUT_DIR) : path.join(root, "dist");
const buildRoot = await mkdtemp(path.join(os.tmpdir(), "chiron-horizon-hello-package-"));
const stage = path.join(buildRoot, "stage");
const detectedTarget = platformTarget();
const target = process.env.CHIRON_HORIZON_PLUGIN_TARGET || detectedTarget;
if (target !== detectedTarget) throw new Error(`CHIRON_HORIZON_PLUGIN_TARGET ${target} does not match native build host ${detectedTarget}`);
const executableName = process.platform === "win32" ? "chiron-horizon-example-hello.exe" : "chiron-horizon-example-hello";
const backendTarget = path.join(buildRoot, "backend-target");
const packagerTarget = path.join(buildRoot, "packager-target");
const output = path.join(dist, `chiron.horizon.example.hello-1.0.0-${target}.chiron-horizonp`);
const artifactMetadata = output.replace(/\.chiron-horizonp$/, ".artifact.json");

if (process.env.CHIRON_HORIZON_PLUGIN_SIGNING_KEY || process.env.CHIRON_HORIZON_PLUGIN_SIGNING_KEY_ID) {
