...
export interface AiChatMessage {
  ...
  /** Set on the assistant message whose generation failed; persisted (mirrors gauss-horizon-core `AiChatMessage.failed`). */
  failed?: boolean;
}
...
export async function executeMulti(
  ...
  try {
    ...
    if (diagnosticsEnabled) {
      appendDebugLog("info", "[Gauss Horizon][query-transport:tauri]", {
    ...
    if (diagnosticsEnabled) {
      appendDebugLog("warn", "[Gauss Horizon][query-transport:tauri:error]", {
        traceId: executionId?.slice(0, 8),
        totalMs: Math.round(performance.now() - startedAt),
      });
    }
...
export async function installPluginPackage(pathOrFile: string | File, allowUnsigned = false): Promise<PluginInstallResult> {
  if (typeof pathOrFile !== "string") throw new Error("Desktop plugin installation requires a local .gauss-horizonp file path");
...
export async function subscribePluginEvents(onEvent: (event: PluginEvent) => void, onBinary?: (event: PluginBinaryEvent) => void): Promise<UnlistenFn> {
  const unlistenEvent = await listen<PluginEvent>("gauss-horizon-plugin-event", (event) => onEvent(event.payload));
  const unlistenBinary = await listen<PluginBinaryEvent>("gauss-horizon-plugin-binary", (event) => onBinary?.(event.payload));
  return () => {
    unlistenEvent();
    unlistenBinary();
  };
