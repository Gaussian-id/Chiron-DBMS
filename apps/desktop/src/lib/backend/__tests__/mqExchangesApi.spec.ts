...
describe("mq exchanges/bindings tauri API", () => {
  ...
  it("invokes mq_list_exchanges with the namespace ref", async () => {
    ...
    mocks.invoke.mockResolvedValue([{ name: "gauss-horizon-events", type: "topic", durable: true, autoDelete: false, internal: false }]);
    ...
    expect(result[0]?.name).toBe("gauss-horizon-events");
  });
  ...
  it("invokes mq_create_exchange with flattened fields", async () => {
    ...
    await mqCreateExchange("conn-1", NS, { name: "gauss-horizon-events", type: "topic", durable: true, autoDelete: false });

    expect(mocks.invoke).toHaveBeenCalledWith("mq_create_exchange", {
      connectionId: "conn-1",
      ns: NS,
      name: "gauss-horizon-events",
      exchangeType: "topic",
      durable: true,
      autoDelete: false,
    });
  ...
  it("invokes mq_delete_exchange with the exchange name", async () => {
    ...
    await mqDeleteExchange("conn-1", NS, "gauss-horizon-events");

    expect(mocks.invoke).toHaveBeenCalledWith("mq_delete_exchange", { connectionId: "conn-1", ns: NS, name: "gauss-horizon-events" });
  });
  ...
  it("invokes mq_list_bindings with optional filters", async () => {
    ...
    await mqListBindings("conn-1", NS, { exchange: "gauss-horizon-events" });
    expect(mocks.invoke).toHaveBeenCalledWith("mq_list_bindings", { connectionId: "conn-1", ns: NS, exchange: "gauss-horizon-events", queue: undefined });
  ...
  it("invokes mq_bind and mq_unbind with the binding object", async () => {
    ...
    mocks.invoke.mockResolvedValue(undefined);
    const binding = { source: "gauss-horizon-events", destination: "gauss-horizon-queue", destinationType: "queue", routingKey: "orders.*" };
...
describe("mq exchanges/bindings HTTP API", () => {
  ...
  it("posts to the exchanges endpoints", async () => {
    ...
    await mqCreateExchange("conn-1", NS, { name: "gauss-horizon-events", type: "fanout", durable: false, autoDelete: true });
    expect(lastCall(fetchMock)).toEqual({
      url: "/api/mq/exchanges/create",
      body: { connectionId: "conn-1", ns: NS, name: "gauss-horizon-events", exchangeType: "fanout", durable: false, autoDelete: true },
    });

    await mqDeleteExchange("conn-1", NS, "gauss-horizon-events");
    expect(lastCall(fetchMock)).toEqual({ url: "/api/mq/exchanges/delete", body: { connectionId: "conn-1", ns: NS, name: "gauss-horizon-events" } });
  });
  ...
  it("posts to the bindings endpoints", async () => {
    ...
    await mqListBindings("conn-1", NS, { queue: "gauss-horizon-queue" });
    expect(lastCall(fetchMock)).toEqual({ url: "/api/mq/bindings/list", body: { connectionId: "conn-1", ns: NS, exchange: undefined, queue: "gauss-horizon-queue" } });
