...
describe("mq users/permissions tauri API", () => {
  ...
  it("invokes mq_list_users with only the connection id", async () => {
    ...
    mocks.invoke.mockResolvedValue([{ name: "chiron-horizon-app", tags: ["administrator"] }]);

    const result = await mqListUsers("conn-1");

    expect(mocks.invoke).toHaveBeenCalledWith("mq_list_users", { connectionId: "conn-1" });
    expect(result).toHaveLength(1);
    expect(result[0]?.name).toBe("chiron-horizon-app");
  ...
  it("invokes mq_create_user with name, password and optional tags", async () => {
    ...
    await mqCreateUser("conn-1", "chiron-horizon-app", "secret", ["monitoring", "management"]);
    expect(mocks.invoke).toHaveBeenCalledWith("mq_create_user", { connectionId: "conn-1", name: "chiron-horizon-app", password: "secret", tags: ["monitoring", "management"] });

    await mqCreateUser("conn-1", "chiron-horizon-app2", "secret2");
    expect(mocks.invoke).toHaveBeenCalledWith("mq_create_user", { connectionId: "conn-1", name: "chiron-horizon-app2", password: "secret2", tags: undefined });
  });
  ...
  it("invokes mq_delete_user with the user name", async () => {
    ...
    await mqDeleteUser("conn-1", "chiron-horizon-app");

    expect(mocks.invoke).toHaveBeenCalledWith("mq_delete_user", { connectionId: "conn-1", name: "chiron-horizon-app" });
  });
  ...
  it("invokes mq_list_user_permissions with flattened filters", async () => {
    ...
    mocks.invoke.mockResolvedValue([{ user: "chiron-horizon-app", vhost: "/", configure: ".*", write: ".*", read: ".*" }]);
    ...
    await mqListUserPermissions("conn-1", { allVhosts: true, user: "chiron-horizon-app" });
    expect(mocks.invoke).toHaveBeenCalledWith("mq_list_user_permissions", { connectionId: "conn-1", virtualHost: undefined, user: "chiron-horizon-app", allVhosts: true });
  ...
  it("invokes mq_grant_user_permission with optional patterns", async () => {
    ...
    await mqGrantUserPermission("conn-1", "chiron-horizon-app", "/", { configure: "^chiron-horizon-", write: ".*", read: ".*" });
    expect(mocks.invoke).toHaveBeenCalledWith("mq_grant_user_permission", { connectionId: "conn-1", user: "chiron-horizon-app", virtualHost: "/", configure: "^chiron-horizon-", write: ".*", read: ".*" });

    await mqGrantUserPermission("conn-1", "chiron-horizon-app", "/");
    expect(mocks.invoke).toHaveBeenCalledWith("mq_grant_user_permission", { connectionId: "conn-1", user: "chiron-horizon-app", virtualHost: "/", configure: undefined, write: undefined, read: undefined });
  });
  ...
  it("invokes mq_revoke_user_permission with user and vhost", async () => {
    ...
    await mqRevokeUserPermission("conn-1", "chiron-horizon-app", "/");

    expect(mocks.invoke).toHaveBeenCalledWith("mq_revoke_user_permission", { connectionId: "conn-1", user: "chiron-horizon-app", virtualHost: "/" });
  });
});
...
describe("mq users/permissions HTTP API", () => {
  ...
  it("posts to the users endpoints", async () => {
    ...
    await mqCreateUser("conn-1", "chiron-horizon-app", "secret", ["monitoring"]);
    expect(lastCall(fetchMock)).toEqual({ url: "/api/mq/users/create", body: { connectionId: "conn-1", name: "chiron-horizon-app", password: "secret", tags: ["monitoring"] } });

    await mqDeleteUser("conn-1", "chiron-horizon-app");
    expect(lastCall(fetchMock)).toEqual({ url: "/api/mq/users/delete", body: { connectionId: "conn-1", name: "chiron-horizon-app" } });
  });
  ...
  it("posts to the user-permissions endpoints", async () => {
    ...
    await mqGrantUserPermission("conn-1", "chiron-horizon-app", "/", { configure: "^chiron-horizon-" });
    expect(lastCall(fetchMock)).toEqual({ url: "/api/mq/user-permissions/grant", body: { connectionId: "conn-1", user: "chiron-horizon-app", virtualHost: "/", configure: "^chiron-horizon-", write: undefined, read: undefined } });

    await mqRevokeUserPermission("conn-1", "chiron-horizon-app", "/");
    expect(lastCall(fetchMock)).toEqual({ url: "/api/mq/user-permissions/revoke", body: { connectionId: "conn-1", user: "chiron-horizon-app", virtualHost: "/" } });
  });
