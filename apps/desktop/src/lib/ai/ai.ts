...
const VECTOR_DB_TYPES: ReadonlySet<DatabaseType> = new Set([
  ...
  "weaviate",
  "chromadb",
  // If modifying this, also update is_vector_db() in crates/chiron-horizon-core/src/agent_tools.rs.
]);
...
function buildBasePromptLines(isZh: boolean): string[] {
  return [
    isZh ? "你是 Chiron Horizon 内置的数据库助手。用中文回复。" : "You are Chiron Horizon's built-in database assistant. Reply in English.",
...
function buildVectorSystemPrompt(context: AiContext, mode: AiAssistantMode, custom?: CustomPromptContext): string {
  ...
  const referencedSqlFiles = formatReferencedSqlFiles(context);
  const lines: string[] = [
    isZh ? `你是 Chiron Horizon 内置的向量数据库助手。当前连接的是 ${dbLabel(context.databaseType)} 数据库。用中文回复。` : `You are Chiron Horizon's vector database assistant. Connected to ${dbLabel(context.databaseType)}. Reply in English.`,
