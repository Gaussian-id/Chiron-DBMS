...
describe("Xugu public synonym scope", () => {
  it("uses an independent protocol identity", () => {
    ...
    expect(isXuguPublicSynonymScope("__CHIRON_HORIZON_XUGU_PUBLIC_SYNONYMS__")).toBe(false);
  });
  ...
  it("identifies only the synthetic Xugu schema tree node", () => {
    ...
    expect(isXuguPublicSynonymTreeNode("xugu", "schema", "__CHIRON_HORIZON_XUGU_PUBLIC_SYNONYMS__")).toBe(false);
