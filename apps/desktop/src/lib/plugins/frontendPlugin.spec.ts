...
describe("FrontendPluginRegistry", () => {
  ...
  it("treats a host-serialized null default as unset instead of a value", () => {
    ...
    const config = buildPluginConnectionConfig("id.chiron.horizon.ssh", provider, {
      sudo_password: null,
      mode: null,
      read_only: false,
  ...
  it("preserves opaque 'null' credentials when reopening and saving connections", () => {
    ...
    const existing = buildPluginConnectionConfig("id.chiron.horizon.ssh", provider, {});
    ...
    const saved = buildPluginConnectionConfig("id.chiron.horizon.ssh", provider, values, existing);
