...
const PLUGIN_MESSAGE_SOURCE = "gauss-horizon-plugin";
const HOST_MESSAGE_SOURCE = "gauss-horizon-host";
...
export interface PluginBridgeTheme {
  ...
  /** Resolved Gauss Horizon design tokens (`--color-*`, `--radius-*`, ...) for the current theme. */
...
export class PluginHostBridge {
  ...
  /** Push resolved theme tokens so the plugin UI can follow Gauss Horizon light/dark and palette changes. */
 ...
 * crates/gauss-horizon-core/src/plugins/manifest.rs.
 */
 ...
 * on the Gauss Horizon design tokens the host pushes through the bridge, so plugin UI
...
.gauss-horizon-btn:hover { background: var(--color-muted, #f4f4f5); }
.gauss-horizon-btn--primary { background: var(--color-primary, #2563eb); border-color: var(--color-primary, #2563eb); color: var(--color-primary-foreground, #ffffff); }
.gauss-horizon-btn--primary:hover { background: var(--color-primary, #2563eb); opacity: 0.9; }
.gauss-horizon-btn--danger { background: var(--color-destructive, #dc2626); border-color: var(--color-destructive, #dc2626); color: var(--color-destructive-foreground, #ffffff); }
.gauss-horizon-btn--ghost { border-color: transparent; background: transparent; }
.gauss-horizon-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.gauss-horizon-label { display: inline-flex; font-size: 12px; font-weight: 500; color: var(--color-foreground, #18181b); }
.gauss-horizon-input, .gauss-horizon-select, .gauss-horizon-textarea {
  width: 100%;
  height: 30px;
  padding: 0 10px;
  border-radius: var(--radius-md, 8px);
...
.gauss-horizon-textarea { height: auto; min-height: 64px; padding: 6px 10px; resize: vertical; }
.gauss-horizon-input:focus, .gauss-horizon-select:focus, .gauss-horizon-textarea:focus { outline: 2px solid var(--color-ring, #93c5fd); outline-offset: 1px; border-color: var(--color-ring, #93c5fd); }
.gauss-horizon-hint { font-size: 12px; color: var(--color-muted-foreground, #71717a); }
.gauss-horizon-row { display: grid; grid-template-columns: minmax(96px, auto) minmax(0, 1fr); gap: 8px 12px; align-items: center; margin-bottom: 10px; }
.gauss-horizon-table { width: 100%; border-collapse: collapse; font-size: 12px; }
.gauss-horizon-table th { text-align: left; font-weight: 600; color: var(--color-muted-foreground, #71717a); border-bottom: 1px solid var(--color-border, #e4e4e7); padding: 6px 8px; }
.gauss-horizon-table td { border-bottom: 1px solid var(--color-border, #e4e4e7); padding: 6px 8px; }
.gauss-horizon-badge { display: inline-flex; align-items: center; height: 20px; padding: 0 8px; border-radius: 999px; background: var(--color-primary-alpha, rgba(37, 99, 235, 0.12)); color: var(--color-primary, #2563eb); font-size: 11px; font-weight: 500; }
.gauss-horizon-link { color: var(--color-primary, #2563eb); text-decoration: none; cursor: pointer; }
.gauss-horizon-link:hover { text-decoration: underline; }
`.trim();
}
...
function pluginSdkSource(): string {
  return `(() => {
    ...
    const applyTheme = (value) => {
      ...
      root.dataset.gaussHorizonTheme = value.appearance === 'dark' ? 'dark' : 'light';
    ...
    addEventListener('message', (event) => {
      ...
      if (message.type === 'response') {
        ...
        dispatchEvent(new CustomEvent('gauss-horizon-plugin-init', { detail: message }));
        ...
        dispatchEvent(new CustomEvent('gauss-horizon-plugin-env', { detail: message }));
      } else if (message.type === 'event') {
        listeners.event.forEach((listener) => listener(message));
        dispatchEvent(new CustomEvent('gauss-horizon-plugin-event', { detail: message }));
