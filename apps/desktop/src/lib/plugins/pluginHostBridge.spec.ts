...
describe("PluginHostBridge", () => {
  it("binds backend calls to the owning plugin identity", async () => {
    ...
    expect(
      bridge.handleWindowMessage({
        source: target,
        data: { source: "gauss-horizon-plugin", version: 1, type: "request", id: "1", method: "backend.invoke", params: { method: "sample/hello", params: { name: "Gauss Horizon" } } },
      } as MessageEvent),
    ).toBe(true);
    await vi.waitFor(() => expect(messages).toHaveLength(1));

    expect(invoke).toHaveBeenCalledWith("sample", "sample/hello", { name: "Gauss Horizon" }, undefined);
    expect(messages[0]).toMatchObject({ source: "gauss-horizon-host", type: "response", id: "1", result: { ok: true } });
  });

  it("sends the current Gauss Horizon locale in the init message", () => {
    const messages: unknown[] = [];
    const target = { postMessage: (message: unknown) => messages.push(message) } as unknown as Window;
    ...
    expect(messages[0]).toMatchObject({ source: "gauss-horizon-host", type: "init", locale: "zh-CN", context: { connectionId: "connection" } });
  });
  ...
  it("rejects privileged host calls without manifest permission", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "gauss-horizon-plugin", version: 1, type: "request", id: "2", method: "host.openWorkbench", params: { contributionId: "sample.other" } },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(1));
  ...
  it("forwards events and binary traffic only with declared permissions", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "gauss-horizon-plugin", version: 1, type: "request", id: "3", method: "backend.sendBinary", params: { channel: "pty", dataBase64: "AQI=" } },
    } as MessageEvent);
  ...
  it("returns delayed responses to the iframe that issued the request", async () => {
    ...
    bridge.handleWindowMessage({
      source: first,
      data: { source: "gauss-horizon-plugin", version: 1, type: "request", id: "4", method: "backend.invoke", params: { method: "sample/slow" } },
    } as MessageEvent);
    current = second;
  ...
  it("pushes context and locale updates without rebuilding the iframe", async () => {
    ...
    expect(messages[0]).toMatchObject({ source: "gauss-horizon-host", type: "context", context: { connectionId: "second" } });
    expect(messages[1]).toMatchObject({ source: "gauss-horizon-host", type: "env", locale: "ja" });

    bridge.handleWindowMessage({ source: target, data: { source: "gauss-horizon-plugin", version: 1, type: "request", id: "ctx", method: "host.getContext" } } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(3));
  ...
  it("accepts zero-copy binary requests and forwards binary frames as raw buffers", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "gauss-horizon-plugin", version: 1, type: "request", id: "bin", method: "backend.sendBinary", params: { channel: "pty" }, data: bytes },
    } as MessageEvent);
  ...
  it("rejects binary transfer requests without the host.binary permission", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "gauss-horizon-plugin", version: 1, type: "request", id: "denied", method: "backend.sendBinary", params: { channel: "pty" }, data: new Uint8Array([1]).buffer },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(1));
  ...
  it("injects the SDK, the official UI kit, and a restrictive sandbox CSP", () => {
    ...
    expect(document).toContain("window.gaussHorizonPlugin");
    ...
    expect(document).toContain(".gauss-horizon-btn");
  ...
  it("forwards the plugin close-tab shortcut to the host", () => {
    ...
    expect(
      bridge.handleWindowMessage({
        source: target,
        data: { source: "gauss-horizon-plugin", version: 1, type: "shortcut", shortcut: "closeTab" },
      } as MessageEvent),
    ).toBe(true);
