...
describe("PluginHostBridge", () => {
  ...
  it("injects the SDK, the official UI kit, and a restrictive sandbox CSP", () => {
    ...
    expect(document).toContain("document.dispatchEvent(new CustomEvent('chiron-horizon-plugin-env'");
  ...
  it("bootstraps the initial theme before the plugin document paints", () => {
    ...
    expect(document).toContain('root.dataset.chironHorizonTheme = theme.appearance === "dark" ? "dark" : "light"');
  ...
  it("pre-seeds the current theme as the sandbox first paint", () => {
    ...
    expect(themed.indexOf("<style>:root{")).toBeGreaterThan(themed.indexOf(".chiron-horizon-btn"));
  ...
  it("routes host.saveFile transfers through the host save dialog and reports cancellation", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "save1", method: "host.saveFile", params: { fileName: "report.csv", contentType: "text/csv" }, data: new Uint8Array([1, 2, 3]).buffer },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(1));
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "save2", method: "host.saveFile", params: { dataBase64: "AQID" } },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(2));
  ...
  it("routes host.copy through the host clipboard and rejects bad payloads", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "copy1", method: "host.copy", params: { text: "https://example.com/share?sig=1" } },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(1));
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "copy2", method: "host.copy", params: { text: "" } },
    } as MessageEvent);
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "copy3", method: "host.copy", params: { text: "x".repeat(2 * 1024 * 1024 + 1) } },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(3));
  ...
  it("rejects host.copy when the host has no clipboard support", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "nocopy", method: "host.copy", params: { text: "hello" } },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(1));
  ...
  it("rejects host.saveFile without payload or host support", async () => {
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "empty", method: "host.saveFile", params: { fileName: "x.bin" } },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(1));
    ...
    bridge.handleWindowMessage({
      source: target,
      data: { source: "chiron-horizon-plugin", version: 1, type: "request", id: "unsupported", method: "host.saveFile", params: { dataBase64: "AQID" } },
    } as MessageEvent);
    await vi.waitFor(() => expect(messages).toHaveLength(2));
...
describe("plugin SDK source", () => {
  interface SdkWindow {
    chironHorizonPlugin?: { invoke: (method: string, params?: unknown, options?: { timeoutMs?: number }) => Promise<unknown> };
  }
  ...
  it("plainifies reactive (Proxy) params before postMessage", () => {
    ...
    const { chironHorizonPlugin } = loadSdk(posted);
    expect(chironHorizonPlugin).toBeDefined();
    ...
    void chironHorizonPlugin!.invoke("ssh/terminal/batchInput", { sessionIds: proxy, command: "ls -la" });
  ...
  it("boots successfully when the host provides the initial theme", () => {
    const posted: unknown[] = [];
    const { chironHorizonPlugin } = loadSdk(posted, { appearance: "dark", tokens: { "--color-background": "#131416" } });

    expect(chironHorizonPlugin).toBeDefined();
    expect(posted[0]).toMatchObject({ source: "chiron-horizon-plugin", type: "ready" });
  });
  ...
  it("keeps cloneable values intact and plain scalars by reference", () => {
    ...
    void chironHorizonPlugin!.invoke("sample/hello", { count: 3, stamp, nested: new Map([["k", "v"]]) });
