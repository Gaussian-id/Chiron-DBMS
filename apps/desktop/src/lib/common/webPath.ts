...
export function gaussHorizonWebBasePath(pathname = globalThis.location?.pathname ?? "", buildBase = import.meta.env.BASE_URL): string {
  const configured = normalizeBasePath(buildBase);
  if (configured) return configured;
  return normalizeBasePath(inferredRuntimeBasePath(pathname));
}

export function webPath(path: string, basePath = gaussHorizonWebBasePath()): string {
  const normalizedPath = path.startsWith("/") ? path : `/${path}`;
  const base = normalizeBasePath(basePath);
  return `${base}${normalizedPath}` || "/";
}

export function apiUrl(path: string, basePath = gaussHorizonWebBasePath()): string {
...
export function apiWebSocketUrl(path: string, basePath = gaussHorizonWebBasePath(), currentLocation: WebSocketLocation = globalThis.location): string {
