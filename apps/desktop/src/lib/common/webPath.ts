...
export function chironHorizonWebBasePath(pathname = globalThis.location?.pathname ?? "", buildBase = import.meta.env.BASE_URL): string {
  const configured = normalizeBasePath(buildBase);
  if (configured) return configured;
  return normalizeBasePath(inferredRuntimeBasePath(pathname));
}

export function webPath(path: string, basePath = chironHorizonWebBasePath()): string {
  const normalizedPath = path.startsWith("/") ? path : `/${path}`;
  const base = normalizeBasePath(basePath);
  return `${base}${normalizedPath}` || "/";
}

export function apiUrl(path: string, basePath = chironHorizonWebBasePath()): string {
...
export function apiWebSocketUrl(path: string, basePath = chironHorizonWebBasePath(), currentLocation: WebSocketLocation = globalThis.location): string {
