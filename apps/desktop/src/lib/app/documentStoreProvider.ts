...
import { normalizeJsonArgument } from "@gauss-horizon/mongo-shell";
...
const meilisearchDocumentProvider: DocumentStoreProvider = {
  ...
  queryPreview: ({ collection, filterJson, sortJson, skip, limit }) => {
    const lines = ["Gauss Horizon MEILISEARCH FETCH DOCUMENTS", `index: ${JSON.stringify(collection)}`, `offset: ${skip}`, `limit: ${limit}`];
...
const dynamodbDocumentProvider: DocumentStoreProvider = {
  ...
  queryPreview: ({ collection, filterJson, sortJson, limit }) => {
    ...
    const lines = [`Gauss Horizon DYNAMODB ${operation}`, `table: ${JSON.stringify(collection)}`, `limit: ${limit}`];
    if (filter) lines.push("filter:", filter);
    if (sort) lines.push("sort:", sort);
    return lines.join("\n");
  },
