...
/**
 * Runtime UI tuning knobs, loaded once from `~/.chiron-horizon/ui-tuning.json` at
...
/** Reads `~/.chiron-horizon/ui-tuning.json` once; safe to call repeatedly. */
export function loadUiTuning(): Promise<void> {
  loading ??= (async () => {
    ...
    try {
      ...
      const file = await join(await homeDir(), ".chiron_horizon", "ui-tuning.json");
      ...
      console.info("[ui-tuning] using defaults (could not read ~/.chiron-horizon/ui-tuning.json)", error);
    }
  })();
  return loading;
}
