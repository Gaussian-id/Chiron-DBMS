...
describe("Chiron Horizon theme", () => {
  ...
  it("defaults new installations to Chiron Horizon light and preserves saved preferences", () => {
    expect(normalizeAppThemePalette(null)).toBe("chiron");
  ...
  it("provides DOM and canvas colors without overwriting corner preferences", () => {
    const css = readFileSync(new URL("../../../styles/chiron.css", import.meta.url), "utf8");
    expect(css).toContain("html.theme-chiron.dark");
    expect(css).toContain("--chiron-horizon-primary-rgb: 46, 123, 255");
    expect(css).toContain("--chiron-horizon-content: #070b16");
    expect(css).toContain("html.theme-chiron[data-corner-style='large']");
