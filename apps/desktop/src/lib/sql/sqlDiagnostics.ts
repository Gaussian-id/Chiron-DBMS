...
function removeChironHorizonPagination(sql: string): string {
 ...
 * errors while allowing this known Chiron Horizon rewrite.
 */
export function sqlErrorSqlMatchesEditor(editorSql: string, executedSql: string): boolean {
  ...
  return norm(removeChironHorizonPagination(executedSql)) === norm(removeChironHorizonPagination(editorSql));
}
