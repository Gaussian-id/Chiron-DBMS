 ...
 * the optional Chiron Horizon setting needs to retain visual paragraph boundaries such as
 * the blank line between a heading comment and a query. Line comments are
 * valid at every SQL code boundary and are restored only after all Chiron Horizon layout
 * post-processing is complete.
 */
function protectEmptyLines(sql: string): EmptyLineProtection {
  let namespace = 0;
  while (sql.includes(`__CHIRON_HORIZON_PRESERVE_EMPTY_LINE_${namespace}_`)) namespace += 1;
  ...
  while ((match = lineBreakPattern.exec(sql))) {
    ...
    if (line.trim().length === 0) {
      const marker = `-- __CHIRON_HORIZON_PRESERVE_EMPTY_LINE_${namespace}_${markers.length}__`;
      markers.push(marker);
      output += marker;
    } else {
      output += line;
...
function protectDuckDbPrefixAliasSeparators(sql: string): { sql: string; marker: string | null } {
  ...
  let marker = "";
  do {
    marker = `/*__CHIRON_HORIZON_DUCKDB_PREFIX_ALIAS_COLON_${markerIndex}__*/`;
    markerIndex += 1;
  ...
  while (index < sql.length) {
    ...
    if (prefixAlias) {
      ...
      // through formatting preserves the boundary used by Chiron Horizon's parameter scan.
      protectedSql += marker;
      replaced = true;
      index += 1;
      continue;
...
export async function formatSqlText(sql: string, dialect: SqlFormatDialect = "generic", settings: Partial<SqlFormatterSettings> = DEFAULT_SQL_FORMATTER_SETTINGS): Promise<string> {
  ...
  // Chiron Horizon's own layout printer produces the default style. It needs the AST and
