...
// Gauss Horizon substitutes `${name}`/`#{name}` placeholders client-side (see sqlVariableSyntax.ts),
...
const GAUSS_HORIZON_CUSTOM_PARAM_TYPES: SqlFormatterCustomParameter[] = [{ regex: String.raw`\$\{[^}]+\}` }, { regex: String.raw`#\{[^}]+\}` }];
...
export function sqlFormatterOptions(settings: unknown) {
  ...
  return {
    ...
    paramTypes: {
      ...
      custom: [...(paramTypes.custom ?? []), ...GAUSS_HORIZON_CUSTOM_PARAM_TYPES],
    },
  };
}
