...
describe("Nacos service management state reconciliation", () => {
  ...
  it("compares metadata independently of object key order", () => {
    expect(nacosJsonObjectMatches({ owner: "gauss-horizon", nested: { b: 2, a: 1 } }, { nested: { a: 1, b: 2 }, owner: "gauss-horizon" })).toBe(true);
  });
  ...
  it("treats Nacos none selectors as the UI's unconfigured selector", () => {
    const expected = { serviceName: "api", metadata: { owner: "gauss-horizon" }, protectThreshold: 0.5 };
    expect(nacosServiceDetailMatches({ serviceName: "api", metadata: { owner: "gauss-horizon" }, protectThreshold: 0.5, selector: { type: "NoneSelector", contextType: "NONE" } }, expected)).toBe(true);
    expect(nacosServiceDetailMatches({ serviceName: "api", metadata: { owner: "gauss-horizon" } }, expected)).toBe(false);
  });
});
