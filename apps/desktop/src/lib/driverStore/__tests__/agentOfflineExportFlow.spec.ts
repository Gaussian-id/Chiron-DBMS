...
describe("runAgentOfflineExportFlow", () => {
  ...
  it("exports the selected drivers to the chosen destination", async () => {
    ...
    await expect(
      runAgentOfflineExportFlow({
        driverKeys,
        chooseDestination: vi.fn().mockResolvedValue("/tmp/chiron-horizon-agents.zip"),
        exportPackage,
      }),
    ).resolves.toEqual({
      destination: "/tmp/chiron-horizon-agents.zip",
      result: { driverCount: 2 },
    });
    expect(exportPackage).toHaveBeenCalledWith("/tmp/chiron-horizon-agents.zip", ["duckdb", "redis"]);
  ...
  it.each([
    ...
    ["package creation", vi.fn().mockResolvedValue("/tmp/chiron-horizon-agents.zip")],
  ...
  it("holds the busy state across destination selection and package creation", async () => {
    ...
    const options = createOptions({
      ...
      chooseDestination: vi.fn(async () => {
        events.push("choose");
        return "/tmp/chiron-horizon-agents.zip";
      }),
