...
describe("driver import selection", () => {
  it("recognizes ZIP and tar.zst packages case-insensitively", () => {
    expect(isOfflineDriverPackage("C:\\Downloads\\chiron-horizon-agent-h2-0.2.5.ZIP")).toBe(true);
    expect(isOfflineDriverPackage({ name: "chiron-horizon-agent-kingbase-0.1.34-macos-aarch64.zip" })).toBe(true);
    expect(isOfflineDriverPackage({ name: "chiron-horizon-agent-duckdb-0.1.0-macos-aarch64.TAR.ZST" })).toBe(true);
    expect(isOfflineDriverPackage({ name: "chiron-horizon-agent-h2-0.2.5.jar" })).toBe(false);
  });
