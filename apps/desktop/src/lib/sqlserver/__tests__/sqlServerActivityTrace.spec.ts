...
describe("SQL Server activity trace", () => {
  it("builds bounded, database-scoped XE sessions", () => {
    const sql = buildCreateSqlServerTraceSessionSql({
      sessionName: "GAUSS_HORIZON_TRACE_ABC_123",
      databaseId: 7,
      maxEvents: 99_999,
  ...
  it("rejects unsafe session names and escapes database literals", () => {
    expect(() => buildReadSqlServerTraceEventsSql("GAUSS_HORIZON_TRACE_X]; DROP TABLE x;--")).toThrow(/Invalid/);
  ...
  it("normalizes resource limits and generates safe names", () => {
    ...
    expect(buildSqlServerTraceSessionName(123456789, 0.25)).toMatch(/^GAUSS_HORIZON_TRACE_[A-Z0-9]+_[A-Z0-9]{8}$/);
    expect(sqlServerTraceSessionExpiresAt(buildSqlServerTraceSessionName(123456789, 0.25))).toBe(123456789);
  });

  it("identifies only expired Gauss Horizon sessions for cleanup", () => {
    ...
    expect(buildListSqlServerTraceSessionsSql()).toContain("GAUSS_HORIZON_TRACE_%");
  ...
  it("bounds each ring-buffer parse and reads incrementally from the timestamp cursor", () => {
    const initialSql = buildReadSqlServerTraceEventsSql("GAUSS_HORIZON_TRACE_ABC_123");
    const sql = buildReadSqlServerTraceEventsSql("GAUSS_HORIZON_TRACE_ABC_123", "2026-08-12T13:23:45.0840000");
  ...
  it("maps XE rows by column name, converts microseconds and removes internal events", () => {
    const events = parseSqlServerTraceEvents(
      result(
        ...
        [
          ["sql_batch_completed", "2026-08-12 13:23:45.084", "demo", "SELECT 1", 60_429, "6000", 8, 1, 1, 69, "sa", "host", "gauss-horizon", "OK"],
          ["sql_batch_completed", "2026-08-12 13:23:46.000", "demo", "/* GAUSS_HORIZON_INTERNAL_TRACE */ SELECT 1", 100, 0, 0, 0, 0, 70, "sa", "host", "gauss-horizon", "OK"],
        ],
      ),
    );
