...
describe("SQL Server activity trace runtime", () => {
  ...
  it("cleans persisted crash leftovers independently on the next connection", async () => {
    ...
    data.set(
      "chiron-horizon:sqlserver-trace:pending-sessions:v1",
      JSON.stringify([
    ...
    expect(JSON.parse(data.get("chiron-horizon:sqlserver-trace:pending-sessions:v1") || "[]")).toEqual([{ connectionId: "sqlserver-1", database: "app", sessionName: activeSessionName, expiresAt: activeExpiresAt }]);
  });
});
