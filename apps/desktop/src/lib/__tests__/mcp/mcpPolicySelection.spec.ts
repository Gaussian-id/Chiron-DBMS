...
const mcpServerSource = readFileSync(new URL("../../../../../../crates/chiron-horizon-mcp/src/server.rs", import.meta.url), "utf8");
...
describe("MCP tool permission selection", () => {
  it("lists every tool registered by the MCP server", () => {
    const registeredToolNames = [...mcpServerSource.matchAll(/name\s*=\s*"(chiron_horizon_[^"]+)"/g)].map((match) => match[1]).sort();
  ...
  it("keeps batch execution allowed when allow-all becomes an explicit allowlist", () => {
    const next = toggleMcpAllowedToolName(null, "chiron_horizon_send_message", false);

    expect(next).toContain("chiron_horizon_execute_batch");
    expect(next).not.toContain("chiron_horizon_send_message");
  });

  it("keeps Kafka reading independent from message sending", () => {
    const next = toggleMcpAllowedToolName(null, "chiron_horizon_send_message", false);
    expect(next).toContain("chiron_horizon_peek_messages");
    expect(toggleMcpAllowedToolName(next, "chiron_horizon_peek_messages", false)).not.toContain("chiron_horizon_peek_messages");
    expect(toggleMcpAllowedToolName([], "chiron_horizon_peek_messages", true)).toEqual(["chiron_horizon_peek_messages"]);
  });

  it("lets batch execution be enabled and disabled independently", () => {
    expect(toggleMcpAllowedToolName(["chiron_horizon_execute_query"], "chiron_horizon_execute_batch", true)).toEqual(["chiron_horizon_execute_query", "chiron_horizon_execute_batch"]);
    expect(toggleMcpAllowedToolName(["chiron_horizon_execute_query", "chiron_horizon_execute_batch"], "chiron_horizon_execute_batch", false)).toEqual(["chiron_horizon_execute_query"]);
    expect(MCP_TOOL_OPTIONS.find((tool) => tool.name === "chiron_horizon_execute_batch")?.labelKey).toBe("settings.mcpToolExecuteBatch");
