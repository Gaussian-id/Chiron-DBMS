...
import { createGaussHorizonCodeMirrorSqlDialect, postgresKeywordSyntaxTerms, sqlServerBuiltinSyntaxTerms } from "@/lib/editor/codemirrorSqlDialect";
...
describe("codemirrorSqlDialect", () => {
  ...
  it("highlights common functions dropped from Postgres/MySQL keyword lists as builtins", () => {
    const postgresBuiltins = new Set(createGaussHorizonCodeMirrorSqlDialect(langSql, "postgres", "postgres").spec.builtin?.split(/\s+/));
    const mysqlBuiltins = new Set(createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", "mysql").spec.builtin?.split(/\s+/));
  ...
  it("adds Dolt routines to highlighting without changing standard MySQL", () => {
    const doltBuiltins = new Set(createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", "mysql", "dolt").spec.builtin?.split(/\s+/));
    const mysqlBuiltins = new Set(createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", "mysql", "mysql").spec.builtin?.split(/\s+/));
  ...
  it("tokenizes SET as a keyword for SQL Server statements", () => {
    const dialect = createGaussHorizonCodeMirrorSqlDialect(langSql, "sqlserver", "sqlserver");
  ...
  it("tokenizes T-SQL temp table names instead of erroring on the hash (#8267)", () => {
    ...
    const mysql = createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", "mysql");
  ...
  it("keeps double quotes as identifier delimiters for Oracle-family dialects", () => {
    ...
    for (const databaseType of databaseTypes) {
      expect(createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", databaseType).spec.doubleQuotedStrings, databaseType).toBe(false);
    }
  });
  ...
  it("highlights core keywords for StandardSQL-based dialects (#8123)", () => {
    ...
    for (const databaseType of ["iris", "jdbc", "h2", "db2", "hive"] as DatabaseType[]) {
      const dialect = createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", databaseType);
    ...
    expect(createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", "iris").spec.types).toBe("array binary bit boolean char character clob date decimal double float int integer interval large national nchar nclob numeric object precision real smallint time timestamp varchar varying");
    const clickhouseDialect = createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", "clickhouse");
  ...
  it("enables backslashEscapes for MySQL-family dialects and ClickHouse while keeping it disabled for standard dialects", () => {
    ...
    for (const databaseType of backslashEscapesTypes) {
      expect(createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", databaseType).spec.backslashEscapes, databaseType).toBe(true);
    }

    const standardTypes: DatabaseType[] = ["postgres", "sqlserver", "sqlite", "oracle", "dameng"];
    for (const databaseType of standardTypes) {
      expect(createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", databaseType).spec.backslashEscapes, databaseType).toBeUndefined();
    }
  });

  it("correctly tokenizes string literals containing escaped quotes in MySQL statements without corrupting trailing code", () => {
    const dialect = createGaussHorizonCodeMirrorSqlDialect(langSql, "mysql", "mysql");
