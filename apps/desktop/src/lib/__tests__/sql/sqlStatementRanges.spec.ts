...
BEGIN
  ...
  FROM "GAUSS_HORIZON_TEST"."ORDERS_10K";

  IF v_order_count = 0 THEN
    INSERT INTO "GAUSS_HORIZON_TEST"."STORES"
      ("ID", "STORE_CODE", "STORE_NAME", "CITY", "OPENED_AT")
    SELECT 10001, 'TEST_STORE_001', '测试门店', '上海', SYSDATE
    FROM DUAL
    WHERE NOT EXISTS (
      SELECT 1 FROM "GAUSS_HORIZON_TEST"."STORES" WHERE "ID" = 10001
    );

    INSERT INTO "GAUSS_HORIZON_TEST"."PRODUCTS"
      ("ID", "SKU", "PRODUCT_NAME", "CATEGORY", "PRICE")
    SELECT 10001, 'TEST_SKU_001', '测试商品', '测试分类', 99.90
    FROM DUAL
    WHERE NOT EXISTS (
      SELECT 1 FROM "GAUSS_HORIZON_TEST"."PRODUCTS" WHERE "ID" = 10001
    );

    INSERT INTO "GAUSS_HORIZON_TEST"."ORDERS_10K"
    ...
    FROM DUAL
    WHERE NOT EXISTS (
      SELECT 1 FROM "GAUSS_HORIZON_TEST"."ORDERS_10K" WHERE "ORDER_NO" = 'TEST_ORDER_001'
    );
...
const oracleConsecutiveNestedBlocks = `CREATE OR REPLACE PROCEDURE gauss_horizon_consecutive_blocks AS
BEGIN
  BEGIN
    NULL;
  END;
...
const gaussDbNestedProcedure = `CREATE OR REPLACE PROCEDURE public.gauss_horizon_issue_4318()
AS
BEGIN
  BEGIN
    NULL;
...
const gaussDbDollarQuotedFunctionScript = `DROP FUNCTION IF EXISTS gauss_horizon_issue_4572_tmp_md5_uuid;

CREATE OR REPLACE FUNCTION gauss_horizon_issue_4572_tmp_md5_uuid (v_str IN TEXT) RETURNS varchar(36) LANGUAGE PLPGSQL IMMUTABLE AS $function$
DECLARE
    str1 TEXT;
BEGIN
    str1 := md5(v_str);
...
DROP FUNCTION IF EXISTS gauss_horizon_issue_4572_tmp_missing;`;
...
const xuguProgrammableObjectFixtures = [
  `CREATE OR REPLACE PROCEDURE gauss_horizon_xugu_procedure AS
  v_value INTEGER;
BEGIN
  v_value := 1;
END;`,
  `CREATE PROCEDURE gauss_horizon_xugu_procedure_without_replace AS
  v_value INTEGER;
BEGIN
  v_value := 1;
END;`,
  `CREATE OR REPLACE FUNCTION gauss_horizon_xugu_function RETURN INTEGER AS
BEGIN
  RETURN 1;
END;`,
  `CREATE FUNCTION gauss_horizon_xugu_function_without_replace RETURN INTEGER AS
BEGIN
  RETURN 1;
END;`,
  `CREATE OR REPLACE TRIGGER gauss_horizon_xugu_trigger
BEFORE INSERT ON gauss_horizon_xugu_events
FOR EACH ROW
BEGIN
  NULL;
END;`,
  `CREATE TRIGGER gauss_horizon_xugu_trigger_without_replace
BEFORE INSERT ON gauss_horizon_xugu_events
FOR EACH ROW
BEGIN
  NULL;
END;`,
  `CREATE OR REPLACE PACKAGE BODY gauss_horizon_xugu_package AS
  PROCEDURE ping AS
  BEGIN
    NULL;
  END ping;
END gauss_horizon_xugu_package;`,
  `CREATE PACKAGE BODY gauss_horizon_xugu_package_without_replace AS
  PROCEDURE ping AS
  BEGIN
    NULL;
  END ping;
END gauss_horizon_xugu_package_without_replace;`,
  `CREATE OR REPLACE FORCE PACKAGE BODY gauss_horizon_xugu_force_package AS
  PROCEDURE ping AS
  BEGIN
    NULL;
  END ping;
END gauss_horizon_xugu_force_package;`,
  `CREATE OR REPLACE NOFORCE PACKAGE BODY gauss_horizon_xugu_noforce_package AS
  PROCEDURE ping AS
  BEGIN
    NULL;
  END ping;
END gauss_horizon_xugu_noforce_package;`,
  `CREATE OR REPLACE TYPE BODY gauss_horizon_xugu_type AS
  MEMBER PROCEDURE ping IS
  BEGIN
    NULL;
  END;
END;`,
  `CREATE TYPE BODY gauss_horizon_xugu_type_without_replace AS
  MEMBER PROCEDURE ping IS
  BEGIN
    NULL;
  END;
...
describe("splitSqlStatementRanges", () => {
  ...
  it.each([
    ...
    const procedure = `CREATE OR REPLACE PROCEDURE gauss_horizon_delimited_scope AS
BEGIN
  BEGIN
    NULL;
  END;
...
END big_pkg;`;
  ...
  it("separates GaussDB dollar-quoted functions from surrounding statements", () => {
    ...
    expect(rangeSqlTexts(ranges)).toEqual([
      "DROP FUNCTION IF EXISTS gauss_horizon_issue_4572_tmp_md5_uuid",
...
END packageName;`;
  ...
  it("keeps Oracle-style CASE expressions inside Xugu and Oracle routines", () => {
    const routine = `CREATE OR REPLACE FUNCTION gauss_horizon_case_expr RETURN NUMBER AS
BEGIN
  RETURN CASE WHEN 1 = 1 THEN CASE WHEN 2 = 2 THEN 1 ELSE 2 END ELSE 0 END;
END;`;
    const caseStatementRoutine = `CREATE OR REPLACE PROCEDURE gauss_horizon_case_statement AS
BEGIN
