...
describe("editable query hidden primary keys", () => {
  it("appends quoted MySQL primary keys without changing the visible projection", () => {
    expect(
      ...
      sql: "SELECT name, `id` AS `__GAUSS_HORIZON_PK_0` FROM users WHERE active = 1",
      projections: [{ sourceName: "id", alias: "__GAUSS_HORIZON_PK_0" }],
    });
  });
  ...
  it("supports PostgreSQL composite primary keys and avoids alias collisions", () => {
    const result = buildQueryWithHiddenPrimaryKeys({
      ...
      existingResultNames: ["value", "__GAUSS_HORIZON_PK_0"],
    });

    expect(result?.sql).toBe('SELECT "value", "tenant_id" AS "__GAUSS_HORIZON_PK_1", "item_id" AS "__GAUSS_HORIZON_PK_2"\nFROM "items"');
    expect(result?.projections.map((projection) => projection.alias)).toEqual(["__GAUSS_HORIZON_PK_1", "__GAUSS_HORIZON_PK_2"]);
  });
  ...
  it("preserves SQL Server TOP and Oracle optimizer hints", () => {
    ...
    ).toBe("SELECT TOP 10 name, [id] AS [__GAUSS_HORIZON_PK_0] FROM dbo.users ORDER BY name");
    ...
    ).toBe('SELECT /*+ INDEX(t IDX_USERS_NAME) */ t.NAME, "ID" AS "__GAUSS_HORIZON_PK_0"\nFROM USERS t');
  });
  ...
  it("supports an Oracle ROWID expression for keyless base tables", () => {
    expect(
      buildQueryWithHiddenPrimaryKeys({
        ...
        sourceExpressions: { __GAUSS_HORIZON_ROWID: "ROWIDTOCHAR(ROWID)" },
      }),
    ).toEqual({
      sql: 'SELECT t.*, ROWIDTOCHAR(ROWID) AS "__GAUSS_HORIZON_PK_0" FROM APP.USERS t WHERE t.ACTIVE = 1',
      projections: [{ sourceName: "__GAUSS_HORIZON_ROWID", alias: "__GAUSS_HORIZON_PK_0" }],
    });
  });
  ...
  it("supports Xugu's unqualified ROWID expression for keyless base tables", () => {
    expect(
      buildQueryWithHiddenPrimaryKeys({
        ...
        primaryKeys: ["__GAUSS_HORIZON_ROWID"],
        existingResultNames: ["ID", "NAME"],
        sourceExpressions: { __GAUSS_HORIZON_ROWID: "ROWID" },
      }),
    ).toEqual({
      sql: 'SELECT *, ROWID AS "__GAUSS_HORIZON_PK_0" FROM APP.USERS t WHERE t.ACTIVE = 1',
      projections: [{ sourceName: "__GAUSS_HORIZON_ROWID", alias: "__GAUSS_HORIZON_PK_0" }],
    });
  });
  ...
  it("preserves an Oracle FOR UPDATE clause when appending a hidden row key", () => {
    ...
    ).toBe('SELECT USERS.*, ROWIDTOCHAR(ROWID) AS "__GAUSS_HORIZON_PK_0" FROM APP.USERS FOR UPDATE SKIP LOCKED');
  });
  ...
  it("qualifies a bare Oracle wildcard when appending a hidden key", () => {
    ...
    ).toBe('SELECT /*+ FULL("Users") */ "Users".*, ROWIDTOCHAR(ROWID) AS "__GAUSS_HORIZON_PK_0" FROM APP."Users"');
    ...
    ).toBe('SELECT "u".*, ROWIDTOCHAR(ROWID) AS "__GAUSS_HORIZON_PK_0" FROM APP.USERS "u"');
  });
  ...
  it("rewrites the reported Oracle queries without changing their filters", () => {
    ...
    ).toBe('select t_zyys_vte_yyfxbd.*, ROWIDTOCHAR(ROWID) AS "__GAUSS_HORIZON_PK_0" from t_zyys_vte_yyfxbd');
    ...
    ).toBe(`SELECT T_XT_MB.*, ROWIDTOCHAR(ROWID) AS "__GAUSS_HORIZON_PK_0" from T_XT_MB WHERE   mbmc ='结束时'`);
  });
  ...
  it("preserves a WHERE subquery when adding an Oracle ROWID", () => {
    ...
    ).toBe('SELECT t.*, ROWIDTOCHAR(ROWID) AS "__GAUSS_HORIZON_PK_0" FROM APP.PLATFORM_CARS t WHERE t.CUSTOMER_NO IN (SELECT c.CUSTOMER_NO FROM APP.CUSTOMERS c WHERE c.ENABLED = 1)');
  });
  ...
  it("inserts hidden keys before a trailing line comment", () => {
    ...
    ).toBe("SELECT name, `id` AS `__GAUSS_HORIZON_PK_0` -- visible user name\nFROM users");
  });
  ...
  it("maps a qualified Oracle ROWID projection to the synthetic row key", () => {
    ...
    expect(allPrimaryKeysPresent(["__GAUSS_HORIZON_ROWID"], ["ROWID", "NAME", "LABEL"], analysis, "t:0", "oracle")).toBe(true);
    expect(sourceColumnsForResult(analysis, ["ROWID", "NAME", "LABEL"], "t:0", "oracle", ["__GAUSS_HORIZON_ROWID"])).toEqual(["__GAUSS_HORIZON_ROWID", "NAME", undefined]);
    expect(allPrimaryKeysPresent(["__GAUSS_HORIZON_ROWID"], ["ROWID", "NAME", "LABEL"], analysis, "o:1", "oracle")).toBe(false);
  });
  ...
  it("does not assign an unqualified multi-source ROWID to a target table", () => {
    ...
    expect(allPrimaryKeysPresent(["__GAUSS_HORIZON_ROWID"], ["ROWID"], analysis, "t:0", "oracle")).toBe(false);
    expect(sourceColumnsForResult(analysis, ["ROWID"], "t:0", "oracle", ["__GAUSS_HORIZON_ROWID"])).toEqual([undefined]);
  });

  it("resolves appended aliases to result indexes", () => {
    expect(
      hiddenResultColumnIndexes(
        ["name", "__GAUSS_HORIZON_PK_0", "__GAUSS_HORIZON_PK_1"],
        [
          { sourceName: "tenant_id", alias: "__GAUSS_HORIZON_PK_0" },
          { sourceName: "item_id", alias: "__GAUSS_HORIZON_PK_1" },
        ],
      ),
    ).toEqual([1, 2]);
    expect(hiddenResultColumnIndexes(["name"], [{ sourceName: "id", alias: "__GAUSS_HORIZON_PK_0" }])).toEqual([]);
    expect(
      hiddenResultColumnIndexes(
        ["name", "__GAUSS_HORIZON_PK_1"],
        [
          { sourceName: "tenant_id", alias: "__GAUSS_HORIZON_PK_0" },
          { sourceName: "item_id", alias: "__GAUSS_HORIZON_PK_1" },
        ],
