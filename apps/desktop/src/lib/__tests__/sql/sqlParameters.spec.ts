...
describe("substituteSqlParameters", () => {
  ...
  it("supports embedded placeholders in the issue reproduction", () => {
    const sql = "INSERT INTO ${dbSchema}.gauss_horizon_smoke (note) VALUES ('${FOO} Gauss Horizon smoke 中文 🚀')";
    expect(
      substituteSqlParameters(sql, {
        dbSchema: { kind: "raw", value: "public" },
        FOO: { kind: "string", value: "O'Reilly" },
      }),
    ).toBe("INSERT INTO public.gauss_horizon_smoke (note) VALUES ('O''Reilly Gauss Horizon smoke 中文 🚀')");
  });
