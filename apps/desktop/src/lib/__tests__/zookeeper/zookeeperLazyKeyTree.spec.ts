...
describe("zookeeper lazy key tree", () => {
  ...
  it("keeps an exact focused Key without children as a leaf", () => {
    ...
    replaceLazyKvFocusedRoot(state, { key: "gauss-horizon-demo/locks/persistent", numChildren: 0, valueSize: 12, hasValue: true }, [], null);

    const leaf = state.nodeByKey.get("gauss-horizon-demo/locks/persistent");
