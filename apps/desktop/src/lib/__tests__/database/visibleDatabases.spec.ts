...
describe("visibleDatabases schema filtering", () => {
  ...
  it("keeps Oracle DIP visible while hiding default system schemas", () => {
    expect(filterSchemaNamesForConnection(["CHIRON_HORIZON_TEST", "DIP", "SYSTEM"], { db_type: "oracle", database: "XE" }, "XE")).toEqual(["CHIRON_HORIZON_TEST", "DIP"]);
  });
  ...
  it("uses Oracle schema filtering for inferred JDBC connections", () => {
    ...
    expect(filterSchemaNamesForConnection(["ANONYMOUS", "CHIRON_HORIZON_TEST", "SYS", "SYSTEM"], connection, "")).toEqual(["CHIRON_HORIZON_TEST"]);
  });
