...
describe("jdbc dialect inference", () => {
  ...
  it("detects TDengine JDBC connections and keeps the selected database in the object tree", () => {
    ...
    expect(connectionObjectTreeQuerySchema(connection, "gauss_horizon_test")).toBe("gauss_horizon_test");
  });
  ...
  it.each([
    ...
    expect(connectionUsesDatabaseObjectTreeMode(connection)).toBe(false);
    expect(connectionObjectTreeQuerySchema(connection, "GAUSS_HORIZON_TEST", "GAUSS_HORIZON_TEST")).toBe("GAUSS_HORIZON_TEST");
    expect(connectionObjectTreeNodeSchema(connection, "GAUSS_HORIZON_TEST", "GAUSS_HORIZON_TEST")).toBe("GAUSS_HORIZON_TEST");
  });
  ...
  it("detects GBase JDBC connections for transfer dialect selection", () => {
    const gbaseConnection = {
      db_type: "jdbc" as const,
      connection_string: "jdbc:gbase://localhost:5258/gauss_horizon_test",
    ...
    const gbase8sByUrl = {
      db_type: "jdbc" as const,
      connection_string: "jdbc:gbasedbt-sqli://localhost:9088/gauss_horizon_test:INFORMIXSERVER=ol_gbasedbt",
  ...
  it("resolves transfer types for non-doris-family connections through the effective type", () => {
    ...
    expect(transferDatabaseTypeForConnection({ db_type: "jdbc", connection_string: "jdbc:gbase://localhost:5258/gauss_horizon_test" })).toBe("gbase");
    ...
    expect(transferDatabaseTypeForConnection({ db_type: "jdbc", connection_string: "jdbc:doris://localhost:9030/gauss_horizon_test" })).toBe("jdbc");
  });
...
describe("object tree node schema", () => {
  ...
  it("never uses the database name as the schema when a schema-tree engine has no schema yet", () => {
    ...
    // `schema || database` sends "gauss_horizon_test" as the schema and matches nothing —
    ...
    for (const dbType of ["postgres", "gaussdb", "opengauss", "kingbase", "redshift", "duckdb"] as const) {
      expect(connectionObjectTreeQuerySchema({ db_type: dbType }, "gauss_horizon_test")).toBe("");
      expect(connectionObjectTreeNodeSchema({ db_type: dbType }, "gauss_horizon_test")).toBeUndefined();
      expect(metadataSchemaForConnection({ db_type: dbType }, "gauss_horizon_test")).toBe("");
      // An explicit schema still wins, and the database name is a legitimate
      // schema name when the user really did select it.
      expect(connectionObjectTreeQuerySchema({ db_type: dbType }, "gauss_horizon_test", "public")).toBe("public");
      expect(connectionObjectTreeNodeSchema({ db_type: dbType }, "gauss_horizon_test", "public")).toBe("public");
    }
    // SQL Server keeps its own dbo default for metadata lookups.
    expect(connectionObjectTreeQuerySchema({ db_type: "sqlserver" }, "gauss_horizon_test")).toBe("");
    expect(connectionObjectTreeNodeSchema({ db_type: "sqlserver" }, "gauss_horizon_test")).toBeUndefined();
    expect(metadataSchemaForConnection({ db_type: "sqlserver" }, "gauss_horizon_test")).toBe("dbo");
  });
  ...
  it("keeps the database-as-schema fallback for engines whose database is the schema", () => {
    ...
    expect(connectionObjectTreeNodeSchema({ db_type: "oracle" }, "GAUSS_HORIZON_TEST")).toBe("GAUSS_HORIZON_TEST");
    expect(connectionObjectTreeQuerySchema({ db_type: "oracle" }, "GAUSS_HORIZON_TEST")).toBe("GAUSS_HORIZON_TEST");
    expect(connectionObjectTreeNodeSchema({ db_type: "dameng" }, "GAUSS_HORIZON_TEST")).toBe("GAUSS_HORIZON_TEST");
