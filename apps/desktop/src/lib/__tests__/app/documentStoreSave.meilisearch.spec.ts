...
describe("Meilisearch document saves", () => {
  it("preserves the distinction between string and numeric document ids", () => {
    expect(serializeDocumentStoreId("001", "meilisearch")).toBe('__chiron_horizon_meilisearch_string_id__"001"');
  ...
  it("describes Chiron Horizon write semantics without inventing a native primary-key field", () => {
    ...
    expect(preview).toContain("Chiron Horizon MEILISEARCH UPDATE DOCUMENT");
  ...
  it("writes the new identity before deleting the old document", async () => {
    ...
    expect(calls).toEqual(['update:__chiron_horizon_meilisearch_string_id__"002"', 'delete:__chiron_horizon_meilisearch_string_id__"001"']);
    expect(update).toHaveBeenCalledWith('__chiron_horizon_meilisearch_string_id__"002"', '{"title":"Arrival"}', undefined);
  });
});
