...
describe("tunnelProfileSummary", () => {
  ...
  it("returns the url for http tunnel profiles", () => {
    const http = { ...createTunnelProfile("http_tunnel"), url: "https://example.com/chiron_horizon_tunnel.php" } as TunnelProfile;
    expect(tunnelProfileSummary(http)).toBe("https://example.com/chiron_horizon_tunnel.php");
  });
