...
describe("SQL completion theme", () => {
  it("uses the configurable medium radius for the popup container", () => {
    ...
    expect(rules[".cm-tooltip.cm-tooltip-autocomplete"]).toMatchObject({ borderRadius: "var(--gauss-horizon-radius-md)" });
    expect(rules[".cm-tooltip.cm-tooltip-autocomplete > ul > li"]).toMatchObject({ borderRadius: "var(--gauss-horizon-radius-sm)" });
  });
...
describe("SQL builtin highlight tag", () => {
  ...
  it("gives builtin SQL functions their own highlight class, distinct from plain identifiers and keywords", async () => {
    ...
    const dialect = createGaussHorizonCodeMirrorSqlDialect(langSql, "postgres", "postgres");
...
describe("editor font theme", () => {
  it("disables ligatures on the editor content so repainted character runs stay stable", () => {
    ...
    // CodeMirror's per-keystroke span patching (gauss-horizon#7900); dropping either
