...
describe("looksLikeWriteSqlProposal", () => {
  it("detects the backend-generated write confirmation proposal", () => {
    const proposal = ["This database change requires your explicit confirmation before Gauss Horizon can execute it.", "", "```sql", "CREATE TABLE users (id INT);", "```", "", "Should I execute this SQL statement?"].join("\n");
...
describe("shouldGrantWriteSqlOnShortAffirmative (manual-typing confirmation)", () => {
  ...
  it("grants for a backend-generated localized confirmation via its structural kind", () => {
    // The exact wording Gauss Horizon renders from i18n (en): it does NOT contain an
