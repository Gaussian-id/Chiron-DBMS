...
describe("tableEditing", () => {
  it("synthesizes ROWID only for Oracle-compatible base tables", () => {
    ...
    expect(editablePrimaryKeys("oracle", [column("ID"), column("NAME")], "TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
    expect(editablePrimaryKeys("oceanbase-oracle", [column("ID"), column("NAME")], "TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
  ...
  it("uses Xugu ROWID for ordinary, partitioned, and temporary tables but not views", () => {
    ...
    expect(editablePrimaryKeys("xugu", columns, "TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
    expect(editablePrimaryKeys("xugu", columns, "PARTITIONED TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
    expect(editablePrimaryKeys("xugu", columns, "TEMPORARY TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
    expect(editablePrimaryKeys("xugu", columns, "VIEW")).toEqual([]);
    expect(usesSyntheticRowIdKey("xugu", [GAUSS_HORIZON_ROWID_COLUMN], "TABLE")).toBe(true);
    expect(usesSyntheticRowIdKey("xugu", [GAUSS_HORIZON_ROWID_COLUMN], "PARTITIONED TABLE")).toBe(true);
    expect(usesSyntheticRowIdKey("xugu", [GAUSS_HORIZON_ROWID_COLUMN], "TEMPORARY TABLE")).toBe(true);
    expect(usesSyntheticRowIdKey("xugu", [GAUSS_HORIZON_ROWID_COLUMN], "VIEW")).toBe(false);
    expect(isTableDataEditable("xugu", [GAUSS_HORIZON_ROWID_COLUMN], "TABLE")).toBe(true);
    expect(isTableDataEditable("xugu", [GAUSS_HORIZON_ROWID_COLUMN], "VIEW")).toBe(false);
  ...
  it("treats view data tabs as readonly", () => {
    expect(isTableDataEditable("oracle", [GAUSS_HORIZON_ROWID_COLUMN], "VIEW")).toBe(false);
  });
  ...
  it("does not include Oracle ROWID for view data tabs", () => {
    expect(usesSyntheticRowIdKey("oracle", [GAUSS_HORIZON_ROWID_COLUMN])).toBe(true);
    expect(shouldIncludeSyntheticRowId("oracle", [GAUSS_HORIZON_ROWID_COLUMN])).toBe(false);
    expect(shouldIncludeSyntheticRowId("oracle", [GAUSS_HORIZON_ROWID_COLUMN], "TABLE")).toBe(true);
    expect(usesSyntheticRowIdKey("oracle", [GAUSS_HORIZON_ROWID_COLUMN], "VIEW")).toBe(false);
    expect(usesSyntheticRowIdKey("oracle", [GAUSS_HORIZON_ROWID_COLUMN], "MATERIALIZED_VIEW")).toBe(false);
    expect(usesSyntheticRowIdKey("oceanbase-oracle", [GAUSS_HORIZON_ROWID_COLUMN], "TABLE")).toBe(true);
    expect(usesSyntheticRowIdKey("oceanbase-oracle", [GAUSS_HORIZON_ROWID_COLUMN], "VIEW")).toBe(false);
  });
  ...
  it.each(["oracle", "oceanbase-oracle"] as const)("prefers physical %s indexes over the ROWID fallback", (databaseType) => {
    ...
    expect(editableRowIdentifierColumns(databaseType, columns, [index(["ORI_OFFER_ID"], false)], "TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
    expect(editableRowIdentifierColumns(databaseType, columns, [index(["ORI_OFFER_ID"], true, "ORI_OFFER_ID IS NOT NULL")], "TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
    expect(editableRowIdentifierColumns(databaseType, columns, [], "TABLE")).toEqual([GAUSS_HORIZON_ROWID_COLUMN]);
  });
  ...
  it("keeps synthetic row identifiers scoped to their existing fallbacks", () => {
    ...
    expect(editableRowIdentifierColumns("neo4j", columns, [index(["ID"])], "TABLE")).toEqual([GAUSS_HORIZON_NEO4J_ELEMENT_ID_COLUMN]);
  });
  ...
  it("uses tbname only when editing TDengine stable rows", () => {
    ...
    expect(editablePrimaryKeys("tdengine", columns, "STABLE")).toEqual([GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN, "ts", "seq"]);
  ...
  it("requires every TDengine row identifier in editable results", () => {
    const stableKeys = [GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN, "ts", "seq"];
  ...
  it("disables existing-row deletion for TDengine composite keys", () => {
    ...
    expect(canDeleteExistingTdengineRows("tdengine", [GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN, "ts"])).toBe(true);
    expect(canDeleteExistingTdengineRows("tdengine", ["ts", "seq"])).toBe(false);
    expect(canDeleteExistingTdengineRows("tdengine", [GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN, "ts", "seq"])).toBe(false);
