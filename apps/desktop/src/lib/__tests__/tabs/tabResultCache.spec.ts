...
describe("tab result cache statement execution metadata", () => {
  ...
  it("uses explicit browser runtime configuration", () => {
    expect(resultCacheRuntimeConfig(false, { VITE_CHIRON_HORIZON_RESULT_CACHE_BACKEND: "http", VITE_CHIRON_HORIZON_RESULT_CACHE_FALLBACK: "true" } as ImportMetaEnv)).toEqual({
      primary: "runtime",
