...
describe("tokenizeDataGridCondition", () => {
  it("colors keywords, fields and values in a where condition", () => {
    expect(pairs("name = 'gauss-horizon' and age >= 18")).toEqual(["field:name", "plain: = ", "value:'gauss-horizon'", "plain: ", "keyword:and", "plain: ", "field:age", "plain: >= ", "value:18"]);
  });
  ...
  it("preserves the full text across tokens", () => {
    const text = "name = 'gauss-horizon' and (age >= 18 or city like 'sh%') order by id desc";
