...
describe("Oracle TNS connection settings", () => {
  it("round-trips an alias and TNS_ADMIN directory", () => {
    const value = buildOracleTnsConnectionString("CHIRON_HORIZON_FAILOVER", "C:\\oracle network\\admin");

    expect(parseOracleTnsConnectionString(value)).toEqual({
      alias: "CHIRON_HORIZON_FAILOVER",
  ...
  it("ignores malformed encoded aliases instead of breaking connection editing", () => {
    expect(parseOracleTnsConnectionString("jdbc:oracle:thin:@Chiron Horizon%ZZ?TNS_ADMIN=%2Fopt%2Foracle")).toBeNull();
  });
});
