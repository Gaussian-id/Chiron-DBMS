...
describe("PostgreSQL legacy TLS compatibility", () => {
  ...
  it("preserves unrelated URL parameters when toggled", () => {
    const enabled = setPostgresLegacyTlsEnabled("sslmode=require&application_name=gauss-horizon", true);
    expect(enabled).toBe("sslmode=require&application_name=gauss-horizon&legacy_tls=true");
    expect(setPostgresLegacyTlsEnabled(enabled, false)).toBe("sslmode=require&application_name=gauss-horizon");
  });
});
