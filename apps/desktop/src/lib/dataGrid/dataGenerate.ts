...
export interface GeneratedSqlExpression {
  __gaussHorizonGeneratedSqlExpression: true;
  sql: string;
  display: string;
}
...
export function isGeneratedSqlExpression(value: unknown): value is GeneratedSqlExpression {
  return !!value && typeof value === "object" && (value as GeneratedSqlExpression).__gaussHorizonGeneratedSqlExpression === true;
}

function generatedSqlExpression(sql: string, display = sql): GeneratedSqlExpression {
  return { __gaussHorizonGeneratedSqlExpression: true, sql, display };
}
...
function ensureRange(start: Date, end: Date): [Date, Date] {
  if (start.getTime() > end.getTime()) {
    console.warn("[gauss-horizon] date range reversed, swapping start/end", start, end);
    return [end, start];
  }
  return [start, end];
}
...
export function generateValue(columnName: string, dataType: string, generatorKey: string | undefined, rowIndex: number, params?: GeneratorParams, columnDefault?: string | null): unknown {
  ...
  if ((key === "date" || key === "datetime") && rowIndex === 0) {
    console.log("[gauss-horizon:gen] col=%s key=%s start=%j end=%j allDay=%j", columnName, key, params?.start, params?.end, params?.allDay);
  }
...
function generateTdengineChildTableName(): string {
  ...
  return `gauss_horizon_gen_${Date.now().toString(36)}_${randomSuffix}`;
}
