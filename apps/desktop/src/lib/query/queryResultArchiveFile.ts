...
const ARCHIVE_MIME_TYPE = "application/vnd.chiron.horizon.results";
const ARCHIVE_EXTENSIONS = ["chiron-horizonresults"];
...
function openArchiveFileInBrowser(): Promise<Uint8Array | undefined> {
  return new Promise((resolve, reject) => {
    ...
    input.type = "file";
    input.accept = ".chiron-horizonresults,application/octet-stream,application/vnd.chiron.horizon.results";
...
export async function saveQueryResultArchiveFile(fileName: string, bytes: Uint8Array): Promise<string | undefined> {
  ...
  const path = await save({
    ...
    filters: [{ name: "Chiron Horizon Result Archive", extensions: ARCHIVE_EXTENSIONS }],
  });
