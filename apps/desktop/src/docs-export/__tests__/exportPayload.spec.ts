...
describe("readPayload", () => {
  ...
  it("names the missing element rather than throwing something opaque", () => {
    document.body.innerHTML = `<div id="app"></div>`;
    expect(() => readPayload()).toThrow(/application\/gauss-horizon-snapshot/);
  });

  it("throws on a payload that is not base64", () => {
    document.body.innerHTML = `<div id="app"></div><script type="application/gauss-horizon-snapshot">not base64 at all!</script>`;
    ...
    expect(() => readPayload()).not.toThrow(/application\/gauss-horizon-snapshot/);
  });
});
