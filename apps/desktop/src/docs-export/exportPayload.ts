 ...
 * `<script type="application/chiron-horizon-snapshot">`. Nothing else in the emitted
...
export function readPayload(): ExportPayload {
  const node = document.querySelector("script[type='application/chiron-horizon-snapshot']");
  if (node === null) throw new Error("no <script type='application/chiron-horizon-snapshot'> in this document");
