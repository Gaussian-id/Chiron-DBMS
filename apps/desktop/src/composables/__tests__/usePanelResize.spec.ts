...
describe("usePanelResize", () => {
  ...
  it("lets the AI panel consume the available editor width beyond the legacy 800px cap", () => {
    localStorage.setItem("chiron-horizon-ai-panel-width", "360");
    ...
    expect(localStorage.getItem("chiron-horizon-ai-panel-width")).toBe("1060");
  });

  it("resizes from the flex-shrunk width after a wide panel is restored in a narrow window", () => {
    localStorage.setItem("chiron-horizon-ai-panel-width", "1060");
    ...
    expect(localStorage.getItem("chiron-horizon-ai-panel-width")).toBe("600");
  });
  ...
  it("persists the collapsed vertical tab bar state across composable instances", () => {
    ...
    expect(localStorage.getItem("chiron-horizon-tab-bar-collapsed")).toBe("true");

    const restored = usePanelResize();
    expect(restored.tabBarCollapsed.value).toBe(true);

    restored.setTabBarCollapsed(false);
    expect(localStorage.getItem("chiron-horizon-tab-bar-collapsed")).toBe("false");
  });
});
