...
describe("useDataGridActions", () => {
  ...
  it.each(["TABLE", "VIEW", "MATERIALIZED_VIEW"])("preserves Oracle synthetic projection only for eligible tables (%s)", async (tableType) => {
    ...
    tab.tableMeta!.primaryKeys = tableType === "TABLE" ? ["__CHIRON_HORIZON_ROWID"] : [];
    mocks.tabs.push(tab);
    ...
    expect(mocks.buildTableSelectSql).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        primaryKeys: tableType === "TABLE" ? ["__CHIRON_HORIZON_ROWID"] : [],
      }),
    );
    expect(tab.tableMetaPending).toBe(true);
    resolveIndexes([]);
    await vi.waitFor(() => expect(tab.tableMetaPending).toBe(false));
    expect(tab.tableMeta?.primaryKeys).toEqual(tableType === "TABLE" ? ["__CHIRON_HORIZON_ROWID"] : []);
  });
  ...
  it("excludes hidden primary keys and remaps the selected column for database sorting", async () => {
    const tab = {
      ...
      result: {
        columns: ["name", "__CHIRON_HORIZON_PK_0", "email"],
  ...
  it("advances Elasticsearch cursors when jumping across pages", async () => {
    ...
    const tab = reactive({
      ...
      database: "",
      title: "Query",
      sql: "SELECT * FROM `chiron-horizon-app-logs-v1` AS dalv",
      resultBaseSql: "SELECT * FROM `chiron-horizon-app-logs-v1` AS dalv",
      resultPageLimit: 100,
      resultPageOffset: 0,
