...
describe("useMcpUpdateBadge", () => {
  ...
  it("更新后通过事件 payload 清除红点", () => {
    ...
    expect(badge.mcpUpdateAvailable.value).toBe(true);
    badge.handleMcpStatusChanged(new CustomEvent("gauss-horizon:mcp-status-changed", { detail: { updateAvailable: false } }));
    expect(badge.mcpUpdateAvailable.value).toBe(false);
  });
  ...
  it("忽略早于当前根组件请求的设置页事件", async () => {
    ...
    badge.handleMcpStatusChanged(new CustomEvent("gauss-horizon-mcp-status-changed", { detail: { updateAvailable: true, requestId: settingsRequestId } }));
    expect(badge.mcpUpdateAvailable.value).toBe(false);
  ...
  it("无 payload 的事件回退到重新检查", async () => {
    mockedCheck.mockResolvedValue(makeStatus(true));
    const badge = makeBadge(true);
    badge.handleMcpStatusChanged(new CustomEvent("gauss-horizon:mcp-status-changed"));
