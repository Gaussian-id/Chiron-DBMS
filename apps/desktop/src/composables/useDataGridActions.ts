...
export function useDataGridActions(activeTab: ComputedRef<QueryTab | undefined>) {
  ...
  async function refreshDataTabTableMeta(tab: QueryTab, options: { force?: boolean; columnsOnly?: boolean; trace?: { traceId: string; elapsed: () => string } } = {}): Promise<boolean> {
    ...
    console.info("[Chiron Horizon][reloadData:metadata:get-columns:start]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), schema: querySchema, table: target.tableName, columnsOnly: options.columnsOnly === true });
    ...
    if (tab.mode === "data" && tableMetaForDataTab(tab)) {
      ...
      try {
        ...
        if (lifecycleStale || intent === "refresh") {
          tab.tableMetaPending = true;
          console.info("[Chiron Horizon][reloadData:metadata:await:start]", { traceId, elapsed: elapsed(), reason: intent === "refresh" ? "manual-refresh" : "lifecycle-stale", metadataAgeMs });
          try {
            if (intent === "refresh") {
              const meta = tableMetaForDataTab(tab)!;
              const config = connectionStore.getConfig(tab.connectionId);
              ...
              console.info("[Chiron Horizon][reloadData:object-cache-invalidate:start]", { traceId, elapsed: elapsed() });
              invalidateObjectMetadataCache(match).catch((e: unknown) => {
                console.warn("[Chiron Horizon][reloadData:object-cache-invalidate:error]", { traceId, elapsed: elapsed(), error: e });
              });
          ...
          if (intent === "refresh" && !lifecycleStale) {
            void refreshDataTabTableMeta(tab, { force: false, trace: { traceId, elapsed } })
              .then(() => {
                console.info("[Chiron Horizon][reloadData:metadata:background-indexes:done]", { traceId, elapsed: elapsed() });
              })
              .catch((e: any) => {
                console.warn("[Chiron Horizon][reloadData:metadata:background-indexes:error]", { traceId, elapsed: elapsed(), error: e });
              });
          }
