...
export function useDataGridActions(activeTab: ComputedRef<QueryTab | undefined>) {
  ...
  async function refreshDataTabTableMeta(tab: QueryTab, options: { force?: boolean; trace?: { traceId: string; elapsed: () => string } } = {}): Promise<boolean> {
    ...
    console.info("[Gauss Horizon][reloadData:metadata:ensure-connected:start]", { traceId: trace?.traceId, elapsed: trace?.elapsed() });
    await connectionStore.ensureConnected(target.connectionId);
    console.info("[Gauss Horizon][reloadData:metadata:ensure-connected:done]", { traceId: trace?.traceId, elapsed: trace?.elapsed() });
    if (connectionStore.metadataGenerationFor(target.connectionId, target.database) !== metadataGenerationAtStart) {
      console.info("[Gauss Horizon][reloadData:metadata:superseded-by-connection-generation]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), table: target.tableName });
      return false;
    }
    const config = connectionStore.getConfig(target.connectionId);
    const querySchema = metadataSchemaForConnection(config, target.database, target.schema);
    console.info("[Gauss Horizon][reloadData:metadata:get-columns:start]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), schema: querySchema, table: target.tableName });
    ...
    console.info("[Gauss Horizon][reloadData:metadata:get-columns:done]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), columnCount: columns.length });
    if (connectionStore.metadataGenerationFor(target.connectionId, target.database) !== metadataGenerationAtStart) {
      console.info("[Gauss Horizon][reloadData:metadata:superseded-by-connection-generation]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), table: target.tableName });
      return false;
    }
    ...
    if (!current || current.mode !== "data" || current.connectionId !== target.connectionId || currentSourceDatabase !== target.database || currentMeta?.tableName !== target.tableName || (currentMeta.schema ?? "") !== (target.schema ?? "") || (currentMeta.catalog ?? "") !== (target.catalog ?? "")) {
      console.info("[Gauss Horizon][reloadData:metadata:stale-tab]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), table: target.tableName });
      return false;
    }
  ...
  async function onReloadData(tabId: string | undefined, sql?: string, _searchText?: string, whereInput?: string, orderBy?: string, limit?: number, offset?: number, intent?: DataGridReloadIntent) {
    ...
    if (tab.mode === "data" && tableMetaForDataTab(tab)) {
      ...
      console.info("[Gauss Horizon][reloadData:start]", {
        traceId,
        tabId: tab.id,
        connectionId: tab.connectionId,
      ...
      if (hasRealTableMetaColumns) {
        try {
          console.info("[Gauss Horizon][reloadData:ensure-connected:start]", { traceId, elapsed: elapsed() });
          await connectionStore.ensureConnected(tab.connectionId);
          console.info("[Gauss Horizon][reloadData:ensure-connected:done]", { traceId, elapsed: elapsed() });
        } catch (e: any) {
          console.warn("[Gauss Horizon][reloadData:ensure-connected:error]", { traceId, elapsed: elapsed(), error: e });
      ...
      const startMetadataRefresh = () => {
        console.info("[Gauss Horizon][reloadData:metadata:background:start]", { traceId, elapsed: elapsed(), reason: hasRealTableMetaColumns ? "stale" : "missing", metadataAgeMs });
        void refreshDataTabTableMeta(tab, { force: lifecycleStale, trace: { traceId, elapsed } })
          .then(() => {
            console.info("[Gauss Horizon][reloadData:metadata:background:done]", { traceId, elapsed: elapsed() });
          })
          .catch((e: any) => {
            console.warn("[Gauss Horizon][reloadData:metadata:background:error]", { traceId, elapsed: elapsed(), error: e });
            toast(e?.message || String(e), 5000);
          });
      };
      if (lifecycleStale) {
        if (!hasRealTableMetaColumns) tab.tableMetaPending = true;
        console.info("[Gauss Horizon][reloadData:metadata:await:start]", { traceId, elapsed: elapsed(), reason: hasRealTableMetaColumns ? "lifecycle-stale" : "missing", metadataAgeMs });
        try {
          const rebuilt = await refreshDataTabTableMeta(tab, { force: true, trace: { traceId, elapsed } });
          console.info("[Gauss Horizon][reloadData:metadata:await:done]", { traceId, elapsed: elapsed(), rebuilt });
          if (!rebuilt) {
            queryStore.setExecuting(tab.id, false);
            return;
          }
        } catch (e: any) {
          console.warn("[Gauss Horizon][reloadData:metadata:await:error]", { traceId, elapsed: elapsed(), error: e });
          queryStore.setExecuting(tab.id, false);
          toast(e?.message || String(e), 5000);
          throw e;
        }
        ...
        console.info("[Gauss Horizon][reloadData:metadata:skip]", { traceId, elapsed: elapsed(), columnCount: tab.tableMeta!.columns.length, metadataAgeMs });
      }
      try {
        console.info("[Gauss Horizon][reloadData:build-sql:start]", { traceId, elapsed: elapsed() });
        ...
        console.info("[Gauss Horizon][reloadData:build-sql:done]", { traceId, elapsed: elapsed() });
        queryStore.updateSql(tab.id, nextSql);
        console.info("[Gauss Horizon][reloadData:execute:start]", { traceId, elapsed: elapsed() });
        await queryStore.executeTabSql(tab.id, nextSql, {
          pagination: { limit: pageLimit, offset: pageOffset },
          preserveResultDuringExecution: true,
        });
        console.info("[Gauss Horizon][reloadData:execute:done]", { traceId, elapsed: elapsed() });
      } catch (e) {
        console.error("[Gauss Horizon][reloadData:error]", { traceId, elapsed: elapsed(), error: e });
        queryStore.setExecuting(tab.id, false);
