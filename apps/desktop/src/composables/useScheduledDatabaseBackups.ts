...
export function useScheduledDatabaseBackups(options: { scheduler?: boolean } = {}) {
  ...
  async function pruneScheduleRuns(schedule: DatabaseBackupSchedule) {
    ...
    try {
      ...
      appendDebugLog("error", "[Gauss Horizon][database-backup:retention-error]", error);
    }
  }
  ...
  async function cancelRun(runId: string): Promise<boolean> {
    ...
    try {
      ...
      appendDebugLog("warn", "[Gauss Horizon][database-backup:cancel-request-error]", error);
      ...
      try {
        ...
        if (connection.db_type === "mysql" && config.tableFilterMode !== "all") {
          try {
            ...
            appendDebugLog("warn", "[Gauss Horizon][database-backup:table-name-case-detection-error]", error);
          }
          if (cancellationRequested.has(runId)) {
            finalStatus = "cancelled";
            return;
        ...
        await api.clearDatabaseExportCancellation(runId).catch((error) => {
          appendDebugLog("warn", "[Gauss Horizon][database-backup:cancel-clear-error]", error);
        });
        if (finalStatus !== "success" && generatedPaths.length > 0) {
          try {
            ...
            appendDebugLog("error", "[Gauss Horizon][database-backup:partial-cleanup-error]", error);
        ...
        appendDebugLog(finalStatus === "success" ? "info" : "error", `[Gauss Horizon][database-backup:${finalStatus}]`, {
          scheduleId: request.scheduleId,
          runId,
    ...
    try {
      ...
      for (const schedule of dueSchedules) {
        await runSchedule(schedule.id, "scheduled").catch((error) => {
          appendDebugLog("error", "[Gauss Horizon][database-backup:scheduler-error]", error);
        });
      }
    } catch (error) {
      appendDebugLog("error", "[Gauss Horizon][database-backup:scheduler-init-error]", error);
    } finally {
