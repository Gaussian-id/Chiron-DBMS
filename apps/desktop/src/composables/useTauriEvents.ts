      ...
      .then(({ listen }) => {
        listen<{ connection_id: string; database: string; schema?: string; table: string }>("mcp-open-table", async (event) => {
          try {
            ...
            console.error("[Gauss Horizon] mcp-open-table error:", e);
          }
        ...
        listen("mcp-reload-connections", async () => {
          try {
            ...
            console.error("[Gauss Horizon] mcp-reload-connections error:", e);
          }
        }).then((unlisten) => unlistenHandles.push(unlisten));
        ...
        listen<{
          ...
          try {
            ...
            console.error("[Gauss Horizon] mcp-execute-query error:", e);
          }
        }).then((unlisten) => unlistenHandles.push(unlisten));

        listen<string[]>("gauss-horizon-open-sql-files", async (event) => {
          try {
            ...
            console.error("[Gauss Horizon] gauss-horizon-open-sql-files error:", e);
          }
        }).then((unlisten) => unlistenHandles.push(unlisten));

        listen<string[]>("gauss-horizon-open-db-files", async (event) => {
          try {
            for (const path of event.payload) {
            ...
            console.error("[Gauss Horizon] gauss-horizon-open-db-files error:", e);
          }
        }).then((unlisten) => unlistenHandles.push(unlisten));

        listen<string[]>("gauss-horizon-open-connection-links", async (event) => {
          try {
            ...
            console.error("[Gauss Horizon] gauss-horizon-open-connection-links error:", e);
          }
        }).then((unlisten) => unlistenHandles.push(unlisten));

        listen<string[]>("gauss-horizon-open-ai-config-links", async (event) => {
          try {
            for (const url of event.payload) {
            ...
            console.error("[Gauss Horizon] gauss-horizon-open-ai-config-links error:", e);
          }
        }).then((unlisten) => unlistenHandles.push(unlisten));

        listen("gauss-horizon-close-active-tab", () => {
