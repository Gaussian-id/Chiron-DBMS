...
import { GAUSS_HORIZON_NEO4J_ELEMENT_ID_COLUMN, usesSyntheticRowIdKey } from "@/lib/table/tableEditing";
import { effectiveDatabaseTypeForConnection } from "@/lib/database/jdbcDialect";
import { normalizeBackendError } from "@/lib/backend/errorUtils";
import { uuid } from "@/lib/common/utils";
import i18n from "@/i18n";

const KEYLESS_GUARD_UNVERIFIED_ERROR = "Cannot safely update or delete this row: the table has no primary key, and Gauss Horizon could not check on the server whether the row can be targeted uniquely. Add a primary key or unique index before editing.";
...
export function useDataGridEditor(options: UseDataGridEditorOptions) {
  ...
  function shouldClearClonedColumn(columnName: string, columnInfo: ColumnInfo | undefined): boolean {
    ...
    if (resolvedDatabaseType.value === "neo4j" && columnName === GAUSS_HORIZON_NEO4J_ELEMENT_ID_COLUMN) return true;
  ...
  async function recordFailedDataGridHistory(statements: string[], rollbackStatements: string[], start: number, snapshot: PendingSaveSnapshot, error: unknown) {
    ...
    try {
      ...
      console.warn("[Gauss Horizon] failed to record data grid history", historyError);
    }
    return message;
  }
  ...
  function isConditionalUpdateTerminalFailure(error: unknown) {
    ...
    return backendError?.operationOutcome === "not_started" || backendError?.code === "Gauss Horizon-JDBC-4001" || backendError?.diagnostics?.category === "sql";
  }
  ...
  async function executeConditionalUpdate(statement: string): Promise<{ affectedRows?: number } | null> {
    ...
    try {
      ...
      try {
        ...
        console.warn("[Gauss Horizon] failed to record conditional data grid update history", historyError);
      }
      reloadCurrentData();
  ...
  async function saveChanges(saveOptions: SaveChangesOptions = {}) {
    ...
    console.info("[Gauss Horizon][dataGrid:save-statements]", {
    ...
    try {
      ...
      console.warn("[Gauss Horizon] failed to record data grid history", e);
    }
    applyDirtyRowsToResult(snapshot);
    options.onResultPayloadMutated?.();
    ...
    if (!joinedWriteTargets.value?.length && !manualTransactionSessionId.value && !shouldReloadAfterSave && snapshot.dirtyRows.size > 0 && options.refreshSavedRows) {
      try {
        ...
        console.warn("[Gauss Horizon] failed to refresh saved data grid rows", error);
      }
    }
    snapshot.newRowRefs.forEach((row) => savingNewRows.delete(row));
    clearSavedPendingChanges(snapshot);
