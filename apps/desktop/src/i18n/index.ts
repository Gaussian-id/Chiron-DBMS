...
const savedLocale = normalizeLocale(safeLocalStorageGet("chiron-horizon-locale"));
...
async function syncLocaleToBackend(locale: Locale) {
  ...
  try {
    ...
    console.warn("[Chiron Horizon][i18n] failed to sync locale to backend", error);
  }
}
...
export async function setLocale(locale: Locale) {
  ...
  safeLocalStorageSet("chiron-horizon-locale", locale);
