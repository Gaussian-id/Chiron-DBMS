...
export default withEnglishFallback({
  ...
  pluginPlatform: {
    ...
    localInstallDescription: "安裝從外掛商店之外取得的已簽署 .chiron-horizonp，Chiron Horizon 會自動驗證套件完整性與倉庫簽章。",
  ...
  updates: {
    ...
    portableAutomaticUpdate: "Chiron Horizon 將下載已簽署的可攜版 ZIP，結束後僅替換 Chiron Horizon.exe 並自動重新啟動。portable.chiron-horizon 和 data 會保留。",
  ...
  ai: {
    ...
    cliErrors: {
      ...
      chironHorizonMcpMissing: "找不到相容的 Chiron Horizon MCP Server。請在 設定 > MCP 中檢查並重新安裝或更新。",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows: "無法刪除舊的 JRE 目錄：{path}\n可能的原因：\n  - 仍有 chiron-horizon Agent / java 程序占用該目錄\n  - 防毒軟體正在掃描\n請關閉可能持有該目錄的程序，或重新啟動 chiron-horizon 後重試。\n（原始錯誤：{error}）",
