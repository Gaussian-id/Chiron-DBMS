...
export default withEnglishFallback({
  ...
  pluginPlatform: {
    ...
    centerTitle: "外掛中心",
    centerDescription: "在統一商店中探索選用的 Gauss Horizon 能力，並分別管理已安裝外掛與外掛倉庫。",
    marketplace: "外掛商店",
    installed: "已安裝",
    settings: "設定",
    ...
    localInstallDescription: "安裝從外掛商店之外取得的已簽署 .gauss-horizonp，Gauss Horizon 會自動驗證套件完整性與倉庫簽章。",
    repositoriesTitle: "外掛倉庫",
    repositoriesDescription: "官方來源由 Gauss Horizon 管理；加入自託管或第三方倉庫後，才會顯示對應的倉庫信任設定。",
    ...
    allowUnsignedDevelopmentPackageDescription: "僅影響手動安裝的 .gauss-horizonp，不會放寬官方外掛商店的簽章驗證。",
    installPackage: "安裝 .gauss-horizonp",
    dropToInstall: "放開以安裝外掛套件",
    dropInstallHint: "也可拖放 .gauss-horizonp 到外掛中心頁面安裝",
    packageFileType: "Gauss Horizon 外掛套件",
    ...
    noInstalledPluginsDescription: "安裝已簽署的 .gauss-horizonp，或僅在本機開發時明確允許未簽署套件。",
    compatible: "相容",
    blocked: "已阻擋",
    rollback: "回復版本",
    uninstall: "解除安裝",
  ...
  externalSqlFile: {
    modifiedTitle: "檔案已在 Gauss Horizon 外部變更",
    modifiedDescription: "{path} 已被其他程式修改，請選擇要保留的版本。",
    deletedTitle: "檔案已不存在",
    deletedDescription: "{path} 已在 Gauss Horizon 外部刪除或移動，編輯器中的內容仍然保留。",
  ...
  updates: {
    ...
    availableMessage: "Gauss Horizon {latest} 已發布，目前版本為 {current}。",
    ...
    cancelDownload: "取消下載",
    downloadStarted: "正在背景下載 Gauss Horizon {version}。",
    activeTasksBlockUpdate: "有 {count} 個任務正在執行，請等待任務完成後再更新 Gauss Horizon。",
    portableAutomaticUpdate: "Gauss Horizon 將下載已簽署的可攜版 ZIP，結束後僅替換 Gauss Horizon.exe 並自動重新啟動。portable.gauss-horizon 和 data 會保留。",
    ...
    readyToInstall: "Gauss Horizon {version} 已下載完成，可以安裝了。",
    installNow: "立即安裝",
    installedReadyToRestart: "Gauss Horizon 已更新完成，重新啟動後生效。",
  ...
  sidebar: {
    ...
    importExport: "匯入/匯出",
    import: "匯入連線",
    importGaussHorizon: "匯入 Gauss Horizon 設定",
  ...
  sqlLibrary: {
    title: "SQL 庫",
    storageHelp: "SQL 庫內容儲存在 Gauss Horizon 應用程式資料中，並非磁碟上的獨立 .sql 檔案。\n如需在檔案系統中使用，請匯出 SQL 庫。",
    unassociated: "未關聯",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "連線成功，但 Gauss Horizon 無法記住密碼：{message}",
    ...
    oracleTnsPathHint: "此路徑由 Gauss Horizon 後端/Oracle Agent 讀取。桌面版填寫本機路徑，Web 和 Docker 部署填寫伺服器路徑。",
    ...
    nacosWebConsoleUrlHint: "選填，用於在瀏覽器開啟 Nacos 控制台；Nacos 3 需填寫獨立的 Web 控制台位址。瀏覽器需可直接存取，且不會沿用 Gauss Horizon 的通道或 API 驗證。",
    ...
    gaussdbConnectionModeNativeHint: "使用 Gauss Horizon 內建的 PostgreSQL 相容協定，適用於 GaussDB 的 A/PG/Oracle 相容模式。",
    ...
    agentJavaTooOld: "此驅動程式需要 Java 21。請在驅動程式管理器中使用 Gauss Horizon 託管 JRE 21，或選擇 Java 21 可執行檔。",
    ...
    jdbcDocs: "檢視 JDBC 文件",
    jdbcPluginHint: "先安裝 Gauss Horizon JDBC 外掛程式，再匯入資料庫廠商提供的 JDBC 驅動程式 JAR。",
    ...
    kafkaKerberosKeytabPlaceholder: "Gauss Horizon Agent 所在機器路徑，例如 /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "可選，Gauss Horizon Agent 所在機器路徑，例如 /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab 和 krb5.conf 路徑由 Gauss Horizon Agent 讀取，必須存在於執行 Gauss Horizon Agent 的機器上；不會從當前瀏覽器上傳檔案。",
    ...
    sqliteWorkerPlacementSessionHint: "由 Gauss Horizon 透過 SSH 把 Worker 上傳到檔案主機，中斷連線後自動刪除遠端檔案。適合大多數臨時使用情境。",
    sqliteWorkerPlacementPersistHint: "由 Gauss Horizon 透過 SSH 把 Worker 上傳到檔案主機，中斷後仍保留在遠端，不會自動刪除。",
    sqliteWorkerPlacementPreplacedHint: "需要你先把 Worker 手動上傳到檔案主機，再填寫該絕對路徑。Gauss Horizon 不會代為上傳或刪除，但會校驗檔案摘要是否與目前 Desktop 版本一致。",
    ...
    sshWorkerUploadConsentMessage: "Gauss Horizon 需要把短生命週期的 SQLite worker 複製到 {host}:{port}。這是特權操作。請先核對目標路徑與摘要。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」有未儲存的 SQL 變更，結束 Gauss Horizon 後將遺失。儲存後再結束嗎？",
    unsavedChangesAppCloseMultipleMessage: "目前有 {count} 個 SQL 分頁未儲存，目前已定位到「{title}」。結束 Gauss Horizon 後將遺失，儲存後再結束嗎？",
  ...
  tabs: {
    ...
    externalFileMissing: "已在 Gauss Horizon 外部刪除或移動",
  ...
  grid: {
    ...
    esDeepPageJumpConfirmMessage: "Elasticsearch 的 search_after 無法直接定位到指定頁面。Gauss Horizon 必須依序取得缺少的游標，跳回較小頁碼時還會從第 1 頁重新開始。跳轉到第 {page} 頁預計會傳送 {requests} 次查詢，可能耗時較長並增加叢集負載。是否繼續？",
    ...
    conditionalBulkEditConfirmMessage: "此操作會立即更新執行時符合條件的全部資料列，包括尚未載入的資料列；Gauss Horizon 無法為這些資料列產生可靠的回復 SQL。",
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "Gauss Horizon 無法為此結果唯一確定一個安全的回寫目標資料表。",
      ...
      "metadata-unavailable": "Gauss Horizon 無法載入資料表 metadata，因此已停用結果編輯。",
    },
  ...
  backendErrors: {
    ...
    transaction: {
      sessionExpired: "手動交易因 {timeoutSecs} 秒無操作而過期並回滾，Gauss Horizon 將自動開始新的交易。",
    },
    legacy: "後端回傳了錯誤。",
    unknown: "發生未預期的後端錯誤。",
  },
  ...
  configExport: {
    ...
    cryptoUnavailable: "目前瀏覽器環境不支援加密設定匯入，請使用 HTTPS、localhost 或 Gauss Horizon 桌面版。",
  ...
  ai: {
    ...
    closeWithRunsDescription: "結束 Gauss Horizon 會中斷 {count} 個正在執行或排隊的 AI 任務。你也可以取消結束並等待任務完成。",
    ...
    productionWriteBlocked: "AI 無法在生產資料庫執行資料或結構變更。請核對此 SQL 後在 Gauss Horizon 中手動執行。",
    ...
    customHeadersReserved: "此請求標頭由 Gauss Horizon 管理：{name}",
    ...
    codexMcpRequiredDescription: "Codex CLI 會透過 Gauss Horizon MCP Server 存取資料庫結構與查詢工具。請先安裝後再於 AI 助理中使用 Codex。",
    ...
    codexCliEnvReservedName: "{name} 由 Gauss Horizon 用於 MCP Server，不能在這裡設定。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "設定覆寫了由 Gauss Horizon 管理的 MCP 環境變數。請從供應商設定中移除 GAUSS_HORIZON_MCP_* 變數。",
      claudeCodeNotAuthenticated: "Claude Code CLI 尚未登入。請在終端機執行 `claude auth login` 後重試。",
      claudeCodeMcpConfigInvalid: "Claude Code 無法讀取或解析 Gauss Horizon 產生的 MCP 設定。請檢查 Claude Code CLI 版本和可執行檔路徑。",
      gaussHorizonMcpMissing: "找不到相容的 Gauss Horizon MCP Server。請在 設定 > MCP 中檢查並重新安裝或更新。",
      claudeCodeMcpStartupFailed: "Claude Code 已載入 MCP 設定，但無法啟動 Gauss Horizon MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
      claudeCodeCommandLineTooLong: "Windows 拒絕了過長的 Claude Code 命令。請更新 Gauss Horizon，並使用產生的 MCP 設定檔重試。",
      ...
      piAgentEnvReserved: "設定覆寫了由 Gauss Horizon 管理的 Pi 或 MCP 環境變數。請移除 GAUSS_HORIZON_PI_* 與 GAUSS_HORIZON_MCP_* 變數。",
      piAgentNotAuthenticated: "Pi Coding Agent 沒有可用的已驗證模型供應商。請先在 Pi 中設定或登入供應商。",
      piAgentMcpStartupFailed: "Pi 無法啟動 Gauss Horizon 的作用域 MCP 橋接。請檢查 設定 > MCP 和下方診斷詳情。",
      ...
      openCodeEnvReserved: "設定覆寫了由 OpenCode 或 Gauss Horizon 管理的環境變數。請移除 OPENCODE 控制變數和 GAUSS_HORIZON_MCP_* 變數。",
      openCodeNotAuthenticated: "OpenCode 尚未設定已驗證的模型供應商。請執行 `opencode auth login` 或在 OpenCode 中設定供應商後重試。",
      openCodeMcpStartupFailed: "OpenCode 無法啟動受限範圍的 Gauss Horizon MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
      ...
      cursorEnvReserved: "設定覆寫了由 Cursor 或 Gauss Horizon 管理的環境變數。請移除 CURSOR_CONFIG_DIR、CURSOR_DATA_DIR 和 GAUSS_HORIZON_MCP_* 變數。",
      cursorNotAuthenticated: "Cursor CLI 尚未登入。請在終端執行 `agent login` 後重試。",
      cursorMcpStartupFailed: "Cursor 無法啟動受限範圍的 Gauss Horizon MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
      ...
      grokCliEnvReserved: "覆寫了 Gauss Horizon 管理的 MCP 環境變數。請從供應商設定中移除 GAUSS_HORIZON_MCP_* 變數。",
      grokCliNotAuthenticated: "Grok CLI 尚未登入。請在終端機執行 `grok login` 後重試。",
      grokCliMcpStartupFailed: "Grok 已載入 MCP 設定，但無法啟動 Gauss Horizon MCP Server。請檢查 設定 > MCP 與下方診斷資訊。",
      grokCliCommandLineTooLong: "命令列過長，Windows 拒絕執行 Grok CLI。請更新 Gauss Horizon 並使用產生的 prompt 檔案重試。",
      ...
      codeBuddyEnvReserved: "覆寫了 Gauss Horizon 管理的 MCP 環境變數。請從供應商設定中移除 GAUSS_HORIZON_MCP_* 變數。",
      codeBuddyNotAuthenticated: "CodeBuddy Code 尚未登入。請在終端機開啟 `codebuddy`，完成登入後重試。",
      codeBuddyTimeout: "CodeBuddy Code CLI 未在操作逾時前回應。",
      codeBuddyMcpConfigInvalid: "CodeBuddy Code 無法讀取暫存 Gauss Horizon MCP 設定。請檢查下方診斷資訊。",
      codeBuddyMcpStartupFailed: "CodeBuddy Code 已載入 MCP 設定，但無法啟動 Gauss Horizon MCP Server。請檢查 設定 > MCP 與下方診斷資訊。",
      ...
      qoderEnvReserved: "設定覆蓋了由 Gauss Horizon 管理的 MCP 環境變數。請從供應商設定中移除 GAUSS_HORIZON_MCP_* 變數。",
      ...
      qoderMcpConfigInvalid: "Qoder CLI 無法讀取暫時的 Gauss Horizon MCP 設定。請檢查 Qoder CLI 版本和下方診斷詳情。",
      qoderMcpStartupFailed: "Qoder CLI 已載入 MCP 設定，但無法啟動 Gauss Horizon MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
    ...
    htmlCopyRiskBody: "此 HTML 由 AI 產生，在 Gauss Horizon 外開啟可能執行指令碼並存取網路。",
    htmlCopyRiskAccept: "仍要複製",
    htmlCopyRiskRemember: "本次工作階段不再詢問",
    htmlCopyRiskToast: "已複製。此 AI 產生的 HTML 在 Gauss Horizon 外開啟可能執行指令碼或存取網路。",
  ...
  sqlParameters: {
    title: "SQL 參數",
    description: "為 SQL 模板佔位符填寫參數值，Gauss Horizon 會在執行前替換為 SQL 字面量。",
    name: "參數名",
    syntax: "Syntax",
    type: "類型",
    value: "參數值",
  ...
  settings: {
    ...
    uiFontFamily: "介面字型",
    uiFontAppDefault: "Gauss Horizon 預設",
    ...
    iconThemeDefault: "預設",
    iconThemeDefaultDescription: "使用彩色 Gauss Horizon 應用程式圖示。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "應用程式內使用黑色 Gauss Horizon 標誌。",
    ...
    quitOnCloseDescription: "開啟後，點擊視窗關閉按鈕將徹底退出 Gauss Horizon，而不是隱藏到系統匣。",
    ...
    duckDbWorkerProcessIsolationDescription: "將 DuckDB 查詢放到獨立 worker 進程中執行。停止長查詢時，如果 DuckDB 未及時回應取消請求，Gauss Horizon 會結束 worker 進程以釋放資料庫檔案鎖。",
    duckDbWorkerMaxProcesses: "DuckDB 最大並行 worker 數",
    duckDbWorkerMaxProcessesDescription: "限制 Gauss Horizon 同時執行的 DuckDB worker 進程數量。範圍：1-16，預設：4。",
    duckDbWorkerProcessIsolationRestartRequired: "重啟 Gauss Horizon 後生效。",
    restartGaussHorizon: "重啟 Gauss Horizon",
    restartGaussHorizonFailed: "重啟 Gauss Horizon 失敗：{error}",
    ...
    openTabsRestoreModeDescription: "選擇 Gauss Horizon 啟動時如何還原上次開啟的分頁。",
    ...
    syncWebDavDescription: "上傳包含 SQL 庫的 Gauss Horizon 快照到相容 WebDAV 的儲存服務，或從該服務還原快照。",
    syncWebDavWebDescription: "Web 模式由 Gauss Horizon 伺服器端存取 WebDAV，不受瀏覽器 CORS 限制；儲存的密碼會保留在目前 Gauss Horizon 執行個體中。",
    ...
    syncRememberWebDavPasswordDescription: "密碼會加密儲存在目前 Gauss Horizon 執行個體中，不會同步到 WebDAV，也不會取代同步密碼。",
    syncRemotePath: "遠端快照路徑",
    syncRemotePathDescription: "上傳時 Gauss Horizon 會自動建立缺失的父目錄。",
    syncSecretNotice: "預設只同步連線詳細資料和設定。資料庫密碼、SSH 密碼、代理伺服器密碼、連線字串和 AI API Key 會保留在本機。",
    syncSecrets: "同步加密後的敏感資訊",
    syncSecretsDescription: "開啟後，Gauss Horizon 會先加密資料庫密碼、SSH 密碼和 AI API Key，再上傳到 WebDAV。",
    syncSecretsPassphrase: "同步密碼",
    syncSecretsPassphraseDescription: "此密碼只用於加密和還原敏感資料，並會加密儲存在目前 Gauss Horizon 執行個體中；在其他執行個體還原時需要再次輸入。",
    syncTest: "測試",
    syncUpload: "上傳",
    syncDownload: "下載",
    ...
    syncAutoUploadDescription: "僅在 Gauss Horizon 開啟時執行。下載仍需手動確認，避免意外覆蓋本機已儲存的 SQL。",
    ...
    syncDownloadConfirm: "要下載並套用遠端 Gauss Horizon 快照嗎？本機 metadata 和已儲存的 SQL 會被取代。",
    apply: "套用",
    applyAndClose: "套用並關閉",
    reset: "重設",
    resetDefaults: "還原預設",
    ...
    appCloseUnsavedTabsModeDescription: "選擇結束 Gauss Horizon 時如何處理有未儲存內容的 SQL 分頁。",
    appCloseUnsavedTabsModePrompt: "結束前詢問是否儲存",
    appCloseUnsavedTabsModeKeepDrafts: "直接結束並保留草稿",
    appCloseUnsavedTabsModeHint: "草稿會以未儲存分頁的形式保留，下次啟動 Gauss Horizon 時還原；需要「啟動時還原分頁」不是「不保留分頁」。",
    appCloseDraftPersistFailed: "Gauss Horizon 無法儲存開啟的分頁草稿，因此已取消退出。請檢查磁碟空間和檔案權限後重試。詳細資料：{message}",
    ...
    jdbcPlugin: "Gauss Horizon JDBC 外掛程式",
    ...
    mcpTab: "MCP",
    mcpTitle: "MCP Server",
    mcpDescription: "檢查 Claude Code、Cursor 等程式助理使用的 Gauss Horizon MCP Server 安裝與版本狀態。",
    mcpChecking: "檢查中",
    ...
    mcpConfig: "MCP 設定",
    mcpConfigOptionsHint: "連線範圍與執行權限均由 Gauss Horizon 統一控制，並對所有 MCP 用戶端立即生效；產生的設定只包含啟動 MCP Server 與連線目前 Gauss Horizon 所需的執行參數，不包含權限或連線範圍變數。",
    mcpScopeConnection: "允許 MCP 存取的連線",
    mcpScopeConnectionDescription: "Gauss Horizon 只向 MCP 公開選取的連線，變更後對所有 MCP 用戶端生效。",
    mcpScopeMode: "連線範圍模式",
    ...
    mcpExecutionModeReadOnlyDescription: "僅允許 Gauss Horizon 判定為讀取的請求；禁止已識別的資料庫寫入與連線管理變更。資料庫帳號權限仍是最終硬邊界。",
    ...
    mcpNpmBoundary: "Gauss Horizon 只檢測和提示 MCP 狀態；安裝與升級仍由 npm 完成。",
    mcpRefresh: "重新檢查",
    mcpGuide: "MCP 指南",
    ...
    mcpDatabaseScopePermissionNote: "選擇範圍不會改變此連線在 Gauss Horizon 中原有的資料庫權限。",
    ...
    mcpHttpServiceDescription: "預設關閉。啟用後由 Gauss Horizon Desktop 託管服務；儲存後才會將上方設定套用到實際監聽服務。",
    ...
    mcpHttpErrorPortInUse: "該埠已被其他應用程式佔用。Gauss Horizon 會在儲存時自動重新啟動自己的 MCP 服務；請關閉佔用該埠的其他應用程式，或改用其他埠。",
    ...
    mcpToolOpenTable: "在 Gauss Horizon 中開啟資料表",
    mcpToolExecuteAndShow: "在 Gauss Horizon 中執行並顯示",
    ...
    syncSnippetMigrateLegacyConfirm: "Gauss Horizon 會先建立新的加密程式碼片段，再永久刪除舊明文程式碼片段 {id}。此操作無法復原，是否繼續？",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows: "無法刪除舊的 JRE 目錄：{path}\n可能的原因：\n  - 仍有 gauss-horizon Agent / java 程序占用該目錄\n  - 防毒軟體正在掃描\n請關閉可能持有該目錄的程序，或重新啟動 gauss-horizon 後重試。\n（原始錯誤：{error}）",
    ...
    offlineExportDescription: "選擇要包含的 Gauss Horizon 託管 Agent 驅動程式。離線包僅適用於目前平台。",
    offlineExportPlatform: "適用平台",
    offlineExportSelectedCount: "已選擇 {selected}/{total}",
    offlineExportSelectAll: "全選",
    offlineExportClear: "清除",
    offlineExportNoDrivers: "目前沒有可匯出的 Gauss Horizon 託管 Agent 驅動程式。",
    ...
    offlineExportSecurityHint: "僅包含 Gauss Horizon 託管的驅動程式產物及所需 JRE；本機路徑、設定和外部驅動程式不會被匯出。",
    ...
    offlineExportUnavailable_unmanagedInstall: "此產物未被記錄為 Gauss Horizon 託管安裝。",
    ...
    offlineExportUnavailable_missingManagedJre: "尚未安裝所需的 Gauss Horizon 託管 JRE。",
    offlineExportUnavailable_invalidManagedJre: "所需的 Gauss Horizon 託管 JRE 無效或不安全。",
    refresh: "重新整理",
    javaRuntime: "Java 執行環境",
    javaRuntimeManaged: "Gauss Horizon 託管 JRE",
    ...
    update: "更新",
    localInstall: "本機安裝",
    localInstallHint: "適用於完全內網環境：在另一台可連網的電腦上，從 Gauss Horizon 的 GitHub Releases 頁面下載官方 gauss-horizon-jdbc-plugin-*.zip 安裝包，傳輸到本機後在此選取該檔案——本機不需要連網即可完成安裝。",
    runtimeDrivers: "執行環境",
    pluginsTab: "外掛程式",
    pluginContributionsTitle: "前端外掛程式貢獻",
    pluginContributionsDescription: "已安裝的外掛程式可以描述連線表單和其他 UI 貢獻，而無需將程式碼注入 Gauss Horizon 主視窗。",
  ...
  nacos: {
    ...
    namespaceId: "命名空間 ID",
    namespaceIdPlaceholder: "留空則由 Gauss Horizon 自動產生 UUID",
    ...
    nacosDetectAccessibleNamespacesHint: "Gauss Horizon 會驗證「{name}」中每個命名空間的設定與服務讀取權限後再供選擇。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "目前使用 Nacos 3 Console API（通常為 8080）。可填寫命名空間名稱或 ID；Gauss Horizon 能讀取目錄時會自動將名稱轉換為實際 ID。",
    rnacosNamespaceAccessScopeHint: "設定 r-nacos Console 使用者資料後，Gauss Horizon 會依目前使用者的命名空間權限決定存取範圍。",
    nacosAuth: "驗證",
    ...
    persistentInstanceHint: "Gauss Horizon 不註冊臨時實例，因為無法維持其心跳。",
    ipAddress: "IP 位址",
    port: "連接埠",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "叢集狀態仍可正常檢視。Gauss Horizon 會直接存取目前 etcd endpoint 拼接後的 /metrics，請確認 Gauss Horizon 應用程式可以存取該位址。",
  ...
  databaseBackup: {
    title: "資料庫備份",
    schedules: "備份計畫",
    runtimeRequirement: "Gauss Horizon 處於開啟狀態時會執行定時備份，包含最小化到托盤時。",
    addSchedule: "新建計畫",
    ...
    fileNamePatternHint: "可使用 {'{'}schedule{'}'}、{'{'}date{'}'}、{'{'}timestamp{'}'}、{'{'}database{'}'} 和 {'{'}runId{'}'}。Gauss Horizon 會自動補上缺少的資料庫名稱，並自動加入副檔名。",
