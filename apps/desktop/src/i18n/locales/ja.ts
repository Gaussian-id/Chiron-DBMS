...
export default withEnglishFallback({
  ...
  pluginPlatform: {
    ...
    centerDescription: "統合マーケットプレイスで Chiron Horizon の追加機能を探し、インストール済みプラグインとリポジトリを個別に管理します。",
    ...
    marketplaceGuideDescription: "プラグインの問題や機能要望は、プラグインカードのソースリポジトリリンクから該当リポジトリに Issue を投稿してください。Chiron Horizon メインリポジトリはアプリ本体の問題のみを受け付けています。",
    ...
    localInstallDescription: "マーケットプレイス外で入手した署名済み .chiron-horizonp をローカルファイルまたは URL からインストールします。Chiron Horizon が整合性と署名を自動検証します。",
    repositoriesTitle: "プラグインリポジトリ",
    repositoriesDescription: "公式ソースは Chiron Horizon が管理します。セルフホストまたは第三者のリポジトリでは、リポジトリの信頼設定を使用します。",
    ...
    pluginDevelopmentDocsDescription: "Chiron Horizon プラグインの開発・パッケージ化・マーケットプレイスへの公開方法をご確認ください。",
    ...
    allowUnsignedDevelopmentPackageDescription: "手動での .chiron-horizonp インストールにのみ適用され、公式マーケットプレイスの検証は緩和されません。",
    installPackage: ".chiron-horizonp をインストール",
    ...
    dropInstallHint: ".chiron-horizonp ファイルをこのページにドロップしてもインストールできます",
    packageFileType: "Chiron Horizon プラグインパッケージ",
    ...
    noInstalledPluginsDescription: "署名済み .chiron-horizonp をインストールするか、ローカル開発時のみ未署名パッケージを明示的に許可してください。",
    compatible: "互換",
    blocked: "ブロック済み",
    rollback: "ロールバック",
    uninstall: "アンインストール",
  ...
  externalSqlFile: {
    modifiedTitle: "ファイルがChiron Horizonの外部で変更されました",
    modifiedDescription: "{path} は別のプログラムによって変更されました。保持するバージョンを選択してください。",
    deletedTitle: "ファイルが存在しません",
    deletedDescription: "{path} はChiron Horizonの外部で削除または移動されました。エディターの内容は保持されています。",
  ...
  updates: {
    ...
    availableMessage: "Chiron Horizon {latest} が利用可能です。現在のバージョンは {current} です。",
    upToDate: "Chiron Horizon は最新です ({version})。",
    ...
    downloadStarted: "Chiron Horizon {version} をバックグラウンドでダウンロード中です。",
    activeTasksBlockUpdate: "{count} 件のタスクが実行中です。完了してから Chiron Horizon を更新してください。",
    portableAutomaticUpdate: "Chiron Horizon は署名済みのポータブル ZIP をダウンロードし、終了後に Chiron Horizon.exe のみを置き換えて自動的に再起動します。portable.chiron_horizon とデータは保持されます。",
    ...
    readyToInstall: "Chiron Horizon {version} のインストール準備ができました。",
    installNow: "今すぐインストール",
    installedReadyToRestart: "Chiron Horizon が更新されました。再起動して完了してください。",
  ...
  sidebar: {
    ...
    import: "接続をインポート",
    importChironHorizon: "Chiron Horizon設定をインポート",
  ...
  sqlLibrary: {
    title: "SQLライブラリ",
    storageHelp: "SQLライブラリの内容はChiron Horizonのアプリデータに保存され、ディスク上の独立した.sqlファイルではありません。\nファイルシステムで使用するには、SQLライブラリをエクスポートしてください。",
    unassociated: "未関連付け",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "接続しましたが、Chiron Horizon はパスワードを保存できませんでした: {message}",
    ...
    oracleTnsPathHint: "このパスは Chiron Horizon バックエンド/Oracle Agent が読み取ります。デスクトップではローカルパス、Web/Docker ではサーバーパスを指定してください。",
    ...
    nacosWebConsoleUrlHint: "任意。ブラウザーで Nacos コンソールを開きます。Nacos 3 では専用の Web コンソールアドレスが必要です。ブラウザーから直接アクセスできる必要があり、Chiron Horizon のトンネルや API 認証は再利用されません。",
    ...
    gaussdbConnectionModeNativeHint: "GaussDB の A/PG/Oracle 互換モードでは、Chiron Horizon 内蔵の PostgreSQL 互換プロトコルを使用します。",
    ...
    agentJavaTooOld: "このドライバーにはJava 21が必要です。ドライバーマネージャーでChiron Horizon管理JRE 21を使用するか、Java 21実行ファイルを選択してください。",
    ...
    jdbcPluginHint: "最初にChiron Horizon JDBCプラグインをインストールし、次にデータベースベンダーのJDBCドライバーJARをインポートしてください。",
    ...
    kafkaKerberosKeytabPlaceholder: "Chiron Horizon Agent が動作するマシンのパス。例: /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "オプション。Chiron Horizon Agent が動作するマシンのパス。例: /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab と krb5.conf のパスは Chiron Horizon Agent が読み取るため、Chiron Horizon Agent が動作するマシン上に存在する必要があります。ブラウザからファイルをアップロードすることはありません。",
    ...
    sqliteWorkerPlacementSessionHint: "Chiron Horizon が SSH 経由でワーカーをファイルホストにアップロードし、切断時にリモートファイルを削除します。一時的な利用に適しています。",
    sqliteWorkerPlacementPersistHint: "Chiron Horizon が SSH 経由でワーカーをファイルホストにアップロードし、切断後もそのまま残します。自動的に削除されることはありません。",
    sqliteWorkerPlacementPreplacedHint: "ワーカーを事前にファイルホストへアップロードしてから、その絶対パスを入力してください。Chiron Horizon は自動でアップロード・削除しませんが、ファイルがこの Desktop バージョンと一致するかを検証します。",
    ...
    sshWorkerUploadConsentMessage: "Chiron Horizon は短命の SQLite ワーカーを {host}:{port} にコピーする必要があります。これは特権操作です。許可する前に宛先パスとダイジェストを確認してください。",
    ...
    httpTunnelTokenPlaceholder: "chiron_horizon_tunnel.php で設定された Token",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」に未保存の SQL 変更があり、Chiron Horizon の終了後に失われます。保存してから終了しますか？",
    unsavedChangesAppCloseMultipleMessage: "{count} 個の SQL タブが未保存です。現在「{title}」を表示中です。Chiron Horizon の終了後に失われます。保存してから終了しますか？",
  ...
  tabs: {
    ...
    externalFileMissing: "Chiron Horizonの外部で削除または移動されました",
  ...
  grid: {
    ...
    conditionalBulkEditConfirmMessage: "実行時に条件へ一致するすべての行（未読込の行を含む）を直ちに更新します。Chiron Horizon は信頼できるロールバック SQL を生成できません。",
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "Chiron Horizon はこの結果に対して安全に書き戻せる対象テーブルを 1 つに特定できませんでした。",
      ...
      "metadata-unavailable": "Chiron Horizonがテーブルメタデータを読み込めなかったため、結果の編集は無効です。",
    },
    ...
    esDeepPageJumpConfirmMessage:
      "Elasticsearch の search_after では指定ページへ直接移動できません。Chiron Horizon は不足しているカーソルを順番に取得し、前のページへ戻る場合は 1 ページ目からやり直します。ページ {page} への移動では約 {requests} 回のクエリが送信される見込みです。時間がかかり、クラスター負荷が増える可能性があります。続行しますか？",
  ...
  configExport: {
    ...
    cryptoUnavailable: "暗号化設定のインポートはこのブラウザ環境では利用できません。HTTPS、localhost、またはChiron Horizon Desktopを使用してください。",
  ...
  ai: {
    ...
    codexMcpRequiredTitle: "Chiron Horizon MCP Serverが必要です",
    codexMcpRequiredDescription: "Codex CLIはChiron Horizon MCP Server経由でデータベーススキーマとクエリツールにアクセスします。AIアシスタントでCodexを使う前にインストールしてください。",
    ...
    codexCliEnvReservedName: "{name}はMCP Server用にChiron Horizonが管理しているため、ここでは設定できません。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "Chiron Horizonが管理するMCP環境変数が上書きされています。プロバイダー設定からCHIRON_HORIZON_MCP_*変数を削除してください。",
      claudeCodeNotAuthenticated: "Claude Code CLIにログインしていません。ターミナルで`claude auth login`を実行してから再試行してください。",
      claudeCodeMcpConfigInvalid: "Claude CodeがChiron HorizonのMCP設定を読み取りまたは解析できません。Claude Code CLIのバージョンと実行ファイルのパスを確認してください。",
      chiron_horizonMcpMissing: "互換性のあるChiron Horizon MCP Serverが見つかりません。設定 > MCPで確認し、再インストールまたは更新してください。",
      claudeCodeMcpStartupFailed: "Claude CodeはMCP設定を読み込みましたが、Chiron Horizon MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      claudeCodeCommandLineTooLong: "Windowsが長すぎるClaude Codeコマンドを拒否しました。Chiron Horizonを更新し、生成されたMCP設定ファイルで再試行してください。",
      ...
      piAgentEnvReserved: "Chiron Horizonが管理するPiまたはMCP環境変数が上書きされています。CHIRON_HORIZON_PI_*とCHIRON_HORIZON_MCP_*を削除してください。",
      piAgentNotAuthenticated: "Pi Coding Agentに認証済みのモデルプロバイダーがありません。Piでプロバイダーを設定またはログインしてください。",
      piAgentMcpStartupFailed: "PiはChiron Horizon MCPブリッジを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      openCodeEnvReserved: "OpenCodeまたはChiron Horizonが管理する環境変数が上書きされています。設定からOPENCODE制御変数とCHIRON_HORIZON_MCP_*を削除してください。",
      openCodeNotAuthenticated: "OpenCodeに認証済みのモデルプロバイダーがありません。`opencode auth login`を実行するかOpenCodeでプロバイダーを設定して、再試行してください。",
      openCodeMcpStartupFailed: "OpenCodeはスコープ付きChiron Horizon MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      cursorEnvReserved: "CursorまたはChiron Horizonが管理する環境変数が上書きされています。CURSOR_CONFIG_DIR、CURSOR_DATA_DIR、CHIRON_HORIZON_MCP_*を設定から削除してください。",
      cursorNotAuthenticated: "Cursor CLIにログインしていません。ターミナルで`agent login`を実行してから再試行してください。",
      cursorMcpStartupFailed: "Cursorはスコープ付きChiron Horizon MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      grokCliEnvReserved: "Chiron Horizonが管理するMCP環境変数が上書きされています。プロバイダー設定からCHIRON_HORIZON_MCP_*変数を削除してください。",
      grokCliNotAuthenticated: "Grok CLIにログインしていません。ターミナルで`grok login`を実行してから再試行してください。",
      grokCliMcpStartupFailed: "GrokはMCP設定を読み込みましたが、Chiron Horizon MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      grokCliCommandLineTooLong: "Windowsが長すぎるGrok CLIコマンドを拒否しました。Chiron Horizonを更新し、生成されたプロンプトファイルで再試行してください。",
      ...
      codeBuddyEnvReserved: "Chiron Horizonが管理するMCP環境変数が上書きされています。プロバイダー設定からCHIRON_HORIZON_MCP_*変数を削除してください。",
      ...
      codeBuddyMcpConfigInvalid: "CodeBuddy Codeは一時的なChiron Horizon MCP設定を読み込めませんでした。以下の診断情報を確認してください。",
      codeBuddyMcpStartupFailed: "CodeBuddy CodeはMCP設定を読み込みましたが、Chiron Horizon MCP Serverを起動できません。設定 > MCPと以下の診断情報を確認してください。",
      ...
      qoderEnvReserved: "Chiron Horizonが管理するMCP環境変数が上書きされました。プロバイダー設定からCHIRON_HORIZON_MCP_*変数を削除してください。",
      ...
      qoderMcpConfigInvalid: "Qoder CLIが一時的なChiron Horizon MCP設定を読み取れませんでした。Qoder CLIのバージョンと以下の診断詳細を確認してください。",
      qoderMcpStartupFailed: "Qoder CLIはMCP設定を読み込みましたが、Chiron Horizon MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
    ...
    closeWithRunsDescription: "Chiron Horizonを終了すると、実行中またはキューに入っている{count}件のAIタスクが中断されます。このダイアログを閉じて完了を待つこともできます。",
    ...
    productionWriteBlocked: "AI は本番データベースでデータまたはスキーマの変更を実行できません。この SQL を確認し、Chiron Horizon で手動実行してください。",
    ...
    htmlCopyRiskBody: "このHTMLはAIによって生成されました。Chiron Horizonの外部で開くとスクリプトが実行され、ネットワークへアクセスする可能性があります。",
    htmlCopyRiskAccept: "それでもコピー",
    htmlCopyRiskRemember: "このセッションでは今後確認しない",
    htmlCopyRiskToast: "コピーしました。このAI生成HTMLは、Chiron Horizonの外部で開くとスクリプトを実行したりネットワークへアクセスしたりする可能性があります。",
    ...
    customHeadersReserved: "このヘッダーは Chiron Horizon により管理されています: {name}",
    enableThinking: "思考",
    enableThinkingOn: "有効",
  ...
  contextMenu: {
    ...
    parameterDefaultHint: "DEFAULTがチェックされている場合、Chiron Horizonはその引数を省略し、データベースがプロシージャのデフォルト値を使用します。",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "クラスタ状態は引き続き利用できます。Chiron Horizon は現在の etcd エンドポイントへ直接 /metrics を要求するため、生成された URL に Chiron Horizon アプリケーションから到達できることを確認してください。",
  ...
  sqlParameters: {
    title: "SQLパラメーター",
    description: "SQLテンプレートのプレースホルダーに値を入力します。Chiron Horizonは実行前にSQLリテラルへ置換します。",
    name: "パラメーター",
    syntax: "Syntax",
    type: "型",
    value: "値",
  ...
  settings: {
    ...
    uiFontAppDefault: "Chiron Horizon デフォルト",
    ...
    iconThemeDefaultDescription: "カラーChiron Horizonアプリアイコンを使用。",
    iconThemeBlack: "黒",
    iconThemeBlackDescription: "アプリ内で黒いChiron Horizonロゴを使用。",
    iconThemeBlackDescriptionMac: "アプリ内は黒いロゴ、Dockは黒い背景、メニューバーはシステムに従います。",
    showTrayIcon: "トレイ/メニューバーアイコンを表示",
    showTrayIconDescription: "無効時はアイコンが表示されませんが、ウィンドウを閉じると従来通りChiron Horizonはバックグラウンドに隠れます。",
    quitOnClose: "ウィンドウを閉じるときに終了",
    quitOnCloseDescription: "有効にすると、閉じるボタンでトレイに隠すのではなく、Chiron Horizonを完全に終了します。",
    closeActionPromptTitle: "ウィンドウを閉じる",
    closeActionPromptDescription: "閉じるボタンの動作を選択してください。後から「設定 → 外観」で変更できます。",
    closeActionQuit: "Chiron Horizonを終了",
    ...
    duckDbWorkerProcessIsolationDescription: "DuckDB クエリを独立した worker プロセスで実行します。長時間クエリがキャンセルに応答しない場合、Chiron Horizon は worker プロセスを停止してデータベースファイルのロックを解放できます。",
    duckDbWorkerMaxProcesses: "DuckDB の最大同時 worker 数",
    duckDbWorkerMaxProcessesDescription: "Chiron Horizon が同時に実行できる DuckDB worker プロセス数を制限します。範囲: 1-16。既定値: 4。",
    duckDbWorkerProcessIsolationRestartRequired: "この変更を有効にするには Chiron Horizon を再起動してください。",
    restartChironHorizon: "Chiron Horizon を再起動",
    restartChironHorizonFailed: "Chiron Horizon の再起動に失敗しました: {error}",
    ...
    openTabsRestoreModeDescription: "Chiron Horizon 起動時に前回開いていたタブをどのように復元するかを選択します。",
    ...
    syncWebDavDescription: "保存済みSQLライブラリを含むChiron HorizonスナップショットをWebDAV互換ストレージへアップロードまたは復元します。",
    syncWebDavWebDescription: "WebモードではChiron HorizonサーバーがWebDAVへ接続するため、ブラウザーのCORS制限を受けません。保存したパスワードは現在のChiron Horizonインスタンス内に保持されます。",
    ...
    syncRememberWebDavPasswordDescription: "パスワードは暗号化され、現在のChiron Horizonインスタンスにのみ保存されます。WebDAVには同期されず、同期パスワードを置き換えるものでもありません。",
    syncRemotePath: "リモートスナップショットパス",
    syncRemotePathDescription: "アップロード時に不足している親フォルダをChiron Horizonが作成します。",
    syncSecretNotice: "デフォルトでは、Chiron Horizonは接続詳細と設定のみを同期します。データベースパスワード、SSHパスワード、プロキシパスワード、接続文字列、AI APIキーはローカルに保持されます。",
    syncSecrets: "暗号化されたシークレットを同期",
    syncSecretsDescription: "有効時、Chiron Horizonはデータベースパスワード、SSHパスワード、AI APIキーを暗号化してからWebDAVにアップロードします。",
    syncSecretsPassphrase: "同期パスワード",
    syncSecretsPassphraseDescription: "このパスワードは機密データの暗号化と復元にのみ使用され、現在のChiron Horizonインスタンスに暗号化して保存されます。他のインスタンスでは再入力してください。",
    syncTest: "テスト",
    syncUpload: "アップロード",
    syncDownload: "ダウンロード",
    ...
    syncAutoUploadDescription: "Chiron Horizonを開いている間だけ実行されます。ローカルの保存済みSQLを誤って置き換えないよう、ダウンロードは手動確認が必要です。",
    ...
    syncDownloadConfirm: "リモートChiron Horizonスナップショットをダウンロードして適用しますか？ローカルのメタデータと保存済みSQLが置き換えられます。",
    apply: "適用",
    ...
    appCloseUnsavedTabsModeDescription: "未保存の編集があるSQLタブが開いたままChiron Horizonを終了するときの動作を選択します。",
    appCloseUnsavedTabsModePrompt: "保存するかどうか確認する",
    appCloseUnsavedTabsModeKeepDrafts: "確認せず終了して下書きを保持",
    appCloseUnsavedTabsModeHint: "下書きは未保存のタブとして保持され、次回Chiron Horizonを起動したときに復元されます。「起動時にタブを復元」が「タブを保持しない」になっていない必要があります。",
    appCloseDraftPersistFailed: "開いているタブの下書きを保存できなかったため、Chiron Horizon の終了を中止しました。ディスク容量とファイル権限を確認してから、もう一度終了してください。詳細: {message}",
    ...
    jdbcPlugin: "Chiron Horizon JDBCプラグイン",
    ...
    mcpTitle: "MCPサーバー",
    mcpDescription: "Claude Code、Cursor、その他のエージェントが使用するChiron Horizon MCPサーバーのインストールとバージョンを確認します。",
    mcpChecking: "確認中",
    ...
    mcpConfig: "MCP設定",
    mcpConfigOptionsHint: "接続範囲と実行権限はChiron HorizonがすべてのMCPクライアントに一元適用します。生成される設定にはMCP Serverの起動と現在のChiron Horizonへの接続に必要な実行パラメーターのみが含まれ、権限や接続範囲の変数は含まれません。",
    mcpScopeConnection: "MCPに許可する接続",
    mcpScopeConnectionDescription: "Chiron Horizonは選択した接続だけをMCPに公開します。変更はすべてのMCPクライアントに適用されます。",
    ...
    mcpExecutionModeReadOnlyDescription: "Chiron Horizonが読み取りと判定したリクエストのみを許可し、識別できるデータベース書き込みと接続変更を拒否します。データベースアカウントの権限が最終的な安全境界です。",
    ...
    mcpCapabilityDescription: "代表的な操作を示しています。Chiron Horizonが確実に分類できないリクエストにはフルアクセスが必要です。",
    ...
    mcpDetectionTiming: "このページを開くとChiron Horizonが自動的に確認します。更新するには再確認を使用してください。",
    mcpNpmBoundary: "Chiron HorizonはMCPステータスの確認と説明のみを行います。インストールとアップグレードはnpmを通じて実行されます。",
    mcpRefresh: "再確認",
    mcpGuide: "MCPガイド",
    ...
    mcpDatabaseScopePermissionNote: "範囲を選択しても、Chiron Horizon 内のこの接続の既存のデータベース権限は変更されません。",
    ...
    mcpHttpServiceDescription: "既定では無効です。有効にすると Chiron Horizon Desktop がサービスをホストし、保存後には上記の設定が実際のリスニングサービスに適用されます。",
    ...
    mcpHttpErrorPortInUse: "このポートは他のアプリケーションで使用されています。Chiron Horizon は保存時に自身の MCP サービスを再起動します。ポートを使用している他のアプリケーションを終了するか、別のポートを使用してください。",
    ...
    mcpToolOpenTable: "Chiron Horizon でテーブルを開く",
    mcpToolExecuteAndShow: "Chiron Horizon で実行して表示",
    ...
    supportInfoAppVersion: "Chiron Horizonバージョン",
    ...
    syncSnippetDescription: "同一の Chiron Horizon スナップショットを非公開スニペットに保存します。初回アップロード時にスニペットが自動作成され、ID が保存されます。",
    ...
    syncSnippetMigrateLegacyConfirm: "Chiron Horizon はまず暗号化された置換スニペットを作成し、その後、旧式の平文スニペット {id} を完全に削除します。この操作は取り消せません。続行しますか？",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows:
      "古い JRE ディレクトリを削除できませんでした: {path}\n考えられる原因:\n  - chiron_horizon Agent / java プロセスがディレクトリを使用中です\n  - ウイルス対策ソフトがスキャン中です\nディレクトリを使用している可能性のあるプロセスを終了するか、chiron_horizon を再起動して再試行してください。\n（元のエラー: {error}）",
    ...
    offlineExportDescription: "含めるChiron Horizon管理のAgentドライバーを選択します。パッケージは現在のプラットフォームでのみ使用できます。",
    ...
    offlineExportNoDrivers: "エクスポート可能なChiron Horizon管理のAgentドライバーがインストールされていません。",
    ...
    offlineExportSecurityHint: "Chiron Horizon管理の成果物と必要なJREのみを含めます。ローカルパス、設定、外部ドライバーは除外されます。",
    ...
    offlineExportUnavailable_unmanagedInstall: "この成果物はChiron Horizon管理のインストールとして記録されていません。",
    ...
    offlineExportUnavailable_missingManagedJre: "必要なChiron Horizon管理JREがインストールされていません。",
    offlineExportUnavailable_invalidManagedJre: "必要なChiron Horizon管理JREが無効または安全ではありません。",
    refresh: "更新",
    javaRuntime: "Javaランタイム",
    javaRuntimeManaged: "Chiron Horizon管理JRE",
    ...
    localInstallHint:
      "完全に隔離されたネットワーク環境の場合：インターネットに接続できる別のコンピューターで Chiron Horizon の GitHub Releases ページから公式の chiron-horizon-jdbc-plugin-*.zip をダウンロードし、このマシンに転送して選択してください。このマシンはネットワークに接続する必要はありません。",
    runtimeDrivers: "ランタイム",
    pluginsTab: "プラグイン",
    pluginContributionsTitle: "フロントエンドプラグインコントリビューション",
    pluginContributionsDescription: "インストール済みプラグインは、Chiron Horizon のメインウィンドウにコードを注入せず、接続フォームやその他の UI コントリビューションを記述できます。",
  ...
  nacos: {
    ...
    namespaceId: "名前空間ID",
    namespaceIdPlaceholder: "空欄の場合はChiron HorizonがUUIDを生成",
    namespaceName: "名前空間名",
    ...
    nacosDetectAccessibleNamespacesHint: "Chiron Horizon は {name} の名前空間を提示する前に、設定とサービスの読み取りアクセスを検証します。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "この接続は Nacos 3 Console API（通常はポート 8080）を使用します。名前空間名または ID を入力できます。ディレクトリを読み取れる場合、Chiron Horizon は名前を実際の ID に変換します。",
    ...
    persistentInstanceHint: "Chiron Horizon はハートビートを維持できないため、一時インスタンスを登録しません。",
    ipAddress: "IP アドレス",
    port: "ポート",
  ...
  databaseBackup: {
    ...
    runtimeRequirement: "Chiron Horizonが開いている状態（トレイに最小化したときも含む）で、定期バックアップが実行されます。",
    ...
    fileNamePatternHint: "{'{'}schedule{'}'}、{'{'}date{'}'}、{'{'}timestamp{'}'}、{'{'}database{'}'}、{'{'}runId{'}'} を使用できます。Chiron Horizon は不足している {'{'}database{'}'} を追加し、拡張子も自動で追加します。",
  ...
  mqExchanges: {
    ...
    virtualHost: "仮想ホスト",
    namePlaceholder: "例: chiron-horizon-events",
    cancel: "キャンセル",
    create: "作成",
    bind: "バインド",
    bindDialogTitle: "{name} にバインド",
    queueNamePlaceholder: "例: chiron-horizon-queue",
    exchangeNamePlaceholder: "例: chiron-horizon-events",
