...
export default withEnglishFallback({
  ...
  pluginPlatform: {
    ...
    centerDescription: "統合マーケットプレイスで Gauss Horizon の追加機能を探し、インストール済みプラグインとリポジトリを個別に管理します。",
    ...
    localInstallDescription: "マーケットプレイス外で入手した署名済み .gauss-horizonp をインストールします。Gauss Horizon が整合性とリポジトリ署名を自動検証します。",
    repositoriesTitle: "プラグインリポジトリ",
    repositoriesDescription: "公式ソースは Gauss Horizon が管理します。セルフホストまたは第三者のリポジトリでは、リポジトリの信頼設定を使用します。",
    ...
    allowUnsignedDevelopmentPackageDescription: "手動での .gauss-horizonp インストールにのみ適用され、公式マーケットプレイスの検証は緩和されません。",
    installPackage: ".gauss-horizonp をインストール",
    dropToInstall: "ドロップしてプラグインをインストール",
    dropInstallHint: ".gauss-horizonp ファイルをこのページにドロップしてもインストールできます",
    packageFileType: "Gauss Horizon プラグインパッケージ",
    ...
    noInstalledPluginsDescription: "署名済み .gauss-horizonp をインストールするか、ローカル開発時のみ未署名パッケージを明示的に許可してください。",
    compatible: "互換",
    blocked: "ブロック済み",
    rollback: "ロールバック",
    uninstall: "アンインストール",
  ...
  externalSqlFile: {
    modifiedTitle: "ファイルがGauss Horizonの外部で変更されました",
    modifiedDescription: "{path} は別のプログラムによって変更されました。保持するバージョンを選択してください。",
    deletedTitle: "ファイルが存在しません",
    deletedDescription: "{path} はGauss Horizonの外部で削除または移動されました。エディターの内容は保持されています。",
  ...
  updates: {
    ...
    availableMessage: "Gauss Horizon {latest} が利用可能です。現在のバージョンは {current} です。",
    upToDate: "Gauss Horizon は最新です ({version})。",
    ...
    downloadStarted: "Gauss Horizon {version} をバックグラウンドでダウンロード中です。",
    activeTasksBlockUpdate: "{count} 件のタスクが実行中です。完了してから Gauss Horizon を更新してください。",
    portableAutomaticUpdate: "Gauss Horizon は署名済みのポータブル ZIP をダウンロードし、終了後に Gauss Horizon.exe のみを置き換えて自動的に再起動します。portable.gauss-horizon とデータは保持されます。",
    ...
    readyToInstall: "Gauss Horizon {version} のインストール準備ができました。",
    installNow: "今すぐインストール",
    installedReadyToRestart: "Gauss Horizon が更新されました。再起動して完了してください。",
  ...
  sidebar: {
    ...
    import: "接続をインポート",
    importGaussHorizon: "Gauss Horizon設定をインポート",
  ...
  sqlLibrary: {
    title: "SQLライブラリ",
    storageHelp: "SQLライブラリの内容はGauss Horizonのアプリデータに保存され、ディスク上の独立した.sqlファイルではありません。\nファイルシステムで使用するには、SQLライブラリをエクスポートしてください。",
    unassociated: "未関連付け",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "接続しましたが、Gauss Horizon はパスワードを保存できませんでした: {message}",
    ...
    oracleTnsPathHint: "このパスは Gauss Horizon バックエンド/Oracle Agent が読み取ります。デスクトップではローカルパス、Web/Docker ではサーバーパスを指定してください。",
    ...
    nacosWebConsoleUrlHint: "任意。ブラウザーで Nacos コンソールを開きます。Nacos 3 では専用の Web コンソールアドレスが必要です。ブラウザーから直接アクセスできる必要があり、Gauss Horizon のトンネルや API 認証は再利用されません。",
    ...
    gaussdbConnectionModeNativeHint: "GaussDB の A/PG/Oracle 互換モードでは、Gauss Horizon 内蔵の PostgreSQL 互換プロトコルを使用します。",
    ...
    agentJavaTooOld: "このドライバーにはJava 21が必要です。ドライバーマネージャーでGauss Horizon管理JRE 21を使用するか、Java 21実行ファイルを選択してください。",
    ...
    jdbcPluginHint: "最初にGauss Horizon JDBCプラグインをインストールし、次にデータベースベンダーのJDBCドライバーJARをインポートしてください。",
    ...
    kafkaKerberosKeytabPlaceholder: "Gauss Horizon Agent が動作するマシンのパス。例: /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "オプション。Gauss Horizon Agent が動作するマシンのパス。例: /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab と krb5.conf のパスは Gauss Horizon Agent が読み取るため、Gauss Horizon Agent が動作するマシン上に存在する必要があります。ブラウザからファイルをアップロードすることはありません。",
    ...
    sqliteWorkerPlacementSessionHint: "Gauss Horizon が SSH 経由でワーカーをファイルホストにアップロードし、切断時にリモートファイルを削除します。一時的な利用に適しています。",
    sqliteWorkerPlacementPersistHint: "Gauss Horizon が SSH 経由でワーカーをファイルホストにアップロードし、切断後もそのまま残します。自動的に削除されることはありません。",
    sqliteWorkerPlacementPreplacedHint: "ワーカーを事前にファイルホストへアップロードしてから、その絶対パスを入力してください。Gauss Horizon は自動でアップロード・削除しませんが、ファイルがこの Desktop バージョンと一致するかを検証します。",
    ...
    sshWorkerUploadConsentMessage: "Gauss Horizon は短命の SQLite ワーカーを {host}:{port} にコピーする必要があります。これは特権操作です。許可する前に宛先パスとダイジェストを確認してください。",
    ...
    httpTunnelTokenPlaceholder: "gauss_horizon_tunnel.php で設定された Token",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」に未保存の SQL 変更があり、Gauss Horizon の終了後に失われます。保存してから終了しますか？",
    unsavedChangesAppCloseMultipleMessage: "{count} 個の SQL タブが未保存です。現在「{title}」を表示中です。Gauss Horizon の終了後に失われます。保存してから終了しますか？",
  ...
  tabs: {
    ...
    externalFileMissing: "Gauss Horizonの外部で削除または移動されました",
  ...
  grid: {
    ...
    conditionalBulkEditConfirmMessage: "実行時に条件へ一致するすべての行（未読込の行を含む）を直ちに更新します。Gauss Horizon は信頼できるロールバック SQL を生成できません。",
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "Gauss Horizon はこの結果に対して安全に書き戻せる対象テーブルを 1 つに特定できませんでした。",
      ...
      "metadata-unavailable": "Gauss Horizonがテーブルメタデータを読み込めなかったため、結果の編集は無効です。",
    },
    ...
    esDeepPageJumpConfirmMessage:
      "Elasticsearch の search_after では指定ページへ直接移動できません。Gauss Horizon は不足しているカーソルを順番に取得し、前のページへ戻る場合は 1 ページ目からやり直します。ページ {page} への移動では約 {requests} 回のクエリが送信される見込みです。時間がかかり、クラスター負荷が増える可能性があります。続行しますか？",
  ...
  configExport: {
    ...
    cryptoUnavailable: "暗号化設定のインポートはこのブラウザ環境では利用できません。HTTPS、localhost、またはGauss Horizon Desktopを使用してください。",
  ...
  ai: {
    ...
    codexMcpRequiredTitle: "Gauss Horizon MCP Serverが必要です",
    codexMcpRequiredDescription: "Codex CLIはGauss Horizon MCP Server経由でデータベーススキーマとクエリツールにアクセスします。AIアシスタントでCodexを使う前にインストールしてください。",
    ...
    codexCliEnvReservedName: "{name}はMCP Server用にGauss Horizonが管理しているため、ここでは設定できません。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "Gauss Horizonが管理するMCP環境変数が上書きされています。プロバイダー設定からGAUSS_HORIZON_MCP_*変数を削除してください。",
      claudeCodeNotAuthenticated: "Claude Code CLIにログインしていません。ターミナルで`claude auth login`を実行してから再試行してください。",
      claudeCodeMcpConfigInvalid: "Claude CodeがGauss HorizonのMCP設定を読み取りまたは解析できません。Claude Code CLIのバージョンと実行ファイルのパスを確認してください。",
      gaussHorizonMcpMissing: "互換性のあるGauss Horizon MCP Serverが見つかりません。設定 > MCPで確認し、再インストールまたは更新してください。",
      claudeCodeMcpStartupFailed: "Claude CodeはMCP設定を読み込みましたが、Gauss Horizon MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      claudeCodeCommandLineTooLong: "Windowsが長すぎるClaude Codeコマンドを拒否しました。Gauss Horizonを更新し、生成されたMCP設定ファイルで再試行してください。",
      ...
      piAgentEnvReserved: "Gauss Horizonが管理するPiまたはMCP環境変数が上書きされています。GAUSS_HORIZON_PI_*とGAUSS_HORIZON_MCP_*を削除してください。",
      piAgentNotAuthenticated: "Pi Coding Agentに認証済みのモデルプロバイダーがありません。Piでプロバイダーを設定またはログインしてください。",
      piAgentMcpStartupFailed: "PiはGauss Horizon MCPブリッジを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      openCodeEnvReserved: "OpenCodeまたはGauss Horizonが管理する環境変数が上書きされています。設定からOPENCODE制御変数とGAUSS_HORIZON_MCP_*を削除してください。",
      openCodeNotAuthenticated: "OpenCodeに認証済みのモデルプロバイダーがありません。`opencode auth login`を実行するかOpenCodeでプロバイダーを設定して、再試行してください。",
      openCodeMcpStartupFailed: "OpenCodeはスコープ付きGauss Horizon MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      cursorEnvReserved: "CursorまたはGauss Horizonが管理する環境変数が上書きされています。CURSOR_CONFIG_DIR、CURSOR_DATA_DIR、GAUSS_HORIZON_MCP_*を設定から削除してください。",
      cursorNotAuthenticated: "Cursor CLIにログインしていません。ターミナルで`agent login`を実行してから再試行してください。",
      cursorMcpStartupFailed: "Cursorはスコープ付きGauss Horizon MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      grokCliEnvReserved: "Gauss Horizonが管理するMCP環境変数が上書きされています。プロバイダー設定からGAUSS_HORIZON_MCP_*変数を削除してください。",
      grokCliNotAuthenticated: "Grok CLIにログインしていません。ターミナルで`grok login`を実行してから再試行してください。",
      grokCliMcpStartupFailed: "GrokはMCP設定を読み込みましたが、Gauss Horizon MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      grokCliCommandLineTooLong: "Windowsが長すぎるGrok CLIコマンドを拒否しました。Gauss Horizonを更新し、生成されたプロンプトファイルで再試行してください。",
      ...
      codeBuddyEnvReserved: "Gauss Horizonが管理するMCP環境変数が上書きされています。プロバイダー設定からGAUSS_HORIZON_MCP_*変数を削除してください。",
      ...
      codeBuddyMcpConfigInvalid: "CodeBuddy Codeは一時的なGauss Horizon MCP設定を読み込めませんでした。以下の診断情報を確認してください。",
      codeBuddyMcpStartupFailed: "CodeBuddy CodeはMCP設定を読み込みましたが、Gauss Horizon MCP Serverを起動できません。設定 > MCPと以下の診断情報を確認してください。",
      ...
      qoderEnvReserved: "Gauss Horizonが管理するMCP環境変数が上書きされました。プロバイダー設定からGAUSS_HORIZON_MCP_*変数を削除してください。",
      ...
      qoderMcpConfigInvalid: "Qoder CLIが一時的なGauss Horizon MCP設定を読み取れませんでした。Qoder CLIのバージョンと以下の診断詳細を確認してください。",
      qoderMcpStartupFailed: "Qoder CLIはMCP設定を読み込みましたが、Gauss Horizon MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
    ...
    closeWithRunsDescription: "Gauss Horizonを終了すると、実行中またはキューに入っている{count}件のAIタスクが中断されます。このダイアログを閉じて完了を待つこともできます。",
    ...
    productionWriteBlocked: "AI は本番データベースでデータまたはスキーマの変更を実行できません。この SQL を確認し、Gauss Horizon で手動実行してください。",
    ...
    htmlCopyRiskBody: "このHTMLはAIによって生成されました。Gauss Horizonの外部で開くとスクリプトが実行され、ネットワークへアクセスする可能性があります。",
    htmlCopyRiskAccept: "それでもコピー",
    htmlCopyRiskRemember: "このセッションでは今後確認しない",
    htmlCopyRiskToast: "コピーしました。このAI生成HTMLは、Gauss Horizonの外部で開くとスクリプトを実行したりネットワークへアクセスしたりする可能性があります。",
    ...
    customHeadersReserved: "このヘッダーは Gauss Horizon により管理されています: {name}",
    enableThinking: "思考",
    enableThinkingOn: "有効",
  ...
  contextMenu: {
    ...
    parameterDefaultHint: "DEFAULTがチェックされている場合、Gauss Horizonはその引数を省略し、データベースがプロシージャのデフォルト値を使用します。",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "クラスタ状態は引き続き利用できます。Gauss Horizon は現在の etcd エンドポイントへ直接 /metrics を要求するため、生成された URL に Gauss Horizon アプリケーションから到達できることを確認してください。",
  ...
  sqlParameters: {
    title: "SQLパラメーター",
    description: "SQLテンプレートのプレースホルダーに値を入力します。Gauss Horizonは実行前にSQLリテラルへ置換します。",
    name: "パラメーター",
    syntax: "Syntax",
    type: "型",
    value: "値",
  ...
  settings: {
    ...
    uiFontAppDefault: "Gauss Horizon デフォルト",
    ...
    iconThemeDefaultDescription: "カラーGauss Horizonアプリアイコンを使用。",
    iconThemeBlack: "黒",
    iconThemeBlackDescription: "アプリ内で黒いGauss Horizonロゴを使用。",
    iconThemeBlackDescriptionMac: "アプリ内は黒いロゴ、Dockは黒い背景、メニューバーはシステムに従います。",
    showTrayIcon: "トレイ/メニューバーアイコンを表示",
    showTrayIconDescription: "無効時はアイコンが表示されませんが、ウィンドウを閉じると従来通りGauss Horizonはバックグラウンドに隠れます。",
    quitOnClose: "ウィンドウを閉じるときに終了",
    quitOnCloseDescription: "有効にすると、閉じるボタンでトレイに隠すのではなく、Gauss Horizonを完全に終了します。",
    closeActionPromptTitle: "ウィンドウを閉じる",
    closeActionPromptDescription: "閉じるボタンの動作を選択してください。後から「設定 → 外観」で変更できます。",
    closeActionQuit: "Gauss Horizonを終了",
    ...
    duckDbWorkerProcessIsolationDescription: "DuckDB クエリを独立した worker プロセスで実行します。長時間クエリがキャンセルに応答しない場合、Gauss Horizon は worker プロセスを停止してデータベースファイルのロックを解放できます。",
    duckDbWorkerMaxProcesses: "DuckDB の最大同時 worker 数",
    duckDbWorkerMaxProcessesDescription: "Gauss Horizon が同時に実行できる DuckDB worker プロセス数を制限します。範囲: 1-16。既定値: 4。",
    duckDbWorkerProcessIsolationRestartRequired: "この変更を有効にするには Gauss Horizon を再起動してください。",
    restartGaussHorizon: "Gauss Horizon を再起動",
    restartGaussHorizonFailed: "Gauss Horizon の再起動に失敗しました: {error}",
    ...
    openTabsRestoreModeDescription: "Gauss Horizon 起動時に前回開いていたタブをどのように復元するかを選択します。",
    ...
    syncWebDavDescription: "保存済みSQLライブラリを含むGauss HorizonスナップショットをWebDAV互換ストレージへアップロードまたは復元します。",
    syncWebDavWebDescription: "WebモードではGauss HorizonサーバーがWebDAVへ接続するため、ブラウザーのCORS制限を受けません。保存したパスワードは現在のGauss Horizonインスタンス内に保持されます。",
    ...
    syncRememberWebDavPasswordDescription: "パスワードは暗号化され、現在のGauss Horizonインスタンスにのみ保存されます。WebDAVには同期されず、同期パスワードを置き換えるものでもありません。",
    syncRemotePath: "リモートスナップショットパス",
    syncRemotePathDescription: "アップロード時に不足している親フォルダをGauss Horizonが作成します。",
    syncSecretNotice: "デフォルトでは、Gauss Horizonは接続詳細と設定のみを同期します。データベースパスワード、SSHパスワード、プロキシパスワード、接続文字列、AI APIキーはローカルに保持されます。",
    syncSecrets: "暗号化されたシークレットを同期",
    syncSecretsDescription: "有効時、Gauss Horizonはデータベースパスワード、SSHパスワード、AI APIキーを暗号化してからWebDAVにアップロードします。",
    syncSecretsPassphrase: "同期パスワード",
    syncSecretsPassphraseDescription: "このパスワードは機密データの暗号化と復元にのみ使用され、現在のGauss Horizonインスタンスに暗号化して保存されます。他のインスタンスでは再入力してください。",
    syncTest: "テスト",
    syncUpload: "アップロード",
    syncDownload: "ダウンロード",
    ...
    syncAutoUploadDescription: "Gauss Horizonを開いている間だけ実行されます。ローカルの保存済みSQLを誤って置き換えないよう、ダウンロードは手動確認が必要です。",
    ...
    syncDownloadConfirm: "リモートGauss Horizonスナップショットをダウンロードして適用しますか？ローカルのメタデータと保存済みSQLが置き換えられます。",
    apply: "適用",
    ...
    appCloseUnsavedTabsModeDescription: "未保存の編集があるSQLタブが開いたままGauss Horizonを終了するときの動作を選択します。",
    appCloseUnsavedTabsModePrompt: "保存するかどうか確認する",
    appCloseUnsavedTabsModeKeepDrafts: "確認せず終了して下書きを保持",
    appCloseUnsavedTabsModeHint: "下書きは未保存のタブとして保持され、次回Gauss Horizonを起動したときに復元されます。「起動時にタブを復元」が「タブを保持しない」になっていない必要があります。",
    appCloseDraftPersistFailed: "開いているタブの下書きを保存できなかったため、Gauss Horizon の終了を中止しました。ディスク容量とファイル権限を確認してから、もう一度終了してください。詳細: {message}",
    ...
    jdbcPlugin: "Gauss Horizon JDBCプラグイン",
    ...
    mcpTitle: "MCPサーバー",
    mcpDescription: "Claude Code、Cursor、その他のエージェントが使用するGauss Horizon MCPサーバーのインストールとバージョンを確認します。",
    mcpChecking: "確認中",
    ...
    mcpConfig: "MCP設定",
    mcpConfigOptionsHint: "接続範囲と実行権限はGauss HorizonがすべてのMCPクライアントに一元適用します。生成される設定にはMCP Serverの起動と現在のGauss Horizonへの接続に必要な実行パラメーターのみが含まれ、権限や接続範囲の変数は含まれません。",
    mcpScopeConnection: "MCPに許可する接続",
    mcpScopeConnectionDescription: "Gauss Horizonは選択した接続だけをMCPに公開します。変更はすべてのMCPクライアントに適用されます。",
    ...
    mcpExecutionModeReadOnlyDescription: "Gauss Horizonが読み取りと判定したリクエストのみを許可し、識別できるデータベース書き込みと接続変更を拒否します。データベースアカウントの権限が最終的な安全境界です。",
    ...
    mcpCapabilityDescription: "代表的な操作を示しています。Gauss Horizonが確実に分類できないリクエストにはフルアクセスが必要です。",
    ...
    mcpDetectionTiming: "このページを開くとGauss Horizonが自動的に確認します。更新するには再確認を使用してください。",
    mcpNpmBoundary: "Gauss HorizonはMCPステータスの確認と説明のみを行います。インストールとアップグレードはnpmを通じて実行されます。",
    mcpRefresh: "再確認",
    mcpGuide: "MCPガイド",
    ...
    mcpDatabaseScopePermissionNote: "範囲を選択しても、Gauss Horizon 内のこの接続の既存のデータベース権限は変更されません。",
    ...
    mcpHttpServiceDescription: "既定では無効です。有効にすると Gauss Horizon Desktop がサービスをホストし、保存後には上記の設定が実際のリスニングサービスに適用されます。",
    ...
    mcpHttpErrorPortInUse: "このポートは他のアプリケーションで使用されています。Gauss Horizon は保存時に自身の MCP サービスを再起動します。ポートを使用している他のアプリケーションを終了するか、別のポートを使用してください。",
    ...
    mcpToolOpenTable: "Gauss Horizon でテーブルを開く",
    mcpToolExecuteAndShow: "Gauss Horizon で実行して表示",
    ...
    supportInfoAppVersion: "Gauss Horizonバージョン",
    ...
    syncSnippetDescription: "同一の Gauss Horizon スナップショットを非公開スニペットに保存します。初回アップロード時にスニペットが自動作成され、ID が保存されます。",
    ...
    syncSnippetMigrateLegacyConfirm: "Gauss Horizon はまず暗号化された置換スニペットを作成し、その後、旧式の平文スニペット {id} を完全に削除します。この操作は取り消せません。続行しますか？",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows:
      "古い JRE ディレクトリを削除できませんでした: {path}\n考えられる原因:\n  - gauss-horizon Agent / java プロセスがディレクトリを使用中です\n  - ウイルス対策ソフトがスキャン中です\nディレクトリを使用している可能性のあるプロセスを終了するか、gauss-horizon を再起動して再試行してください。\n（元のエラー: {error}）",
    ...
    offlineExportDescription: "含めるGauss Horizon管理のAgentドライバーを選択します。パッケージは現在のプラットフォームでのみ使用できます。",
    ...
    offlineExportNoDrivers: "エクスポート可能なGauss Horizon管理のAgentドライバーがインストールされていません。",
    ...
    offlineExportSecurityHint: "Gauss Horizon管理の成果物と必要なJREのみを含めます。ローカルパス、設定、外部ドライバーは除外されます。",
    ...
    offlineExportUnavailable_unmanagedInstall: "この成果物はGauss Horizon管理のインストールとして記録されていません。",
    ...
    offlineExportUnavailable_missingManagedJre: "必要なGauss Horizon管理JREがインストールされていません。",
    offlineExportUnavailable_invalidManagedJre: "必要なGauss Horizon管理JREが無効または安全ではありません。",
    refresh: "更新",
    javaRuntime: "Javaランタイム",
    javaRuntimeManaged: "Gauss Horizon管理JRE",
    ...
    localInstallHint:
      "完全に隔離されたネットワーク環境の場合：インターネットに接続できる別のコンピューターで Gauss Horizon の GitHub Releases ページから公式の gauss-horizon-jdbc-plugin-*.zip をダウンロードし、このマシンに転送して選択してください。このマシンはネットワークに接続する必要はありません。",
    runtimeDrivers: "ランタイム",
    pluginsTab: "プラグイン",
    pluginContributionsTitle: "フロントエンドプラグインコントリビューション",
    pluginContributionsDescription: "インストール済みプラグインは、Gauss Horizon のメインウィンドウにコードを注入せず、接続フォームやその他の UI コントリビューションを記述できます。",
  ...
  nacos: {
    ...
    namespaceId: "名前空間ID",
    namespaceIdPlaceholder: "空欄の場合はGauss HorizonがUUIDを生成",
    namespaceName: "名前空間名",
    ...
    nacosDetectAccessibleNamespacesHint: "Gauss Horizon は {name} の名前空間を提示する前に、設定とサービスの読み取りアクセスを検証します。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "この接続は Nacos 3 Console API（通常はポート 8080）を使用します。名前空間名または ID を入力できます。ディレクトリを読み取れる場合、Gauss Horizon は名前を実際の ID に変換します。",
    ...
    persistentInstanceHint: "Gauss Horizon はハートビートを維持できないため、一時インスタンスを登録しません。",
    ipAddress: "IP アドレス",
    port: "ポート",
  ...
  databaseBackup: {
    ...
    runtimeRequirement: "Gauss Horizonが開いている状態（トレイに最小化したときも含む）で、定期バックアップが実行されます。",
    ...
    fileNamePatternHint: "{'{'}schedule{'}'}、{'{'}date{'}'}、{'{'}timestamp{'}'}、{'{'}database{'}'}、{'{'}runId{'}'} を使用できます。Gauss Horizon は不足している {'{'}database{'}'} を追加し、拡張子も自動で追加します。",
  ...
  mqExchanges: {
    ...
    virtualHost: "仮想ホスト",
    namePlaceholder: "例: gauss-horizon-events",
    cancel: "キャンセル",
    create: "作成",
    bind: "バインド",
    bindDialogTitle: "{name} にバインド",
    queueNamePlaceholder: "例: gauss-horizon-queue",
    exchangeNamePlaceholder: "例: gauss-horizon-events",
