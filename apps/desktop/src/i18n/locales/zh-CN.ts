...
export default withEnglishFallback({
  ...
  pluginPlatform: {
    ...
    centerTitle: "插件中心",
    centerDescription: "在统一商店中发现可选的 Chiron Horizon 能力，并分别管理已安装插件和插件仓库。",
    marketplace: "插件商店",
    installed: "已安装",
    settings: "设置",
    ...
    marketplaceGuideDescription: "插件问题或功能建议请通过插件卡片上的源码仓库入口，前往对应插件仓库提交 Issue；Chiron Horizon 主仓库仅受理应用本身的问题。",
    ...
    localInstallDescription: "从本地文件或 URL 安装插件商店之外的 .chiron-horizonp 插件包，Chiron Horizon 会自动校验包完整性和签名。",
    repositoriesTitle: "插件仓库",
    repositoriesDescription: "官方源由 Chiron Horizon 管理；添加自托管或第三方仓库后，才会显示对应的仓库信任设置。",
    ...
    pluginDevelopmentDocsDescription: "了解如何开发、打包并向插件商店发布 Chiron Horizon 插件。",
    ...
    allowUnsignedDevelopmentPackageDescription: "仅影响手动安装的 .chiron-horizonp，不会放宽官方插件商店的签名校验。",
    installPackage: "安装 .chiron-horizonp",
    ...
    dropInstallHint: "也可拖放 .chiron-horizonp 到插件中心页面安装",
    packageFileType: "Chiron Horizon 插件包",
    ...
    noInstalledPluginsDescription: "安装已签名的 .chiron-horizonp，或仅在本地开发时明确允许未签名包。",
    compatible: "兼容",
    blocked: "已阻止",
    rollback: "回滚",
    uninstall: "卸载",
  ...
  externalSqlFile: {
    ...
    deletedTitle: "文件已不存在",
    deletedDescription: "{path} 已在 Chiron Horizon 外部被删除或移动，编辑器中的内容仍然保留。",
  ...
  updates: {
    ...
    availableMessage: "Chiron Horizon {latest} 已发布，当前版本为 {current}。",
    upToDate: "Chiron Horizon 已是最新版本 ({version})。",
    failed: "检查更新失败：{error}",
    ...
    cancelDownload: "取消下载",
    downloadStarted: "正在后台下载 Chiron Horizon {version}。",
    activeTasksBlockUpdate: "有 {count} 个任务正在执行，请等待任务完成后再更新 Chiron Horizon。",
    portableAutomaticUpdate: "Chiron Horizon 将下载已签名的便携版 ZIP，退出后仅替换 Chiron Horizon.exe 并自动重启。portable.chiron_horizon 和 data 会保留。",
    ...
    readyToInstall: "Chiron Horizon {version} 已下载完成，可以安装了。",
    installNow: "立即安装",
    installedReadyToRestart: "Chiron Horizon 已更新完成，重启后生效。",
  ...
  sidebar: {
    ...
    importExport: "导入/导出",
    import: "导入连接",
    importChironHorizon: "导入 Chiron Horizon 配置",
  ...
  sqlLibrary: {
    title: "SQL 库",
    storageHelp: "SQL 库内容保存在 Chiron Horizon 应用数据中，并非磁盘上的独立 .sql 文件。\n如需在文件系统中使用，请导出 SQL 库。",
    unassociated: "未关联",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "连接成功，但 Chiron Horizon 无法记住密码：{message}",
    ...
    oracleTnsPathHint: "该路径由 Chiron Horizon 后端/Oracle Agent 读取。桌面端填写本机路径，Web 和 Docker 部署填写服务器路径。",
    ...
    nacosWebConsoleUrlHint: "可选，用于在浏览器打开 Nacos 控制台；Nacos 3 需填写独立的 Web 控制台地址。浏览器需可直接访问，且不会复用 Chiron Horizon 的隧道或 API 认证。",
    ...
    kafkaKerberosKeytabPlaceholder: "Chiron Horizon Agent 所在机器路径，例如 /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "可选，Chiron Horizon Agent 所在机器路径，例如 /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab 和 krb5.conf 路径由 Chiron Horizon Agent 读取，必须存在于运行 Chiron Horizon Agent 的机器上；不会从当前浏览器上传文件。",
    ...
    gaussdbConnectionModeNativeHint: "使用 Chiron Horizon 内置的 PostgreSQL 兼容协议，适用于 GaussDB 的 A/PG/Oracle 兼容模式。",
    ...
    agentJavaTooOld: "该驱动需要 Java 21。请在驱动管理器中使用 Chiron Horizon 托管 JRE 21，或选择 Java 21 可执行文件。",
    ...
    sqliteWorkerPlacementSessionHint: "由 Chiron Horizon 经 SSH 把 Worker 上传到文件主机，断开连接后自动删除远端文件。适合大多数临时使用场景。",
    sqliteWorkerPlacementPersistHint: "由 Chiron Horizon 经 SSH 把 Worker 上传到文件主机，断开后仍保留在远端，不会自动删除。",
    sqliteWorkerPlacementPreplacedHint: "需要你先把 Worker 手动上传到文件主机，再填写该绝对路径。Chiron Horizon 不会代为上传或删除，但会校验文件摘要是否与当前 Desktop 版本一致。",
    ...
    sshWorkerUploadConsentMessage: "Chiron Horizon 需要把短生命周期的 SQLite worker 复制到 {host}:{port}。这是特权操作。请先核对目标路径和摘要。",
    ...
    httpTunnelTokenPlaceholder: "chiron_horizon_tunnel.php 中配置的 token",
    ...
    jdbcDocs: "查看 JDBC 文档",
    jdbcPluginHint: "先安装 Chiron Horizon JDBC 插件。有外部依赖的驱动请从驱动管理使用 Maven 安装，或同时导入驱动及全部依赖 JAR。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」有未保存的 SQL 更改，退出 Chiron Horizon 后将丢失。是否保存后再退出？",
    unsavedChangesAppCloseMultipleMessage: "当前有 {count} 个 SQL 标签未保存，当前已定位到「{title}」。退出 Chiron Horizon 后将丢失，是否保存后再退出？",
  ...
  tabs: {
    ...
    externalFileMissing: "已在 Chiron Horizon 外部删除或移动",
  ...
  grid: {
    ...
    esDeepPageJumpConfirmMessage: "Elasticsearch 的 search_after 不能直接定位到指定页面。Chiron Horizon 必须按顺序获取缺失的游标，跳回较小页码时还会从第 1 页重新开始。跳转到第 {page} 页预计会发送 {requests} 次查询，可能耗时较长并增加集群负载。是否继续？",
    ...
    conditionalBulkEditConfirmMessage: "该操作会立即更新执行时条件匹配的全部行，包括尚未加载的行；Chiron Horizon 无法为这些行生成可靠的回滚 SQL。",
  ...
  welcome: {
    title: "数据库工作台",
    subtitle: "选择左侧连接开始浏览结构，或直接创建查询标签页。Chiron Horizon 支持多种数据库、查询历史、数据表格编辑和 AI SQL 助手。",
    connections: "连接",
    connected: "已连接",
    databaseTypes: "数据库类型",
  ...
  backendErrors: {
    ...
    transaction: {
      sessionExpired: "手动事务因 {timeoutSecs} 秒无操作已过期并回滚，Chiron Horizon 将自动开始新事务。",
    },
    legacy: "后端返回了错误。",
    unknown: "发生了未预期的后端错误。",
  },
  ...
  configExport: {
    ...
    cryptoUnavailable: "当前浏览器环境不支持加密配置导入，请使用 HTTPS、localhost 或 Chiron Horizon 桌面版。",
  ...
  ai: {
    ...
    closeWithRunsDescription: "退出 Chiron Horizon 会中断 {count} 个正在运行或排队的 AI 任务。你也可以取消退出并等待任务完成。",
    ...
    productionWriteBlocked: "AI 不能在生产数据库执行数据或结构变更。请核对此 SQL 后在 Chiron Horizon 中手动执行。",
    ...
    customHeadersReserved: "该请求头由 Chiron Horizon 管理：{name}",
    ...
    codexCliEnvReservedName: "{name} 由 Chiron Horizon 用于 MCP Server，不能在这里设置。",
    cliMcpRequiredTitle: "需要 Chiron Horizon MCP Server",
    cliMcpRequiredDescription: "{provider} 会通过 Chiron Horizon MCP Server 访问数据库结构和查询工具。请先安装后再在 AI 助手中使用该提供商。",
    cliPath: "{provider} 路径",
    ...
    cliEnvReservedName: "{name} 由 Chiron Horizon 用于 MCP Server，不能在这里设置。",
    codexMcpRequiredTitle: "需要 Chiron Horizon MCP Server",
    codexMcpRequiredDescription: "Codex CLI 通过 Chiron Horizon MCP Server 访问数据库结构和查询工具。请先安装后再在 AI 助手中使用 Codex。",
    reasoningLevel: "推理级别",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "配置覆盖了由 Chiron Horizon 管理的 MCP 环境变量。请从供应商配置中移除 CHIRON_HORIZON_MCP_* 变量。",
      claudeCodeNotAuthenticated: "Claude Code CLI 尚未登录。请在终端执行 `claude auth login` 后重试。",
      claudeCodeMcpConfigInvalid: "Claude Code 无法读取或解析 Chiron Horizon 生成的 MCP 配置。请检查 Claude Code CLI 版本和可执行文件路径。",
      chiron_horizonMcpMissing: "未找到兼容的 Chiron Horizon MCP Server。请在 设置 > MCP 中检查并重新安装或更新。",
      claudeCodeMcpStartupFailed: "Claude Code 已加载 MCP 配置，但无法启动 Chiron Horizon MCP Server。请检查 设置 > MCP 和下方诊断详情。",
      claudeCodeCommandLineTooLong: "Windows 拒绝了过长的 Claude Code 命令。请更新 Chiron Horizon，并使用生成的 MCP 配置文件重试。",
      ...
      piAgentEnvReserved: "配置覆盖了由 Chiron Horizon 管理的 Pi 或 MCP 环境变量。请移除 CHIRON_HORIZON_PI_* 和 CHIRON_HORIZON_MCP_* 变量。",
      piAgentNotAuthenticated: "Pi Coding Agent 没有可用的已认证模型供应商。请先在 Pi 中配置或登录供应商。",
      piAgentMcpStartupFailed: "Pi 无法启动 Chiron Horizon 的作用域 MCP 桥接。请检查 设置 > MCP 和下方诊断详情。",
      ...
      openCodeEnvReserved: "配置覆盖了由 OpenCode 或 Chiron Horizon 管理的环境变量。请移除 OPENCODE 控制变量和 CHIRON_HORIZON_MCP_* 变量。",
      openCodeNotAuthenticated: "OpenCode 尚未配置已认证的模型提供商。请执行 `opencode auth login` 或在 OpenCode 中配置提供商后重试。",
      openCodeMcpStartupFailed: "OpenCode 无法启动受限范围的 Chiron Horizon MCP Server。请检查 设置 > MCP 和下方诊断详情。",
      ...
      grokCliEnvReserved: "覆盖了 Chiron Horizon 管理的 MCP 环境变量。请从提供商配置中移除 CHIRON_HORIZON_MCP_* 变量。",
      grokCliNotAuthenticated: "Grok CLI 尚未登录。请在终端执行 `grok login` 后重试。",
      grokCliMcpStartupFailed: "Grok 已加载 MCP 配置，但无法启动 Chiron Horizon MCP Server。请检查 设置 > MCP 与下方诊断信息。",
      grokCliCommandLineTooLong: "命令行过长，Windows 拒绝执行 Grok CLI。请更新 Chiron Horizon 并使用生成的 prompt 文件重试。",
      ...
      codeBuddyEnvReserved: "覆盖了 Chiron Horizon 管理的 MCP 环境变量。请从提供商配置中移除 CHIRON_HORIZON_MCP_* 变量。",
      codeBuddyNotAuthenticated: "CodeBuddy Code 尚未登录。请在终端打开 `codebuddy`，完成登录后重试。",
      codeBuddyTimeout: "CodeBuddy Code CLI 未在操作超时前响应。",
      codeBuddyMcpConfigInvalid: "CodeBuddy Code 无法读取临时 Chiron Horizon MCP 配置。请检查下方诊断信息。",
      codeBuddyMcpStartupFailed: "CodeBuddy Code 已加载 MCP 配置，但无法启动 Chiron Horizon MCP Server。请检查 设置 > MCP 与下方诊断信息。",
      ...
      qoderEnvReserved: "配置覆盖了由 Chiron Horizon 管理的 MCP 环境变量。请从供应商配置中移除 CHIRON_HORIZON_MCP_* 变量。",
      ...
      qoderMcpConfigInvalid: "Qoder CLI 无法读取临时 Chiron Horizon MCP 配置。请检查 Qoder CLI 版本和下方诊断信息。",
      qoderMcpStartupFailed: "Qoder CLI 已加载 MCP 配置，但无法启动 Chiron Horizon MCP Server。请检查 设置 > MCP 和下方诊断信息。",
      ...
      cursorEnvReserved: "配置覆盖了由 Cursor 或 Chiron Horizon 管理的环境变量。请移除 CURSOR_CONFIG_DIR、CURSOR_DATA_DIR 和 CHIRON_HORIZON_MCP_* 变量。",
      cursorNotAuthenticated: "Cursor CLI 尚未登录。请在终端执行 `agent login` 后重试。",
      cursorMcpStartupFailed: "Cursor 无法启动受限范围的 Chiron Horizon MCP Server。请检查 设置 > MCP 和下方诊断详情。",
    ...
    htmlCopySource: "复制源码",
    htmlCopyRiskBody: "此 HTML 由 AI 生成，在 Chiron Horizon 外打开可能执行脚本并访问网络。",
    htmlCopyRiskAccept: "仍要复制",
    htmlCopyRiskRemember: "本会话不再询问",
    htmlCopyRiskToast: "已复制。此 AI 生成的 HTML 在 Chiron Horizon 外打开可能执行脚本或访问网络。",
  ...
  sqlServerTrace: {
    ...
    cleanedStaleSessions: "已清理 {count} 个超时遗留的 Chiron Horizon 跟踪会话。",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "集群状态仍可正常查看。Chiron Horizon 会直接访问当前 etcd endpoint 拼接后的 /metrics，请确认该地址可由 Chiron Horizon 应用访问。",
  ...
  sqlParameters: {
    title: "SQL 参数",
    description: "为 SQL 模板占位符填写参数值，Chiron Horizon 会在执行前替换为 SQL 字面量。",
    name: "参数名",
    syntax: "Syntax",
    type: "类型",
    value: "参数值",
  ...
  settings: {
    ...
    uiFontFamily: "界面字体",
    uiFontAppDefault: "Chiron Horizon 默认",
    ...
    iconThemeDefault: "默认",
    iconThemeDefaultDescription: "使用彩色 Chiron Horizon 应用图标。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "应用内使用黑色 Chiron Horizon 标识。",
    ...
    quitOnCloseDescription: "开启后，点击窗口关闭按钮将彻底退出 Chiron Horizon，而不是隐藏到系统托盘。",
    ...
    duckDbWorkerProcessIsolationDescription: "将 DuckDB 查询放到独立 worker 进程中运行。停止长查询时，如果 DuckDB 未及时响应取消请求，Chiron Horizon 会结束 worker 进程以释放数据库文件锁。",
    duckDbWorkerMaxProcesses: "DuckDB 最大并发 worker 数",
    duckDbWorkerMaxProcessesDescription: "限制 Chiron Horizon 同时运行的 DuckDB worker 进程数量。范围：1-16，默认：4。",
    duckDbWorkerProcessIsolationRestartRequired: "重启 Chiron Horizon 后生效。",
    restartChironHorizon: "重启 Chiron Horizon",
    restartChironHorizonFailed: "重启 Chiron Horizon 失败：{error}",
    ...
    openTabsRestoreModeDescription: "选择 Chiron Horizon 启动时如何恢复上次打开的标签页。",
    ...
    syncWebDavDescription: "将包含 SQL 库的 Chiron Horizon 快照上传到兼容 WebDAV 的存储服务，或从远端恢复。",
    syncWebDavWebDescription: "Web 模式下由 Chiron Horizon 服务端访问 WebDAV，不受浏览器 CORS 限制；保存的密码会留在当前 Chiron Horizon 实例中。",
    ...
    syncRememberWebDavPasswordDescription: "密码会加密保存在当前 Chiron Horizon 实例中，不会同步到 WebDAV，也不会替代同步密码。",
    syncRemotePath: "远端快照路径",
    syncRemotePathDescription: "上传时 Chiron Horizon 会自动创建缺失的父目录。",
    syncSecretNotice: "默认只同步连接信息和设置，不会同步数据库密码、SSH 密码、代理密码、HTTP 隧道 Token、连接串或 AI API Key。",
    syncSecrets: "同步加密后的敏感信息",
    syncSecretsDescription: "开启后，Chiron Horizon 会先加密数据库密码、SSH 密码、代理密码、HTTP 隧道 Token 和 AI API Key，再上传到远端存储。",
    syncSecretsSharedDescription: "WebDAV 使用此密码加密包含的敏感信息。GitHub Gist 和 Gitee 代码片段使用独立的加密和敏感信息选项。",
    syncSecretsPassphrase: "同步密码",
    syncSecretsPassphraseDescription: "这个密码只用于加密和恢复敏感信息，并会加密保存在当前 Chiron Horizon 实例中；在其他实例恢复时需要再次输入。",
    syncTest: "测试",
    syncUpload: "上传",
    syncDownload: "下载",
    ...
    syncAutoUploadDescription: "仅在 Chiron Horizon 打开时运行。下载仍需手动确认，避免意外覆盖本地保存的 SQL。",
    ...
    syncDownloadConfirm: "要下载并应用远端 Chiron Horizon 快照吗？本地元信息和保存的 SQL 会被替换。",
    syncSnippetTitle: "GitHub / Gitee 代码片段同步",
    syncSnippetDescription: "将同一份 Chiron Horizon 快照保存到私有代码片段。首次上传会自动创建代码片段并保存其 ID。",
    ...
    syncSnippetMigrateLegacyConfirm: "Chiron Horizon 会先创建新的加密代码片段，再永久删除旧明文代码片段 {id}。此操作无法撤销，是否继续？",
    ...
    appCloseUnsavedTabsModeDescription: "选择退出 Chiron Horizon 时如何处理有未保存内容的 SQL 标签页。",
    ...
    appCloseDraftPersistFailed: "Chiron Horizon 无法保存打开的标签页草稿，因此已取消退出。请检查磁盘空间和文件权限后重试。详情：{message}",
    ...
    jdbcPlugin: "Chiron Horizon JDBC 插件",
    ...
    mcpTitle: "MCP Server",
    mcpDescription: "检查 Claude Code、Cursor 等编程助手使用的 Chiron Horizon MCP Server 安装与版本状态。",
    mcpChecking: "检查中",
    ...
    mcpUninstalling: "卸载中...",
    mcpUninstallConfirm: "确定要卸载 Chiron Horizon MCP Server 吗？卸载后，已配置的 MCP 客户端将无法启动它。",
    mcpUninstallFailed: "卸载失败：{error}",
    mcpConfig: "MCP 配置",
    mcpConfigOptionsHint: "连接范围和执行权限均由 Chiron Horizon 统一控制并对所有 MCP 客户端立即生效；生成配置只包含启动 MCP Server 和连接当前 Chiron Horizon 所需的运行参数，不包含权限或连接范围变量。",
    mcpScopeConnection: "允许 MCP 访问的连接",
    mcpScopeConnectionDescription: "Chiron Horizon 只向 MCP 暴露选中的连接，修改后对所有 MCP 客户端生效。",
    mcpScopeMode: "连接范围模式",
    ...
    mcpExecutionModeReadOnlyDescription: "仅允许 Chiron Horizon 判定为读取的请求；禁止已识别的数据库写入和连接管理变更。数据库账号权限仍是最终硬边界。",
    ...
    mcpNpmBoundary: "Chiron Horizon 负责检查 MCP 状态；安装、升级和卸载由检测到的 npm 或 pnpm 运行环境执行。",
    mcpRefresh: "重新检查",
    mcpGuide: "MCP 指南",
    ...
    mcpDatabaseScopePermissionNote: "选择范围不会改变此连接在 Chiron Horizon 中原有的数据库权限。",
    ...
    mcpHttpServiceDescription: "默认关闭。启用后由 Chiron Horizon Desktop 托管服务；保存后才会将上方配置应用到实际监听服务。",
    ...
    mcpHttpErrorPortInUse: "该端口已被其他应用占用。Chiron Horizon 会在保存时自动重启自己的 MCP 服务；请关闭占用该端口的其他应用，或改用其他端口。",
    ...
    mcpToolOpenTable: "在 Chiron Horizon 中打开表",
    mcpToolExecuteAndShow: "在 Chiron Horizon 中执行并展示",
    ...
    supportInfoAppVersion: "Chiron Horizon 版本",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows: "无法删除旧的 JRE 目录：{path}\n可能的原因：\n  - 仍有 chiron_horizon Agent / java 进程占用该目录\n  - 防病毒软件正在扫描\n请关闭可能持有该目录的进程，或重启 chiron_horizon 后重试。\n（原始错误：{error}）",
    ...
    offlineExportDescription: "选择要包含的 Chiron Horizon 托管 Agent 驱动。离线包仅适用于当前平台。",
    offlineExportPlatform: "适用平台",
    offlineExportSelectedCount: "已选择 {selected}/{total}",
    offlineExportSelectAll: "全选",
    offlineExportClear: "清空",
    offlineExportNoDrivers: "当前没有可导出的 Chiron Horizon 托管 Agent 驱动。",
    ...
    offlineExportSecurityHint: "仅包含 Chiron Horizon 托管的驱动产物及所需 JRE；本地路径、配置和外部驱动不会被导出。",
    ...
    offlineExportUnavailable_unmanagedInstall: "该产物未被记录为 Chiron Horizon 托管安装。",
    ...
    offlineExportUnavailable_missingManagedJre: "尚未安装所需的 Chiron Horizon 托管 JRE。",
    offlineExportUnavailable_invalidManagedJre: "所需的 Chiron Horizon 托管 JRE 无效或不安全。",
    refresh: "刷新",
    javaRuntime: "Java 运行时",
    javaRuntimeManaged: "Chiron Horizon 托管 JRE",
    ...
    update: "更新",
    localInstall: "本地安装",
    localInstallHint: "适用于完全内网环境：在另一台可联网的电脑上，从 Chiron Horizon 的 GitHub Releases 页面下载官方 chiron-horizon-jdbc-plugin-*.zip 安装包，拷贝到本机后在此选择该文件——本机无需联网即可完成安装。",
    runtimeDrivers: "运行时",
    pluginsTab: "插件",
    pluginContributionsTitle: "前端插件贡献",
    pluginContributionsDescription: "已安装插件可以声明连接表单和其他 UI 贡献点，不需要把第三方代码注入 Chiron Horizon 主窗口。",
  ...
  databaseBackup: {
    title: "数据库备份",
    schedules: "备份计划",
    runtimeRequirement: "Chiron Horizon 处于打开状态时会执行定时备份，包括最小化到托盘时。",
    addSchedule: "新建计划",
    oneShotBackup: "立即备份",
    ...
    fileNamePatternHint: "可使用 {'{'}schedule{'}'}、{'{'}date{'}'}、{'{'}timestamp{'}'}、{'{'}database{'}'} 和 {'{'}runId{'}'}。Chiron Horizon 会自动补齐缺失的数据库名，并自动添加扩展名。",
  ...
  mqExchanges: {
    ...
    virtualHost: "虚拟主机",
    namePlaceholder: "例如 chiron-horizon-events",
    cancel: "取消",
    create: "创建",
    bind: "绑定",
    bindDialogTitle: "绑定到 {name}",
    queueNamePlaceholder: "例如 chiron-horizon-queue",
    exchangeNamePlaceholder: "例如 chiron-horizon-events",
  ...
  nacos: {
    ...
    protectedRole: "Chiron Horizon 不允许移除内置管理员角色。",
    close: "关闭",
    ...
    namespaceId: "命名空间 ID",
    namespaceIdPlaceholder: "留空则由 Chiron Horizon 自动生成 UUID",
    namespaceName: "命名空间名",
    ...
    nacosDetectAccessibleNamespacesHint: "Chiron Horizon 会校验「{name}」中每个命名空间的配置与服务读取权限后再供选择。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "当前使用 Nacos 3 Console API（通常为 8080）。可填写命名空间名称或 ID；Chiron Horizon 能读取目录时会自动将名称转换为实际 ID。",
    rnacosNamespaceAccessScopeHint: "配置了 r-nacos Console 用户资料后，Chiron Horizon 会根据当前用户的命名空间权限确定访问范围。",
    nacosAuth: "认证",
    ...
    persistentInstanceHint: "Chiron Horizon 不注册临时实例，因为无法为其提供心跳维持。",
    ipAddress: "IP 地址",
    port: "端口",
