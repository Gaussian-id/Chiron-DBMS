...
export default withEnglishFallback({
  ...
  pluginPlatform: {
    ...
    localInstallDescription: "安装从插件商店之外获得的已签名 .chiron-horizonp，Chiron Horizon 会自动校验包完整性和仓库签名。",
  ...
  updates: {
    ...
    portableAutomaticUpdate: "Chiron Horizon 将下载已签名的便携版 ZIP，退出后仅替换 Chiron Horizon.exe 并自动重启。portable.chiron-horizon 和 data 会保留。",
  ...
  ai: {
    ...
    cliErrors: {
      ...
      chironHorizonMcpMissing: "未找到兼容的 Chiron Horizon MCP Server。请在 设置 > MCP 中检查并重新安装或更新。",
  ...
  driverStore: {
    ...
    jreDirRemoveFailedWindows: "无法删除旧的 JRE 目录：{path}\n可能的原因：\n  - 仍有 chiron-horizon Agent / java 进程占用该目录\n  - 防病毒软件正在扫描\n请关闭可能持有该目录的进程，或重启 chiron-horizon 后重试。\n（原始错误：{error}）",
