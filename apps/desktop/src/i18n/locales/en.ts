...
export default {
  chiron: {
    ...
    resultsHint: "Run a ChironQL script or browse a collection to view results.",
    workspace: "ChironDB workspace",
    collection: "Collection",
    noCollection: "Explicit query target",
    refreshCollections: "Refresh collections",
    browse: "Browse points",
    sessionHint: "Run the selection or the whole editor. Statements execute in order; each write requires approval. USE applies to this Run only.",
    query: "ChironQL query editor",
    trace: "Execution trace",
    run: "Run ChironQL",
    running: "Running…",
    rows: "rows",
    affected: "Affected",
    degraded: "Degraded result — inspect diagnostics",
    nextPage: "Next page",
    diagnostics: "Query diagnostics and cursor",
    bytePosition: "Byte position",
    affectedEstimate: "Estimated affected points",
    confirmTitle: "Confirm ChironQL operation",
    confirmHint: "Review the exact statement below. ChironDB may ask again with an affected-point count. Cancelling sends no further request.",
    cancel: "Cancel",
  ...
  app: {
    name: "Gauss Horizon by Gaussian",
  },
  docs,
  pluginPlatform: {
    openResultView: "Open with {label}",
    centerTitle: "Plugin Center",
    centerDescription: "Discover optional Gauss Horizon capabilities in one marketplace, then manage installed plugins and repositories separately.",
    ...
    localInstallDescription: "Install a signed .gauss-horizonp obtained outside the marketplace. Gauss Horizon verifies package integrity and repository signatures automatically.",
    repositoriesTitle: "Plugin repositories",
    repositoriesDescription: "The official source is signed and managed by Gauss Horizon. Custom repositories use one repository-level trust key configured below.",
    ...
    allowUnsignedDevelopmentPackageDescription: "Applies only to manual .gauss-horizonp installation and never relaxes official marketplace verification.",
    installPackage: "Install .gauss-horizonp",
    dropToInstall: "Drop to install the plugin package",
    dropInstallHint: "You can also drop a .gauss-horizonp file onto this page",
    packageFileType: "Gauss Horizon plugin package",
    ...
    noInstalledPluginsDescription: "Install a signed .gauss-horizonp, or explicitly enable unsigned packages for local development.",
  ...
  externalSqlFile: {
    modifiedTitle: "File changed outside Gauss Horizon",
    ...
    deletedDescription: "{path} was deleted or moved outside Gauss Horizon. The editor content is still available.",
  ...
  updates: {
    ...
    availableMessage: "Gauss Horizon {latest} is available. You are using {current}.",
    upToDate: "Gauss Horizon is up to date ({version}).",
    ...
    downloadStarted: "Downloading Gauss Horizon {version} in the background.",
    activeTasksBlockUpdate: "{count} task(s) are still running. Wait for them to finish before updating Gauss Horizon.",
    portableAutomaticUpdate: "Gauss Horizon will download the signed portable ZIP, replace only Gauss Horizon.exe after exit, and restart automatically. portable.gauss-horizon and data will be kept.",
    ...
    readyToInstall: "Gauss Horizon {version} is ready to install.",
    installNow: "Install Now",
    installedReadyToRestart: "Gauss Horizon has been updated. Restart to finish.",
  ...
  sidebar: {
    ...
    importGaussHorizon: "Import Gauss Horizon Config",
  ...
  sqlLibrary: {
    title: "SQL Library",
    storageHelp: "SQL Library content is stored in Gauss Horizon app data, not as standalone .sql files on disk.\nTo use files in the file system, export the SQL Library.",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "Connected, but Gauss Horizon could not remember the password: {message}",
    ...
    oracleTnsPathHint: "The Gauss Horizon backend/Oracle agent reads this path. Desktop paths are local; Web and Docker paths are on the server.",
    ...
    nacosWebConsoleUrlHint: "Optional. Opens the Nacos console in your browser. Nacos 3 requires its separate web-console address; the browser needs direct access and does not reuse Gauss Horizon tunnels or API authentication.",
    ...
    kafkaKerberosKeytabPlaceholder: "Path on Gauss Horizon Agent machine, e.g. /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "Optional path on Gauss Horizon Agent machine, e.g. /etc/krb5.conf",
    kafkaKerberosPathHint: "The keytab and krb5.conf paths are read by Gauss Horizon Agent and must exist on the machine running Gauss Horizon Agent; files are not uploaded from this browser.",
    ...
    gaussdbConnectionModeNativeHint: "Uses Gauss Horizon's built-in PostgreSQL-compatible protocol for GaussDB A/PG/Oracle compatibility modes.",
    ...
    agentJavaTooOld: "This driver requires Java 21. Use Gauss Horizon managed JRE 21 or select a Java 21 executable in Driver Manager.",
    ...
    sqliteWorkerPlacementSessionHint: "Gauss Horizon uploads the worker over SSH to the file host, then deletes the remote file when you disconnect. Suitable for temporary use.",
    sqliteWorkerPlacementPersistHint: "Gauss Horizon uploads the worker over SSH to the file host and leaves it there after disconnect. It is not deleted automatically.",
    sqliteWorkerPlacementPreplacedHint: "Upload the worker to the file host yourself, then enter the file's absolute path. Gauss Horizon will not upload or delete it automatically, but it will verify the file matches this Desktop version.",
    sqliteWorkerPlacementDefault: "Default",
    sqliteWorkerPersistPathPlaceholder: "~/.cache/gauss-horizon/sqlite-worker/<digest>",
    sqliteWorkerPreplacedPathPlaceholder: "/opt/gauss-horizon/gauss-horizon-sqlite-worker",
    sshWorkerUploadConsentTitle: "Upload SQLite Worker",
    sshWorkerUploadConsentMessage: "Gauss Horizon needs to copy a short-lived SQLite worker onto {host}:{port}. This is a privileged action. Review the destination path and digest before allowing the upload.",
    ...
    docsNotesPathPlaceholder: "docs/gauss-horizon-docs.json",
    ...
    httpTunnelTokenPlaceholder: "Token configured in gauss_horizon_tunnel.php",
    ...
    jdbcPluginHint: "Install the Gauss Horizon JDBC plugin first. For drivers with external dependencies, install from Maven in Driver Manager or import the driver and all dependency JARs together.",
  ...
  editor: {
    ...
    unsavedChangesBatchCloseMultipleMessage: '{count} SQL tabs you are closing have unsaved changes. Gauss Horizon is currently showing "{title}". Save before continuing?',
    unsavedChangesAppCloseMessage: '"{title}" has unsaved SQL changes that will be lost if you quit Gauss Horizon. Save before quitting?',
    unsavedChangesAppCloseMultipleMessage: '{count} SQL tabs have unsaved changes. Gauss Horizon is currently showing "{title}". Save before quitting?',
  ...
  tabs: {
    ...
    externalFileMissing: "Deleted or moved outside Gauss Horizon",
  ...
  grid: {
    ...
    esDeepPageJumpConfirmMessage:
      "Elasticsearch search_after cannot jump directly to a page. Gauss Horizon must fetch missing cursors in order, and backward navigation restarts from page 1. Jumping to page {page} is expected to send {requests} queries. This may take a long time and increase cluster load. Continue?",
    ...
    conditionalBulkEditConfirmMessage: "This executes immediately against every row matching the condition at execution time. Unloaded rows are included, and Gauss Horizon cannot generate a reliable rollback for them.",
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "Gauss Horizon could not identify exactly one safe writeback target table for this result.",
      ...
      "metadata-unavailable": "Gauss Horizon could not load table metadata, so result editing is disabled.",
    },
  ...
  welcome: {
    gaussianTagline: "Your database workbench, by Gaussian",
  ...
  configExport: {
    ...
    cryptoUnavailable: "Encrypted config import is unavailable in this browser context. Use HTTPS, localhost, or Gauss Horizon Desktop.",
  ...
  ai: {
    ...
    tokenrouterDescription: "Optional ChironQL POC preset. Enter your own TokenRouter key. Availability and pricing are controlled by your provider; no fallback model is selected automatically.",
    ...
    closeWithRunsDescription: "Quitting Gauss Horizon will interrupt {count} running or queued AI task(s). You can close this dialog and let them finish instead.",
    ...
    productionWriteBlocked: "AI cannot execute data or schema changes on a production database. Review this SQL and run it manually in Gauss Horizon.",
    ...
    customHeadersReserved: "This header is managed by Gauss Horizon: {name}",
    ...
    codexCliEnvReservedName: "{name} is managed by Gauss Horizon for the MCP server and cannot be set here.",
    cliMcpRequiredTitle: "Gauss Horizon MCP Server required",
    cliMcpRequiredDescription: "{provider} uses the Gauss Horizon MCP Server to access database schema and query tools. Install it before using this provider in the AI assistant.",
    ...
    cliEnvReservedName: "{name} is managed by Gauss Horizon for the MCP server and cannot be set here.",
    codexMcpRequiredTitle: "Gauss Horizon MCP Server required",
    codexMcpRequiredDescription: "Codex CLI uses the Gauss Horizon MCP Server to access database schema and query tools. Install it before using Codex in the AI assistant.",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "A Gauss Horizon-managed MCP environment variable was overridden. Remove GAUSS_HORIZON_MCP_* variables from the provider configuration.",
      claudeCodeNotAuthenticated: "Claude Code CLI is not authenticated. Run `claude auth login` in a terminal and try again.",
      claudeCodeMcpConfigInvalid: "Claude Code could not read or parse the Gauss Horizon MCP configuration. Check the Claude Code CLI version and executable path.",
      gaussHorizonMcpMissing: "A compatible Gauss Horizon MCP Server installation was not found. Check Settings > MCP, then reinstall or update it.",
      claudeCodeMcpStartupFailed: "Claude Code loaded the MCP configuration but could not start Gauss Horizon MCP Server. Check Settings > MCP and the diagnostics below.",
      claudeCodeCommandLineTooLong: "Windows rejected the Claude Code command because it was too long. Update Gauss Horizon and retry using the generated MCP configuration file.",
      ...
      piAgentEnvReserved: "A Gauss Horizon-managed Pi or MCP environment variable was overridden. Remove GAUSS_HORIZON_PI_* and GAUSS_HORIZON_MCP_* variables from the provider configuration.",
      ...
      piAgentMcpStartupFailed: "Pi could not start the scoped Gauss Horizon MCP bridge. Check Settings > MCP and the diagnostics below.",
      ...
      openCodeEnvReserved: "An OpenCode or Gauss Horizon-managed environment variable was overridden. Remove OPENCODE control variables and GAUSS_HORIZON_MCP_* variables from the provider configuration.",
      ...
      openCodeMcpStartupFailed: "OpenCode could not start the scoped Gauss Horizon MCP server. Check Settings > MCP and the diagnostics below.",
      ...
      grokCliEnvReserved: "A Gauss Horizon-managed MCP environment variable was overridden. Remove GAUSS_HORIZON_MCP_* variables from the provider configuration.",
      grokCliNotAuthenticated: "Grok CLI is not authenticated. Run `grok login` in a terminal and try again.",
      grokCliMcpStartupFailed: "Grok loaded the MCP configuration but could not start Gauss Horizon MCP Server. Check Settings > MCP and the diagnostics below.",
      grokCliCommandLineTooLong: "Windows rejected the Grok CLI command because it was too long. Update Gauss Horizon and retry using the generated prompt file.",
      ...
      codeBuddyEnvReserved: "A Gauss Horizon-managed MCP environment variable was overridden. Remove GAUSS_HORIZON_MCP_* variables from the provider configuration.",
      ...
      codeBuddyMcpConfigInvalid: "CodeBuddy Code could not read the temporary Gauss Horizon MCP configuration. Check the diagnostics below.",
      codeBuddyMcpStartupFailed: "CodeBuddy Code loaded the MCP configuration but could not start Gauss Horizon MCP Server. Check Settings > MCP and the diagnostics below.",
      ...
      qoderEnvReserved: "A Gauss Horizon-managed MCP environment variable was overridden. Remove GAUSS_HORIZON_MCP_* variables from the provider configuration.",
      ...
      qoderMcpConfigInvalid: "Qoder CLI could not read the temporary Gauss Horizon MCP configuration. Check the Qoder CLI version and diagnostics below.",
      qoderMcpStartupFailed: "Qoder CLI loaded the MCP configuration but could not start Gauss Horizon MCP Server. Check Settings > MCP and the diagnostics below.",
      ...
      cursorEnvReserved: "A Cursor or Gauss Horizon-managed environment variable was overridden. Remove CURSOR_CONFIG_DIR, CURSOR_DATA_DIR, and GAUSS_HORIZON_MCP_* variables from the provider configuration.",
      cursorNotAuthenticated: "Cursor CLI is not authenticated. Run `agent login` in a terminal and try again.",
      cursorMcpStartupFailed: "Cursor could not start the scoped Gauss Horizon MCP server. Check Settings > MCP and the diagnostics below.",
    ...
    htmlCopyRiskBody: "This HTML was AI-generated. Opening it outside Gauss Horizon may execute scripts and access the network.",
    htmlCopyRiskAccept: "Copy anyway",
    htmlCopyRiskRemember: "Don't ask again this session",
    htmlCopyRiskToast: "Copied. This AI-generated HTML may execute scripts or access the network when opened outside Gauss Horizon.",
  ...
  contextMenu: {
    ...
    parameterDefaultHint: "When DEFAULT is checked, Gauss Horizon omits that argument so the database uses the procedure default.",
  ...
  structureEditor: {
    ...
    geometryEmptyTypeHint: "Empty geometry type equals GEOMETRY (any sub-type); gauss-horizon emits geometry(GEOMETRY, SRID).",
  ...
  etcd: {
    ...
    access: {
      ...
      permissionUpdateHint: "etcd Auth has no atomic update. Saving revokes the old permission and then grants the new one. If granting fails, Gauss Horizon attempts to restore the old permission.",
      saveChanges: "Save changes",
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "Cluster status remains available. Gauss Horizon requests /metrics directly from the current etcd endpoint; verify that the resulting URL is reachable from the Gauss Horizon application.",
  ...
  sqlParameters: {
    ...
    description: "Fill values for SQL template placeholders. Gauss Horizon replaces them before executing the SQL.",
    name: "Parameter",
    syntax: "Syntax",
    type: "Type",
    value: "Value",
  ...
  settings: {
    ...
    uiFontAppDefault: "Gaussian Default (Poppins)",
    ...
    iconThemeDefaultDescription: "Use the Gaussian app icon.",
    iconThemeBlack: "Black",
    iconThemeBlackDescription: "Use the Black Gaussian artwork, independently of the light or dark theme.",
    iconThemeBlackDescriptionMac: "Use the Black Gaussian artwork, independently of the light or dark theme.",
    showTrayIcon: "Show tray/menu bar icon",
    showTrayIconDescription: "When disabled, no icon is shown, but closing the window still hides Gauss Horizon in the background as before.",
    quitOnClose: "Quit when closing window",
    quitOnCloseDescription: "When enabled, clicking the close button exits Gauss Horizon completely instead of hiding it to the tray. You can change this later in Appearance settings.",
    ...
    closeActionQuit: "Quit Gauss Horizon",
    closeActionMinimize: "Minimize to tray",
    updateNotificationsEnabled: "Enable update reminders",
    updateNotificationsEnabledDescription: "Automatic updates and release downloads are unavailable in Gauss Horizon 0.1.0.",
    ...
    duckDbWorkerProcessIsolationDescription: "Runs DuckDB queries in a separate worker process. If a long query does not respond to cancellation, Gauss Horizon can stop the worker process to release the database file lock.",
    duckDbWorkerMaxProcesses: "DuckDB max concurrent workers",
    duckDbWorkerMaxProcessesDescription: "Limits how many DuckDB worker processes Gauss Horizon can run at the same time. Range: 1-16. Default: 4.",
    duckDbWorkerProcessIsolationRestartRequired: "Restart Gauss Horizon for this change to take effect.",
    restartGaussHorizon: "Restart Gauss Horizon",
    restartGaussHorizonFailed: "Failed to restart Gauss Horizon: {error}",
    ...
    openTabsRestoreModeDescription: "Choose how Gauss Horizon restores tabs that were open last time.",
    ...
    syncWebDavDescription: "Upload or restore a Gauss Horizon snapshot, including saved SQL library entries, from a WebDAV-compatible storage service.",
    syncWebDavWebDescription: "In Web mode, the Gauss Horizon server accesses WebDAV without browser CORS restrictions. Saved passwords remain in the current Gauss Horizon instance.",
    ...
    syncRememberWebDavPasswordDescription: "The password is encrypted and stored in the current Gauss Horizon instance only. It is not synced to WebDAV and does not replace the sync password.",
    syncRemotePath: "Remote snapshot path",
    syncRemotePathDescription: "Gauss Horizon will create missing parent folders when uploading.",
    syncSecretNotice: "By default, Gauss Horizon syncs connection details and settings only. Database passwords, SSH passwords, proxy passwords, HTTP tunnel tokens, connection strings, and AI API keys stay local.",
    syncSecrets: "Sync encrypted secrets",
    syncSecretsDescription: "When enabled, Gauss Horizon encrypts database passwords, SSH passwords, proxy passwords, HTTP tunnel tokens, and AI API keys before uploading them to remote storage.",
    ...
    syncSecretsPassphraseDescription: "This password is only used to encrypt and restore sensitive data. It is encrypted in the current Gauss Horizon instance; enter it again in other instances.",
    syncTest: "Test",
    syncUpload: "Upload",
    ...
    syncAutoUploadDescription: "Runs while Gauss Horizon is open. Downloads still require manual confirmation to avoid replacing local saved SQL unexpectedly.",
    ...
    syncDownloadConfirm: "Download and apply the remote Gauss Horizon snapshot? Local metadata and saved SQL will be replaced.",
    syncSnippetTitle: "GitHub / Gitee Snippet Sync",
    syncSnippetDescription: "Store the same Gauss Horizon snapshot in a private code snippet. The first upload creates a snippet and saves its ID.",
    ...
    syncSnippetMigrateLegacyConfirm: "Gauss Horizon will first create an encrypted replacement, then permanently delete legacy plaintext snippet {id}. This cannot be undone. Continue?",
    ...
    appCloseUnsavedTabsModeDescription: "Choose what happens when quitting Gauss Horizon while SQL tabs have unsaved edits.",
    appCloseUnsavedTabsModePrompt: "Ask whether to save",
    appCloseUnsavedTabsModeKeepDrafts: "Quit and keep drafts",
    appCloseUnsavedTabsModeHint: 'Drafts are kept as unsaved tabs and restored the next time you start Gauss Horizon. This requires "Restore tabs on launch" to not be set to "Do not keep tabs".',
    appCloseDraftPersistFailed: "Gauss Horizon could not save the open tab drafts, so quitting was cancelled. Check disk space and file permissions, then try again. Details: {message}",
    ...
    sqlVariableSyntaxDescription: "Choose which variable and placeholder syntaxes Gauss Horizon substitutes before running SQL, per database type. All are enabled by default.",
    ...
    jdbcPlugin: "Gauss Horizon JDBC plugin",
    ...
    mcpTitle: "MCP Server",
    mcpDescription: "Check the Gauss Horizon MCP Server installation and version used by Claude Code, Cursor, and other agents.",
    ...
    mcpUninstallConfirm: "Uninstall Gauss Horizon MCP Server? Configured MCP clients will no longer be able to start it.",
    mcpUninstallFailed: "Uninstallation failed: {error}",
    mcpConfig: "MCP config",
    mcpConfigOptionsHint:
      "Gauss Horizon centrally controls connection access and execution permissions for every MCP client. Generated configs contain only the runtime settings needed to start the MCP server and connect to this Gauss Horizon instance, with no permission or connection-scope variables.",
    mcpScopeConnection: "Connections allowed for MCP",
    mcpScopeConnectionDescription: "Gauss Horizon exposes only the selected connections to MCP. Changes apply to every MCP client.",
    ...
    mcpExecutionModeReadOnlyDescription: "Allows only requests Gauss Horizon classifies as reads; blocks recognized database writes and connection changes. Database-account permissions remain the final hard boundary.",
    ...
    mcpCapabilityDescription: "These are representative operations; requests Gauss Horizon cannot classify reliably require full access.",
    ...
    mcpDetectionTiming: "Gauss Horizon checks automatically when this page opens; use Check again to refresh.",
    mcpNpmBoundary: "Gauss Horizon checks MCP status; installation, upgrades, and uninstallation run through the detected npm or pnpm runtime.",
    ...
    mcpDatabaseScopePermissionNote: "The selected scope does not change the connection's existing database permissions in Gauss Horizon.",
    ...
    mcpHttpServiceDescription: "Off by default. When enabled, Gauss Horizon Desktop hosts the service; the configuration above is applied to the actual listening service only after saving.",
    ...
    mcpHttpErrorPortInUse: "The port is already used by another application. Gauss Horizon restarts its own MCP service when saving; close the other application using the port, or choose a different port.",
    ...
    mcpToolOpenTable: "Open table in Gauss Horizon",
    mcpToolExecuteAndShow: "Execute and show in Gauss Horizon",
    ...
    supportInfoAppVersion: "Application Version",
    ...
    project: "Project",
    openSource: "Upstream Gauss Horizon repository",
    officialDocs: "Upstream Gauss Horizon docs",
    ...
    changelogOpenWebsite: "View repository",
  ...
  driverStore: {
    ...
    offlineExportDescription: "Select the Gauss Horizon-managed Agent drivers to include. The package is compatible with the current platform only.",
    ...
    offlineExportNoDrivers: "No eligible Gauss Horizon-managed Agent drivers are installed.",
    ...
    offlineExportSecurityHint: "Only Gauss Horizon-managed artifacts and required managed JREs are included. Local paths, settings, and external drivers are excluded.",
    ...
    offlineExportUnavailable_unmanagedInstall: "This artifact is not tracked as a Gauss Horizon-managed installation.",
    ...
    offlineExportUnavailable_missingManagedJre: "The required Gauss Horizon-managed JRE is not installed.",
    offlineExportUnavailable_invalidManagedJre: "The required Gauss Horizon-managed JRE is invalid or unsafe.",
    refresh: "Refresh",
    javaRuntime: "Java Runtime",
    javaRuntimeManaged: "Gauss Horizon managed JRE",
    ...
    localInstallHint: "For air-gapped machines: on another computer with internet access, download the official gauss-horizon-jdbc-plugin-*.zip from the Gauss Horizon GitHub Releases page, transfer it here, and select it — no network access is needed on this machine.",
    runtimeDrivers: "Runtime",
    pluginsTab: "Plugins",
    pluginContributionsTitle: "Frontend plugin contributions",
    pluginContributionsDescription: "Installed plugins can describe connection forms and other UI contributions without injecting code into the main Gauss Horizon window.",
  ...
  databaseBackup: {
    ...
    runtimeRequirement: "Scheduled backups run while Gauss Horizon is open, including when it is minimized to the tray.",
    ...
    fileNamePatternHint: "Use {'{'}schedule{'}'}, {'{'}date{'}'}, {'{'}timestamp{'}'}, {'{'}database{'}'}, and {'{'}runId{'}'}. Gauss Horizon adds a missing {'{'}database{'}'} value and adds the extension automatically.",
  ...
  mqExchanges: {
    ...
    namePlaceholder: "e.g. gauss-horizon-events",
    cancel: "Cancel",
    create: "Create",
    bind: "Bind",
    bindDialogTitle: "Bind to {name}",
    queueNamePlaceholder: "e.g. gauss-horizon-queue",
    exchangeNamePlaceholder: "e.g. gauss-horizon-events",
  ...
  nacos: {
    ...
    namespaceIdPlaceholder: "Leave empty for Gauss Horizon to generate a UUID",
    namespaceName: "Namespace Name",
    ...
    nacosDetectAccessibleNamespacesHint: "Gauss Horizon verifies configuration and service read access before offering namespaces for {name}.",
    ...
    nacosV3ConsoleManagedNamespacesHint: "This uses the Nacos 3 Console API (normally port 8080). Enter a namespace name or ID; Gauss Horizon converts a name to its actual ID when it can read the directory.",
    ...
    persistentInstanceHint: "Gauss Horizon does not register ephemeral instances because it cannot maintain their heartbeat.",
