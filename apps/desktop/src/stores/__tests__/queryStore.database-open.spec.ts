...
describe("queryStore database open state", () => {
  ...
  it("does not restore open tabs when launch restore mode is none", async () => {
    ...
    storage.set("gauss-horizon-editor-settings", JSON.stringify({ openTabsRestoreMode: "none" }));
    storage.set("gauss-horizon-app-state:open_tabs", JSON.stringify({ tabs: JSON.parse(persistedTabs), activeTabId: "tab-1" }));
    ...
    expect(JSON.parse(storage.get("gauss-horizon-app-state:open_tabs") ?? "{}")).toEqual({ tabs: [], activeTabId: null });
  });
  ...
  it("restores only pinned tabs when launch restore mode is pinned", async () => {
    ...
    vi.unstubAllGlobals();
    const storage = installLocalStorage();
    storage.set("gauss-horizon-editor-settings", JSON.stringify({ openTabsRestoreMode: "pinned" }));
    storage.set(OPEN_TABS_STORAGE_KEY, persistedTabs);
