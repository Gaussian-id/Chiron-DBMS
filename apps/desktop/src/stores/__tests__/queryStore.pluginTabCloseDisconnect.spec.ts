...
describe("queryStore plugin tab close disconnects the plugin connection", () => {
  ...
  it("disconnects the plugin connection when its last plugin tab closes", async () => {
    ...
    const id = queryStore.openPluginWorkbench("chiron.horizon.ssh", "workbench", { connectionId: "conn-1", context: { connectionId: "conn-1" } });
  ...
  it("keeps the plugin connection while another plugin tab for it stays open", async () => {
    ...
    const workbenchId = queryStore.openPluginWorkbench("chiron.horizon.ssh", "workbench", { connectionId: "conn-1", context: { connectionId: "conn-1" } });
    queryStore.openPluginFilesystem("chiron.horizon.ssh", "fs", { connectionId: "conn-1", rootUri: "sftp:/" });
