...
describe("queryStore hidden primary key editing", () => {
  ...
  it("keeps manual non-Oracle queries out of table-data preview mode", async () => {
    ...
    expect(executeInManualTransaction).toHaveBeenCalledWith("txn-1", "SELECT name, `id` AS `__CHIRON_HORIZON_PK_0` FROM users", "app", undefined, 100000, false, undefined, undefined, "SELECT name FROM users");
  });
