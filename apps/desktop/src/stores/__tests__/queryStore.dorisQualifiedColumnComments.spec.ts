...
describe("queryStore Doris qualified-table metadata target", () => {
  beforeEach(async () => {
    ...
    // is the database explicitly qualified in the SQL (`gauss-horizon6590_test.tbl`).
    ...
    analyzeEditableQueryEditability.mockResolvedValue({
      editable: true,
      analysis: {
        // `gauss-horizon6590_test.gauss_horizon_comment_test` → the qualified database lands in
        // `schema` (MySQL-family two-part names), matching the Rust parser.
        schema: "gauss-horizon6590_test",
        schemaQuoted: false,
        tableName: "gauss_horizon_comment_test",
    ...
    buildDataGridCountSql.mockResolvedValue("SELECT COUNT(*) FROM `gauss-horizon6590_test`.`gauss_horizon_comment_test`");
  ...
  it("sends the SQL-qualified database in `schema` so the backend can resolve the column lookup", async () => {
    ...
    await store.executeTabSql(tabId, "SELECT * FROM gauss-horizon6590_test.gauss_horizon_comment_test");
    ...
    expect(getColumns).toHaveBeenCalledWith("doris-1", "", "gauss-horizon6590_test", "gauss_horizon_comment_test", undefined);
