...
describe("queryStore reconnectRestoredPluginTabs", () => {
  ...
  it("replays ensureConnected once per distinct plugin connection, active tab first", async () => {
    ...
    queryStore.openPluginWorkbench("chiron.horizon.ssh", "workbench", { connectionId: "conn-1", context: { connectionId: "conn-1" } });
    queryStore.openPluginFilesystem("chiron.horizon.files", "files", { connectionId: "conn-1" });
    // A non-plugin tab must not participate in the replay.
    queryStore.openPluginWorkbench("chiron.horizon.ssh", "workbench", { connectionId: "conn-sql", context: { connectionId: "conn-sql" } });
    const activeTabId = queryStore.openPluginWorkbench("chiron.horizon.ssh", "workbench", { connectionId: "conn-2", context: { connectionId: "conn-2" } });
  ...
  it("a per-connection failure does not abort the remaining replays", async () => {
    const queryStore = useQueryStore();
    queryStore.openPluginWorkbench("chiron.horizon.ssh", "workbench", { connectionId: "conn-1", context: { connectionId: "conn-1" } });
    queryStore.openPluginWorkbench("chiron.horizon.ldap", "workbench", { connectionId: "conn-2", context: { connectionId: "conn-2" } });
