...
describe("connectionStore completion assistant", () => {
  ...
  it("uses the Dameng login schema for unqualified column completion", async () => {
    ...
    expect(getColumns).toHaveBeenCalledWith("dameng-1", "", "chiron_horizon_test", "tb_user", undefined, undefined);
    expect(first).toEqual([expect.objectContaining({ name: "ID", table: "tb_user", schema: "chiron_horizon_test" })]);
