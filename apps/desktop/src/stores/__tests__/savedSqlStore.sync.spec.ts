...
const mocks = vi.hoisted(() => ({
  syncDir: { value: "/workspace/chiron-horizon-sql-sync" as string | null },
}));
...
describe("savedSqlStore directory sync", () => {
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    mocks.syncDir.value = "/workspace/chiron-horizon-sql-sync";
  ...
  it("hydrates an unloaded library with one batch request and reuses it", async () => {
    ...
    expect(api.syncSavedSqlDirectory).toHaveBeenLastCalledWith({
      targetDir: "/workspace/chiron-horizon-sql-sync",
      entries: [
