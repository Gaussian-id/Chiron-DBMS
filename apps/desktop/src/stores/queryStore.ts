...
import { canInsertTableRows, canUseKeylessRowPredicate, CHIRON_HORIZON_ROWID_COLUMN, editablePrimaryKeys, shouldIncludeSyntheticRowId, usesSyntheticRowIdKey } from "@/lib/table/tableEditing";
...
const QUERY_SURFACE_ACTIVATION_EVENT = "chiron-horizon:activate-query-surface";
...
function finishBatchSqlExecution(tab: QueryTab, executionId: string, cancelled: boolean) {
  ...
  if (!batch) return;
  if (cancelled) {
    const cancelledError = [...batch.items].reverse().find((item) => item.status === "error" && (item.errorDetails?.code === "Chiron Horizon-JDBC-2003" || /cancel|取消/i.test(item.error ?? "")));
    if (cancelledError) {
  ...
  function missingPrimaryKeysForSource(databaseType: DatabaseType, primaryKeys: string[], analysis: EditableQueryInfo, sourceKey: string): string[] {
    ...
    const selectedColumns = new Set(
      analysis.columns.flatMap((column) => {
        ...
        if (databaseType === "oracle" && !column.sourceNameQuoted && column.sourceName.toUpperCase() === "ROWID") return [CHIRON_HORIZON_ROWID_COLUMN, column.sourceName];
  ...
  function buildHiddenPrimaryKeyPreparation(tab: QueryTab, sql: string, databaseType: DatabaseType, loaded: LoadedEditableSource, primaryKeys: string[], declaredPrimaryKeys: string[], traceId: string, elapsed: () => string): EditableQueryExecutionPreparation {
    ...
    const missingPrimaryKeys =
      declaredPrimaryKeys.length === 0
        ? primaryKeys.filter((primaryKey) => !(databaseType === "oracle" && primaryKey === CHIRON_HORIZON_ROWID_COLUMN && metadataAnalysis.columns.some((column) => column.sourceKey === loaded.source.key && !column.sourceNameQuoted && column.sourceName?.toUpperCase() === "ROWID")))
    ...
    const rewritten = buildQueryWithHiddenPrimaryKeys({
      ...
      sourceExpressions: missingPrimaryKeys.includes(CHIRON_HORIZON_ROWID_COLUMN) && (databaseType === "oracle" || databaseType === "xugu") ? { [CHIRON_HORIZON_ROWID_COLUMN]: databaseType === "oracle" ? "ROWIDTOCHAR(ROWID)" : "ROWID" } : undefined,
    });
  ...
  async function buildQueryMetadataPatch(tab: QueryTab, sql: string, executionDatabase: string, traceId?: string, elapsed?: () => string, hiddenPrimaryKeys: HiddenPrimaryKeyProjection[] = []): Promise<QueryMetadataPatch | undefined> {
    ...
    try {
      ...
      if (loadedSources.length === 1) {
        ...
        const metadataAnalysis = expandStarProjectionColumnsForSource(bindColumnsForSource(dbType, loaded.analysis, loaded.source, loaded.tableMeta.columns, allSourceColumns), loaded.source, loaded.tableMeta.columns);
        const syntheticRowIdProjection = hiddenPrimaryKeys.find((projection) => projection.sourceName.toUpperCase() === CHIRON_HORIZON_ROWID_COLUMN);
        const primaryKeys = loaded.tableMeta.primaryKeys.length === 0 && syntheticRowIdProjection ? [CHIRON_HORIZON_ROWID_COLUMN] : loaded.tableMeta.primaryKeys;
        ...
        if (sourceColumns && syntheticRowIdProjection) {
          ...
          if (resultIndex >= 0) sourceColumns[resultIndex] = CHIRON_HORIZON_ROWID_COLUMN;
        }
        ...
        const primaryKeysPresent = syntheticRowIdProjection ? sourceColumns?.some((column) => column?.toUpperCase() === CHIRON_HORIZON_ROWID_COLUMN) === true : primaryKeysPresentForSource(dbType, primaryKeys, tab.result.columns, metadataAnalysis, loaded.source.key, loaded.tableMeta.columns);
