...
const LEGACY_STORAGE_KEY = "chiron-horizon-saved-sql-library";
...
export const useSavedSqlStore = defineStore("savedSql", () => {
  ...
  async function recordFileUsage(id: string) {
    ...
    try {
      ...
      console.warn("[Chiron Horizon][saved-sql:usage:error]", error);
      return existing;
    }
  }
  ...
  async function syncToLocalDirectory() {
    ...
    try {
      ...
      console.warn("[Chiron Horizon][saved-sql:sync:error]", error);
    } finally {
