...
describe("legacy WebView CSS fallbacks", () => {
  it("keeps globals.css balanced and free of min-width media wrappers", () => {
    ...
    // placed inside one. The html.gauss-horizon-legacy-webview class is the only gate, so
    ...
    expect(globalsCss).toContain("html.gauss-horizon-legacy-webview .sm\\:flex-row");
    expect(globalsCss).toContain("html.gauss-horizon-legacy-webview .md\\:w-full");
    expect(globalsCss).toContain("html.gauss-horizon-legacy-webview .lg\\:grid-cols-6");
  });

  it("scopes component overrides to the runtime legacy WebView class", () => {
    const fallbackStart = globalsCss.indexOf("html.gauss-horizon-legacy-webview .sm\\:block");
    const tabsOverride = globalsCss.indexOf('html.gauss-horizon-legacy-webview [data-slot="tabs-trigger"]');
    ...
    expect(globalsCss.slice(fallbackStart, splitpanesStart)).toContain('html.gauss-horizon-legacy-webview [data-slot="tabs-trigger"]');
  });

  it("falls back to the legacy viewport height when dynamic viewport units are unavailable", () => {
    const fallback = globalsCss.indexOf("--gauss-horizon-viewport-height: 100vh;");
    const supports = globalsCss.indexOf("@supports (height: 100dvh)");
    const enhanced = globalsCss.indexOf("--gauss-horizon-viewport-height: min(100vh, 100dvh);");

    expect(fallback).toBeGreaterThan(-1);
    expect(supports).toBeGreaterThan(fallback);
    expect(enhanced).toBeGreaterThan(supports);
    expect(dialogContentSource).toContain("max-h-[calc(var(--gauss-horizon-viewport-height)-2rem)]");
    expect(dialogScrollContentSource).toContain("max-h-[calc(var(--gauss-horizon-viewport-height)-6rem)]");
    expect(connectionDialogSource).toContain("max-height: calc(var(--gauss-horizon-viewport-height) - 2rem);");
  });
  ...
  it("centralizes legacy dialog positioning and layout utility fallbacks", () => {
    ...
    expect(scheduledDatabaseBackupSource).toContain("gauss-horizon-form-dialog gauss-horizon-form-dialog--lg");
    ...
    expect(scheduledDatabaseBackupSource).toContain("gauss-horizon-backup-dialog");
    ...
    expect(fallback).toContain('[data-slot="dialog-content"].gauss-horizon-backup-dialog > *');
    expect(fallback).toContain('[data-slot="dialog-content"].gauss-horizon-form-dialog');
    expect(fallback).toContain('[data-slot="dialog-content"].gauss-horizon-form-dialog--lg');
    expect(fallback).toContain("max-width: 45rem !important;");
    expect(fallback).toContain('[data-slot="dialog-content"].gauss-horizon-form-dialog [data-slot="select-trigger"]');
  ...
  it("uses a lightweight theme-aware mask without full-window filters", () => {
    ...
    expect(globalsCss).not.toContain("gauss-horizon-dialog-backdrop");
  ...
  it("keeps the data transfer dialog width fallback scoped to legacy WebViews", () => {
    expect(dataTransferDialogSource).toContain('class="gauss-horizon-transfer-dialog sm:max-w-[1120px] max-h-[80vh] flex flex-col overflow-hidden resize"');
    expect(dataTransferDialogSource).toContain('width: "min(1120px, calc(100vw - 2rem))"');
    expect(dataTransferDialogSource).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"].gauss-horizon-transfer-dialog[class~="max-w-sm"]');
    ...
    expect(globalsCss).not.toContain(".gauss-horizon-transfer-dialog");
  });
  ...
  it("keeps the code snapshot dialog layout on the global legacy dialog fallbacks", () => {
    expect(codeSnapshotDialogSource).toContain('class="flex max-h-[calc(var(--gauss-horizon-viewport-height)-2rem)] flex-col overflow-hidden border border-border !bg-background text-foreground shadow-2xl !backdrop-blur-none sm:max-w-[860px]"');
    expect(codeSnapshotDialogSource).not.toContain("gauss-horizon-legacy-webview");
    expect(codeSnapshotDialogSource).not.toContain("@media");
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class*="sm:max-w-[860px]"]');
  });
  ...
  it("keeps the update dialog layout on the global legacy dialog fallbacks", () => {
    ...
    expect(updateDialogSource).not.toContain("gauss-horizon-legacy-webview");
    expect(updateDialogSource).not.toContain("@media");
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class*="sm:max-w-[700px]"]');
  });
  ...
  it("keeps the DDL dialog layout on the global legacy dialog fallbacks", () => {
    ...
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class~="sm:max-w-190"]');
  });
  ...
  it("keeps the global dialog fallback block outside media queries", () => {
    ...
    const footerStart = globalsCss.indexOf('html.gauss-horizon-legacy-webview [data-slot="dialog-footer"]');
    ...
    expect(dialogFallbackBlock).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class*="sm:max-w-[1120px]"]');
    expect(dialogFallbackBlock).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class*="sm:max-w-[min(1180px,calc(100vw-32px))]"]');
  ...
  it("covers unprefixed arbitrary dialog max-widths that lose to the generic cap", () => {
    ...
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class*="max-w-[min(1080px"]');
    expect(globalsCss).toContain("max-width: min(1080px, calc(100vw - 2rem)) !important;");
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class*="max-w-[1800px]"]');
  });
  ...
  it("keeps the schema diagram dialog width covered by the legacy width fallbacks", () => {
    ...
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class*="sm:max-w-[94vw]"]');
  });
  ...
  it("keeps the schema diagram fullscreen mode above the generic legacy width cap", () => {
    ...
    expect(schemaDiagramDialogSource).toContain("gauss-horizon-diagram-fullscreen");
    const ruleStart = globalsCss.indexOf('html.gauss-horizon-legacy-webview [data-slot="dialog-content"].gauss-horizon-diagram-fullscreen');
    expect(ruleStart).toBeGreaterThan(globalsCss.indexOf('html.gauss-horizon-legacy-webview [data-slot="dialog-content"][class~="max-w-sm"]'));
    ...
    expect(rule).toContain("var(--gauss-horizon-viewport-height)");
  });
  ...
  it("uses an explicit tooltip copy-button hover color in legacy WebViews", () => {
    ...
    expect(dataGridColumnHeaderSource).toContain("html.gauss-horizon-legacy-webview [data-column-header-copy-name]:hover");
    expect(dataGridColumnHeaderSource).toContain("background-color: rgba(255, 255, 255, 0.1);");
    expect(dataGridColumnHeaderSource).toContain("html.gauss-horizon-legacy-webview.dark [data-column-header-copy-name]:hover");
  ...
  it("keeps primary alpha utilities readable in legacy WebViews", () => {
    expect(globalsCss).toContain("--gauss-horizon-primary-rgb: 23, 23, 23;");
    expect(globalsCss).toContain("--gauss-horizon-primary-rgb: 46, 95, 166;");
    expect(globalsCss).toContain(".bg-primary\\/10");
    expect(globalsCss).toContain("background-color: rgba(var(--gauss-horizon-primary-rgb), 0.1) !important;");
    expect(globalsCss).toContain(".border-primary\\/30");
    expect(globalsCss).toContain("border-color: rgba(var(--gauss-horizon-primary-rgb), 0.3) !important;");
  ...
  it("keeps legacy tab triggers connected to the configured corner style", () => {
    ...
    expect(tabsTriggerRule).toContain("border-radius: var(--gauss-horizon-radius-fixed-6);");
  });
  ...
  it("loads connection dialog media fallbacks without CSS transformation", () => {
    ...
    expect(connectionDialogLegacyCss).toContain("html.gauss-horizon-legacy-webview .connection-db-picker-grid");
    expect(connectionDialogLegacyCss).toContain('html.gauss-horizon-legacy-webview [data-slot="dialog-content"].connection-dialog-content--config');
  ...
  it("keeps the sidebar table tree scrollbar unchanged outside legacy WebViews", () => {
    ...
    expect(connectionTreeSource).toContain("html.gauss-horizon-legacy-webview .sidebar-tree-scrollbar");
    expect(connectionTreeSource).toMatch(/html\.gauss-horizon-legacy-webview \.sidebar-tree-scrollbar \{[\s\S]*?opacity: 0\.9;/);
    expect(connectionTreeSource).toContain("html.gauss-horizon-legacy-webview .sidebar-tree-scrollbar__thumb");
    expect(connectionTreeSource).toContain("background: rgba(82, 82, 82, 0.42);");
    expect(connectionTreeSource).toContain("html.gauss-horizon-legacy-webview.dark .sidebar-tree-scrollbar__thumb");
  ...
  it("keeps AI table scrollbars visible without OKLCH color mixing", () => {
    ...
    const legacyStart = aiAssistantSource.indexOf("html.gauss-horizon-legacy-webview.dark .ai-markdown", hoverStart);
  ...
  it("keeps selected tiles readable in WebViews without color-mix support", () => {
    ...
    const choiceFallback = globalsCss.indexOf(".gauss-horizon-choice-selected", fallbackStart);
    const tileFallback = globalsCss.indexOf(".gauss-horizon-tile-selected", fallbackStart);
    ...
    expect(globalsCss.slice(fallbackStart, tileFallback)).toContain(".gauss-horizon-choice-selected .text-muted-foreground");
    expect(globalsCss.slice(fallbackStart, tileFallback)).toContain(".dark .gauss-horizon-choice-selected");
    ...
    expect(globalsCss.slice(fallbackStart, nextSupports)).toContain(".dark .gauss-horizon-tile-selected");
  });

  it("keeps the driver manager category navigation scoped to legacy WebViews", () => {
    const fallbackStart = driverStoreDialogSource.indexOf("html.gauss-horizon-legacy-webview .driver-store-tab");
  ...
  it("keeps driver manager local import buttons large enough to target", () => {
    const fallbackStart = driverStoreDialogSource.indexOf("html.gauss-horizon-legacy-webview .driver-store-local-import-button");
    const fallbackEnd = driverStoreDialogSource.indexOf("@media (max-width: 900px)", fallbackStart);
    const fallback = driverStoreDialogSource.slice(fallbackStart, fallbackEnd);
  ...
  it("keeps tunnel profile selection readable in legacy WebViews", () => {
    ...
    expect(tunnelProfileManagerSource).toContain("html.gauss-horizon-legacy-webview .tunnel-profile-option--selected");
    ...
    expect(tunnelProfileManagerSource).toContain("html.gauss-horizon-legacy-webview .tunnel-profile-option--selected .text-muted-foreground");
  });

  it("keeps transport layer selection readable in legacy WebViews", () => {
    const fallbackStart = connectionDialogSource.indexOf("html.gauss-horizon-legacy-webview .connection-db-category-option--selected");
  ...
  it("keeps dark switches visible in legacy WebViews", () => {
    const fallbackStart = switchSource.indexOf('html.gauss-horizon-legacy-webview.dark .gauss-horizon-switch[data-state="unchecked"] .gauss-horizon-switch-thumb');
    const fallback = switchSource.slice(fallbackStart);

    expect(fallbackStart).toBeGreaterThan(-1);
    expect(fallback).toContain('html.gauss-horizon-legacy-webview.dark .gauss-horizon-switch[data-state="unchecked"] .gauss-horizon-switch-thumb');
    expect(fallback).toContain("background-color: rgb(215, 215, 219) !important;");
    expect(fallback).toContain("html.gauss-horizon-legacy-webview.dark .gauss-horizon-switch {");
  ...
  it("keeps native number input steppers scoped to legacy WebViews", () => {
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview input[type="number"]');
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview input[type="number"]:not([class*="appearance-none"])::-webkit-inner-spin-button');
    ...
    expect(globalsCss).toContain('html.gauss-horizon-legacy-webview input[type="number"]:not([class*="appearance-none"]):disabled::-webkit-inner-spin-button');
  });

  it("keeps settings field stacks spaced in legacy WebViews", () => {
    const fallbackStart = editorSettingsDialogSource.indexOf("html.gauss-horizon-legacy-webview .settings-layout");
    ...
    expect(editorSettingsDialogSource).toContain("html.gauss-horizon-legacy-webview .settings-shortcut-row:hover .settings-shortcut-action-button");
  ...
  it("uses a runtime capability check instead of an OKLCH-only CSS proxy", () => {
    ...
    expect(legacyWebViewSource).toContain("gauss-horizon-legacy-webview");
