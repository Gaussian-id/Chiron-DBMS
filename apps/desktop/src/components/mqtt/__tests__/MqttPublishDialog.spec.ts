...
describe("MQTT 消息发布输入框", () => {
  it("支持从顶部拖动调节高度并记住设置", async () => {
    ...
    expect(localStorage.getItem("chiron-horizon-mqtt-payload-height")).toBe("200");
  });
  ...
  it("支持键盘调节高度并记住设置", async () => {
    ...
    expect(localStorage.getItem("chiron-horizon-mqtt-payload-height")).toBe("90");
    ...
    expect(localStorage.getItem("chiron-horizon-mqtt-payload-height")).toBe("80");
  });
  ...
  it("将键盘调节限制在分隔条声明的高度范围内", async () => {
    ...
    expect(localStorage.getItem("chiron-horizon-mqtt-payload-height")).toBe(minHeight.toString());
    ...
    expect(localStorage.getItem("chiron-horizon-mqtt-payload-height")).toBe(maxHeight.toString());
  });
});
