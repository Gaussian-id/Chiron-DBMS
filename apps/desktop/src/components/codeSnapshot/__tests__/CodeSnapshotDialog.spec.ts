...
beforeEach(() => {
  localStorage.clear();
  renderCodeSnapshotHtmlMock.mockReset().mockResolvedValue('<div class="chiron-horizon-code-snapshot">SELECT 1</div>');
});
...
describe("CodeSnapshotDialog option switches", () => {
  it("rerenders and persists when window controls are disabled", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("chiron-horizon:code-snapshot-settings") ?? "null")).toMatchObject({ showTrafficLights: false, showLineNumbers: true });
  });
  ...
  it("rerenders and persists when line numbers are disabled", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("chiron-horizon:code-snapshot-settings") ?? "null")).toMatchObject({ showTrafficLights: true, showLineNumbers: false });
  });
});
