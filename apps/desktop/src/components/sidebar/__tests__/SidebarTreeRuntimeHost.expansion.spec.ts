...
describe("SidebarTreeRuntimeHost expansion", () => {
  it.each(["single", "double"] as const)("focuses object search when %s-click activation opens a database", async (activation) => {
    const database: TreeNode = {
      id: "mysql:chiron_horizon_test",
      label: "chiron_horizon_test",
      type: "database",
      connectionId: "mysql",
    ...
    await vi.waitFor(() => expect(queryStore.openObjectBrowser).toHaveBeenCalledWith("mysql", "chiron_horizon_test", undefined, undefined, undefined, false, undefined, undefined));
