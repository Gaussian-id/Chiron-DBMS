...
describe("DocumentBrowser Elasticsearch field search", () => {
  ...
  it("migrates hidden columns and passes a stable index layout scope without changing the query result", async () => {
    ...
    storedValues.set(`gauss-horizon-document-grid-column-visibility:v1:${legacyScopeKey}`, JSON.stringify(["title"]));
...
describe("DocumentBrowser MongoDB filter value types", () => {
  ...
  it("labels new DynamoDB rows as conditional inserts in the executable preview", async () => {
    ...
    expect(preview[0]).toContain('Gauss Horizon DYNAMODB INSERT ITEM\ntable: "orders"');
  ...
  it("exports all DynamoDB pages with opaque cursors instead of offsets", async () => {
    ...
    backend.documentFindDocuments
      ...
      .mockResolvedValueOnce({
        documents: [
          { _id: { tenant_id: "tenant-a", order_id: "001" }, tenant_id: "tenant-a", order_id: "001", amount: { $gaussHorizonDynamoDb: { version: 1, type: "number", value: "9007199254740993" } } },
          { _id: { tenant_id: "tenant-a", order_id: "002" }, tenant_id: "tenant-a", order_id: "002", tags: { $gaussHorizonDynamoDb: { version: 1, type: "stringSet", value: ["new", "paid"] } } },
        ],
        total: 3,
        total_is_exact: false,
        next_cursor: "cursor-1",
    ...
    expect(result?.rows[0]).toContain('{"$gaussHorizonDynamoDb":{"version":1,"type":"number","value":"9007199254740993"}}');
    expect(result?.rows[1]).toContain('{"$gaussHorizonDynamoDb":{"version":1,"type":"stringSet","value":["new","paid"]}}');
  ...
  it("deletes a document without opening the danger dialog when confirmation is disabled", async () => {
    ...
    expect(backend.documentDeleteDocument).toHaveBeenCalledWith("mongo-1", "test", "typed_ids", '__gauss_horizon_mongo_string_id__"document-1"', undefined, undefined);
  });
  ...
  it("saves Meilisearch grid changes in one batch request", async () => {
    ...
    expect(updates[0].id).toBe('__gauss_horizon_meilisearch_string_id__"001"');
