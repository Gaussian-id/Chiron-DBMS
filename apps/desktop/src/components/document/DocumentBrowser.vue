...
async function closeElasticsearchCursor(cursor?: string) {
  ...
  try {
    ...
    console.warn("[Chiron Horizon] failed to close Elasticsearch cursor", error);
  }
}
...
function formatDynamoDbOperationPreview(action: "insert" | "put" | "delete", id: unknown, document?: Record<string, unknown>): string {
  ...
  const lines = [`Chiron Horizon DYNAMODB ${operation}`, `table: ${JSON.stringify(props.collection)}`];
...
.json-edit {
  ...
  font-size: var(--chiron-horizon-editor-font-size);
  line-height: 1.6;
  tab-size: 2;
