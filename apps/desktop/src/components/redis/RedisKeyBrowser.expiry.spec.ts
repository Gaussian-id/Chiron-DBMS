...
function groupRow(label: string): HTMLElement {
  const labelElement = Array.from(document.querySelectorAll<HTMLElement>(".chiron-horizon-editor-font-family")).find((element) => element.textContent === label);
...
function leafCheckbox(label: string): HTMLElement {
  const checkbox = redisCheckboxes().find((el) => el.closest(".group")?.querySelector(".chiron-horizon-editor-font-family")?.textContent === label);
  expect(checkbox, label).toBeDefined();
  return checkbox!;
}
...
describe("RedisKeyBrowser fuzzy key hierarchy", () => {
  ...
  it("preserves the cursor and finds a sparse fuzzy match after a bounded continuation", async () => {
    ...
    await vi.waitFor(() => {
      const labels = Array.from(document.querySelectorAll<HTMLElement>(".chiron-horizon-editor-font-family")).map((element) => element.textContent);
  ...
  it("selects hierarchy folders in the normal tree the same way as fuzzy groups", async () => {
    ...
    for (const input of redisCheckboxes()) {
      const label = input.closest(".group")?.querySelector(".chiron-horizon-editor-font-family")?.textContent;
