...
describe("RedisKeyBrowser infinite scroll auto-continue (issue #6022)", () => {
  ...
  it("does not publish a prepared old scope after the database changes or keys are cleared", async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("chiron-horizon-redis-db-flushed", { detail: { connectionId: "connection", db: 1 } }));
      await settleThoroughly();
...
// Redis SCAN calls (see `crates/chiron-horizon-core/src/db/redis_driver.rs`).
...
// user compared Chiron Horizon's tree against `redis-cli --scan` and saw folder counts
