          ...
          <template #default="{ item }">
            ...
            <CustomContextMenu v-else :items="[{ label: t('redis.copyKeyName'), icon: Copy, action: () => emit('copy', item.key) }]" v-slot="{ onContextMenu }">
              ...
              >
                ...
                <KeyRound class="h-3 w-3 shrink-0 text-muted-foreground" /><span class="gauss-horizon-editor-font-family min-w-0 flex-1 truncate" :title="item.key.key_display">{{ item.label || t("redisGrouping.emptyKey") }}</span>
