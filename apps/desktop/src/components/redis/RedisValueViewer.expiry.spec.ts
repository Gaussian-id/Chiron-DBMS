...
afterEach(() => {
  ...
  localStorage.removeItem("chiron-horizon-redis-auto-refresh-enabled-v2");
  localStorage.removeItem("chiron-horizon-redis-auto-refresh-interval-seconds-v2");
  vi.unstubAllGlobals();
  vi.useRealTimers();
});
...
describe("RedisValueViewer expiry saving", () => {
  ...
  it("polls the full value at the configured interval and reports it to the parent", async () => {
    vi.useFakeTimers();
    localStorage.setItem("chiron-horizon-redis-auto-refresh-enabled-v2", "true");
    localStorage.setItem("chiron-horizon-redis-auto-refresh-interval-seconds-v2", "5");
  ...
  it("pauses automatic value polling while a collection member is open", async () => {
    vi.useFakeTimers();
    localStorage.setItem("chiron-horizon-redis-auto-refresh-enabled-v2", "true");
    localStorage.setItem("chiron-horizon-redis-auto-refresh-interval-seconds-v2", "1");
