...
function registerRedisDbFlushedListener() {
  ...
  window.addEventListener("chiron-horizon-redis-db-flushed", onRedisDbFlushed);
  redisDbFlushedListenerRegistered = true;
}

function unregisterRedisDbFlushedListener() {
  if (!redisDbFlushedListenerRegistered) return;
  window.removeEventListener("chiron-horizon-redis-db-flushed", onRedisDbFlushed);
...
watch(
  ...
  async ([connectionId, db]) => {
    ...
    try {
      ...
      console.warn("[Chiron Horizon] ensureConnected failed for", connectionId, error);
    }
              ...
              <div class="relative min-w-0">
                ...
                >
                  <div
                    ...
                    class="chiron-horizon-editor-font-family flex w-full cursor-pointer items-center px-3 py-1.5 text-left text-xs hover:bg-accent hover:text-accent-foreground"
                ...
                <div v-if="keyTemplateMenuVisible" :id="keyTemplateListboxId" role="listbox" :aria-label="t('redis.keyTemplateSuggestions')" class="absolute left-0 right-0 top-[calc(100%+0.25rem)] z-30 max-h-60 overflow-y-auto rounded-md border bg-popover py-1 text-popover-foreground shadow-md">
                  <button
                    ...
                    type="button"
                    role="option"
                    class="chiron-horizon-editor-font-family flex w-full items-center px-3 py-1.5 text-left text-xs hover:bg-accent hover:text-accent-foreground"
                ...
                >
                  <div class="min-w-0 flex flex-1 items-center gap-1 overflow-hidden" :style="{ paddingLeft: `${4 + row.depth * 10}px` }">
                    <template v-if="row.node.kind === 'group'">
                      ...
                      <span class="chiron-horizon-editor-font-family truncate">{{ row.node.label }}</span>
        ...
        <div class="h-full min-w-0 bg-background flex flex-col overflow-hidden">
          <Tabs v-model="activeSidePanel" :unmount-on-hide="false" class="h-full min-h-0 gap-0">
            ...
            <TabsContent value="command" class="m-0 min-h-0 flex-1 flex flex-col">
              <div class="chiron-horizon-editor-font-family relative flex min-h-0 flex-1 flex-col bg-[#171b21] text-[13px] leading-5 text-slate-200" @click="onCommandAreaClick">
                ...
                <form class="flex shrink-0 items-center gap-2 border-t border-white/10 bg-[#171b21] px-4 py-2" @submit.prevent="executeCommand">
                  ...
                  <div class="relative min-w-0 flex-1">
                    ...
                    <input
                      ...
                      class="chiron-horizon-editor-font-family min-w-0 w-full border-0 bg-transparent p-0 text-[13px] text-slate-200 caret-[#d7ba7d] outline-none placeholder:text-slate-500"
        ...
        <div class="grid gap-3">
          ...
          <div class="grid gap-1.5 text-xs font-medium">
            ...
            <label v-if="batchExpiryMode === 'ttl'" class="grid gap-1.5 text-xs font-medium">
              ...
              <Input v-model="batchExpiryTtl" data-redis-batch-expiry-ttl class="chiron-horizon-editor-font-family h-8 text-xs" :disabled="savingBatchExpiry" inputmode="numeric" :placeholder="t('redis.createKeyTtlPlaceholder')" @keydown.enter="saveBatchExpiry" />
            </label>
      ...
      <DialogContent class="sm:max-w-md" :show-close-button="!creatingKey" :style="editorFontFamilyStyle">
        ...
        <div class="grid gap-3">
          <label class="grid gap-1.5 text-xs font-medium">
            ...
            <Input v-model="createKeyName" class="chiron-horizon-editor-font-family h-8 text-xs" :disabled="creatingKey || createKeyPartiallyWritten" :placeholder="t('redis.createKeyNamePlaceholder')" @keydown.enter="createRedisKey" />
          </label>
          ...
          <label v-if="createKeyType === 'hash' && createKeyRawMode" class="grid gap-1.5 text-xs font-medium">
            ...
            <Input v-model="createKeyField" class="chiron-horizon-editor-font-family h-8 text-xs" :disabled="creatingKey" :placeholder="t('redis.createFieldPlaceholder')" @keydown.enter="createRedisKey" />
          </label>

          <label v-if="createKeyType === 'zset' && createKeyRawMode" class="grid gap-1.5 text-xs font-medium">
            <span>{{ t("redis.createScore") }}</span>
            <Input v-model="createKeyScore" class="chiron-horizon-editor-font-family h-8 text-xs" :disabled="creatingKey" placeholder="0" @keydown.enter="createRedisKey" />
          </label>
          ...
          <div class="grid gap-1.5 text-xs font-medium">
            ...
            <label v-if="createKeyExpiryMode === 'ttl'" class="grid gap-1.5 text-xs font-medium">
              <span>{{ t("redis.createKeyTtl") }}</span>
              <Input v-model="createKeyTtl" class="chiron-horizon-editor-font-family h-8 text-xs" :disabled="creatingKey" inputmode="numeric" :placeholder="t('redis.createKeyTtlPlaceholder')" @keydown.enter="createRedisKey" />
            </label>
          ...
          <template v-if="createKeyType !== 'string' && createKeyType !== 'json' && !createKeyRawMode">
            ...
            <label v-if="createKeyType === 'stream'" class="grid gap-1.5 text-xs font-medium">
              ...
              <Input v-model="createKeyEntryId" class="chiron-horizon-editor-font-family h-8 text-xs font-mono" :disabled="creatingKey" placeholder="*" />
            </label>
            ...
            <div class="grid gap-2">
              ...
              <div v-for="(entry, idx) in createKeyEntries" :key="entry.id" class="flex items-start gap-2">
                ...
                <template v-if="createKeyType === 'hash' || createKeyType === 'stream'">
                  <Input v-model="entry.field" class="chiron-horizon-editor-font-family h-8 w-2/5 text-xs" :disabled="creatingKey" :placeholder="t('redis.createFieldPlaceholder')" />
                  <Input v-model="entry.value" class="chiron-horizon-editor-font-family h-8 flex-1 text-xs" :disabled="creatingKey" :placeholder="t('redis.createValuePlaceholder')" />
                </template>
                <!-- ZSet: score + member -->
                <template v-else-if="createKeyType === 'zset'">
                  <Input v-model="entry.score" class="chiron-horizon-editor-font-family h-8 w-20 text-xs" :disabled="creatingKey" type="number" step="any" placeholder="0" />
                  <Input v-model="entry.value" class="chiron-horizon-editor-font-family h-8 flex-1 text-xs" :disabled="creatingKey" :placeholder="t('redis.createMember')" />
                </template>
          ...
          <label v-if="createKeyType === 'string' || createKeyType === 'json' || createKeyRawMode" class="grid gap-1.5 text-xs font-medium">
            ...
            <textarea
              ...
              class="chiron-horizon-editor-font-family min-h-28 resize-y rounded-md border bg-background p-2 text-xs outline-none focus-visible:ring-1 focus-visible:ring-ring"
