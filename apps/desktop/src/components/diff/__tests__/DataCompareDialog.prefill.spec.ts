...
const mocks = vi.hoisted(() => ({
  ...
  listSchemas: vi.fn().mockResolvedValue(["CHIRON_HORIZON_TEST", "REPORTING", "SYS"]),
...
function completedSession(): DataCompareSession {
  return {
    ...
    config: {
      ...
      sourceDatabase: "CHIRON_HORIZON_TEST",
      sourceSchema: "CHIRON_HORIZON_TEST",
      sourceDatabases: ["CHIRON_HORIZON_TEST"],
      sourceSchemas: ["CHIRON_HORIZON_TEST"],
      sourceTables: ["ORDERS"],
      selectedSourceTables: ["ORDERS"],
      targetConnectionId: "oracle-jdbc-11g",
      targetDatabase: "REPORTING",
      targetSchema: "CHIRON_HORIZON_TEST",
      targetDatabases: ["REPORTING"],
      targetSchemas: ["CHIRON_HORIZON_TEST"],
      targetTables: ["ORDERS"],
      targetTable: "ORDERS",
      keyColumns: ["ID"],
...
describe("DataCompareDialog source prefill", () => {
  it("keeps the Oracle source table after loading database and schema prefills", async () => {
      ...
      defineComponent({
        setup: () => () =>
          h(DataCompareDialog, {
            ...
            prefillDatabase: "CHIRON_HORIZON_TEST",
            prefillSchema: "CHIRON_HORIZON_TEST",
    ...
    const searchableSelectTriggers = [...document.querySelectorAll<HTMLButtonElement>("button.chiron-horizon-searchable-select-trigger")];
    const sourceDatabaseTrigger = searchableSelectTriggers[0];
    expect(sourceDatabaseTrigger?.title).toBe("CHIRON_HORIZON_TEST");
    expect(sourceDatabaseTrigger?.disabled).toBe(false);
    sourceDatabaseTrigger?.click();
    await flushAsyncSetup();

    const databaseOptions = [...document.querySelectorAll<HTMLButtonElement>(".chiron-horizon-searchable-select-list button")].map((button) => button.textContent?.trim());
    expect(databaseOptions).toEqual(expect.arrayContaining(["CHIRON_HORIZON_TEST", "REPORTING"]));
    expect(mocks.listTables).toHaveBeenCalledWith("oracle-11g", "CHIRON_HORIZON_TEST", "CHIRON_HORIZON_TEST");
  ...
  it("loads schemas and tables after selecting an Oracle JDBC target connection", async () => {
    ...
    const targetConnectionTrigger = document.querySelectorAll<HTMLButtonElement>("button.chiron-horizon-diff-connection-trigger")[1];
    ...
    const triggersAfterTargetLoad = [...document.querySelectorAll<HTMLButtonElement>("button.chiron-horizon-searchable-select-trigger")];
    ...
    const targetDatabaseOptions = [...document.querySelectorAll<HTMLButtonElement>(".chiron-horizon-searchable-select-list button")].map((button) => button.textContent?.trim());
    expect(targetDatabaseOptions).toEqual(expect.arrayContaining(["CHIRON_HORIZON_TEST", "REPORTING"]));

    const reportingOption = [...document.querySelectorAll<HTMLButtonElement>(".chiron-horizon-searchable-select-list button")].find((button) => button.textContent?.trim() === "REPORTING");
    expect(reportingOption).toBeDefined();
    reportingOption?.click();
    await flushAsyncSetup();

    expect(mocks.listTables).toHaveBeenCalledWith("oracle-jdbc-11g", "REPORTING", "CHIRON_HORIZON_TEST");
    expect(document.body.textContent).toContain("CODEX_7467_META");
  });
});
