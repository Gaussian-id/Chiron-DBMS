...
function onExport() {
  ...
  if (config) {
    emit("export", config);
    downloadJson(JSON.stringify(config, null, 2), `chiron-horizon-schema-diff-${config.name}.json`);
  }
}

function onExportAll() {
  emit("exportAll", props.configs);
  downloadJson(JSON.stringify(props.configs, null, 2), "chiron-horizon-schema-diff-configs.json");
}
