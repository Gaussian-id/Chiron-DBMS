...
function createDragGhost(sourceEl: HTMLElement, x: number, y: number) {
  ...
  const textNode = sourceEl.querySelector(".chiron-horizon-sql-library-drag-label");
            ...
            <div v-if="sortMode === 'date'" class="space-y-0">
              <div v-for="item in itemsByDate" :key="item.type + '-' + item.item.id">
                ...
                >
                  ...
                  <span v-else class="chiron-horizon-sql-library-drag-label min-w-0 flex-1 truncate">
                    {{ item.item.name }}
