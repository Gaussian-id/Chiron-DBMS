...
function createDragGhost(sourceEl: HTMLElement, x: number, y: number) {
  ...
  const textNode = sourceEl.querySelector(".gauss-horizon-sql-library-drag-label");
            ...
            <div v-if="sortMode === 'date'" class="space-y-0">
              <div v-for="item in itemsByDate" :key="item.type + '-' + item.item.id">
                ...
                >
                  ...
                  <span v-else class="gauss-horizon-sql-library-drag-label min-w-0 flex-1 truncate">
                    {{ item.item.name }}
                ...
                >
                  ...
                  <span v-else class="gauss-horizon-sql-library-drag-label min-w-0 flex-1 truncate" :title="fileTitleLabel(item.item)" :style="fileTitleStyle(item.item)">{{ item.item.name }}</span>
            ...
            <div v-else>
              <div v-for="row in visibleFolderRows" :key="row.type === 'folder' ? row.folder.id : row.file.id">
                ...
                >
                  ...
                  <span v-else class="gauss-horizon-sql-library-drag-label min-w-0 flex-1 truncate" :title="fileTitleLabel(row.file)" :style="fileTitleStyle(row.file)">{{ row.file.name }}</span>
              ...
              <div v-if="visibleFiles.length > 0 || dragState.draggedType === 'file'">
                ...
                >
                  ...
                  <span v-else class="gauss-horizon-sql-library-drag-label min-w-0 flex-1 truncate" :title="fileTitleLabel(file)" :style="fileTitleStyle(file)">{{ file.name }}</span>
