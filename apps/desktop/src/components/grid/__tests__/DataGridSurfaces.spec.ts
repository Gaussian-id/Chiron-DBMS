...
beforeEach(() => {
  vi.clearAllMocks();
  localStorage.removeItem("chiron-horizon-filter-builder-value-shortcut-hint-days");
});
...
describe("DataGridColumnHeader", () => {
  ...
  it("renders the metadata tooltip type value with the softened type palette", () => {
    ...
    const tooltipContent = findOne(mounted.root, (node) => String(node.props?.class ?? "").includes("chiron-horizon-column-info-tooltip"));
    ...
    expect(globalsCss).toMatch(/\.dark \.chiron-horizon-column-info-tooltip \{[^}]*--data-grid-type-string-fg: #4ade80/);
  });
...
describe("DataGridFilterBuilder", () => {
  ...
  it("shows the value editor shortcut hint from the second rule twice per day for up to three days and adds a rule on shift-enter", async () => {
    ...
    await nextTick();
    expect(hostText(mounted.root)).not.toContain("grid.filterBuilderValueShortcutHint");
    expect(JSON.parse(localStorage.getItem("chiron-horizon-filter-builder-value-shortcut-hint-days") ?? "[]")).toEqual([{ date: localDateKey(), count: 1 }]);

    dispatch(secondValueEditor, "focus");
    await nextTick();
    expect(hostText(mounted.root)).toContain("grid.filterBuilderValueShortcutHint");
    expect(JSON.parse(localStorage.getItem("chiron-horizon-filter-builder-value-shortcut-hint-days") ?? "[]")).toEqual([{ date: localDateKey(), count: 2 }]);
    dispatch(secondValueEditor, "blur");
    dispatch(secondValueEditor, "focus");
    await nextTick();
    expect(hostText(mounted.root)).not.toContain("grid.filterBuilderValueShortcutHint");
    ...
    localStorage.setItem(
      "chiron-horizon-filter-builder-value-shortcut-hint-days",
      JSON.stringify([
    ...
    expect(JSON.parse(localStorage.getItem("chiron-horizon-filter-builder-value-shortcut-hint-days") ?? "[]")).toHaveLength(3);
...
describe("DataGridCopyColumnNamesDialog", () => {
  beforeEach(() => {
    localStorage.removeItem("chiron-horizon-copy-column-names-separator");
  });
  ...
  it("previews the formatted names and copies with the chosen separator and quoting", async () => {
    ...
    expect(localStorage.getItem("chiron-horizon-copy-column-names-separator")).toBe("comma-newline");
  });
