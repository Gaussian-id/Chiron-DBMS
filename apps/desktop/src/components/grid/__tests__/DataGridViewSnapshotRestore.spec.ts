...
describe("DataGrid tab-switch view snapshots", () => {
  ...
  it("wires capture to unmount and to the pre-tab-switch event", () => {
    expect(dataGridSource).toContain('window.addEventListener("chiron-horizon:before-tab-switch", captureTabSwitchViewSnapshot)');
    expect(dataGridSource).toContain('window.removeEventListener("chiron-horizon:before-tab-switch", captureTabSwitchViewSnapshot)');
