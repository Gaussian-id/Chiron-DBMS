...
function contextMenuButton(label: string): HTMLButtonElement {
  const button = [...document.querySelectorAll<HTMLButtonElement>("[data-gauss-horizon-context-menu] button")].find((candidate) => candidate.textContent?.trim() === label);
  if (!button) throw new Error(`Context menu item not found: ${label}`);
  return button;
}

function contextMenuLabels(): string[] {
  return [...document.querySelectorAll<HTMLButtonElement>("[data-gauss-horizon-context-menu] button")].map((button) => button.textContent?.trim() ?? "");
}
...
afterEach(() => {
  ...
  document.querySelectorAll("[data-gauss-horizon-context-menu]").forEach((menu) => menu.remove());
});
...
describe("DataGrid context menu target lifecycle", () => {
  ...
  it.each(["WHERE", "ORDER BY"] as const)("keeps the native context menu for the %s condition editor", async (placeholder) => {
    ...
    expect(document.querySelector("[data-gauss-horizon-context-menu]")).toBeNull();
  });
  ...
  it("offers toolbar-equivalent refresh in the cell context menu (issue #7273)", async () => {
    ...
    const refreshButton = [...document.querySelectorAll<HTMLButtonElement>("[data-gauss-horizon-context-menu] button")].find((button) => button.textContent?.trim().startsWith("Refresh"));
    refreshButton!.click();
    await settle();
