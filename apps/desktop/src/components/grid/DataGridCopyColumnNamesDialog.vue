    ...
    <DialogContent class="sm:max-w-[440px]">
      ...
      <div class="space-y-3">
        <div class="flex items-center justify-between gap-3">
          ...
          <Select :model-value="separator" @update:model-value="onSeparatorChange">
            ...
            <SelectContent position="popper" align="end">
              <SelectItem v-for="option in COLUMN_NAME_COPY_SEPARATOR_OPTIONS" :key="option" :value="option" class="gauss-horizon-data-grid-value-font text-xs">{{ COLUMN_NAME_COPY_SEPARATOR_LABELS[option] }}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <pre class="gauss-horizon-data-grid-value-font max-h-40 min-h-16 w-full overflow-auto whitespace-pre-wrap break-all rounded-[6px] border border-input bg-muted/30 px-2.5 py-1.5 text-xs" data-copy-column-names-preview>{{ previewText }}</pre>
        </div>
      </div>
      <DialogFooter
