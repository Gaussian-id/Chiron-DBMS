          ...
          <tbody>
            <tr v-for="(field, index) in filteredRowFields" :key="`${field.colIndex}:${field.column}`" class="border-b align-top last:border-b-0">
              ...
              <td class="w-full max-w-0 px-3 py-2">
                ...
                /></a>
                <pre class="gauss-horizon-data-grid-value-font max-h-44 overflow-auto rounded border bg-muted/20 p-2 text-xs whitespace-pre-wrap break-words" :class="{ 'italic text-muted-foreground': field.isNull ?? field.value === null }">{{ field.rawValuePreview }}</pre>
                ...
                <div v-if="field.formattedJson" class="mt-2 space-y-1">
                  ...
                  <pre class="gauss-horizon-data-grid-value-font max-h-44 overflow-auto rounded border bg-muted/20 p-2 text-xs whitespace-pre-wrap break-words">{{ field.formattedJson }}</pre>
                </div>
              </td>
              <td class="px-2 py-2">
          ...
          <tbody>
            <tr v-for="field in filteredColumnFields" :key="`${field.rowId}:${field.colIndex}`" class="border-b align-top last:border-b-0">
              ...
              <td class="w-full max-w-0 px-3 py-2">
                ...
                /></a>
                <pre class="gauss-horizon-data-grid-value-font max-h-36 overflow-auto rounded border bg-muted/20 p-2 text-xs whitespace-pre-wrap break-words" :class="{ 'italic text-muted-foreground': field.isNull ?? field.value === null }">{{ field.rawValuePreview }}</pre>
                <div v-if="field.isValuePreviewTruncated" class="mt-1 text-[11px] text-muted-foreground">{{ t("grid.largeValuePreviewHint", { count: field.rawValuePreview.length }) }}</div>
                <div v-if="field.formattedJson" class="mt-2 space-y-1">
                  <div class="text-muted-foreground">{{ t("grid.formattedJson") }}</div>
                  <pre class="gauss-horizon-data-grid-value-font max-h-36 overflow-auto rounded border bg-muted/20 p-2 text-xs whitespace-pre-wrap break-words">{{ field.formattedJson }}</pre>
                </div>
              </td>
              <td class="px-2 py-2">
