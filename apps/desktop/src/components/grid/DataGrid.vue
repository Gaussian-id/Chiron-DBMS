...
const FORMATTED_JSON_EDIT_WARNING_COUNT_STORAGE_KEY = "gauss-horizon-cell-detail-formatted-json-edit-warning-count";
...
if (isDebugLoggingEnabled()) {
  logDataGridTiming("[Gauss Horizon][DataGrid:setup]", {
...
watch(
  ...
  (result) => {
    ...
    logDataGridTiming("[Gauss Horizon][DataGrid:result:prop]", {
      traceId: dataGridTraceId,
      cacheKey: props.cacheKey,
    ...
    nextTick(() => {
      logDataGridTiming("[Gauss Horizon][DataGrid:result:nextTick]", {
        traceId: dataGridTraceId,
        cacheKey: props.cacheKey,
      ...
      requestAnimationFrame(() => {
        logDataGridTiming("[Gauss Horizon][DataGrid:result:first-frame]", {
          traceId: dataGridTraceId,
          cacheKey: props.cacheKey,
          elapsed: `${Math.round(performance.now() - startedAt)}ms`,
          loading: props.loading,
...
const gridStyle = computed(() => ({
  ...
  "--gauss-horizon-data-grid-font-family": tableFontFamily.value,
  "--gauss-horizon-table-font-size": `${tableFontSize.value}px`,
}));
...
watch(
  ...
  (length) => {
    ...
    if (shouldLogTiming) {
      logDataGridTiming("[Gauss Horizon][DataGrid:display-items:ready]", {
        traceId: dataGridTraceId,
        cacheKey: props.cacheKey,
    ...
    nextTick(() => {
      ...
      requestAnimationFrame(() => {
        ...
        logDataGridTiming("[Gauss Horizon][DataGrid:display-items:first-frame]", {
          traceId: dataGridTraceId,
          cacheKey: props.cacheKey,
          displayItemCount: length,
          renderedRows,
...
onMounted(() => {
  ...
  window.addEventListener("gauss-horizon:ui-scale-applied", refreshDataGridViewportMetrics);
  window.addEventListener(TABLE_DATA_GRID_COLUMN_ORDER_CHANGED_EVENT, onSynchronizedTableDataGridColumnOrderChanged);
  window.addEventListener("gauss-horizon:before-tab-switch", captureTabSwitchViewSnapshot);
...
onUnmounted(() => {
  ...
  // `gauss-horizon:before-tab-switch` capture and wins, because it observes the final
  ...
  window.removeEventListener("gauss-horizon:ui-scale-applied", refreshDataGridViewportMetrics);
  window.removeEventListener(TABLE_DATA_GRID_COLUMN_ORDER_CHANGED_EVENT, onSynchronizedTableDataGridColumnOrderChanged);
  window.removeEventListener("gauss-horizon:before-tab-switch", captureTabSwitchViewSnapshot);
...
async function syncUserFacingSql() {
  ...
  const shouldRebuildSql = executionSql.includes("__GAUSS_HORIZON_LARGE_VALUE_BYTES_") || includeDatabaseName;
      ...
      try {
        ...
        if (config.referenceMode === "manual") {
          ...
          if (!manualReferenceKeyColumnIsUnique(columns, referenceKeys, config.refColumn, config.filter)) {
            appendDebugLog("warn", "[Gauss Horizon][DataGrid:foreign-key-display:unsafe-manual-reference]", {
        ...
        if (!foreignKeyDisplayRequests.isCurrent(requestGeneration)) return;
        appendDebugLog("warn", "[Gauss Horizon][DataGrid:foreign-key-display:error]", {
          column: props.result.columns[columnIndex],
...
watch(gridSurfaceBusy, (isLoading) => {
  stopLoadingElapsedTimer();
  if (isDebugLoggingEnabled()) {
    logDataGridTiming(isLoading ? "[Gauss Horizon][DataGrid:loading:start]" : "[Gauss Horizon][DataGrid:loading:stop]", {
      traceId: dataGridTraceId,
      cacheKey: props.cacheKey,
      elapsedSinceSetup: dataGridElapsed(),
    });
  ...
  if (isLoading) {
    ...
    nextTick(() => {
      requestAnimationFrame(() => {
        logDataGridTiming("[Gauss Horizon][DataGrid:loading:stop:first-frame]", {
          traceId: dataGridTraceId,
          cacheKey: props.cacheKey,
          elapsedSinceSetup: dataGridElapsed(),
        });
            ...
            <template v-else>
              ...
              >
                ...
                >
                  <canvas
                    ref="canvasRef"
                    class="canvas-grid-surface gauss-horizon-data-grid-font-family sticky left-0 top-0 z-0 block font-normal"
                    :style="{
                  ...
                  <canvas
                    ...
                    @contextmenu="onCanvasContext"
                  />
                  <div ref="canvasOverlayRef" class="canvas-grid-overlay gauss-horizon-data-grid-font-family sticky left-0 top-0 z-10 overflow-visible" :style="canvasOverlayStyle" @dblclick.stop>
              ...
              <div v-else-if="hasVisibleRows" class="relative min-h-0 flex-1">
                <RecycleScroller
                  ref="scrollerRef"
                  class="data-grid-scroller gauss-horizon-data-grid-font-family h-full overflow-x-auto overscroll-none"
                  :class="{
                    'is-scrolling': isScrolling,
                    'has-horizontal-scrollbar': hasGridHorizontalOverflow,
                  }"
...
.data-grid-transpose-row {
  font-family: var(--gauss-horizon-data-grid-font-family);
