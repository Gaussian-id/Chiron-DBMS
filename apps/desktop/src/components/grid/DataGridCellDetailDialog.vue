    ...
    <DialogContent v-if="detail" class="sm:max-w-[840px] max-h-[85vh] flex flex-col overflow-hidden">
      ...
      <div class="min-h-0 flex-1 overflow-auto pr-1 text-xs space-y-4">
        ...
        <div class="space-y-2">
          ...
          <pre v-else class="chiron-horizon-data-grid-value-font max-h-[44vh] overflow-auto rounded border bg-muted/20 p-3 text-xs whitespace-pre-wrap break-words" :class="{ 'italic text-muted-foreground': detail.value === null }">{{ presentedValuePreview }}</pre>
