...
const backend = vi.hoisted(() => ({ chirondbRequest: vi.fn(), vectorListCollections: vi.fn(), execution: vi.fn() }));
...
beforeEach(() => {
  backend.execution.mockReset();
  backend.chirondbRequest.mockReset();
  backend.chirondbRequest.mockImplementation((_id, request) => {
    if (request.operation === "parse") return Promise.resolve({ status: 200, body: { ok: true, kind: /DELETE|UPSERT/.test(request.query) ? "write" : "read", statement: request.query.trim().split(" ")[0] } });
    return backend.execution(request);
  });
...
describe("ChironDB workspace", () => {
  it("executes one unchanged statement and shows native diagnostics", async () => {
    backend.execution.mockResolvedValue({ status: 200, body: { kind: "rows", columns: ["id", "payload"], rows: [{ id: "α", payload: { tags: [1, true] } }], stats: { took_ms: 1.25 }, query_id: "q-test" } });
  ...
  it("does not send a confirmation after cancellation", async () => {
    backend.execution.mockResolvedValue({ status: 400, body: { code: "dbm.confirmation_required", error: "Confirm delete" } });
    ...
    await flush();
    expect(backend.execution).not.toHaveBeenCalled();
  ...
  it("confirms the exact snapshot and retains the server's affected-count gate", async () => {
    ...
    button("chiron.confirm").click();
    await flush();
    expect(backend.execution.mock.calls[1][0]).toMatchObject({ confirm: false, allow_destructive: true });
    expect(root.textContent).toContain("12");
    button("chiron.confirm").click();
    await flush();
    expect(backend.execution.mock.calls[2][0]).toMatchObject({ query: "DELETE FROM demo WHERE category = 'old';", confirm: true, allow_destructive: true });
  });

  it("passes opaque cursors without transforming them", async () => {
    backend.execution.mockResolvedValueOnce({ status: 200, body: { points: [{ id: "1" }], next_offset: "opaque/+α" } }).mockResolvedValueOnce({ status: 200, body: { points: [], next_offset: null } });
  ...
  it("never retries an uncertain write", async () => {
    backend.execution.mockRejectedValue(new Error("Outcome unknown"));
    await mount("UPSERT demo;");
    button("chiron.run").click();
    await flush();
    button("chiron.confirm").click();
    await flush();
    expect(backend.execution).toHaveBeenCalledTimes(1);
