...
describe("MeilisearchDocumentsPage identity and canonical documents", () => {
  it("serializes numeric-looking string ids with the store-id serializer on delete", async () => {
    ...
    expect(mocks.deleteDocument).toHaveBeenCalledWith("c1", "default", "movies", '__gauss_horizon_meilisearch_string_id__"123"');
  });
  ...
  it("edits the canonical document instead of the partial search hit", async () => {
    ...
    expect(mocks.getDocument).toHaveBeenCalledWith("c1", "movies", '__gauss_horizon_meilisearch_string_id__"123"');
    await vi.waitFor(() => {
