...
beforeEach(() => {
  localStorage.removeItem("gauss-horizon:meilisearch:key-columns:v1");
...
describe("MeilisearchKeysPage", () => {
  ...
  it("copies the full key while displaying only its masked value, and persists column visibility", async () => {
    ...
    checkboxes[1].click();
    const stored = JSON.parse(localStorage.getItem("gauss-horizon:meilisearch:key-columns:v1") || "null");
