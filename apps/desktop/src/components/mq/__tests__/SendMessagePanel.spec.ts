...
describe("SendMessagePanel post-send browsing", () => {
  for (const system of ["kafka", "rabbitmq"] as const) {
    it(`keeps ${system} results unchanged after sending`, async () => {
      ...
      expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expectedTopic(system), "__chiron_horizon_kafka_viewer__", 20, system === "kafka" ? { startPosition: "latest" } : {});
  ...
  it("sends Kafka's selected start position and specific offset", async () => {
    ...
    await loadMessages(panel);
    expect(backend.mqPeekMessages).toHaveBeenLastCalledWith("mq-1", expectedTopic("kafka"), "__chiron_horizon_kafka_viewer__", 20, { startPosition: "earliest" });
    ...
    await loadMessages(panel);
    expect(backend.mqPeekMessages).toHaveBeenLastCalledWith("mq-1", expectedTopic("kafka"), "__chiron_horizon_kafka_viewer__", 20, { startPosition: "offset", partition: 2, offset: 17 });
  });
  ...
  it("keeps Kafka's result limit within the supported range", async () => {
    ...
    expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expectedTopic("kafka"), "__chiron_horizon_kafka_viewer__", 100, { startPosition: "latest" });

    await setInputValue(countInput, "");
    await loadMessages(panel);

    expect(countInput.value).toBe("20");
    expect(backend.mqPeekMessages).toHaveBeenLastCalledWith("mq-1", expectedTopic("kafka"), "__chiron_horizon_kafka_viewer__", 20, { startPosition: "latest" });
  });
  ...
  it("does not request Kafka offset mode without an offset", async () => {
    ...
    await setInputValue(offsetInput, "17");
    await loadMessages(panel);
    expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expectedTopic("kafka"), "__chiron_horizon_kafka_viewer__", 20, { startPosition: "offset", offset: 17 });
  });
  ...
  it("does not leak a saved Kafka offset after switching start positions", async () => {
    ...
    expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expectedTopic("kafka"), "__chiron_horizon_kafka_viewer__", 20, { startPosition: "latest", partition: 2 });
    await setSelectValue(startPosition, "offset");
  ...
  it("keeps RabbitMQ advanced-filter requests unchanged", async () => {
    ...
    await loadMessages(panel);
    expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expectedTopic("rabbitmq"), "__chiron_horizon_kafka_viewer__", 20, { partition: 2, offset: 17 });
  });
});
