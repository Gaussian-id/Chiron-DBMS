...
describe("MessageBrowser", () => {
  ...
  it("loads Kafka's latest messages by default", async () => {
    ...
    expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expect.objectContaining({ topic: "events" }), "__gauss_horizon_kafka_viewer__", 20, { startPosition: "latest" });
  });
  ...
  it("normalizes a decimal count before sending the request", async () => {
    ...
    expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expect.objectContaining({ topic: "events" }), "__gauss_horizon_kafka_viewer__", 2, { startPosition: "latest" });
  });
  ...
  it("sends explicit earliest and offset read positions", async () => {
    ...
    await setSelectValue(startPosition, "earliest");
    await loadMessages(browser);
    expect(backend.mqPeekMessages).toHaveBeenLastCalledWith("mq-1", expect.objectContaining({ topic: "events" }), "__gauss_horizon_kafka_viewer__", 20, { startPosition: "earliest" });
    ...
    expect(backend.mqPeekMessages).toHaveBeenLastCalledWith("mq-1", expect.objectContaining({ topic: "events" }), "__gauss_horizon_kafka_viewer__", 20, { startPosition: "offset", partition: 2, offset: 17 });
  });
  ...
  it("allows an all-partition offset read but requires an offset", async () => {
    ...
    await setInputValue(offset, "17");
    await loadMessages(browser);
    expect(backend.mqPeekMessages).toHaveBeenLastCalledWith("mq-1", expect.objectContaining({ topic: "events" }), "__gauss_horizon_kafka_viewer__", 20, { startPosition: "offset", offset: 17 });
  });
  ...
  it("clears results and does not leak an offset into a different read mode", async () => {
    ...
    await loadMessages(browser);
    expect(backend.mqPeekMessages).toHaveBeenLastCalledWith("mq-1", expect.objectContaining({ topic: "events" }), "__gauss_horizon_kafka_viewer__", 20, { startPosition: "latest", partition: 2 });
  });
  ...
  it("keeps RabbitMQ's advanced filters collapsed and sends its existing request shape", async () => {
    ...
    expect(backend.mqPeekMessages).toHaveBeenCalledWith("mq-1", expect.objectContaining({ tenant: "_rabbitmq", topic: "events" }), "__gauss_horizon_kafka_viewer__", 20, { partition: 2, offset: 17 });
  });
});
