...
/** Collect resolved Chiron Horizon design tokens so the sandbox can theme itself with the same values. */
function currentBridgeTheme(): PluginBridgeTheme {
  const tokens: Record<string, string> = {};
  if (typeof document !== "undefined") {
    const style = getComputedStyle(document.documentElement);
    for (const name of style) {
      if (!name.startsWith("--") || name.startsWith("--chiron-horizon-")) continue;
...
function localUiAssetPath(source: string): string | undefined {
  ...
  try {
    const resolved = new URL(trimmed, "https://chiron-horizon-plugin.invalid/");
    if (resolved.origin !== "https://chiron-horizon-plugin.invalid") return undefined;
