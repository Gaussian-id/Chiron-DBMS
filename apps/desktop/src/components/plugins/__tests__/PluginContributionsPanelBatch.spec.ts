...
function installed(id: string, version = "1.0.0"): InstalledPlugin {
  return {
    manifest: { manifest_version: 1, id, name: id, version, publisher: "Chiron Horizon", description: "", engines: { chiron_horizon: "", host_api: "" }, permissions: [], entrypoints: {}, contributions: [], drivers: [], protocol_version: 1 },
  ...
  return {
    ...
    catalog: {
      ...
      plugins: ids.map((id) => ({
        ...
        versions: [{ version, artifacts: [{ target: "darwin-arm64", url: "https://example.invalid/plugin.chiron_horizonp", sha256: "a".repeat(64), signingKeyId: "test.key" }] }],
      })),
    },
  };
}
...
beforeEach(async () => {
  ...
  state.installUrl = "https://example.invalid/plugin.chiron_horizonp";
...
function startSingle(single: Single) {
  switch (single) {
    ...
    case "package":
      return state.installPlugin("plugin.chiron_horizonp");
    case "url":
...
describe("PluginContributionsPanel completed batch outcomes", () => {
  it.each(["marketplace", "package", "url", "rollback"] as const)("shows related connection names when %s is blocked", async (entry) => {
    ...
    else if (entry === "package") await state.installPlugin("plugin.chiron_horizonp");
