...
import {
  ...
  GAUSS_HORIZON_TABLE_REFERENCE_DROP_EVENT,
  GAUSS_HORIZON_TABLE_REFERENCE_HOVER_EVENT,
  GAUSS_HORIZON_TABLE_REFERENCE_DRAG_END_EVENT,
...
import { createGaussHorizonCodeMirrorSqlDialect, type CodeMirrorSqlDialectName } from "@/lib/editor/codemirrorSqlDialect";
...
let gaussHorizonVimCommandsConfigured = false;
...
const GAUSS_HORIZON_VIM_SAVE_EVENT = "gauss-horizon-vim-save";
const BEFORE_TAB_SWITCH_EVENT = "gauss-horizon:before-tab-switch";
...
function syncEditorDiagnosticCssVars() {
  ...
  editorRef.value.style.setProperty("--gauss-horizon-editor-diagnostic-error", colors.error);
  editorRef.value.style.setProperty("--gauss-horizon-editor-diagnostic-warning", colors.warning);
}
...
// anyway, ignoring their setting (gauss-horizon#6236).
...
function updateEditorSelectionDropCursor(currentView: EditorViewType, event: MouseEvent) {
  ...
  if (!editorSelectionDropCursorEl) {
    ...
    cursor.className = "gauss-horizon-editor-selection-drop-cursor";
...
function vimModeExtension(enabled = settingsStore.editorSettings.vimModeEnabled) {
  ...
  // Beekeeper treats Vim as a first-class editor keymap. Keep it above Gauss Horizon's
...
function configureGaussHorizonVimCommands(vimApi: typeof import("@replit/codemirror-vim").Vim) {
  if (gaussHorizonVimCommandsConfigured) return;
  gaussHorizonVimCommandsConfigured = true;
  vimApi.defineEx("write", "w", (cm) => {
    cm.cm6?.contentDOM.dispatchEvent(new CustomEvent(GAUSS_HORIZON_VIM_SAVE_EVENT, { bubbles: true }));
  });
}
...
async function ensureCodeMirrorVim() {
  ...
  configureGaussHorizonVimCommands(Vim);
  return true;
}
...
async function ensureForeignKeysForTable(table: { name: string; database?: string | null; schema?: string | null }) {
  ...
  try {
    ...
    console.warn(`[Gauss Horizon] Failed to load foreign keys for ${cacheKey}:`, e);
...
async function resolveSqlHoverTooltip(currentView: EditorViewType, pos: number) {
  ...
  if (SEMANTIC_SQL_COMPLETION_ENABLED) {
    try {
      ...
      semanticModel = null;
      console.warn(`[Gauss Horizon] Failed to build semantic model for hover tooltip:`, error);
    }
  }
  ...
  try {
    ...
    if (table && settingsStore.editorSettings.showTableDdlHoverPreview && !semanticQualifierIsRowSource && (!qualifier || table.schema?.toLowerCase() === qualifier.toLowerCase() || table.name === name)) {
      ...
      try {
        ...
        console.warn(`[Gauss Horizon] Failed to load table DDL for ${hoverDatabase}.${hoverSchema}.${table.name}:`, error);
      }
      ...
      if (!sqlContent) {
        ...
        try {
          ...
          console.warn(`[Gauss Horizon] Failed to load table metadata for ${hoverDatabase}.${hoverSchema}.${table.name}:`, error);
        }
        if (!metadataLoadFailed) {
          try {
            ...
            console.warn(`[Gauss Horizon] Failed to load table comment for ${hoverDatabase}.${hoverSchema}.${table.name}:`, error);
          }
        }
      ...
      return {
        ...
        create: () => createHoverDom(table.name, sqlObjectHoverDetail(table), sqlContent, metadataLoadFailed ? ["[Gauss Horizon] Failed to load table structure — check connection"] : undefined),
      };
    }
...
function droppedTableReference(event: DragEvent) {
  return activeTableReferencePayloadValue() ?? parseTableReferencePayload(event.dataTransfer?.getData(GAUSS_HORIZON_TABLE_REFERENCE_MIME));
}
...
function registerTableReferenceDropListener() {
  ...
  window.addEventListener(GAUSS_HORIZON_TABLE_REFERENCE_HOVER_EVENT, onTableReferenceHoverEvent);
  window.addEventListener(GAUSS_HORIZON_TABLE_REFERENCE_DRAG_END_EVENT, onTableReferenceDragEndEvent);
...
function unregisterTableReferenceDropListener() {
  ...
  window.removeEventListener(GAUSS_HORIZON_TABLE_REFERENCE_HOVER_EVENT, onTableReferenceHoverEvent);
  window.removeEventListener(GAUSS_HORIZON_TABLE_REFERENCE_DRAG_END_EVENT, onTableReferenceDragEndEvent);
...
type QueryCompletionOption = Completion & {
  gaussHorizonBatchColumnSelection?: { sessionKey: string; candidateKey: string };
  gaussHorizonBatchColumnSelectionAction?: { sessionKey: string };
};

let batchColumnSelectionSession: BatchColumnSelectionSession | null = null;
type BatchColumnSelectionCheckboxMarker = NonNullable<QueryCompletionOption["gaussHorizonBatchColumnSelection"]>;
  ...
  restoreStartTimer = window.setTimeout(() => {
    ...
    const focusIndex =
      codeMirrorCurrentCompletions?.(view.state).findIndex((completion) => {
        const marker = (completion as QueryCompletionOption).gaussHorizonBatchColumnSelection;
...
function batchColumnSelectionMarkerForItem(item: QueryCompletionItem): QueryCompletionOption["gaussHorizonBatchColumnSelection"] | undefined {
...
function cacheBatchColumnSelectionOption(marker: QueryCompletionOption["gaussHorizonBatchColumnSelection"], option: QueryCompletionOption): QueryCompletionOption {
...
function toggleSelectedBatchColumnSelection(view: EditorViewType): boolean {
  ...
  const marker = completion?.gaussHorizonBatchColumnSelection;
...
function renderBatchColumnSelectionActionMarker(completion: Completion): Node | null {
  const action = (completion as QueryCompletionOption).gaussHorizonBatchColumnSelectionAction;
...
function completionOptionForItem(item: QueryCompletionItem | BatchColumnSelectionActionItem) {
  ...
  if (isBatchColumnSelectionAction(item)) {
    return {
      ...labelPresentation,
      gaussHorizonBatchColumnSelectionAction: { sessionKey: item.sessionKey },
      type: item.type,
      detail: item.detail,
      boost: item.boost,
  ...
  if (shouldApplyCompletionAsSnippet(item) && item.apply) {
    ...
    return cacheBatchColumnSelectionOption(batchColumnSelection, {
      ...completion,
      ...(batchColumnSelection ? { gaussHorizonBatchColumnSelection: batchColumnSelection } : {}),
    ...
    await Promise.all(
      columnRefs.map(async (refTable) => {
        ...
        try {
          ...
          console.error(`[Gauss Horizon] Failed to load columns for ${cacheKey}:`, e);
        }
      }),
    );
  }
...
onMounted(async () => {
  ...
  const diagnosticTheme = EditorView.baseTheme({
    ".cm-sql-error": {
      textDecoration: "underline wavy var(--gauss-horizon-editor-diagnostic-error, var(--destructive))",
      textUnderlineOffset: "3px",
    },
    ".cm-sql-semantic-warning": {
      textDecoration: "underline wavy var(--gauss-horizon-editor-diagnostic-warning, var(--warning))",
      textUnderlineOffset: "3px",
    },
  });
  ...
  buildSqlCompletionExtension = () =>
    autocompletion({
      ...
      optionClass: (completion) => ((completion as QueryCompletionOption).gaussHorizonBatchColumnSelectionAction ? "cm-batch-column-selection-action" : ""),
      addToOptions: [
  ...
  buildSqlLanguageExtension = () => [
    langSql.sql({
      dialect: createGaussHorizonCodeMirrorSqlDialect(langSql, props.syntaxDialect ?? props.dialect, props.databaseType, sqlDriverProfile.value),
    }),
  ...
  const tooltipParent = editorElement.closest<HTMLElement>("#root")?.querySelector<HTMLElement>("#gauss-horizon-query-editor-tooltip-root") ?? editorElement;
  const state = EditorState.create({
    ...
    extensions: [
      ...
      // Vim must be mounted before Gauss Horizon/default keymaps so normal-mode keys are handled first.
        ...
        mousedown: (event: MouseEvent) => {
          ...
          setTimeout(async () => {
            try {
              ...
              console.error("[Gauss Horizon] Ctrl+click error:", e);
            }
          }, 0);
          return true;
        },
...
:deep(.cm-db-execution-preview) {
  background: var(--gauss-horizon-editor-selection-background, rgba(59, 130, 246, 0.35));
}

:deep(.cm-db-result-source-highlight) {
  background: var(--gauss-horizon-editor-selection-background, rgba(126, 34, 206, 0.2));
}
...
:deep(.cm-statement-execution-marker) {
  ...
  justify-content: center;
  box-sizing: border-box;
  width: min(24px, calc(var(--gauss-horizon-editor-font-size, 13px) * 1.6));
  height: min(24px, calc(var(--gauss-horizon-editor-font-size, 13px) * 1.6));
  margin: 0;
  padding: 0;
...
:deep(.cm-statement-execution-spinner) {
  animation: gauss-horizon-statement-execution-spin 0.8s linear infinite;
}

@keyframes gauss-horizon-statement-execution-spin {
  to {
