...
describe("QueryEditor context menu lifecycle", () => {
  ...
  it("uses the target synchronized in the current context-menu event", async () => {
    ...
    const open = async (id: string) => {
      ...
      await nextTick();
      return document.querySelector<HTMLButtonElement>("[data-gauss-horizon-context-menu] button");
    };
...
describe("QueryEditor batch column selection", () => {
  ...
  it("marks the insertion action so the completion menu can keep it sticky", () => {
    expect(source).toContain("gaussHorizonBatchColumnSelectionAction: { sessionKey: item.sessionKey }");
    expect(source).toContain('optionClass: (completion) => ((completion as QueryCompletionOption).gaussHorizonBatchColumnSelectionAction ? "cm-batch-column-selection-action" : "")');
  });
