...
describe("EditorSettingsDialog live preview placement", () => {
  ...
  it("places execution behavior before SQL completion with high-priority controls first", () => {
    ...
    expect(editorSection).toContain('class="settings-form-row settings-item flex items-start justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2" data-editor-completion-trigger-mode');
    expect(editorSection).toContain('<SelectTrigger class="h-8 w-44 shrink-0">');
    expect(editorSection).toContain('class="settings-responsive-grid grid gap-4 md:grid-cols-2" data-editor-execution-settings');
    expect(editorSection).toContain('class="settings-form-row settings-item flex items-center justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2 md:col-span-2" data-editor-execute-mode');
