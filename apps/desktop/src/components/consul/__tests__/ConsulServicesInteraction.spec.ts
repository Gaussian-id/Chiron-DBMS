    ...
    serviceMaintenance: vi.fn(async (_connectionId: string, id: string, enable: boolean) => {
      ...
      if (enable) {
        agentChecks[checkId] = {
          ...
          Output: "",
          ServiceID: id,
          ServiceName: "chiron-horizon-demo-api",
          ServiceTags: ["demo"],
          Type: "maintenance",
          ExposedPort: 0,
          Definition: {},
      ...
      items: [
        {
          ...
          ServicePort: 19080,
          ServiceMeta: { owner: "Chiron Horizon" },
...
vi.mock("@/lib/backend/api", () => ({
  consulCatalogServices: vi.fn(async () => ({
    items: { consul: [], "chiron-horizon-demo-api": ["v1", "demo", "api"] },
    metadata: { index: "1", knownLeader: true, lastContact: 0, queryBackend: null },
  })),
  ...
  consulCatalogNodeServices: vi.fn(async () => ({
    items: {
      ...
      Services: {
        "chiron-horizon-demo-node-service-1": {
          ID: "chiron-horizon-demo-node-service-1",
          Service: "chiron-horizon-demo-node-service",
          Tags: ["demo", "node"],
          Address: "127.0.0.1",
          TaggedAddresses: { lan: { Address: "127.0.0.1", Port: 19090 } },
          Meta: { owner: "Chiron Horizon" },
          Port: 19090,
  ...
  consulAgentServices: vi.fn(async () => ({
    "chiron-horizon-demo-api-1": { Kind: "", ID: "chiron-horizon-demo-api-1", Service: "chiron-horizon-demo-api", Tags: ["demo"], Meta: {}, Port: 19080, Address: "127.0.0.1", TaggedAddresses: {}, Weights: { Passing: 1, Warning: 1 }, EnableTagOverride: false, Datacenter: "dc1" },
  })),
  consulAgentChecks: vi.fn(async () => ({ ...mocks.agentChecks })),
  consulAgentService: vi.fn(async (_connectionId: string, id: string) => ({
    Kind: "",
    ID: id,
    Service: "chiron-horizon-demo-api",
    Tags: ["demo"],
    Meta: { owner: "Chiron Horizon" },
    Port: 19080,
    Address: "127.0.0.1",
...
describe("ConsulServices interactions", () => {
  ...
  it("selects a Catalog service and renders its instances on click", async () => {
    ...
    const serviceButton = Array.from(host.querySelectorAll("button")).find((button) => button.textContent?.includes("chiron-horizon-demo-api"));
    expect(serviceButton).toBeTruthy();
    expect(serviceButton!.textContent).toBe("chiron-horizon-demo-api");
    serviceButton!.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
    await settle();

    expect(host.querySelector("h2")?.textContent).toContain("chiron-horizon-demo-api");
    expect(host.textContent).toContain("chiron-horizon-demo-api-1");
    expect(mocks.catalogServiceNodes).toHaveBeenLastCalledWith("connection-1", "chiron-horizon-demo-api");
  ...
  it("keeps local service cards interactive and deregistration disabled when the Agent target is missing", async () => {
    ...
    const localServiceButton = Array.from(host.querySelectorAll("button")).find((button) => button.textContent?.includes("chiron-horizon-demo-api-1"));
  ...
  it("shows and toggles the current service maintenance state", async () => {
    ...
    expect(mocks.serviceMaintenance).toHaveBeenLastCalledWith("connection-1", "chiron-horizon-demo-api-1", true, "consul.ui.maintenanceReason");
    ...
    expect(mocks.serviceMaintenance).toHaveBeenLastCalledWith("connection-1", "chiron-horizon-demo-api-1", false, "consul.ui.maintenanceReason");
  ...
  it("expands and collapses full service details in Catalog node view", async () => {
    ...
    const nodeServiceButton = Array.from(host.querySelectorAll("button")).find((button) => button.textContent?.includes("chiron-horizon-demo-node-service-1"));
    expect(nodeServiceButton).toBeTruthy();
    nodeServiceButton!.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
    await settle();
    expect(host.textContent).toContain("owner=Chiron Horizon");
    expect(host.textContent).toContain("consul.ui.serviceWeights");
