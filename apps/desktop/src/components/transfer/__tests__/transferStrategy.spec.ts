...
function preview(overrides: Partial<TransferOwnershipPreview> = {}): TransferOwnershipPreview {
  return {
    ...
    rebuild: {
      sql: 'ALTER TABLE "reporting"."orders" RENAME TO "orders__gauss_horizon_bak_123";\nCREATE TABLE "reporting"."orders" ("id" INTEGER);',
      tables: [{ sourceTable: "Orders", targetTable: '"reporting"."orders"', backupTable: '"reporting"."orders__gauss_horizon_bak_123"' }],
    },
    ...overrides,
  };
}
...
describe("transfer submission", () => {
  it("reviews backend SQL and executes the frozen request only after confirmation", async () => {
    ...
    expect(reviewed[0]?.preview.rebuild?.sql).toBe('ALTER TABLE "reporting"."orders" RENAME TO "orders__gauss_horizon_bak_123";\nCREATE TABLE "reporting"."orders" ("id" INTEGER);');
