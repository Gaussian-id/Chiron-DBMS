...
mod tests {
    ...
    fn open_query_and_backup() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-sqlite-worker-{}", uuid_like()));
    ...
    fn open_rejects_missing_files_instead_of_creating_them() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-sqlite-worker-missing-{}.db", uuid_like()));
        let mut connection = None;
        let open = handle(
            &mut connection,
    ...
    fn multi_statement_scripts_run_every_statement() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-sqlite-worker-multi-{}", uuid_like()));
        std::fs::create_dir_all(&dir).unwrap();
        let db = dir.join("app.db");
        rusqlite::Connection::open(&db).unwrap();
        let mut connection = None;
    ...
    fn returning_dml_still_produces_a_result_set() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-sqlite-worker-returning-{}", uuid_like()));
        std::fs::create_dir_all(&dir).unwrap();
        let db = dir.join("app.db");
        rusqlite::Connection::open(&db).unwrap();
        let mut connection = None;
    ...
    fn blobs_are_hex_and_text_affinity_columns_show_as_text() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-sqlite-worker-blob-{}", uuid_like()));
        std::fs::create_dir_all(&dir).unwrap();
        let db = dir.join("app.db");
        {
