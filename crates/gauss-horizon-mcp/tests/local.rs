...
use gauss_horizon_core::{models::connection::ConnectionConfig, storage::Storage};
use gauss_horizon_mcp::{GaussHorizonBackend, GaussHorizonMcpServer, LocalBackend, McpScope};
...
impl EnvVarGuard {
    fn set(name: &'static str, value: &str) -> Self {
        let original = gauss_horizon_core::legacy::var_os(name);
...
#[tokio::test]
async fn local_backend_reads_gauss_horizon_storage_without_desktop_process() {
    ...
    let result = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_list_connections"))
        .await
...
async fn duplicate_connection_preserves_secrets_ssh_and_sidebar_group() {
    ...
    let storage = Storage::open(&db_path).await.expect("open storage");
    storage
        .save_mcp_global_policy(&gauss_horizon_core::storage::McpGlobalPolicy {
            read_only: false,
    ...
    let source: ConnectionConfig = serde_json::from_value(json!({
        ...
        "url_params": "application_name=gauss-horizon",
        "connection_string": "postgres://app:database-secret@db.internal/app",
        "init_script": "SET application_name = 'gauss-horizon-secret-script'",
        "save_password": true,
        "ssl": true,
        "transport_layers": [{
            "type": "ssh",
    ...
    let result = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_duplicate_connection").with_arguments(arguments.clone()))
        .await
    ...
    let duplicate_again = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_duplicate_connection").with_arguments(arguments))
        .await
    ...
    let missing = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_duplicate_connection").with_arguments(missing_arguments))
        .await
    ...
    let root_copy = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_duplicate_connection").with_arguments(root_arguments))
        .await
    ...
    let read_only = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_duplicate_connection").with_arguments(read_only_arguments))
        .await
...
async fn local_backend_picks_up_connections_added_after_startup_without_reload() {
    // Regression for issue #5428: after an agent connects to MCP, a connection created in the Gauss Horizon
    ...
    // Simulate a connection created in the Gauss Horizon desktop UI: write directly to the shared storage,
...
async fn local_backend_uses_the_installed_duckdb_sidecar() {
    ...
    let _driver_path = EnvVarGuard::set("GAUSS_HORIZON_DUCKDB_DRIVER_PATH", &missing_driver.to_string_lossy());
    let db_path = directory.path().join("gauss-horizon.db");
    let storage = Storage::open(&db_path).await.expect("open storage");
    let connection: ConnectionConfig = serde_json::from_value(json!({
        "id": "local-duckdb",
    ...
    assert!(error.contains("GAUSS_HORIZON_DUCKDB_DRIVER_PATH"), "unexpected DuckDB error: {error}");
...
async fn legacy_read_only_config_applies_before_settings_are_opened() {
    const CHILD_ENV: &str = "GAUSS_HORIZON_MCP_LEGACY_READ_ONLY_TEST_CHILD";
    if gauss_horizon_core::legacy::var_os(CHILD_ENV).is_none() {
        let status = std::process::Command::new(std::env::current_exe().expect("locate test executable"))
            ...
            .env(CHILD_ENV, "1")
            .env("GAUSS_HORIZON_MCP_ALLOW_WRITES", "0")
            .status()
...
async fn executes_mongo_shell_commands_without_desktop_process() {
    ...
    let connection: ConnectionConfig = serde_json::from_value(json!({
        ...
        "username": "root",
        "password": password,
        "database": "gauss_horizon_mcp_test",
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_MCP_TEST_MONGO_HOST and GAUSS_HORIZON_MCP_TEST_MONGO_PORT pointing at MongoDB 4.0+"]
...
async fn executes_legacy_mongo_find_explain_without_desktop_process() {
    ...
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_PORT")
        ...
        .parse::<u16>()
        .expect("MongoDB port");
    let collection = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_COLLECTION").unwrap_or_else(|_| "im_msg".to_string());
    let directory = tempdir().expect("temporary data directory");
    let db_path = directory.path().join("gauss-horizon.db");
    let storage = Storage::open(&db_path).await.expect("open storage");
    let connection: ConnectionConfig = serde_json::from_value(json!({
...
fn mcp_default_features_include_message_queue_admin() {
    assert_eq!(gauss_horizon_core::mq::MqSystemKind::Kafka.as_str(), "kafka");
}
