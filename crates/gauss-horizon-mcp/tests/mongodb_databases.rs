...
fn live_mongo_connection() -> ConnectionConfig {
    ...
    connection.host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_HOST").expect("MongoDB host");
    connection.port =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_PORT").unwrap_or_else(|_| "27017".to_string()).parse().expect("MongoDB port");
    connection.password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_PASSWORD").expect("MongoDB password");
    connection
}
...
async fn list_databases(
    ...
    let result = tokio::time::timeout(
        ...
        client.peer().call_tool(
            CallToolRequestParams::new("gauss_horizon_list_databases")
...
async fn mongodb_agent_discovery_keeps_one_health_check_for_a_warm_pool() {
    ...
    use gauss_horizon_mcp::GaussHorizonBackend;
...
"#,
    ...
    let storage_path = directory.path().join("gauss-horizon.db");
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_MCP_TEST_MONGO_HOST and GAUSS_HORIZON_MCP_TEST_MONGO_PASSWORD (root, authSource=admin)"]
