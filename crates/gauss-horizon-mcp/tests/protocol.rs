...
use gauss_horizon_mcp::{GaussHorizonBackend, GaussHorizonMcpServer, McpScope};
...
#[async_trait]
impl GaussHorizonBackend for EmptyBackend {
...
#[async_trait]
impl GaussHorizonBackend for PolicyBackend {
    async fn load_mcp_global_policy(&self) -> Result<McpGlobalPolicy, String> {
...
#[async_trait]
impl GaussHorizonBackend for CapturingBackend {
    ...
    async fn peek_messages(
        ...
        Ok(gauss_horizon_core::mq::PeekMessagesResult::complete(vec![gauss_horizon_core::mq::PeekedMessage {
...
/// Drives one `gauss_horizon_execute_query` call against a `CapturingBackend`, returning
...
async fn captured_query_arguments(backend: Arc<CapturingBackend>, sql_arguments: Value) -> Vec<Value> {
    ...
    let _ = client
        .peer()
        .call_tool(
            CallToolRequestParams::new("gauss_horizon_execute_query")
...
/// `timeout_secs` into the `gauss_horizon_execute_query` arguments whenever the persisted
...
async fn initializes_lists_tools_and_calls_a_tool() {
    let (server_transport, client_transport) = tokio::io::duplex(16 * 1024);
    let server = GaussHorizonMcpServer::with_runtime_options(Arc::new(EmptyBackend), McpScope::default(), false);
    let server_task = tokio::spawn(async move { server.serve(server_transport).await });
    let client = ().serve(client_transport).await.expect("initialize MCP client");
    ...
    let result = client.peer().call_tool(CallToolRequestParams::new("gauss_horizon_list_connections")).await.expect("call tool");
    let response = result.content[0].as_text().expect("text response");
    assert_eq!(response.text, "No connections configured in Gauss Horizon.");
...
async fn kafka_peek_round_trips_over_mcp_in_local_and_web_modes() {
    ...
    connection.db_type = gauss_horizon_core::models::connection::DatabaseType::MessageQueue;
    ...
    for web_mode in [false, true] {
        ...
        let (server_transport, client_transport) = tokio::io::duplex(16 * 1024);
        let server = GaussHorizonMcpServer::with_runtime_options(backend.clone(), McpScope::default(), web_mode);
        let server_task = tokio::spawn(async move { server.serve(server_transport).await });
        let client = ().serve(client_transport).await.unwrap();
        let result = client.peer().call_tool(CallToolRequestParams::new("gauss_horizon_peek_messages").with_arguments(json!({"connection_name":"Kafka", "topic":"events", "count":100, "start_position":"offset", "partition":0, "offset":120}).as_object().unwrap().clone())).await.unwrap();
        ...
        let invalid = client
            .peer()
            .call_tool(
                CallToolRequestParams::new("gauss_horizon_peek_messages").with_arguments(
...
async fn remove_connection_respects_global_connection_scope() {
    ...
    let (server_transport, client_transport) = tokio::io::duplex(16 * 1024);
    let server = GaussHorizonMcpServer::with_runtime_options(Arc::new(backend), McpScope::default(), false);
    let server_task = tokio::spawn(async move { server.serve(server_transport).await });
    let client = ().serve(client_transport).await.expect("initialize MCP client");
    ...
    let result = client
        .peer()
        .call_tool(
            CallToolRequestParams::new("gauss_horizon_remove_connection")
...
async fn enforces_global_connection_scope_and_read_only_policy() {
    ...
    let listed =
        client.peer().call_tool(CallToolRequestParams::new("gauss_horizon_list_connections")).await.expect("list connections");
    ...
    let blocked = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_execute_query").with_arguments(
    ...
    let read_only = client
        .peer()
        .call_tool(
            CallToolRequestParams::new("gauss_horizon_execute_query").with_arguments(
...
async fn duplicate_connection_rejects_ambiguous_source_names() {
    ...
    let result = client
        .peer()
        .call_tool(CallToolRequestParams::new("gauss_horizon_duplicate_connection").with_arguments(
