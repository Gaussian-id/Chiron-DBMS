...
fn legacy_mcp_allow_writes() -> Option<bool> {
    match gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_ALLOW_WRITES").ok()?.trim().to_ascii_lowercase().as_str() {
        "1" | "true" => Some(true),
        "0" | "false" => Some(false),
        _ => None,
    }
...
fn effective_mcp_policy_with_legacy_allow_writes(
    ...
    // When GAUSS_HORIZON_MCP_ALLOW_WRITES is explicitly set to false by the CLI agent
    ...
    if legacy_allow_writes == Some(false) {
        ...
        for rule in &mut policy.connection_policies {
            ...
            rule.execution_mode_policy_version = Some(gauss_horizon_core::mcp_policy::MCP_EXECUTION_POLICY_VERSION);
...
/// Wire-level result for one statement within a `gauss_horizon_execute_batch` call.
///
/// Mirrors the per-statement metadata the Web `/api/query/execute-multi` route
/// returns. `gauss_horizon_core::query::ExecuteMultiResult` only derives `Serialize`, so
...
pub struct BatchStatementResult {
    #[serde(flatten)]
    pub result: gauss_horizon_core::db::QueryResult,
...
impl From<gauss_horizon_core::query::ExecuteMultiResult> for BatchStatementResult {
    fn from(result: gauss_horizon_core::query::ExecuteMultiResult) -> Self {
...
///
/// The Web route serializes `gauss_horizon_core::query::ExecuteMultiResult`, whose fields
...
fn batch_statement_result_from_json(value: &Value) -> Result<BatchStatementResult, String> {
    let result: gauss_horizon_core::db::QueryResult = serde_json::from_value(value.clone())
...
/// `gauss_horizon_core::docs::CollectOptions` minus the fields the backend fills in.
...
#[async_trait]
pub trait GaussHorizonBackend: Send + Sync {
    async fn load_mcp_global_policy(&self) -> Result<McpGlobalPolicy, String>;

    async fn load_connections(&self) -> Result<Vec<ConnectionConfig>, String>;
    /// Return database names visible to the Gauss Horizon connection itself. The MCP
    ...
    async fn send_message(
        &self,
        connection: &ConnectionConfig,
        request: gauss_horizon_core::mq::SendMessageRequest,
    ...
    ) -> Result<gauss_horizon_core::db::QueryResult, String> {
    ...
    async fn execution_plan(
        ...
        database: &str,
        sql: &str,
    ) -> gauss_horizon_core::sql::SqlExecutionPlan {
        let _ = database;
        gauss_horizon_core::sql::sql_execution_plan_for_database(sql, connection.db_type)
    }
    ...
    ) -> Result<Vec<gauss_horizon_core::db::ObjectInfo>, String> {
    ...
    ) -> Result<gauss_horizon_core::db::ObjectSource, String> {
    ...
    async fn bridge_request(&self, path: &str, body: Value) -> Result<(), String> {
        let _ = (path, body);
        Err("Gauss Horizon is not running. Please start Gauss Horizon first.".to_string())
    }
    ...
    ) -> Result<gauss_horizon_core::docs::SchemaSnapshot, String> {
...
impl WebBackend {
    pub fn new(base_url: String, password: String) -> Result<Self, String> {
        ...
        let tls_skip_verify = gauss_horizon_core::legacy::var("GAUSS_HORIZON_WEB_INSECURE_SKIP_VERIFY")
            .ok()
        ...
        Self::new_with_config(
            ...
            proxy,
            no_proxy,
            gauss_horizon_core::legacy::var("GAUSS_HORIZON_WEB_HEADERS").ok(),
            tls_skip_verify,
            gauss_horizon_core::legacy::var("GAUSS_HORIZON_WEB_CA_CERT").ok().filter(|value| !value.trim().is_empty()),
        )
    }
    ...
    fn new_with_config(
        ...
        if base_url.is_empty() {
            return Err("GAUSS_HORIZON_WEB_URL cannot be empty.".to_string());
        }
        ...
        // verification is on by default; opt out with GAUSS_HORIZON_WEB_INSECURE_SKIP_VERIFY
        // for self-signed endpoints, or trust a private CA with GAUSS_HORIZON_WEB_CA_CERT.
        if tls_skip_verify {
            builder = builder.danger_accept_invalid_certs(true);
        }
        if let Some(ca_cert_path) = ca_cert_path.as_deref().map(str::trim).filter(|value| !value.is_empty()) {
            let path = gauss_horizon_core::path_utils::expand_tilde(ca_cert_path);
            let bytes =
                std::fs::read(&path).map_err(|error| format!("Failed to read GAUSS_HORIZON_WEB_CA_CERT at {path}: {error}"))?;
            let certificates = reqwest::Certificate::from_pem_bundle(&bytes)
                ...
                .map_err(|error| format!("Failed to parse GAUSS_HORIZON_WEB_CA_CERT at {path}: {error}"))?;
    ...
    async fn ensure_auth(&self) -> Result<(), String> {
        ...
        if check.setup_required {
            return Err("Gauss Horizon Web password setup is required before MCP Web mode can access APIs.".to_string());
        }
        ...
        if self.password.is_empty() {
            return Err("Gauss Horizon Web authentication is required. Set GAUSS_HORIZON_WEB_PASSWORD for MCP Web mode.".to_string());
        }
        ...
        let cookie = response
            ...
            .ok_or_else(|| "Authentication failed: Gauss Horizon Web did not return a session cookie.".to_string())?;
    ...
    async fn request(
        ...
        loop {
            ...
            let mut request = self
                ...
                .header("x-gauss-horizon-mcp-request", "1");
            for (name, value) in &self.headers {
                request = request.header(name, value);
            }
            if let Some(cookie) = cookie {
                request = request.header(reqwest::header::COOKIE, format!("gauss_horizon_session={cookie}"));
            }
...
impl LocalBackend {
    /// Reuse an already initialized Gauss Horizon application state, for hosts that
    ...
    /// shares the Gauss Horizon process so it is unaffected by this cache desync.
...
fn parse_routine_kind(object_type: &str) -> Result<gauss_horizon_core::db::ObjectSourceKind, String> {
    match object_type.trim().to_ascii_uppercase().as_str() {
        "PROCEDURE" => Ok(gauss_horizon_core::db::ObjectSourceKind::Procedure),
        "FUNCTION" => Ok(gauss_horizon_core::db::ObjectSourceKind::Function),
...
fn local_agent_dir(settings: &DesktopSettings, data_dir: &Path) -> PathBuf {
    ...
    settings
        ...
        .unwrap_or_else(|| {
            if gauss_horizon_core::legacy::var_os("GAUSS_HORIZON_DATA_DIR").filter(|value| !value.is_empty()).is_some() {
...
#[async_trait]
impl GaussHorizonBackend for LocalBackend {
    async fn load_mcp_global_policy(&self) -> Result<McpGlobalPolicy, String> {
    ...
    async fn load_connections(&self) -> Result<Vec<ConnectionConfig>, String> {
        ...
        // Connections created/modified/deleted in the Gauss Horizon desktop UI after this process started
    ...
    async fn list_databases(&self, connection: &ConnectionConfig) -> Result<Vec<String>, String> {
        if connection.db_type == DatabaseType::MongoDb {
            ...
            if matches!(self.state.pool_handle(&connection.id).await, Some(gauss_horizon_core::connection::PoolKind::MongoDb(_)))
            {
                return gauss_horizon_core::mongo_ops::mongo_list_databases_core(&self.state, &connection.id).await;
            }
        ...
        Box::pin(gauss_horizon_core::schema::list_databases_core(&self.state, &connection.id))
            .await
    ...
    async fn send_message(
        ...
        gauss_horizon_core::mq::service::mq_send_message_core(&self.state, &connection.id, request).await
    }
    ...
    async fn peek_messages(
        ...
        gauss_horizon_core::mq::service::mq_peek_messages_core(
            &self.state,
            &connection.id,
            topic,
            "__gauss_horizon_kafka_viewer__".into(),
            count,
            Some(options),
        )
        .await
    ...
    async fn execute_query(
        ...
        gauss_horizon_core::query::execute_sql_statement_with_options(
            &self.state,
            &connection.id,
            database,
            sql,
            None,
            None,
            gauss_horizon_core::query::QueryExecutionOptions { max_rows, timeout_secs, ..Default::default() },
        )
        .await
    }
    ...
    async fn execute_batch(
        ...
        let results = Box::pin(gauss_horizon_core::query::execute_multi_core_with_options_for_client(
            &self.state,
            &connection.id,
            database,
            sql,
    ...
    async fn execution_plan(
        ...
        let is_sqlserver_agent =
            gauss_horizon_core::query::connection_pool_is_sqlserver_agent(self.state.as_ref(), &connection.id, database).await;
        gauss_horizon_core::query::query_execution_plan(sql, Some(connection.db_type), is_sqlserver_agent)
    }
    ...
    async fn list_tables(
        ...
        gauss_horizon_core::schema::list_tables_core(&self.state, &connection.id, database, schema, None, None, None, None, None)
            .await
    }
    ...
    async fn get_columns(
        ...
        gauss_horizon_core::schema::get_columns_core(&self.state, &connection.id, database, schema, table).await
    }
    ...
    async fn list_routines(
        ...
        let objects = gauss_horizon_core::schema::list_objects_core(
            &self.state,
            &connection.id,
            database,
            schema,
    ...
    async fn get_routine_source(
        ...
        gauss_horizon_core::schema::get_object_source_core(
            &self.state,
            &connection.id,
            database,
            schema,
    ...
    async fn collect_docs_snapshot(
        ...
        let collect_options = gauss_horizon_core::docs::CollectOptions {
            database: database.to_string(),
            schemas: options.schemas,
            tables: options.tables,
            project_name: options.project_name.unwrap_or_else(|| connection.name.clone()),
        };
        gauss_horizon_core::docs::collect_snapshot(
            &self.state,
            connection,
            &collect_options,
            &|_progress| {},
    ...
    async fn execute_mongo_command(
        ...
        gauss_horizon_core::mongo_ops::execute_mongo_command_core(&self.state, &connection.id, database, command, 100).await
    }
    ...
    async fn bridge_request(&self, path: &str, body: Value) -> Result<(), String> {
        let port = tokio::fs::read_to_string(self.data_dir.join("mcp-bridge-port"))
            .await
            .map_err(|_| "Gauss Horizon is not running. Please start Gauss Horizon first.".to_string())?;
        ...
        if response.status().is_success() {
            ...
            Err(response.text().await.unwrap_or_else(|_| "Gauss Horizon bridge request failed.".to_string()))
        }
    }
}

#[async_trait]
impl GaussHorizonBackend for WebBackend {
    async fn load_mcp_global_policy(&self) -> Result<McpGlobalPolicy, String> {
    ...
    async fn list_databases(&self, connection: &ConnectionConfig) -> Result<Vec<String>, String> {
        ...
        match connection.db_type {
            ...
            _ => self
                ...
                .await?
                .json::<Vec<gauss_horizon_core::db::DatabaseInfo>>()
                .await
                .map(|databases| databases.into_iter().map(|database| database.name).collect())
    ...
    async fn execute_agent_tool(
        ...
        let result = async {
            if tool_name != "execute_query" {
                return Err(format!("Unsupported Gauss Horizon Web agent tool: {tool_name}"));
            }
            if connection.db_type == DatabaseType::MongoDb {
                return Err(
                    "MongoDB shell commands in Gauss Horizon Web mode are not implemented by the Rust MCP yet.".to_string()
                );
            }
            self.ensure_connected(connection).await?;
            ...
            let risk = gauss_horizon_core::sql_risk::classify_sql_risk_for_database(sql, connection.db_type)
                .map_err(|error| format!("SQL risk classification failed: {error}"))?;
            if risk != gauss_horizon_core::sql_risk::SqlRisk::ReadOnly {
            ...
            let query_result: gauss_horizon_core::db::QueryResult =
    ...
    async fn peek_messages(
        ...
        self.request(
            ...
            Some(json!({ "connectionId": connection.id, "topic": topic, "sub": "__gauss_horizon_kafka_viewer__", "count": count, "options": options })),
    ...
    async fn execute_query(
        ...
        if connection.db_type == DatabaseType::MongoDb {
            return Err("MongoDB shell commands in Gauss Horizon Web mode are not implemented by the Rust CLI yet.".to_string());
        }
        self.ensure_connected(connection).await?;
        self.request(
            reqwest::Method::POST,
    ...
    async fn execute_batch(
        ...
        if connection.db_type == DatabaseType::MongoDb {
            return Err("MongoDB batch execution in Gauss Horizon Web mode is not implemented by the Rust CLI yet.".to_string());
        }
        self.ensure_connected(connection).await?;
        let mut body = json!({
    ...
    async fn list_routines(
        ...
        .await?
        .json::<Vec<gauss_horizon_core::db::ObjectInfo>>()
        .await
    ...
    async fn get_routine_source(
        ...
        let kind_name = match kind {
            gauss_horizon_core::db::ObjectSourceKind::Procedure => "PROCEDURE",
            gauss_horizon_core::db::ObjectSourceKind::Function => "FUNCTION",
    ...
    async fn execute_mongo_command(
        ...
        match command {
            ...
            MongoCommand::ShowDatabases => {
                let result = self
                    .request(
                        ...
                        Some(json!({
                            "connectionId": connection_id,
                            "database": gauss_horizon_core::mongo_ops::MONGO_SHOW_DATABASES_DATABASE,
                            "commandJson": gauss_horizon_core::mongo_ops::MONGO_SHOW_DATABASES_COMMAND_JSON,
                        })),
                    )
                    .await?
                ...
                gauss_horizon_core::mongo_ops::mongo_show_databases_query_result(result.documents, 100)
            }
            MongoCommand::RunCommand { .. } => {
                Err("MongoDB runCommand is not available through the Gauss Horizon MCP backend".to_string())
            }
            ...
            MongoCommand::GetIndexes { collection } => {
                ...
                Ok(gauss_horizon_core::mongo_ops::mongo_indexes_query_result(specs, 100))
            }
...
fn extract_session_cookie(header: &str) -> Option<String> {
    header
        .split(';')
        .find_map(|part| part.trim().strip_prefix("gauss_horizon_session=").map(ToOwned::to_owned))
...
/// Resolves the standard proxy environment variables for a Gauss Horizon Web backend.
///
...
/// Parses the `GAUSS_HORIZON_WEB_HEADERS` JSON object into a `HeaderMap`. Every parsed
/// header is attached to each Gauss Horizon Web request, including auth checks.
fn parse_custom_headers(headers_json: Option<&str>) -> Result<HeaderMap, String> {
    ...
    let entries: serde_json::Map<String, Value> = serde_json::from_str(value)
        .map_err(|error| format!("Invalid GAUSS_HORIZON_WEB_HEADERS: expected a JSON object, got {error}"))?;
    for (name, value) in entries {
        let header_name = HeaderName::from_bytes(name.as_bytes())
            .map_err(|_| format!("Invalid GAUSS_HORIZON_WEB_HEADERS header name: {name}"))?;
        ...
        if RESERVED.iter().any(|reserved| header_name.as_str().eq_ignore_ascii_case(reserved)) {
            return Err(format!("Invalid GAUSS_HORIZON_WEB_HEADERS header name: {name} (reserved by Gauss Horizon)"));
        }
        let Value::String(header_value) = value else {
            return Err(format!("Invalid GAUSS_HORIZON_WEB_HEADERS value for {name}: expected a string"));
        };
        let header_value = HeaderValue::from_str(&header_value)
            .map_err(|error| format!("Invalid GAUSS_HORIZON_WEB_HEADERS value for {name}: {error}"))?;
...
pub(crate) fn format_query_result(result: &gauss_horizon_core::db::QueryResult, max_rows: usize) -> String {
...
fn query_result(columns: Vec<String>, rows: Vec<Vec<Value>>, affected_rows: u64) -> gauss_horizon_core::db::QueryResult {
...
fn scalar_query_result(column: impl Into<String>, value: Value) -> gauss_horizon_core::db::QueryResult {
    query_result(vec![column.into()], vec![vec![value]], 0)
}

fn affected_query_result(affected_rows: u64) -> gauss_horizon_core::db::QueryResult {
...
fn mongo_documents_query_result(documents: Vec<Value>) -> gauss_horizon_core::db::QueryResult {
...
mod tests {
    ...
    fn legacy_read_only_overrides_configured_and_unconfigured_policies() {
        // GAUSS_HORIZON_MCP_ALLOW_WRITES=0 always forces read_only, even when the
    ...
    #[test]
    fn parses_database_type_using_gauss_horizon_protocol_names() {
    ...
    async fn web_peek_messages_forwards_kafka_options_and_preserves_partial_results() {
        use gauss_horizon_core::mq::{PeekMessagesOptions, PeekStartPosition, TopicRef};
        use std::io::BufRead;
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let address = listener.local_addr().unwrap();
        let server = std::thread::spawn(move || {
            ...
            assert_eq!(body["sub"], "__gauss_horizon_kafka_viewer__");
    ...
    fn format_query_result_appends_server_messages() {
        ...
        result.messages = vec![
            gauss_horizon_core::db::QueryMessage {
    ...
    async fn web_backend_applies_custom_headers_to_auth_and_api_requests() {
        ...
        assert!(list.contains("x-gauss-horizon-mcp-request: 1"), "{list}");
    }
    ...
    fn web_backend_rejects_invalid_proxy_and_header_config() {
        ...
        .unwrap_err();
        assert!(error.contains("GAUSS_HORIZON_WEB_HEADERS"), "{error}");
        ...
        // Reserved headers that Gauss Horizon manages internally are rejected.
        let error = WebBackend::new_with_config(
            "http://127.0.0.1:1".to_string(),
            String::new(),
            None,
        ...
        .unwrap_err();
        assert!(error.contains("reserved by Gauss Horizon"), "{error}");
        ...
        let error = WebBackend::new_with_config(
            ...
            None,
            None,
            Some(r#"{"x-gauss-horizon-mcp-request":"1"}"#.to_string()),
            false,
            None,
        )
        .unwrap_err();
    ...
    /// for 127.0.0.1, answering the Gauss Horizon Web auth/check + connection/list
    ...
    async fn local_backend_uses_desktop_plugin_directory() {
        ...
        let database_path = data_dir.path().join("gauss-horizon.db");
        ...
        std::fs::write(
            ...
            r#"{
                "id": "jdbc",
                "name": "Gauss Horizon JDBC Plugin",
                "drivers": [{
                    "id": "jdbc",
                    "label": "JDBC",
                    "kind": "external",
    ...
    fn local_plugin_directory_honors_desktop_storage_settings() {
        let data_dir = Path::new("C:/Users/user/AppData/Roaming/id.gaussian.gauss-horizon");
        let explicit = DesktopSettings {
            plugin_store_dir: Some("D:/Gauss Horizon/plugins-custom".to_string()),
            ..DesktopSettings::default()
        };
        let legacy =
            DesktopSettings { driver_store_dir: Some("D:/Gauss Horizon/drivers".to_string()), ..DesktopSettings::default() };

        assert_eq!(local_plugin_dir(&explicit, data_dir), PathBuf::from("D:/Gauss Horizon/plugins-custom"));
        assert_eq!(local_plugin_dir(&legacy, data_dir), PathBuf::from("D:/Gauss Horizon/drivers/plugins"));
    }
    ...
    fn local_agent_directory_honors_desktop_storage_settings() {
        let data_dir = Path::new("C:/Users/user/AppData/Roaming/id.gaussian.gauss-horizon");
        let explicit =
            DesktopSettings { agent_store_dir: Some("D:/Gauss Horizon/agents-custom".to_string()), ..DesktopSettings::default() };
        let legacy = DesktopSettings { driver_store_dir: Some("D:/Gauss Horizon/drivers".to_string()), ..Default::default() };

        assert_eq!(local_agent_dir(&explicit, data_dir), PathBuf::from("D:/Gauss Horizon/agents-custom"));
        assert_eq!(local_agent_dir(&legacy, data_dir), PathBuf::from("D:/Gauss Horizon/drivers/agents"));
    }

    struct StubBackend;

    #[async_trait]
    impl GaussHorizonBackend for StubBackend {
        async fn load_mcp_global_policy(&self) -> Result<McpGlobalPolicy, String> {
