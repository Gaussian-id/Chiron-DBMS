...
use crate::{
    ...
    GaussHorizonBackend, GaussHorizonMcpServer, McpScope,
};
...
fn build_streamable_http_router(
    ...
    let service: StreamableHttpService<GaussHorizonMcpServer, LocalSessionManager> = StreamableHttpService::new(
        move || Ok(GaussHorizonMcpServer::with_runtime_options(server_backend.clone(), scope.clone(), web_mode)),
        Default::default(),
        rmcp_config,
    );
...
/// session receives a fresh `GaussHorizonMcpServer`, while the database backend remains
/// shared and all authorization happens before rmcp sees a request.
pub async fn serve_streamable_http(backend: Arc<dyn GaussHorizonBackend>, config: HttpRuntimeConfig) -> io::Result<()> {
...
/// as healthy (for example, Gauss Horizon Desktop settings UI).
pub async fn serve_streamable_http_on_listener(
    ...
    eprintln!("Gauss Horizon MCP Streamable HTTP listening on http://{}{}", config.bind_addr, config.path);
