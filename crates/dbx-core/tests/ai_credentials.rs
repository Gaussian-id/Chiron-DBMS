...
async fn plaintext_migration_preserves_ids_models_and_missing_keys_never_reinitialize() {
    ...
    if std::env::var_os("DBX_AI_MASTER_KEY").is_none() {
        #[cfg(unix)]
        {
...
fn chiron_preview_is_bounded_and_vectors_are_removed_recursively() {
    assert!(dbx_core::ai_chiron::validate_vector_sources("SEARCH demo NEAR [1,0,0];", "Find similar items").is_err());
    assert!(dbx_core::ai_chiron::validate_vector_sources("SEARCH demo NEAR [1, 0, 0];", "Use [1,0,0]").is_ok());
    assert!(dbx_core::ai_chiron::validate_vector_sources("SEARCH demo NEAR @first;", "Use point first").is_ok());
    assert!(dbx_core::ai_chiron::validate_vector_sources("SEARCH demo NEAR @unknown;", "Find similar items").is_err());
    let rows: Vec<_> =
        (0..30).map(|i| json!({"id":i,"vector":[0.1,0.2],"payload":{"vectors":[1,2],"field":"hello"}})).collect();
    let preview = dbx_core::ai_chiron::sharing_preview(&json!({"rows":rows}));
    ...
    let huge = dbx_core::ai_chiron::sharing_preview(&json!({"rows":[{"text":"x".repeat(17000)}]}));
    assert_eq!(huge, "[]");
    assert!(dbx_core::ai_chiron::proposal("```chironql\nGET c POINTS 'x';\n```\n```chironql\nDROP COLLECTION c;\n```")
        .is_err());
    assert!(dbx_core::ai_chiron::proposal("Which point ID?").unwrap().is_none());
}
