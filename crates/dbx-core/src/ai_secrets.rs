...
pub const REFERENCE_PREFIX: &str = "dbx-ai-secret:v1:";
...
fn parse_reference(value: &str) -> Result<Option<(String, String)>, String> {
    let Some(encoded) = value.strip_prefix(REFERENCE_PREFIX) else { return Ok(None) };
...
fn master_key(dir: &Path, create: bool) -> Result<String, String> {
    if let Ok(key) = std::env::var("DBX_AI_MASTER_KEY") {
        if key.len() < 32 {
            return Err("DBX_AI_MASTER_KEY must contain at least 32 characters".into());
        }
        return Ok(key);
    }
    ...
    if !path.exists() && create {
        ...
        #[cfg(not(unix))]
        return Err("Set DBX_AI_MASTER_KEY from a protected environment on this platform".into());
    ...
    let metadata = std::fs::symlink_metadata(&path).map_err(|_| {
        "AI master key missing; restore it or set DBX_AI_MASTER_KEY. Existing secrets were not changed."
    })?;
    if !metadata.is_file() {
...
impl Storage {
    ...
    pub(crate) async fn migrate_ai_secrets(&self) -> Result<(), String> {
        ...
        self.with_conn(move |conn| {
            ...
            Ok(())
        }).await.map_err(|e|format!("AI credential migration failed; no configuration rows changed: {e}. Check the original ai-master.key or DBX_AI_MASTER_KEY."))
    }
}
