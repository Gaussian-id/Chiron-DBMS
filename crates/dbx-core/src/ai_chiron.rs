...
use crate::{
    ai::{self, AiCompletionRequest, AiMessage},
    connection::AppState,
...
async fn model(state: &AppState, config_id: &str, model: &str, system: &str, prompt: String) -> Result<String, String> {
    let item = state
        .storage
        .load_ai_configs()
        .await?
...
pub async fn run(state: &AppState, connection: &str, request: Request) -> Result<Reply, String> {
    ...
    match request {
        ...
        Request::Generate {
            ...
            let mut text = model(state, &config_id, &chosen, REFERENCE, context.clone()).await?;
            ...
            if !(200..300).contains(&parsed.status) {
                ...
                text=model(state,&config_id,&chosen,REFERENCE,format!("{context}\nPrevious proposal:\n{query}\nParser code: {code}. Correct the syntax once; do not broaden the requested operation.")).await?;
            ...
            validate_vector_sources(&query, &prompt)?;
