...
pub fn gauss_horizon_mcp_enabled_tools(agent_mode: bool) -> Vec<&'static str> {
    let mut tools = vec![
        "gauss_horizon_list_connections",
        "gauss_horizon_list_tables",
        "gauss_horizon_describe_table",
        "gauss_horizon_get_schema_context",
    ];
    if agent_mode {
        tools.push("gauss_horizon_execute_query");
        tools.push("gauss_horizon_execute_redis_command");
    }
    tools
}

pub fn gauss_horizon_mcp_scope_env(options: &CliAgentRunOptions) -> Vec<(&'static str, String)> {
    let mut env = vec![
        ("GAUSS_HORIZON_MCP_ALLOW_WRITES", if options.allow_writes { "1" } else { "0" }.to_string()),
        ("GAUSS_HORIZON_MCP_ALLOW_DANGEROUS_SQL", if options.allow_dangerous { "1" } else { "0" }.to_string()),
        ("GAUSS_HORIZON_MCP_SCOPE_CONNECTION_ID", options.connection_id.clone()),
        ("GAUSS_HORIZON_MCP_SCOPE_CONNECTION_NAME", options.connection_name.clone()),
        ("GAUSS_HORIZON_MCP_SCOPE_DATABASE", options.database.clone()),
    ];
    if let Some(schema) = options.schema.as_deref().filter(|schema| !schema.trim().is_empty()) {
        env.push(("GAUSS_HORIZON_MCP_SCOPE_SCHEMA", schema.to_string()));
    }
    if let Some(ref sql) = options.confirmed_write_sql {
        env.push(("GAUSS_HORIZON_MCP_CONFIRMED_WRITE_SQL", sql.clone()));
    }
    env
}
...
mod scope_env_tests {
    ...
    fn confirmed_write_sql_is_passed_to_the_scoped_mcp_subprocess() {
        ...
        assert!(
            env.contains(&("GAUSS_HORIZON_MCP_CONFIRMED_WRITE_SQL", "DELETE FROM sessions WHERE id = 7".to_string()))
        );
        assert!(env.contains(&("GAUSS_HORIZON_MCP_ALLOW_WRITES", "1".to_string())));
        assert!(env.contains(&("GAUSS_HORIZON_MCP_ALLOW_DANGEROUS_SQL", "1".to_string())));
        assert!(env.contains(&("GAUSS_HORIZON_MCP_SCOPE_DATABASE", "APPDB".to_string())));
        assert!(env.contains(&("GAUSS_HORIZON_MCP_SCOPE_SCHEMA", "REPORTING".to_string())));
    }
}
...
pub fn build_cli_agent_prompt(
    ...
    let database_access = if allow_write_sql {
        "The user explicitly confirmed the proposed database change. Gauss Horizon MCP tools may execute write and DDL SQL for this run only."
    } else {
        "Use the Gauss Horizon MCP tools when you need live database schema or read-only query results."
    };
    let mut sections = vec![
        format!("You are running inside Gauss Horizon Desktop as the {provider_label} CLI provider."),
        database_access.to_string(),
        "Do not modify files or run shell commands. The Gauss Horizon MCP server is the only intended tool surface."
            .to_string(),
        String::new(),
...
mod grok_streaming_json_tests {
    ...
    fn parses_tool_call_lifecycle() {
        let start = parse_cli_jsonl_event(
            r#"{"type":"tool_call","toolCallId":"call_1","toolName":"gauss_horizon__gauss_horizon_list_tables","status":"in_progress","rawInput":{"schema":"public"}}"#,
            CliAgentJsonlDialect::GrokStreamingJson,
        )
        .unwrap();
        assert!(matches!(
            ...
            AgentEvent::ToolCallStart { tool_call_id, tool_name, args }
                if tool_call_id == "call_1"
                    && tool_name == "gauss_horizon__gauss_horizon_list_tables"
                    && args.get("schema").and_then(Value::as_str) == Some("public")
        ));

        let end = parse_cli_jsonl_event(
            r#"{"type":"tool_call_update","toolCallId":"call_1","toolName":"gauss_horizon__gauss_horizon_list_tables","status":"completed","rawOutput":{"tables":["users"]}}"#,
            CliAgentJsonlDialect::GrokStreamingJson,
        )
        .unwrap();
        assert!(matches!(
...
mod tests {
    ...
    async fn jsonl_agent_applies_env_to_child() {
        let spec = CliAgentProcessSpec {
            command: CliAgentCommandSpec {
                ...
                args: vec![
                    "-c".to_string(),
                    "printf '%s\n' \"{\\\"type\\\":\\\"item.completed\\\",\\\"item\\\":{\\\"type\\\":\\\"agent_message\\\",\\\"text\\\":\\\"$GAUSS_HORIZON_TEST_ENV\\\"}}\" \"{\\\"type\\\":\\\"turn.completed\\\"}\"".to_string(),
                ],
            },
            env: vec![("GAUSS_HORIZON_TEST_ENV".to_string(), "from-env".to_string())],
            env_remove: Vec::new(),
            current_dir: None,
            stdin: None,
    ...
    async fn jsonl_cancellation_kills_and_waits_for_child() {
        let pid_file = std::env::temp_dir().join(format!(
            "gauss-horizon-cli-agent-cancel-{}-{}",
            std::process::id(),
            SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()
        ));
    ...
    async fn jsonl_eof_cancel_kills_and_reaps_hanging_child() {
        ...
        let pid_file = std::env::temp_dir().join(format!(
            "gauss-horizon-cli-agent-eof-cancel-{}-{}",
            std::process::id(),
            SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()
        ));
        let script = format!(
