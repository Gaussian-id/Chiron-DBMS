...
pub const METADATA_POOL_BUSY_ERROR: &str = "Gauss Horizon metadata pool is busy; please retry";
...
pub fn is_pool_saturation_error(err: &str) -> bool {
    ...
    lower.contains("connection pool checkout timed out [stage=wait")
        || lower.contains(&METADATA_POOL_BUSY_ERROR.to_ascii_lowercase())
}
...
fn strip_large_value_marker_prefix(column: &str) -> Option<&str> {
    let prefix = crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX;
...
pub fn is_connection_error(err: &str) -> bool {
    let lower = err.to_lowercase();
    if is_gauss_horizon_query_timeout_error(&lower)
...
pub(crate) fn is_gauss_horizon_query_timeout_error(lower: &str) -> bool {
...
pub fn pool_error_action(db_type: Option<DatabaseType>, err: &str) -> PoolErrorAction {
    ...
    if db::sqlserver::is_driver_panic_error(err)
        || (is_gauss_horizon_query_timeout_error(&lower) && should_discard_pool_after_query_timeout(db_type))
            ...
            match client.execute_typed(database, sql, max_rows, cancel_token, query_timeout).await {
                ...
                Err(error) => {
                    let is_control_error = error.message == QUERY_CANCELED
                        || is_gauss_horizon_query_timeout_error(&error.message.to_ascii_lowercase());
                    if !is_control_error {
...
fn classify_query_error(db_type: Option<DatabaseType>, error: QueryExecutionError) -> QueryExecutionError {
    match error {
        ...
        QueryExecutionError::Legacy(message) if is_gauss_horizon_query_timeout_error(&message.to_ascii_lowercase()) => {
            QueryExecutionError::Timeout(message)
        }
...
async fn invoke_external_driver_query_page(
    ...
    match session.invoke_with_timeout::<db::QueryResult>("executeQueryPage", params.clone(), plugin_timeout).await {
        ...
        Err(error) if is_external_driver_method_unsupported(&error, "executeQueryPage") => {
            // Plugins installed by older Gauss Horizon releases predate cursor pagination. Keep
...
fn external_driver_preview_fallback_sql(sql: &str) -> Option<String> {
    ...
    if !projection.contains(crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX) {
        return None;
    }
                ...
                for (i, sql) in statements.iter().enumerate() {
                    ...
                    match conn.execute_batch(sql) {
                        ...
                        Err(e) => {
                            ...
                            // `is_gauss_horizon_query_timeout_error` recognizes it). The
...
pub struct ManualTransactionExecutionOptions {
    ...
    /// execution SQL, pairing them by count and position so Gauss Horizon-owned
    ...
    async fn sqlite_cancelled_query_can_execute_again_on_same_client_session() {
        ...
        sqlite
            .with_connection(|conn| {
                conn.create_scalar_function(
                    "gauss_horizon_test_query_started",
                    0,
        ...
        let first = tokio::spawn(async move {
            execute_sql_statement_with_options_typed(
                ...
                connection_id,
                "",
                "INSERT INTO t (value) SELECT 'slow' FROM (WITH RECURSIVE cnt(x) AS (SELECT gauss_horizon_test_query_started() UNION ALL SELECT x + 1 FROM cnt WHERE x < 100000000) SELECT x FROM cnt)",
                None,
...
"#
    ...
    async fn query_and_transaction_paths_resolve_catalog_dialect_from_connection() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-catalog-dialect-{}", uuid::Uuid::new_v4()));
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_DYNAMODB_ENDPOINT and an orders table"]
    async fn live_dynamodb_editor_scan_serializes_one_thousand_rows() {
        let endpoint =
            crate::legacy::var("GAUSS_HORIZON_DYNAMODB_ENDPOINT").expect("GAUSS_HORIZON_DYNAMODB_ENDPOINT is required");
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-query-dynamodb-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    async fn sqlserver_agent_echo_state(
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-query-sqlserver-agent-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    async fn agent_error_state(
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-query-agent-error-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let script_path = dir.join("agent.py");
        std::fs::write(
            &script_path,
...
"#
    ...
    async fn native_pre_dispatch_cancellation_stays_typed() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-query-native-cancel-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
        ...
        let backend_error = error.into_backend_error();
        assert_eq!(backend_error.code(), "Gauss Horizon-JDBC-2003");
    ...
    async fn assert_sqlite_batch_error_behavior(failure_first: bool, continue_on_error: bool) {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-query-batch-error-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    async fn transactional_sqlite_batch_rolls_back_when_a_later_statement_fails() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-query-transaction-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    async fn transactional_batch_rejects_an_unsupported_backend_before_execution() {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-query-unsupported-transaction-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    async fn connection_pool_is_sqlserver_agent_detects_agent_and_native_pools() {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-query-sqlserver-agent-flag-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    async fn transaction_kernel_dispatches_by_backend_capability_not_ddl_content() {
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-query-tx-predispatch-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    async fn gaussdb_on_error_stop_overrides_continue_on_error() {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-query-gaussdb-on-error-stop-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    async fn mysql_pipelined_batch_reports_the_first_failed_statement() {
        ...
        assert_eq!(results[1].statement_index, Some(1));
        assert!(results[1].execution_error);
        assert_eq!(results[1].error.as_ref().map(|error| error.code()), Some("Gauss Horizon-JDBC-4001"));
        assert_eq!(
            progress_events
                .lock()
                .unwrap()
    ...
    async fn mysql_batch_stops_on_connection_errors_when_continue_is_enabled() {
        ...
        assert_eq!(results.iter().map(|result| result.statement_index).collect::<Vec<_>>(), vec![Some(0), Some(1)]);
        assert!(results[1].execution_error);
        assert_eq!(results[1].error.as_ref().map(|error| error.code()), Some("Gauss Horizon-LEGACY-0001"));
    ...
    fn execute_multi_result_serializes_client_metadata_only_when_present() {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!("Gauss Horizon-LEGACY-0001"))
        );
    ...
    fn query_execution_error_preserves_structured_catalog_identity() {
        ...
        assert_eq!(error.into_backend_error().code(), "Gauss Horizon-JDBC-1002");
    }
    ...
    fn duckdb_worker_error_preserves_catalog_identity_and_detail() {
        ...
        assert_eq!(backend_error.code(), "Gauss Horizon-JDBC-4001");
    ...
    fn query_timeout_preserves_timeout_catalog_identity_and_detail() {
        ...
        assert_eq!(backend_error.code(), "Gauss Horizon-JDBC-2002");
        assert_eq!(
    ...
    fn postgres_server_error_preserves_sql_catalog_identity_and_detail() {
        let error = classify_query_error(
            ...
            QueryExecutionError::Legacy(
                "ERROR: relation \"gauss_horizon_table_that_does_not_exist\" does not exist".to_string(),
            ),
        )
        .with_omitted_sql_context("SELECT * FROM gauss_horizon_table_that_does_not_exist");
        let backend_error = error.into_backend_error();
        ...
        assert_eq!(
            backend_error.detail(),
            Some(
                "ERROR: relation \"gauss_horizon_table_that_does_not_exist\" does not exist\nSQL text omitted from user-facing error; enable debug SQL diagnostics to inspect the original statement."
            )
        );
    }
    ...
    fn postgres_transaction_statement_error_preserves_sql_catalog_identity() {
        let error = postgres_transaction_statement_error(
            1,
            "ERROR: relation \"gauss_horizon_table_that_does_not_exist\" does not exist",
            "SELECT * FROM gauss_horizon_table_that_does_not_exist",
            true,
        );
        let backend_error = error.into_backend_error();
        ...
        assert_eq!(
            backend_error.detail(),
            Some(
                "Statement 1 failed: ERROR: relation \"gauss_horizon_table_that_does_not_exist\" does not exist\nSQL text omitted from user-facing error; enable debug SQL diagnostics to inspect the original statement."
            )
        );
    }
    ...
    fn postgres_transaction_uses_driver_fact_not_error_wording() {
        ...
        assert_eq!(error.into_backend_error().code(), "Gauss Horizon-JDBC-4001");
    }
    ...
    fn external_driver_preview_fallback_rewrites_generated_postgres_projections() {
        let sql = concat!(
            ...
            "'T:139' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1\" FROM \"job_details\" WHERE left(note, 1) = 'x' LIMIT 100"
        );
        assert_eq!(
            external_driver_preview_fallback_sql(sql).as_deref(),
            Some(
                "SELECT \"id\", \"description\" AS \"description\", \"metadata\"::text AS \"metadata\", 'left(\"literal\", 1)' AS \"note\", 'T:139' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1\" FROM \"job_details\" WHERE left(note, 1) = 'x' LIMIT 100"
            )
        );
    }
    ...
    fn external_driver_preview_fallback_handles_nested_commas_without_touching_literals() {
        let sql = concat!(
            ...
            "'T:140' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_0\" FROM \"job_details\""
        );
        assert_eq!(
            external_driver_preview_fallback_sql(sql).as_deref(),
            Some(
                "SELECT coalesce(\"description\", concat('a,b', \"fallback\")) AS \"description\", 'FROM left(\"literal\", 1)' AS \"note\", 'T:140' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_0\" FROM \"job_details\""
            )
        );
    }
    ...
    fn external_driver_preview_fallback_requires_generated_marker_and_select_shape() {
        ...
        .is_none());
        assert!(external_driver_preview_fallback_sql(
            "SELECT left(value, 10) AS value FROM t WHERE note = '__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_0'"
        )
        .is_none());
    }
    ...
    fn postgres_preview_fallback_retries_only_generated_projections() {
        ...
        let sql = concat!(
            ...
            "'T:226' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1\" FROM \"t_large\" LIMIT 100"
        );
        ...
        assert_eq!(
            ...
            Some(
                "SELECT \"id\", \"content\" AS \"content\", 'T:226' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1\" FROM \"t_large\" LIMIT 100"
            )
        );
    ...
    async fn external_driver_preview_retry_preserves_marker_truncation() {
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-jdbc-preview-retry-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        ...
        std::fs::write(
            &executable,
            format!(
                "#!/bin/sh\nwhile IFS= read -r line; do\n  id=$(printf '%s' \"$line\" | sed -E 's/.*\"id\":([0-9]+).*/\\1/')\n  case \"$line\" in\n    *'\"method\":\"executeQueryPage\"'*)\n      echo executeQueryPage >> '{}'\n      case \"$line\" in\n        *'left('*) printf '{{\"id\":%s,\"error\":{{\"message\":\"ERROR: invalid byte sequence for encoding \\\\\"UTF8\\\\\": 0xe2\"}}}}\\n' \"$id\" ;;\n        *) printf '{{\"id\":%s,\"result\":{{\"columns\":[\"id\",\"description\",\"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1\"],\"column_types\":[\"integer\",\"text\",\"text\"],\"rows\":[[1,\"abcdef\",\"T:3\"]],\"affected_rows\":0,\"execution_time_ms\":1,\"truncated\":false}}}}\\n' \"$id\" ;;\n      esac\n      ;;\n    *'\"method\":\"executeQuery\"'*)\n      echo executeQuery >> '{}'\n      case \"$line\" in\n        *'left('*) printf '{{\"id\":%s,\"error\":{{\"message\":\"ERROR: invalid byte sequence for encoding \\\\\"UTF8\\\\\": 0xe2\"}}}}\\n' \"$id\" ;;\n        *) printf '{{\"id\":%s,\"result\":{{\"columns\":[\"id\",\"description\",\"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1\"],\"column_types\":[\"integer\",\"text\",\"text\"],\"rows\":[[1,\"abcdef\",\"T:3\"]],\"affected_rows\":0,\"execution_time_ms\":1,\"truncated\":false}}}}\\n' \"$id\" ;;\n      esac\n      ;;\n  esac\ndone\n",
                calls.display(),
                calls.display()
            ),
        )
        ...
        let marker = format!("{}T_1", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX);
        let sql = format!(
    ...
    async fn external_driver_manual_transaction_executes_commits_and_releases_session_pool() {
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-jdbc-manual-txn-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let executable = dir.join("plugin.sh");
        let calls = dir.join("calls.log");
        std::fs::write(
        ...
        config.database = Some("gauss_horizon_test".to_string());
        config.connection_string = Some("jdbc:test".to_string());
        state.configs.write().await.insert(config.id.clone(), config.clone());
        ...
        session
            .invoke::<serde_json::Value>(
                "beginManualTransaction",
                serde_json::json!({ "connection": &config, "database": "gauss_horizon_test" }),
            )
            .await
            .unwrap();
        ...
        let cleanup_guard = state
            .workload_session_pool_cleanup_guard("jdbc-conn", Some("gauss_horizon_test"), client_session_id)
            .await
            .unwrap();
        state.transaction_sessions.write().await.insert(
            ...
            TransactionSession {
                connection: Arc::new(tokio::sync::Mutex::new(TxnConnection::ExternalDriver {
                    ...
                    database: Some("gauss_horizon_test".to_string()),
                    cleanup_guard,
                })),
                ...
                database: "gauss_horizon_test".to_string(),
                schema: None,
            },
        );

        let results =
            execute_in_manual_transaction(&state, "txn-test", "SELECT 42", "gauss_horizon_test", None, Some(10))
                .await
                .unwrap();
    ...
    async fn external_driver_query_page_falls_back_to_legacy_execute_query() {
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-legacy-jdbc-plugin-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let executable = dir.join("plugin.sh");
        let calls = dir.join("calls.log");
        std::fs::write(
    ...
    async fn external_driver_query_page_does_not_retry_jdbc_errors() {
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-jdbc-query-error-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let executable = dir.join("plugin.sh");
        let calls = dir.join("calls.log");
        std::fs::write(
...
"#
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-manual-txn-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new(storage);
    ...
    fn structured_agent_disposition_controls_pool_recovery() {
        let quarantined = "Agent RPC error (-1): lost\nGAUSS_HORIZON_AGENT_ERROR_DATA:{\"category\":\"connection\",\"sessionDisposition\":\"quarantine\"}";
        let replace_runtime = "Agent RPC error (-1): saturated\nGAUSS_HORIZON_AGENT_ERROR_DATA:{\"category\":\"resource\",\"sessionDisposition\":\"replace_runtime\"}";
    ...
    fn extracts_server_large_value_markers_and_restores_source_columns() {
        let mut result = db::QueryResult {
            columns: vec![
                "id".to_string(),
                "payload".to_string(),
                format!("{}T_1", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
                "note".to_string(),
            ],
    ...
    fn extracts_lowercased_server_large_value_markers_from_case_folding_servers() {
        ...
        let mut result = db::QueryResult {
            columns: vec![
                "id".to_string(),
                "payload".to_string(),
                "__gauss_horizon_large_value_bytes_t_1".to_string(),
                "note".to_string(),
                "__gauss_horizon_large_value_bytes_t_2".to_string(),
            ],
            column_types: vec![
                "integer".to_string(),
                "text".to_string(),
    ...
    fn truncates_server_binary_preview_after_the_configured_byte_count() {
        let mut result = db::QueryResult {
            columns: vec![
                "raw_value".to_string(),
                format!("{}B_0", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
            ],
    ...
    fn extracts_deferred_oracle_clob_markers_without_changing_placeholder() {
        let mut result = db::QueryResult {
            columns: vec![
                "id".to_string(),
                "payload".to_string(),
                format!("{}C_1", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
            ],
    ...
    fn restores_pgvector_previews_as_arrays_and_marks_only_truncated_values() {
        let marker = format!("{}V_0", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX);
        let mut result = db::QueryResult {
    ...
    fn restores_server_preview_types_without_rows() {
        let mut result = db::QueryResult {
            columns: vec![
                "embedding".to_string(),
                format!("{}V_0", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
                "metadata".to_string(),
                format!("{}K_1", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
            ],
    ...
    fn ordinary_queries_preserve_columns_that_resemble_preview_markers() {
        let marker = format!("{}0", crate::sql_dialect::GAUSS_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX);
