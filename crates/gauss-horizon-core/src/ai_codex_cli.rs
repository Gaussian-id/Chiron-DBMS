...
use crate::ai_cli_agent::{
    append_config_overrides, build_cli_agent_prompt, cli_command, gauss_horizon_mcp_enabled_tools,
    gauss_horizon_mcp_scope_env, model_infos, parse_cli_jsonl_event, run_cli_jsonl_agent, toml_string,
...
#[cfg(not(windows))]
const CODEX_PATH_MARKER: &str = "__GAUSS_HORIZON_CODEX_PATH__";
...
pub fn codex_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.codex_cli_env {
        ...
        if is_reserved_gauss_horizon_mcp_env_name(key) {
            return Err(format!(
                "[codexEnvReserved] `{key}` is managed by Gauss Horizon for the scoped MCP server and cannot be set here."
            ));
        }
...
fn is_reserved_gauss_horizon_mcp_env_name(name: &str) -> bool {
    name.to_ascii_uppercase().starts_with("GAUSS_HORIZON_MCP_")
}

pub fn codex_enabled_tools(agent_mode: bool) -> Vec<&'static str> {
    gauss_horizon_mcp_enabled_tools(agent_mode)
}
...
fn codex_mcp_config_overrides(options: &CodexRunOptions) -> Vec<String> {
    let mcp_command = options
        ...
        .unwrap_or("gauss-horizon-mcp-server");
    let mut overrides = vec![
        format!("mcp_servers.\"gauss-horizon\".command={}", toml_string(mcp_command)),
        "mcp_servers.\"gauss-horizon\".required=true".to_string(),
        "mcp_servers.\"gauss-horizon\".startup_timeout_sec=20".to_string(),
        "mcp_servers.\"gauss-horizon\".tool_timeout_sec=120".to_string(),
        "mcp_servers.\"gauss-horizon\".default_tools_approval_mode=\"approve\"".to_string(),
        format!(
            "mcp_servers.\"gauss-horizon\".enabled_tools={}",
            toml_string_array(&gauss_horizon_mcp_enabled_tools(options.agent_mode))
        ),
    ];
    if let Some(command) = options.mcp_server_command.as_ref().filter(|command| !command.args.is_empty()) {
        let args = command.args.iter().map(String::as_str).collect::<Vec<_>>();
        overrides.push(format!("mcp_servers.\"gauss-horizon\".args={}", toml_string_array(&args)));
    }
    overrides.extend(
        gauss_horizon_mcp_scope_env(options)
            .into_iter()
            .map(|(name, value)| format!("mcp_servers.\"gauss-horizon\".env.{name}={}", toml_string(&value))),
    );
    overrides
}
...
fn materialize_codex_images(
    ...
    let directory = tempfile::Builder::new().prefix("gauss-horizon-ai-images-").tempdir().map_err(|error| {
...
fn classify_codex_spawn_error(message: &str) -> String {
    if message.contains("No such file") || message.contains("not found") {
        "[codexNotInstalled] Codex CLI was not found. Install Codex CLI or set the Codex CLI path in Gauss Horizon AI settings."
            .to_string()
    } else if is_command_line_too_long_error(message) {
        "[codexCommandLineTooLong] Codex CLI command line is too long. Update Gauss Horizon so Codex prompts are sent through stdin instead of process arguments."
            .to_string()
    } else {
...
fn classify_codex_run_error(stderr: &str) -> String {
    ...
    } else if lower.contains("gauss-horizon-mcp-server") || lower.contains("enoent") {
        format!("[gaussHorizonMcpMissing] Gauss Horizon MCP server was not found. Install @gauss-horizon/mcp-server and try again. {stderr}")
    } else if lower.contains("mcp") && (lower.contains("gauss-horizon") || lower.contains("server")) {
        format!("[codexMcpStartupFailed] Codex could not start the Gauss Horizon MCP server. {stderr}")
    } else {
...
mod tests {
    ...
    fn builds_codex_command_with_scoped_mcp_and_omits_default_model() {
        ...
        assert!(spec.args.contains(&"mcp_servers.\"gauss-horizon\".command=\"gauss-horizon-mcp-server\"".to_string()));
        assert!(spec
            .args
            .contains(&"mcp_servers.\"gauss-horizon\".default_tools_approval_mode=\"approve\"".to_string()));
        assert!(spec
            .args
            .contains(&"mcp_servers.\"gauss-horizon\".env.GAUSS_HORIZON_MCP_ALLOW_WRITES=\"0\"".to_string()));
        assert!(spec
            .args
            .contains(&"mcp_servers.\"gauss-horizon\".env.GAUSS_HORIZON_MCP_ALLOW_DANGEROUS_SQL=\"0\"".to_string()));
        assert!(spec.args.contains(
            &"mcp_servers.\"gauss-horizon\".env.GAUSS_HORIZON_MCP_SCOPE_CONNECTION_ID=\"conn-1\"".to_string()
        ));
        assert!(spec.args.iter().any(|arg| arg.contains("gauss_horizon_execute_query")));
    }
    ...
    fn builds_codex_command_with_non_default_model_and_ask_tools() {
        ...
        assert!(!codex_enabled_tools(false).contains(&"gauss_horizon_execute_query"));
        assert!(spec.args.iter().any(|arg| arg.contains("gauss_horizon_get_schema_context")));
        assert!(!spec.args.iter().any(|arg| arg.contains("gauss_horizon_execute_query")));
    }
    ...
    fn builds_codex_command_with_resolved_mcp_server_path() {
        ...
        options.mcp_server_command = Some(CliAgentCommandSpec {
            program: "/opt/gauss-horizon/bin/gauss-horizon-mcp-server".to_string(),
            args: vec!["--stdio".to_string()],
        });
        let spec = build_codex_exec_command(&codex_config("default"), "hello", &options);

        assert!(spec.args.contains(
            &"mcp_servers.\"gauss-horizon\".command=\"/opt/gauss-horizon/bin/gauss-horizon-mcp-server\"".to_string()
        ));
        assert!(spec.args.contains(&"mcp_servers.\"gauss-horizon\".args=[\"--stdio\"]".to_string()));
    }
    ...
    fn windows_shell_lookup_prefers_launchable_command() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-codex-test-{}", std::process::id()));
    ...
    fn windows_direct_extensionless_path_uses_cmd_sibling() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-codex-direct-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let extensionless = dir.join("codex");
        let cmd = dir.join("codex.cmd");
        std::fs::write(&extensionless, "#!/bin/sh\n").unwrap();
    ...
    fn windows_direct_extensionless_path_is_not_launchable_without_sibling() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-codex-direct-missing-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let extensionless = dir.join("codex");
        std::fs::write(&extensionless, "#!/bin/sh\n").unwrap();
    ...
    fn windows_direct_directory_path_uses_codex_cmd_inside() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-codex-direct-dir-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let extensionless = dir.join("codex");
        let cmd = dir.join("codex.cmd");
        std::fs::write(&extensionless, "#!/bin/sh\n").unwrap();
    ...
    async fn windows_configured_nvm_directory_resolves_codex_cmd_shim() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-codex-nvm-dir-test-{}", std::process::id()));
    ...
    fn windows_npm_codex_cmd_shim_uses_node_directly() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-codex-npm-shim-test-{}", std::process::id()));
        let bin_dir = dir.join("node_modules").join("@openai").join("codex").join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let cmd = dir.join("codex.cmd");
        let node = dir.join("node.exe");
    ...
    fn native_codex_binary_command_remains_direct() {
        let command = codex_command_for_program("/Applications/Gauss Horizon.app/Contents/MacOS/codex".to_string());

        assert_eq!(command.program, "/Applications/Gauss Horizon.app/Contents/MacOS/codex");
    ...
    #[test]
    fn rejects_reserved_gauss_horizon_mcp_env_name() {
        let mut config = codex_config("default");
        config.codex_cli_env.insert("GAUSS_HORIZON_MCP_ALLOW_DANGEROUS_SQL".to_string(), "1".to_string());
    ...
    fn rejects_directory_without_codex_in_codex_cli_path() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-codex-empty-dir-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let mut config = codex_config("default");
        config.codex_cli_path = Some(dir.to_string_lossy().to_string());
    ...
    fn parses_codex_jsonl_events() {
        ...
        let started = parse_codex_jsonl_event(
            r#"{"type":"item.started","item":{"id":"tool-1","type":"mcp_tool_call","tool_name":"gauss_horizon_list_tables","arguments":{"schema":"public"}}}"#,
        )
        .unwrap();
        assert!(
            matches!(&started[0], AgentEvent::ToolCallStart { tool_name, .. } if tool_name == "gauss_horizon_list_tables")
        );
        ...
        let tool_done = parse_codex_jsonl_event(
            r#"{"type":"item.completed","item":{"id":"tool-1","type":"mcp_tool_call","tool_name":"gauss_horizon_list_tables","result":{"content":"users"}}}"#,
        )
        .unwrap();
        assert!(
            matches!(&tool_done[0], AgentEvent::ToolCallEnd { tool_name, is_error, .. } if tool_name == "gauss_horizon_list_tables" && !is_error)
        );

        let current_started = parse_codex_jsonl_event(
            r#"{"type":"item.started","item":{"id":"item_0","type":"mcp_tool_call","server":"gauss-horizon","tool":"gauss_horizon_execute_query","arguments":{"sql":"SELECT 1"},"result":null,"error":null,"status":"in_progress"}}"#,
        )
        .unwrap();
        assert!(
            matches!(&current_started[0], AgentEvent::ToolCallStart { tool_name, args, .. } if tool_name == "gauss_horizon_execute_query" && args["sql"] == "SELECT 1")
        );

        let current_tool_done = parse_codex_jsonl_event(
            r#"{"type":"item.completed","item":{"id":"item_0","type":"mcp_tool_call","server":"gauss-horizon","tool":"gauss_horizon_execute_query","arguments":{"sql":"SELECT 1"},"result":{"content":[{"type":"text","text":"ok"}],"structured_content":null},"error":null,"status":"completed"}}"#,
        )
        .unwrap();
        assert!(
            matches!(&current_tool_done[0], AgentEvent::ToolCallEnd { tool_name, result, is_error, .. } if tool_name == "gauss_horizon_execute_query" && result["content"][0]["text"] == "ok" && !is_error)
        );

        let current_tool_error = parse_codex_jsonl_event(
            r#"{"type":"item.completed","item":{"id":"item_0","type":"mcp_tool_call","server":"gauss-horizon","tool":"gauss_horizon_execute_query","arguments":{"sql":"SELECT 1"},"result":null,"error":{"message":"user cancelled MCP tool call"},"status":"failed"}}"#,
        )
        .unwrap();
        assert!(
            matches!(&current_tool_error[0], AgentEvent::ToolCallEnd { tool_name, result, is_error, .. } if tool_name == "gauss_horizon_execute_query" && result["message"] == "user cancelled MCP tool call" && *is_error)
        );
