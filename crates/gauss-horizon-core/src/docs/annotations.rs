...
/// every platform Gauss Horizon targets.
...
/// `data_dir` is a parameter because `gauss-horizon-core` cannot reach the caller's
...
mod tests {
  ...
  "tables": {
    ...
    fn temp_notes(contents: &str) -> std::path::PathBuf {
        let path = std::env::temp_dir().join(format!("gauss-horizon-notes-test-{}.json", uuid::Uuid::new_v4()));
        std::fs::write(&path, contents).expect("write temp notes file");
        path
    }

    fn temp_case_dir(label: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-notes-{label}-{}", uuid::Uuid::new_v4().simple()));
    ...
    fn an_absent_file_is_not_an_error() {
        let missing = std::path::Path::new("/nonexistent/gauss-horizon-notes-does-not-exist.json");
