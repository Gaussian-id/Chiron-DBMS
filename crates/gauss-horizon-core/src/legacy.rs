//! Explicit compatibility boundary for pre-Gaussian profiles and environment names.
...
pub const APP_ID: &str = "id.gaussian.gauss-horizon";
...
pub fn var_os(name: impl AsRef<OsStr>) -> Option<OsString> {
    ...
    std::env::var_os(name).or_else(|| {
        name.to_str()?.strip_prefix("GAUSS_HORIZON_").and_then(|suffix| std::env::var_os(format!("DBX_{suffix}")))
    })
}
...
fn migrate_database(stage: &Path, source: &Path, destination: &Path) -> Result<(), String> {
    ...
    for table in ["app_state", "state_store"] {
        if table_exists(&tx, table)? {
            ...
            tx.execute(
                &format!(
                    "UPDATE {table} SET key='gauss-horizon'||substr(key,4) WHERE key LIKE 'dbx-%' OR key LIKE 'dbx:%'"
                ),
                [],
            )
            .map_err(error)?;
    ...
    drop(connection);
    std::fs::rename(old, stage.join("gauss-horizon.db")).map_err(error)?;
...
pub fn migrate_profile(source: &Path, destination: &Path, old_running: bool) -> Result<bool, String> {
    ...
    let stage = parent.join(format!(".gauss-horizon-migration-{}", uuid::Uuid::new_v4()));
    ...
    std::fs::write(stage.join("gaussian-profile-migration.json"), b"{\"version\":1,\"source\":\"com.dbx.app\"}")
        .map_err(error)?;
    if destination.exists() {
...
pub fn migrate_default_profile() -> Result<(), String> {
    if var_os("GAUSS_HORIZON_DATA_DIR").is_some() {
        return Ok(());
    }
    #[cfg(windows)]
    if std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|d| d.join("portable.gauss-horizon").exists() || d.join("portable.dbx").exists()))
        .unwrap_or(false)
    {
        return Ok(());
    }
    ...
    let lock_path = base.join(".gauss-horizon-migration.lock");
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        for folder in ["Library/WebKit"] {
            ...
            if source.exists() && !target.exists() {
                let staging = root.join(format!(".gauss-horizon-webview-{}", uuid::Uuid::new_v4()));
                let before = profile_stamp(&source)?;
...
mod tests {
    ...
    fn copy_preserves_source_history_and_namespaces() {
        ...
        let c = Connection::open(new.join("gauss-horizon.db")).unwrap();
        assert_eq!(
            c.query_row("SELECT value_json FROM app_state WHERE key='gauss-horizon-theme'", [], |r| r
                .get::<_, String>(0))
                .unwrap(),
            "dark"
        );
    ...
    fn missing_source_and_existing_destination_are_not_imported() {
        ...
        assert!(!existing.join("gauss-horizon.db").exists());
    }
    #[test]
    fn interrupted_staging_is_preserved_and_retry_succeeds() {
        let (root, old, new) = fixture();
        let interrupted = root.path().join(".gauss-horizon-migration-interrupted");
    ...
    fn explicit_profile_database_prefers_current_then_legacy() {
        ...
        std::fs::write(old.join("gauss-horizon.db"), b"current").unwrap();
        assert_eq!(storage_db_path(&old), old.join("gauss-horizon.db"));
    }
    #[cfg(unix)]
    #[test]
    ...
    fn new_environment_wins_and_legacy_is_fallback() {
        ...
        let new = format!("GAUSS_HORIZON_{suffix}");
    ...
    fn empty_old_profile_copies_without_inventing_database_data() {
        ...
        assert!(migrate_profile(&old, &new, false).unwrap());
        assert!(new.is_dir());
        assert!(!new.join("gauss-horizon.db").exists());
        assert!(old.is_dir());
    }
    #[test]
...
pub fn storage_db_path(directory: &Path) -> PathBuf {
    let current = directory.join("gauss-horizon.db");
...
pub fn install_environment_compatibility() {
    for (key, value) in std::env::vars_os() {
        if let Some(suffix) = key.to_str().and_then(|k| k.strip_prefix("DBX_")) {
            let current = format!("GAUSS_HORIZON_{suffix}");
