...
/// Returns true when an Agent attempted a write or DDL call before Gauss Horizon has a
...
fn execute_query_tool(sql_permissions: AgentSqlPermissions) -> ToolDefinition {
    let description = if sql_permissions.allow_dangerous {
        ...
        "Execute a read-only SQL query and return results (max 50 rows). This run cannot execute writes or DDL because no specific SQL has been confirmed yet; this does not mean the database itself is read-only. When the user requests a write, first propose the exact SQL in one ```sql code block and ask for confirmation. After confirmation, Gauss Horizon starts a new run that can execute only that exact SQL. Only SELECT, WITH, SHOW, DESCRIBE, EXPLAIN statements may be executed in this run."
    };
    ToolDefinition {
        name: "execute_query",
        description,
        parameters: json!({
            ...
            "properties": {
                ...
                "client_session_id": {
                    "type": "string",
                    "description": "Opaque Gauss Horizon session handle that pins this query to the same backend connection as earlier queries in the session (preserves USE/SET/session state). Managed by Gauss Horizon; agents should not invent values."
                }
            },
            "required": ["sql"]
        }),
...
async fn execute_execute_query(
    ...
    if targets_production {
        return Err("Blocked: AI agents cannot execute writes or DDL on a production database. Return the SQL for the user to review and execute manually in Gauss Horizon.".to_string());
    }
    if !risk_allowed {
...
async fn execute_mongo_query(
    ...
    if command.is_mutating() {
        return Err(
            "Blocked: MongoDB Agent queries are read-only. Return the command for the user to review and execute manually in Gauss Horizon."
                .to_string(),
        );
    }
...
"#
    ...
    fn query_result_formatter_marks_the_default_character_window() {
        let value = format!("{}GAUSS_HORIZON_ISSUE_5620_SENTINEL", "A".repeat(200));
        ...
        assert!(!output.contains("GAUSS_HORIZON_ISSUE_5620_SENTINEL"));
    }

    #[test]
    fn query_result_formatter_supports_expanded_and_sliding_character_windows() {
        let value = format!("{}GAUSS_HORIZON_ISSUE_5620_SENTINEL{}", "A".repeat(200), "Z".repeat(20));
        let result = query_result(vec!["message"], vec![vec![serde_json::json!(value)]], 0);

        let expanded =
            format_query_result_as_text(&result, 50, QueryCellWindow::from_options(None, Some(400))).unwrap();
        assert!(expanded.contains("GAUSS_HORIZON_ISSUE_5620_SENTINEL"));
        ...
        let expected_window = value.chars().skip(200).take(23).collect::<String>();
        assert!(sliding.contains(&format!("...{expected_window}... [chars 200..223; next cell_char_offset=223]")));
    }
