...
pub fn preview_mongodb_import_bytes(
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-mongo-import-preview-{}", uuid::Uuid::new_v4()));
...
fn temp_export_path(target: &Path) -> PathBuf {
    ...
    target.with_file_name(format!(".{name}.gauss-horizon-export.tmp"))
}
...
mod tests {
    ...
    fn execute_source(
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-mongo-import-exec-{}", uuid::Uuid::new_v4()));
    ...
    fn csv_export_reimport_with_default_options_restores_objectid_and_nested_types() {
        let document = serde_json::json!({
            ...
            "_gauss_horizon_issue_5792_all_types": true,
        ...
        assert_eq!(
            fields,
            vec!["_id", "_gauss_horizon_issue_5792_all_types", "scenario", "text", "nested.ok", "count"]
        );
        ...
        assert_eq!(imported[0]["_gauss_horizon_issue_5792_all_types"], true);
