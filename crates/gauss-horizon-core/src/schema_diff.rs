    ...
    while index < bytes.len() {
        ...
        let (end, kind, replacement) = match bytes[index] {
            ...
            b'`' => {
                ...
                (end, MysqlViewTokenKind::Atom, is_schema_qualifier.then_some("`__gauss_horizon_schema__`"))
            }
            b'#' => {
...
fn drop_index_sql(table_name: &str, index_name: &str, db_type: DatabaseType, schema: Option<&str>) -> String {
    ...
    if db_type == DatabaseType::SqlServer {
        ...
        let batch = format!(
            "DECLARE @gauss_horizon_constraint_name sysname, @gauss_horizon_drop_sql NVARCHAR(MAX); \
             SELECT @gauss_horizon_constraint_name = kc.name \
             ...
             IF @gauss_horizon_constraint_name IS NOT NULL BEGIN \
               SET @gauss_horizon_drop_sql = N'ALTER TABLE {table_literal} DROP CONSTRAINT ' + QUOTENAME(@gauss_horizon_constraint_name); \
               EXEC sys.sp_executesql @gauss_horizon_drop_sql; \
...
mod tests {
    ...
    async fn live_sqlserver_alter_column_preserves_unchanged_runtime_dependencies() {
        let database =
            crate::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_DATABASE").unwrap_or_else(|_| "tempdb".to_string());
        ...
        let lookup_table = format!("gauss_horizon_dep_lookup_{suffix}");
        let parent_table = format!("gauss_horizon_dep_parent_{suffix}");
        let child_table = format!("gauss_horizon_dep_child_{suffix}");
        let primary_key = format!("PK_gauss_horizon_dep_parent_{suffix}");
        let unique_constraint = format!("UQ_gauss_horizon_dep_parent_code_{suffix}");
        let lookup_unique_constraint = format!("UQ_gauss_horizon_dep_lookup_code_{suffix}");
        let check_constraint = format!("CK_gauss_horizon_dep_parent_values_{suffix}");
        let outbound_foreign_key = format!("FK_gauss_horizon_dep_parent_lookup_{suffix}");
        let inbound_foreign_key = format!("FK_gauss_horizon_dep_child_parent_{suffix}");
        let ordinary_index = format!("IX_gauss_horizon_dep_parent_code_{suffix}");
        let cleanup = format!(
    ...
    fn detects_mysql_functional_index_changes_and_preserves_expression_ddl() {
        ...
        let sql = generate_schema_sync_sql(
            ...
            &[],
            DatabaseType::Mysql,
            Some("gauss_horizon_issue_4114"),
            false,
            None,
            &[],
        );

        assert!(sql.contains("DROP INDEX `test_UNIQUE` ON `gauss_horizon_issue_4114`.`test`;"));
        assert!(sql.contains(&format!(
            "CREATE UNIQUE INDEX `test_UNIQUE` ON `gauss_horizon_issue_4114`.`test` (`attr`, `attr2`, {functional_key_part});"
        )));
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a writable PostgreSQL-family database"]
    async fn real_postgres_round_trip_quotes_columns_and_leaves_expressions_bare() {
        ...
        let schema = format!("gauss_horizon_key_expr_{}", uuid::Uuid::new_v4().simple());
