...
/// AV scanning) and suggests restarting gauss-horizon; on POSIX returns a concise
...
const GAUSS_HORIZON_RELEASE_PREFIX: &str = "https://github.com/Gaussian-id/Gauss-Horizon/releases/";
...
fn agent_download_candidate_urls(source: DownloadSource, github_url: &str) -> Result<Vec<String>, String> {
    match source {
        DownloadSource::Official => {
            ...
            // compiled product only accepts the immutable Gauss Horizon
            // release location.
            let is_loopback_test_url = cfg!(test) && github_url.starts_with("http://127.0.0.1");
            if !github_url.starts_with(GAUSS_HORIZON_RELEASE_PREFIX) && !is_loopback_test_url {
                return Err(format!("Unsupported Gauss Horizon agent download URL: {github_url}"));
            }
            Ok(vec![github_url.to_string()])
        }
        DownloadSource::Cnb => Err("Gauss Horizon agent mirror is not available.".to_string()),
    }
}
...
pub fn local_agent_jar_candidates(db_type: &str) -> Vec<PathBuf> {
    let jar_name = format!("gauss-horizon-agent-{db_type}.jar");
...
fn local_agents_dir_candidates() -> Vec<PathBuf> {
    ...
    candidates.push(PathBuf::from("..").join("gauss-horizon-agents"));
    candidates.push(PathBuf::from("gauss-horizon-agents"));
    candidates
}
...
async fn fetch_registry_from_urls(
    ...
    let resp = open_download_response(&client, urls, "gauss-horizon-agent-manager", cancellations)
        .await
...
pub fn ensure_driver_app_version(
    ...
    Err(format!(
        "{db_type} driver {} requires Gauss Horizon {} or newer. Current Gauss Horizon version is {}.",
...
fn extract_jre_key_from_filename(name: &str) -> Option<String> {
    ...
    let rest = filename.strip_prefix("gauss-horizon-jre-").or_else(|| filename.strip_prefix("jre-"))?;
...
fn extract_db_type_from_filename(name: &str) -> Option<String> {
    let filename = name.rsplit('/').next()?;
    let rest = filename.strip_prefix("gauss-horizon-agent-")?;
...
mod agent_download_url_tests {
    ...
    #[test]
    fn official_agent_downloads_only_use_gauss_horizon_releases() {
        ...
        let asset = "https://github.com/Gaussian-id/Gauss-Horizon/releases/download/v0.1.0/gauss-horizon-jre-21-linux-x64.tar.zst";
    ...
    fn agent_downloads_reject_mirrors_and_non_gaussian_urls() {
        assert_eq!(
            ...
            "Gauss Horizon agent mirror is not available."
        );
        ...
        .unwrap_err()
        .contains("Unsupported Gauss Horizon agent download URL"));
    }

    #[test]
    fn offline_jre_filename_parser_accepts_release_and_legacy_names() {
        assert_eq!(
            extract_jre_key_from_filename("jre/gauss-horizon-jre-21-macos-aarch64.tar.gz").as_deref(),
            Some("21")
        );
    ...
    fn windows_native_header_validator_checks_cpu_architecture() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-agent-pe-test-{}", uuid::Uuid::new_v4()));
...
mod agent_registry_install_tests {
    ...
    fn test_manager(name: &str) -> AgentManager {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-agent-registry-install-{name}-{}", uuid::Uuid::new_v4()));
        ...
        drivers.insert(
            ...
            DriverInfo {
                ...
                jar: Some(ArtifactInfo {
                    url: format!("https://example.com/gauss-horizon-agent-{db_type}-legacy-placeholder.jar"),
                    sha256: None,
                    size: 0,
                    format: None,
                }),
    ...
    fn build_tar_zstd_driver_package(
        ...
        let (filename, artifact) = match kind {
            DriverArtifactKind::Jar => (
                format!("gauss-horizon-agent-{db_type}-{version}.jar"),
                serde_json::json!({
                    "jar": {
                        "url": format!("gauss-horizon-agent-{db_type}-{version}.jar"),
            ...
            DriverArtifactKind::Native => {
                ...
                let filename = format!("gauss-horizon-agent-{db_type}-{version}-{platform}{extension}");
                (
                    filename.clone(),
                    serde_json::json!({
                        "native": {
    ...
    async fn ensure_agent_runtime_installs_missing_driver_and_jre() {
        ...
        let driver_url = "https://example.com/gauss-horizon-agent-mongodb.jar";
        let driver_bytes = test_agent_jar();
        let jre_version = "21.0.12";
        let jre_url = "https://example.com/gauss-horizon-jre.tar.gz";
    ...
    async fn ensure_agent_runtime_ready_fast_path_preserves_valid_old_driver() {
        ...
        let latest_url = "https://example.com/gauss-horizon-agent-mongodb-latest.jar";
    ...
    async fn ensure_agent_runtime_repairs_damaged_jre_without_upgrading_valid_driver() {
        ...
        let registry = mongodb_registry_with_jre(
            "9.9.9",
            "https://example.com/gauss-horizon-agent-mongodb-latest.jar",
    ...
    async fn registry_install_replaces_hive_legacy_jar_with_native_driver() {
        ...
        let db_type = "hive";
        let version = "0.1.31";
        let native_url = "https://example.com/gauss-horizon-agent-hive";
    ...
    async fn registry_install_sqlite_worker_downloads_both_linux_platforms() {
        ...
        let version = "0.1.0";
        let x64_url = "https://example.com/gauss-horizon-agent-sqlite-worker-linux-x64";
        let arm_url = "https://example.com/gauss-horizon-agent-sqlite-worker-linux-aarch64";
        ...
        drivers.insert(
            ...
            DriverInfo {
                ...
                jre: DEFAULT_JRE_KEY.to_string(),
                jar: Some(ArtifactInfo {
                    url: "https://example.com/gauss-horizon-agent-sqlite-worker-legacy-placeholder.jar".to_string(),
                    sha256: None,
                    size: 0,
                    format: None,
                }),
    ...
    async fn registry_install_cancel_after_cached_download_preserves_existing_driver() {
        ...
        let url = "https://example.com/gauss-horizon-agent-mongodb.jar";
    ...
    async fn registry_install_extracts_tar_zstd_native_driver_package() {
        ...
        let db_type = "duckdb";
        let version = "0.1.0";
        let package_url = "https://example.com/gauss-horizon-agent-duckdb.tar.zst";
    ...
    async fn registry_install_extracts_tar_zstd_java_driver_package() {
        ...
        let db_type = "dameng";
        let version = "0.2.0";
        let package_url = "https://example.com/gauss-horizon-agent-dameng.tar.zst";
    ...
    async fn batch_upgrade_preserves_successful_state_and_reports_independent_failure() {
        ...
        let oracle_url = "https://example.com/gauss-horizon-agent-oracle";
        let dameng_url = "https://example.com/gauss-horizon-agent-dameng";
        let kingbase_url = "https://example.com/gauss-horizon-agent-kingbase.jar";
    ...
    async fn batch_cancel_all_aborts_in_flight_downloads_without_persisting() {
        ...
        let native_url = format!("http://{addr}/gauss-horizon-agent-oracle");
        let registry = registry_with_native_and_legacy_jar(db_type, version, &native_url, native_bytes.len() as u64);
    ...
    async fn shared_jre_is_not_downloaded_again_after_the_first_install_persists_its_version() {
        ...
        let jre_key = DEFAULT_JRE_KEY;
        let version = "21.0.12";
        let url = "https://example.com/gauss-horizon-jre.tar.gz";
    ...
    async fn managed_jre_install_extracts_tar_zstd_archive() {
        ...
        let jre_key = DEFAULT_JRE_KEY;
        let version = "21.0.12";
        let url = "https://example.com/gauss-horizon-jre.tar.zst";
    ...
    async fn batch_registry_install_waits_for_an_existing_driver_operation() {
        ...
        let db_type = "oracle";
        let version = "0.1.31";
        let native_url = "https://example.com/gauss-horizon-agent-oracle";
        let native_bytes = b"native-agent";
        let registry = registry_with_native_and_legacy_jar(db_type, version, native_url, native_bytes.len() as u64);
        write_cached_driver_download(
            &manager,
    ...
    async fn manual_import_waits_for_an_existing_driver_operation() {
        ...
        let db_type = "h2";
        let source = manager.base_dir().join("gauss-horizon-agent-h2.jar");
    ...
    async fn download_with_progress_returns_cancelled_when_token_prefired() {
        ...
        let error = download_with_progress(
            ...
            "driver",
            DownloadSource::Official,
            "https://example.com/gauss-horizon-agent-oracle",
            &dest,
            100,
            None,
            None,
    ...
    async fn registry_install_rejects_corrupt_downloaded_jar() {
        ...
        let db_type = "h2";
        let version = "0.2.0";
        let jar_url = "https://example.com/gauss-horizon-agent-h2.jar";
        let jar_bytes = b"jar";
    ...
    async fn offline_zip_rejects_a_cross_platform_java_dependency_before_install_changes() {
        ...
        let jar_name = "gauss-horizon-agent-h2.jar";
        let jre_name = format!("gauss-horizon-jre-{jre_key}-{foreign_platform}.tar.gz");
        let jar_bytes = test_agent_jar();
    ...
    fn offline_zip_rejects_a_java_dependency_missing_from_a_registry_with_jre_metadata() {
        ...
        let jar_name = "gauss-horizon-agent-h2.jar";
        let jar_bytes = test_agent_jar();
        let unrelated_jre_name = format!("gauss-horizon-jre-temurin-17-{platform}.tar.gz");
    ...
    fn offline_zip_rejects_registry_size_and_sha256_tampering() {
        let platform = AgentManager::current_platform();
        let entry_name = format!("gauss-horizon-agent-h2-{platform}");
    ...
    fn offline_zip_resolves_a_hyphenated_jre_key_from_the_registry_basename() {
        ...
        let jre_key = "temurin-21";
        let jar_name = "gauss-horizon-agent-h2.jar";
        let jre_name = format!("gauss-horizon-jre-{jre_key}-21.0.7-{platform}.tar.gz");
        let jar_bytes = test_agent_jar();
...
mod jre_dir_remove_tests {
    ...
    fn unique_tmp(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("gauss-horizon-jre-remove-{name}-{}", uuid::Uuid::new_v4()))
    }
    ...
    fn windows_error_message_lists_root_causes_and_path() {
        let path = PathBuf::from("/tmp/gauss-horizon-jre-test");
        ...
        if cfg!(windows) {
            ...
            assert!(rendered.contains("restart gauss-horizon and try again"), "missing restart advice: {rendered}");
        } else {
