...
pub(super) fn build_tdengine_data_grid_save_statements(options: &DataGridSaveStatementOptions) -> Vec<String> {
    ...
    let has_tbname = save_columns.iter().any(|column| {
        column.as_deref().is_some_and(|column| column.eq_ignore_ascii_case(GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN))
    });
    let requires_tbname = options
        .table_meta
        .primary_keys
        .iter()
        .any(|primary_key| primary_key.eq_ignore_ascii_case(GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN));
...
fn tdengine_row_primary_keys(options: &DataGridSaveStatementOptions) -> Vec<String> {
    options
        ...
        .primary_keys
        .iter()
        .filter(|primary_key| !primary_key.eq_ignore_ascii_case(GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN))
        .cloned()
        .collect()
}
...
fn tdengine_can_insert_column(column: &str, table_name: &str, tbname: Option<&str>, tag_columns: &[String]) -> bool {
    ...
    if normalized == GAUSS_HORIZON_TDENGINE_TBNAME_COLUMN {
        return false;
    }
