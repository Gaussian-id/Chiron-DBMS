...
impl CursorIsolatedRuntime {
    fn create(options: Option<&CursorRunOptions>) -> Result<Self, String> {
        let root = env::temp_dir().join(format!("gauss-horizon-cursor-{}", uuid::Uuid::new_v4()));
    ...
    fn write_mcp_config(cursor_dir: &Path, options: &CursorRunOptions) -> Result<(), String> {
        let command = options
            ...
            .as_ref()
            .cloned()
            .unwrap_or_else(|| CursorCommandSpec { program: "gauss-horizon-mcp-server".to_string(), args: Vec::new() });
        let env = gauss_horizon_mcp_scope_env(options).into_iter().collect::<BTreeMap<_, _>>();
        let config = json!({
            "mcpServers": {
                "gauss-horizon": {
    ...
    fn write_permission_config(cursor_dir: &Path, agent_mode: bool) -> Result<(), String> {
        let allow = gauss_horizon_mcp_enabled_tools(agent_mode)
            .into_iter()
            .map(|tool| format!("Mcp(gauss-horizon:{tool})"))
            .collect::<Vec<_>>();
        let config = json!({
            "permissions": {
                "allow": allow,
...
fn is_reserved_env_name(name: &str) -> bool {
    ...
    upper.starts_with("GAUSS_HORIZON_MCP_") || CURSOR_CONTROL_ENV.iter().any(|reserved| upper == *reserved)
}
...
pub fn cursor_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.cursor_cli_env {
        ...
        if is_reserved_env_name(key) {
            return Err(format!(
                "[cursorEnvReserved] `{key}` is managed by Gauss Horizon for the isolated Cursor session and cannot be set here."
            ));
        }
...
fn classify_cursor_run_error(message: &str) -> String {
    if message.starts_with("[cursor") || message.starts_with("[gaussHorizonMcpMissing]") {
...
mod tests {
    ...
    fn default_command_uses_stream_json_and_stdin_mode() {
        let workspace = Path::new("/tmp/gauss-horizon-cursor-workspace");
        ...
        assert_eq!(
            ...
            [
                ...
                "--trust",
                "--workspace",
                "/tmp/gauss-horizon-cursor-workspace"
            ]
        );
    }
    ...
    fn cursor_env_rejects_runtime_and_mcp_overrides() {
        ...
        invalid.cursor_cli_env.insert("GAUSS_HORIZON_MCP_ALLOW_WRITES".to_string(), "1".to_string());
        assert!(cursor_cli_env(&invalid).unwrap_err().starts_with("[cursorEnvReserved]"));
    }
    ...
    fn cursor_stream_parser_maps_usage_and_tool_events() {
        let start = parse_cursor_jsonl_event(
            r#"{"type":"tool_call","subtype":"started","call_id":"call-1","tool_call":{"mcpToolCall":{"args":{"toolName":"gauss_horizon_list_tables","args":{"database":"main"}}}}}"#,
        )
        .unwrap();
        assert!(
            matches!(&start[0], AgentEvent::ToolCallStart { tool_call_id, tool_name, .. } if tool_call_id == "call-1" && tool_name == "gauss_horizon_list_tables")
        );
    ...
    fn isolated_runtime_writes_scoped_mcp_and_permissions() {
        ...
        assert_eq!(mcp.pointer("/mcpServers/gauss-horizon/command").and_then(Value::as_str), Some("node"));
        assert_eq!(
            mcp.pointer("/mcpServers/gauss-horizon/env/GAUSS_HORIZON_MCP_SCOPE_CONNECTION_ID").and_then(Value::as_str),
            Some("sqlite-1")
        );
        ...
        assert!(allow.iter().any(|value| value == "Mcp(gauss-horizon:gauss_horizon_list_connections)"));
        assert!(!allow.iter().any(|value| value == "Mcp(gauss-horizon:gauss_horizon_execute_query)"));
    }

    fn live_config() -> AiConfig {
        let mut config = config("default");
        config.cursor_cli_path = crate::legacy::var("GAUSS_HORIZON_LIVE_CURSOR_PATH").ok();
        config
    }
    ...
    #[tokio::test]
    #[ignore = "requires Cursor, a running Gauss Horizon MCP bridge, and GAUSS_HORIZON_LIVE_MCP_CONNECTION_NAME/DATABASE"]
    async fn live_scoped_gauss_horizon_mcp_agent_run() {
        ...
        assert!(
            ...
            "Cursor events did not expose the expected Gauss Horizon tool name: {events:#?}"
        );
    }
}
