...
impl client::Handler for SshClient {
    ...
    async fn check_server_key(
        ...
        // known-hosts stores (system + gauss-horizon-managed). A trusted key is accepted
        // immediately; an unknown key triggers explicit TOFU; a changed key in
        // the gauss-horizon store prompts for replace-or-session-only; a changed key in
        // ~/.ssh/known_hosts is still a hard reject (gauss-horizon never writes that file).
        match self.host_key_verifier.check(&self.host, self.port, server_public_key) {
            ...
            Err(e) => {
                // Changed host key in ~/.ssh/known_hosts => possible MITM. gauss-horizon
...
pub struct TunnelManager {
    ...
    /// gauss-horizon-managed known_hosts path (`<data_dir>/known_hosts`) threaded into
...
-----END OPENSSH PRIVATE KEY-----"#;
    ...
    async fn unknown_host_without_persist_permission_does_not_leak_password() {
        ...
        // host key can NEVER be persisted. gauss-horizon must fail-closed: abort the
    ...
    async fn reconnect_auth_failure_is_reported_instead_of_a_dead_local_port() {
        ...
        // session. gauss-horizon closes the accepted socket without sending anything,
