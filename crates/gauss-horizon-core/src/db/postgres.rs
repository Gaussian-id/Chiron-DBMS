...
fn pg_type_requires_text_protocol(pg_type: &Type, col_type: PgColType) -> bool {
    ...
    // which makes the Gauss Horizon host timezone win over the server session timezone.
...
///
/// Session-scoped Gauss Horizon pools use a single connection so temporary tables and
...
pub async fn connect_with_max_connections(
    ...
    // timezone used for timestamptz text output; deriving one from Gauss Horizon's host
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a PostgreSQL-compatible database"]
    async fn postgres_connection_identity_supports_compatible_servers() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_URL").expect("GAUSS_HORIZON_TEST_POSTGRES_URL");
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a PostgreSQL database"]
    async fn postgres_connection_identity_preserves_notice_capture() {
        ...
        let result = execute_query_with_max_rows_inner(
            &client,
            "DO $$ BEGIN RAISE NOTICE 'gauss-horizon notice identity regression'; END $$",
            None,
            false,
            None,
            false,
        ...
        assert!(result.messages.iter().any(|message| message.message == "gauss-horizon notice identity regression"));
    }
    ...
    fn postgres_search_path_replaces_matching_user_placeholder_with_selected_schema() {
        let baseline = PostgresSearchPathBaseline {
            configured: "\"$user\", public".to_string(),
            first_resolved_schema: Some("gauss_horizon_test".to_string()),
            has_explicit_pg_catalog: false,
        };

        assert_eq!(
            postgres_set_preserved_search_path_sql("gauss_horizon_test", PostgresSearchPathContext::Query, &baseline),
            "SET search_path TO \"gauss_horizon_test\", public, pg_catalog"
        );

        let explicit_baseline = PostgresSearchPathBaseline {
            configured: "gauss_horizon_test, \"$user\", public".to_string(),
            first_resolved_schema: Some("gauss_horizon_test".to_string()),
            has_explicit_pg_catalog: false,
        };
        assert_eq!(
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_GAUSSDB_URL, GAUSS_HORIZON_TEST_GAUSSDB_SCHEMA, and GAUSS_HORIZON_TEST_GAUSSDB_TABLE"]
    async fn gaussdb_selected_schema_matching_user_is_applied() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_GAUSSDB_URL").expect("GAUSS_HORIZON_TEST_GAUSSDB_URL");
        let schema =
            crate::legacy::var("GAUSS_HORIZON_TEST_GAUSSDB_SCHEMA").expect("GAUSS_HORIZON_TEST_GAUSSDB_SCHEMA");
        let table = crate::legacy::var("GAUSS_HORIZON_TEST_GAUSSDB_TABLE").expect("GAUSS_HORIZON_TEST_GAUSSDB_TABLE");
    ...
    async fn start_docker_postgres_image_with_postgres_args(
        ...
        let container = DockerPostgres { name: format!("gauss-horizon-postgres-enum-{}", uuid::Uuid::new_v4()), port };
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a writable PostgreSQL 18 database"]
    async fn postgres_custom_composite_result_uses_server_text_output() {
        ...
        let schema = format!("gauss_horizon_custom_text_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn postgres_custom_type_arrays_and_exports_use_server_text_output() {
        ...
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        assert_postgres_18(&pool).await;
        let schema = format!("gauss_horizon_custom_array_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
        let payload_type = format!("{schema_ident}.payload");
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_GAUSSDB_URL pointing at a writable GaussDB database"]
    async fn gaussdb_list_custom_types_excludes_relation_row_types_and_arrays() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_GAUSSDB_URL").expect("GAUSS_HORIZON_TEST_GAUSSDB_URL");
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect gaussdb");
        let schema = format!("gauss_horizon_types_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a writable PostgreSQL database"]
    ...
    async fn postgres_custom_type_details_reads_members_and_ddl() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_URL").expect("GAUSS_HORIZON_TEST_POSTGRES_URL");
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let schema = format!("gauss_horizon_details_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
        let status_type = format!("{schema_ident}.status");
        let email_domain = format!("{schema_ident}.email");
        let address_type = format!("{schema_ident}.address");
    ...
    async fn gaussdb_custom_type_details_reads_members_and_ddl() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_GAUSSDB_URL").expect("GAUSS_HORIZON_TEST_GAUSSDB_URL");
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect gaussdb");
        let schema = format!("gauss_horizon_gdetails_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
        let status_type = format!("{schema_ident}.status");
        let address_type = format!("{schema_ident}.address");
        let price_range_type = format!("{schema_ident}.price_range");
    ...
    async fn postgres_custom_type_fallback_refreshes_stale_cached_metadata() {
        ...
        let schema = format!("gauss_horizon_custom_stale_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
        let payload_type = format!("{schema_ident}.payload");
    ...
    async fn postgres_text_fallback_stops_before_late_row_error_at_limit() {
        ...
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        assert_postgres_18(&pool).await;
        let schema = format!("gauss_horizon_custom_limit_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
        let payload_type = format!("{schema_ident}.payload");
    ...
    fn postgres_connection_url_strips_ssl_file_params_before_driver_parse() {
        ...
        let cert = dir.join(format!("gauss-horizon-postgres-cert-{}.pem", std::process::id()));
        let key = dir.join(format!("gauss-horizon-postgres-key-{}.pem", std::process::id()));
        let root = dir.join(format!("gauss-horizon-postgres-root-{}.pem", std::process::id()));
        std::fs::write(&cert, "not a real cert").unwrap();
        std::fs::write(&key, "not a real key").unwrap();
        std::fs::write(&root, "not a real root").unwrap();

        let url = format!(
            "postgres://localhost/db?sslmode=verify-full&sslcert={}&sslkey={}&sslrootcert={}&application_name=gauss-horizon",
            cert.display(),
            key.display(),
            root.display()
        );
        ...
        assert_eq!(parsed.url, "postgres://localhost/db?sslmode=require&application_name=gauss-horizon");
    ...
    fn postgres_connection_url_extracts_legacy_tls_before_driver_parse() {
        let parsed = postgres_connection_url(
            "postgres://localhost/db?sslmode=require&legacy_tls=true&application_name=gauss-horizon",
        )
        .unwrap();
    ...
    fn postgres_tls_rejects_empty_client_certificate() {
        ...
        let cert = dir.join(format!("gauss-horizon-postgres-empty-client-{suffix}.crt"));
        let key = dir.join(format!("gauss-horizon-postgres-empty-client-{suffix}.key"));
    ...
    fn postgres_tls_rejects_malformed_private_key() {
        ...
        let dir = std::env::temp_dir();
        let suffix = uuid::Uuid::new_v4().simple();
        let cert = dir.join(format!("gauss-horizon-postgres-client-{suffix}.crt"));
        let key = dir.join(format!("gauss-horizon-postgres-malformed-client-{suffix}.key"));
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_MTLS_URL pointing at PostgreSQL 14 with required client certificates"]
    async fn postgres_mtls_accepts_configured_client_identity() {
        let url =
            crate::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_MTLS_URL").expect("GAUSS_HORIZON_TEST_POSTGRES_MTLS_URL");
    ...
    #[test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_MTLS_URL with a valid client certificate"]
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_MTLS_REJECT_URL that the TLS server must reject"]
    async fn postgres_mtls_rejects_invalid_tls_identity() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_MTLS_REJECT_URL")
            .expect("GAUSS_HORIZON_TEST_POSTGRES_MTLS_REJECT_URL");
        let error =
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL pointing at a disposable PostgreSQL"]
    async fn live_postgres_progress_read_survives_a_total_duration_beyond_the_timeout() {
        let Ok(url) = crate::legacy::var("GAUSS_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL") else {
            return;
        };
    ...
    fn postgres_prefer_fallback_url_preserves_other_options() {
        assert_eq!(
            postgres_ssl_fallback_url(
                "postgres://localhost/db?application_name=gauss-horizon&sslmode=prefer&connect_timeout=5"
            ),
            Some(
                "postgres://localhost/db?application_name=gauss-horizon&sslmode=disable&connect_timeout=5".to_string()
            )
        );
        assert_eq!(
            postgres_ssl_fallback_url("postgres://localhost/db?application_name=gauss-horizon"),
            Some("postgres://localhost/db?sslmode=disable&application_name=gauss-horizon".to_string())
        );
    }
    ...
    async fn postgres_column_metadata_query_returns_enum_values_against_real_postgres() {
        ...
        let pool = connect(&container.url(), Duration::from_secs(5)).await.expect("connect postgres");
        let schema = format!("gauss_horizon_enum_meta_{}", std::process::id());
    ...
    async fn postgres_unqualified_metadata_follows_search_path() {
        ...
        let schema = format!("gauss_horizon_search_path_meta_{suffix}");
    ...
    async fn postgres_owned_serial_table_ddl_round_trips_without_external_sequences() {
        ...
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let suffix = uuid::Uuid::new_v4().simple();
        let schema = format!("gauss-horizon 6405 \"{suffix}");
        let schema_ident = pg_quote_ident(&schema);
    ...
    async fn postgres_column_metadata_dependency_joins_return_each_column_once() {
        ...
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let suffix = uuid::Uuid::new_v4().simple();
        let schema = format!("gauss-horizon 6547 \"{suffix}");
        let schema_ident = pg_quote_ident(&schema);
    ...
    async fn postgres_column_metadata_compat_query_returns_each_column_once() {
        ...
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let suffix = uuid::Uuid::new_v4().simple();
        let schema = format!("gauss-horizon 6547 compat \"{suffix}");
        let schema_ident = pg_quote_ident(&schema);
        let table_name = "issue6547_repro";
        let table_ident = pg_quote_ident(table_name);
        let table = format!("{schema_ident}.{table_ident}");
    ...
    async fn postgres_batched_column_metadata_returns_each_column_once() {
        ...
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let suffix = uuid::Uuid::new_v4().simple();
        let schema = format!("gauss-horizon 6547 batch \"{suffix}");
        let schema_ident = pg_quote_ident(&schema);
        let table_name = "issue6547_repro";
        let table_ident = pg_quote_ident(table_name);
        let table = format!("{schema_ident}.{table_ident}");
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a writable PostgreSQL 9.x database"]
    async fn postgres_legacy_table_ddl_uses_compatible_partition_probe() {
        ...
        let schema = format!("gauss_horizon_legacy_ddl_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
    ...
    async fn postgres_legacy_batched_ddl_metadata_preserves_relation_pairs_across_compat_tiers() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_URL").expect("GAUSS_HORIZON_TEST_POSTGRES_URL");
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let schema = format!("gauss_horizon_batch_fkeys_{}", uuid::Uuid::new_v4().simple());
        let schema_ident = pg_quote_ident(&schema);
        let client = pool.get().await.expect("get postgres client");
        client
            .batch_execute(&format!(
    ...
    async fn postgres_ddl_emits_not_valid_check_constraints_as_separate_replayable_alter() {
        ...
        let schema = format!("gauss_horizon_not_valid_check_{pid}");
        let schema_ident = pg_quote_ident(&schema);
    ...
    async fn postgres_partition_metadata_renders_replayable_children_and_subpartitions() {
        ...
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let pid = std::process::id();
        let schema = format!("gauss_horizon_partition_meta_{pid}");
        let schema_ident = pg_quote_ident(&schema);
        let replay_schema = format!("{schema}_replay");
        let replay_schema_ident = pg_quote_ident(&replay_schema);
        let parent = format!("{schema_ident}.parent");
        ...
        let trigger_fn = format!("public.gauss_horizon_partition_meta_trigger_fn_{pid}");
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a writable PostgreSQL database with postgres_fdw available"]
    async fn postgres_partition_ddl_uses_foreign_table_syntax_for_foreign_partitions() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_URL").expect("GAUSS_HORIZON_TEST_POSTGRES_URL");
        let pool = connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
        let pid = std::process::id();
        let schema = format!("gauss_horizon_partition_fdw_{pid}");
        let schema_ident = pg_quote_ident(&schema);
        let server = format!("gauss_horizon_partition_fdw_server_{pid}");
    ...
    async fn postgres_schema_context_prioritizes_selected_schema_and_cleans_up() {
        ...
        let schema = format!("gauss_horizon_issue_830_\"{suffix}");
        let schema_ident = pg_quote_ident(&schema);
        let helper = format!("gauss_horizon_issue_830_public_{suffix}");
    ...
    fn transaction_recovery_statement_detection_ignores_regular_queries() {
        ...
        assert!(!is_transaction_recovery_statement("UPDATE users SET name = 'gauss-horizon'"));
    }
    ...
    async fn automatic_invalid_timezone_keeps_connected_server_default() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_URL").expect("GAUSS_HORIZON_TEST_POSTGRES_URL");
        let pool = connect_with_local_timezone(&url, Duration::from_secs(5), "Invalid/GAUSS_HORIZON_Timezone")
            .await
            .expect("automatic local timezone rejection must not reject a valid connection");
        let client = pool.get().await.expect("checkout postgres");
        let timezone: String = client.query_one("SHOW timezone", &[]).await.unwrap().get(0);
        assert_ne!(timezone, "Invalid/GAUSS_HORIZON_Timezone");
    }
    ...
    async fn missing_statement_fallback_uses_current_metadata_for_empty_results() {
        ...
        let pool = connect_with_local_timezone(&url, Duration::from_secs(5), "UTC")
            ...
            .expect("connect PostgreSQL database");
        let client = pool.get().await.expect("checkout postgres");
        let select_sql = "SELECT * FROM gauss_horizon_statement_metadata_test WHERE false";

        client
            .execute_typed("CREATE TEMP TABLE gauss_horizon_statement_metadata_test (id int4)", &[])
            .await
        ...
        client
            .execute_typed("ALTER TABLE gauss_horizon_statement_metadata_test ADD COLUMN label text", &[])
            .await
    ...
    async fn explicit_timezone_remains_strict_and_overrides_local_timezone() {
        ...
        let invalid_url = format!("{url}{separator}options=-c%20TimeZone%3DInvalid%2FGAUSS_HORIZON_Timezone");
        ...
        assert!(error.contains("Invalid/GAUSS_HORIZON_Timezone") || error.contains("time zone"), "{error}");
    }
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_URL pointing at a writable PostgreSQL-compatible database"]
    async fn list_tables_filtered_supports_risingwave_pagination_and_filtering() {
        ...
        client
            .batch_execute(
                r#"DROP TABLE IF EXISTS public."gauss_horizon_issue_5584_live_page_a";
                   DROP TABLE IF EXISTS public."gauss_horizon_issue_5584_live_page_b";
                   DROP TABLE IF EXISTS public."gauss_horizon_issue_5584_live_order_100%";
                   DROP TABLE IF EXISTS public."gauss_horizon_issue_5584_live_back\slash";
                   DROP TABLE IF EXISTS public."gauss_horizon_issue_5584_live_system_users";
                   CREATE TABLE public."gauss_horizon_issue_5584_live_page_a"(id int);
                   CREATE TABLE public."gauss_horizon_issue_5584_live_page_b"(id int);
                   CREATE TABLE public."gauss_horizon_issue_5584_live_order_100%"(id int);
                   CREATE TABLE public."gauss_horizon_issue_5584_live_back\slash"(id int);
                   CREATE TABLE public."gauss_horizon_issue_5584_live_system_users"(id int);"#,
            )
            .await
        ...
        let first_page =
            list_tables_filtered(&pool, "public", Some("gauss_horizon_issue_5584_live_page_"), Some(1), Some(0))
                .await
                .expect("list first table page");
        let second_page =
            list_tables_filtered(&pool, "public", Some("gauss_horizon_issue_5584_live_page_"), Some(1), Some(1))
                .await
                .expect("list second table page");
        let wildcard_match =
            list_tables_filtered(&pool, "public", Some("gauss_horizon_issue_5584_live_order_100%"), Some(10), Some(0))
                .await
                .expect("filter table with wildcard characters");
        let backslash_match =
            list_tables_filtered(&pool, "public", Some(r"gauss_horizon_issue_5584_live_back\slash"), Some(10), Some(0))
                .await
        ...
        assert!(all_tables.iter().any(|table| table.name == "gauss_horizon_issue_5584_live_page_a"));
        assert!(all_tables.iter().any(|table| table.name == "gauss_horizon_issue_5584_live_page_b"));
        assert_eq!(
            first_page.iter().map(|table| table.name.as_str()).collect::<Vec<_>>(),
            ["gauss_horizon_issue_5584_live_page_a"]
        );
        assert_eq!(
            second_page.iter().map(|table| table.name.as_str()).collect::<Vec<_>>(),
            ["gauss_horizon_issue_5584_live_page_b"]
        );
        assert_eq!(
            wildcard_match.iter().map(|table| table.name.as_str()).collect::<Vec<_>>(),
            ["gauss_horizon_issue_5584_live_order_100%"]
        );
        assert_eq!(
            backslash_match.iter().map(|table| table.name.as_str()).collect::<Vec<_>>(),
            [r"gauss_horizon_issue_5584_live_back\slash"]
        );
        assert_eq!(
            fuzzy_match.iter().map(|table| table.name.as_str()).collect::<Vec<_>>(),
            ["gauss_horizon_issue_5584_live_system_users"]
        );
    ...
    async fn completion_assistant_searches_visible_and_qualified_sequences() {
        ...
        client
            .batch_execute(
                r#"DROP SCHEMA IF EXISTS gauss_horizon_issue_4005_visible CASCADE;
                   DROP SCHEMA IF EXISTS gauss_horizon_issue_4005_shadowed CASCADE;
                   CREATE SCHEMA gauss_horizon_issue_4005_visible;
                   CREATE SCHEMA gauss_horizon_issue_4005_shadowed;
                   CREATE SEQUENCE gauss_horizon_issue_4005_visible.order_seq;
                   CREATE SEQUENCE gauss_horizon_issue_4005_shadowed.order_seq;
                   CREATE SEQUENCE gauss_horizon_issue_4005_visible."OrderSequence";
                   SET search_path = gauss_horizon_issue_4005_visible, gauss_horizon_issue_4005_shadowed, public;"#,
            )
            .await
        ...
        assert_eq!(
            visible
                ...
                .collect::<Vec<_>>(),
            [("order_seq", Some("gauss_horizon_issue_4005_visible"))]
        );
        ...
        let qualified = completion_assistant_search(
            ...
            &CompletionAssistantRequest {
                connection_id: "live".into(),
                database: "postgres".into(),
                schema: Some("gauss_horizon_issue_4005_visible".into()),
                object_kinds: vec![CompletionAssistantObjectKind::Sequence],
                mask: "OrderS".into(),
                case_sensitive: true,
                global_search: false,
