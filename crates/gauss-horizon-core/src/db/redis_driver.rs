...
pub fn classify_command(command: &str) -> RedisCommandSafety {
    ...
    // version, module, or proxy remain high-risk until Gauss Horizon reviews them.
...
mod tests {
    ...
    ///
    /// Set `GAUSS_HORIZON_TEST_REDIS_URL` to a Redis that predates hash-field expiry
    ...
    #[tokio::test]
    #[ignore = "requires a Redis without hash-field expiry via GAUSS_HORIZON_TEST_REDIS_URL"]
    async fn hash_field_update_degrades_on_redis_without_hash_field_ttl() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_REDIS_URL").expect("GAUSS_HORIZON_TEST_REDIS_URL");
        let mut con = super::connect(&url, std::time::Duration::from_secs(5)).await.unwrap();

        let key = b"gauss-horizon-7584-regression";
