...
const SQLSERVER_RESULT_TYPE_PROBE_SQL: &str = "\
    DECLARE @gauss_horizon_use_describe_dmv bit = CASE \
        WHEN @P2 = 0 AND OBJECT_ID(N'sys.dm_exec_describe_first_result_set') IS NOT NULL THEN 1 ELSE 0 END; \
    SELECT @gauss_horizon_use_describe_dmv AS gauss_horizon_use_describe_dmv; \
    IF @gauss_horizon_use_describe_dmv = 1 \
    BEGIN \
    ...
    ELSE \
    BEGIN \
        DECLARE @gauss_horizon_probe_table sysname = N'##gauss_horizon_result_type_probe_' + CONVERT(nvarchar(12), @@SPID); \
        DECLARE @gauss_horizon_probe_object nvarchar(258) = N'tempdb..' + QUOTENAME(@gauss_horizon_probe_table); \
        DECLARE @gauss_horizon_probe_sql nvarchar(max); \
        BEGIN TRY \
            SET @gauss_horizon_probe_sql = CAST(@P4 AS nvarchar(max)) + N'SELECT TOP (0) * INTO ' + QUOTENAME(@gauss_horizon_probe_table) + \
                N' FROM ' + @P3; \
            EXEC sys.sp_executesql @gauss_horizon_probe_sql; \
            ...
            WHERE c.object_id = OBJECT_ID(@gauss_horizon_probe_object) \
            ORDER BY c.column_id; \
            SET @gauss_horizon_probe_sql = N'DROP TABLE ' + QUOTENAME(@gauss_horizon_probe_table); \
            EXEC sys.sp_executesql @gauss_horizon_probe_sql; \
        END TRY \
        BEGIN CATCH \
            IF OBJECT_ID(@gauss_horizon_probe_object) IS NOT NULL \
            BEGIN \
                SET @gauss_horizon_probe_sql = N'DROP TABLE ' + QUOTENAME(@gauss_horizon_probe_table); \
                EXEC sys.sp_executesql @gauss_horizon_probe_sql; \
            END \
            DECLARE @gauss_horizon_probe_error nvarchar(2048) = ERROR_MESSAGE(); \
            RAISERROR(@gauss_horizon_probe_error, 16, 1); \
        END CATCH \
    END";
const SIMPLE_QUERY_MODULE_KEYWORDS: &[&str] = &["FUNCTION", "PROC", "PROCEDURE", "TRIGGER", "VIEW"];
const SQLSERVER_INTERNAL_HEALTH_CHECK_SQL: &str = "/* GAUSS_HORIZON_INTERNAL_TRACE */ SELECT 1";
...
// Some very old SQL Server setups only accepted Gauss Horizon <= 0.5.48 because the fallback
                ...
                .map_err(|plain_error| {
                    if is_sqlserver_tls_handshake_error(&encrypted_error) {
                        ...
                        "{encrypted_error}\n\nThis may be caused by an old SQL Server TLS/encryption configuration. \
                         ...
                         when explicitly enabled, Gauss Horizon can also fall back to the SQL Server legacy compatibility \
...
fn is_sqlserver_legacy_duplicate_probe_error(error: &str) -> bool {
    error.starts_with(SQLSERVER_UNSAFE_PROBE_BLOCK_ERROR_PREFIX)
        && error.to_ascii_lowercase().contains("gauss_horizon_probe_source")
}
...
fn sqlserver_legacy_wildcard_metadata_query(sql: &str) -> Option<String> {
    ...
    let selects = relations
        ...
        .map(|(index, relation)| {
            ...
            format!(
                "SELECT {} AS gauss_horizon_source_ordinal, c.column_id AS gauss_horizon_column_ordinal, c.name, \
    ...
    Some(format!(
        "{checks} SELECT name, system_type_name, user_type_schema, user_type_name \
         FROM ({selects}) AS gauss_horizon_wildcard_metadata \
         ORDER BY gauss_horizon_source_ordinal, gauss_horizon_column_ordinal"
    ))
}
...
fn sqlserver_legacy_probe_with_nonce(sql: &str, nonce: &str) -> Option<SqlServerLegacyProbe> {
    ...
    let source_alias = quote_sqlserver_identifier("gauss_horizon_probe_source");
...
fn sqlserver_probe_explicit_alias(nonce: &str, projection_ordinal: usize) -> String {
    format!("__gauss_horizon_probe_{nonce}_explicit_{projection_ordinal}__")
}
...
fn build_sqlserver_unsafe_type_query(
    ...
    let statement = normalized_sqlserver_select_statement(sql)?;
    let source_alias = quote_sqlserver_identifier("gauss_horizon_unsafe_source");
...
fn sqlserver_source_column_name(index: usize) -> String {
    format!("gauss_horizon_col_{}", index + 1)
}
...
fn sqlserver_completion_assistant_sql(request: &crate::types::CompletionAssistantRequest, limit: usize) -> String {
        ...
        && object_kinds.iter().any(crate::types::CompletionAssistantObjectKind::is_table_like)
    {
        ...
        return format!("SELECT * FROM ({}) AS gauss_horizon_completion ORDER BY name", queries.remove(0));
    }
    ...
    if queries.is_empty() {
        ...
        format!("SELECT * FROM ({}) AS gauss_horizon_completion ORDER BY name", queries.remove(0))
    } else {
        format!(
            "SELECT TOP ({limit}) * FROM ({}) AS gauss_horizon_completion ORDER BY name",
...
fn sqlserver_list_tables_sql_with_kind(
    ...
    match (limit, offset) {
        (Some(limit), Some(offset)) if offset > 0 => {
            ...
            format!(
                "SELECT * FROM (\
                 SELECT {base_columns}, ROW_NUMBER() OVER ({order_by}) AS __gauss_horizon_rn \
                 {base_from} {base_where}\
                 ) AS __gauss_horizon_page WHERE __gauss_horizon_rn > {offset} AND __gauss_horizon_rn <= {end} ORDER BY __gauss_horizon_rn"
            )
        }
        (Some(limit), _) => {
...
async fn execute_query_with_max_rows_inner(
        ...
        || sqlserver_dml_output_returns_rows(sql)
    {
        ...
        strip_gauss_horizon_sqlserver_row_number_column(&mut result, sql);
        Ok(result)
        ...
        match sqlserver_unsafe_type_query(client, sql).await {
            Ok(Some(query)) => {
                ...
                return result.map(|result| {
                    ...
                    strip_gauss_horizon_sqlserver_row_number_column(&mut result.result, sql);
                    vec![result]
                });
            }
            Err(error) if is_blocking_sqlserver_unsafe_probe_error(&error) => return Err(error),
...
fn strip_gauss_horizon_sqlserver_row_number_column(result: &mut QueryResult, sql: &str) {
    if !is_gauss_horizon_sqlserver_row_number_page_sql(sql) {
        return;
    }
    if !result.columns.last().is_some_and(|column| column.eq_ignore_ascii_case("__gauss_horizon_row_num")) {
        return;
    }
...
fn is_gauss_horizon_sqlserver_row_number_page_sql(sql: &str) -> bool {
    let normalized = sql.to_ascii_uppercase();
    normalized.contains("ROW_NUMBER() OVER")
        && normalized.contains("[__GAUSS_HORIZON_ROW_NUM]")
        && normalized.contains("GAUSS_HORIZON_PAGE_SOURCE.*")
}
...
mod tests {
    use super::{
        ...
        sqlserver_visible_object_predicate, strip_gauss_horizon_sqlserver_row_number_column, SqlServerDescribedColumn,
    ...
    async fn sqlserver_ignores_non_info_tiberius_events() {
        let (_, messages) = capture_sqlserver_messages(async {
            ...
            tracing::event!(target: "gauss_horizon_core::db::sqlserver", tracing::Level::INFO, "not a TDS token");
        })
        .await;
    ...
    #[test]
    fn sqlserver_native_encryption_flag_accepts_gauss_horizon_and_jdbc_params() {
        ...
        assert!(super::sqlserver_native_encryption_disabled(Some(
            "applicationName=gauss-horizon;sqlserverEncryption=off"
        )));
        assert!(super::sqlserver_native_encryption_disabled(Some(
            "?sqlserverEncryption=false&applicationName=gauss-horizon"
        )));
        assert!(super::sqlserver_native_encryption_disabled(Some("applicationName=gauss-horizon;encrypt=false")));
        assert!(super::sqlserver_native_encryption_disabled(Some("?Encrypt=0&applicationName=gauss-horizon")));
    }
    ...
    fn sqlserver_single_result_entrypoint_keeps_result_returning_batches() {
        ...
        let first_result = first_result.split("fn strip_gauss_horizon_sqlserver_row_number_column").next().unwrap();
        assert!(first_result
    ...
    fn sqlserver_explicit_simple_batch_bypasses_result_type_probing() {
        ...
        let simple_batch = simple_batch.split("fn strip_gauss_horizon_sqlserver_row_number_column").next().unwrap();
    ...
    fn sqlserver_table_objects_sql_excludes_views_before_pagination() {
        ...
        assert!(sql.contains("__gauss_horizon_rn > 100 AND __gauss_horizon_rn <= 201"));
    }
    ...
    fn sqlserver_table_objects_sql_preserves_sidebar_probe_limits_above_1000() {
        ...
        assert!(next_page.contains("__gauss_horizon_rn > 1000 AND __gauss_horizon_rn <= 2001"));
    }
    ...
    fn sqlserver_completion_assistant_searches_columns_by_parent_table() {
        ...
        assert!(!sql.contains("AS gauss_horizon_completion ORDER BY name"));
    }
    ...
    fn sqlserver_strips_generated_row_number_pagination_column() {
        let sql = "SELECT * FROM (SELECT gauss_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS [__gauss_horizon_row_num] FROM (SELECT id FROM users) gauss_horizon_page_source) gauss_horizon_page WHERE [__gauss_horizon_row_num] > 100 AND [__gauss_horizon_row_num] <= 200 ORDER BY [__gauss_horizon_row_num];";
        let mut result = QueryResult {
            columns: vec!["id".to_string(), "__gauss_horizon_row_num".to_string()],
    ...
    fn sqlserver_wraps_geometry_columns_as_text() {
        ...
        assert_eq!(
            rewritten.sql,
            "SELECT [landId] = [gauss_horizon_unsafe_source].[gauss_horizon_col_1], [polygon] = CASE WHEN [gauss_horizon_unsafe_source].[gauss_horizon_col_2] IS NULL THEN NULL ELSE N'SRID=' + CONVERT(nvarchar(20), [gauss_horizon_unsafe_source].[gauss_horizon_col_2].STSrid) + N';' + [gauss_horizon_unsafe_source].[gauss_horizon_col_2].AsTextZM() END FROM (SELECT * FROM dbo.tLandPolygon) AS [gauss_horizon_unsafe_source]([gauss_horizon_col_1], [gauss_horizon_col_2])"
        );
        assert_eq!(
    ...
    fn sqlserver_wraps_hierarchyid_columns_as_logical_paths() {
        ...
        assert_eq!(
            rewritten.sql,
            "SELECT [id] = [gauss_horizon_unsafe_source].[gauss_horizon_col_1], [path] = CASE WHEN [gauss_horizon_unsafe_source].[gauss_horizon_col_2] IS NULL THEN NULL ELSE [gauss_horizon_unsafe_source].[gauss_horizon_col_2].ToString() END, [label] = [gauss_horizon_unsafe_source].[gauss_horizon_col_3] FROM (SELECT id, path, label FROM dbo.nodes) AS [gauss_horizon_unsafe_source]([gauss_horizon_col_1], [gauss_horizon_col_2], [gauss_horizon_col_3]) ORDER BY [id]"
        );
    ...
    fn sqlserver_orders_hierarchyid_by_the_original_server_value() {
        ...
        assert!(rewritten.sql.ends_with("ORDER BY [gauss_horizon_unsafe_source].[gauss_horizon_col_2]"));
    }
    ...
    fn sqlserver_preserves_order_by_when_wrapping_geometry_columns() {
        ...
        assert!(rewritten
            .sql
            .contains("FROM (SELECT landId, polygon FROM dbo.tLandPolygon) AS [gauss_horizon_unsafe_source]"));
    ...
    fn sqlserver_migrates_order_by_ordinals_and_offset_fetch_to_outer_query() {
        ...
        assert!(rewritten
            .sql
            .contains("FROM (SELECT id, landId, polygon FROM dbo.t) AS [gauss_horizon_unsafe_source]"));
    ...
    fn sqlserver_keeps_order_by_inside_for_top_queries_and_reapplies_outside() {
        ...
        assert!(rewritten
            .sql
            .contains("FROM (SELECT TOP 10 id, polygon FROM dbo.t ORDER BY id) AS [gauss_horizon_unsafe_source]"));
        assert!(rewritten.sql.ends_with(" ORDER BY [id]"));
    }
    ...
    fn sqlserver_keeps_unmapped_order_by_inside_variant_rewrite() {
        ...
        assert!(rewritten.sql.contains(
            "FROM (SELECT TOP (100) PERCENT id, value FROM sys.extended_properties ORDER BY major_id) AS [gauss_horizon_unsafe_source]"
        ));
    ...
    fn sqlserver_skips_top_percent_when_inner_order_by_carries_offset_fetch() {
        ...
        assert!(rewritten.sql.contains(
            "FROM (SELECT id, value FROM sys.extended_properties ORDER BY major_id OFFSET 10 ROWS FETCH NEXT 5 ROWS ONLY) AS [gauss_horizon_unsafe_source]"
        ));
    ...
    fn sqlserver_result_type_probe_keeps_modern_and_legacy_paths_in_one_round_trip() {
        ...
        assert!(SQLSERVER_RESULT_TYPE_PROBE_SQL.contains("##gauss_horizon_result_type_probe_"));
        ...
        assert_eq!(SQLSERVER_RESULT_TYPE_PROBE_SQL.matches("SELECT @gauss_horizon_use_describe_dmv").count(), 1);
    }

    #[test]
    fn sqlserver_legacy_probe_keeps_long_projection_text_untruncated() {
        let aliases = (1..=136).map(|index| format!("[gauss_horizon_col_{index}]")).collect::<Vec<_>>().join(", ");
        let source_sql = format!(
            "(SELECT TOP (100) {} FROM [dbo].[ProjectInfo] WHERE ([PrjId] = N'123')) AS [gauss_horizon_probe_source]({aliases})",
    ...
    fn sqlserver_legacy_duplicate_probe_uses_bounded_metadata_recovery() {
        assert!(is_sqlserver_legacy_duplicate_probe_error(
            "SQL Server unsafe result type: The column 'usergrp' was specified multiple times for 'gauss_horizon_probe_source'."
        ));
        assert!(!is_sqlserver_legacy_duplicate_probe_error(
        ...
        assert!(query.contains("ORDER BY gauss_horizon_source_ordinal, gauss_horizon_column_ordinal"));
    ...
    fn sqlserver_legacy_probe_uses_unique_internal_names_for_duplicate_outputs() {
        ...
        assert_eq!(
            probe.source_sql,
            "(SELECT a.HJRQ, b.HJRQ, a.id + b.id AS total, GETDATE() FROM dbo.a a JOIN dbo.b b ON b.id = a.id) AS [gauss_horizon_probe_source]([gauss_horizon_col_1], [gauss_horizon_col_2], [gauss_horizon_col_3], [gauss_horizon_col_4])"
        );
        assert_eq!(
            probe.output_names,
    ...
    fn sqlserver_legacy_probe_preserves_cte_prefix() {
        ...
        assert_eq!(probe.source_sql, "(SELECT num FROM source) AS [gauss_horizon_probe_source]([gauss_horizon_col_1])");
    ...
    fn sqlserver_legacy_probe_preserves_comments_before_select() {
        ...
        assert_eq!(probe.source_sql, "(SELECT 1 AS num) AS [gauss_horizon_probe_source]([gauss_horizon_col_1])");
    }
    ...
    fn sqlserver_legacy_probe_aliases_explicit_columns_next_to_wildcards() {
        ...
        assert!(probe.source_sql.ends_with(") AS [gauss_horizon_probe_source]"));
    ...
    fn sqlserver_legacy_probe_does_not_collide_with_old_probe_alias_column() {
        ...
        let old_probe_name = "__gauss_horizon_probe_explicit_1__";
        let probe = sqlserver_legacy_probe_with_nonce(
            "SELECT t.[__gauss_horizon_probe_explicit_1__], t.* FROM dbo.t AS t",
            nonce,
        )
        .unwrap();
    ...
    fn sqlserver_wraps_cte_sql_variant_columns_as_nvarchar() {
        ...
        assert!(rewritten.sql.contains("SELECT [major_id] = [gauss_horizon_unsafe_source].[gauss_horizon_col_1]"));
        assert!(rewritten
            .sql
            .contains("[value] = CAST([gauss_horizon_unsafe_source].[gauss_horizon_col_2] AS NVARCHAR(MAX))"));
        assert!(rewritten.sql.contains(
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD"]
    async fn live_sqlserver_legacy_probe_casts_variant_and_keeps_connection_usable() {
        let host = crate::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST").expect("GAUSS_HORIZON_LIVE_SQLSERVER_HOST");
        let port = crate::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
            .expect("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
            .parse::<u16>()
            .expect("valid GAUSS_HORIZON_LIVE_SQLSERVER_PORT");
        let user = crate::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_USER").expect("GAUSS_HORIZON_LIVE_SQLSERVER_USER");
        let password =
            crate::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD").expect("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD");
        let mut client =
        ...
        let setup = "\
            IF OBJECT_ID('tempdb..#gauss_horizon_issue_4002') IS NOT NULL DROP TABLE #gauss_horizon_issue_4002; \
            IF OBJECT_ID('tempdb..#gauss_horizon_issue_5606_extra') IS NOT NULL DROP TABLE #gauss_horizon_issue_5606_extra; \
            CREATE TABLE #gauss_horizon_issue_4002 (\
                id int NOT NULL, ybbz int NULL, cytzrq date NULL, jzrq datetime NULL, payload sql_variant NULL\
            ); \
            CREATE TABLE #gauss_horizon_issue_5606_extra (id int NOT NULL); \
            INSERT INTO #gauss_horizon_issue_4002 (id, ybbz, cytzrq, jzrq, payload) \
            VALUES (1, 2, '2026-07-28', '2026-07-28T12:34:56', CAST(N'legacy' AS nvarchar(20))); \
            INSERT INTO #gauss_horizon_issue_5606_extra VALUES (2)";
        client.simple_query(setup).await.unwrap().into_results().await.unwrap();

        let sql = "SELECT id, payload FROM #gauss_horizon_issue_4002";
        ...
        let cte_sql =
            "WITH source AS (SELECT id, payload FROM #gauss_horizon_issue_4002) SELECT id, payload FROM source";
        ...
        let duplicate_sql = "SELECT id AS HJRQ, payload AS HJRQ FROM #gauss_horizon_issue_4002";
        ...
        let wildcard_sql = "SELECT ybbz, cytzrq, jzrq, * FROM #gauss_horizon_issue_4002";
        let previous_wildcard_probe = super::SqlServerLegacyProbe {
            prefix: String::new(),
            source_sql: format!("({wildcard_sql}) AS [gauss_horizon_probe_source]"),
            output_names: None,
        ...
        let previous_error =
            super::describe_sqlserver_result_set_with_mode(&mut client, wildcard_sql, &previous_wildcard_probe, true)
                .await
                .unwrap_err();
        assert!(previous_error.contains("gauss_horizon_probe_source"));
        ...
        let pure_wildcard_variant_sql =
            "SELECT * FROM #gauss_horizon_issue_4002 AS a CROSS JOIN #gauss_horizon_issue_5606_extra AS b";
        ...
        client
            .simple_query(
                "DROP TABLE #purctl; DROP TABLE #deptctl; DROP TABLE #whctl; \
                 DROP TABLE #gauss_horizon_issue_5606_extra; DROP TABLE #gauss_horizon_issue_4002",
            )
            .await
            .unwrap()
            .into_results()
    ...
    async fn live_sqlserver_point_zm_query_and_export_preserve_z_and_m() {
        ...
        let setup = "\
            IF OBJECT_ID('tempdb..#gauss_horizon_point_zm') IS NOT NULL DROP TABLE #gauss_horizon_point_zm; \
            CREATE TABLE #gauss_horizon_point_zm (id int NOT NULL, geom geometry NOT NULL); \
            INSERT INTO #gauss_horizon_point_zm (id, geom) VALUES \
        ...
        let sql = "SELECT id, geom FROM #gauss_horizon_point_zm ORDER BY id";
        ...
        client.simple_query("DROP TABLE #gauss_horizon_point_zm").await.unwrap().into_results().await.unwrap();
    }
    ...
    async fn live_sqlserver_geometry_order_and_pagination_are_preserved() {
        ...
        let setup = "\
            IF OBJECT_ID('tempdb..#gauss_horizon_geometry_order') IS NOT NULL DROP TABLE #gauss_horizon_geometry_order; \
            CREATE TABLE #gauss_horizon_geometry_order (id int NOT NULL, name varchar(20) NOT NULL, geom geometry NOT NULL); \
            INSERT INTO #gauss_horizon_geometry_order (id, name, geom) VALUES \
        ...
        let ordered =
            super::execute_query(&mut client, "SELECT id, name, geom FROM #gauss_horizon_geometry_order ORDER BY id")
                .await
                .unwrap();
        ...
        let page = super::execute_query(
            &mut client,
            "SELECT id, name, geom FROM #gauss_horizon_geometry_order ORDER BY id OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY",
        )
        .await
        .unwrap();
        ...
        let beyond = super::execute_query(
            &mut client,
            "SELECT id, name, geom FROM #gauss_horizon_geometry_order ORDER BY id OFFSET 10 ROWS",
        )
        .await
        .unwrap();
        ...
        let descending = super::execute_query(
            &mut client,
            "SELECT id, name, geom FROM #gauss_horizon_geometry_order ORDER BY id DESC",
        )
        .await
        .unwrap();
        ...
        let top = super::execute_query(
            &mut client,
            "SELECT TOP 2 id, name, geom FROM #gauss_horizon_geometry_order ORDER BY id",
        )
        .await
        .unwrap();
        assert_eq!(ids(&top), vec![1, 2]);

        client.simple_query("DROP TABLE #gauss_horizon_geometry_order").await.unwrap().into_results().await.unwrap();
    }
    ...
    async fn live_sqlserver_mixed_srid_columns_keep_per_cell_srids() {
        ...
        let setup = "\
            IF OBJECT_ID('tempdb..#gauss_horizon_mixed_srid') IS NOT NULL DROP TABLE #gauss_horizon_mixed_srid; \
            CREATE TABLE #gauss_horizon_mixed_srid (id int NOT NULL, geom_a geometry NOT NULL, geom_b geometry NOT NULL); \
            INSERT INTO #gauss_horizon_mixed_srid (id, geom_a, geom_b) VALUES \
        ...
        let result =
            super::execute_query(&mut client, "SELECT id, geom_a, geom_b FROM #gauss_horizon_mixed_srid ORDER BY id")
                .await
                .unwrap();
        ...
        client.simple_query("DROP TABLE #gauss_horizon_mixed_srid").await.unwrap().into_results().await.unwrap();
    }
}
