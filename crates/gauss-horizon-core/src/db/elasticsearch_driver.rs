...
/// Close a previously returned ES cursor. Supports Gauss Horizon-wrapped paged cursors,
/// Gauss Horizon-wrapped SQL cursors, and raw ES SQL cursors.
...
/// form is produced by the Gauss Horizon pagination plan; it must be removed before
...
mod tests {
  ...
  "products": {
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_KIBANA_URL pointing to a reachable Kibana instance"]
    async fn live_kibana_proxy_supports_metadata_queries_and_document_writes() {
        let url =
            crate::legacy::var("GAUSS_HORIZON_TEST_KIBANA_URL").expect("GAUSS_HORIZON_TEST_KIBANA_URL is required");
        let username = crate::legacy::var("GAUSS_HORIZON_TEST_KIBANA_USERNAME").ok();
        let password = crate::legacy::var("GAUSS_HORIZON_TEST_KIBANA_PASSWORD").ok();
        let base_path = crate::legacy::var("GAUSS_HORIZON_TEST_KIBANA_BASE_PATH").unwrap_or_default();
        ...
        let index = format!("gauss-horizon-kibana-proxy-{}", uuid::Uuid::new_v4().simple());
