...
use gauss_horizon_sqlite_worker::{WorkerBody, WorkerOp, WorkerRequest, WorkerResponse};
...
const WORKER_PATH_ENV: &str = "GAUSS_HORIZON_SQLITE_WORKER_PATH";
const DEFAULT_PERSIST_DIR: &str = "~/.cache/gauss-horizon/sqlite-worker";
...
fn sqlite_worker_placement(config: &ConnectionConfig) -> SqliteWorkerPlacement {
    match url_param(config.url_params.as_deref(), "gauss_horizon_sqlite_worker").as_deref() {
...
pub fn sqlite_worker_remote_path(config: &ConnectionConfig) -> String {
    url_param(config.url_params.as_deref(), "gauss_horizon_sqlite_worker_path").unwrap_or_default()
}
...
pub async fn connect_sqlite_worker(
    ...
    if !sqlite_ssh_runtime_enabled() {
        return Err("Remote SQLite over SSH is only available in the Gauss Horizon Desktop app".to_string());
    }
...
async fn remote_xfer_path(session: &Handle<SshClient>) -> Result<String, String> {
    ...
    let parent = format!("{}/.cache/gauss-horizon/sqlite-worker", home);
...
fn empty_cache_dirs_to_remove(worker_path: &str) -> Vec<String> {
    ...
    if let Some(gauss_horizon_dir) = sqlite_worker_dir.parent() {
        if gauss_horizon_dir.file_name().is_some_and(|name| name == "gauss-horizon") {
            dirs.push(gauss_horizon_dir.to_string_lossy().into_owned());
        }
    }
    dirs
}
...
fn is_remote_linux_path_absolute_or_home_relative(path: &str) -> bool {
    // The remote host is Linux even when the Gauss Horizon Desktop client runs on Windows.
...
mod tests {
    ...
    fn default_placement_is_session() {
        ...
        config.url_params = Some("gauss_horizon_sqlite_worker=persist".into());
        assert_eq!(sqlite_worker_placement(&config), SqliteWorkerPlacement::Persist);
        config.url_params =
            Some("gauss_horizon_sqlite_worker=preplaced&gauss_horizon_sqlite_worker_path=%2Fopt%2Fworker".into());
    ...
    fn session_placement_removes_worker_on_disconnect() {
        ...
        assert_eq!(
            ...
            "~/.cache/gauss-horizon/sqlite-worker/session-conn-1-abc123"
        );
        assert_eq!(
            empty_cache_dirs_to_remove("/home/u/.cache/gauss-horizon/sqlite-worker/session-conn-1-abc123"),
            vec!["/home/u/.cache/gauss-horizon/sqlite-worker".to_string(), "/home/u/.cache/gauss-horizon".to_string()]
        );
        assert!(empty_cache_dirs_to_remove("/opt/gauss-horizon/gauss-horizon-sqlite-worker").is_empty());
        assert!(remove_session_worker_command("/home/u/.cache/gauss-horizon/sqlite-worker/session-conn-1-abc123")
            .contains("rmdir '/home/u/.cache/gauss-horizon/sqlite-worker' 2>/dev/null; rmdir '/home/u/.cache/gauss-horizon' 2>/dev/null"));
    }
