...
pub async fn connect_with_oidc(
    ...
    with_connection_timeout("MongoDB", parse_timeout, async {
        ...
        if let Some(credential) = options.credential.as_mut().filter(|credential| {
            ...
            // install Gauss Horizon's browser callback for interactive human OIDC.
            if !mongo_oidc_uses_machine_environment(credential.mechanism_properties.as_ref()) {
                let opener = oidc_browser_opener.ok_or_else(|| {
                    "MongoDB OIDC browser authentication is only available in the Gauss Horizon desktop app".to_string()
                })?;
...
pub async fn create_database(client: &Client, database: &str) -> Result<(), String> {
    ...
    client.database(database).create_collection("gauss_horizon_init").await.map_err(|e| e.to_string())
}
...
fn decode_string_document_id(id: &str) -> Option<String> {
    id.strip_prefix("__gauss_horizon_mongo_string_id__")
...
mod tests {
    ...
    fn document_id_filters_decode_explicit_string_ids_before_extended_json() {
        ...
        let id = format!("__gauss_horizon_mongo_string_id__{}", serde_json::to_string(original).unwrap());
        let filters = document_id_filters(&id);
    ...
    fn document_id_filters_keep_explicit_hex_string_ids_as_strings() {
        ...
        let id = format!("__gauss_horizon_mongo_string_id__{}", serde_json::to_string(hex).unwrap());
        let filters = document_id_filters(&id);
