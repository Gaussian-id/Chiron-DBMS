...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_LIVE_VICTORIAMETRICS_* env vars pointing at a VictoriaMetrics endpoint"]
    async fn live_connection_metadata_and_query() {
        let url = crate::legacy::var("GAUSS_HORIZON_LIVE_VICTORIAMETRICS_URL")
            .expect("GAUSS_HORIZON_LIVE_VICTORIAMETRICS_URL");
        let username = crate::legacy::var("GAUSS_HORIZON_LIVE_VICTORIAMETRICS_USER").unwrap_or_default();
        let password = crate::legacy::var("GAUSS_HORIZON_LIVE_VICTORIAMETRICS_PASSWORD").unwrap_or_default();
        let api_path =
            crate::legacy::var("GAUSS_HORIZON_LIVE_VICTORIAMETRICS_API_PATH").unwrap_or_else(|_| "/prometheus".into());
        let metric = crate::legacy::var("GAUSS_HORIZON_LIVE_VICTORIAMETRICS_METRIC")
            .expect("GAUSS_HORIZON_LIVE_VICTORIAMETRICS_METRIC");
