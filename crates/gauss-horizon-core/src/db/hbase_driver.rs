...
mod tests {
    ...
    fn scanner_location_reuses_the_configured_gateway_origin() {
        assert_eq!(
            scanner_resource_path("http://hbase.internal:8080/gauss-horizon/scanner/123?token=abc").unwrap(),
            "/gauss-horizon/scanner/123?token=abc"
        );
        assert_eq!(scanner_resource_path("/gauss-horizon/scanner/123").unwrap(), "/gauss-horizon/scanner/123");
        assert_eq!(scanner_resource_path("gauss-horizon/scanner/123").unwrap(), "/gauss-horizon/scanner/123");
    }

    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_TEST_HBASE_URL and a writable HBase REST gateway"]
    async fn live_hbase_rest_crud_and_scan() {
        let url = crate::legacy::var("GAUSS_HORIZON_TEST_HBASE_URL").expect("GAUSS_HORIZON_TEST_HBASE_URL is required");
        let username = crate::legacy::var("GAUSS_HORIZON_TEST_HBASE_USERNAME").ok();
        let password = crate::legacy::var("GAUSS_HORIZON_TEST_HBASE_PASSWORD").ok();
        ...
        let table = format!("gauss_horizon_hbase_live_{suffix}");
