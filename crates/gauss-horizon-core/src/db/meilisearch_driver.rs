...
const STRING_ID_PREFIX: &str = "__gauss_horizon_meilisearch_string_id__";
...
pub async fn update_document(client: &MeilisearchClient, index: &str, id: &str, doc_json: &str) -> Result<u64, String> {
    ...
    let mut document = parse_document_object(doc_json)?;
    // `_id` is only the gauss-horizon browse alias when it matches the target identity; a
    ...
    // Gauss Horizon sends the complete edited document so removed fields must disappear.
...
/// Search hit with gauss-horizon identity and Meilisearch response metadata kept outside
...
mod tests {
    ...
    fn search_identity_never_shadows_a_real_underscore_id_field() {
        // A document may legitimately store its own `_id` field; the gauss-horizon identity
    ...
    fn string_primary_keys_keep_their_string_identity() {
        ...
        let serialized = r#"__gauss_horizon_meilisearch_string_id__"123""#;
    ...
    async fn get_document_fetches_the_canonical_record_by_identity() {
        ...
        let document_json =
            super::get_document(&client, "movies", r#"__gauss_horizon_meilisearch_string_id__"SN-0001""#)
                .await
                .unwrap();
