...
mod tests {
    ...
    fn test_manager(name: &str) -> AgentManager {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-agent-manager-{name}-{}", uuid::Uuid::new_v4()));
    ...
    fn stores_configured_app_version_for_agent_handshake() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-agent-manager-version-{}", uuid::Uuid::new_v4()));
    ...
    fn resolves_relative_native_agent_launch_with_absolute_paths() {
        let base_dir = PathBuf::from("target")
            .join(format!("gauss-horizon-agent-manager-relative-native-{}", uuid::Uuid::new_v4()));
    ...
    fn relative_native_agent_launch_keeps_missing_driver_error() {
        let base_dir = PathBuf::from("target")
            .join(format!("gauss-horizon-agent-manager-relative-missing-{}", uuid::Uuid::new_v4()));
...
impl AgentManager {
    pub fn new() -> Self {
        ...
        Self::new_with_base_dir(PathBuf::from(home).join(".gauss-horizon").join("agents"))
    }
