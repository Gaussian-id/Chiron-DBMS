...
async fn write_callback_response(stream: &mut tokio::net::TcpStream, success: bool) {
    let (status, title, body) = if success {
        (
            ...
            "Gauss Horizon received the authorization response. Return to Gauss Horizon while it completes the connection.",
        )
    } else {
        (
            "400 Bad Request",
            "Authentication failed",
            "Gauss Horizon could not complete authentication. Return to Gauss Horizon for details.",
        )
    };
    let html = format!(
...
mod tests {
        ...
        CallbackContext::builder()
            ...
            .idp_info(Some(
                IdpServerInfo::builder()
                    .issuer(issuer)
                    .client_id(Some("gauss-horizon-client".to_string()))
    ...
    fn oidc_endpoints_require_https_and_exact_issuer_match() {
        ...
        .is_err());
        let issuer = "https://idp.example/realms/gauss-horizon";
    ...
    fn discovery_url_preserves_issuer_path() {
        assert_eq!(
            discovery_url_with_policy("https://idp.example/realms/gauss-horizon", OidcEndpointPolicy::HttpsOnly)
                .unwrap()
                .as_str(),
            "https://idp.example/realms/gauss-horizon/.well-known/openid-configuration"
        );
    }
