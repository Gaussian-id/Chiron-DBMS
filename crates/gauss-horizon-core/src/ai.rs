...
fn validate_config(config: &AiConfig) -> Result<(), String> {
    ...
    if matches!(config.provider, AiProvider::MiniMax) && config.api_style != AiApiStyle::Completions {
        return Err(
            "MiniMax currently supports the Chat Completions API style in Gauss Horizon; select Completions and retry"
                .to_string(),
        );
    }
...
/// Adds user-supplied gateway headers after Gauss Horizon's provider headers. This
...
fn append_custom_headers(headers: &mut HeaderMap, config: &AiConfig) -> Result<(), String> {
    ...
    for (raw_name, raw_value) in &config.custom_headers {
        ...
        if RESERVED.iter().any(|reserved| name.as_str().eq_ignore_ascii_case(reserved)) {
            return Err(format!("Invalid AI custom header name: {raw_name} (reserved by Gauss Horizon)"));
        }
...
/// than ordinary request metadata. Gauss Horizon lets users override `Authorization`, so
...
pub async fn complete(request: &AiCompletionRequest) -> Result<String, String> {
    ...
    if is_cli_provider(&request.config.provider) {
        return Err("CLI providers are only supported in Gauss Horizon AI agent mode".to_string());
    }
...
pub async fn stream_with_tools(
    ...
    validate_config(config)?;
    if is_cli_provider(&config.provider) {
        return Err("CLI providers are only supported through the Gauss Horizon AI agent loop".to_string());
    }
...
mod tests {
    ...
    fn image_like_text_is_never_interpreted_as_an_attachment() {
        let marker = "<gauss-horizon-image media-type=\"image/png\">aGVsbG8=</gauss-horizon-image>";
        let message = AiMessage {
            role: "tool".to_string(),
    ...
    fn custom_ai_headers_are_merged_and_protect_http_framing() {
        ...
        config.custom_headers = HashMap::from([
            ("X-GoModel-User-Path".to_string(), "/gauss-horizon-assistant".to_string()),
            ("Authorization".to_string(), "Bearer gateway-token".to_string()),
        ]);

        let headers = maybe_bearer_headers(&config).unwrap();
        assert_eq!(headers.get("x-gomodel-user-path").unwrap(), "/gauss-horizon-assistant");
        ...
        assert!(maybe_bearer_headers(&config).unwrap_err().contains("reserved by Gauss Horizon"));
    ...
    #[test]
    fn jalapeno_requests_include_gauss_horizon_referer() {
        let mut config = test_config(AiProvider::OpenaiCompatible);
