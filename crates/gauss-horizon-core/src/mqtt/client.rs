...
mod tests {
    ...
    fn verified_tls_does_not_depend_on_platform_certificate_store() {
        let missing_cert_file =
            std::env::temp_dir().join(format!("gauss-horizon-mqtt-missing-ca-{}.pem", uuid::Uuid::new_v4()));
    ...
    fn mqtt_config(port: u16, protocol_version: crate::mqtt::types::MqttProtocolVersion) -> MqttConnectionConfig {
        MqttConnectionConfig {
            ...
            port,
            client_id: format!("gauss-horizon-test-{}", uuid::Uuid::new_v4()),
            protocol_version,
            ...
            for _ in 0..expected_connections {
                ...
                match packet {
                    rumqttc::v5::mqttbytes::v5::Packet::Connect(connect, _, _) => {
                        assert!(connect.client_id.starts_with("gauss-horizon-test-"));
                    }
                    other => panic!("unexpected packet: {other:?}"),
                }
