...
mod tests {
    ...
    async fn test_shared_runtime(
        ...
        let script_path = std::env::temp_dir().join(format!("gauss-horizon-agent-runtime-{name}-{test_id}.py"));
        std::fs::write(
            &script_path,
            r#"import json, sys
...
"#,
        ...
        let manager_dir = std::env::temp_dir().join(format!("gauss-horizon-agent-manager-{name}-{test_id}"));
