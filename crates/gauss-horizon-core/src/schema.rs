...
fn oracle_object_statistics_user_segments_sql(schema: &str) -> String {
    // USER_SEGMENTS exposes objects owned by the login/current user, while Gauss Horizon
...
/// URL. The prefixes mirror `GaussHorizonJdbcPlugin.driverQuirks`, which picks its own
...
mod tests {
        ...
        let server = tokio::spawn(async move {
            ...
            while !saw_table_query {
                ...
                let body = if is_table_query {
                    r#"{"results":[{"type":"ok","response":{"type":"execute","result":{"cols":[{"name":"name","decltype":"TEXT"},{"name":"type","decltype":"TEXT"}],"rows":[[{"type":"text","value":"gauss_horizon_test_records"},{"type":"text","value":"table"}]],"rows_read":1,"rows_written":0}}}]}"#
                } else {
    ...
    async fn turso_test_state(base_url: &str) -> (AppState, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-turso-schema-test-{}", uuid::Uuid::new_v4()));
    ...
    async fn turso_schema_dispatch_lists_databases_and_tables() {
        ...
        assert_eq!(tables[0].name, "gauss_horizon_test_records");
    ...
    async fn mysql_external_driver_ddl_falls_back_to_generic_source() {
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-mysql-wrapper-ddl-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    fn external_driver_statistics_query_plan_reuses_vendor_sql() {
        let oracle =
            external_driver_statistics_query_plan(ExternalDriverStatisticsDialect::Oracle, "gauss_horizon_test");
        assert_eq!(
            oracle.iter().map(|(source, ..)| *source).collect::<Vec<_>>(),
            vec!["all-segments", "dba-segments", "user-segments", "rows-only"]
        );
        assert_eq!(oracle[0].1, oracle_object_statistics_sql("gauss_horizon_test"));
        assert!(oracle[0].1.contains("t.OWNER = 'GAUSS_HORIZON_TEST'"));

        let dameng =
            external_driver_statistics_query_plan(ExternalDriverStatisticsDialect::Dameng, "gauss_horizon_test");
        assert_eq!(dameng[0].1, dameng_object_statistics_dba_segments_sql("gauss_horizon_test"));
    ...
    fn mysql_object_source_sql_emits_show_create_materialized_view() {
        ...
        // real statement (used at crates/gauss-horizon-core/src/schema.rs:5395-5404 by
    ...
    fn metadata_retry_excludes_pool_saturation_from_reconnects() {
        ...
        assert!(!is_retryable_metadata_error(
            "Agent RPC error (-1): connection text in SQL error\nGAUSS_HORIZON_AGENT_ERROR_DATA:{\"category\":\"sql\",\"sessionDisposition\":\"keep\"}"
        ));
        assert!(!is_retryable_metadata_error(
            "Agent RPC error (-1): connection kept\nGAUSS_HORIZON_AGENT_ERROR_DATA:{\"category\":\"connection\",\"sessionDisposition\":\"keep\"}"
        ));
        assert!(!is_retryable_metadata_error(
    ...
    async fn metadata_pool_races_retry_once_and_saturation_returns_busy() {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-schema-metadata-pool-race-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn metadata_pool_snapshot_releases_global_connections_lock() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-schema-metadata-snapshot-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = crate::storage::Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = crate::connection::AppState::new(storage);
    ...
    async fn metadata_fail_stop_detaches_base_pool_without_client_session() {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-schema-metadata-fail-stop-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = crate::storage::Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = crate::connection::AppState::new(storage);
    ...
    async fn metadata_timeout_detaches_pool_without_replaying_operation() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-schema-metadata-timeout-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = crate::storage::Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = crate::connection::AppState::new(storage);
        let mut config = test_connection_config(DatabaseType::Dameng);
    ...
    async fn metadata_second_quarantine_detaches_replacement_pool() {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-schema-metadata-quarantine-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = crate::storage::Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = crate::connection::AppState::new(storage);
    ...
    async fn table_ddl_timeout_detaches_metadata_pool_without_replay() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-schema-table-ddl-timeout-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
...
"#
    ...
    fn tdengine_table_comment_sql_targets_one_name_and_escapes_literals() {
        let sql = tdengine_table_comment_sql("gauss-horizon's", "meter's");

        assert!(sql.contains("information_schema.ins_stables"));
        assert!(sql.contains("information_schema.ins_tables"));
        assert!(sql.contains("db_name = 'gauss-horizon''s'"));
    ...
    fn tdengine_table_comments_sql_only_queries_comments_matching_the_filter() {
        let sql = tdengine_table_comments_sql("gauss-horizon's", "s_%\\q");
    ...
    fn oracle_table_comments_sql_targets_current_page_tables() {
        let sql =
            oracle_table_comments_sql("gauss_horizon_test", &["ORDERS".to_string(), "USER'S".to_string()]).unwrap();

        assert!(sql.contains("ALL_TAB_COMMENTS"));
        assert!(sql.contains("OWNER = 'GAUSS_HORIZON_TEST'"));
        assert!(sql.contains("TABLE_NAME IN ('ORDERS', 'USER''S')"));
        assert!(sql.contains("TABLE_TYPE IN ('TABLE', 'VIEW')"));
        assert!(sql.contains("COMMENTS IS NOT NULL"));
        assert_eq!(oracle_table_comments_sql("GAUSS_HORIZON_TEST", &[]), None);
    }
    ...
    fn oracle_columns_sql_uses_exact_table_name_for_quoted_lowercase_tables() {
        let sql = oracle_columns_sql("GAUSS_HORIZON_TEST", "test").unwrap();
        ...
        assert!(sql.contains("c.OWNER = 'GAUSS_HORIZON_TEST'"));
    ...
    fn oracle_synonym_sql_resolves_private_before_public_without_hierarchical_queries() {
        let sql = oracle_synonym_target_sql("GaussHorizon'Owner", "ORDERS_ALIAS", true).unwrap();

        assert!(!sql.contains("SYS_CONTEXT"));
        assert!(sql.contains("FROM ALL_SYNONYMS s"));
        assert!(sql.contains("s.OWNER IN ('GaussHorizon''Owner', 'PUBLIC')"));
        assert!(sql.contains("CASE WHEN s.OWNER = 'GaussHorizon''Owner' THEN 0 ELSE 1 END"));
    ...
    fn oracle_current_schema_result_rejects_missing_empty_and_non_string_values() {
        assert!(oracle_current_schema_from_query_result(oracle_current_schema_result(
            &["OTHER"],
            vec![vec![serde_json::json!("GAUSS_HORIZON_TEST")]],
        ))
        .is_err());
    ...
    fn oracle_synonym_resolver_follows_two_levels_and_rejects_cycles() {
        let initial = OracleObjectRef { owner: "GAUSS_HORIZON_TEST".to_string(), name: "ORDERS_ALIAS".to_string() };
    ...
    fn oracle_columns_sql_preserves_quoted_case_synonym_names_and_excludes_database_links() {
        let sql = oracle_synonym_target_sql("GAUSS_HORIZON_TEST", "Order Alias", true).unwrap();
    ...
    fn oracle_synonym_resolver_enforces_maximum_depth() {
        let mut resolver = OracleSynonymResolver::new("GAUSS_HORIZON_TEST".to_string(), "ALIAS_0".to_string());
        for depth in 1..=ORACLE_SYNONYM_MAX_DEPTH {
            assert!(resolver
                .follow(OracleObjectRef { owner: "GAUSS_HORIZON_TEST".to_string(), name: format!("ALIAS_{depth}") }));
        }

        assert!(!resolver.can_follow());
        assert!(!resolver
            .follow(OracleObjectRef { owner: "GAUSS_HORIZON_TEST".to_string(), name: "ALIAS_TOO_DEEP".to_string() }));
    }
    ...
    match kind {
        db::ObjectSourceKind::View | db::ObjectSourceKind::MaterializedView => {
            ...
            let search_path_cte = if isolate_view_search_path {
                "WITH gauss_horizon_search_path AS (SELECT pg_catalog.set_config('search_path', '', true) AS applied) "
            } else {
                ""
            };
            let search_path_join =
                if isolate_view_search_path { " CROSS JOIN gauss_horizon_search_path path_guard" } else { "" };
    ...
    if isolate_search_path {
        format!(
            "WITH gauss_horizon_search_path AS (SELECT pg_catalog.set_config('search_path', '', true) AS applied) \
             ...
             CROSS JOIN gauss_horizon_search_path path_guard \
...
mod object_source_tests {
    ...
    fn builds_postgres_object_source_sql_for_views_and_functions() {
        ...
        assert!(view_sql.contains("CROSS JOIN gauss_horizon_search_path path_guard"));
    ...
    fn builds_postgres_view_source_fallback_sql_from_pg_views() {
        assert_eq!(
            ...
            "WITH gauss_horizon_search_path AS (SELECT pg_catalog.set_config('search_path', '', true) AS applied) SELECT v.definition FROM pg_catalog.pg_views v CROSS JOIN gauss_horizon_search_path path_guard WHERE v.schemaname = 'tenant''s schema' AND v.viewname = 'active users' AND path_guard.applied IS NOT NULL LIMIT 1"
        );
        assert_eq!(
...
fn ensure_display_ddl_terminated(sql: String) -> String {
    ...
    // SHOW CREATE TABLE returns a table definition, not a runnable script; Gauss Horizon
    ...
    for idx in indexes {
        ...
        let cols = idx
            ...
            .map(|(i, c)| {
                ...
                // opclass — see `crates/gauss-horizon-core/src/db/postgres.rs`), so quoting the whole
