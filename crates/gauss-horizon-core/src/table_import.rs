...
/// files that gauss-horizon did not produce.
...
async fn execute_sqlserver_bulk_rows_batch(
    ...
    ensure_import_write_allowed(import_id, is_cancelled, 0).await?;
    let staging_name = format!("#gauss_horizon_import_{}", uuid::Uuid::new_v4().simple());
...
mod tests {
    ...
    fn xlsx_named_spill_files() -> std::collections::HashSet<std::path::PathBuf> {
        std::fs::read_dir(std::env::temp_dir())
            ...
            .flatten()
            .flatten()
            .filter(|entry| entry.file_name().to_string_lossy().starts_with("gauss-horizon-xlsx-shared-"))
    ...
    fn prepared_import_source_is_reused_only_while_fingerprint_matches() {
        ...
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-prepared-{}.csv", uuid::Uuid::new_v4()));
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <cellXfs count="1"><xf numFmtId="0"/></cellXfs>
    ...
    fn assert_xlsx_absolute_row_selection(path_kind: &str) {
        for origin in [1, 3, 7] {
            for first_column in *b"AC" {
                ...
                let path =
                    std::env::temp_dir().join(format!("gauss-horizon-absolute-rows-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn xlsx_absolute_rows_preview_limit_excludes_leading_unused_rows() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-leading-unused-rows-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn xlsx_absolute_rows_preserve_header_and_value_styles() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-absolute-row-styles-{}.xlsx", uuid::Uuid::new_v4()));
        std::fs::write(
            &path,
    ...
    fn assert_xlsx_empty_string_option(options: TableImportParseOptions, expected_row: Vec<serde_json::Value>) {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-empty-string-option-{}.xlsx", uuid::Uuid::new_v4()));
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="1" uniqueCount="1">
  <si><t></t></si>
    ...
    fn xlsx_duplicate_headers_are_unique_in_preview_parse_and_streaming() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-duplicate-headers-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:B2"/>
  <sheetData>
    ...
    #[test]
    fn gauss_horizon_exported_xlsx_round_trip_preserves_empty_strings_when_configured() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-empty-round-trip-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn legacy_xls_rejects_numeric_to_text_without_affecting_numeric_targets() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-formatted-{}.xls", uuid::Uuid::new_v4()));
    ...
    async fn legacy_xls_preview_reads_sheet_names_without_zip_parser() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-preview-{}.xls", uuid::Uuid::new_v4()));
        std::fs::write(&path, include_bytes!("../tests/fixtures/issue3683-formatted-numbers.xls")).unwrap();
        let options = TableImportParseOptions { has_header: Some(false), ..TableImportParseOptions::default() };
    ...
    fn xlsx_style_rss_helper() {
        let Ok(sheet_path) = crate::legacy::var("GAUSS_HORIZON_XLSX_STYLE_RSS_PATH") else {
            return;
        };
        let ready_path = crate::legacy::var("GAUSS_HORIZON_XLSX_STYLE_RSS_READY").unwrap();
        let go_path = crate::legacy::var("GAUSS_HORIZON_XLSX_STYLE_RSS_GO").unwrap();
    ...
    fn streaming_xlsx_style_scan_keeps_peak_rss_bounded() {
        ...
        let sheet_path = std::env::temp_dir().join(format!("gauss-horizon-xlsx-style-rss-{suffix}.xml"));
        let ready_path = std::env::temp_dir().join(format!("gauss-horizon-xlsx-style-rss-{suffix}.ready"));
        let go_path = std::env::temp_dir().join(format!("gauss-horizon-xlsx-style-rss-{suffix}.go"));
        ...
        let mut child = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("GAUSS_HORIZON_XLSX_STYLE_RSS_PATH", &sheet_path)
            .env("GAUSS_HORIZON_XLSX_STYLE_RSS_READY", &ready_path)
            .env("GAUSS_HORIZON_XLSX_STYLE_RSS_GO", &go_path)
            .spawn()
            .unwrap();
        for _ in 0..10_000 {
    ...
    fn file_encoding_detection_validates_utf8_and_gbk_in_one_monotonic_scan() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-encoding-{}.csv", uuid::Uuid::new_v4()));
        let bytes = b"id,name\n1,\xD6\xD0\n";
    ...
    fn explicit_encoding_validation_rejects_invalid_tail_before_import() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-invalid-tail-{}.csv", uuid::Uuid::new_v4()));
    ...
    fn bom_encoding_detection_reports_the_file_as_fully_detected() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-bom-progress-{}.csv", uuid::Uuid::new_v4()));
    ...
    async fn preview_reads_real_gbk_file_and_reports_detected_encoding() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-table-import-gbk-{}.csv", uuid::Uuid::new_v4()));
    ...
    fn streams_delimited_rows_in_batches_and_preserves_selected_range() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-table-import-stream-{}.csv", uuid::Uuid::new_v4()));
    ...
    fn streaming_csv_uses_the_same_unique_duplicate_headers_as_preview() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-duplicate-stream-{}.csv", uuid::Uuid::new_v4()));
    ...
    fn legacy_xls_uses_a_stricter_non_streaming_file_limit() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-table-import-limit-{}.xls", uuid::Uuid::new_v4()));
    ...
    fn parses_selected_excel_sheet() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-table-import-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn fast_excel_preview_reads_only_requested_rows_and_reuses_sheet_metadata() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-preview-{}.xlsx", uuid::Uuid::new_v4()));
        let workbook = build_xlsx_workbook_multi(&[
            XlsxWorksheetData {
                sheet_name: Some("First".to_string()),
                columns: vec!["id".to_string()],
    ...
    fn xlsx_shared_strings_use_disk_index_above_memory_limit() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-shared-index-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:A2"/>
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="2" uniqueCount="2">
  <si><t>name</t></si><si><r><t>Ada</t></r><rPh sb="0" eb="3"><t>ignored</t></rPh></si>
    ...
    async fn xlsx_shared_strings_validation_reports_progress_and_cancels_before_header() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-shared-cancel-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:A2"/>
  <sheetData><row r="1"><c r="A1" t="s"><v>0</v></c></row><row r="2"><c r="A2" t="s"><v>1</v></c></row></sheetData>
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="8193" uniqueCount="8193">
  <si><t>name</t></si>{shared_items}
    ...
    fn xlsx_disk_shared_strings_do_not_leave_named_spill_files() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-shared-anonymous-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:A1"/>
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="1" uniqueCount="1">
  <si><t>sensitive-value</t></si>
    ...
    fn xlsx_disk_shared_strings_are_owner_only() {
        ...
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-shared-perms-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:A2"/>
  <sheetData><row r="1"><c r="A1" t="s"><v>0</v></c></row><row r="2"><c r="A2" t="s"><v>1</v></c></row></sheetData>
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="2" uniqueCount="2">
  <si><t>name</t></si><si><t>Ada</t></si>
    ...
    fn xlsx_disk_shared_strings_are_cleaned_up_after_parse_failure() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-shared-fail-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:A1"/>
  <sheetData><row r="1"><c r="A1" t="s"><v>0</v></c></row></sheetData>
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="2" uniqueCount="2">
  <si><t>name</t></si><si><t>Ada</t></si> <<< broken"#;
    ...
    fn streaming_excel_rows_avoid_materializing_the_full_range() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-stream-{}.xlsx", uuid::Uuid::new_v4()));
        let workbook = build_xlsx_workbook_multi(&[XlsxWorksheetData {
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    ...
    fn streaming_excel_rows_preserve_offset_ranges_and_temporal_styles() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-stream-offset-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn streaming_excel_rows_preserve_numeric_display_text_for_text_targets() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-stream-format-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn streaming_excel_rows_preserve_custom_title_and_data_range() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-stream-range-{}.xlsx", uuid::Uuid::new_v4()));
        let workbook = build_xlsx_workbook_multi(&[XlsxWorksheetData {
            sheet_name: Some("Rows".to_string()),
    ...
    fn streaming_excel_rows_reject_data_beyond_preview_columns() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-stream-wide-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:B2"/>
  <sheetData>
    ...
    fn streaming_excel_rows_accept_sparse_empty_cells() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-stream-sparse-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:C2"/>
  <sheetData>
    ...
    fn fast_excel_preview_matches_calamine_when_row_and_cell_references_are_omitted() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-preview-implicit-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:B2"/>
  <sheetData>
    ...
    fn fast_excel_preview_advances_past_empty_cells_with_implicit_references() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-preview-empty-cell-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:C2"/>
  <sheetData>
    ...
    fn fast_excel_preview_excludes_phonetic_runs_from_shared_and_inline_strings() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-preview-phonetic-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:B2"/>
  <sheetData>
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="2" uniqueCount="2">
  ...
  <si><r><t>東京</t></r><rPh sb="0" eb="2"><t>とうきょう</t></rPh></si>
    ...
    fn fast_excel_preview_ignores_stale_and_overwide_dimensions() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-preview-dimension-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <sheetData>
    ...
    fn excel_text_cell_with_thousands_separator_imports_to_postgres_numeric() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-table-import-6491-{}.xlsx", uuid::Uuid::new_v4()));
        let sheet_xml = r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <dimension ref="A1:A3"/>
  <sheetData>
    ...
    fn csv_import_keeps_values_that_only_look_like_the_force_text_wrapper() {
        // Formula-looking data that gauss-horizon did not write must survive untouched,
    ...
    fn formats_only_excel_columns_mapped_to_text_targets() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-display-formats-{}.xlsx", uuid::Uuid::new_v4()));
        std::fs::write(
            &path,
            build_styled_test_xlsx(
                false,
    ...
    fn mysql_varchar_import_uses_excel_numeric_display_text() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-number-format-{}.xlsx", uuid::Uuid::new_v4()));
        std::fs::write(
            &path,
    ...
    fn excel_zero_fraction_numbers_infer_integer_columns() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-integer-inference-{}.xlsx", uuid::Uuid::new_v4()));
        let workbook = build_xlsx_workbook_multi(&[XlsxWorksheetData {
    ...
    fn parses_excel_temporal_styles_before_type_inference() {
        let path =
            std::env::temp_dir().join(format!("gauss-horizon-table-import-temporal-{}.xlsx", uuid::Uuid::new_v4()));
        std::fs::write(
            &path,
    ...
    fn parses_excel_temporal_styles_with_1904_date_system() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-temporal-1904-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn parses_excel_temporal_styles_from_non_a1_used_range() {
        let path = std::env::temp_dir()
            .join(format!("gauss-horizon-table-import-temporal-offset-{}.xlsx", uuid::Uuid::new_v4()));
        std::fs::write(&path, build_styled_test_xlsx(false, &[("C3", 1, 45996.0), ("D3", 2, 45996.0)])).unwrap();
        let options = TableImportParseOptions { has_header: Some(false), ..TableImportParseOptions::default() };
    ...
    fn parses_excel_with_custom_title_and_data_rows() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-table-import-rows-{}.xlsx", uuid::Uuid::new_v4()));
        let workbook = build_xlsx_workbook_multi(&[XlsxWorksheetData {
            sheet_name: Some("Rows".to_string()),
            columns: vec!["report".to_string(), "ignored".to_string()],
            column_types: vec![],
    ...
    fn sqlserver_bulk_plan_uses_staging_conversions_and_identity_scope() {
        ...
        let sql = plan.batch_sql("#gauss_horizon_import_test", true);
    ...
    fn sqlserver_bulk_identity_truncate_turns_identity_insert_off_before_commit() {
        ...
        let sql = plan.batch_sql("#gauss_horizon_import_test", true).write_target;
    ...
    fn sqlserver_bulk_identity_append_commits_only_after_identity_insert_is_off() {
        ...
        let sql = plan.batch_sql("#gauss_horizon_import_test", false).write_target;
    ...
    fn sqlserver_bulk_plan_omits_unmapped_identity_and_default_columns() {
        ...
        let plan = compile_sqlserver_bulk_import_plan(&import_plan, &metadata, "events", "dbo").unwrap();
        let sql = plan.batch_sql("#gauss_horizon_import_test", false);
    ...
    async fn preview_missing_source_fails_before_parsing() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-missing-import-{}.csv", uuid::Uuid::new_v4()));
