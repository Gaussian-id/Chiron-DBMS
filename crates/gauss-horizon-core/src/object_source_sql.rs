...
fn mysql_routine_script_delimiter(source: &str) -> &'static str {
    ["//", "$$", ";;", "__GAUSS_HORIZON_DELIMITER__"]
        .into_iter()
        .find(|delimiter| !source.contains(delimiter))
        .unwrap_or("__GAUSS_HORIZON_DELIMITER__")
}
...
fn informix_validation_view_name(target_name: &str) -> String {
    ...
    format!("gauss_horizon_view_check_{hash:08x}")
}
...
mod tests {
    ...
    fn kingbase_view_body_opens_as_create_or_replace_view() {
        let sql = build_editable_object_source(EditableObjectSourceSqlInput {
            ...
            object_type: ObjectSourceKind::View,
            schema: Some("gauss_horizon_issue3895_repro".to_string()),
        ...
        assert_eq!(
            sql,
            "CREATE OR REPLACE VIEW \"gauss_horizon_issue3895_repro\".\"edited_view\" AS\nSELECT id, label FROM source_rows WHERE id >= 1;"
        );
    }
    ...
    fn kingbase_view_create_source_saves_as_create_or_replace_view() {
        let sql = build_executable_object_source_sql(EditableObjectSourceSqlInput {
            ...
            schema: Some("gauss_horizon_issue3895_repro".to_string()),
            name: "edited_view".to_string(),
            source: "CREATE VIEW gauss_horizon_issue3895_repro.edited_view AS SELECT id, label FROM source_rows"
                .to_string(),
        })
        .unwrap();
        ...
        assert_eq!(
            sql,
            "CREATE OR REPLACE VIEW gauss_horizon_issue3895_repro.edited_view AS SELECT id, label FROM source_rows;"
        );
    }
    ...
    fn oracle_view_body_saves_as_create_or_replace_view() {
        ...
        assert_eq!(
            sql,
            "CREATE OR REPLACE VIEW \"GAUSS_HORIZON_TEST\".\"V_ACTIVE_USERS\" AS\nSELECT id, name FROM users WHERE active = 1;"
        );
    }
    ...
    fn oracle_view_create_source_saves_as_create_or_replace_view() {
        let sql = build_executable_object_source_sql(EditableObjectSourceSqlInput {
            ...
            schema: Some("GAUSS_HORIZON_TEST".to_string()),
            name: "V_ACTIVE_USERS".to_string(),
            source: "CREATE FORCE EDITIONABLE VIEW GAUSS_HORIZON_TEST.V_ACTIVE_USERS AS SELECT id FROM users"
                .to_string(),
        })
        .unwrap();
        ...
        assert_eq!(
            sql,
            "CREATE OR REPLACE FORCE EDITIONABLE VIEW GAUSS_HORIZON_TEST.V_ACTIVE_USERS AS SELECT id FROM users;"
        );
    }
