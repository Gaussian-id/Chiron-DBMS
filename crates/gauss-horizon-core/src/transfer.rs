...
use crate::query::{
    agent_execute_query_params, is_gauss_horizon_query_timeout_error, pool_error_action, query_timeout_duration,
...
fn protect_sql_literals(sql: &str, double_quote_is_string: bool) -> (String, Vec<(String, String)>) {
    ...
    for (start, end) in spans {
        ...
        let placeholder = format!("__GAUSS_HORIZON_LIT_{}__", restores.len());
...
/// source columns. Gauss Horizon never alters an existing target table's columns, so
    ...
    if !missing.is_empty() {
        return Err(format!(
            "Target table '{target_table}' already exists with a different structure and is missing column(s) \
             {} present in the source table. Gauss Horizon does not alter an existing target table's columns during \
    ...
    if !required.is_empty() {
        return Err(format!(
            "Target table '{target_table}' already exists with a different structure and has required column(s) \
             {} that are not present in the source table and have no default or generated value. Gauss Horizon does not \
...
fn oracle_rownum_page_sql(col_list: &str, base_sql: String, offset: u64, limit: usize) -> String {
    ...
    format!(
        "SELECT {col_list} FROM (SELECT gauss_horizon_inner.*, ROWNUM AS \"__gauss_horizon_row_num\" FROM ({base_sql}) gauss_horizon_inner WHERE ROWNUM <= {end}) WHERE \"__gauss_horizon_row_num\" > {offset}"
    )
}
...
fn sqlserver_row_number_page_sql(
    ...
    let end = offset + limit as u64;
    format!(
        "SELECT {col_list} FROM (SELECT {col_list}, ROW_NUMBER() OVER (ORDER BY {order_by}) AS __gauss_horizon_row_num FROM {from_clause}) AS __gauss_horizon_page WHERE __gauss_horizon_row_num > {offset} AND __gauss_horizon_row_num <= {end}"
    )
}
...
fn postgres_index_column_sql(
    ...
    // opclass — see `crates/gauss-horizon-core/src/db/postgres.rs`), so quoting the whole thing as
...
fn generate_mysql_foreign_key_alter_statements(
    ...
    // fallback (crates/gauss-horizon-core/src/schema.rs), which is private to that module.
...
fn is_transfer_query_timeout(error: &str) -> bool {
    let lower = error.to_ascii_lowercase();
    is_gauss_horizon_query_timeout_error(&lower) || lower.contains("查询超时") || lower.contains("查詢逾時")
}
    ...
    F: FnMut(TransferProgress),
{
    ...
    // The user asked Gauss Horizon to sync structure (create_table), but the target
...
mod tests {
    ...
    fn gbase_8s_jdbc_stays_generic_for_transfer() {
        assert_eq!(
            effective_transfer_database_type(&jdbc_transfer_config(
                "jdbc:gbasedbt-sqli://localhost:9088/gauss_horizon_test:INFORMIXSERVER=ol_gbasedbt",
        ...
        assert_eq!(
            effective_transfer_database_type(&jdbc_transfer_config(
                "jdbc:gbase://localhost:5258/gauss_horizon_test",
                "cn.gbase.Driver",
                "gbase8s"
            )),
            DatabaseType::Jdbc
    ...
    async fn test_app_state() -> (AppState, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-transfer-test-{}", uuid::Uuid::new_v4()));
    ...
    fn postgres_transfer_ddl_preserves_constraint_after_inline_foreign_key() {
        let ddl = "CREATE TABLE \"source_7931\".\"gauss_horizon_child\" (\n  \"id\" integer NOT NULL,\n  \"parent_id\" integer NOT NULL,\n  \"status\" integer NOT NULL,\n  CONSTRAINT \"gauss_horizon_child_parent_fk\" FOREIGN KEY (\"parent_id\") REFERENCES \"source_7931\".\"gauss_horizon_parent\"(\"id\"),\n  CONSTRAINT \"gauss_horizon_child_status_check\" CHECK (status >= 0)\n);";

        let statements = transfer_ddl_statements(ddl, &DatabaseType::Postgres);

        assert_eq!(
            statements,
            vec!["CREATE TABLE \"source_7931\".\"gauss_horizon_child\" (\n  \"id\" integer NOT NULL,\n  \"parent_id\" integer NOT NULL,\n  \"status\" integer NOT NULL,\n  CONSTRAINT \"gauss_horizon_child_status_check\" CHECK (status >= 0)\n)".to_string()]
        );
    }
...
/* COLLATE utf8mb4_comment_ci */"#;
  ...
  PRIMARY KEY (`id`)
    ...
    fn sqlserver_export_pagination_uses_row_number_subquery() {
        ...
        assert_eq!(
            sql,
            "SELECT [id], [name] FROM (SELECT [id], [name], ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS __gauss_horizon_row_num FROM [dbo].[users]) AS __gauss_horizon_page WHERE __gauss_horizon_row_num > 500 AND __gauss_horizon_row_num <= 600"
        );
        assert!(!sql.contains("OFFSET"));
    ...
    fn sqlserver_ordered_pagination_uses_row_number_subquery() {
        ...
        assert!(sql.contains("ROW_NUMBER() OVER (ORDER BY [id]) AS __gauss_horizon_row_num"));
        assert!(sql.contains("WHERE __gauss_horizon_row_num > 200 AND __gauss_horizon_row_num <= 300"));
        assert!(!sql.contains("OFFSET"));
        assert!(!sql.contains(" FETCH "));
    }
    ...
    fn sqlserver_filtered_pagination_preserves_filter_in_subquery() {
        ...
        assert!(sql.contains("ROW_NUMBER() OVER (ORDER BY [id] DESC) AS __gauss_horizon_row_num"));
        assert!(sql.contains("FROM [dbo].[users] WHERE (status = 'active')"));
        assert!(sql.contains("WHERE __gauss_horizon_row_num > 10000 AND __gauss_horizon_row_num <= 12000"));
        assert!(!sql.contains("OFFSET"));
        assert!(!sql.contains(" FETCH "));
    }
    ...
    fn oracle_filtered_pagination_uses_rownum_for_legacy_compatibility() {
        ...
        assert_eq!(
            sql,
            "SELECT \"id\", \"status\" FROM (SELECT gauss_horizon_inner.*, ROWNUM AS \"__gauss_horizon_row_num\" FROM (SELECT \"id\", \"status\" FROM \"APP\".\"users\" WHERE (status = 'active') ORDER BY \"id\" DESC) gauss_horizon_inner WHERE ROWNUM <= 12000) WHERE \"__gauss_horizon_row_num\" > 10000"
        );
        assert!(!sql.contains("OFFSET"));
        ...
        let sql = generate_postgres_sequence_sync_sql(
            &[
                db::ColumnInfo {
                    ...
                    is_nullable: false,
                    column_default: Some("nextval('public.gauss_horizon_text_id_seq'::regclass)::text".to_string()),
                    is_primary_key: true,
                    extra: None,
                    comment: None,
                    numeric_precision: None,
                ...
                db::ColumnInfo {
                    ...
                    is_nullable: false,
                    column_default: Some("nextval('public.gauss_horizon_numeric_id_seq'::regclass)".to_string()),
                    is_primary_key: false,
                    extra: None,
                    comment: None,
                    numeric_precision: None,
    ...
    fn dameng_insert_formats_bit_booleans_as_numeric_literals() {
        ...
        assert_eq!(
            sql,
            "INSERT INTO \"GAUSS_HORIZON_TEST\".\"flags\" (\"enabled\", \"deleted\", \"optional\") VALUES\n(1, 0, NULL)"
        );
...
SELECT 1 FROM dual"#
    ...
    fn non_oracle_jdbc_url_keeps_multi_row_values_insert() {
        let config =
            jdbc_transfer_config("jdbc:mysql://localhost:3306/gauss_horizon_test", "com.mysql.cj.jdbc.Driver", "");
    ...
    fn mysql_spatial_transfer_reuses_validated_wkb_markers_for_all_modes() {
        ...
        let rows = [vec![
            json!(1),
            json!("GAUSS_HORIZON_WKB:4326:0101000000000000000000F03F0000000000000040"),
            json!("alpha"),
        ]];
    ...
    fn mysql_spatial_transfer_rejects_invalid_markers_and_keeps_public_insert_shape() {
        let invalid = json!("GAUSS_HORIZON_WKB:4326:0101000000");
    ...
    fn mysql_spatial_transfer_projection_is_native_mysql_to_mysql_only() {
        ...
        assert!(
            native_sql.contains("CONCAT('GAUSS_HORIZON_WKB:', ST_SRID(`location`), ':', HEX(ST_AsWKB(`location`)))")
        );
    ...
    fn resolve_external_transfer_catalog_for_config_accepts_starrocks_driver_profile() {
        ...
        gbase_jdbc.connection_string = Some("jdbc:gbase://127.0.0.1:5258/gauss_horizon_test".to_string());
        ...
        gbase_jdbc.connection_string = Some("jdbc:unknown://127.0.0.1:1234/gauss_horizon_test".to_string());
