...
/// Marker that identifies a table as a Gauss Horizon transfer backup.
pub const BACKUP_TABLE_MARKER: &str = "__gauss_horizon_bak_";
...
mod tests {
    ...
    fn backup_name_uses_the_full_budget_boundaries() {
        ...
        assert!(short.starts_with("orders__gauss_horizon_bak_"), "unexpected short name: {short}");
    }
    ...
    fn failure_messages_point_at_the_retained_backup() {
        let annotated = annotate_error_with_retained_backup(
            ...
            "`shop`.`orders__gauss_horizon_bak_1a2b3c4d`",
        );
        assert!(annotated.starts_with("Failed to insert batch: duplicate key."), "original error must lead");
        assert!(
            annotated.contains("`shop`.`orders__gauss_horizon_bak_1a2b3c4d`"),
    ...
    fn a_message_that_already_names_the_backup_is_left_alone() {
        ...
        let original = "Transfer completed successfully, but failed to drop backup table \
                        'orders__gauss_horizon_bak_1a2b3c4d': permission denied."
            .to_string();
        assert_eq!(
            annotate_error_with_retained_backup(original.clone(), "orders__gauss_horizon_bak_1a2b3c4d"),
            original
        );
    }
