...
pub use installer::{
    ...
    PluginRollbackResult, PluginSignatureStatus, PluginTrustStore, PluginTrustedKey, GAUSS_HORIZON_PLUGIN_EXTENSION,
...
mod tests {
    ...
    async fn shutdown_kills_plugin_child_process() {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-plugin-shutdown-test-{}", uuid::Uuid::new_v4()));
    ...
    fn registry_lists_frontend_contributions_from_disk() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-plugin-registry-test-{}", uuid::Uuid::new_v4()));
    ...
    fn registry_keeps_other_plugins_visible_when_one_manifest_is_corrupt() {
        let root =
            std::env::temp_dir().join(format!("gauss-horizon-plugin-corrupt-registry-test-{}", uuid::Uuid::new_v4()));
