    ...
    async fn initialize(&self) -> Result<PluginHandshake, String> {
        let params = PluginInitializeParams {
            host: PluginHostDescription {
                gauss_horizon_version: &self.app_version,
...
fn spawn_plugin_child(plugin: &InstalledPlugin, app_version: &str, env: &PluginRuntimeEnv) -> Result<Child, String> {
    ...
    command
        ...
        .stderr(Stdio::piped())
        .kill_on_drop(true)
        .env("GAUSS_HORIZON_PLUGIN_ID", &plugin.manifest.id)
        .env("GAUSS_HORIZON_PLUGIN_VERSION", &plugin.manifest.version)
        .env("GAUSS_HORIZON_APP_VERSION", app_version)
        .env("GAUSS_HORIZON_HOST_API_VERSION", SUPPORTED_PLUGIN_HOST_API_VERSION)
        .env("GAUSS_HORIZON_PLUGIN_PROTOCOL_VERSION", SUPPORTED_PLUGIN_PROTOCOL_VERSION.to_string());
