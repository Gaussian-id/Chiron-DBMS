...
pub const GAUSS_HORIZON_PLUGIN_EXTENSION: &str = "gauss-horizonp";
...
impl PluginPackageInstaller {
    ...
    pub fn install_file(
        ...
        if package_path.extension().and_then(|extension| extension.to_str()) != Some(GAUSS_HORIZON_PLUGIN_EXTENSION) {
            return Err(format!("Plugin package must use the .{GAUSS_HORIZON_PLUGIN_EXTENSION} extension"));
        }
    ...
    fn install_bytes_locked(
        ...
        if manifest.manifest_version == 0 {
            return Err(".gauss-horizonp packages must use plugin manifest version 1 or newer".to_string());
        }
...
fn extract_package(package: &[u8], destination: &Path) -> Result<HashSet<String>, String> {
    let mut archive = zip::ZipArchive::new(Cursor::new(package))
        .map_err(|error| format!("Invalid .gauss-horizonp archive: {error}"))?;
