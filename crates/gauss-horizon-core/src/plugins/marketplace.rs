...
pub const OFFICIAL_PLUGIN_REPOSITORY_ID: &str = "gauss-horizon-official";
...
const OFFICIAL_CATALOG_URL: &str = "https://distribution-disabled.invalid/main/catalog/index.json";
const ADDITIONAL_OFFICIAL_TRUSTED_KEYS_JSON: Option<&str> =
    option_env!("GAUSS_HORIZON_PLUGIN_MARKETPLACE_TRUSTED_KEYS_JSON");
const BUILTIN_OFFICIAL_TRUSTED_KEYS: &[(&str, &str)] = &[
    ("gauss-horizon-store-preview-2026", "VRb0VscZfWwuFa7LYfeD/wEOJeyNP8wPGND9br8Icmk="),
    ("gauss-horizon-store-release-2026", "6WbMG2UDx+EZ/oauMtHjdinvSD5MuFWSgXbI0n7eL+k="),
];
...
impl PluginMarketplace {
    pub fn new(root_dir: PathBuf, app_version: impl Into<String>) -> Result<Self, String> {
        let client = Client::builder()
            ...
            .user_agent(format!("Gauss Horizon/{}/plugin-marketplace", env!("CARGO_PKG_VERSION")))
            .build()
    ...
    async fn download_limited(&self, url: Url, max_bytes: usize, label: &str) -> Result<Vec<u8>, String> {
        if url.host_str() == Some("distribution-disabled.invalid") {
...
fn official_repository() -> PluginRepository {
    PluginRepository {
        ...
        name: "Gauss Horizon Marketplace".to_string(),
...
fn repository_catalog_url(repository: &PluginRepository) -> Result<Url, String> {
    let raw = repository.catalog_url.as_deref().and_then(trimmed_nonempty).ok_or_else(|| {
        if repository.kind == PluginRepositoryKind::Official {
            "Official Gauss Horizon Marketplace catalog is not configured in this build".to_string()
        } else {
...
fn validate_and_resolve_catalog(
    ...
    if catalog.catalog_version != SUPPORTED_PLUGIN_CATALOG_VERSION {
        return Err(format!(
            "Unsupported plugin catalog version {}; this Gauss Horizon build supports version {}",
...
fn marketplace_trust_store(root_dir: &Path, kind: PluginRepositoryKind) -> Result<PluginTrustStore, String> {
    ...
    if let Some(raw) = ADDITIONAL_OFFICIAL_TRUSTED_KEYS_JSON.and_then(trimmed_nonempty) {
        let additional: BTreeMap<String, String> = serde_json::from_str(raw).map_err(|error| {
            format!("Failed to parse additional official Gauss Horizon Marketplace signing keys: {error}")
        })?;
        for (key_id, public_key) in additional {
            if keys.insert(key_id.clone(), public_key).is_some() {
                return Err(format!("Duplicate official Gauss Horizon Marketplace signing key '{key_id}'"));
            }
        }
    }
    let store = PluginTrustStore::from_base64_keys(keys)?;
    if store.is_empty() {
        return Err("Official Gauss Horizon Marketplace signing keys are empty".to_string());
    }
    Ok(store)
}
...
mod tests {
            ...
            plugins: vec![PluginMarketplacePlugin {
                ...
                versions: vec![PluginMarketplaceVersion {
                    ...
                    artifacts: vec![PluginMarketplaceArtifact {
                        target: current_plugin_target(),
                        url: "../dist/plugin.gauss-horizonp".to_string(),
                        sha256: "a".repeat(64),
    ...
    fn validates_catalog_and_resolves_relative_urls() {
        ...
        assert_eq!(
            ...
            "https://plugins.example.com/team-marketplace/dist/plugin.gauss-horizonp"
        );
    }
        ...
        let version = PluginMarketplaceVersion {
            ...
            artifacts: vec![
                PluginMarketplaceArtifact {
                    ...
                    url: "https://plugins.example.com/universal.gauss-horizonp".to_string(),
                    sha256: "a".repeat(64),
                    signing_key_id: "example.release".to_string(),
                    size: None,
                },
                PluginMarketplaceArtifact {
                    ...
                    url: "https://plugins.example.com/darwin-arm64.gauss-horizonp".to_string(),
                    sha256: "b".repeat(64),
                    signing_key_id: "example.release".to_string(),
                    size: None,
                },
    ...
    fn verifies_artifact_size_and_sha256() {
        ...
        let artifact = PluginMarketplaceArtifact {
            target: current_plugin_target(),
            url: "https://plugins.example.com/plugin.gauss-horizonp".to_string(),
        ...
        let catalog = PluginMarketplaceCatalog {
            ...
            plugins: vec![PluginMarketplacePlugin {
                ...
                versions: vec![PluginMarketplaceVersion {
                    ...
                    artifacts: vec![PluginMarketplaceArtifact {
                        target: UNIVERSAL_PLUGIN_TARGET.to_string(),
                        url: "package.gauss-horizonp".to_string(),
    ...
    fn signed_package(signing_key: &SigningKey, key_id: &str) -> Vec<u8> {
        let manifest = serde_json::to_vec_pretty(&serde_json::json!({
            ...
            "version": "1.0.0",
            "publisher": "example",
            "engines": { "gauss-horizon": ">=0.5.0", "host_api": "^1.0" },
            "entrypoints": {
                "backend": {
                    "protocol_versions": [1],
