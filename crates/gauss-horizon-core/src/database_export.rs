...
/// Database exports encode MySQL spatial cells as `GAUSS_HORIZON_WKB:<srid>:<hex>` while
...
pub(crate) fn format_mysql_spatial_export_literal(
    ...
    let marker = value.strip_prefix("GAUSS_HORIZON_WKB:")?;
...
fn format_mysql_binary_export_literal_text(text: &str) -> Option<String> {
    ...
    if hex.chars().all(|ch| ch.is_ascii_hexdigit()) {
        // Gauss Horizon exposes MySQL binary cells as 0x-prefixed hex text. Keep it as a
...
pub(crate) fn is_internal_export_column(database_type: Option<DatabaseType>, column: &str) -> bool {
    ...
    crate::sql_dialect::uses_synthetic_row_id(database_type)
        && column.eq_ignore_ascii_case(crate::sql_dialect::GAUSS_HORIZON_ROWID_COLUMN)
}
...
pub fn build_database_sql_export(options: BuildDatabaseSqlExportOptions) -> Result<String, String> {
    ...
    let mut lines = vec![
        "-- Gauss Horizon database export".to_string(),
...
fn mysql_spatial_export_marker_expression(column: &str) -> String {
    ...
    format!(
        "CASE WHEN {quoted} IS NULL THEN NULL ELSE CONCAT('GAUSS_HORIZON_WKB:', ST_SRID({quoted}), ':', HEX(ST_AsWKB({quoted}))) END AS {quoted}"
    )
}
...
const EXPORT_DESTINATION_IDENTITY_MAGIC: &[u8; 5] = b"CDBEI";
...
impl ExportDestinationIdentity {
    ...
    fn decode(encoded: &[u8]) -> Result<Option<Self>, String> {
        ...
        if &header[..EXPORT_DESTINATION_IDENTITY_MAGIC.len()] != EXPORT_DESTINATION_IDENTITY_MAGIC
            && &header[..EXPORT_DESTINATION_IDENTITY_MAGIC.len()] != crate::legacy::OLD_EXPORT_IDENTITY_MAGIC
        {
...
/// `st_dev` identity written by Gauss Horizon versions before persistent volume UUIDs
...
async fn ensure_recorded_export_destination_ancestor(
    ...
    while let Some(candidate) = ancestor {
        ...
        if let Some(recorded_identity) = recorded_identity {
            if !candidate.is_dir() {
                return Err(format!(
                    "Backup directory {} is missing. Its parent {} was configured or previously used for exports, so gauss-horizon will not recreate a child directory automatically -- if this is on a removable or network drive, reconnect it and try again.",
                    dir.display(),
                    candidate.display()
                ));
            }
        ...
        if recorded_identity.is_some() {
            return Err(format!(
                "Backup directory {} is missing. It was configured or previously used for exports to \
                 this location, so gauss-horizon will not recreate it automatically -- if this is on a removable \
...
/// and then again against the file gauss-horizon actually opened, indicates the mount
...
async fn export_postgres_all_schemas_sql_core(
    ...
    let result = async {
        ...
        writeln!(file, "-- Generated by Gauss Horizon").map_err(|e| format!("Failed to write file: {e}"))?;
...
mod tests {
    ...
    fn mysql_spatial_export_uses_wkb_constructor_and_preserves_srid() {
        let statements = build_export_insert_statements(BuildExportInsertStatementsOptions {
            ...
            spatial_values: Vec::new(),
            rows: vec![vec![
                json!("GAUSS_HORIZON_WKB:4326:0101000000AE47E17A14AE5C4052B81E85EBF34240"),
                json!("GAUSS_HORIZON_WKB:0:0101000000000000000000F03F0000000000000040"),
            ]],
            batch_size: Some(10),
        })
        .unwrap();
    ...
    fn mysql_spatial_export_rejects_markers_for_unknown_or_nonspatial_types() {
        let marker = json!("GAUSS_HORIZON_WKB:4326:0101000000AE47E17A14AE5C4052B81E85EBF34240");
        ...
        assert!(format_mysql_spatial_export_literal(
            &json!("GAUSS_HORIZON_WKB:4326:0101000000"),
    ...
    fn mysql_spatial_export_select_normalizes_geometry_columns_to_wkb_markers() {
        ...
        assert_eq!(
            sql,
            "SELECT `id`, CASE WHEN `location` IS NULL THEN NULL ELSE CONCAT('GAUSS_HORIZON_WKB:', ST_SRID(`location`), ':', HEX(ST_AsWKB(`location`))) END AS `location`, `name` FROM `places`"
        );
    }
    ...
    fn mysql_spatial_export_keeps_keyset_pagination_when_replacing_select_list() {
        ...
        assert!(sql.starts_with(
            "SELECT `id`, CASE WHEN `location` IS NULL THEN NULL ELSE CONCAT('GAUSS_HORIZON_WKB:', ST_SRID(`location`), ':', HEX(ST_AsWKB(`location`))) END AS `location` FROM `places`"
        ));
    ...
    #[test]
    fn postgres_bytea_export_decodes_valid_gauss_horizon_hex_values() {
    ...
    fn opengauss_export_inserts_escape_quotes_without_doubling_backslashes() {
        ...
        let statements = build_export_insert_statements(BuildExportInsertStatementsOptions {
            ...
            identifier_quote: None,
            schema: Some("public".to_string()),
            table_name: Some("gauss_horizon_issue_json".to_string()),
            qualified_table_name: None,
        ...
        assert_eq!(
            statements,
            vec![format!(
                "INSERT INTO \"public\".\"gauss_horizon_issue_json\" (\"comment_text\", \"style\") VALUES ('逻辑删除标志：''0''-未删除，''1''-已删除', {expected_style});"
            )]
        );
    }
    ...
    fn postgres_export_keeps_json_escape_sequences_for_the_same_value() {
        ...
        assert_eq!(
            statements,
            vec![format!("INSERT INTO \"public\".\"gauss_horizon_issue_json\" (\"style\") VALUES ({expected_style});")]
        );
    }
    ...
    fn oracle_export_omits_synthetic_rowid_from_insert_columns() {
        let statements = build_export_insert_statements(BuildExportInsertStatementsOptions {
            ...
            table_name: Some("USERS".to_string()),
            qualified_table_name: None,
            columns: vec!["__GAUSS_HORIZON_ROWID".to_string(), "ID".to_string(), "NAME".to_string()],
    ...
    #[test]
    fn non_oracle_export_preserves_gauss_horizon_rowid_named_column() {
        let statements = build_export_insert_statements(BuildExportInsertStatementsOptions {
            database_type: Some(DatabaseType::Mysql),
            identifier_quote: None,
            schema: None,
            table_name: Some("users".to_string()),
            qualified_table_name: None,
            columns: vec!["__GAUSS_HORIZON_ROWID".to_string(), "name".to_string()],
            column_types: Vec::new(),
            column_extras: Vec::new(),
            spatial_columns: Vec::new(),
            spatial_values: Vec::new(),
        ...
        assert_eq!(statements, vec!["INSERT INTO `users` (`__GAUSS_HORIZON_ROWID`, `name`) VALUES (7, 'Ada');"]);
    }
    ...
    fn dameng_bit_columns_export_as_numeric_literals() {
        ...
        assert_eq!(
            statements,
            vec!["INSERT INTO \"GAUSS_HORIZON_TEST\".\"FLAGS\" (\"ENABLED\", \"DELETED\", \"OPTIONAL\") VALUES (1, 0, NULL);"]
        );
    }
    ...
    fn dameng_strings_export_nul_as_chr_expression() {
        ...
        assert_eq!(
            statements,
            vec![concat!(
                "INSERT INTO \"GAUSS_HORIZON_TEST\".\"NUL_VALUES\" ",
    ...
    fn opengauss_export_escapes_single_quotes_in_comment_ddl() {
        let ddl = concat!(
            "CREATE TABLE \"public\".\"gauss_horizon_issue_comment\" (\"flag\" varchar(8));\n",
            "COMMENT ON COLUMN \"public\".\"gauss_horizon_issue_comment\".\"flag\" IS '逻辑删除标志：'0'-未删除，'1'-已删除';"
        );
    ...
    fn strict_exports_return_object_errors_instead_of_writing_error_comments() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-strict-export-{}.sql", uuid::Uuid::new_v4()));
        let mut file = std::fs::File::create(&path).unwrap();
    ...
    fn lenient_export_errors_track_count_and_first_failure_for_terminal_progress() {
        let path = std::env::temp_dir().join(format!("gauss-horizon-lenient-export-{}.sql", uuid::Uuid::new_v4()));
        let mut file = std::fs::File::create(&path).unwrap();
    ...
    async fn ensure_export_destination_dir_creates_a_never_before_seen_local_directory() {
        let scratch = std::env::temp_dir().join(format!("gauss-horizon-export-dest-new-{}", uuid::Uuid::new_v4()));
    ...
    async fn ensure_export_destination_dir_refuses_to_recreate_a_destination_that_disappeared() {
        let scratch = std::env::temp_dir().join(format!("gauss-horizon-export-dest-vanished-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
        let state = test_app_state(&scratch).await;
    ...
    async fn record_export_destination_identity_protects_a_mount_that_vanishes_before_its_first_run() {
        let scratch = std::env::temp_dir().join(format!("gauss-horizon-export-dest-eager-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
        let state = test_app_state(&scratch).await;
    ...
    async fn ensure_export_destination_dir_refuses_a_new_child_when_its_recorded_parent_is_missing() {
        let scratch = std::env::temp_dir().join(format!("gauss-horizon-export-dest-child-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
        let state = test_app_state(&scratch).await;
        ...
        std::fs::remove_dir_all(scratch.join("mounted-drive")).unwrap();
        let run_directory = destination.join("gauss-horizon-backup__nightly__20260908-220000__12345678");
    ...
    async fn record_export_destination_identity_rejects_a_directory_that_disappeared_before_save() {
        let scratch =
            std::env::temp_dir().join(format!("gauss-horizon-export-dest-eager-new-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
        let state = test_app_state(&scratch).await;
    ...
    async fn ensure_export_destination_dir_refuses_a_changed_recorded_identity() {
        let scratch = std::env::temp_dir().join(format!("gauss-horizon-export-dest-changed-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
        let state = test_app_state(&scratch).await;
    ...
    async fn explicitly_recording_a_macos_destination_replaces_legacy_device_state() {
        ...
        let scratch =
            std::env::temp_dir().join(format!("gauss-horizon-export-dest-macos-migration-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
        let state = test_app_state(&scratch).await;
        let destination = scratch.join("backups");
        std::fs::create_dir_all(&destination).unwrap();
    ...
    fn export_destination_identity_for_path_and_open_file_agree_on_macos() {
        let scratch = std::env::temp_dir().join(format!("gauss-horizon-export-dest-macos-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
    ...
    fn export_destination_device_id_for_path_and_open_file_agree_on_windows() {
        let scratch = std::env::temp_dir().join(format!("gauss-horizon-export-dest-win-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&scratch).unwrap();
