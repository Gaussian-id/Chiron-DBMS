...
use crate::ai_cli_agent::{
    ...
    CliAgentCommandSpec, CliAgentRunOptions,
};
...
const PI_PRIVATE_ENV_PREFIX: &str = "GAUSS_HORIZON_PI_";
#[cfg(not(windows))]
const PI_SHELL_PATH_MARKER: &str = "__GAUSS_HORIZON_PI_SHELL_PATH__";
...
impl PiIsolatedRuntime {
    fn create() -> Result<Self, String> {
        let path = env::temp_dir().join(format!("gauss-horizon-pi-agent-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir(&path)
            .map_err(|error| format!("[piAgentRunFailed] Failed to create isolated Pi directory: {error}"))?;
        let extension_path = path.join("gauss-horizon-mcp-bridge.mjs");
        let ready_path = path.join("gauss-horizon-mcp-ready");
...
impl PiRpcProcess {
    ...
    async fn request(&mut self, command_type: &str, data: Value) -> Result<(Value, Vec<Value>), String> {
        let id = format!("gauss-horizon-{}", self.next_id);
        self.next_id += 1;
...
fn pi_agent_process_env_with_paths(
    ...
    for (key, value) in &config.pi_agent_cli_env {
        ...
        if upper.starts_with("GAUSS_HORIZON_MCP_") || upper.starts_with(PI_PRIVATE_ENV_PREFIX) {
            return Err(format!(
                "[piAgentEnvReserved] `{key}` is managed by Gauss Horizon for the scoped MCP bridge and cannot be set here."
            ));
        }
...
fn classify_pi_run_error(message: &str) -> String {
    ...
    } else if lower.contains("gauss-horizon mcp") || lower.contains("gauss-horizon-mcp") {
...
fn configure_pi_bridge(
    ...
    let mcp = options.mcp_server_command.as_ref().ok_or_else(|| {
        "[gaussHorizonMcpMissing] Gauss Horizon MCP server was not resolved for Pi Coding Agent".to_string()
    })?;
    process.env("GAUSS_HORIZON_PI_MCP_PROGRAM", &mcp.program);
    process.env(
        "GAUSS_HORIZON_PI_MCP_ARGS",
        serde_json::to_string(&mcp.args).map_err(|error| format!("[piAgentRunFailed] {error}"))?,
    );
    process.env(
        "GAUSS_HORIZON_PI_ENABLED_TOOLS",
        serde_json::to_string(&gauss_horizon_mcp_enabled_tools(options.agent_mode))
            .map_err(|error| format!("[piAgentRunFailed] {error}"))?,
    );
    process.env("GAUSS_HORIZON_PI_BRIDGE_READY_FILE", &runtime.ready_path);
...
async fn wait_for_bridge(process: &mut PiRpcProcess, runtime: &PiIsolatedRuntime) -> Result<(), String> {
        ...
        loop {
            ...
            if let Some(status) = process.child.try_wait().map_err(|error| classify_pi_run_error(&error.to_string()))? {
                ...
                let message = if stderr.is_empty() {
                    format!("Pi exited before the Gauss Horizon MCP bridge started: {status}")
                } else {
                    stderr
                };
                return Err(classify_pi_run_error(&message));
    ...
    .map_err(|_| {
        let stderr = process.stderr_text();
        classify_pi_run_error(if stderr.is_empty() { "Gauss Horizon MCP bridge startup timed out" } else { &stderr })
    })?
}
...
mod tests {
    ...
    #[test]
    fn rejects_gauss_horizon_managed_pi_environment_variables() {
        let mut config = config();
        config.pi_agent_cli_env.insert("GAUSS_HORIZON_PI_ENABLED_TOOLS".to_string(), "[]".to_string());
    ...
    fn bridge_reuses_gauss_horizon_mcp_scope_and_write_policy() {
        ...
        let options = CliAgentRunOptions {
            ...
            mcp_server_command: Some(CliAgentCommandSpec {
                program: "/usr/local/bin/gauss-horizon-mcp-server".to_string(),
        ...
        let env = process
            ...
            .collect::<HashMap<_, _>>();
        assert_eq!(
            env.get("GAUSS_HORIZON_PI_MCP_PROGRAM").map(String::as_str),
            Some("/usr/local/bin/gauss-horizon-mcp-server")
        );
        assert_eq!(env.get("GAUSS_HORIZON_PI_MCP_ARGS").map(String::as_str), Some("[\"--stdio\"]"));
        assert_eq!(env.get("GAUSS_HORIZON_MCP_ALLOW_WRITES").map(String::as_str), Some("1"));
        assert_eq!(env.get("GAUSS_HORIZON_MCP_ALLOW_DANGEROUS_SQL").map(String::as_str), Some("0"));
        assert_eq!(env.get("GAUSS_HORIZON_MCP_SCOPE_CONNECTION_ID").map(String::as_str), Some("connection-1"));
        assert_eq!(env.get("GAUSS_HORIZON_MCP_SCOPE_CONNECTION_NAME").map(String::as_str), Some("Test connection"));
        assert_eq!(env.get("GAUSS_HORIZON_MCP_SCOPE_DATABASE").map(String::as_str), Some("gauss_horizon_test"));
        assert_eq!(env.get("GAUSS_HORIZON_MCP_SCOPE_SCHEMA").map(String::as_str), Some("reporting"));

        let enabled_tools =
            serde_json::from_str::<Vec<String>>(env.get("GAUSS_HORIZON_PI_ENABLED_TOOLS").unwrap()).unwrap();
        assert!(enabled_tools.iter().any(|tool| tool == "gauss_horizon_execute_query"));
        assert!(enabled_tools.iter().any(|tool| tool == "gauss_horizon_execute_redis_command"));
    }
    ...
    #[test]
    fn maps_pi_stream_events_to_gauss_horizon_agent_events_and_usage() {
