...
mod tests {
    ...
    fn table_options() -> DataGridSaveStatementOptions {
        DataGridSaveStatementOptions {
            ...
            table_meta: DataGridTableMeta {
                catalog: None,
                database: None,
                schema: Some("gauss_horizon_edit_preview".to_string()),
    ...
    fn prepares_iotdb_timestamp_field_overwrite_and_rollback() {
        ...
        assert_eq!(
            result.statements,
            vec!["INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time) VALUES (1786954706123, 'sensor-a', '2026-08-17 17:18:27.123');"]
        );
        assert_eq!(
            result.rollback_statements,
            vec!["INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time) VALUES (1786954706123, 'sensor-a', 1786954706123);"]
        );
    }
    ...
    fn prepares_iotdb_only_with_changed_fields() {
        ...
        assert_eq!(
            result.statements,
            vec!["INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time, temperature) VALUES (1786954706123, 'sensor-a', 1786958307123, 24.75);"]
        );
        assert_eq!(
            result.rollback_statements,
            vec!["INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time, temperature) VALUES (1786954706123, 'sensor-a', 1786954706123, 23.5);"]
        );
    }
    ...
    fn prepares_iotdb_insert_delete_and_rollback() {
        ...
        assert_eq!(
            result.statements,
            vec![
                "DELETE FROM gauss_horizon_edit_preview.events WHERE time = 1786954706123 AND device = 'sensor-a';",
                "INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time, temperature) VALUES (1786961906789, 'sensor-b', 1786961906789, 22.75);",
            ]
        );
        assert_eq!(
            result.rollback_statements,
            vec![
                "DELETE FROM gauss_horizon_edit_preview.events WHERE time = 1786961906789 AND device = 'sensor-b';",
                "INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time, temperature) VALUES (1786954706123, 'sensor-a', 1786954706123, 23.5);",
            ]
        );
    }
    ...
    fn restores_iotdb_original_null_with_delete_and_full_row_insert() {
        ...
        assert_eq!(
            result.statements,
            vec!["INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time) VALUES (1786954706123, 'sensor-a', 1786958307123);"]
        );
        assert_eq!(
            result.rollback_statements,
            vec![
                "DELETE FROM gauss_horizon_edit_preview.events WHERE time = 1786954706123 AND device = 'sensor-a';",
                "INSERT INTO gauss_horizon_edit_preview.events (time, device, temperature) VALUES (1786954706123, 'sensor-a', 23.5);",
            ]
        );
    }
    ...
    fn preserves_timestamp_precision_strings_as_unquoted_integer_literals() {
        for (original, changed) in [
            ...
            assert_eq!(
                result.statements,
                vec![format!(
                    "INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time) VALUES ({original}, 'sensor-a', {changed});"
                )]
            );
            assert_eq!(
                result.rollback_statements,
                vec![format!(
                    "INSERT INTO gauss_horizon_edit_preview.events (time, device, event_time) VALUES ({original}, 'sensor-a', {original});"
                )]
            );
        }
    }
    ...
    fn keeps_iotdb_tree_model_on_the_existing_sql_path() {
        ...
        options.table_meta.schema = Some("root.gauss-horizon".to_string());
