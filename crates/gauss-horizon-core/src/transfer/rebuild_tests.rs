...
#[test]
fn transfer_rebuild_preflights_every_backup_collision_before_renaming_any_table() {
    let handle = std::thread::Builder::new()
        .name("transfer-test-rebuild-preflights-every-backup-collision-before-renaming-any-table".to_string())
        .stack_size(8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("build transfer test runtime")
                .block_on(run_transfer_rebuild_preflights_every_backup_collision_before_renaming_any_table());
        })
        .expect("spawn transfer test thread");
    if let Err(panic) = handle.join() {
        std::panic::resume_unwind(panic);
    }
}

async fn run_transfer_rebuild_preflights_every_backup_collision_before_renaming_any_table() {
...
#[test]
fn transfer_rebuild_cancellation_remains_a_cancellation_after_backup() {
    // The transfer recovery path builds a deep future in debug test builds.
    // Run it in an explicit 8 MiB thread instead of relying on RUST_MIN_STACK.
    let handle = std::thread::Builder::new()
        .name("transfer-rebuild-cancellation".to_string())
        .stack_size(8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("build transfer rebuild test runtime")
                .block_on(run_transfer_rebuild_cancellation_remains_a_cancellation_after_backup());
        })
        .expect("spawn transfer rebuild test thread");
    if let Err(panic) = handle.join() {
        std::panic::resume_unwind(panic);
    }
}

async fn run_transfer_rebuild_cancellation_remains_a_cancellation_after_backup() {
...
#[test]
fn transfer_rebuild_preview_plans_without_executing_ddl() {
    let handle = std::thread::Builder::new()
        .name("transfer-test-rebuild-preview-plans-without-executing-ddl".to_string())
        .stack_size(8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("build transfer test runtime")
                .block_on(run_transfer_rebuild_preview_plans_without_executing_ddl());
        })
        .expect("spawn transfer test thread");
    if let Err(panic) = handle.join() {
        std::panic::resume_unwind(panic);
...
async fn run_transfer_rebuild_preview_plans_without_executing_ddl() {
...
#[test]
fn transfer_keyset_pagination_copies_every_row_across_many_batches() {
    let handle = std::thread::Builder::new()
        .name("transfer-test-keyset-pagination-copies-every-row-across-many-batches".to_string())
        .stack_size(8 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("build transfer test runtime")
                .block_on(run_transfer_keyset_pagination_copies_every_row_across_many_batches());
        })
        .expect("spawn transfer test thread");
    if let Err(panic) = handle.join() {
        std::panic::resume_unwind(panic);
...
async fn run_transfer_keyset_pagination_copies_every_row_across_many_batches() {
    let directory = tempfile::tempdir().unwrap();
    let storage = crate::storage::Storage::open(&directory.path().join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new_with_plugin_dir(storage, directory.path().join("plugins")));
    for id in ["source", "target"] {
