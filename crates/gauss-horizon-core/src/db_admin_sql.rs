...
mod tests {
    ...
    fn builds_duplicate_table_structure_sql() {
        ...
        assert_eq!(
            ...
            "CREATE TABLE `gauss_horizon_demo`.`connection_test_copy` LIKE `gauss_horizon_demo`.`connection_test`;"
        );
        assert_eq!(
            build_duplicate_table_structure_sql(DuplicateTableStructureSqlOptions {
            ...
            "CREATE TABLE `gauss_horizon_demo`.`orders_copy` LIKE `gauss_horizon_demo`.`orders`;"
        );
        assert_eq!(
            build_duplicate_table_structure_sql(DuplicateTableStructureSqlOptions {
                database_type: Some(DatabaseType::Postgres),
    ...
    fn builds_oracle_family_table_and_view_rename_sql() {
        ...
        assert_eq!(
            build_rename_object_sql(RenameObjectSqlOptions {
                ...
                schema: Some("APP".to_string()),
                old_name: "ORDERS".to_string(),
                new_name: "ORDERS__GAUSS_HORIZON_BAK".to_string(),
            })
            .unwrap(),
            "ALTER TABLE \"APP\".\"ORDERS\" RENAME TO \"ORDERS__GAUSS_HORIZON_BAK\";"
        );
    }
