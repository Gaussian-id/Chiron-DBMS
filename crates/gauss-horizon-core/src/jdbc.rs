...
const JDBC_PLUGIN_DOWNLOAD_URL: &str =
    "https://github.com/Gaussian-id/Gauss-Horizon/releases/download/v0.1.0/gauss-horizon-jdbc-plugin-0.1.0.zip";
...
pub async fn install_prestosql_jdbc_driver(plugins_root: &Path) -> Result<Vec<JdbcDriverInfo>, String> {
    ...
    let mut resp = client
        ...
        .header(reqwest::header::USER_AGENT, "gauss-horizon-prestosql-driver-installer")
        .send()
        .await
...
fn jdbc_maven_resolver_executable(plugin_dir: &Path) -> PathBuf {
    #[cfg(windows)]
    {
        plugin_dir.join("bin").join("gauss-horizon-maven-resolver.bat")
    }
    #[cfg(not(windows))]
    {
        plugin_dir.join("bin").join("gauss-horizon-maven-resolver")
    }
}
...
/// desktop updater is disabled in 0.1.0, so normal status checks supply no
...
async fn download_jdbc_plugin_zip_with_progress(progress: &impl Fn(AgentProgressEvent)) -> Result<Vec<u8>, String> {
    ...
    let mut resp = client
        ...
        .map_err(|err| format!("Failed to download JDBC plugin from the Gauss Horizon release: {err}"))?;
...
fn install_jdbc_plugin_zip(bytes: &[u8], plugin_dir: &Path) -> Result<(), String> {
    ...
    if !temp_dir.join("manifest.json").exists() {
        ...
        return Err(format!(
            "This ZIP is not a valid Gauss Horizon JDBC plugin package (missing manifest.json). \
    ...
    if manifest.protocol_version != SUPPORTED_PLUGIN_PROTOCOL_VERSION {
        let _ = std::fs::remove_dir_all(&temp_dir);
        return Err(format!(
            "Downloaded JDBC plugin uses protocol version {}, but this Gauss Horizon build supports protocol version {}",
    ...
    #[cfg(unix)]
    {
        ...
        for executable in ["gauss-horizon-jdbc-plugin", "gauss-horizon-maven-resolver"] {
...
mod tests {
    ...
    fn test_jdbc_plugin_zip() -> Vec<u8> {
        ...
        zip.start_file("gauss-horizon-jdbc-plugin-test/manifest.json", options).unwrap();
        zip.write_all(
            br#"{
              "id": "jdbc",
              ...
              "version": "0.1.30",
              "protocol_version": 1,
              "executable": "bin/gauss-horizon-jdbc-plugin",
              "drivers": [{"id":"jdbc","label":"JDBC","kind":"external","database_type":"jdbc"}]
            }"#,
        )
        .unwrap();
        zip.start_file("gauss-horizon-jdbc-plugin-test/bin/gauss-horizon-jdbc-plugin", options).unwrap();
    ...
    fn maven_bundle_install_lists_nested_jars() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-jdbc-maven-test-{}", uuid::Uuid::new_v4()));
    ...
    fn imports_offline_jdbc_plugin_and_rewrites_bundle_paths() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-offline-jdbc-test-{}", uuid::Uuid::new_v4()));
    ...
    fn rejects_corrupt_offline_jdbc_artifact_before_installing_plugin() {
        let root =
            std::env::temp_dir().join(format!("gauss-horizon-offline-jdbc-corrupt-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let package = root.join("offline.zip");
    ...
    fn multiple_local_jars_are_imported_as_one_bundle() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-jdbc-local-bundle-test-{}", uuid::Uuid::new_v4()));
        let source_dir = root.join("source");
        std::fs::create_dir_all(&source_dir).unwrap();
    ...
    fn single_local_jar_keeps_legacy_standalone_behavior() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-jdbc-single-jar-test-{}", uuid::Uuid::new_v4()));
    ...
    fn single_local_jar_already_in_drivers_dir_is_not_duplicated() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-jdbc-managed-jar-test-{}", uuid::Uuid::new_v4()));
    ...
    fn single_local_jar_with_existing_name_keeps_unique_import_behavior() {
        let root =
            std::env::temp_dir().join(format!("gauss-horizon-jdbc-duplicate-name-test-{}", uuid::Uuid::new_v4()));
    ...
    fn installs_prestosql_direct_driver_as_builtin_bundle() {
        let root =
            std::env::temp_dir().join(format!("gauss-horizon-prestosql-direct-driver-test-{}", uuid::Uuid::new_v4()));
        let source_dir = root.join("source");
        std::fs::create_dir_all(&source_dir).unwrap();
    ...
    async fn maven_resolver_receives_runtime_env() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-jdbc-maven-env-test-{}", uuid::Uuid::new_v4()));
        ...
        if cfg!(windows) {
            std::fs::write(
                &resolver,
                format!(
                    "@echo off\r\nif \"%GAUSS_HORIZON_JAVA_BIN%\"==\"\" (\r\n  echo missing GAUSS_HORIZON_JAVA_BIN 1>&2\r\n  exit /b 2\r\n)\r\necho {output}\r\n"
                ),
            )
            .unwrap();
        } else {
            std::fs::write(
                &resolver,
                format!(
                    "#!/usr/bin/env sh\nif [ -z \"${{GAUSS_HORIZON_JAVA_BIN:-}}\" ]; then\n  echo missing GAUSS_HORIZON_JAVA_BIN >&2\n  exit 2\nfi\necho '{}'\n",
                    output.replace('\'', "'\\''")
                ),
            )
            .unwrap();
        ...
        let drivers = install_jdbc_driver_from_maven(
            ...
            PluginRuntimeEnv::default().with_var("GAUSS_HORIZON_JAVA_BIN", "java-from-managed-jre"),
        )
        .await
        .unwrap();
    ...
    async fn delayed_network_status_check_does_not_revert_a_concurrent_local_install() {
        let root = std::env::temp_dir().join(format!("gauss-horizon-jdbc-status-race-test-{}", uuid::Uuid::new_v4()));
        let plugin_dir = root.join("jdbc");
        ...
        std::fs::write(
            ...
            r#"{"id":"jdbc","name":"Gauss Horizon JDBC Plugin","version":"0.1.30","protocol_version":1,"executable":"bin/gauss-horizon-jdbc-plugin","drivers":[]}"#,
        )
        .unwrap();
