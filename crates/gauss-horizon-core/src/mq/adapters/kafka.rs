...
mod tests {
    ...
    fn connection_params_preserve_kafka_security_extra_and_nested_tls() {
        ...
        assert_eq!(params.pointer("/properties/client.id").and_then(|v| v.as_str()), Some("gauss-horizon"));
    }
    ...
    fn connection_params_preserve_zookeeper_sasl_and_tls_properties() {
        let cfg = kafka_config(
            serde_json::json!({
                ...
                "properties": {
                    ...
                    "zookeeper.sasl.clientconfig": "GaussHorizonZooKeeperClient",
                    "zookeeper.client.secure": "true",
                    "zookeeper.clientCnxnSocket": "org.apache.zookeeper.ClientCnxnSocketNetty",
                    "zookeeper.ssl.trustStore.location": "/etc/gauss-horizon/zookeeper-truststore.p12"
                }
            }),
            MqAuth::None,
            false,
        ...
        assert_eq!(
            ...
            Some("GaussHorizonZooKeeperClient")
        );
        ...
        assert_eq!(
            ...
            Some("/etc/gauss-horizon/zookeeper-truststore.p12")
        );
    }
    ...
    async fn topic_listing_does_not_preempt_agent_timeout_fallback() {
        let script_path =
            std::env::temp_dir().join(format!("gauss-horizon-kafka-topic-timeout-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            r#"import json
import sys
