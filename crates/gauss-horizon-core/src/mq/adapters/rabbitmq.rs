...
mod tests {
    ...
    fn connection_params_map_addresses_vhost_and_basic_auth() {
        ...
        assert_eq!(params.pointer("/properties/connectionName").and_then(|v| v.as_str()), Some("gauss-horizon"));
    }
    ...
    fn user_entries_parse_from_agent_payload() {
        let payload = serde_json::json!({
            "users": [
                { "name": "gauss-horizon-app", "tags": ["management"] },
                { "name": "gauss-horizon-svc" }
            ]
        });
        ...
        assert_eq!(users[0].name, "gauss-horizon-app");
        assert_eq!(users[0].tags, vec!["management".to_string()]);
        assert_eq!(users[1].name, "gauss-horizon-svc");
    ...
    fn permission_entries_parse_from_agent_payload() {
        let payload = serde_json::json!({
            "permissions": [
                { "user": "gauss-horizon-app", "vhost": "orders", "configure": ".*", "write": "gauss-horizon-.*", "read": ".*" },
                { "user": "gauss-horizon-app", "vhost": "/", "configure": "", "write": "", "read": "" }
            ]
        });
        ...
        assert_eq!(permissions[0].user, "gauss-horizon-app");
        assert_eq!(permissions[0].vhost, "orders");
        assert_eq!(permissions[0].write, "gauss-horizon-.*");
    ...
    fn binding_params_match_agent_contract_and_attach_vhost() {
        let binding = MqBindingInfo {
            source: "gauss-horizon-events".to_string(),
            destination: "gauss-horizon-queue".to_string(),
        ...
        assert_eq!(params.get("source").and_then(|v| v.as_str()), Some("gauss-horizon-events"));
        assert_eq!(params.get("destination").and_then(|v| v.as_str()), Some("gauss-horizon-queue"));
        ...
        let bare = MqBindingInfo {
            source: "gauss-horizon-a".to_string(),
            destination: "gauss-horizon-b".to_string(),
    ...
    fn map_item_namespace_moves_vhost_into_namespace_field() {
        let item = map_item_namespace(serde_json::json!({ "name": "gauss-horizon-q", "vhost": "orders" }));
        assert_eq!(item.get("namespace").and_then(|v| v.as_str()), Some("orders"));

        // Entries without a vhost stay untouched (single-vhost listings).
        let plain = map_item_namespace(serde_json::json!({ "name": "gauss-horizon-q" }));
        ...
        let exchange: MqExchangeInfo = serde_json::from_value(map_item_namespace(
            serde_json::json!({ "name": "gauss-horizon-ex", "type": "direct", "vhost": "orders" }),
        ))
    ...
    fn policy_entries_parse_from_agent_payload() {
        let payload = serde_json::json!({
            "policies": [
                {
                    "name": "gauss-horizon-ttl",
                    "vhost": "orders",
                    "pattern": "^gauss-horizon-",
                    "applyTo": "queues",
                    "priority": 5,
                    "definition": { "message-ttl": 60000 }
                },
                {
                    "name": "gauss-horizon-ha",
                    "vhost": "/",
                    "pattern": ".*",
                    "applyTo": "all",
                    "priority": 0,
        ...
        assert_eq!(policies[0].name, "gauss-horizon-ttl");
    ...
    fn policy_params_match_agent_contract_and_attach_vhost() {
        ...
        let policy = MqPolicyInfo {
            name: "gauss-horizon-ttl".to_string(),
            vhost: String::new(),
            pattern: "^gauss-horizon-".to_string(),
        ...
        assert_eq!(params.get("name").and_then(|v| v.as_str()), Some("gauss-horizon-ttl"));
        assert_eq!(params.get("pattern").and_then(|v| v.as_str()), Some("^gauss-horizon-"));
    ...
    fn policy_params_omit_empty_apply_to_so_agent_default_applies() {
        let policy =
            MqPolicyInfo { name: "gauss-horizon-ha".to_string(), pattern: ".*".to_string(), ..MqPolicyInfo::default() };
