...
mod tests {
    ...
    fn exchange_info_round_trips_agent_camel_case() {
        let exchange: super::MqExchangeInfo = serde_json::from_str(
            r#"{"name":"gauss-horizon-events","type":"topic","durable":true,"autoDelete":false,"internal":false}"#,
        )
        .expect("exchange info");
        assert_eq!(exchange.name, "gauss-horizon-events");
    ...
    fn binding_info_skips_absent_routing_key_and_arguments() {
        let binding: super::MqBindingInfo = serde_json::from_str(
            r#"{"source":"gauss-horizon-events","destination":"gauss-horizon-queue","destinationType":"queue","routingKey":"orders.*"}"#,
        )
        .expect("binding info");
        assert_eq!(binding.source, "gauss-horizon-events");
        ...
        let bare: super::MqBindingInfo = serde_json::from_str(
            r#"{"source":"gauss-horizon-a","destination":"gauss-horizon-b","destinationType":"exchange"}"#,
        )
    ...
    fn send_message_request_defaults_new_rabbitmq_fields() {
        let req: super::SendMessageRequest =
            serde_json::from_str(r#"{"topic":"gauss-horizon-queue","payloadBase64":"aGVsbG8="}"#)
                .expect("send request");
    ...
    fn user_info_round_trips_agent_camel_case() {
        let user: super::MqUserInfo =
            serde_json::from_str(r#"{"name":"gauss-horizon-app","tags":["management","policymaker"]}"#)
                .expect("user info");
        assert_eq!(user.name, "gauss-horizon-app");
        ...
        let bare: super::MqUserInfo =
            serde_json::from_str(r#"{"name":"gauss-horizon-svc"}"#).expect("user without tags");
        assert!(bare.tags.is_empty());

        let json = serde_json::to_value(&user).expect("serialize user info");
        assert_eq!(json.get("name").and_then(|v| v.as_str()), Some("gauss-horizon-app"));
    ...
    fn vhost_permission_round_trips_agent_fields() {
        let perm: super::MqVhostPermission = serde_json::from_str(
            r#"{"user":"gauss-horizon-app","vhost":"orders","configure":".*","write":"gauss-horizon-.*","read":".*"}"#,
        )
        .expect("vhost permission");
        assert_eq!(perm.user, "gauss-horizon-app");
        assert_eq!(perm.vhost, "orders");
        assert_eq!(perm.configure, ".*");
        assert_eq!(perm.write, "gauss-horizon-.*");
        assert_eq!(perm.read, ".*");
        ...
        assert_eq!(json.get("write").and_then(|v| v.as_str()), Some("gauss-horizon-.*"));
    }
    ...
    fn policy_info_round_trips_agent_fields() {
        let policy: super::MqPolicyInfo = serde_json::from_str(
            r#"{"name":"gauss-horizon-ttl","vhost":"orders","pattern":"^gauss-horizon-","applyTo":"queues","priority":5,"definition":{"message-ttl":60000,"max-length":1000}}"#,
        )
        .expect("policy info");
        assert_eq!(policy.name, "gauss-horizon-ttl");
        assert_eq!(policy.vhost, "orders");
        assert_eq!(policy.pattern, "^gauss-horizon-");
    ...
    fn policy_info_defaults_absent_apply_to_and_priority() {
        let policy: super::MqPolicyInfo = serde_json::from_str(
            r#"{"name":"gauss-horizon-ha","vhost":"/","pattern":".*","definition":{"ha-mode":"all"}}"#,
        )
