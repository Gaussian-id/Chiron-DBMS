...
//! (`crates/gauss-horizon-web/src/routes/mq.rs`), keeping the business logic unified.
//!
...
mod tests {
    ...
    async fn test_state_with(config: ConnectionConfig) -> (AppState, std::path::PathBuf) {
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-mq-service-test-{stamp}"));
    ...
    async fn mutating_exchange_calls_block_read_only_connections_before_adapter_build() {
        ...
        let err = mq_create_exchange_core(&state, "readonly-mq", ns, "gauss-horizon-events", "topic", true, false)
            .await
    ...
    async fn mutating_user_permission_calls_block_read_only_connections_before_adapter_build() {
        ...
        let err = mq_create_user_core(&state, "readonly-mq", "gauss-horizon-app", "secret", vec![])
            .await
            .expect_err("read-only user create should fail");
        assert!(err.contains("Read-only mode"));

        let err = mq_delete_user_core(&state, "readonly-mq", "gauss-horizon-app")
            .await
            .expect_err("read-only user delete should fail");
        assert!(err.contains("Read-only mode"));

        let err =
            mq_grant_user_permission_core(&state, "readonly-mq", ns.clone(), "gauss-horizon-app", ".*", ".*", ".*")
                .await
                .expect_err("read-only permission grant should fail");
        assert!(err.contains("Read-only mode"));

        let err = mq_revoke_user_permission_core(&state, "readonly-mq", ns, "gauss-horizon-app")
            .await
    ...
    async fn mutating_policy_calls_block_read_only_connections_before_adapter_build() {
        ...
        let err = mq_delete_policy_core(&state, "readonly-mq", ns, "gauss-horizon-ttl")
            .await
