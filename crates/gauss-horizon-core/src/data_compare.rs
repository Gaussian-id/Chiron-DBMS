...
fn build_rownum_data_compare_select_sql(
    ...
    let row_number_alias = quote_table_identifier(Some(database_type), "__gauss_horizon_row_num");
    let end = offset + row_limit;
    ...
    format!(
        "SELECT {outer_columns} FROM (SELECT gauss_horizon_inner.*, ROWNUM AS {row_number_alias} FROM ({base}) gauss_horizon_inner WHERE ROWNUM <= {end}) WHERE {row_number_alias} > {offset}"
    )
}
...
mod tests {
    ...
    fn builds_backend_table_select_sql_for_oceanbase_oracle_rownum_pages() {
        ...
        assert_eq!(
            ...
            "SELECT \"ID\", \"NAME\" FROM (SELECT gauss_horizon_inner.*, ROWNUM AS \"__gauss_horizon_row_num\" FROM (SELECT \"ID\", \"NAME\" FROM \"APP\".\"EVENTS\" ORDER BY \"ID\" ASC) gauss_horizon_inner WHERE ROWNUM <= 75) WHERE \"__gauss_horizon_row_num\" > 50"
        );
    }
