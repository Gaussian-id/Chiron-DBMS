...
DECLARE @gauss_horizon_qualified_table nvarchar(517) =
    QUOTENAME(OBJECT_SCHEMA_NAME(@gauss_horizon_object_id)) + N'.' + QUOTENAME(OBJECT_NAME(@gauss_horizon_object_id));

DECLARE @gauss_horizon_unsupported_index sysname;
SELECT TOP (1) @gauss_horizon_unsupported_index = idx.name
FROM sys.indexes AS idx
WHERE idx.object_id = @gauss_horizon_object_id
  AND idx.index_id > 0
  ...
  AND EXISTS (
      ...
      WHERE ic.object_id = idx.object_id
        ...
        AND ic.column_id = @gauss_horizon_column_id
  );
IF @gauss_horizon_unsupported_index IS NOT NULL
BEGIN
    DECLARE @gauss_horizon_unsupported_index_message nvarchar(2048) =
        N'Gauss Horizon cannot safely preserve affected SQL Server index ' + QUOTENAME(@gauss_horizon_unsupported_index) +
        N' because its index type is not a supported rowstore type.';
    ;THROW 50001, @gauss_horizon_unsupported_index_message, 1;
END;
    ...
    WHERE idx.object_id = @gauss_horizon_object_id
      ...
      AND EXISTS (
          ...
          WHERE affected_ic.object_id = idx.object_id
            ...
            AND affected_ic.column_id = @gauss_horizon_column_id
      )
      AND NOT EXISTS (
          SELECT 1
...
BEGIN
    ;THROW 50002, 'Gauss Horizon could not resolve the partition column for an affected SQL Server index.', 1;
END;

DECLARE @gauss_horizon_started_transaction bit = 0;
DECLARE @gauss_horizon_has_savepoint bit = 0;

BEGIN TRY
    IF @@TRANCOUNT = 0
    BEGIN
        BEGIN TRANSACTION;
        SET @gauss_horizon_started_transaction = 1;
    END
    ELSE
    BEGIN
        SAVE TRANSACTION gauss_horizon_alter_column_dependencies;
        SET @gauss_horizon_has_savepoint = 1;
    END;

    DECLARE @gauss_horizon_dependencies TABLE (
    ...
    INSERT INTO @gauss_horizon_dependencies (drop_order, create_order, drop_sql, create_sql)
    SELECT
        10,
        50,
    ...
    WHERE EXISTS (
        ...
        FROM sys.foreign_key_columns AS fkc
        WHERE fkc.constraint_object_id = fk.object_id
          AND ((fkc.parent_object_id = @gauss_horizon_object_id AND fkc.parent_column_id = @gauss_horizon_column_id)
            OR (fkc.referenced_object_id = @gauss_horizon_object_id AND fkc.referenced_column_id = @gauss_horizon_column_id))
    );
    ...
    SELECT
        20,
        40,
        N'ALTER TABLE ' + @gauss_horizon_qualified_table + N' DROP CONSTRAINT ' + QUOTENAME(cc.name) + N';',
        N'ALTER TABLE ' + @gauss_horizon_qualified_table +
            ...
            cc.definition + N'; ALTER TABLE ' + @gauss_horizon_qualified_table +
            CASE WHEN cc.is_disabled = 1 THEN N' NOCHECK' ELSE N' CHECK' END +
            N' CONSTRAINT ' + QUOTENAME(cc.name) + N';'
    FROM sys.check_constraints AS cc
    WHERE cc.parent_object_id = @gauss_horizon_object_id
      AND (cc.parent_column_id = @gauss_horizon_column_id OR EXISTS (
          SELECT 1
          FROM sys.sql_expression_dependencies AS sed
          WHERE sed.referencing_id = cc.object_id
            AND sed.referenced_id = @gauss_horizon_object_id
            AND sed.referenced_minor_id = @gauss_horizon_column_id
      ));
    ...
    SELECT
        30,
        30,
        N'DROP STATISTICS ' + @gauss_horizon_qualified_table + N'.' + QUOTENAME(stats.name) + N';',
        N'CREATE STATISTICS ' + QUOTENAME(stats.name) + N' ON ' + @gauss_horizon_qualified_table + N' (' +
            STUFF((
    ...
    WHERE stats.object_id = @gauss_horizon_object_id
      ...
      AND EXISTS (
          ...
          WHERE sc.object_id = stats.object_id
            AND sc.stats_id = stats.stats_id
            AND sc.column_id = @gauss_horizon_column_id
      )
      AND NOT EXISTS (
          SELECT 1
    ...
    SELECT
        40,
        20,
        N'DROP INDEX ' + QUOTENAME(idx.name) + N' ON ' + @gauss_horizon_qualified_table + N';',
        N'CREATE ' + CASE WHEN idx.is_unique = 1 THEN N'UNIQUE ' ELSE N'' END + idx.type_desc +
            N' INDEX ' + QUOTENAME(idx.name) + N' ON ' + @gauss_horizon_qualified_table + N' (' +
            STUFF((
            ...
            CASE WHEN idx.is_disabled = 1 THEN N' ALTER INDEX ' + QUOTENAME(idx.name) +
                 N' ON ' + @gauss_horizon_qualified_table + N' DISABLE;' ELSE N'' END
    FROM sys.indexes AS idx
    ...
    SELECT
        50,
        10,
        N'ALTER TABLE ' + @gauss_horizon_qualified_table + N' DROP CONSTRAINT ' + QUOTENAME(key_constraint.name) + N';',
        N'ALTER TABLE ' + @gauss_horizon_qualified_table + N' ADD CONSTRAINT ' + QUOTENAME(key_constraint.name) + N' ' +
    ...
    LEFT JOIN sys.data_spaces AS data_space ON data_space.data_space_id = idx.data_space_id
    WHERE key_constraint.parent_object_id = @gauss_horizon_object_id
      AND EXISTS (
          SELECT 1
          FROM sys.index_columns AS affected_ic
          WHERE affected_ic.object_id = idx.object_id
    ...
    DECLARE @gauss_horizon_dependency_sql nvarchar(max);
    DECLARE gauss_horizon_dependency_drop_cursor CURSOR LOCAL FAST_FORWARD FOR
        SELECT drop_sql
        FROM @gauss_horizon_dependencies
        ORDER BY drop_order, dependency_id;
    OPEN gauss_horizon_dependency_drop_cursor;
    FETCH NEXT FROM gauss_horizon_dependency_drop_cursor INTO @gauss_horizon_dependency_sql;
    WHILE @@FETCH_STATUS = 0
    BEGIN
        EXEC sys.sp_executesql @gauss_horizon_dependency_sql;
        FETCH NEXT FROM gauss_horizon_dependency_drop_cursor INTO @gauss_horizon_dependency_sql;
    END;
    CLOSE gauss_horizon_dependency_drop_cursor;
    DEALLOCATE gauss_horizon_dependency_drop_cursor;
"#;

const DEPENDENCY_RECREATE_AND_FINISH_SQL: &str = r#"
    DECLARE gauss_horizon_dependency_create_cursor CURSOR LOCAL FAST_FORWARD FOR
        SELECT create_sql
        FROM @gauss_horizon_dependencies
        ORDER BY create_order, dependency_id;
    OPEN gauss_horizon_dependency_create_cursor;
    FETCH NEXT FROM gauss_horizon_dependency_create_cursor INTO @gauss_horizon_dependency_sql;
    WHILE @@FETCH_STATUS = 0
    BEGIN
        EXEC sys.sp_executesql @gauss_horizon_dependency_sql;
        FETCH NEXT FROM gauss_horizon_dependency_create_cursor INTO @gauss_horizon_dependency_sql;
    END;
    CLOSE gauss_horizon_dependency_create_cursor;
    DEALLOCATE gauss_horizon_dependency_create_cursor;

    IF @gauss_horizon_started_transaction = 1
        COMMIT TRANSACTION;
END TRY
BEGIN CATCH
    IF CURSOR_STATUS('local', 'gauss_horizon_dependency_drop_cursor') >= 0
        CLOSE gauss_horizon_dependency_drop_cursor;
    IF CURSOR_STATUS('local', 'gauss_horizon_dependency_drop_cursor') >= -1
        DEALLOCATE gauss_horizon_dependency_drop_cursor;
    IF CURSOR_STATUS('local', 'gauss_horizon_dependency_create_cursor') >= 0
        CLOSE gauss_horizon_dependency_create_cursor;
    IF CURSOR_STATUS('local', 'gauss_horizon_dependency_create_cursor') >= -1
        DEALLOCATE gauss_horizon_dependency_create_cursor;

    IF @gauss_horizon_started_transaction = 1 AND XACT_STATE() <> 0
        ROLLBACK TRANSACTION;
    ELSE IF @gauss_horizon_has_savepoint = 1 AND XACT_STATE() = 1
        ROLLBACK TRANSACTION gauss_horizon_alter_column_dependencies;
...
pub(super) fn build_dependency_aware_alter_column_batch(table: &str, column_name: &str, alter_batch: &str) -> String {
    ...
    batch.push_str(
        "SET ANSI_NULLS ON;\n\
         ...
         DECLARE @gauss_horizon_object_id int = OBJECT_ID(N'",
    );
    batch.push_str(&table_literal);
    batch.push_str("');\nDECLARE @gauss_horizon_column_id int = COLUMNPROPERTY(@gauss_horizon_object_id, N'");
    batch.push_str(&column_literal);
    batch.push_str("', 'ColumnId');\nIF @gauss_horizon_object_id IS NULL OR @gauss_horizon_column_id IS NULL\n");
    batch.push_str(
        "    THROW 50000, 'Gauss Horizon could not resolve the SQL Server column before ALTER COLUMN.', 1;\n",
    );
...
mod tests {
    ...
    fn dependency_batch_escapes_catalog_literals_and_keeps_alter_sql() {
        ...
        assert!(sql.contains("COLUMNPROPERTY(@gauss_horizon_object_id, N'amount''s value', 'ColumnId')"), "{sql}");
    ...
    fn dependency_batch_covers_every_supported_dependency_kind() {
        ...
        assert!(sql.contains("fkc.referenced_object_id = @gauss_horizon_object_id"), "inbound FK predicate: {sql}");
    ...
    fn dependency_batch_rejects_unsupported_indexes_before_dropping_anything() {
        ...
        let drop_cursor = sql.find("gauss_horizon_dependency_drop_cursor CURSOR").expect("drop cursor");
    ...
    fn dependency_batch_is_atomic_and_recreates_after_the_alter() {
        ...
        let create_cursor = sql.find("gauss_horizon_dependency_create_cursor CURSOR").expect("create cursor");
        ...
        assert!(sql.contains("SAVE TRANSACTION gauss_horizon_alter_column_dependencies"), "{sql}");
        assert!(sql.contains("ROLLBACK TRANSACTION gauss_horizon_alter_column_dependencies"), "{sql}");
        assert!(sql.contains("IF @gauss_horizon_started_transaction = 1\n        COMMIT TRANSACTION"), "{sql}");
