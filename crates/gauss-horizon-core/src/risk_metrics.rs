...
impl DegradationMetrics {
    ...
    pub fn snapshot(&self) -> Vec<MetricEntry> {
        let mut entries = vec![
            MetricEntry {
                name: "gauss_horizon_degradation_total".to_string(),
            ...
            MetricEntry {
                name: "gauss_horizon_degradation_level_full".to_string(),
                metric_type: MetricType::Counter,
            ...
            MetricEntry {
                name: "gauss_horizon_degradation_level_sample".to_string(),
                metric_type: MetricType::Counter,
            ...
            MetricEntry {
                name: "gauss_horizon_degradation_level_skip".to_string(),
                metric_type: MetricType::Counter,
            ...
            MetricEntry {
                name: "gauss_horizon_auto_upgrade_total".to_string(),
                metric_type: MetricType::Counter,
            ...
            MetricEntry {
                name: "gauss_horizon_auto_downgrade_total".to_string(),
                metric_type: MetricType::Counter,
            ...
            MetricEntry {
                name: "gauss_horizon_confidence_gauge".to_string(),
        ...
        if let Ok(h) = self.osc_probe_latency.lock() {
            if !h.is_empty() {
                entries.push(MetricEntry {
                    name: "gauss_horizon_osc_probe_latency".to_string(),
        ...
        entries.extend(vec![
            MetricEntry {
                name: "gauss_horizon_osc_execution_status".to_string(),
                metric_type: MetricType::Gauge,
            ...
            MetricEntry {
                name: "gauss_horizon_rollback_trigger_total".to_string(),
                metric_type: MetricType::Counter,
            ...
            MetricEntry {
                name: "gauss_horizon_tag_block_total".to_string(),
                metric_type: MetricType::Counter,
            ...
            MetricEntry {
                name: "gauss_horizon_trace_dropped_total".to_string(),
                metric_type: MetricType::Counter,
        ...
        if let Ok(h) = self.sample_rate_histogram.lock() {
            entries.push(MetricEntry {
                name: "gauss_horizon_sample_rate_histogram".to_string(),
                metric_type: MetricType::Histogram,
...
mod tests {
    ...
    fn degradation_metrics_records_levels() {
        ...
        let total = snapshot.iter().find(|e| e.name == "gauss_horizon_degradation_total").unwrap();
        assert_eq!(total.value, MetricValue::Counter(3));
        let full = snapshot.iter().find(|e| e.name == "gauss_horizon_degradation_level_full").unwrap();
        assert_eq!(full.value, MetricValue::Counter(1));
        let sample = snapshot.iter().find(|e| e.name == "gauss_horizon_degradation_level_sample").unwrap();
        assert_eq!(sample.value, MetricValue::Counter(1));
        let skip = snapshot.iter().find(|e| e.name == "gauss_horizon_degradation_level_skip").unwrap();
    ...
    fn degradation_metrics_auto_chain_events() {
        ...
        let snapshot = metrics.snapshot();
        let up = snapshot.iter().find(|e| e.name == "gauss_horizon_auto_upgrade_total").unwrap();
        assert_eq!(up.value, MetricValue::Counter(2));
        let down = snapshot.iter().find(|e| e.name == "gauss_horizon_auto_downgrade_total").unwrap();
    ...
    fn degradation_metrics_confidence_gauge() {
        ...
        let snapshot = metrics.snapshot();
        let gauge = snapshot.iter().find(|e| e.name == "gauss_horizon_confidence_gauge").unwrap();
