...
mod tests {
    ...
    fn docs_notes_path_defaults_to_none_and_round_trips() {
        ...
        let with_path: ConnectionConfig = serde_json::from_value(serde_json::json!({
            ...
            "password": "",
            "database": null,
            "docs_notes_path": "docs/gauss-horizon-docs.json"
        }))
        .expect("config with a notes path must parse");
        assert_eq!(with_path.docs_notes_path.as_deref(), Some("docs/gauss-horizon-docs.json"));
    }
    ...
    fn kyuubi_database_type_and_connection_urls_are_stable() {
        ...
        let mut config = mysql_config("gauss-horizon", "", Some("analytics"));
        ...
        config.username = "gauss-horizon".to_string();
        config.password = "secret".to_string();
        assert_eq!(
            config.connection_url_with_host(&config.host, config.port),
    ...
    fn historical_sqlserver_legacy_setting_is_canonicalized_to_driver_profile() {
        ...
        config.url_params = Some(
            "applicationName={Gauss Horizon; Client};password=50%;sqlserverEncryption=disabled;encrypt=false"
                .to_string(),
        );
        ...
        assert_eq!(
            ...
            Some("applicationName={Gauss Horizon; Client};password=50%;encrypt=false")
        );
    ...
    fn sqlserver_legacy_migration_respects_escaped_braces() {
        let params = "applicationName={Gauss Horizon}}; Client};sqlserverEncryption=disabled;encrypt=false";

        assert!(sqlserver_legacy_compatibility_param(Some(params)));
        assert_eq!(
            without_sqlserver_legacy_compatibility_param(Some(params)).as_deref(),
            Some("applicationName={Gauss Horizon}}; Client};encrypt=false")
        );
    }
    ...
    fn sqlserver_auto_profile_without_historical_setting_stays_native() {
        ...
        config.driver_profile = Some("sqlserver".to_string());
        config.driver_label = Some("SQL Server".to_string());
        config.url_params = Some("applicationName=gauss-horizon&encrypt=false".to_string());

        let canonical = config.canonicalized();

        assert_eq!(canonical.driver_profile.as_deref(), Some("sqlserver"));
        assert_eq!(canonical.driver_label.as_deref(), Some("SQL Server"));
        assert_eq!(canonical.url_params.as_deref(), Some("applicationName=gauss-horizon&encrypt=false"));
    }
    ...
    fn postgres_url_uses_only_the_structured_endpoint() {
        ...
        config.db_type = DatabaseType::Postgres;
        config.url_params = Some(
            "HOST=origin.example.com&%68ostaddr=203.0.113.10&%70ort=6432&currentSchema=app&application_name=gauss-horizon"
                .to_string(),
        );
        ...
        assert_eq!(
            url,
            "postgres://postgres:secret@127.0.0.1:6543/test?sslmode=prefer&application_name=gauss-horizon&options=%2Dc%20search%5Fpath%3Dapp"
        );
    ...
    fn postgres_url_query_password_keeps_priority_but_is_redacted() {
        ...
        config.db_type = DatabaseType::Postgres;
        config.url_params = Some("%70assword=query-secret&application_name=gauss-horizon".to_string());
        ...
        assert_eq!(
            ...
            "postgres://10.1.2.3:2883/test?sslmode=prefer&application_name=gauss-horizon"
        );
    }
    ...
    fn postgres_url_redacts_case_insensitive_and_encoded_password_keys() {
        ...
        config.db_type = DatabaseType::Postgres;
        config.url_params =
            Some("PASSWORD=upper-secret&%70assword=encoded-secret&application_name=gauss-horizon".to_string());

        let url = config.redacted_connection_url();

        assert_eq!(url, "postgres://10.1.2.3:2883/test?sslmode=prefer&application_name=gauss-horizon");
