...
impl QoderIsolatedCwd {
    fn create() -> Result<Self, String> {
        let path = env::temp_dir().join(format!("gauss-horizon-qoder-{}", uuid::Uuid::new_v4()));
...
pub fn qoder_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.qoder_cli_env {
        ...
        if key.to_ascii_uppercase().starts_with("GAUSS_HORIZON_MCP_") {
            return Err(format!(
                "[qoderEnvReserved] `{key}` is managed by Gauss Horizon for the scoped MCP server and cannot be set here."
            ));
        }
...
fn qoder_process_env(config: &AiConfig, command: &QoderCommandSpec) -> Result<Vec<(String, String)>, String> {
    let current_path = crate::legacy::var("PATH").ok();
...
fn classify_qoder_spawn_error(message: &str) -> String {
    if message.contains("No such file") || message.contains("not found") || message.contains("os error 2") {
        format!("[qoderNotInstalled] Qoder CLI was not found. Install qodercli or set its executable path in Gauss Horizon AI settings. {message}")
    } else {
...
mod tests {
    ...
    fn builds_scoped_headless_qoder_command() {
        ...
        assert!(command.args.windows(2).any(|args| args == ["--allowed-mcp-server-names", "gauss-horizon"]));
        ...
        assert!(tools.contains("mcp__gauss_horizon__gauss_horizon_execute_query"));
    ...
    #[test]
    fn validates_environment_and_reserves_gauss_horizon_scope() {
        let mut config = config("default");
        ...
        config.qoder_cli_env.insert("GAUSS_HORIZON_MCP_SCOPE_DATABASE".to_string(), "other".to_string());
    ...
    fn classifies_authentication_and_mcp_failures() {
        ...
        assert!(
            classify_qoder_run_error("MCP server gauss-horizon failed to start").starts_with("[qoderMcpStartupFailed]")
        );
    }
