...
use crate::ai_cli_agent::{
    build_cli_agent_prompt, cli_command, gauss_horizon_mcp_enabled_tools, gauss_horizon_mcp_scope_env,
...
const CODEBUDDY_AUTH_PROBE_PROMPT: &str = "Gauss Horizon CodeBuddy authentication test. Reply with OK only.";
...
impl CodeBuddyIsolatedCwd {
    fn create() -> Result<Self, String> {
        let path = env::temp_dir().join(format!("gauss-horizon-codebuddy-{}", uuid::Uuid::new_v4()));
...
pub fn codebuddy_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.codebuddy_cli_env {
        ...
        if is_reserved_gauss_horizon_mcp_env_name(key) {
            return Err(format!(
                "[codeBuddyEnvReserved] `{key}` is managed by Gauss Horizon for the scoped MCP server and cannot be set here."
            ));
        }
...
pub fn codebuddy_enabled_tools(agent_mode: bool) -> Vec<String> {
    gauss_horizon_mcp_enabled_tools(agent_mode).into_iter().map(|tool| format!("mcp__gauss_horizon__{tool}")).collect()
}
...
fn codebuddy_mcp_config(options: &CodeBuddyRunOptions) -> String {
    ...
    let env = gauss_horizon_mcp_scope_env(options)
        .into_iter()
        .map(|(name, value)| (name.to_string(), Value::String(value)))
        .collect::<Map<_, _>>();
    server.insert("env".to_string(), Value::Object(env));
    json!({ "mcpServers": { "gauss-horizon": Value::Object(server) } }).to_string()
}
...
fn build_codebuddy_command_with_mcp_arg(
    ...
    let mut args = vec![
        ...
        "--settings".to_string(),
        json!({ "enabledMcpjsonServers": ["gauss-horizon"] }).to_string(),
    ];
    append_codebuddy_isolation_args(&mut args);
    // Supplying only NoDefer(MCP tool) entries keeps built-in file/shell tools
    // out of the model context while making the scoped Gauss Horizon tools immediately
...
async fn initialize_codebuddy(config: &AiConfig) -> Result<CodeBuddyInitializeResult, String> {
    ...
    let mut request = serde_json::to_vec(&json!({
        ...
        "request_id": "gauss_horizon_model_discovery",
    ...
    let result = async {
        acp.request(
            ...
            json!({
                ...
                "clientInfo": { "name": "Gauss Horizon", "version": env!("CARGO_PKG_VERSION") },
            }),
        )
        .await?;
        let session = acp
...
fn classify_codebuddy_spawn_error(message: &str) -> String {
    if message.contains("No such file") || message.contains("not found") || message.contains("os error 2") {
        format!("[codeBuddyNotInstalled] CodeBuddy Code CLI was not found. Install it or set its executable path in Gauss Horizon AI settings. {message}")
    } else {
...
fn classify_codebuddy_run_error(diagnostic: &str) -> String {
        ...
        || lower.contains("unauthorized")
    {
        ...
        format!("[gaussHorizonMcpMissing] {diagnostic}")
...
mod tests {
    ...
    fn builds_scoped_non_persistent_command() {
        ...
        assert!(spec.args.iter().any(|arg| arg.contains("\"GAUSS_HORIZON_MCP_SCOPE_CONNECTION_ID\":\"conn-1\"")));
        assert!(spec.args.iter().any(|arg| arg.contains("\"type\":\"stdio\"")));
        let tools_index = spec.args.iter().position(|arg| arg == "--tools").unwrap();
        assert!(spec.args[tools_index + 1].contains("NoDefer(mcp__gauss_horizon__gauss_horizon_execute_query)"));
        ...
        assert!(spec.args[allowed_tools_index + 1..]
            .iter()
            .any(|arg| arg == "mcp__gauss_horizon__gauss_horizon_execute_query"));
    }
    ...
    fn builds_explicit_model_effort_and_ask_tool_policy() {
        ...
        assert!(spec
            .args
            .iter()
            .any(|arg| arg.contains("\"command\":\"/opt/gauss-horizon/bin/gauss-horizon-mcp-server\"")));
        assert!(!codebuddy_enabled_tools(false)
            .iter()
            .any(|tool| tool == "mcp__gauss_horizon__gauss_horizon_execute_query"));
    }
    ...
    fn validates_path_and_environment() {
        ...
        config.codebuddy_cli_env.insert("GAUSS_HORIZON_MCP_ALLOW_WRITES".to_string(), "1".to_string());
    ...
    fn reuses_claude_compatible_stream_events() {
        let events = parse_codebuddy_jsonl_event(
            r#"{"type":"assistant","message":{"content":[{"type":"text","text":"done"},{"type":"tool_use","id":"call-1","name":"mcp__gauss_horizon__gauss_horizon_list_tables","input":{}}]}}"#,
        )
        .unwrap();
    ...
    fn classifies_codebuddy_specific_runtime_failures() {
        ...
        assert!(
            classify_codebuddy_run_error("gauss-horizon-mcp-server: ENOENT").starts_with("[gaussHorizonMcpMissing]")
        );
        assert!(classify_codebuddy_run_error("MCP gauss-horizon server failed to connect")
    ...
    fn isolated_cli_test_config() -> (AiConfig, std::path::PathBuf, std::path::PathBuf) {
        let project_dir =
            std::env::temp_dir().join(format!("gauss-horizon-codebuddy-project-test-{}", uuid::Uuid::new_v4()));
...
if [ "$user_settings" != "true" ] || [ "$PWD" = "$GAUSS_HORIZON_TEST_PROJECT_DIR" ]; then
  printf loaded > "$GAUSS_HORIZON_TEST_HOOK_MARKER"
fi
...
input=$(cat)
if printf '%s' "$input" | grep -q 'Gauss Horizon CodeBuddy authentication test'; then
  printf probed > "$GAUSS_HORIZON_TEST_AUTH_PROBE_MARKER"
...
"#,
        ...
        config
            .codebuddy_cli_env
            .insert("GAUSS_HORIZON_TEST_PROJECT_DIR".to_string(), project_dir.to_string_lossy().to_string());
        config
            .codebuddy_cli_env
            .insert("GAUSS_HORIZON_TEST_HOOK_MARKER".to_string(), hook_marker.to_string_lossy().to_string());
        config.codebuddy_cli_env.insert(
            "GAUSS_HORIZON_TEST_AUTH_PROBE_MARKER".to_string(),
    ...
    async fn null_account_allows_discovery_but_connection_probe_rejects_signed_out() {
        let project_dir =
            std::env::temp_dir().join(format!("gauss-horizon-codebuddy-signed-out-test-{}", uuid::Uuid::new_v4()));
...
"#,
    ...
    fn live_config() -> AiConfig {
        let mut config = codebuddy_config(
            &crate::legacy::var("GAUSS_HORIZON_LIVE_CODEBUDDY_MODEL").unwrap_or_else(|_| "default".to_string()),
        );
        config.codebuddy_cli_path = crate::legacy::var("GAUSS_HORIZON_LIVE_CODEBUDDY_PATH").ok();
        config
    }
    ...
    #[cfg(unix)]
    #[tokio::test]
    #[ignore = "requires CodeBuddy Code, a running Gauss Horizon MCP bridge, and GAUSS_HORIZON_LIVE_MCP_CONNECTION_NAME/DATABASE"]
    async fn live_scoped_gauss_horizon_mcp_agent_run() {
        let config = live_config();
        let connection_name = crate::legacy::var("GAUSS_HORIZON_LIVE_MCP_CONNECTION_NAME")
            .expect("set GAUSS_HORIZON_LIVE_MCP_CONNECTION_NAME to a saved Gauss Horizon connection");
        let database = crate::legacy::var("GAUSS_HORIZON_LIVE_MCP_DATABASE")
            .expect("set GAUSS_HORIZON_LIVE_MCP_DATABASE to a visible database");
        let mcp_command = crate::legacy::var("GAUSS_HORIZON_LIVE_MCP_COMMAND")
            .unwrap_or_else(|_| "gauss-horizon-mcp-server".to_string());
        ...
        let result = tokio::time::timeout(
            ...
            run_codebuddy_agent(
                &config,
                "You must use the Gauss Horizon MCP list-connections tool. Reply with the visible connection name only.",
                options,
                &Notify::new(),
        ...
        assert!(
            events.iter().any(
                |event| matches!(event, AgentEvent::ToolCallStart { tool_name, .. } if tool_name.contains("gauss_horizon_list_connections"))
            ),
            "CodeBuddy did not emit a Gauss Horizon tool-call start; result={result:?}, events={events:#?}"
        );
        assert!(
            events.iter().any(|event| matches!(event, AgentEvent::ToolCallEnd { is_error: false, .. })),
            "CodeBuddy did not emit a successful Gauss Horizon tool-call end; result={result:?}, events={events:#?}"
        );
        assert!(
    ...
    #[cfg(unix)]
    #[tokio::test]
    #[ignore = "requires CodeBuddy Code, a running Gauss Horizon MCP bridge, and a seeded GAUSS_HORIZON_LIVE_MCP connection"]
    async fn live_scoped_gauss_horizon_mcp_query() {
        let config = live_config();
        let connection_name = crate::legacy::var("GAUSS_HORIZON_LIVE_MCP_CONNECTION_NAME")
            .expect("set GAUSS_HORIZON_LIVE_MCP_CONNECTION_NAME to a saved Gauss Horizon connection");
        let database = crate::legacy::var("GAUSS_HORIZON_LIVE_MCP_DATABASE")
            .expect("set GAUSS_HORIZON_LIVE_MCP_DATABASE to a visible database");
        let expected = crate::legacy::var("GAUSS_HORIZON_LIVE_MCP_EXPECTED_VALUE")
            .expect("set GAUSS_HORIZON_LIVE_MCP_EXPECTED_VALUE");
        let mcp_command = crate::legacy::var("GAUSS_HORIZON_LIVE_MCP_COMMAND")
            .unwrap_or_else(|_| "gauss-horizon-mcp-server".to_string());
        let options = CodeBuddyRunOptions {
            connection_id: String::new(),
        ...
        let result = tokio::time::timeout(
            ...
            run_codebuddy_agent(
                &config,
                "Use the Gauss Horizon MCP execute-query tool to run exactly this read-only SQL: SELECT metric_value FROM codebuddy_e2e_metrics WHERE metric_name = 'verified_rows'. Reply with the returned numeric value only.",
                options,
                &Notify::new(),
                move |event| captured.lock().unwrap().push(event),
            ),
        ...
        assert!(
            events.iter().any(
                |event| matches!(event, AgentEvent::ToolCallStart { tool_name, .. } if tool_name.contains("gauss_horizon_execute_query"))
            ),
            "CodeBuddy did not invoke Gauss Horizon execute-query; result={result:?}, events={events:#?}"
        );
