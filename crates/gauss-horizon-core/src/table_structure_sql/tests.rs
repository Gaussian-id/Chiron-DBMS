...
fn mysql_table_engine_change_generates_alter_table() {
    let mut options =
        structure_change_options(DatabaseType::Mysql, Some("gauss_horizon_test"), "remote_orders", Vec::new());
...
fn mysql_create_table_includes_engine_before_comment() {
    let mut options =
        structure_change_options(DatabaseType::Mysql, Some("gauss_horizon_test"), "archive", vec![column("id")]);
...
fn dameng_replaces_same_name_index_before_validating_uniqueness() {
    ...
    options.table_name = "GAUSS_HORIZON_6002_USERS".to_string();
    let result = build_table_structure_change_sql(options);

    assert_eq!(result.warnings, Vec::<String>::new());
    assert_eq!(
        result.statements,
        vec!["CREATE OR REPLACE UNIQUE INDEX \"IDX_USERS_EMAIL\" ON \"SYSDBA\".\"GAUSS_HORIZON_6002_USERS\" (\"EMAIL\");"]
    );
}
...
fn dameng_standalone_unique_index_keeps_index_level_ddl() {
    ...
    // Editing it must stay on the index-level path, otherwise Gauss Horizon breaks the lifecycle of the
...
fn oracle_timestamp_default_precedes_nullability_in_modify_sql() {
    ...
    assert_eq!(
        result.statements,
        vec!["ALTER TABLE \"GAUSS_HORIZON_TEST\".\"test\" MODIFY (\"time\" TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP);"]
    );
}
...
fn oracle_create_table_preserves_character_length_units() {
    ...
    let result = build_create_table_sql(TableStructureSqlOptions {
        database_type: Some(DatabaseType::Oracle),
        driver_profile: None,
        schema: Some("GAUSS_HORIZON_APP".to_string()),
        table_name: "GAUSS_HORIZON_ISSUE_4739".to_string(),
...
fn oracle_alter_column_preserves_character_length_unit() {
    ...
    assert_eq!(
        result.statements,
        vec!["ALTER TABLE \"GAUSS_HORIZON_APP\".\"GAUSS_HORIZON_ISSUE_4739\" MODIFY (\"DISPLAY_NAME\" VARCHAR2(64 CHAR));"]
    );
}
...
fn oracle_timestamp_precision_change_does_not_repeat_unchanged_nullability() {
    ...
    assert_eq!(result.warnings, Vec::<String>::new());
    assert_eq!(result.statements, vec!["ALTER TABLE \"GAUSS_HORIZON_TEST\".\"test\" MODIFY (\"time\" TIMESTAMP(9));"]);
}
...
fn iris_ignores_comment_changes_but_keeps_supported_column_alters() {
    ...
    let result = build_table_structure_change_sql(TableStructureSqlOptions {
        ...
        driver_profile: None,
        schema: Some("SQLUSER".to_string()),
        table_name: "GAUSS_HORIZON_ISSUE_1678".to_string(),
    ...
    assert_eq!(
        result.statements,
        vec![
            "ALTER TABLE \"SQLUSER\".\"GAUSS_HORIZON_ISSUE_1678\" ALTER COLUMN \"NAME\" RENAME \"DISPLAY_NAME\";",
            "ALTER TABLE \"SQLUSER\".\"GAUSS_HORIZON_ISSUE_1678\" MODIFY (\"DISPLAY_NAME\" VARCHAR(40) DEFAULT 'after' NULL);",
            "ALTER TABLE \"SQLUSER\".\"GAUSS_HORIZON_ISSUE_1678\" ADD (\"CREATED_AT\" TIMESTAMP DEFAULT CURRENT_TIMESTAMP);",
        ]
    );
    assert_eq!(
        result.warnings,
...
fn sqlserver_type_change_preserves_existing_default_constraint() {
    ...
    assert!(sql.contains("QUOTENAME(@gauss_horizon_default_sql_"));
    assert!(sql.contains("+ N' DEFAULT ' + @gauss_horizon_default_sql_"));
...
fn rejects_editing_existing_oracle_trigger_without_complete_source() {
    ...
    let result = build_table_structure_change_sql(TableStructureSqlOptions {
        ...
        driver_profile: None,
        schema: Some("APP".to_string()),
        table_name: "GAUSS_HORIZON_TRIGGER_4320".to_string(),
        columns: Vec::new(),
        indexes: Vec::new(),
        foreign_keys: Vec::new(),
        triggers: vec![existing],
    ...
    assert_eq!(
        result.warnings,
        vec!["Editing existing Oracle trigger \"GAUSS_HORIZON_TRIGGER_4320_AUDIT\" requires its complete source definition."]
    );
}
...
fn oscar_drop_index_with_schema_qualifier() {
    let mut idx = index("GAUSS_HORIZON_PROBE_IDX", &["NAME"]);
    idx.marked_for_drop = true;
    idx.original = Some(IndexInfo {
        name: "GAUSS_HORIZON_PROBE_IDX".to_string(),
        columns: vec!["NAME".to_string()],
        is_unique: false,
        is_primary: false,
        filter: None,
    ...
    assert_eq!(result.warnings, Vec::<String>::new());
    assert_eq!(result.statements, vec!["DROP INDEX \"SYSDBA\".\"GAUSS_HORIZON_PROBE_IDX\";"]);
}
