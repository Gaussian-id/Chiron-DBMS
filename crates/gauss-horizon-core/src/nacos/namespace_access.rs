...
/// Access-control changes can affect a different Gauss Horizon connection that targets
...
fn namespace_authorization_unavailable(detail: &str) -> String {
    ...
    format!(
        "NACOS_ERROR[{error_code}]: Gauss Horizon cannot safely determine readable namespaces without per-namespace probes: {detail}"
    )
}
