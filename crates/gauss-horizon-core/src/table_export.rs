...
mod tests {
    ...
    async fn external_driver_export_fixture(
        ...
        let dir = std::env::temp_dir().join(format!("gauss-horizon-jdbc-table-export-test-{}", uuid::Uuid::new_v4()));
        ...
        let server = tokio::spawn(async move {
            ...
            loop {
                ...
                if query != "SHOW DATABASES" {
                    assert_eq!(table_export_query_param(target, "db").as_deref(), Some("gauss_horizon_issue_8022"));
                }
                let final_page = query.ends_with("ORDER BY \"time\" LIMIT 1 OFFSET 2");
                let body = match query.as_str() {
                    "SHOW DATABASES" => {
                        r#"{"results":[{"statement_id":0,"series":[{"name":"databases","columns":["name"],"values":[["gauss_horizon_issue_8022"]]}]}]}"#
                    }
    ...
    async fn run_influxdb_table_export_exports_paginated_rows() {
        ...
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-influxdb-table-export-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let config: ConnectionConfig = serde_json::from_value(json!({
            ...
            "username": "",
            "password": "",
            "database": "gauss_horizon_issue_8022",
            "query_timeout_secs": 30
        }))
        .unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        ...
        let request = TableExportRequest {
            ...
            database: "gauss_horizon_issue_8022".to_string(),
            schema: None,
            identifier_quote: None,
    ...
    fn oracle_requested_export_columns_omit_synthetic_rowid_and_keep_metadata_aligned() {
        let columns = vec!["__GAUSS_HORIZON_ROWID".to_string(), "ID".to_string(), "NAME".to_string()];
        let column_types = vec![Some("VARCHAR2".to_string()), Some("NUMBER".to_string()), Some("VARCHAR2".to_string())];
        let primary_keys = vec!["__GAUSS_HORIZON_ROWID".to_string()];
    ...
    fn requested_export_columns_preserve_regular_oracle_and_non_oracle_columns() {
        ...
        let mysql_columns = vec!["__GAUSS_HORIZON_ROWID".to_string(), "name".to_string()];
