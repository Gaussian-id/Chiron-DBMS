...
const ENCRYPTED_SNIPPET_SNAPSHOT_FORMAT: &str = "gauss-horizon-encrypted-sync-snapshot";
const ENCRYPTED_SNIPPET_SNAPSHOT_VERSION: u32 = 1;
const DEFAULT_REMOTE_PATH: &str = "Gauss Horizon/sync/snapshot.json";
const DEFAULT_SNIPPET_FILE_NAME: &str = "gauss-horizon-sync.json";
    ...
    pub async fn put_snapshot(
        ...
        let legacy_snapshot = if let Some(id) = existing_id {
            ...
            if !is_encrypted_snippet_snapshot(&existing_content) {
                ...
                // encrypted Gauss Horizon envelope. It must first prove to be a legacy Gauss Horizon
                // snapshot, and the caller must explicitly request migration.
                if !is_legacy_gauss_horizon_snapshot(&existing_content) {
                    return Err(
                        "The selected snippet is not a Gauss Horizon sync snapshot; refusing to replace or delete it."
                            .to_string(),
                    );
                }
                if !self.config.replace_legacy_snippet {
                    return Err(
                        "This snippet contains a legacy unencrypted Gauss Horizon snapshot. Use the secure migration action to create an encrypted replacement and delete the legacy snippet only after the new one is created."
                            .to_string(),
                    );
                }
                let legacy_snapshot = parse_legacy_gauss_horizon_snapshot(&existing_content)?;
                Some((
                    id,
        ...
        let response = match self.config.provider {
            SnippetProvider::GitHub => {
                let payload = serde_json::json!({
                    "description": "Gauss Horizon encrypted configuration sync",
                    "public": false,
    ...
    fn request(&self, method: Method, url: &str) -> Result<reqwest::RequestBuilder, String> {
        ...
        Ok(match self.config.provider {
            SnippetProvider::GitHub => request
                ...
                .header(header::USER_AGENT, "Gauss Horizon")
...
fn parse_snippet_snapshot(content: &str, secrets_passphrase: Option<&str>) -> Result<SyncSnapshot, String> {
    if is_encrypted_snippet_snapshot(content) {
        ...
        if envelope.format != ENCRYPTED_SNIPPET_SNAPSHOT_FORMAT && envelope.format != crate::legacy::OLD_SYNC_FORMAT
...
fn is_encrypted_snippet_snapshot(content: &str) -> bool {
    serde_json::from_str::<EncryptedSnippetSnapshot>(content).ok().is_some_and(|envelope| {
        envelope.format == ENCRYPTED_SNIPPET_SNAPSHOT_FORMAT || envelope.format == crate::legacy::OLD_SYNC_FORMAT
    })
}

/// Keep the migration guard deliberately more tolerant than deserializing the
/// current `SyncSnapshot`: older Gauss Horizon releases may not contain fields added
/// since their snapshot was written. At the same time, require the stable Gauss Horizon
/// snapshot markers before a destructive remote delete is allowed.
fn is_legacy_gauss_horizon_snapshot(content: &str) -> bool {
...
fn parse_legacy_gauss_horizon_snapshot(content: &str) -> Result<SyncSnapshot, String> {
    if !is_legacy_gauss_horizon_snapshot(content) {
        return Err(
            "The selected snippet is not a Gauss Horizon sync snapshot; refusing to replace or delete it.".to_string()
        );
    }
    serde_json::from_str(content).map_err(|_| {
        "The legacy Gauss Horizon snapshot is incompatible with this version, so it will not be replaced or deleted.".to_string()
    })
}
...
fn gitee_snippet_payload(content: String) -> serde_json::Value {
    serde_json::json!({
        "description": "Gauss Horizon configuration sync",
        "public": false,
        "files": { DEFAULT_SNIPPET_FILE_NAME: { "content": content } }
    })
}
...
mod tests {
    use super::{
        ...
        gitee_snippet_payload, is_legacy_gauss_horizon_snapshot, normalized_remote_path, parent_collection_paths,
        parse_legacy_gauss_horizon_snapshot, parse_snippet_snapshot, prepare_legacy_snippet_snapshot,
    ...
    fn temp_db_path(name: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!("gauss-horizon-cloud-sync-{name}-{}.db", uuid::Uuid::new_v4()))
    }
    ...
    fn gitee_snippet_payload_keeps_files_as_nested_object() {
        ...
        assert_eq!(payload["files"]["gauss-horizon-sync.json"]["content"], "snapshot");
    ...
    fn normalizes_empty_remote_path_to_default() {
        assert_eq!(normalized_remote_path(None), "Gauss Horizon/sync/snapshot.json");
        assert_eq!(normalized_remote_path(Some("")), "Gauss Horizon/sync/snapshot.json");
        assert_eq!(normalized_remote_path(Some("///\\\\//")), "Gauss Horizon/sync/snapshot.json");
    }
    ...
    fn normalizes_remote_path_separators() {
        ...
        assert_eq!(
            normalized_remote_path(Some(r"\Gauss Horizon\sync\snapshot.json")),
            "Gauss Horizon/sync/snapshot.json"
        );
        assert_eq!(
            normalized_remote_path(Some("///Gauss Horizon//sync/./snapshot.json")),
            "Gauss Horizon/sync/snapshot.json"
        );
        assert_eq!(normalized_remote_path(Some("Gauss Horizon/sync/../snapshot.json")), "Gauss Horizon/snapshot.json");
    }

    #[test]
    fn bypasses_proxy_for_local_webdav_endpoints() {
        for endpoint in [
            "http://172.27.31.29:8088/gauss-horizon/",
    ...
    fn returns_parent_collection_paths_from_leaf() {
        assert_eq!(
            parent_collection_paths("gauss-horizon/sync/snapshot.json"),
            vec!["gauss-horizon".to_string(), "gauss-horizon/sync".to_string()]
        );
        assert_eq!(
            parent_collection_paths(&normalized_remote_path(Some(r"\Gauss Horizon\sync\snapshot.json"))),
            vec!["Gauss Horizon".to_string(), "Gauss Horizon/sync".to_string()]
        );
    }
    ...
    async fn webdav_upload_terminates_collection_request_paths() {
        ...
        let client = WebDavClient::new(WebDavConfig {
            ...
            username: None,
            password: None,
            remote_path: Some("Gauss Horizon-home/sync/snapshot.json".to_string()),
        });
        ...
        assert_eq!(
            server.await.unwrap(),
            vec![
                "MKCOL /Gauss%20Horizon-home/ HTTP/1.1",
                "MKCOL /Gauss%20Horizon-home/sync/ HTTP/1.1",
                "PUT /Gauss%20Horizon-home/sync/snapshot.json HTTP/1.1",
            ]
        );
    }
    ...
    async fn webdav_upload_preserves_collection_conflicts() {
        ...
        assert_eq!(error, "Failed to create WebDAV collection 'Gauss Horizon-home' with HTTP 409 Conflict");
        assert_eq!(server.await.unwrap(), vec!["MKCOL /Gauss%20Horizon-home/ HTTP/1.1"]);
    }
        ...
        let mut config = ConnectionConfig {
            ...
            transport_layers: vec![
                ...
                TransportLayerConfig::HttpTunnel(crate::models::connection::HttpTunnelConfig {
                    ...
                    name: String::new(),
                    enabled: true,
                    url: "https://gauss-horizon.example.com/gauss_horizon_tunnel.php".to_string(),
    ...
    async fn legacy_snippet_migration_preserves_remote_snapshot() {
        ...
        // This field did not exist in snapshots written by older Gauss Horizon versions.
        legacy.as_object_mut().unwrap().remove("tunnelProfiles");

        let content = serde_json::to_string(&legacy).unwrap();
        assert!(is_legacy_gauss_horizon_snapshot(&content));
        let parsed_legacy = parse_legacy_gauss_horizon_snapshot(&content).unwrap();
        ...
        assert!(!is_legacy_gauss_horizon_snapshot(r#"{"schemaVersion":1,"connections":[]}"#));
        assert!(parse_legacy_gauss_horizon_snapshot(r#"{"schemaVersion":1,"connections":[]}"#).is_err());
    }
    ...
    async fn legacy_snippet_migration_refuses_unverifiable_encrypted_secrets() {
        ...
        let legacy = parse_legacy_gauss_horizon_snapshot(&content).unwrap();
    ...
    fn snippet_file_content_uses_raw_url_for_truncated_github_files() {
        ...
        assert_eq!(
            snippet_file_content(&value, "gauss-horizon-sync.json").unwrap(),
    ...
    fn snippet_file_content_parses_gitee_string_files() {
        let files = serde_json::json!({ "gauss-horizon-sync.json": { "content": "{}" } }).to_string();
        let value = serde_json::json!({ "files": files });
        assert_eq!(snippet_file_content(&value, "gauss-horizon-sync.json").unwrap(), (Some("{}".to_string()), None));
    }
