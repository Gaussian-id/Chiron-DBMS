...
fn qualifies_schema_only_for_schema_aware_databases() {
    ...
    assert_eq!(
        qualified_table_name(Some(DatabaseType::Databend), Some("gauss_horizon_test"), "users"),
        "`gauss_horizon_test`.`users`"
    );
    assert_eq!(
        qualified_table_name(Some(DatabaseType::Xugu), Some("GAUSS_HORIZON_TEST"), "PRODUCTS"),
        "\"GAUSS_HORIZON_TEST\".\"PRODUCTS\""
    );
    ...
    // ArgoDB parses as a string literal — see gauss-horizon-argo-double-quote-bug memory note).
    ...
    assert_eq!(
        qualified_table_name(
            ...
            Some("__gauss_horizon_sqlserver_linked__:ERP%5D01|Finance%20DB|dbo"),
            "Orders]2026"
        ),
...
fn builds_select_sql_with_limit_syntax_for_database_type() {
    ...
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
            ...
            schema: Some("GaussHorizonTEST"),
            table_name: "USERS",
            columns: &columns,
            order_columns: &keys,
            limit: 100,
        }),
        "SELECT \"id\", \"name\" FROM (SELECT \"id\", \"name\" FROM \"GaussHorizonTEST\".\"USERS\" ORDER BY \"id\" ASC) WHERE ROWNUM <= 100"
    );
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
    ...
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
            ...
            schema: Some("gauss_horizon_test"),
            table_name: "jdbc_probe",
            columns: &[],
            order_columns: &[],
            limit: 500,
        }),
        "SELECT * FROM `gauss_horizon_test`.`jdbc_probe` LIMIT 500;"
    );
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
        ...
        "SELECT * FROM \"GAUSS_HORIZON_TEST\".\"PRODUCTS\" LIMIT 100;"
    );
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
    ...
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
            ...
            schema: Some("gauss_horizon_demo"),
            table_name: "connection_test",
            columns: &[],
            order_columns: &[],
            limit: 100,
        }),
        "SELECT * FROM `gauss_horizon_demo`.`connection_test` LIMIT 100;"
    );
    assert_eq!(
        build_table_select_sql(TableSelectSqlOptions {
...
fn builds_table_data_where_and_schema_queries() {
    ...
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Impala),
            schema: Some("gauss_horizon_demo".to_string()),
        ...
        "SELECT `id`, `name` FROM `gauss_horizon_demo`.`connection_test` ORDER BY 1 LIMIT 2 OFFSET 1;"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
    ...
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Oracle),
            schema: Some("GaussHorizonTEST".to_string()),
            table_name: "ORDERS".to_string(),
            table_type: None,
            primary_keys: vec!["ID".to_string()],
            columns: vec!["ID".to_string(), "AMOUNT".to_string()],
        ...
        "SELECT \"ID\", \"AMOUNT\" FROM (SELECT gauss_horizon_inner.*, ROWNUM AS \"__gauss_horizon_row_num\" FROM (SELECT \"ID\", \"AMOUNT\" FROM \"GaussHorizonTEST\".\"ORDERS\" WHERE (amount > 10) ORDER BY \"ID\" ASC) gauss_horizon_inner WHERE ROWNUM <= 150) WHERE \"__gauss_horizon_row_num\" > 100"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::OceanbaseOracle),
        ...
        "SELECT * FROM (SELECT * FROM \"GaussHorizonTEST\".\"ORDERS\") WHERE ROWNUM <= 100"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::OceanbaseOracle),
        ...
        "SELECT * FROM (SELECT * FROM \"GaussHorizonTEST\".\"ORDERS\" WHERE (amount > 10)) WHERE ROWNUM <= 50"
    );
    assert_eq!(
            build_table_data_select_sql(TableDataSelectSqlOptions {
                database_type: Some(DatabaseType::Db2),
            ...
            "SELECT \"ID\", \"AMOUNT\" FROM (SELECT gauss_horizon_t.\"ID\", gauss_horizon_t.\"AMOUNT\", ROW_NUMBER() OVER () AS \"__gauss_horizon_row_num\" FROM \"DB2INST1\".\"ORDERS\" gauss_horizon_t WHERE (amount > 10)) gauss_horizon_page WHERE \"__gauss_horizon_row_num\" > 100 AND \"__gauss_horizon_row_num\" <= 150 ORDER BY \"__gauss_horizon_row_num\""
        );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Iris),
...
fn builds_mysql_table_data_large_value_previews_without_truncating_keys() {
    ...
    assert!(sql.contains("CONCAT('T:4096:', LENGTH(`payload`)) AS `__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1`"));
    assert!(sql.contains("LEFT(`raw_value`, 4097) AS `raw_value`"));
    assert!(sql.contains("CONCAT('B:4096:', LENGTH(`raw_value`)) AS `__GAUSS_HORIZON_LARGE_VALUE_BYTES_B_2`"));
    assert!(sql.contains("LEFT(`metadata`, 4097) AS `metadata`"));
    assert!(sql.contains("CONCAT('T:4096:', LENGTH(`metadata`)) AS `__GAUSS_HORIZON_LARGE_VALUE_BYTES_J_3`"));
    assert!(!sql.contains("OCTET_LENGTH"));
    assert!(!sql.contains("__GAUSS_HORIZON_LARGE_VALUE_BYTES_0"));
}
...
fn previews_mysql_bounded_string_columns_only_above_the_active_budget() {
    ...
    assert!(sql.contains("CONCAT('B:419:', LENGTH(`image_data`)) AS `__GAUSS_HORIZON_LARGE_VALUE_BYTES_B_2`, LEFT(`image_url`, 420) AS `image_url`"));
    assert!(sql.contains("CONCAT('T:419:', LENGTH(`image_url`)) AS `__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_3`"));
    assert!(sql.contains("LEFT(`large_note`, 420) AS `large_note`"));
    assert!(sql.contains("CONCAT('T:419:', LENGTH(`large_note`)) AS `__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_4`"));
...
fn builds_postgres_table_data_large_value_previews() {
    ...
    assert!(sql.contains("'T:8192' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_1\""));
    assert!(sql.contains("left(\"metadata\"::text, 8193) AS \"metadata\""));
    assert!(sql.contains("'T:8192' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_K_2\""));
}
...
fn preserves_postgres_array_types_in_large_value_previews() {
    ...
    for index in 1..=6 {
        assert!(!sql.contains(&format!("__GAUSS_HORIZON_LARGE_VALUE_BYTES_T_{index}\"")));
        assert!(!sql.contains(&format!("__GAUSS_HORIZON_LARGE_VALUE_BYTES_B_{index}\"")));
        assert!(!sql.contains(&format!("__GAUSS_HORIZON_LARGE_VALUE_BYTES_K_{index}\"")));
        assert!(!sql.contains(&format!("__GAUSS_HORIZON_LARGE_VALUE_BYTES_V_{index}\"")));
    }
    ...
    assert!(sql.contains("'T:8' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_J_10\""));
    assert!(sql.contains("'T:8' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_K_11\""));
    assert!(sql.contains("'T:8' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_S_12\""));
    assert!(sql.contains("'V:8' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_V_13\""));
    assert!(sql.contains("'B:8' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_B_14\""));
}
...
fn builds_pgvector_table_data_preview_with_compatible_text_cast() {
    ...
    assert!(sql.contains("'V:8192' AS \"__GAUSS_HORIZON_LARGE_VALUE_BYTES_V_1\""));
}
...
fn table_data_preview_requires_stable_keys_and_parallel_types() {
    ...
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            primary_keys: vec!["id".to_string()],
            columns: vec!["id".to_string(), "__gauss_horizon_large_value_bytes_t_0".to_string()],
...
fn builds_sqlserver_table_data_pages() {
    ...
    assert_eq!(
            ...
            "WITH [gauss_horizon_page] AS (SELECT [order_id], [customer], ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS [__gauss_horizon_row_num] FROM [sales].[orders]) SELECT [order_id], [customer] FROM [gauss_horizon_page] WHERE [__gauss_horizon_row_num] > 100 AND [__gauss_horizon_row_num] <= 150 ORDER BY [__gauss_horizon_row_num]"
        );
}
...
fn builds_oracle_and_neo4j_table_data_queries() {
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Oracle),
            schema: Some("GaussHorizonTEST".to_string()),
            table_name: "GAUSS_HORIZON_LOAD_TABLE_006".to_string(),
            table_type: Some("TABLE".to_string()),
        ...
        "SELECT * FROM (SELECT ROWIDTOCHAR(t.ROWID) AS \"__GAUSS_HORIZON_ROWID\", t.* FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_LOAD_TABLE_006\" t) WHERE ROWNUM <= 100"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Oracle),
        ...
        "SELECT \"__GAUSS_HORIZON_ROWID\", \"ID\", \"NAME\" FROM (SELECT ROWIDTOCHAR(t.ROWID) AS \"__GAUSS_HORIZON_ROWID\", t.* FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_LOAD_TABLE_006\" t) WHERE ROWNUM <= 100"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::OceanbaseOracle),
        ...
        "SELECT \"__GAUSS_HORIZON_ROWID\", \"ID\", \"SMC_RESPONSE\" FROM (SELECT ROWIDTOCHAR(t.ROWID) AS \"__GAUSS_HORIZON_ROWID\", t.* FROM \"APP\".\"DATA_REPORT_SUB_TASK\" t) WHERE ROWNUM <= 100"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Xugu),
        ...
        "SELECT ROWID AS \"__GAUSS_HORIZON_ROWID\", * FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_LOAD_TABLE_006\" LIMIT 100;"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Xugu),
        ...
        "SELECT ROWID AS \"__GAUSS_HORIZON_ROWID\", \"ID\", \"NAME\" FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_LOAD_TABLE_006\" LIMIT 25 OFFSET 10;"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Xugu),
            schema: Some("GaussHorizonTEST".to_string()),
            table_name: "GAUSS_HORIZON_JOIN_VIEW".to_string(),
        ...
        "SELECT * FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_JOIN_VIEW\" LIMIT 100;"
    );
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Oracle),
        ...
        "SELECT \"ID\", \"NAME\" FROM (SELECT \"ID\", \"NAME\" FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_JOIN_VIEW\") WHERE ROWNUM <= 100"
    );
    assert_eq!(
            build_table_data_select_sql(TableDataSelectSqlOptions {
            ...
            "MATCH (n:`Employee`) RETURN elementId(n) AS `__GAUSS_HORIZON_ELEMENT_ID`, n.`id` AS `id`, n.`first name` AS `first name`, n.`role` AS `role` LIMIT 100;"
        );
}
...
fn oracle_unknown_table_type_does_not_assume_rowid_support() {
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            database_type: Some(DatabaseType::Oracle),
            schema: Some("GaussHorizonTEST".to_string()),
            table_name: "GAUSS_HORIZON_DISTINCT_VIEW".to_string(),
            table_type: None,
            primary_keys: vec![GAUSS_HORIZON_ROWID_COLUMN.to_string()],
            columns: vec!["ID".to_string(), "NAME".to_string()],
            limit: Some(100),
        ...
        "SELECT \"ID\", \"NAME\" FROM (SELECT gauss_horizon_inner.*, ROWNUM AS \"__gauss_horizon_row_num\" FROM (SELECT \"ID\", \"NAME\" FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_DISTINCT_VIEW\") gauss_horizon_inner WHERE ROWNUM <= 200) WHERE \"__gauss_horizon_row_num\" > 100"
    );
}
...
fn builds_oracle_rowid_wrapped_large_value_reload_sql() {
    ...
    // target column with a rowid equality filter; `__GAUSS_HORIZON_ROWID` exists only as
    ...
    assert_eq!(
        build_table_data_select_sql(TableDataSelectSqlOptions {
            ...
            table_type: Some("TABLE".to_string()),
            primary_keys: vec![GAUSS_HORIZON_ROWID_COLUMN.to_string()],
            columns: vec![GAUSS_HORIZON_ROWID_COLUMN.to_string(), "ELM_CONTENT".to_string()],
        ...
        "SELECT \"__GAUSS_HORIZON_ROWID\", \"ELM_CONTENT\" FROM (SELECT ROWIDTOCHAR(t.ROWID) AS \"__GAUSS_HORIZON_ROWID\", t.* FROM \"APP\".\"T_TEST\" t WHERE (ROWIDTOCHAR(ROWID) = 'AAAFd1AAFAAAABSAA/')) WHERE ROWNUM <= 1"
    );
}
...
fn oracle_view_first_page_is_bounded_and_preserves_filter_and_sort() {
    assert_eq!(
        ...
        "SELECT \"ID\", \"NAME\" FROM (SELECT \"ID\", \"NAME\" FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_JOIN_VIEW\" WHERE (STATUS = 'A') ORDER BY \"ID\" DESC) WHERE ROWNUM <= 100"
    );
}
...
fn oracle_view_later_pages_keep_rownum_pagination() {
    assert_eq!(
        ...
        "SELECT \"ID\", \"NAME\" FROM (SELECT gauss_horizon_inner.*, ROWNUM AS \"__gauss_horizon_row_num\" FROM (SELECT \"ID\", \"NAME\" FROM \"GaussHorizonTEST\".\"GAUSS_HORIZON_JOIN_VIEW\" ORDER BY \"ID\" DESC) gauss_horizon_inner WHERE ROWNUM <= 200) WHERE \"__gauss_horizon_row_num\" > 100"
    );
}
