...
async fn run() -> Result<(), String> {
    ...
    let official = results
        ...
        .ok_or("Official Gauss Horizon Marketplace result is missing")?;
    if let Some(error) = &official.error {
        return Err(error.clone());
    }
    let catalog = official.catalog.as_ref().ok_or("Official Gauss Horizon Marketplace catalog is missing")?;
    ...
    if !catalog.plugins.iter().any(|plugin| plugin.id == plugin_id) {
        return Err(format!("Official Gauss Horizon Marketplace does not list '{plugin_id}'"));
    }
