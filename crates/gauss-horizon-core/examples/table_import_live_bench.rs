...
Usage:\n  cargo run -p gauss-horizon-core --example table_import_live_bench --release -- [options]\n\n\
Options:\n\
...
Connection environment:\n\
  GAUSS_HORIZON_BENCH_HOST, GAUSS_HORIZON_BENCH_PORT, GAUSS_HORIZON_BENCH_USER, GAUSS_HORIZON_BENCH_PASSWORD,\n\
  GAUSS_HORIZON_BENCH_DATABASE, GAUSS_HORIZON_BENCH_SCHEMA, GAUSS_HORIZON_BENCH_SSL"
    );
}
...
fn env_required(name: &str) -> Result<String, String> {
    gauss_horizon_core::legacy::var(name).map_err(|_| format!("Missing environment variable: {name}"))
}

fn connection_config(id: &str, database: BenchDatabase) -> Result<ConnectionConfig, String> {
    let database_name = env_required("GAUSS_HORIZON_BENCH_DATABASE")?;
    Ok(ConnectionConfig {
        ...
        host: env_required("GAUSS_HORIZON_BENCH_HOST")?,
        port: gauss_horizon_core::legacy::var("GAUSS_HORIZON_BENCH_PORT")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or_else(|| database.default_port()),
        username: env_required("GAUSS_HORIZON_BENCH_USER")?,
        password: env_required("GAUSS_HORIZON_BENCH_PASSWORD")?,
        ...
        ssl: gauss_horizon_core::legacy::var("GAUSS_HORIZON_BENCH_SSL")
...
async fn execute_sql(
    ...
    gauss_horizon_core::query::execute_sql_statement(state, connection_id, database, sql, Some(schema), None)
        .await
        .map(|_| ())
}
...
async fn run() -> Result<(), String> {
    ...
    let database_name = env_required("GAUSS_HORIZON_BENCH_DATABASE")?;
    let schema = env_required("GAUSS_HORIZON_BENCH_SCHEMA")?;
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let connection_id = format!("table-import-live-bench-{suffix}");
    let table = format!("gauss_horizon_import_bench_{}", &suffix[..12]);
    let temp_dir = tempfile::Builder::new()
        .prefix(&format!("gauss-horizon-table-import-live-bench-{suffix}-"))
        .tempdir()
        .map_err(|error| error.to_string())?;
