...
const PLUGIN_ID: &str = "gauss.horizon.example.hello";
...
async fn run() -> Result<(), String> {
    let package =
        std::env::args().nth(1).map(PathBuf::from).ok_or("Usage: plugin_package_smoke <package.gauss-horizonp>")?;
    let store = tempfile::tempdir().map_err(|error| error.to_string())?;
    let trusted_keys = gauss_horizon_core::legacy::var("GAUSS_HORIZON_PLUGIN_SMOKE_TRUSTED_KEYS_JSON")
        .ok()
    ...
    let provider_icon = installed
        .plugin
        .manifest
        .connection_provider("gauss.horizon.example.hello.connection")?
    ...
    let files = host
        .list_filesystem_entries(
            PLUGIN_ID,
            "gauss.horizon.example.hello.files",
            Some(&config.id),
            Some("hello:/"),
            None,
            Some(20),
