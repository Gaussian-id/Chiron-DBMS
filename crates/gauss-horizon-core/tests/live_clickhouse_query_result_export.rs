...
use gauss_horizon_core::table_import::parse_xlsx_file;
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_CLICKHOUSE_* env vars pointing at a ClickHouse HTTP endpoint"]
async fn live_clickhouse_query_result_export_xlsx_streams_random_order_query_once() {
    let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_CLICKHOUSE_HOST")
        .expect("GAUSS_HORIZON_LIVE_CLICKHOUSE_HOST");
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_CLICKHOUSE_PORT")
        .expect("GAUSS_HORIZON_LIVE_CLICKHOUSE_PORT")
        .parse::<u16>()
        .unwrap();
    let user = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_CLICKHOUSE_USER")
        .expect("GAUSS_HORIZON_LIVE_CLICKHOUSE_USER");
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_CLICKHOUSE_PASSWORD")
        .expect("GAUSS_HORIZON_LIVE_CLICKHOUSE_PASSWORD");
    let database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_CLICKHOUSE_DATABASE")
        .expect("GAUSS_HORIZON_LIVE_CLICKHOUSE_DATABASE");
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-clickhouse-query-export-{suffix}"));
