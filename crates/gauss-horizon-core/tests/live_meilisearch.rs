use gauss_horizon_core::db::meilisearch_driver::{
...
fn live_client() -> MeilisearchClient {
    let url = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MEILISEARCH_URL")
        .expect("GAUSS_HORIZON_LIVE_MEILISEARCH_URL");
    let api_key = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MEILISEARCH_API_KEY").ok();
...
fn unique_index() -> String {
    ...
    format!("gauss_horizon_live_{nanos}")
}
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_MEILISEARCH_URL and optional GAUSS_HORIZON_LIVE_MEILISEARCH_API_KEY"]
    ...
    let affected = save_document_batch(
        ...
        &[
            MeilisearchDocumentUpdate {
                id: "__gauss_horizon_meilisearch_string_id__\"001\"".to_string(),
