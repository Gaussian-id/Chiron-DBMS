...
//!   GAUSS_HORIZON_TEST_OPENGAUSS_HOST=127.0.0.1 \
//!   GAUSS_HORIZON_TEST_OPENGAUSS_PORT=5432 \
//!   GAUSS_HORIZON_TEST_OPENGAUSS_USER=test \
//!   GAUSS_HORIZON_TEST_OPENGAUSS_PASSWORD=secret \
//!   GAUSS_HORIZON_TEST_OPENGAUSS_DATABASE=postgres \
//!   cargo test -p gauss-horizon-core --test live_opengauss_constraints -- --ignored --nocapture

use std::time::Duration;

use gauss_horizon_core::db::postgres;
use gauss_horizon_core::types::ConstraintInfo;
...
fn opengauss_url() -> String {
    let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_OPENGAUSS_HOST")
        .expect("GAUSS_HORIZON_TEST_OPENGAUSS_HOST not set");
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_OPENGAUSS_PORT")
        .expect("GAUSS_HORIZON_TEST_OPENGAUSS_PORT not set");
    let user = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_OPENGAUSS_USER")
        .expect("GAUSS_HORIZON_TEST_OPENGAUSS_USER not set");
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_OPENGAUSS_PASSWORD")
        .expect("GAUSS_HORIZON_TEST_OPENGAUSS_PASSWORD not set");
    let database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_OPENGAUSS_DATABASE")
        .unwrap_or_else(|_| "postgres".to_string());
    let params = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_OPENGAUSS_URL_PARAMS")
        .unwrap_or_else(|_| "sslmode=disable".to_string());
...
#[tokio::test]
#[ignore = "requires a writable OpenGauss instance via GAUSS_HORIZON_TEST_OPENGAUSS_* variables"]
async fn opengauss_constraints_report_structural_metadata() {
    ...
    let schema = format!("gauss_horizon_og_constraints_{}", std::process::id());
