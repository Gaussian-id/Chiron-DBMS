...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_POSTGRES_HOST/PORT/USER/PASSWORD/DATABASE pointing at a live db"]
async fn dump_keycloak_fixture() {
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-dump-docs-fixture-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
    state.configs.write().await.insert(config.id.clone(), config.clone());

    let options = gauss_horizon_core::docs::CollectOptions {
    ...
    let snapshot_result = gauss_horizon_core::docs::collect_snapshot(
        &state,
        &config,
        &options,
        &|_progress| {},
    ...
    gauss_horizon_core::docs::annotations::apply_annotations(&mut snapshot, &annotations, DatabaseType::Postgres);

    let anchor = snapshot.tables.iter().find(|table| table.name == ANCHOR_TABLE).expect("anchor table collected");
    assert_eq!(
        anchor.note_source,
        gauss_horizon_core::docs::NoteSource::Local,
        "annotation setup must have taken effect"
    );
    assert!(
        snapshot
            .warnings
            .iter()
            .any(|warning| matches!(warning, gauss_horizon_core::docs::SnapshotWarning::OrphanedNotes { .. })),
    ...
    snapshot.relationships = gauss_horizon_core::docs::build_relationships(&snapshot.tables);
    assert!(!snapshot.relationships.is_empty(), "the allowlist must form a connected foreign-key subgraph");

    let json = serde_json::to_string_pretty(&snapshot).expect("serialize snapshot");

    let out_path = gauss_horizon_core::legacy::var("GAUSS_HORIZON_FIXTURE_OUT")
