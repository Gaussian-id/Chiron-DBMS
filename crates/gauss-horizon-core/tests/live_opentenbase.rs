...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_TEST_OPENTENBASE_URL pointing at a writable OpenTenBase Coordinator"]
async fn opentenbase_metadata_and_ddl_round_trip() {
    let url = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_OPENTENBASE_URL")
        .expect("GAUSS_HORIZON_TEST_OPENTENBASE_URL");
    let pool = db::postgres::connect(&url, Duration::from_secs(10)).await.expect("connect OpenTenBase Coordinator");
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("gauss_horizon_otb_source_{suffix}");
    let target_schema = format!("gauss_horizon_otb_target_{suffix}");
