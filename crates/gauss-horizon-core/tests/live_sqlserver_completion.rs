...
use gauss_horizon_core::query_result_export::{export_query_result_core, ExportStatus, QueryResultExportRequest};
use gauss_horizon_core::sql::{SqlFileRequest, SqlFileStatus};
use gauss_horizon_core::sql_file_import::execute_sql_file_content;
use gauss_horizon_core::storage::Storage;
use gauss_horizon_core::table_import::{
...
use gauss_horizon_core::xlsx_export::{build_xlsx_workbook, XlsxWorksheetData};
use std::sync::atomic::{AtomicBool, AtomicUsize, Ordering};
use std::sync::Arc;
use std::time::{Duration, Instant};
use tokio_util::sync::CancellationToken;

fn live_sqlserver_config(id: &str, database: &str) -> gauss_horizon_core::models::connection::ConnectionConfig {
    gauss_horizon_core::models::connection::ConnectionConfig {
        ...
        host: gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        port: gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(1433),
        username: gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_USER")
            .unwrap_or_else(|_| "sa".to_string()),
        password: gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD")
            .expect("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD"),
...
async fn live_sqlserver_import_state(
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-sqlserver-import-{suffix}"));
...
) -> Result<gauss_horizon_core::table_import::TableImportSummary, String> {
...
async fn run_live_sqlserver_generated_insert(
    client: &mut gauss_horizon_core::db::sqlserver::SqlServerClient,
    table: &str,
    file_path: &std::path::Path,
    ...
    if include_identity {
        ...
        gauss_horizon_core::db::sqlserver::execute_batch(client, &sql).await?;
        return Ok(rows_imported);
    }
    let mut rows_imported = 0;
    for batch in batches {
        gauss_horizon_core::db::sqlserver::execute_batch(client, &batch.sql).await?;
...
async fn live_sqlserver_matrix_rows(
    client: &mut gauss_horizon_core::db::sqlserver::SqlServerClient,
    table: &str,
) -> gauss_horizon_core::db::QueryResult {
    gauss_horizon_core::db::sqlserver::execute_query(
        client,
        &format!(
...
async fn live_sqlserver_table_count(
    client: &mut gauss_horizon_core::db::sqlserver::SqlServerClient,
    table: &str,
) -> i64 {
    let result =
        gauss_horizon_core::db::sqlserver::execute_query(client, &format!("SELECT COUNT_BIG(*) FROM [dbo].[{table}]"))
            .await
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD pointing at a writable SQL Server database"]
async fn live_sqlserver_bulk_matches_generated_insert_type_and_constraint_matrix() {
    let database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let connection_id = format!("live-sqlserver-matrix-{suffix}");
    let bulk_table = format!("gauss_horizon_bulk_matrix_{suffix}");
    let generated_table = format!("gauss_horizon_insert_matrix_{suffix}");
    let bulk_audit = format!("gauss_horizon_bulk_matrix_audit_{suffix}");
    let generated_audit = format!("gauss_horizon_insert_matrix_audit_{suffix}");
    let bulk_trigger = format!("gauss_horizon_bulk_matrix_trigger_{suffix}");
    let generated_trigger = format!("gauss_horizon_insert_matrix_trigger_{suffix}");
    let mut client = gauss_horizon_core::db::sqlserver::connect(
        &gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(1433),
        &gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_USER").unwrap_or_else(|_| "sa".to_string()),
        &gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD")
            .expect("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD"),
        Some(&database),
        None,
    ...
    for ddl in [
        ...
        gauss_horizon_core::db::sqlserver::execute_batch(&mut client, &ddl)
            .await
    ...
    let bulk_audit_rows = gauss_horizon_core::db::sqlserver::execute_query(
        &mut client,
        &format!("SELECT [target_id], [code], [default_text] FROM [dbo].[{bulk_audit}] ORDER BY [target_id]"),
    )
    .await
    .expect("query SQL Server bulk audit rows");
    let generated_audit_rows = gauss_horizon_core::db::sqlserver::execute_query(
        &mut client,
    ...
    let _ = gauss_horizon_core::db::sqlserver::execute_batch(&mut client, &cleanup).await;
...
async fn live_sqlserver_bulk_imports_zero_fraction_xlsx_numbers_into_bigint() {
    ...
    let table = format!("gauss_horizon_bulk_xlsx_integer_{suffix}");
    let mut client = gauss_horizon_core::db::sqlserver::connect(
        &gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
    ...
    .await
    .expect("connect SQL Server");
    gauss_horizon_core::db::sqlserver::execute_batch(
        &mut client,
    ...
    let rows = gauss_horizon_core::db::sqlserver::execute_query(
        &mut client,
        &format!("SELECT [id], [label] FROM [dbo].[{table}]"),
    )
    .await
    .expect("query SQL Server XLSX integer row");

    let _ =
        gauss_horizon_core::db::sqlserver::execute_batch(&mut client, &format!("DROP TABLE IF EXISTS [dbo].[{table}]"))
            .await;
    state.remove_connection_pools_detached(&connection_id).await;
    let _ = std::fs::remove_dir_all(&dir);
...
async fn live_sqlserver_table_import_bulk_preserves_target_semantics() {
    ...
    let table = format!("gauss_horizon_bulk_target_{suffix}");
    let audit_table = format!("gauss_horizon_bulk_audit_{suffix}");
    let trigger = format!("gauss_horizon_bulk_trigger_{suffix}");
    let mut setup_client = gauss_horizon_core::db::sqlserver::connect(
        &gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
            .ok()
    ...
    let audit_count = gauss_horizon_core::db::sqlserver::execute_query(
        &mut setup_client,
    ...
    let _ = gauss_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &cleanup).await;
    state.remove_connection_pools_detached(&connection_id).await;
    let _ = std::fs::remove_dir_all(&dir);
...
async fn live_sqlserver_table_import_bulk_cancels_before_target_write_and_rolls_back_truncate() {
    ...
    let table = format!("gauss_horizon_bulk_rollback_{suffix}");
    let mut setup_client = gauss_horizon_core::db::sqlserver::connect(
        &gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
    ...
    let rows_after_failed_truncate = gauss_horizon_core::db::sqlserver::execute_query(
        &mut setup_client,
    ...
    .await;
    let rows_after_cancel = gauss_horizon_core::db::sqlserver::execute_query(
        &mut setup_client,
        &format!("SELECT [id], CONVERT(VARCHAR(50), [amount]) FROM [dbo].[{table}]"),
    )
    .await
    .expect("verify cancellation leaves target unchanged");
    let _ = gauss_horizon_core::db::sqlserver::execute_batch(
        &mut setup_client,
...
async fn live_sqlserver_column_metadata_marks_non_positional_insert_columns() {
    let database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST")
        .unwrap_or_else(|_| "127.0.0.1".to_string());
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(1433);
    let user =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_USER").unwrap_or_else(|_| "sa".to_string());
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD")
        .expect("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD");
    let mut client = gauss_horizon_core::db::sqlserver::connect(
        &host,
        port,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let computed_table = format!("gauss_horizon_insert_computed_{suffix}");
    let temporal_table = format!("gauss_horizon_insert_temporal_{suffix}");
    let history_table = format!("gauss_horizon_insert_history_{suffix}");
    ...
    gauss_horizon_core::db::sqlserver::execute_query(&mut client, &create_computed)
        .await
        .expect("create computed table");
    gauss_horizon_core::db::sqlserver::execute_query(&mut client, &create_temporal)
        .await
    ...
    let computed = gauss_horizon_core::db::sqlserver::get_column_metadata(&mut client, "dbo", &computed_table)
        .await
        .expect("read computed metadata");
    let temporal = gauss_horizon_core::db::sqlserver::get_column_metadata(&mut client, "dbo", &temporal_table)
        .await
...
async fn live_sqlserver_execute_query_creates_schema() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("gauss_horizon_schema_{suffix}");
    ...
    let result = gauss_horizon_core::db::sqlserver::execute_query(&mut client, &create).await;
    let verify_result = gauss_horizon_core::db::sqlserver::execute_query(&mut client, &verify).await;
    let schemas = gauss_horizon_core::db::sqlserver::list_schemas(&mut client).await;
    let _ = gauss_horizon_core::db::sqlserver::execute_query(&mut client, &cleanup).await;
...
async fn live_sqlserver_explicit_transaction_batch_can_rollback() {
    ...
    let table = format!("gauss_horizon_txn_{}", uuid::Uuid::new_v4().simple());
    gauss_horizon_core::db::sqlserver::execute_query(
        &mut client,
    ...
    let execution = gauss_horizon_core::db::sqlserver::execute_batch(
        &mut client,
        &format!("BEGIN TRANSACTION\nUPDATE dbo.[{table}] SET name = N'changed' WHERE id = 50;"),
    )
    .await;
    let transaction_count =
        gauss_horizon_core::db::sqlserver::execute_query(&mut client, "SELECT @@TRANCOUNT AS transaction_count").await;
    let rollback = gauss_horizon_core::db::sqlserver::execute_batch(&mut client, "ROLLBACK TRANSACTION").await;
    let verify = gauss_horizon_core::db::sqlserver::execute_query(
        &mut client,
        &format!("SELECT name FROM dbo.[{table}] WHERE id = 50"),
    )
    .await;
    let cleanup =
        gauss_horizon_core::db::sqlserver::execute_query(&mut client, &format!("DROP TABLE dbo.[{table}]")).await;
...
async fn live_sqlserver_table_structure_default_changes_drop_existing_constraints() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("gauss_horizon_default_{suffix}");
    let table = "products";
    ...
    gauss_horizon_core::db::sqlserver::execute_query(&mut client, &create_schema)
        .await
        .expect("create live test schema");
    gauss_horizon_core::db::sqlserver::execute_query(&mut client, &create_table)
        .await
    ...
    let execution_result = async {
        for statement in &result.statements {
            gauss_horizon_core::db::sqlserver::execute_query(&mut client, statement).await?;
        }
        Ok::<(), String>(())
    }
    .await;
    ...
    let verify_result = gauss_horizon_core::db::sqlserver::execute_query(&mut client, &verify_sql).await;
...
async fn live_sqlserver_stream_first_result_set_exports_cte_query_rows() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("gauss_horizon_stream_export_{suffix}");
    let setup = format!(
        "\
        CREATE TABLE [dbo].[{table}] (id INT NOT NULL, name NVARCHAR(64) NULL);\
        INSERT INTO [dbo].[{table}] (id, name) VALUES (2, N'beta'), (1, N'alpha');"
    );
    gauss_horizon_core::db::sqlserver::execute_batch(&mut client, &setup).await.expect("create live test rows");
    ...
    let result = gauss_horizon_core::db::sqlserver::stream_first_result_set(&mut client, &sql, None, None, |item| {
        match item {
            gauss_horizon_core::db::sqlserver::SqlServerStreamItem::Columns { columns: stream_columns, .. } => {
                columns = stream_columns.to_vec();
            }
            gauss_horizon_core::db::sqlserver::SqlServerStreamItem::Row(row) => {
...
async fn live_sqlserver_query_result_export_streams_cte_query_to_csv() {
    ...
    .await
    .expect("connect SQL Server");
    let export_client = gauss_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("gauss_horizon_query_export_{suffix}");
    let setup = format!(
        "\
        CREATE TABLE [dbo].[{table}] (id INT NOT NULL, name NVARCHAR(64) NULL);\
        INSERT INTO [dbo].[{table}] (id, name) VALUES (2, N'beta'), (1, N'alpha');"
    );
    gauss_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &setup).await.expect("create live test rows");

    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-sqlserver-export-{suffix}"));
...
async fn live_sqlserver_sql_file_import_executes_go_batches() {
    ...
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD")
        .expect("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD");
    let client = gauss_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("gauss_horizon_sql_file_{suffix}");
    let procedure = format!("gauss_horizon_sql_file_proc_{suffix}");
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-sqlserver-file-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
    ...
    let rows =
        gauss_horizon_core::db::sqlserver::execute_query(&mut client, &format!("EXEC [dbo].[{procedure}]")).await;
...
async fn live_sqlserver_transfer_table_skips_rowversion_insert_column() {
    let database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let target_database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_TARGET_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_HOST")
        .unwrap_or_else(|_| "127.0.0.1".to_string());
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PORT")
    ...
    .await
    .expect("connect SQL Server");
    let mut target_client = gauss_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_table = format!("gauss_horizon_rowversion_src_{suffix}");
    ...
    let _ = gauss_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &cleanup_source).await;
    let _ = gauss_horizon_core::db::sqlserver::execute_batch(&mut target_client, &cleanup_target).await;
    ...
    gauss_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &setup)
        .await
        .expect("create rowversion source table");

    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-sqlserver-rowversion-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    ...
    let request = gauss_horizon_core::transfer::TransferRequest {
        ...
        content: gauss_horizon_core::transfer::TransferContent::default(),
        objects: Vec::new(),
        mode: gauss_horizon_core::transfer::TransferMode::Append,
        target_table_name_case: gauss_horizon_core::transfer::TransferTableNameCase::Upper,
        quote_target_column_names: true,
        ownership_policy: gauss_horizon_core::transfer::TransferOwnershipPolicy::Preserve,
        batch_size: 100,
    };
    let result = gauss_horizon_core::transfer::transfer_table(
        &state,
        &request,
        &source_table,
        0,
    ...
    let verify_result = gauss_horizon_core::db::sqlserver::execute_query(&mut target_client, &verify_sql).await;
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_SQLSERVER_URL or GAUSS_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD pointing at a writable SQL Server database"]
async fn live_sqlserver_completion_assistant_searches_metadata_before_limiting() {
    ...
    gauss_horizon_core::db::sqlserver::execute_batch(&mut client, &setup).await.expect("create live test objects");

    let request = gauss_horizon_core::types::CompletionAssistantRequest {
        connection_id: "live-sqlserver".to_string(),
        database: database.clone(),
        schema: Some(schema.clone()),
        object_kinds: vec![gauss_horizon_core::types::CompletionAssistantObjectKind::Table],
        mask: prefix.clone(),
        ...
        parent_name: None,
        match_mode: Some(gauss_horizon_core::types::CompletionAssistantMatchMode::Prefix),
    };

    let response = gauss_horizon_core::db::sqlserver::completion_assistant_search(&mut client, &request)
        .await
    ...
    let column_response = gauss_horizon_core::db::sqlserver::completion_assistant_search(
        &mut client,
        &gauss_horizon_core::types::CompletionAssistantRequest {
            object_kinds: vec![gauss_horizon_core::types::CompletionAssistantObjectKind::Column],
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD pointing at a writable SQL Server instance"]
async fn live_sqlserver_cross_database_metadata_and_query() {
    ...
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD")
        .expect("GAUSS_HORIZON_LIVE_SQLSERVER_PASSWORD");
    let default_database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "master".to_string());
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let database_a = format!("gauss_horizon_completion_a_{}", &suffix[..8]);
    let database_b = format!("gauss_horizon_completion_b_{}", &suffix[..8]);

    let mut master = gauss_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let mut client_a = gauss_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let mut client_b = gauss_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    gauss_horizon_core::db::sqlserver::execute_batch(&mut client_a, "CREATE SCHEMA [IN]")
        .await
        .expect("create database A IN schema");
    gauss_horizon_core::db::sqlserver::execute_batch(&mut client_a, "CREATE SCHEMA [OUT]")
        .await
    ...
    gauss_horizon_core::db::sqlserver::execute_batch(&mut client_b, "CREATE SCHEMA [OUT]")
        .await
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-sqlserver-cross-database-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
    ...
    let tables_a = gauss_horizon_core::schema::list_tables_core(
        &state,
        connection_id,
        &database_a,
        "IN",
    ...
    let tables_b = gauss_horizon_core::schema::list_tables_core(
        &state,
        connection_id,
        &database_b,
        "OUT",
    ...
    let columns_a = gauss_horizon_core::schema::get_columns_core(&state, connection_id, &database_a, "OUT", "orders")
        .await
        .expect("load database A columns");
    let columns_b = gauss_horizon_core::schema::get_columns_core(&state, connection_id, &database_b, "OUT", "orders")
        .await
    ...
    let result = gauss_horizon_core::query::execute_sql_statement(
        &state,
        connection_id,
        &default_database,
        &query,
    ...
    for database in [&database_a, &database_b] {
        ...
        gauss_horizon_core::db::sqlserver::execute_batch(&mut master, &cleanup)
            .await
            .expect("drop completion database");
    }
    let _ = std::fs::remove_dir_all(dir);
}
