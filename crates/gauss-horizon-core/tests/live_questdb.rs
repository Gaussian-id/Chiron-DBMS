...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_TEST_QUESTDB_URL pointing at a writable QuestDB database"]
async fn questdb_lists_tables_across_metadata_generations() {
    let url =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_QUESTDB_URL").expect("GAUSS_HORIZON_TEST_QUESTDB_URL");
    let pool = db::postgres::connect(&url, Duration::from_secs(10)).await.expect("connect QuestDB");
    let table = format!("gauss_horizon_questdb_82_{}", uuid::Uuid::new_v4().simple());
