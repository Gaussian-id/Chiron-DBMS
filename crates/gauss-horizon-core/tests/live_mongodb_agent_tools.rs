...
use gauss_horizon_core::agent_events::{ToolCall, ToolResult};
use gauss_horizon_core::agent_tools::{execute_tool, AgentSqlPermissions};
...
use gauss_horizon_core::mongo_ops::mongo_create_index_core;
...
fn live_mongodb_config(id: &str) -> ConnectionConfig {
    let host =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_HOST").expect("GAUSS_HORIZON_LIVE_MONGODB_HOST");
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_PORT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(27017);
    let username =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_USER").expect("GAUSS_HORIZON_LIVE_MONGODB_USER");
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_PASSWORD")
        .expect("GAUSS_HORIZON_LIVE_MONGODB_PASSWORD");
    let database = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_DATABASE")
        .expect("GAUSS_HORIZON_LIVE_MONGODB_DATABASE");
    let auth_source =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_AUTH_SOURCE").unwrap_or_else(|_| database.clone());
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_MONGODB_* env vars pointing at a MongoDB database with a non-empty stores collection"]
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_MONGODB_* env vars pointing at a MongoDB database with a products collection containing at least 8 documents"]
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_MONGODB_42_URL pointing at MongoDB 4.2 with enableTestCommands=1"]
async fn stalled_mongodb_index_build_does_not_block_another_connection() {
    let uri = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_42_URL")
        .expect("GAUSS_HORIZON_LIVE_MONGODB_42_URL");
    let directory = tempfile::tempdir().unwrap();
    let storage = Storage::open(&directory.path().join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    let primary_id = "live-mongodb-42-index";
    let database = "gauss_horizon_issue_7720_primary";
