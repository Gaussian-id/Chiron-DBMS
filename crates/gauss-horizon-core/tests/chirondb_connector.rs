...
async fn fixture() -> Fixture {
    ...
    let ai: gauss_horizon_core::ai::AiConfigItem = serde_json::from_value(json!({"id":"mock","name":"Synthetic provider","provider":"openai-compatible","apiKey":"fixture-key","endpoint":format!("http://127.0.0.1:{port}/v1"),"model":"fixture-model","apiStyle":"completions","authMethod":"bearer","maxRetries":0})).unwrap();
...
fn generate(prompt: &str, generate_only: bool) -> Request {
    Request::Assistant {
        request: gauss_horizon_core::ai_chiron::Request::Generate {
            config_id: "mock".into(),
...
fn approve(body: &Value) -> Request {
    Request::Assistant {
        request: gauss_horizon_core::ai_chiron::Request::Approve {
...
async fn native_history_roundtrips_without_sharing_results_and_is_retained_until_deleted() {
    let f = fixture().await;
    let mut conversation: gauss_horizon_core::ai::AiConversation = serde_json::from_value(json!({
    ...
    let context = gauss_horizon_core::ai_chiron::conversation_context(&loaded, "chiron");
    assert!(context.contains("Which dimension?"));
    assert!(context.contains("COUNT demo"));
    assert!(!context.contains("PRIVATE_"));
    assert!(gauss_horizon_core::ai_chiron::conversation_context(&loaded, "other").is_empty());
...
async fn assistant_reads_keep_native_data_local_and_share_only_exact_preview() {
    ...
    let invalid = Request::Assistant {
        request: gauss_horizon_core::ai_chiron::Request::Explain {
            run_id: run_id.clone(),
    ...
    let shared = Request::Assistant {
        request: gauss_horizon_core::ai_chiron::Request::Explain { run_id, approved_preview: preview.clone() },
    };
...
async fn assistant_cancellation_readonly_stale_target_and_generate_only_do_not_mutate() {
    ...
    let cancel = Request::Assistant {
        request: gauss_horizon_core::ai_chiron::Request::Cancel {
...
async fn chirondb_browse_preserves_cursor_nested_values_and_unknown_metadata() {
    let f = fixture().await;
    let list = gauss_horizon_core::schema::list_vector_collections_core(&f.state, "chiron", "default").await.unwrap();
...
async fn chirondb_rejects_shared_sql_and_mcp_batch_execution_without_dispatch() {
    ...
    let error = gauss_horizon_core::query::execute_sql_statement(&f.state, "chiron", "default", "SELECT 1", None, None)
        .await
        .unwrap_err();
    ...
    let results = Box::pin(gauss_horizon_core::query::execute_multi_core_with_options_for_client(
        ...
        None,
        None,
        gauss_horizon_core::query::QueryExecutionOptions { continue_on_error: true, ..Default::default() },
    ))
    .await
    .unwrap();
...
async fn malformed_native_attachment_fails_before_provider_or_database_request() {
    ...
    if let Request::Assistant { request: gauss_horizon_core::ai_chiron::Request::Generate { images, .. } } =
        &mut request
    {
