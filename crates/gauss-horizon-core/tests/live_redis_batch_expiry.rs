...
use gauss_horizon_core::redis_ops::{
...
/// `GAUSS_HORIZON_LIVE_REDIS_HOST=... GAUSS_HORIZON_LIVE_REDIS_PORT=... GAUSS_HORIZON_LIVE_REDIS_PASSWORD=... \
///  cargo test -p gauss-horizon-core --test live_redis_batch_expiry -- --ignored --nocapture`
async fn live_state(connection_id: &str) -> Arc<AppState> {
    let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_REDIS_HOST").expect("GAUSS_HORIZON_LIVE_REDIS_HOST");
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_REDIS_PORT")
        .expect("GAUSS_HORIZON_LIVE_REDIS_PORT")
        .parse::<u16>()
        .expect("Redis port");
    let username = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_REDIS_USERNAME").unwrap_or_default();
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_REDIS_PASSWORD").unwrap_or_default();
...
fn unique_prefix() -> String {
    ...
    format!("gauss_horizon_8899_{nanos}")
}
...
fn key_raws(prefix: &str) -> Vec<String> {
    ...
    (0..3)
        .map(|index| {
            gauss_horizon_core::db::redis_driver::redis_key_bytes_to_raw(format!("{prefix}:{index}").as_bytes())
        })
        .collect()
}
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_REDIS_HOST and GAUSS_HORIZON_LIVE_REDIS_PORT"]
...
async fn set_keys_ttl_reports_a_concurrently_deleted_key_without_failing_the_batch() {
    ...
    raws.push(gauss_horizon_core::db::redis_driver::redis_key_bytes_to_raw(format!("{prefix}:deleted").as_bytes()));
