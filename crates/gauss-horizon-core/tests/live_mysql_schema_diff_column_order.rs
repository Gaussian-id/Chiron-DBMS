...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_MYSQL_URL pointing to MySQL 8.4.2"]
async fn mysql_schema_diff_add_columns_keep_source_order() {
    let url = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MYSQL_URL").expect("GAUSS_HORIZON_LIVE_MYSQL_URL");
    ...
    conn.query_drop("DROP DATABASE IF EXISTS gauss-horizon4291_live").await.expect("drop prior live database");
    conn.query_drop("CREATE DATABASE gauss-horizon4291_live").await.expect("create live database");
    conn.query_drop("CREATE TABLE gauss-horizon4291_live.positional (`a` INT NOT NULL, `last` INT NOT NULL)")
        .await
    ...
    let prepared = prepare_schema_diff(SchemaDiffPreparationOptions {
        ...
        target_schema: Some("gauss-horizon4291_live".to_string()),
        ..Default::default()
    });

    conn.query_drop(&prepared.sync_sql).await.expect("execute Gauss Horizon-generated positional ALTER TABLE");
    let names: Vec<String> = conn
        .exec_map(
            "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS \
             WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? ORDER BY ORDINAL_POSITION",
            ("gauss-horizon4291_live", "positional"),
            |name: String| name,
        )
        .await
