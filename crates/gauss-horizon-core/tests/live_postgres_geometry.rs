...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_TEST_POSTGRES_GEOMETRY_URL pointing at a PostGIS database such as local-pg/test_geometry"]
async fn live_postgres_geometry_values_render_as_text_in_query_results() {
    let url = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_GEOMETRY_URL")
        .expect("GAUSS_HORIZON_TEST_POSTGRES_GEOMETRY_URL");
    let pool = gauss_horizon_core::db::postgres::connect(&url, Duration::from_secs(5)).await.expect("connect");
    let result = gauss_horizon_core::db::postgres::execute_query(
        &pool,
