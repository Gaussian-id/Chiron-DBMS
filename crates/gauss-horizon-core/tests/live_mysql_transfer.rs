...
fn live_mysql_config(id: &str) -> ConnectionConfig {
    let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_HOST")
        .expect("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_HOST");
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(3306);
    let username = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_USER")
        .unwrap_or_else(|_| "root".to_string());
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_PASSWORD")
        .expect("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_PASSWORD");
...
fn live_cross_version_mysql_config(id: &str, prefix: &str) -> ConnectionConfig {
    let host = gauss_horizon_core::legacy::var(format!("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_HOST"))
        .unwrap_or_else(|_| panic!("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_HOST"));
    let port = gauss_horizon_core::legacy::var(format!("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_PORT"))
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(3306);
    let username = gauss_horizon_core::legacy::var(format!("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_USER"))
        .unwrap_or_else(|_| "root".to_string());
    let password = gauss_horizon_core::legacy::var(format!("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_PASSWORD"))
        .unwrap_or_else(|_| panic!("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_PASSWORD"));
...
#[test]
#[ignore = "requires disposable MySQL 8.0.33 source and 8.0.45 target endpoints via GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_SOURCE_*/TARGET_* variables"]
...
async fn run_live_mysql_cross_version_transfer_completes_on_small_stack() {
    ...
    let source_database = format!("gauss_horizon_stack_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_stack_dst_{}", &suffix[..12]);
    ...
    let task_tmp = std::path::PathBuf::from(
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_TMP_DIR")
            .expect("GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_TMP_DIR"),
    );
...
#[tokio::test]
#[ignore = "requires disposable MySQL 8 endpoints via GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_* variables"]
async fn live_mysql_transfer_keeps_columns_whose_comment_mentions_foreign_key() {
    ...
    let source_database = format!("gauss_horizon_7660_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_7660_dst_{}", &suffix[..12]);
    ...
    let test_result = async {
        ...
        for (_, statement) in &pending_fk_alters {
            gauss_horizon_core::transfer::execute_on_pool_with_max_rows(&state, &target_pool_key, statement, None)
                .await
                .unwrap();
        }
...
#[tokio::test]
#[ignore = "requires disposable MySQL 8 source and 5.7 target endpoints via GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_SOURCE_*/TARGET_* variables"]
async fn live_mysql_transfer_downgrades_unsupported_source_collations() {
    ...
    let source_database = format!("gauss_horizon_6023_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_6023_dst_{}", &suffix[..12]);
    let modern_target_database = format!("gauss_horizon_6023_modern_{}", &suffix[..12]);
    let source_config = live_cross_version_mysql_config(&source_connection_id, "SOURCE");
    let target_config = live_cross_version_mysql_config(&target_connection_id, "TARGET");
    let source_setup_pool = mysql::connect(&mysql_url(&source_config), Duration::from_secs(10)).await.unwrap();
    let target_setup_pool = mysql::connect(&mysql_url(&target_config), Duration::from_secs(10)).await.unwrap();
...
#[tokio::test]
#[ignore = "requires a disposable MySQL 5.7+ endpoint via GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_* variables"]
async fn live_mysql_transfer_preserves_spatial_values_and_modes() {
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let connection_id = format!("live-mysql-transfer-{suffix}");
    let source_database = format!("gauss_horizon_transfer_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_transfer_dst_{}", &suffix[..12]);
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-transfer-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
...
async fn live_mysql_transfer_structure_overwrite_rejects_incompatible_target_columns() {
    ...
    let source_database = format!("gauss_horizon_transfer_struct_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_transfer_struct_dst_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-transfer-struct-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), config);
...
async fn live_mysql_transfer_structure_only_rejects_incompatible_target_columns() {
    ...
    let source_database = format!("gauss_horizon_transfer_structonly_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_transfer_structonly_dst_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-transfer-structonly-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), config);
...
async fn live_mysql_transfer_drop_target_parent_child_foreign_key() {
    ...
    let source_database = format!("gauss_horizon_parent_child_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_parent_child_dst_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
    ...
    mysql::execute_query(&setup_pool, &setup, true).await.unwrap();
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-parent-child-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), config);
...
#[tokio::test]
#[ignore = "requires live MySQL connection via GAUSS_HORIZON_LIVE_MYSQL_TRANSFER_* env vars"]
async fn live_mysql_transfer_drop_target_rebuilds_incompatible_structure() {
    ...
    let source_database = format!("gauss_horizon_drop_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_drop_tgt_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-mysql-drop-rebuild-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), config);
...
async fn live_mysql_transfer_drop_target_rejects_external_incoming_fk() {
    ...
    let database = format!("gauss_horizon_ext_fk_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-mysql-ext-fk-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), config);
...
async fn live_mysql_transfer_drop_target_circular_foreign_keys() {
    ...
    let source_database = format!("gauss_horizon_circ_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_circ_tgt_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-mysql-circular-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), config);
...
async fn live_mysql_transfer_drop_target_retains_backup_on_failure() {
    ...
    let source_database = format!("gauss_horizon_retain_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_retain_tgt_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-mysql-retain-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), config);
...
async fn live_mysql_keyset_pagination_copies_every_row() {
    ...
    let source_database = format!("gauss_horizon_keyset_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_keyset_dst_{}", &suffix[..12]);
    let source_config = live_mysql_config(&source_connection_id);
    let target_config = live_mysql_config(&target_connection_id);
    let source_setup_pool = mysql::connect(&mysql_url(&source_config), Duration::from_secs(10)).await.unwrap();
    let target_setup_pool = mysql::connect(&mysql_url(&target_config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_progress_read_survives_total_duration_beyond_timeout() {
    ...
    let source_database = format!("gauss_horizon_progress_src_{}", &suffix[..12]);
    let target_database = format!("gauss_horizon_progress_dst_{}", &suffix[..12]);
    let source_config = live_mysql_config(&source_connection_id);
    let target_config = live_mysql_config(&target_connection_id);
    let source_setup_pool = mysql::connect(&mysql_url(&source_config), Duration::from_secs(10)).await.unwrap();
    let target_setup_pool = mysql::connect(&mysql_url(&target_config), Duration::from_secs(10)).await.unwrap();
