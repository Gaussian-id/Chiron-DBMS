...
async fn sqlserver_dbcc_messages_do_not_replace_results_or_errors() {
    ...
    let table = "dbo.gauss_horizon_issue_3583_messages";
    let setup = format!(
    ...
    let error = sqlserver::execute_query(&mut client, "SELECT * FROM dbo.gauss_horizon_issue_3583_missing")
        .await
...
async fn sqlserver_print_messages_keep_tds_order_around_results_and_counts() {
    ...
    sqlserver::execute_simple_batch_with_max_rows(
        &mut client,
        "SET NOCOUNT ON; \
         CREATE TABLE #gauss_horizon_print_order (id int NOT NULL PRIMARY KEY, value int NOT NULL); \
         INSERT INTO #gauss_horizon_print_order VALUES (1, 0), (2, 0); \
         SET NOCOUNT OFF;",
        None,
    )
    .await
    ...
    let count_order = sqlserver::execute_batch(
        ...
        "PRINT N'before update'; \
         UPDATE #gauss_horizon_print_order SET value = value + 1; \
...
async fn sqlserver_default_no_count_keeps_select_and_dml_result_shape() {
    ...
    sqlserver::execute_simple_batch_with_max_rows(
        &mut client,
        "SET NOCOUNT ON; \
         CREATE TABLE #gauss_horizon_select_dml_order (id int NOT NULL PRIMARY KEY, value int NOT NULL); \
         INSERT INTO #gauss_horizon_select_dml_order VALUES (1, 0), (2, 0); \
         SET NOCOUNT OFF;",
        None,
    )
    .await
    ...
    let mixed = sqlserver::execute_batch(
        &mut client,
        "SELECT CAST(1 AS int) AS first; \
         UPDATE #gauss_horizon_select_dml_order SET value = value + 1; \
         SELECT CAST(2 AS int) AS second; \
         DELETE FROM #gauss_horizon_select_dml_order;",
    )
    .await
