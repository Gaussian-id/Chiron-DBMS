...
use gauss_horizon_core::database_export::{export_database_sql_core, DatabaseExportRequest};
...
async fn run_database_export_of_partition_tree_has_no_duplicates_and_replays() {
    let Some(container) = start_docker_postgres("gauss-horizon-export-partition-ddl") else {
        return;
    };
    ...
    psql(
        ...
        "DROP TABLE IF EXISTS events CASCADE;\
         ...
         DROP SERVER IF EXISTS gauss_horizon_export_loopback CASCADE;\
         CREATE EXTENSION IF NOT EXISTS postgres_fdw;\
         CREATE SERVER gauss_horizon_export_loopback FOREIGN DATA WRAPPER postgres_fdw \
           OPTIONS (host 'localhost', dbname 'postgres', port '5432');\
         CREATE USER MAPPING FOR CURRENT_USER SERVER gauss_horizon_export_loopback \
         ...
         CREATE FOREIGN TABLE events_foreign PARTITION OF events FOR VALUES FROM ('2028-01-01') TO ('2029-01-01') \
           SERVER gauss_horizon_export_loopback OPTIONS (schema_name 'public', table_name 'foreign_child_data');\
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-export-partition-ddl-{}", uuid::Uuid::new_v4()));
    ...
    let selected_table_ddl = gauss_horizon_core::schema::get_table_export_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
    ...
    let ordinary_table_ddl = gauss_horizon_core::schema::get_table_export_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
    ...
    let missing_table_error = gauss_horizon_core::schema::get_table_export_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
...
async fn partition_tree_ddl_query_count_does_not_scale_with_partition_count() {
    let Some(container) = start_docker_postgres("gauss-horizon-partition-ddl-query-count") else {
        return;
    };
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-partition-query-count-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    ...
    async fn fetch_ddl(state: &AppState, connection_id: &str) -> String {
        gauss_horizon_core::schema::get_table_display_ddl_core(state, connection_id, "postgres", "public", "t", None)
            .await
...
async fn display_ddl_error_distinguishes_wrong_relkind_from_missing_relation() {
    let Some(container) = start_docker_postgres("gauss-horizon-partition-ddl-relkind-error") else {
        return;
    };

    psql(&container, "CREATE VIEW v AS SELECT 1 AS id");

    let dir = std::env::temp_dir().join(format!("gauss-horizon-partition-relkind-error-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    ...
    let view_error =
        gauss_horizon_core::schema::get_table_display_ddl_core(&state, connection_id, "postgres", "public", "v", None)
            .await
    ...
    let missing_error = gauss_horizon_core::schema::get_table_display_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
...
async fn cloudberry_native_ddl_still_includes_every_partition() {
    let Some(container) = start_docker_postgres("gauss-horizon-cloudberry-partition-ddl") else {
        return;
    };
    ...
    let pool =
        gauss_horizon_core::db::postgres::connect(&url, std::time::Duration::from_secs(5)).await.expect("connect");

    let ddl = gauss_horizon_core::schema::cloudberry_ddl(&pool, "public", "t", true)
        .await
        .expect("cloudberry ddl with partitions");
    assert_eq!(
    ...
    let single_ddl = gauss_horizon_core::schema::cloudberry_ddl(&pool, "public", "t", false)
        .await
        .expect("cloudberry ddl without partitions");
...
async fn opentenbase_tree_ddl_gives_every_partition_its_own_distribute_by() {
    let Some(container) = start_docker_postgres_with_server_args(
        "gauss-horizon-opentenbase-partition-ddl",
    ...
    let distributions = gauss_horizon_core::db::opentenbase::table_distribution_for_relations(
        &pool,
    ...
    assert_eq!(
        ...
        Some(&gauss_horizon_core::db::opentenbase::DistributionPolicy::Hash(vec!["region".to_string()]))
    );
    assert_eq!(
        distributions.get(&("public".to_string(), "t_p1".to_string())),
        Some(&gauss_horizon_core::db::opentenbase::DistributionPolicy::Hash(vec!["id".to_string()]))
    );

    let ddl =
        gauss_horizon_core::schema::opentenbase_ddl(&pool, "public", "t", true).await.expect("opentenbase tree ddl");
    assert!(
