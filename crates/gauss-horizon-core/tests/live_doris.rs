...
//
// Run with:
//   GAUSS_HORIZON_TEST_DORIS_HOST=127.0.0.1 GAUSS_HORIZON_TEST_DORIS_PORT=9030 GAUSS_HORIZON_TEST_DORIS_USER=root \
//   GAUSS_HORIZON_TEST_DORIS_PASSWORD= cargo test -p gauss-horizon-core --test live_doris -- --ignored
use std::time::Duration;

use gauss_horizon_core::connection::{connect_bare_metadata_pool, AppState};
use gauss_horizon_core::models::connection::{ConnectionConfig, DatabaseType};
use gauss_horizon_core::schema::get_columns_core;
use gauss_horizon_core::storage::Storage;
use mysql_async::prelude::Queryable;

fn live_doris_config(id: &str) -> ConnectionConfig {
    let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_DORIS_HOST").expect("GAUSS_HORIZON_TEST_DORIS_HOST");
    let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_DORIS_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(9030);
    let username =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_DORIS_USER").expect("GAUSS_HORIZON_TEST_DORIS_USER");
    let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_DORIS_PASSWORD").unwrap_or_default();
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_TEST_DORIS_HOST/GAUSS_HORIZON_TEST_DORIS_PORT/GAUSS_HORIZON_TEST_DORIS_USER pointing at a writable Apache Doris 2.1.8 cluster"]
async fn doris_qualified_table_metadata_resolves_schema_database_with_comments() {
    ...
    let db_path = std::env::temp_dir().join(format!("gauss-horizon-doris-6590-{}.db", uuid::Uuid::new_v4().simple()));
    ...
    Queryable::query_drop(&mut conn, "CREATE DATABASE IF NOT EXISTS gauss-horizon6590_test")
        .await
        .expect("create Doris test database");
    Queryable::query_drop(
        &mut conn,
        "CREATE TABLE IF NOT EXISTS gauss-horizon6590_test.gauss_horizon_comment_test (\
    ...
    let columns =
        get_columns_core(&state, "doris-6590-live", "", "gauss-horizon6590_test", "gauss_horizon_comment_test")
            .await
