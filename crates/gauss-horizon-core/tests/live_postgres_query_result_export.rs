...
use gauss_horizon_core::query::execute_sql_statement;
...
async fn live_postgres_query_result_export_uses_single_streamed_query() {
    ...
    let schema = format!("gauss_horizon_query_export_{}", &suffix[..8]);
    let setup = vec![
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-postgres-query-export-{suffix}"));
...
async fn live_postgres_query_result_xlsx_preserves_temporal_cell_types() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("gauss_horizon_xlsx_temporal_{}", &suffix[..8]);
    let setup = vec![
        format!("CREATE SCHEMA \"{schema}\""),
    ...
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-postgres-xlsx-temporal-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_POSTGRES_HOST/PORT/USER/PASSWORD/DATABASE pointing at PostgreSQL 16.3"]
async fn live_postgres_numeric_xlsx_ignores_fractional_trailing_zeros() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-postgres-xlsx-numeric-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_POSTGRES_* env vars for temporary-table CSV/XLSX export"]
async fn live_postgres_truncated_batch_result_export_replays_safe_temp_setup() {
    ...
    let config = live_postgres_config(&connection_id, &host, port, &user, &password, &database);
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-postgres-temp-export-{short_suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
    state.configs.write().await.insert(config.id.clone(), config);

    let t1 = format!("gauss_horizon_temp_t1_{short_suffix}");
    let t2 = format!("gauss_horizon_temp_t2_{short_suffix}");
    let t5 = format!("gauss_horizon_temp_t5_{short_suffix}");
    let setup_sql = vec![
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_POSTGRES_* env vars for a 650,000-row XLSX export"]
async fn live_postgres_xlsx_export_can_outlive_query_timeout_while_rows_keep_arriving() {
    ...
    let config = live_postgres_config(&connection_id, &host, port, &user, &password, &database);
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-postgres-query-export-timeout-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
    state.configs.write().await.insert(config.id.clone(), config);
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_POSTGRES_* env vars"]
async fn live_postgres_stream_still_times_out_without_progress_and_recovers() {
    ...
    let config = live_postgres_config(&connection_id, &host, port, &user, &password, &database);
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-postgres-query-export-stall-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = AppState::new(storage);
    state.configs.write().await.insert(config.id.clone(), config);
