...
use gauss_horizon_core::{db, schema};

#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_TEST_CLOUDBERRY_URL pointing at a writable Apache Cloudberry database"]
async fn cloudberry_metadata_and_ddl_round_trip() {
    let url = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_CLOUDBERRY_URL")
        .expect("GAUSS_HORIZON_TEST_CLOUDBERRY_URL");
    let pool = db::postgres::connect(&url, Duration::from_secs(10)).await.expect("connect Cloudberry");
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("gauss_horizon_cb_source_{suffix}");
    let target_schema = format!("gauss_horizon_cb_target_{suffix}");
        ...
        &[
            ...
            format!(
                "CREATE READABLE EXTERNAL TABLE {source_ident}.external_events (id integer, payload text) \
                 LOCATION ('file://cdw/tmp/gauss-horizon-cloudberry-live-test.csv') \
    ...
    let exercise = async {
        ...
        for (table, expected) in cases {
            ...
            if table == "external_events" {
                ...
                assert!(ddl.contains("\"location_uris\" 'file://cdw/tmp/gauss-horizon-cloudberry-live-test.csv'"));
            }
