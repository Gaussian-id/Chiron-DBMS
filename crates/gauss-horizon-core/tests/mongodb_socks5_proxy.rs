...
async fn parses_socks5_proxy_credentials() {
    let options = ClientOptions::parse(
        "mongodb://localhost/?proxyHost=proxy.example.com&proxyUsername=gauss-horizon&proxyPassword=secret",
    )
    .await
    ...
    assert_eq!(proxy.authentication, Some(("gauss-horizon".to_string(), "secret".to_string())));
}
...
async fn leaves_direct_connections_without_a_proxy() {
    let options = ClientOptions::parse("mongodb://localhost:27017/gauss-horizon")
        .await
...
#[tokio::test]
#[ignore = "requires GAUSS_HORIZON_LIVE_MONGODB_SOCKS5_URL routed through a SOCKS5 proxy"]
async fn connects_to_mongodb_through_socks5_proxy() {
    let url = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_SOCKS5_URL")
        .expect("GAUSS_HORIZON_LIVE_MONGODB_SOCKS5_URL");
    ...
    if let Ok(expected) = gauss_horizon_core::legacy::var("GAUSS_HORIZON_LIVE_MONGODB_EXPECTED_VERSION") {
