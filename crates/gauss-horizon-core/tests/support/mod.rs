...
pub struct DockerPostgres {
    ...
    /// provided via `GAUSS_HORIZON_TEST_POSTGRES_CONTAINER` (see
...
///
/// If `GAUSS_HORIZON_TEST_POSTGRES_CONTAINER`/`GAUSS_HORIZON_TEST_POSTGRES_PORT` are set, reuses
...
pub fn start_docker_postgres_with_server_args(name_prefix: &str, server_args: &[&str]) -> Option<DockerPostgres> {
    if let Ok(name) = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_CONTAINER") {
        let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_TEST_POSTGRES_PORT")
            .expect("GAUSS_HORIZON_TEST_POSTGRES_PORT must accompany GAUSS_HORIZON_TEST_POSTGRES_CONTAINER")
            .parse()
            .expect("GAUSS_HORIZON_TEST_POSTGRES_PORT must be a valid port");
