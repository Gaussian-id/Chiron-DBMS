...
use gauss_horizon_core::database_export::{
    export_database_sql_core, record_export_destination_identity, DatabaseExportRequest,
};
use gauss_horizon_core::models::connection::{ConnectionConfig, DatabaseType};
use gauss_horizon_core::query::execute_sql_statement;
use gauss_horizon_core::sql::SqlFileRequest;
...
async fn live_mysql_selected_table_restore_preserves_unselected_tables() {
    ...
    use gauss_horizon_core::sql_file_import::{inspect_sql_file_tables, SqlFileTable};
    use std::io::Write;

    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let connection_id = format!("selected-restore-{suffix}");
    let database = format!("gauss_horizon_restore_{suffix}");
    let dir = std::env::temp_dir().join(format!("gauss-horizon-selected-restore-{suffix}"));
...
async fn live_mysql_database_export_restores_dependent_views() {
    ...
    let database = format!("gauss_horizon_export_{suffix}");
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-export-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
...
async fn run_live_mysql_database_export_handles_many_tables_including_empty_tables() {
    ...
    let database = format!("gauss_horizon_export_many_{suffix}");
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-export-many-tables-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), live_mysql_config(&connection_id));
...
async fn live_mysql_database_export_creates_missing_destination_directory() {
    ...
    let database = format!("gauss_horizon_export_missing_dir_{suffix}");
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-export-missing-dir-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), live_mysql_config(&connection_id));
...
async fn live_mysql_database_export_refuses_to_recreate_a_destination_that_disappeared() {
    ...
    let database = format!("gauss_horizon_export_vanished_dir_{suffix}");
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-export-vanished-dir-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), live_mysql_config(&connection_id));
...
async fn live_mysql_database_export_refuses_a_destination_that_vanished_before_its_first_run() {
    ...
    let database = format!("gauss_horizon_export_precfg_vanished_{suffix}");
    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-mysql-export-preconfigured-vanished-dir-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    state.configs.write().await.insert(connection_id.clone(), live_mysql_config(&connection_id));
