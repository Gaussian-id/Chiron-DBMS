...
async fn sqlserver_extended_events_probe_captures_rpc_and_batch_events() {
    ...
    let session_name = format!("GAUSS_HORIZON_TRACE_{}_TEST{}", base36(expires_at), std::process::id());
    let procedure_name = format!("GAUSS_HORIZON_TRACE_TEST_PROC_{}", std::process::id());
    ...
    let create_sql = format!(
        "CREATE EVENT SESSION [{session_name}] ON SERVER \
         ADD EVENT sqlserver.rpc_completed( \
           ...
           WHERE ([sqlserver].[database_id]=({database_id}) AND NOT [sqlserver].[like_i_sql_unicode_string]([sqlserver].[sql_text],N'%GAUSS_HORIZON_INTERNAL_TRACE%'))), \
    ...
    let result = async {
        ...
        .await?;
        run_batch(&mut workload, "SELECT 1701 AS gauss_horizon_trace_probe; WAITFOR DELAY '00:00:00.050';").await?;
        ...
        let read_sql = format!(
             "/* GAUSS_HORIZON_INTERNAL_TRACE */ ;WITH target AS ( \
        ...
        if !events.rows.iter().any(|row| row.get(4).and_then(|value| value.as_str()).is_some_and(|sql| sql.contains("gauss_horizon_trace_probe"))) {
        ...
        if events.rows.iter().any(|row| {
            row.get(4)
                ...
                .is_some_and(|sql| sql.contains("GAUSS_HORIZON_INTERNAL_TRACE"))
        }) {
            return Err("Gauss Horizon internal health check was captured".to_string());
        }
        Ok::<(), String>(())
    }
    .await;
