...
use gauss_horizon_core::docs::{NoteSource, SnapshotWarning};
...
async fn annotations_reach_the_generated_dbml() {
  ...
  "tables": {{
    ...
    let notes_path = std::env::temp_dir().join(format!("gauss-horizon-live-docs-notes-{}.json", uuid::Uuid::new_v4()));
    std::fs::write(&notes_path, &notes_json).expect("write temp notes file");

    let dir = std::env::temp_dir().join(format!("gauss-horizon-live-postgres-docs-annotations-{suffix}"));
    ...
    let annotations_result = gauss_horizon_core::docs::annotations::load_annotations(&notes_path);
    ...
    let dbml = gauss_horizon_core::docs::to_dbml(&snapshot);
    println!("{}", dbml.text);
