...
async fn run_object_cache_regression(postgres: bool) {
    ...
    if !postgres {
        chiron_horizon_core::db::sqlite::connect_path_create_if_missing(&config.host).await.unwrap();
    }
    ...
    let prefix = chiron_horizon_core::object_cache::object_metadata_cache_prefix(&config.id, &database);
    let schema_segment = chiron_horizon_core::object_cache::metadata_cache_segment(&schema_name);
    let front_key = format!("{prefix}{schema_segment}:users::TABLE:columns:");
    let ddl_key = format!(
        "object-ddl:v1:{}:{}:{schema_segment}:users::TABLE:",
        chiron_horizon_core::object_cache::metadata_cache_segment(&config.id),
        chiron_horizon_core::object_cache::metadata_cache_segment(&database)
    );
