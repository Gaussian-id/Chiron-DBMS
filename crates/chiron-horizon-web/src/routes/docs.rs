...
use axum::Json;
use chiron_horizon_core::docs::{CollectOptions, SchemaSnapshot};
...
pub async fn collect_snapshot(
    ...
    let snapshot = chiron_horizon_core::docs::collect_snapshot(
        &state.app,
        &connection,
        &options,
        &|_progress| {},
...
pub struct DocsApplyRequest {
    ...
    pub annotations: chiron_horizon_core::docs::annotations::AnnotationFile,
}
...
/// this needs no lookup and no new dependency — chiron-horizon-web does NOT depend on
/// chiron-horizon-mcp.
fn notes_path_for(state: &Arc<WebState>, config: &ConnectionConfig) -> std::path::PathBuf {
    chiron_horizon_core::docs::annotations::resolve_notes_path(
        &config.id,
        config.docs_notes_path.as_deref(),
        &state.data_dir,
    )
}
...
) -> Result<Json<Option<chiron_horizon_core::docs::annotations::AnnotationFile>>, AppError> {
    let config = load_connection(&state, &request.connection_id).await?;
    let path = notes_path_for(&state, &config);
    Ok(Json(chiron_horizon_core::docs::annotations::load_annotations(&path).map_err(AppError::from)?))
}
...
pub async fn apply_annotations(
    ...
    chiron_horizon_core::docs::annotations::apply_annotations(&mut applied, &request.annotations, config.db_type);
    Ok(Json(applied))
}
...
pub async fn save_annotations(
    ...
    let config = load_connection(&state, &request.connection_id).await?;
    let path = notes_path_for(&state, &config);
    chiron_horizon_core::docs::annotations::save_annotations(&path, &request.annotations).map_err(AppError::from)?;
    Ok(Json(()))
}
...
pub async fn export_html(Json(request): Json<DocsExportRequest>) -> Result<Json<DocsExportResponse>, AppError> {
    let content = chiron_horizon_core::docs::to_standalone_html(&request.snapshot, &request.annotations, &request.lang)
        .map_err(AppError::from)?;
