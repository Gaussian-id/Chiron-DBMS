...
use chiron_horizon_core::storage::{McpGlobalPolicy, McpGlobalPolicyState};
...
pub async fn load_web_mcp_http_status(State(state): State<Arc<WebState>>) -> Json<WebMcpHttpStatus> {
    let token_source = match (
        chiron_horizon_core::legacy::var_os("CHIRON_HORIZON_WEB_MCP_TOKEN"),
        chiron_horizon_core::legacy::var_os("CHIRON_HORIZON_WEB_MCP_TOKEN_FILE"),
    ) {
    ...
    Json(WebMcpHttpStatus {
        ...
        endpoint_path,
        token_source,
        allowed_hosts: comma_separated_env("CHIRON_HORIZON_WEB_MCP_ALLOWED_HOSTS"),
        allowed_origins: comma_separated_env("CHIRON_HORIZON_WEB_MCP_ALLOWED_ORIGINS"),
    })
}
...
fn decrypt_config_payload(payload: &EncryptedConfigPayload, passphrase: &str) -> Result<String, String> {
    if payload.format != "chiron-horizon-encrypted"
        && !chiron_horizon_core::legacy::is_legacy_encrypted_format(&payload.format)
        || payload.version != 1
    {
...
mod tests {
    ...
    fn exported_browser_payload() -> EncryptedConfigPayload {
        EncryptedConfigPayload {
            format: "chiron-horizon-encrypted".to_string(),
            version: 1,
