...
use axum::Json;
use chiron_horizon_core::transfer::{self, TransferRequest, TransferStatus};
...
pub async fn start_transfer(
    ...
    if let Some(name) =
        chiron_horizon_core::query::connection_readonly_name(&state.app, &req.target_connection_id).await
    {
    ...
    tokio::spawn(async move {
        ...
        if matches!(req.content, transfer::TransferContent::StructureOnly)
            && (matches!(source_db_type, chiron_horizon_core::models::connection::DatabaseType::MongoDb)
                || matches!(target_db_type, chiron_horizon_core::models::connection::DatabaseType::MongoDb))
        {
...
) -> Result<Json<chiron_horizon_core::transfer::TransferOwnershipPreview>, AppError> {
    let req = body.request;
    transfer::validate_transfer_request(&req).map_err(AppError::from)?;
