...
use chiron_horizon_core::agent_events::AgentEvent;
use chiron_horizon_core::agent_loop::{run_agent_loop, AgentLoopContext};
use chiron_horizon_core::ai::{
...
fn reject_web_unsupported_ai_provider(config: &AiConfig) -> Result<(), AppError> {
    if chiron_horizon_core::ai::is_cli_provider(&config.provider) {
        return Err(AppError::bad_request("CLI providers are only supported in Chiron Horizon Desktop."));
    }
    Ok(())
}
...
pub async fn ai_complete(
    ...
    chiron_horizon_core::ai::merge_global_max_retries(
        &mut request.config,
        state.app.storage.load_max_retries().await.unwrap_or(chiron_horizon_core::ai::DEFAULT_MAX_RETRIES),
    );
    let result = chiron_horizon_core::ai::complete(&request).await.map_err(AppError::from)?;
    Ok(Json(result))
}
...
pub async fn ai_test_connection(
    ...
    let result = chiron_horizon_core::ai::test_connection_core(&config).await.map_err(AppError::from)?;
    Ok(Json(result))
}

pub async fn ai_resolve_endpoint(Json(body): Json<AiTestConnectionRequest>) -> Result<Json<String>, AppError> {
    reject_web_unsupported_ai_provider(&body.config)?;
    Ok(Json(chiron_horizon_core::ai::resolve_endpoint(&body.config)))
}
...
pub async fn ai_list_models(
    ...
    let result = chiron_horizon_core::ai::list_models_core(&config).await.map_err(AppError::from)?;
    Ok(Json(result))
}
...
pub async fn ai_resolve_model_effort(
    ...
    let result =
        chiron_horizon_core::ai::resolve_model_effort_core(&config, &body.model_id).await.map_err(AppError::from)?;
    Ok(Json(result))
}
...
pub async fn ai_cancel_stream(Json(body): Json<AiCancelStreamRequest>) -> Result<Json<bool>, AppError> {
    let result = chiron_horizon_core::ai::cancel_stream(&body.session_id).await;
    Ok(Json(result))
}
...
pub async fn ai_stream(
    ...
    let cancelled = chiron_horizon_core::ai::register_stream(&session_id).await;
    let (tx, rx) = tokio::sync::broadcast::channel::<String>(256);

    let sid = session_id.clone();
    tokio::spawn(async move {
        let result = chiron_horizon_core::ai::stream(&sid, &request, &cancelled, |chunk: AiStreamChunk| {
        ...
        chiron_horizon_core::ai::unregister_stream(&sid).await;
    });
...
pub async fn ai_agent_stream(
    ...
    let production_database =
        state.app.configs.read().await.get(&body.connection_id).is_some_and(|config| {
            chiron_horizon_core::production_safety::is_production_database(config, &body.database)
        });
    ...
    let max_agent_turns = state.app.storage.load_max_agent_turns().await.unwrap_or_else(|err| {
        ...
        chiron_horizon_core::agent_loop::DEFAULT_MAX_AGENT_TURNS
    });
    let max_retries = state.app.storage.load_max_retries().await.unwrap_or_else(|err| {
        ...
        chiron_horizon_core::ai::DEFAULT_MAX_RETRIES
    });
    ...
    let (allow_write_sql, confirmed_write_sql) = chiron_horizon_core::agent_tools::verify_confirmed_target(
    ...
    let sql_permissions = chiron_horizon_core::agent_tools::confirmed_write_sql_permissions(
        production_database,
    ...
    chiron_horizon_core::ai::merge_global_max_retries(&mut req_config, max_retries);
...
mod tests {
    ...
    use chiron_horizon_core::ai::{AiApiStyle, AiAuthMethod, AiConfig, AiProvider, AiReasoningLevel};
    ...
    async fn web_test_connection_respects_global_max_retries_zero() {
        ...
        let dir = std::env::temp_dir().join(format!("chiron-horizon-web-intg-max-retries-{}", uuid::Uuid::new_v4()));
