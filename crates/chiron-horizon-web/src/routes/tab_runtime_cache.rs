...
pub async fn list_tab_runtime_cache_metadata(
    State(state): State<Arc<WebState>>,
) -> Result<Json<Vec<chiron_horizon_core::storage::TabRuntimeCacheMetadata>>, AppError> {
...
) -> Result<Json<chiron_horizon_core::storage::TabRuntimeCachePruneResult>, AppError> {
    Ok(Json(
        state
            .app
            .storage
