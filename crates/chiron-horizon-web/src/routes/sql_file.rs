...
use axum::Json;
use chiron_horizon_core::sql::{self, SqlFileProgress, SqlFileRequest, SqlFileStatus};
...
async fn sql_file_upload_limit(state: &WebState) -> usize {
    let max_mb = state
        ...
        .await
        .unwrap_or(chiron_horizon_core::sql_file_import::DEFAULT_SQL_FILE_UPLOAD_MAX_MB);
...
) -> Result<Json<Vec<chiron_horizon_core::sql_file_import::SqlFileTable>>, AppError> {
    let path = validated_uploaded_sql_path(&state.data_dir, &body.file_path)?;
    Ok(Json(chiron_horizon_core::sql_file_import::inspect_sql_file_tables(&path).await.map_err(AppError::from)?))
}
...
pub async fn preview_sql_file(
    ...
    if let Some(field) = multipart.next_field().await.map_err(|e| AppError::from(e.to_string()))? {
        ...
        if file_name.to_ascii_lowercase().ends_with(".zip") {
            ...
            let package =
                chiron_horizon_core::sql_file_zip_package::extract_sql_file_zip_package(&file_path, &extraction_dir)
                    .map_err(AppError::from)?;
            let paths = chiron_horizon_core::sql_file_zip_package::extracted_sql_zip_paths(&extraction_dir, &package)
                .into_iter()
            ...
            let bootstrap_analysis =
                chiron_horizon_core::sql_file_import::mysql_like_sql_file_bootstrap_analysis(&content);
        ...
        let content = chiron_horizon_core::sql_file_import::read_sql_file_preview(&file_path, 1_000_000)
            .await
            .map_err(AppError::from)?;
        let preview: String = content.chars().take(20_000).collect();
        let bootstrap_analysis = chiron_horizon_core::sql_file_import::mysql_like_sql_file_bootstrap_analysis(&content);
