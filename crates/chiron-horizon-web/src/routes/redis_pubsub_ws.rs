...
pub async fn ws_handler(
    ...
    let owner = chiron_horizon_core::session_credentials::current_credential_owner();
    ws.on_upgrade(move |socket| async move {
        if params.monitor {
            chiron_horizon_core::session_credentials::with_credential_owner(
                owner,
...
async fn handle_monitor_socket(mut socket: WebSocket, state: Arc<WebState>, connection_id: String) {
    let monitor = tokio::select! {
        result = chiron_horizon_core::redis_ops::redis_create_monitor_core(&state.app, &connection_id) => result,
...
async fn handle_pubsub_socket(socket: WebSocket, state: Arc<WebState>, connection_id: String) {
    ...
    let pubsub = match chiron_horizon_core::redis_ops::redis_create_pubsub_core(&state.app, &connection_id).await {
        Ok(p) => p,
        Err(e) => {
