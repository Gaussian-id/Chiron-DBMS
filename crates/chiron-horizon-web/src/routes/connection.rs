...
use axum::Json;
use chiron_horizon_core::connection::{
...
use chiron_horizon_core::nacos::config::{
    take_transient_passwords, NACOS_CONSOLE_SESSION_PASSWORD, NACOS_PRIMARY_SESSION_PASSWORD,
};
use chiron_horizon_core::runtime_config::{
    release_runtime_config_on_disconnect, should_retain_runtime_config, TEST_PROBE_ID_PREFIX,
};
use chiron_horizon_core::session_credentials::{PurposeSessionCredentialWriteToken, SessionCredentialWriteToken};
        ...
        match app.nacos_admin_config_for_connection(&temp_id, &config).await {
            Ok(admin_config) => match app.nacos_registry.build_transient_config(admin_config).await {
                Ok(adapter) => adapter.test_connection_with_scope_validation().await.map(|info| {
                    nacos_database_info = chiron_horizon_core::nacos::service::database_info_from_connection(&info);
                    temp_id.clone()
                }),
                Err(error) => Err(error),
            },
        ...
        let success_message = if pool_result.is_ok() && config.db_type == DatabaseType::Consul {
            ...
            if let Some(client) = client {
                let configured_target =
                    chiron_horizon_core::consul::ConsulConfig::from_connection(&config)?.agent_target.is_some();
...
pub async fn connect_db(
    ...
    if config.db_type == chiron_horizon_core::models::connection::DatabaseType::Sqlite {
        chiron_horizon_core::db::sqlite::validate_persistent_attachments(
            &config.host,
            &config.password,
    ...
    if config.db_type == chiron_horizon_core::models::connection::DatabaseType::Plugin {
...
pub async fn connection_final_proxy_port(
    ...
    if runtime_config.db_type == chiron_horizon_core::models::connection::DatabaseType::Sqlite {
        chiron_horizon_core::db::sqlite::validate_persistent_attachments(
            &runtime_config.host,
    ...
    let db_config = chiron_horizon_core::connection::metadata_connection_config(&runtime_config);
...
async fn sync_connection_configs(state: &WebState, configs: &[ConnectionConfig]) -> ConnectionConfigSync {
    ...
    runtime_configs.retain(|id, existing| {
        if saved_ids.contains(id.as_str()) || should_retain_runtime_config(id, existing) {
            ...
            if existing.db_type == chiron_horizon_core::models::connection::DatabaseType::Nacos {
                nacos_adapter_ids_to_drop.insert(id.clone());
            }
            if existing.db_type == chiron_horizon_core::models::connection::DatabaseType::MessageQueue {
    ...
    for config in configs {
        if config.db_type == chiron_horizon_core::models::connection::DatabaseType::Nacos {
            nacos_adapter_ids_to_drop.insert(config.id.clone());
        }
        if config.db_type == chiron_horizon_core::models::connection::DatabaseType::MessageQueue {
            mq_adapter_ids_to_drop.insert(config.id.clone());
        }
        if let Some(previous) = runtime_configs.insert(config.id.clone(), config.clone()) {
            if previous.db_type == chiron_horizon_core::models::connection::DatabaseType::Nacos {
                nacos_adapter_ids_to_drop.insert(config.id.clone());
            }
            if previous.db_type == chiron_horizon_core::models::connection::DatabaseType::MessageQueue {
                mq_adapter_ids_to_drop.insert(config.id.clone());
            }
...
mod tests {
    ...
    async fn test_web_state() -> (Arc<WebState>, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-web-test-{}", uuid::Uuid::new_v4()));
    ...
    fn cookie_headers(token: &str) -> HeaderMap {
        ...
        headers.insert("cookie", format!("chiron_horizon_session={token}").parse().unwrap());
        headers
    }
    ...
    async fn connect_db_sanitizes_global_config_and_scopes_password_by_session() {
        ...
        let result =
            chiron_horizon_core::session_credentials::with_credential_owner(Some("token-a".to_string()), async move {
                connect_db(
                    State(connect_guard),
                    headers_a,
                    Json(ConnectRequest { config: config.clone(), client_attempt: None }),
        ...
        chiron_horizon_core::session_credentials::with_credential_owner(Some("token-a".to_string()), async {
            let mut db_config = chiron_horizon_core::connection::metadata_connection_config(&stored);
        ...
        chiron_horizon_core::session_credentials::with_credential_owner(Some("token-b".to_string()), async {
            let mut db_config = chiron_horizon_core::connection::metadata_connection_config(&stored);
            state.app.apply_session_credential(&stored, &mut db_config, "conn-a");
    ...
    async fn disconnect_db_keeps_connections_with_similar_prefixes() {
        ...
        let conn_pool = chiron_horizon_core::db::sqlite::connect_path(&conn_path.to_string_lossy()).await.unwrap();
        let conn2_pool = chiron_horizon_core::db::sqlite::connect_path(&conn2_path.to_string_lossy()).await.unwrap();
