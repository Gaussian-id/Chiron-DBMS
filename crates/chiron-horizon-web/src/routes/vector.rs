...
pub struct ChironDbRequest {
    ...
    pub request: chiron_horizon_core::db::chirondb::Request,
}
...
pub async fn chirondb_request(
...
) -> Result<Json<chiron_horizon_core::db::chirondb::Reply>, AppError> {
    ...
    chiron_horizon_core::db::chirondb::run(&state.app, &req.connection_id, req.request)
        .await
        .map(Json)
...
async fn ensure_writable(state: &WebState, connection_id: &str, action: &str) -> Result<(), AppError> {
    if let Some(name) = chiron_horizon_core::query::connection_readonly_name(&state.app, connection_id).await {
...
) -> Result<Json<chiron_horizon_core::db::vector_driver::CollectionInfo>, AppError> {
    chiron_horizon_core::schema::get_vector_collection_detail_core(
        &state.app,
        &req.connection_id,
        &req.database,
        &req.collection,
...
pub async fn drop_database(
    ...
    chiron_horizon_core::schema::drop_vector_database_core(&state.app, &req.connection_id, &database)
        .await
        .map_err(AppError::from)?;
...
pub async fn drop_collection(
    ...
    chiron_horizon_core::schema::drop_vector_collection_core(
        &state.app,
        &req.connection_id,
        &database,
        &req.collection,
...
pub async fn rename_collection(
    ...
    chiron_horizon_core::schema::rename_vector_collection_core(
        &state.app,
        &req.connection_id,
        &database,
        &req.collection,
