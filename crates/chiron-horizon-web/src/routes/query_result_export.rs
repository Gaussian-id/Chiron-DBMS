...
use chiron_horizon_core::query_result_export::{self, QueryResultExportRequest};
use chiron_horizon_core::table_export::{ExportStatus, TableExportProgress};
...
pub async fn start_query_result_export(
    ...
    chiron_horizon_core::export_runtime::spawn_export_task(async move {
        ...
        chiron_horizon_core::database_export::clear_export_cancelled(&req.export_id).await;
...
pub async fn cancel_query_result_export(
    ...
    chiron_horizon_core::database_export::set_export_cancelled(&req.export_id).await;
...
mod tests {
    ...
    async fn query_result_export_download_uses_the_requested_web_filename() {
        let dir =
            std::env::temp_dir().join(format!("chiron-horizon-web-query-export-download-{}", uuid::Uuid::new_v4()));
