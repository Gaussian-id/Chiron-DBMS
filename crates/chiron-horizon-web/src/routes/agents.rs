...
use chiron_horizon_core::driver_runtime::DriverRuntimeSummary;
use futures::Stream;
...
pub async fn get_driver_runtime_summary(
    ...
    Ok(Json(chiron_horizon_core::driver_runtime::collect_driver_runtime_summary(&state.app).await))
}
...
pub async fn stop_driver_runtime(
    ...
    chiron_horizon_core::driver_runtime::stop_driver_runtime(&state.app, &req.runtime_id)
        .await
...
pub async fn restart_driver_runtime(
    ...
    chiron_horizon_core::driver_runtime::restart_driver_runtime(&state.app, &req.runtime_id)
        .await
        .map_err(AppError::from)?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
...
pub async fn upgrade_all_agents(
    ...
    let result = async {
        ...
        let registry =
            fetch_registry_from_claimed(chiron_horizon_core::DownloadSource::Official, &[cancellation.as_ref()])
                .await
                .map_err(AppError::from)?;
...
pub async fn import_agents_from_zip(
    ...
    while let Some(field) = multipart.next_field().await.map_err(|err| AppError::from(err.to_string()))? {
        ...
        let result = async {
            ...
            .await
            .map_err(AppError::from)?;
            chiron_horizon_core::jdbc::import_offline_jdbc_payload(state.app.plugins.root_dir(), &package_path)
                .map_err(AppError::from)?;
...
async fn ensure_no_agent_update_blockers(
    state: &chiron_horizon_core::connection::AppState,
    db_types: &[String],
) -> Result<(), String> {
...
async fn agent_update_blockers(
    state: &chiron_horizon_core::connection::AppState,
    db_types: &[String],
...
fn update_blockers_from_keys(
    ...
    let mut blockers = active_keys
        ...
        .map(|db_type| AgentUpdateBlocker {
            label: chiron_horizon_core::agent_catalog::label_for_key(&db_type).unwrap_or(&db_type).to_string(),
            db_type,
        })
        .collect::<Vec<_>>();
...
mod tests {
    ...
    fn offline_package_extension_accepts_zip_and_tar_zstd() {
        assert_eq!(offline_package_extension("chiron-horizon-agents.zip"), Some("zip"));
        assert_eq!(offline_package_extension("chiron-horizon-agent-duckdb.TAR.ZST"), Some("tar.zst"));
        assert_eq!(offline_package_extension("chiron-horizon-agent-duckdb.zst"), None);
    }
}
