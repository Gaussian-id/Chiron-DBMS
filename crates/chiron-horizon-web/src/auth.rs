...
pub async fn login(State(state): State<Arc<WebState>>, Json(body): Json<LoginRequest>) -> Result<Response, StatusCode> {
    ...
    let cookie =
        format!("chiron_horizon_session={token}; Path={}; HttpOnly; SameSite=Lax", session_cookie_path(&state));
...
pub async fn logout(State(state): State<Arc<WebState>>, req: Request<axum::body::Body>) -> Response {
    ...
    let cookie = format!("chiron_horizon_session=; Path={}; HttpOnly; Max-Age=0", session_cookie_path(&state));
...
pub fn session_token_from_headers(headers: &axum::http::HeaderMap) -> Option<String> {
    ...
    for pair in cookie_header.split(';') {
        let pair = pair.trim();
        if let Some(value) = pair.strip_prefix("chiron_horizon_session=") {
            if !value.is_empty() {
...
pub async fn auth_middleware(
    ...
    if let Some(ref token) = token {
        if state.sessions.read().await.contains(token) {
            ...
            return chiron_horizon_core::session_credentials::with_credential_owner(Some(owner), async move {
                next.run(req).await
            })
            .await;
        }
...
mod tests {
    ...
    fn api_path_suffix_handles_root_api_paths() {
        ...
        assert_eq!(api_path_suffix("/chiron-horizon/api/auth/check", "/"), None);
    }

    #[test]
    fn api_path_suffix_handles_mounted_api_paths() {
        assert_eq!(api_path_suffix("/chiron-horizon/api/auth/check", "/chiron-horizon"), Some("auth/check"));
        assert_eq!(
            api_path_suffix("/tools/chiron-horizon/api/query/execute", "/tools/chiron-horizon"),
            Some("query/execute")
        );
        assert_eq!(api_path_suffix("/chiron-horizon/login", "/chiron-horizon"), None);
    }
    ...
    fn middleware_api_path_suffix_handles_nested_router_paths() {
        ...
        assert_eq!(
            middleware_api_path_suffix("/chiron-horizon/api/connection/list", "/chiron-horizon"),
            Some("connection/list")
        );
        assert_eq!(middleware_api_path_suffix("/chiron-horizon/login", "/chiron-horizon"), None);
    }
}
