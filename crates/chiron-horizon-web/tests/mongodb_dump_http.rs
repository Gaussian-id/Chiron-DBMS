...
async fn database_dump_upload_and_progress_http_contract() {
    ...
    let mut server = tokio::process::Command::new(env!("CARGO_BIN_EXE_chiron-horizon-web"))
        .env("CHIRON_HORIZON_DATA_DIR", directory.path())
        .env("CHIRON_HORIZON_PORT", port.to_string())
        .env("CHIRON_HORIZON_DISABLE_PASSWORD", "1")
        .kill_on_drop(true)
        .stdout(Stdio::null())
