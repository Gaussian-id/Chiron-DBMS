...
const CONSUL_BUNDLE_FORMAT: &str = "chiron-horizon-consul-kv-bundle";
...
fn validate_bundle(bundle: &ConsulKvBundle) -> Result<(), String> {
    if bundle.format != CONSUL_BUNDLE_FORMAT || bundle.version != CONSUL_BUNDLE_VERSION {
        return Err("CONSUL_BUNDLE_UNSUPPORTED: Expected Chiron Horizon Consul KV bundle version 1".to_string());
    }
