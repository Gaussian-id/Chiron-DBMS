...
const CHIRON_HORIZON_ROWID_COLUMN: &str = "__CHIRON_HORIZON_ROWID";
pub(crate) const CHIRON_HORIZON_NEO4J_ELEMENT_ID_COLUMN: &str = "__CHIRON_HORIZON_ELEMENT_ID";
pub(crate) const CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN: &str = "tbname";
...
const KEYLESS_GUARD_COUNT_ALIAS: &str = "chiron_horizon_keyless_row_matches";
...
pub fn build_data_grid_column_distinct_values_sql(options: DataGridColumnDistinctValuesSqlOptions) -> String {
    ...
    let select_list = if options.include_counts {
        format!("{column} AS chiron_horizon_value, COUNT(*) AS chiron_horizon_count")
    } else {
        format!("{column} AS chiron_horizon_value")
    };
    let group_by = format!(" GROUP BY {column}");
    let order_by = if options.include_counts {
        " ORDER BY chiron_horizon_count DESC, chiron_horizon_value"
    } else {
        " ORDER BY chiron_horizon_value"
    };
...
fn build_neo4j_data_grid_column_distinct_values_sql(options: &DataGridColumnDistinctValuesSqlOptions) -> String {
    ...
    if options.include_counts {
        format!(
            "MATCH (n:{label}){where_clause} RETURN {column} AS chiron_horizon_value, count(*) AS chiron_horizon_count ORDER BY chiron_horizon_count DESC, chiron_horizon_value LIMIT {limit}"
        )
    } else {
        format!(
            "MATCH (n:{label}){where_clause} RETURN DISTINCT {column} AS chiron_horizon_value ORDER BY chiron_horizon_value LIMIT {limit}"
        )
    }
}
...
pub(crate) fn format_grid_sql_literal_with_identifier_quote(
    ...
    if is_mysql_binary_literal_column(database_type, column_info) {
        if let Some(literal) = format_mysql_binary_literal_text(&text) {
            // Chiron Horizon result values expose binary columns as prefixed hex; keep them
...
fn is_synthetic_row_id(database_type: Option<DatabaseType>, name: Option<&str>) -> bool {
    uses_synthetic_row_id(database_type)
        && name.is_some_and(|name| name.eq_ignore_ascii_case(CHIRON_HORIZON_ROWID_COLUMN))
}
...
mod tests {
    ...
    fn builds_oracle_synthetic_rowid_context_filter_conditions() {
        ...
        for database_type in [DatabaseType::Oracle, DatabaseType::OceanbaseOracle] {
            assert_eq!(
                equals(Some(database_type), CHIRON_HORIZON_ROWID_COLUMN, json!("AAAFd1AAFAAAABSAA/")),
        ...
        assert_eq!(
            equals(None, CHIRON_HORIZON_ROWID_COLUMN, json!("row-id")),
            Some("\"__CHIRON_HORIZON_ROWID\" = 'row-id'".to_string())
        );
        assert_eq!(equals(Some(DatabaseType::VictoriaMetrics), CHIRON_HORIZON_ROWID_COLUMN, json!("row-id")), None);
    }
    ...
    fn builds_column_distinct_values_sql() {
        assert_eq!(
            ...
            "SELECT \"status\" AS chiron_horizon_value, COUNT(*) AS chiron_horizon_count FROM \"public\".\"users\" WHERE (deleted_at IS NULL) AND \"status\" LIKE '%act%' GROUP BY \"status\" ORDER BY chiron_horizon_count DESC, chiron_horizon_value LIMIT 1000"
        );
        assert_eq!(
            build_data_grid_column_distinct_values_sql(DataGridColumnDistinctValuesSqlOptions {
                database_type: Some(DatabaseType::SqlServer),
            ...
            "SELECT TOP (25) [status] AS chiron_horizon_value FROM [users] GROUP BY [status] ORDER BY chiron_horizon_value"
        );
        assert_eq!(
            build_data_grid_column_distinct_values_sql(DataGridColumnDistinctValuesSqlOptions {
                database_type: Some(DatabaseType::SqlServer),
            ...
            "SELECT TOP (25) [id] AS chiron_horizon_value, COUNT(*) AS chiron_horizon_count FROM [users] WHERE [id] = 42 GROUP BY [id] ORDER BY chiron_horizon_count DESC, chiron_horizon_value"
        );
        assert_eq!(
            build_data_grid_column_distinct_values_sql(DataGridColumnDistinctValuesSqlOptions {
                database_type: Some(DatabaseType::SqlServer),
            ...
            "SELECT [status] AS chiron_horizon_value, COUNT(*) AS chiron_horizon_count FROM [users] GROUP BY [status] ORDER BY chiron_horizon_count DESC, chiron_horizon_value"
        );
        assert_eq!(
            build_data_grid_column_distinct_values_sql(DataGridColumnDistinctValuesSqlOptions {
                database_type: Some(DatabaseType::Oracle),
            ...
            "SELECT * FROM (SELECT \"KIND\" AS chiron_horizon_value, COUNT(*) AS chiron_horizon_count FROM \"APP\".\"EVENTS\" GROUP BY \"KIND\" ORDER BY chiron_horizon_count DESC, chiron_horizon_value) WHERE ROWNUM <= 10"
        );
        assert_eq!(
            build_data_grid_column_distinct_values_sql(DataGridColumnDistinctValuesSqlOptions {
            ...
            "SELECT \"STATUS\" AS chiron_horizon_value FROM \"USERS\" WHERE (DELETED_AT IS NULL) GROUP BY \"STATUS\" ORDER BY chiron_horizon_value ROWS 25"
        );
        ...
        assert_eq!(
            ...
            "SELECT `status` AS chiron_horizon_value FROM `iceberg_catalog`.`sales`.`orders` GROUP BY `status` ORDER BY chiron_horizon_value LIMIT 10"
        );
        assert_eq!(
            build_data_grid_column_distinct_values_sql(DataGridColumnDistinctValuesSqlOptions {
            ...
            "SELECT `status` AS chiron_horizon_value, COUNT(*) AS chiron_horizon_count FROM `hive_catalog`.`orders` GROUP BY `status` ORDER BY chiron_horizon_count DESC, chiron_horizon_value LIMIT 10"
        );
        ...
        assert_eq!(
            ...
            "SELECT `status` AS chiron_horizon_value FROM `orders` GROUP BY `status` ORDER BY chiron_horizon_value LIMIT 10"
        );
    }
    ...
    fn vastbase_query_result_writes_use_resolved_search_path_schema() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            deleted_rows: vec![],
            new_rows: vec![vec![json!("chiron-horizon-insert-check"), json!(461936049002043_i64)]],
        });
        ...
        assert_eq!(
            ...
            vec![
                ...
                "INSERT INTO \"tenant_b\".\"TBLCUSPOSTMATERIALLOG\" (\"MONO\", \"ID\") VALUES ('chiron-horizon-insert-check', 461936049002043);",
            ]
        );
    }
    ...
    fn prepares_oceanbase_oracle_lob_deletes_with_synthetic_rowid() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            table_meta: DataGridTableMeta {
                ...
                columns: Some(vec![
                    column(CHIRON_HORIZON_ROWID_COLUMN, "VARCHAR2", false, None),
            ...
            columns: vec![
                CHIRON_HORIZON_ROWID_COLUMN.to_string(),
                "ID".to_string(),
    ...
    fn guards_sqlite_keyless_update_with_a_server_side_row_count() {
        ...
        assert_eq!(
            ...
            vec![DataGridSaveGuard {
                sql: r#"SELECT COUNT(*) AS chiron_horizon_keyless_row_matches FROM "daily_stats" WHERE ("stat_date" = '2026-09-07' AND "period" IS NULL)"#
                    .to_string(),
                max_matched_rows: 1,
                message: KEYLESS_AMBIGUOUS_ROW_ERROR.to_string(),
            }]
    ...
    fn dameng_data_grid_writes_do_not_double_escape_backslashes() {
        ...
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            table_meta: DataGridTableMeta {
                ...
                table_name: "CHIRON_HORIZON_NEWLINE_REPRO".to_string(),
                primary_keys: vec!["ID".to_string()],
        ...
        assert_eq!(
            result.statements,
            vec![
                r#"UPDATE "CHIRON_HORIZON_TEST"."CHIRON_HORIZON_NEWLINE_REPRO" SET "VAL" = 'line\n''s' WHERE "ID" = 1;"#,
                r#"INSERT INTO "CHIRON_HORIZON_TEST"."CHIRON_HORIZON_NEWLINE_REPRO" ("ID", "VAL") VALUES (2, '\n');"#,
            ]
        );
    }
    ...
    fn prepares_sqlserver_updates_without_doubling_backslashes() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            table_meta: DataGridTableMeta {
                ...
                database: None,
                schema: Some("dbo".to_string()),
                table_name: "chiron_horizon_issue_4181".to_string(),
                primary_keys: vec!["id".to_string()],
        ...
        assert_eq!(
            result.statements,
            vec![r"UPDATE [dbo].[chiron_horizon_issue_4181] SET [value] = N'.\SQL2016' WHERE [id] = 1;"]
        );
    }
    ...
    fn prepares_tdengine_child_table_delete_from_stable_row() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            table_meta: DataGridTableMeta {
                catalog: None,
                database: None,
                schema: Some("chiron_horizon_tdengine_demo".to_string()),
                table_name: "meters".to_string(),
                primary_keys: vec![CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN.to_string(), "ts".to_string()],
                columns: Some(vec![column("ts", "TIMESTAMP", false, None), column("voltage", "FLOAT", true, None)]),
            },
            columns: vec![CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN.to_string(), "ts".to_string(), "voltage".to_string()],
            source_columns: None,
        ...
        assert_eq!(
            result.statements,
            vec!["DELETE FROM `chiron_horizon_tdengine_demo`.`codex_delete_verify` WHERE `ts` = '2026-07-10T13:59:00.456+08:00';"]
        );
        assert_eq!(
            result.rollback_statements,
            vec!["INSERT INTO `chiron_horizon_tdengine_demo`.`meters` (`tbname`, `ts`, `voltage`) VALUES ('codex_delete_verify', '2026-07-10T13:59:00.456+08:00', 221.5);"]
        );
    }
    ...
    fn rejects_tdengine_composite_key_delete_from_same_timestamp_stable_rows() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            table_meta: DataGridTableMeta {
                ...
                table_name: "meters".to_string(),
                primary_keys: vec![
                    CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN.to_string(),
                    "ts".to_string(),
                    "seq".to_string(),
                ],
                columns: Some(vec![
    ...
    fn prepares_tdengine_delete_from_direct_child_table_row() {
        ...
        assert_eq!(
            result.statements,
            vec!["DELETE FROM `chiron_horizon_tdengine_demo`.`codex_grid_accept_20260710` WHERE `ts` = '2026-07-10T16:00:00.111+08:00';"]
        );
        assert_eq!(
            result.rollback_statements,
            vec!["INSERT INTO `chiron_horizon_tdengine_demo`.`codex_grid_accept_20260710` (`ts`, `voltage`) VALUES ('2026-07-10T16:00:00.111+08:00', 220.1);"]
        );
    }
    ...
    fn prepares_tdengine_overwrite_for_direct_child_table_row() {
        ...
        assert_eq!(
            result.statements,
            vec!["INSERT INTO `chiron_horizon_tdengine_demo`.`codex_grid_update_verify_20260710` (`ts`, `voltage`, `current`) VALUES ('2026-07-10T16:30:00.444+08:00', 229.9, 1.0);"]
        );
        assert_eq!(
            result.rollback_statements,
            vec!["INSERT INTO `chiron_horizon_tdengine_demo`.`codex_grid_update_verify_20260710` (`ts`, `voltage`, `current`) VALUES ('2026-07-10T16:30:00.444+08:00', 220.0, 1.0);"]
        );
    }
    ...
    fn prepares_tdengine_composite_key_overwrite_and_rollback_for_same_timestamp_child_rows() {
        ...
        assert_eq!(
            result.statements,
            vec!["INSERT INTO `chiron_horizon_tdengine_demo`.`device_a` (`ts`, `seq`, `voltage`) VALUES ('2026-07-10T16:30:00.444+08:00', 2, 229.9);"]
        );
        assert_eq!(
            result.rollback_statements,
            vec!["INSERT INTO `chiron_horizon_tdengine_demo`.`device_a` (`ts`, `seq`, `voltage`) VALUES ('2026-07-10T16:30:00.444+08:00', 2, 221.0);"]
        );
    }
    ...
    fn prepares_tdengine_stable_insert_with_child_table_identity() {
        ...
        assert_eq!(
            result.statements,
            vec!["INSERT INTO `chiron_horizon_tdengine_demo`.`issue_3121_devices` (`tbname`, `ts`, `reading`, `site`) VALUES ('codex_issue3121_insert_verify', '2026-07-10T17:48:51.000+08:00', 1.0, 'codex-lab');"]
        );
        assert_eq!(
            result.rollback_statements,
            vec!["DELETE FROM `chiron_horizon_tdengine_demo`.`codex_issue3121_insert_verify` WHERE `ts` = '2026-07-10T17:48:51.000+08:00';"]
        );
    }
    ...
    fn skips_tdengine_composite_key_insert_rollback_for_same_timestamp_rows() {
        ...
        assert_eq!(
            result.statements,
            vec![
                "INSERT INTO `chiron_horizon_tdengine_demo`.`meters` (`tbname`, `ts`, `seq`, `voltage`) VALUES ('device_a', '2026-07-10T17:48:51.000+08:00', 1, 221.0);",
                "INSERT INTO `chiron_horizon_tdengine_demo`.`meters` (`tbname`, `ts`, `seq`, `voltage`) VALUES ('device_a', '2026-07-10T17:48:51.000+08:00', 2, 222.0);",
            ]
        );
        assert!(result.rollback_statements.is_empty());
    }
    ...
    fn rejects_tdengine_stable_insert_without_child_table_identity() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            columns: vec![CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN.to_string(), "ts".to_string(), "reading".to_string()],
            source_columns: None,
            rows: vec![],
            dirty_rows: vec![],
            deleted_rows: vec![],
    ...
    fn prepare_data_grid_save_omits_hidden_oracle_rowid_from_cloned_row_insert() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            columns: vec!["ID".to_string(), "PLATFORM".to_string(), "__CHIRON_HORIZON_PK_0".to_string()],
            source_columns: Some(vec![
                Some("ID".to_string()),
                Some("PLATFORM".to_string()),
                Some(CHIRON_HORIZON_ROWID_COLUMN.to_string()),
            ]),
            rows: vec![],
            dirty_rows: vec![],
            deleted_rows: vec![],
    ...
    fn prepares_xugu_rowid_updates_deletes_and_inserts_without_writing_synthetic_key() {
        let result = prepare_data_grid_save(DataGridSaveStatementOptions {
            ...
            table_meta: DataGridTableMeta {
                ...
                primary_keys: vec![CHIRON_HORIZON_ROWID_COLUMN.to_string()],
                columns: Some(vec![
                    column(CHIRON_HORIZON_ROWID_COLUMN, "ROWID", false, None),
                    column("ID", "INTEGER", false, None),
                    column("VALUE", "VARCHAR(40)", true, None),
                ]),
            },
            columns: vec![CHIRON_HORIZON_ROWID_COLUMN.to_string(), "ID".to_string(), "VALUE".to_string()],
            source_columns: None,
