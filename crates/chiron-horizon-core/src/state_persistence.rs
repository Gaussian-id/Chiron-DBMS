...
mod tests {
    ...
    fn temp_db_path(name: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("chiron-horizon-state-persistence-{name}-{}-{stamp}.db", std::process::id()))
    }
    ...
    fn s3_backend_constructor() {
        let backend = S3Backend::new("my-bucket", "us-east-1", "AKID", "secret", None, "chiron-horizon/state");
        assert_eq!(backend.object_key("test"), "chiron-horizon/state/test");
    }
