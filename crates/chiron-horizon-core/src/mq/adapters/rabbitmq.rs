...
mod tests {
    ...
    fn connection_params_map_addresses_vhost_and_basic_auth() {
        ...
        assert_eq!(params.pointer("/properties/connectionName").and_then(|v| v.as_str()), Some("chiron-horizon"));
    }
    ...
    fn user_entries_parse_from_agent_payload() {
        let payload = serde_json::json!({
            "users": [
                { "name": "chiron-horizon-app", "tags": ["management"] },
                { "name": "chiron-horizon-svc" }
            ]
        });
        ...
        assert_eq!(users[0].name, "chiron-horizon-app");
        assert_eq!(users[0].tags, vec!["management".to_string()]);
        assert_eq!(users[1].name, "chiron-horizon-svc");
    ...
    fn permission_entries_parse_from_agent_payload() {
        let payload = serde_json::json!({
            "permissions": [
                { "user": "chiron-horizon-app", "vhost": "orders", "configure": ".*", "write": "chiron-horizon-.*", "read": ".*" },
                { "user": "chiron-horizon-app", "vhost": "/", "configure": "", "write": "", "read": "" }
            ]
        });
        ...
        assert_eq!(permissions[0].user, "chiron-horizon-app");
        assert_eq!(permissions[0].vhost, "orders");
        assert_eq!(permissions[0].write, "chiron-horizon-.*");
    ...
    fn binding_params_match_agent_contract_and_attach_vhost() {
        let binding = MqBindingInfo {
            source: "chiron-horizon-events".to_string(),
            destination: "chiron-horizon-queue".to_string(),
        ...
        assert_eq!(params.get("source").and_then(|v| v.as_str()), Some("chiron-horizon-events"));
        assert_eq!(params.get("destination").and_then(|v| v.as_str()), Some("chiron-horizon-queue"));
        ...
        let bare = MqBindingInfo {
            source: "chiron-horizon-a".to_string(),
            destination: "chiron-horizon-b".to_string(),
    ...
    fn map_item_namespace_moves_vhost_into_namespace_field() {
        let item = map_item_namespace(serde_json::json!({ "name": "chiron-horizon-q", "vhost": "orders" }));
        assert_eq!(item.get("namespace").and_then(|v| v.as_str()), Some("orders"));

        // Entries without a vhost stay untouched (single-vhost listings).
        let plain = map_item_namespace(serde_json::json!({ "name": "chiron-horizon-q" }));
        ...
        let exchange: MqExchangeInfo = serde_json::from_value(map_item_namespace(
            serde_json::json!({ "name": "chiron-horizon-ex", "type": "direct", "vhost": "orders" }),
        ))
    ...
    fn policy_entries_parse_from_agent_payload() {
        ...
        assert_eq!(policies[0].name, "chiron-horizon-ttl");
    ...
    fn policy_params_match_agent_contract_and_attach_vhost() {
        ...
        let policy = MqPolicyInfo {
            name: "chiron-horizon-ttl".to_string(),
            vhost: String::new(),
            pattern: "^chiron-horizon-".to_string(),
        ...
        assert_eq!(params.get("name").and_then(|v| v.as_str()), Some("chiron-horizon-ttl"));
        assert_eq!(params.get("pattern").and_then(|v| v.as_str()), Some("^chiron-horizon-"));
    ...
    fn policy_params_omit_empty_apply_to_so_agent_default_applies() {
        let policy = MqPolicyInfo {
            name: "chiron-horizon-ha".to_string(),
            pattern: ".*".to_string(),
