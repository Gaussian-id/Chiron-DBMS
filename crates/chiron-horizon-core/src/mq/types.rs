...
mod tests {
    ...
    fn exchange_info_round_trips_agent_camel_case() {
        let exchange: super::MqExchangeInfo = serde_json::from_str(
            r#"{"name":"chiron-horizon-events","type":"topic","durable":true,"autoDelete":false,"internal":false}"#,
        )
        .expect("exchange info");
        assert_eq!(exchange.name, "chiron-horizon-events");
    ...
    fn binding_info_skips_absent_routing_key_and_arguments() {
        let binding: super::MqBindingInfo = serde_json::from_str(
            r#"{"source":"chiron-horizon-events","destination":"chiron-horizon-queue","destinationType":"queue","routingKey":"orders.*"}"#,
        )
        .expect("binding info");
        assert_eq!(binding.source, "chiron-horizon-events");
        ...
        let bare: super::MqBindingInfo = serde_json::from_str(
            r#"{"source":"chiron-horizon-a","destination":"chiron-horizon-b","destinationType":"exchange"}"#,
        )
    ...
    fn send_message_request_defaults_new_rabbitmq_fields() {
        let req: super::SendMessageRequest =
            serde_json::from_str(r#"{"topic":"chiron-horizon-queue","payloadBase64":"aGVsbG8="}"#)
                .expect("send request");
    ...
    fn user_info_round_trips_agent_camel_case() {
        let user: super::MqUserInfo =
            serde_json::from_str(r#"{"name":"chiron-horizon-app","tags":["management","policymaker"]}"#)
                .expect("user info");
        assert_eq!(user.name, "chiron-horizon-app");
        assert_eq!(user.tags, vec!["management".to_string(), "policymaker".to_string()]);

        // Users without tags default to an empty list.
        let bare: super::MqUserInfo =
            serde_json::from_str(r#"{"name":"chiron-horizon-svc"}"#).expect("user without tags");
        assert!(bare.tags.is_empty());

        let json = serde_json::to_value(&user).expect("serialize user info");
        assert_eq!(json.get("name").and_then(|v| v.as_str()), Some("chiron-horizon-app"));
    ...
    fn vhost_permission_round_trips_agent_fields() {
        let perm: super::MqVhostPermission = serde_json::from_str(
            r#"{"user":"chiron-horizon-app","vhost":"orders","configure":".*","write":"chiron-horizon-.*","read":".*"}"#,
        )
        .expect("vhost permission");
        assert_eq!(perm.user, "chiron-horizon-app");
        assert_eq!(perm.vhost, "orders");
        assert_eq!(perm.configure, ".*");
        assert_eq!(perm.write, "chiron-horizon-.*");
        assert_eq!(perm.read, ".*");
        ...
        assert_eq!(json.get("write").and_then(|v| v.as_str()), Some("chiron-horizon-.*"));
    }
    ...
    fn policy_info_round_trips_agent_fields() {
        let policy: super::MqPolicyInfo = serde_json::from_str(
            r#"{"name":"chiron-horizon-ttl","vhost":"orders","pattern":"^chiron-horizon-","applyTo":"queues","priority":5,"definition":{"message-ttl":60000,"max-length":1000}}"#,
        )
        .expect("policy info");
        assert_eq!(policy.name, "chiron-horizon-ttl");
        assert_eq!(policy.vhost, "orders");
        assert_eq!(policy.pattern, "^chiron-horizon-");
    ...
    fn policy_info_defaults_absent_apply_to_and_priority() {
        let policy: super::MqPolicyInfo = serde_json::from_str(
            r#"{"name":"chiron-horizon-ha","vhost":"/","pattern":".*","definition":{"ha-mode":"all"}}"#,
        )
