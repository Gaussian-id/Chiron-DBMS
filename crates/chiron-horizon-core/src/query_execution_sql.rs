...
fn classify_dynamodb_statement_write(source: &str) -> Option<bool> {
    ...
    match header.as_str() {
        "CHIRON HORIZON DYNAMODB SCAN" | "CHIRON HORIZON DYNAMODB QUERY / SCAN" => Some(false),
        "CHIRON HORIZON DYNAMODB INSERT ITEM"
        | "CHIRON HORIZON DYNAMODB PUT ITEM"
        | "CHIRON HORIZON DYNAMODB DELETE ITEM" => Some(true),
        _ if header.starts_with("CHIRON HORIZON DYNAMODB") => Some(true),
        _ => None,
    }
}
...
/// only for an ordinary top-level read that Chiron Horizon can prove harmless: a single
...
pub fn is_oracle_proven_read_only_statement(sql: &str) -> bool {
    ...
    // functions, and subqueries Chiron Horizon cannot prove harmless.
    if lower.contains('(') {
        return false;
    }
...
mod tests {
    ...
    fn is_write_sql_blocks_mysql_and_mariadb_executable_comments() {
        for sql in [
            "SELECT 3156 /*! INTO OUTFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.txt' */",
            "SELECT 3156 /*!50000 INTO OUTFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.txt' */",
            "SELECT 3156 /*M! INTO OUTFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.txt' */",
            "SELECT 3156 /*M!100100 INTO OUTFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.txt' */",
        ] {
    ...
    fn is_write_sql_blocks_dialect_specific_select_into_writes() {
        for sql in [
            "SELECT 3156 INTO OUTFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.txt'",
            "SELECT 3156 INTO DUMPFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.bin'",
            "WITH probe AS (SELECT 3156) SELECT * FROM probe INTO OUTFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.txt'",
        ] {
            assert!(is_write_sql_for_database(sql, DatabaseType::Mysql), "expected write SQL: {sql}");
        }
    ...
    fn check_read_only_blocks_dialect_specific_select_into_writes() {
        for (sql, database_type) in [
            ("SELECT 3156 INTO OUTFILE '/var/lib/mysql-files/chiron_horizon_ro_probe.txt'", DatabaseType::Mysql),
    ...
    fn classifies_dynamodb_generated_statements_for_read_only_protection() {
        ...
        for operation in ["INSERT ITEM", "PUT ITEM", "DELETE ITEM", "UNKNOWN"] {
            assert!(is_write_sql_for_database(
                &format!("Chiron Horizon DYNAMODB {operation}\ntable: \"orders\""),
                DatabaseType::DynamoDb
            ));
        }
    }
    ...
    fn excludes_victoriametrics_from_sql_query_paths() {
        ...
        assert_eq!(
            check_read_only(
                r#"{__name__="chiron_horizon_issue_5352_temperature_celsius"}[5m]"#,
                "VictoriaMetrics",
                DatabaseType::VictoriaMetrics,
            ),
            Ok(())
