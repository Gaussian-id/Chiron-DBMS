...
pub fn resolve_env_var(name: &str) -> Result<String, String> {
    crate::legacy::var(name).map_err(|_| format!("Environment variable '${name}' not set"))
}
...
mod tests {
    ...
    fn test_resolve_env_var() {
        std::env::set_var("CHIRON_HORIZON_TEST_VAR", "test_value");
        let result = resolve_env_var("CHIRON_HORIZON_TEST_VAR");
        assert_eq!(result.unwrap(), "test_value");
        std::env::remove_var("CHIRON_HORIZON_TEST_VAR");
    }

    #[test]
    fn test_resolve_env_var_missing() {
        let result = resolve_env_var("CHIRON_HORIZON_NONEXISTENT_VAR_12345");
    ...
    fn test_resolve_expression_env_var() {
        std::env::set_var("CHIRON_HORIZON_TEST_PORT", "8080");
        let expr = Expression::EnvVar("CHIRON_HORIZON_TEST_PORT".to_string());
        let merged = HashMap::new();
        let result = resolve_expression(&expr, &merged).unwrap();
        assert_eq!(result, serde_json::Value::String("8080".to_string()));
        std::env::remove_var("CHIRON_HORIZON_TEST_PORT");
    }
