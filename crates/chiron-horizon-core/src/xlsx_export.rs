...
mod tests {
    ...
    fn builds_multi_sheet_xlsx_workbook() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-multi-sheet-test-{}.xlsx", uuid::Uuid::new_v4()));
    ...
    fn streams_xlsx_rows_to_a_readable_workbook() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-stream-test-{}.xlsx", uuid::Uuid::new_v4()));
        {
    ...
    fn streams_mysql_numeric_strings_as_numeric_cells() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-mysql-stream-test-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
    ...
    fn streaming_temporal_cells_keep_the_configured_excel_display_format() {
        let path =
            std::env::temp_dir().join(format!("chiron-horizon-temporal-stream-test-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
    ...
    fn streams_xlsx_rows_with_a_trailing_sql_worksheet() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-stream-sql-test-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
    ...
    fn build_multi_sheet_xlsx(rows: &[Vec<Value>], max_rows: usize) -> Vec<u8> {
        let path = std::env::temp_dir().join(format!("chiron-horizon-multi-split-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
    ...
    fn trailing_sheet_placed_after_data_sheets() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-trailing-split-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
            let sql_sheet = XlsxWorksheetData {
                sheet_name: Some("SQL".to_string()),
    ...
    fn varchar_leading_zeros_preserved_across_sheets() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-leading-zeros-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
            let mut writer = start_streaming_xlsx_workbook_with_max_rows(
                file,
    ...
    fn deduplicates_identically_named_trailing_sheets() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-trailing-dedup-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
    ...
    fn streaming_continuation_names_use_original_base_when_first_data_name_collides_with_trailing() {
        ...
        let path = std::env::temp_dir().join(format!("chiron-horizon-collision-base-{}.xlsx", uuid::Uuid::new_v4()));
        {
            let file = fs::File::create(&path).expect("create temp xlsx");
            let sql_sheet = XlsxWorksheetData {
                sheet_name: Some("SQL".to_string()),
