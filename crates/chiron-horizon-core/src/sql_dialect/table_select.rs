...
use super::types::{
    TableDataSelectSqlOptions, TableSelectSqlOptions, CHIRON_HORIZON_NEO4J_ELEMENT_ID_COLUMN,
    CHIRON_HORIZON_ROWID_COLUMN, CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN,
};

pub const CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX: &str = "__CHIRON_HORIZON_LARGE_VALUE_BYTES_";
...
fn build_large_value_preview_columns(options: &TableDataSelectSqlOptions) -> Option<String> {
    ...
    if options.columns.is_empty()
        ...
        || options
            .columns
            .iter()
            .any(|column| column.to_ascii_uppercase().starts_with(CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX))
    {
        return None;
    }
    ...
    for (column_index, (column, data_type)) in options.columns.iter().zip(&options.column_types).enumerate() {
        ...
        let marker_alias = quote_table_identifier(
            database_type,
            &format!("{CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX}{alias_kind}_{column_index}"),
        );
...
pub fn build_table_data_select_sql_with_database(
    ...
    let select_columns = if include_oracle_row_id {
        format!("ROWIDTOCHAR(t.ROWID) AS \"{CHIRON_HORIZON_ROWID_COLUMN}\", t.*")
    } else if let Some(preview_columns) = build_large_value_preview_columns(&options) {
        preview_columns
    } else if include_xugu_row_id {
        if options.columns.is_empty() {
            format!("ROWID AS \"{CHIRON_HORIZON_ROWID_COLUMN}\", *")
        } else {
            format!(
                "ROWID AS \"{CHIRON_HORIZON_ROWID_COLUMN}\", {}",
    ...
    let page_select_columns = if include_oracle_row_id {
        if options.columns.is_empty() {
            ...
            let rest = options
                .columns
                .iter()
                .filter(|column| !column.eq_ignore_ascii_case(CHIRON_HORIZON_ROWID_COLUMN))
                .map(|column| quote_table_identifier(database_type, column))
                .collect::<Vec<_>>()
                .join(", ");
            if rest.is_empty() {
                format!("\"{CHIRON_HORIZON_ROWID_COLUMN}\"")
            } else {
                format!("\"{CHIRON_HORIZON_ROWID_COLUMN}\", {rest}")
            }
        }
    } else {
...
fn build_rownum_table_select_sql(
    ...
    let row_number_alias = quote_table_identifier(Some(DatabaseType::Oracle), "__chiron_horizon_row_num");
    let end = offset + limit;
    format!(
        "SELECT {outer_select_columns} FROM (SELECT chiron_horizon_inner.*, ROWNUM AS {row_number_alias} FROM ({inner_select}) chiron_horizon_inner WHERE ROWNUM <= {end}) WHERE {row_number_alias} > {offset}"
    )
}

pub(super) fn is_tdengine_tbname(database_type: Option<DatabaseType>, name: &str) -> bool {
    database_type == Some(DatabaseType::Tdengine) && name.eq_ignore_ascii_case(CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN)
}
...
pub(super) fn build_select_columns(
    ...
    if columns.is_empty() {
        if database_type == Some(DatabaseType::Tdengine) && include_tdengine_tbname {
            return format!("{CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN}, *");
        }
        return "*".to_string();
    }
    if database_type == Some(DatabaseType::Tdengine) {
        ...
        if include_tdengine_tbname
            && !columns.iter().any(|column| column.eq_ignore_ascii_case(CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN))
        {
            tdengine_columns.push(CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN.to_string());
        }
        tdengine_columns.extend(
            columns
                .iter()
                .filter(|column| {
                    include_tdengine_tbname || !column.eq_ignore_ascii_case(CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN)
                })
                .cloned(),
        );
...
pub(super) fn build_sqlserver_table_select_sql(
    ...
    let page_alias = quote_table_identifier(Some(DatabaseType::SqlServer), "chiron_horizon_page");
    let row_number_alias = quote_table_identifier(Some(DatabaseType::SqlServer), "__chiron_horizon_row_num");
    let end = offset + limit;
    format!(
...
pub(super) fn build_db2_table_select_page_sql(
    ...
    let inner_columns = if columns.is_empty() {
        "chiron_horizon_t.*".to_string()
    } else {
        columns
            .iter()
            .map(|column| format!("chiron_horizon_t.{}", quote_table_identifier(Some(DatabaseType::Db2), column)))
            .collect::<Vec<_>>()
            .join(", ")
    };
    let order = order_by.map(|order_by| format!("ORDER BY {order_by}")).unwrap_or_default();
    let row_number = quote_table_identifier(Some(DatabaseType::Db2), "__chiron_horizon_row_num");
    let end = offset + limit;

    format!(
        "SELECT {columns_sql} FROM (SELECT {inner_columns}, ROW_NUMBER() OVER ({order}) AS {row_number} FROM {table} chiron_horizon_t{where_clause}) chiron_horizon_page WHERE {row_number} > {offset} AND {row_number} <= {end} ORDER BY {row_number}"
    )
}
...
pub(super) fn build_neo4j_table_select_sql(options: &TableDataSelectSqlOptions, limit: usize) -> String {
    ...
    let returns = format!(
        ...
        quote_table_identifier(Some(DatabaseType::Neo4j), CHIRON_HORIZON_NEO4J_ELEMENT_ID_COLUMN)
    );
