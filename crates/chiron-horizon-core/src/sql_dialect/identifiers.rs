...
pub const SQLSERVER_LINKED_SCHEMA_PREFIX: &str = "__chiron_horizon_sqlserver_linked__:";
...
pub fn parse_sqlserver_linked_schema_ref(schema: &str) -> Option<SqlServerLinkedSchemaRef> {
    let payload = schema
        .strip_prefix(SQLSERVER_LINKED_SCHEMA_PREFIX)
        .or_else(|| schema.strip_prefix(crate::legacy::OLD_SQLSERVER_LINKED_SCHEMA_PREFIX))?;
