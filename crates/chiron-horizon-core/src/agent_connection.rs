...
mod tests {
    ...
    fn h2_split_connection_string_is_treated_as_file_mode() {
        ...
        cfg.connection_string =
            Some("jdbc:h2:split:28:C:/chiron-horizon-test/h2/sample-db;AUTO_SERVER=TRUE".to_string());

        let params = agent_connect_params(&cfg, "127.0.0.1", 9092, "test").unwrap();

        assert_eq!(
            h2_file_path_from_jdbc_url(cfg.connection_string.as_deref().unwrap()).as_deref(),
            Some("C:/chiron-horizon-test/h2/sample-db")
        );
        assert_eq!(params["host"], "");
        assert_eq!(params["port"], 0);
        assert_eq!(params["database"], "split:28:C:/chiron-horizon-test/h2/sample-db;AUTO_SERVER=TRUE");
        assert_eq!(
            params["connection_string"],
            "jdbc:h2:split:28:C:/chiron-horizon-test/h2/sample-db;AUTO_SERVER=TRUE"
        );
    }
    ...
    fn oracle_tns_does_not_retry_with_host_based_descriptors() {
        let mut cfg = config(DatabaseType::Oracle, Some("CHIRON_HORIZON_FAILOVER"));
        cfg.oracle_connection_type = Some("tns".to_string());
        cfg.connection_string =
            Some("jdbc:oracle:thin:@CHIRON_HORIZON_FAILOVER?TNS_ADMIN=%2Fopt%2Foracle%2Fnetwork%2Fadmin".to_string());
    ...
    fn prestosql_custom_jdbc_url_rewrites_forwarded_host() {
        ...
        cfg.port = 9090;
        cfg.connection_string =
            Some("jdbc:presto://presto.internal:9090/hive/default?source=chiron-horizon".to_string());

        let params = agent_connect_params(&cfg, "127.0.0.1", 19090, "hive/default").unwrap();

        assert_eq!(params["connection_string"], "jdbc:presto://127.0.0.1:19090/hive/default?source=chiron-horizon");
    }
    ...
    fn trino_agent_custom_jdbc_url_rewrites_forwarded_host_and_preserves_query_ssl() {
        ...
        cfg.ssl = true;
        cfg.connection_string =
            Some("jdbc:trino://trino.internal:8443/hive?SSL=true&source=chiron-horizon".to_string());
        cfg.url_params = Some("SSLVerification=NONE".to_string());
        ...
        assert_eq!(
            params["connection_string"],
            "jdbc:trino://127.0.0.1:15443/hive?SSL=true&source=chiron-horizon&SSLVerification=NONE"
        );
    }
}
