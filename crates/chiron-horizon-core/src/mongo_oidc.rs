...
async fn write_callback_response(stream: &mut tokio::net::TcpStream, success: bool) {
    let (status, title, body) = if success {
        (
            ...
            "Chiron Horizon received the authorization response. Return to Chiron Horizon while it completes the connection.",
        )
    } else {
        (
            "400 Bad Request",
            "Authentication failed",
            "Chiron Horizon could not complete authentication. Return to Chiron Horizon for details.",
        )
    };
    let html = format!(
...
mod tests {
        ...
        CallbackContext::builder()
            ...
            .idp_info(Some(
                IdpServerInfo::builder()
                    .issuer(issuer)
                    .client_id(Some("chiron-horizon-client".to_string()))
    ...
    fn oidc_endpoints_require_https_and_exact_issuer_match() {
        ...
        .is_err());
        let issuer = "https://idp.example/realms/chiron-horizon";
    ...
    fn discovery_url_preserves_issuer_path() {
        assert_eq!(
            discovery_url_with_policy("https://idp.example/realms/chiron-horizon", OidcEndpointPolicy::HttpsOnly)
                .unwrap()
                .as_str(),
            "https://idp.example/realms/chiron-horizon/.well-known/openid-configuration"
        );
    }
