    ...
    if first_word == "CREATE" && create_table_variant_prefix(&upper).is_some() {
        ...
        return match db_type {
            ...
            | DatabaseType::Redshift => {
                ...
                format!(
                    "DO $chiron_horizon_idempotent$\nBEGIN\n  IF NOT EXISTS (\n    SELECT 1 FROM information_schema.tables\n    WHERE {schema_predicate}\n      AND table_name = '{table_literal}'\n  ) THEN\n    EXECUTE $chiron_horizon_ddl${trimmed}$chiron_horizon_ddl$;\n  END IF;\nEND\n$chiron_horizon_idempotent$;"
                )
            }
            DatabaseType::Oracle
            | DatabaseType::Dameng
...
mod tests {
    ...
    fn conditional_check_wraps_global_temporary_table_postgres() {
        ...
        assert!(result.contains("DO $chiron_horizon_idempotent$"), "missing DO wrapper: {result}");
    ...
    fn conditional_check_does_not_emit_unconditional_ddl_after_select() {
        ...
        assert!(pg.contains("DO $chiron_horizon_idempotent$") || pg.contains("IF NOT EXISTS"), "pg={pg}");
        ...
        let after_do = pg.find("DO $chiron_horizon_idempotent$").map(|i| &pg[i..]).unwrap_or(&pg);
