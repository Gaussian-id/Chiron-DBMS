...
mod tests {
    ...
    fn discovers_sql_files_in_directory() {
        let dir = std::env::temp_dir().join("chiron_horizon_test_ddl_discover");
    ...
    fn loads_meta_file_for_sql() {
        let dir = std::env::temp_dir().join("chiron_horizon_test_meta_load");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn empty_ddl_dir_returns_no_files() {
        let dir = std::env::temp_dir().join("chiron_horizon_test_empty_ddl");
        let _ = std::fs::create_dir_all(&dir);
        let files = InputResolver::discover_ddl_files(&dir);
