...
mod tests {
    ...
    async fn external_driver_export_fixture(
        ...
        let dir = std::env::temp_dir().join(format!("chiron-horizon-jdbc-table-export-test-{}", uuid::Uuid::new_v4()));
        ...
        let server = tokio::spawn(async move {
            ...
            loop {
                ...
                if query != "SHOW DATABASES" {
                    assert_eq!(table_export_query_param(target, "db").as_deref(), Some("chiron_horizon_issue_8022"));
                }
                let final_page = query.ends_with("ORDER BY \"time\" LIMIT 1 OFFSET 2");
                let body = match query.as_str() {
                    "SHOW DATABASES" => {
                        r#"{"results":[{"statement_id":0,"series":[{"name":"databases","columns":["name"],"values":[["chiron_horizon_issue_8022"]]}]}]}"#
                    }
    ...
    #[test]
    fn influxdb_table_export_exports_paginated_rows() {
        // The InfluxDB export path captures the same recursive export future as
        // the external-driver cases, so it also needs an explicit test stack.
        let handle = std::thread::Builder::new()
            .name("table-export-influxdb-pagination".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("build InfluxDB table export test runtime")
                    .block_on(run_influxdb_table_export_exports_paginated_rows());
            })
            .expect("spawn InfluxDB table export test thread");
        if let Err(panic) = handle.join() {
            std::panic::resume_unwind(panic);
        }
    }

    async fn run_influxdb_table_export_exports_paginated_rows() {
        let (base_url, server) = spawn_influxdb_table_export_server().await;
        let port = base_url.rsplit_once(':').unwrap().1.parse::<u16>().unwrap();
        let dir =
            std::env::temp_dir().join(format!("chiron-horizon-influxdb-table-export-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let config: ConnectionConfig = serde_json::from_value(json!({
            ...
            "username": "",
            "password": "",
            "database": "chiron_horizon_issue_8022",
            "query_timeout_secs": 30
        }))
        .unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        ...
        let request = TableExportRequest {
            ...
            database: "chiron_horizon_issue_8022".to_string(),
            schema: None,
            identifier_quote: None,
    ...
    fn oracle_requested_export_columns_omit_synthetic_rowid_and_keep_metadata_aligned() {
        let columns = vec!["__CHIRON_HORIZON_ROWID".to_string(), "ID".to_string(), "NAME".to_string()];
        let column_types = vec![Some("VARCHAR2".to_string()), Some("NUMBER".to_string()), Some("VARCHAR2".to_string())];
        let primary_keys = vec!["__CHIRON_HORIZON_ROWID".to_string()];
    ...
    fn requested_export_columns_preserve_regular_oracle_and_non_oracle_columns() {
        ...
        let mysql_columns = vec!["__CHIRON_HORIZON_ROWID".to_string(), "name".to_string()];
    ...
    #[cfg(unix)]
    #[test]
    fn external_driver_table_export_reads_all_cursor_pages() {
        // External-driver export futures are large in debug builds. Keep each
        // regression test self-contained and give its Tokio runtime 8 MiB,
        // rather than requiring a process-wide RUST_MIN_STACK override.
        let handle = std::thread::Builder::new()
            .name("table-export-table-export-reads-all-cursor-pages".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("build table export test runtime")
                    .block_on(run_external_driver_table_export_reads_all_cursor_pages());
            })
            .expect("spawn table export test thread");
        if let Err(panic) = handle.join() {
            std::panic::resume_unwind(panic);
        }
    }
    ...
    #[cfg(unix)]
    async fn run_external_driver_table_export_reads_all_cursor_pages() {
...
case "$line" in
  ...
  *'"method":"closeQuerySession"'*)
    ...
    #[cfg(unix)]
    #[test]
    fn external_driver_table_export_does_not_repeat_legacy_one_shot_results() {
        // The legacy one-shot fallback captures the same large export future
        // as the other fallback-path tests. Give that future a dedicated
        // stack instead of relying on a process-wide RUST_MIN_STACK setting.
        let handle = std::thread::Builder::new()
            .name("table-export-legacy-one-shot".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("build legacy one-shot export test runtime")
                    .block_on(run_external_driver_table_export_does_not_repeat_legacy_one_shot_results());
            })
            .expect("spawn legacy one-shot export test thread");
        if let Err(panic) = handle.join() {
            std::panic::resume_unwind(panic);
        }
    }
    ...
    #[cfg(unix)]
    async fn run_external_driver_table_export_does_not_repeat_legacy_one_shot_results() {
        let fixture = external_driver_export_fixture(
            r#"  case "$line" in
    *'"method":"executeQueryPage"'*)
      echo executeQueryPage >> "$CALLS"
    ...
    *'"method":"executeQuery"'*)
    ...
    #[cfg(unix)]
    #[test]
    fn external_driver_table_export_closes_cursor_at_row_limit() {
        // External-driver export futures are large in debug builds. Keep each
        // regression test self-contained and give its Tokio runtime 8 MiB,
        // rather than requiring a process-wide RUST_MIN_STACK override.
        let handle = std::thread::Builder::new()
            .name("table-export-table-export-closes-cursor-at-row-limit".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("build table export test runtime")
                    .block_on(run_external_driver_table_export_closes_cursor_at_row_limit());
            })
            .expect("spawn table export test thread");
        if let Err(panic) = handle.join() {
            std::panic::resume_unwind(panic);
    ...
    #[cfg(unix)]
    async fn run_external_driver_table_export_closes_cursor_at_row_limit() {
        let fixture = external_driver_export_fixture(
            r#"  case "$line" in
    *'"method":"executeQueryPage"'*)
      echo executeQueryPage >> "$CALLS"
    ...
    *'"method":"closeQuerySession"'*)
    ...
    fn external_driver_table_export_cancels_blocked_execute() {
        ...
        // rather than requiring a process-wide RUST_MIN_STACK override.
        let handle = std::thread::Builder::new()
            .name("table-export-table-export-cancels-blocked-execute".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("build table export test runtime")
                    .block_on(run_external_driver_table_export_cancels_blocked_execute());
            })
            .expect("spawn table export test thread");
        if let Err(panic) = handle.join() {
            std::panic::resume_unwind(panic);
    ...
    #[cfg(unix)]
    async fn run_external_driver_table_export_cancels_blocked_execute() {
        let fixture = external_driver_export_fixture(
            r#"  case "$line" in
    *'"method":"executeQueryPage"'*)
      echo executeQueryPage >> "$CALLS"
    ...
    fn external_driver_table_export_cancels_blocked_fetch() {
        ...
        // rather than requiring a process-wide RUST_MIN_STACK override.
        let handle = std::thread::Builder::new()
            .name("table-export-table-export-cancels-blocked-fetch".to_string())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("build table export test runtime")
                    .block_on(run_external_driver_table_export_cancels_blocked_fetch());
            })
            .expect("spawn table export test thread");
        if let Err(panic) = handle.join() {
            std::panic::resume_unwind(panic);
    ...
    #[cfg(unix)]
    async fn run_external_driver_table_export_cancels_blocked_fetch() {
        let fixture = external_driver_export_fixture(
            r#"  case "$line" in
    *'"method":"executeQueryPage"'*)
      echo executeQueryPage >> "$CALLS"
