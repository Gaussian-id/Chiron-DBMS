...
mod tests {
    ...
    fn test_manager(name: &str) -> AgentManager {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-agent-manager-{name}-{}", uuid::Uuid::new_v4()));
    ...
    fn stores_configured_app_version_for_agent_handshake() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-agent-manager-version-{}", uuid::Uuid::new_v4()));
    ...
    fn resolves_relative_native_agent_launch_with_absolute_paths() {
        let base_dir = PathBuf::from("target")
            .join(format!("chiron-horizon-agent-manager-relative-native-{}", uuid::Uuid::new_v4()));
    ...
    fn relative_native_agent_launch_keeps_missing_driver_error() {
        let base_dir = PathBuf::from("target")
            .join(format!("chiron-horizon-agent-manager-relative-missing-{}", uuid::Uuid::new_v4()));
...
impl AgentManager {
    pub fn new() -> Self {
        ...
        Self::new_with_base_dir(PathBuf::from(home).join(".chiron-horizon").join("agents"))
    }
...
fn resolve_system_java_path(path_var: Option<&OsStr>) -> Option<PathBuf> {
    let path_var = path_var.map(|p| p.to_owned()).or_else(|| crate::legacy::var_os("PATH"))?;
