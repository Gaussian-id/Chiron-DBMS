...
fn build_rebuild_statements(
    ...
    let source_name = format!("__chiron_horizon_source_{}", &snapshot.revision[..12]);
    ...
    let source_alias = quote_ident("__chiron_horizon_source");
    ...
    let backup_source_alias = quote_ident("__chiron_horizon_backup_source");
  ...
  CONSTRAINT "uq_value" UNIQUE ([value])
    ...
    async fn rebuild_preserves_data_generated_columns_indexes_triggers_and_foreign_key_state() {
        ...
        assert!(preview
            .statements
            .iter()
            .any(|sql| sql.starts_with("DROP TABLE") && sql.contains("__chiron_horizon_source_")));
    ...
    async fn cast_copy_preserves_null_and_does_not_cast_unchanged_columns() {
        ...
        assert!(copy.contains("CAST(\"__chiron_horizon_source\".\"value\" AS INTEGER)"));
        assert!(!copy.contains("CAST(\"__chiron_horizon_source\".\"note\""));
    ...
    async fn text_email_to_float4_uses_explicit_real_cast() {
        ...
        let preview = preview_sqlite_table_structure_change_with_pool(pool.clone(), options.clone()).await.unwrap();
        let copy = preview.statements.iter().find(|sql| sql.contains("CAST(")).unwrap();
        assert!(copy.contains("CAST(\"__chiron_horizon_source\".\"email\" AS float4)"));
    ...
    async fn combined_column_rename_and_type_change_casts_source_then_updates_dependencies() {
        ...
        let preview = preview_sqlite_table_structure_change_with_pool(pool.clone(), options.clone()).await.unwrap();
        let copy = preview.statements.iter().find(|sql| sql.contains("CAST(")).unwrap();
        assert!(copy.contains("CAST(\"__chiron_horizon_source\".\"old_value\" AS INTEGER)"));
