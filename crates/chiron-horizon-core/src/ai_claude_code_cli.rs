...
use crate::ai_cli_agent::{
    build_cli_agent_prompt, chiron_horizon_mcp_enabled_tools, chiron_horizon_mcp_scope_env, cli_command, model_infos,
...
impl ClaudeCodeIsolatedCwd {
    fn create() -> Result<Self, String> {
        let path = env::temp_dir().join(format!("chiron-horizon-claude-code-{}", uuid::Uuid::new_v4()));
...
pub fn claude_code_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.claude_code_cli_env {
        ...
        if is_reserved_chiron_horizon_mcp_env_name(key) {
            return Err(format!(
                "[claudeCodeEnvReserved] `{key}` is managed by Chiron Horizon for the scoped MCP server and cannot be set here."
            ));
        }
...
fn claude_code_mcp_config(options: &ClaudeCodeRunOptions) -> String {
    ...
    json!({
        "mcpServers": {
            "chiron-horizon": Value::Object(server)
        }
    })
    .to_string()
}
...
fn classify_claude_code_run_error(stderr: &str) -> String {
        ...
        || lower.contains("please login")
    {
        ...
        format!("[chironHorizonMcpMissing] {stderr}")
...
mod tests {
    ...
    fn isolated_cli_test_config() -> (AiConfig, std::path::PathBuf, std::path::PathBuf) {
        let project_dir =
            std::env::temp_dir().join(format!("chiron-horizon-claude-project-test-{}", uuid::Uuid::new_v4()));
...
"#,
    ...
    fn builds_claude_code_command_with_scoped_mcp_and_default_model() {
        ...
        assert!(spec.args.iter().any(|arg| arg.contains("\"command\":\"chiron-horizon-mcp-server\"")));
        assert!(spec.args.iter().any(|arg| arg.contains("\"CHIRON_HORIZON_MCP_ALLOW_WRITES\":\"0\"")));
        assert!(spec.args.iter().any(|arg| arg.contains("\"CHIRON_HORIZON_MCP_SCOPE_CONNECTION_ID\":\"conn-1\"")));
        assert!(spec.args.iter().any(|arg| arg.contains("mcp__chiron_horizon__chiron_horizon_execute_query")));
    }
    ...
    fn invalid_mcp_config_is_not_reported_as_a_missing_server_package() {
        let error = classify_claude_code_run_error(
            r#"Invalid MCP configuration: MCP config file not found: C:\Temp\{mcpServers:{chiron-horizon:{args:[C:\node_modules\@chiron-horizon\mcp-server\bin\chiron-horizon-mcp-server.js]}}}"#,
        );

        assert!(error.starts_with("[claudeCodeMcpConfigInvalid]"));
        assert!(!error.contains("[chironHorizonMcpMissing]"));
    }

    #[test]
    fn invalid_mcp_config_path_containing_auth_is_not_reported_as_authentication_failure() {
        let error = classify_claude_code_run_error(
            r#"Invalid MCP configuration: MCP config file not found: C:\Users\auth-test\AppData\Local\Temp\chiron-horizon\mcp.json"#,
        );
    ...
    fn rejects_reserved_chiron_horizon_mcp_env_name() {
        let mut config = claude_code_config("default");
        config.claude_code_cli_env.insert("CHIRON_HORIZON_MCP_SCOPE_DATABASE".to_string(), "main".to_string());
    ...
    fn resolves_claude_code_executable_from_configured_directory() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-claude-code-dir-test-{}", std::process::id()));
    ...
    fn parses_claude_code_jsonl_events() {
        let started = parse_claude_code_jsonl_event(
            r#"{"type":"assistant","message":{"content":[{"type":"tool_use","id":"tool-1","name":"mcp__chiron_horizon__chiron_horizon_list_tables","input":{"schema":"public"}}]}}"#,
        )
        .unwrap();
        assert!(
            matches!(&started[0], AgentEvent::ToolCallStart { tool_name, args, .. } if tool_name == "mcp__chiron_horizon__chiron_horizon_list_tables" && args["schema"] == "public")
        );
