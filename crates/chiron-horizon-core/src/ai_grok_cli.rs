...
const GROK_ACP_REQUEST_ID: &str = "chiron_horizon_model_discovery";
...
impl GrokIsolatedHome {
    fn create(config: &AiConfig, options: Option<&GrokRunOptions>) -> Result<Self, String> {
        let path = env::temp_dir().join(format!("chiron-horizon-grok-cli-{}", uuid::Uuid::new_v4()));
    ...
    fn write_prompt(&self, prompt: &str) -> Result<PathBuf, String> {
        let path = self.path.join("chiron-horizon-prompt.txt");
...
pub fn grok_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.grok_cli_env {
        ...
        if is_reserved_chiron_horizon_mcp_env_name(key) {
            return Err(format!(
                "[grokCliEnvReserved] `{key}` is managed by Chiron Horizon for the scoped MCP server and cannot be set here."
            ));
        }
...
fn grok_mcp_permission_rules(options: &GrokRunOptions) -> Vec<String> {
    let mut rules = vec!["MCPTool(chiron_horizon__*)".to_string()];
    for tool in chiron_horizon_mcp_enabled_tools(options.agent_mode) {
        rules.push(format!("MCPTool(chiron_horizon__{tool})"));
    }
    rules
}
...
fn grok_mcp_config_toml(options: &GrokRunOptions) -> String {
    ...
    let mut lines = vec![
        ...
        "[mcp_servers.\"chiron-horizon\"]".to_string(),
    ...
    lines.push("[mcp_servers.\"chiron-horizon\".env]".to_string());
...
pub fn build_grok_command(config: &AiConfig, prompt_file: &Path, options: &GrokRunOptions) -> GrokCommandSpec {
    ...
    // Chiron Horizon system+conversation prompts stay under OS argv limits.
    let mut args = vec![
        "--no-leader".to_string(),
...
fn classify_grok_spawn_error(message: &str) -> String {
    if message.contains("No such file") || message.contains("not found") {
        "[grokCliNotInstalled] Grok CLI was not found. Install Grok CLI or set the Grok CLI path in Chiron Horizon AI settings."
            .to_string()
    } else if is_command_line_too_long_error(message) {
        "[grokCliCommandLineTooLong] Grok CLI command line is too long. Update Chiron Horizon so Grok prompts are sent through a temporary prompt file."
            .to_string()
    } else {
...
fn classify_grok_run_error(stderr: &str) -> String {
        ...
        || lower.contains("authentication required")
    {
        ...
        format!("[grokCliMcpStartupFailed] Grok could not start the Chiron Horizon MCP server. {stderr}")
    } else {
...
mod tests {
    ...
    fn run_options() -> GrokRunOptions {
        GrokRunOptions {
            ...
            mcp_server_command: Some(CliAgentCommandSpec {
                program: "chiron-horizon-mcp-server".to_string(),
    ...
    fn validates_path_and_env() {
        ...
        config.grok_cli_env.insert("CHIRON_HORIZON_MCP_ALLOW_WRITES".to_string(), "1".to_string());
    ...
    fn builds_headless_command_with_prompt_file_and_model() {
        ...
        let prompt = PathBuf::from("/tmp/chiron-horizon-prompt.txt");
        ...
        assert!(command.args.windows(2).any(|pair| pair == ["--prompt-file", "/tmp/chiron-horizon-prompt.txt"]));
        ...
        assert!(command.args.windows(2).any(|pair| pair == ["--allow", "MCPTool(chiron_horizon__*)"]));
        assert!(command
            .args
            .windows(2)
            .any(|pair| pair == ["--allow", "MCPTool(chiron_horizon__chiron_horizon_list_tables)"]));
        assert!(!command.args.iter().any(|arg| arg.contains("mcp__chiron_horizon__")));
    }
    ...
    fn accepts_compatibility_shapes_and_bare_effort_values() {
        let models = parse_grok_acp_initialize(
            r#"{"jsonrpc":"2.0","id":"chiron_horizon_model_discovery","result":{"meta":{"model_state":{"current_model_id":"grok-next","available_models":[{"model_id":"grok-next","display_name":"Grok Next","supports_reasoning_effort":true,"reasoning_efforts":["low","xhigh","quantum","low"]}]}}}}"#,
        )
        .unwrap();
    ...
    fn missing_effort_metadata_and_listing_fallback_do_not_invent_levels() {
        let models = parse_grok_acp_initialize(
            r#"{"jsonrpc":"2.0","id":"chiron_horizon_model_discovery","result":{"_meta":{"modelState":{"currentModelId":"grok-plain","availableModels":[{"modelId":"grok-plain"},{"modelId":"grok-empty","_meta":{"supportsReasoningEffort":true}}]}}}}"#,
        )
        .unwrap();
    ...
    fn fake_grok_config() -> (AiConfig, PathBuf, PathBuf, PathBuf) {
        let project_dir =
            std::env::temp_dir().join(format!("chiron-horizon-grok-project-test-{}", uuid::Uuid::new_v4()));
...
case " $* " in
  *" --prompt-file "*)
    printf used > "$CHIRON_HORIZON_TEST_INFERENCE_MARKER"
    exit 12
    ;;
  *" agent --no-leader stdio "*)
    if [ "$GROK_HOME" = "$CHIRON_HORIZON_TEST_PROJECT_DIR/.grok" ] || [ "$PWD" = "$CHIRON_HORIZON_TEST_PROJECT_DIR" ] || [ "$GROK_AUTH_PATH" != "/selected/auth.json" ]; then
      printf leaked > "$CHIRON_HORIZON_TEST_ISOLATION_MARKER"
      exit 13
    fi
    IFS= read -r request
    case "$request" in
      *'"method":"initialize"'*)
        printf '%s\n' '{"jsonrpc":"2.0","id":"chiron_horizon_model_discovery","result":{"_meta":{"modelState":{"currentModelId":"grok-dynamic","availableModels":[{"modelId":"grok-dynamic","name":"Grok Dynamic","_meta":{"supportsReasoningEffort":true,"reasoningEfforts":[{"value":"xhigh","label":"Extra High"},{"value":"future","label":"Future"}]}}]}}}}'
        ;;
      *)
        exit 14
        ;;
    ...
    ;;
  *" models "*)
    if [ "$CHIRON_HORIZON_TEST_MODELS_HANG" = "1" ]; then
      while :; do :; done
    fi
    printf used > "$CHIRON_HORIZON_TEST_MODELS_MARKER"
...
"#,
        ...
        config
            .grok_cli_env
            .insert("CHIRON_HORIZON_TEST_MODELS_MARKER".to_string(), fallback_marker.to_string_lossy().to_string());
        config
            .grok_cli_env
            .insert("CHIRON_HORIZON_TEST_INFERENCE_MARKER".to_string(), inference_marker.to_string_lossy().to_string());
        config
            .grok_cli_env
            .insert("CHIRON_HORIZON_TEST_ISOLATION_MARKER".to_string(), isolation_marker.to_string_lossy().to_string());
    ...
    async fn connection_test_is_timeout_bounded() {
        ...
        config.grok_cli_env.insert("CHIRON_HORIZON_TEST_MODELS_HANG".to_string(), "1".to_string());
        let error =
    ...
    #[test]
    fn mcp_config_includes_scoped_chiron_horizon_server() {
        let toml = grok_mcp_config_toml(&run_options());
        assert!(toml.contains("[mcp_servers.\"chiron-horizon\"]"));
        assert!(toml.contains("command = \"chiron-horizon-mcp-server\""));
        assert!(toml.contains("args = [\"--stdio\"]"));
        assert!(toml.contains("CHIRON_HORIZON_MCP_SCOPE_CONNECTION_ID = \"conn-1\""));
        assert!(toml.contains("CHIRON_HORIZON_MCP_ALLOW_WRITES = \"0\""));
        assert!(toml.contains("[permission]"));
        assert!(toml.contains("use_leader = false"));
        assert!(toml.contains("MCPTool(chiron_horizon__*)"));
