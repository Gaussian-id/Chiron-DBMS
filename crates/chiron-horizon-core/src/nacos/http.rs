...
/// Short-lived r-nacos console state shared by clients for one Chiron Horizon connection.
/// It deliberately remains in memory so closing a console tab does not trigger
/// a new CAPTCHA, while restarting Chiron Horizon still requires authentication.
...
impl NacosOpenApiAdmin {
    ...
    async fn submit_service_upsert(
        ...
        // Both supported Nacos generations reject `{}`. Chiron Horizon owns the
...
impl NacosAdmin for NacosOpenApiAdmin {
    fn service_capabilities(&self) -> NacosServiceCapabilities {
        ...
        if self.is_rnacos_compatible() && self.cfg.rnacos_console_addr.trim().is_empty() {
            ...
            // Without its console API, Chiron Horizon cannot safely prove a service is
    ...
    async fn test_connection(&self) -> Result<NacosConnectionInfo, String> {
        ...
        // connection can perform the Chiron Horizon operations it exposes.
    ...
    async fn list_instances(&self, query: NacosInstanceQuery) -> Result<Vec<NacosInstanceInfo>, String> {
        ...
        if self.is_rnacos_compatible() {
            ...
            // instances remain operable in Chiron Horizon as well.
            if !self.cfg.rnacos_console_addr.is_empty() {
    ...
    async fn list_instances_for_service_delete(
        ...
        if self.is_rnacos_compatible() {
            if self.cfg.rnacos_console_addr.trim().is_empty() {
                return Err(
                    "NACOS_ERROR[endpointUnavailable]: r-nacos service deletion requires a configured r-nacos console address so Chiron Horizon can verify disabled instances"
                        .to_string(),
                );
            }
            let namespace = self.namespace(query.namespace.as_deref());
...
mod tests {
    ...
    async fn service_upsert_normalizes_empty_selector() {
        ...
        admin
            .create_service(NacosServiceUpsert {
                ...
                service_name: "chiron-horizon-e2e".to_string(),
        ...
        admin
            .update_instance(NacosInstanceUpdateRequest {
                target: NacosInstanceRef {
                    ...
                    port: 19001,
                    group_name: Some("CHIRON_HORIZON_E2E".to_string()),
    ...
    async fn v3_instance_update_uses_partial_patch_endpoint() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert!(request.contains("serviceName=chiron-horizon-e2e"));
            assert!(request.contains("groupName=CHIRON_HORIZON_E2E"));
    ...
    async fn v3_service_and_instance_lists_use_documented_naming_admin_routes() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert_eq!(service_params.get("groupNameParam").map(|value| value.as_ref()), Some("CHIRON_HORIZON_E2E"));
            write_json_response(
                &mut service_socket,
                r#"{"code":0,"message":"success","data":{"totalCount":1,"pageNumber":1,"pagesAvailable":1,"pageItems":[{"name":"chiron-horizon-e2e","groupName":"CHIRON_HORIZON_E2E","clusterCount":1,"ipCount":2,"healthyInstanceCount":1,"triggerFlag":"false"}]}}"#,
            )
            .await;
            ...
            assert_eq!(instance_params.get("serviceName").map(|value| value.as_ref()), Some("chiron-horizon-e2e"));
    ...
    async fn v3_empty_service_instance_list_returns_empty_without_inventing_a_default_cluster() {
        ...
        let instances = admin
            .list_instances(NacosInstanceQuery {
                ...
                group_name: Some("CHIRON_HORIZON_V3".to_string()),
                clusters: None,
            })
            .await
            .unwrap();
    ...
    async fn v2_service_list_uses_catalog_to_enumerate_groups() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert_eq!(params.get("groupNameParam").map(|value| value.as_ref()), Some("CHIRON_HORIZON_CURL"));
            write_json_response(
                &mut socket,
                r#"{"count":1,"serviceList":[{"name":"chiron-horizon-curl-e2e","groupName":"CHIRON_HORIZON_CURL","clusterCount":1,"ipCount":1}]}"#,
            )
            .await;
        });
        let mut config = test_admin_config(format!("http://{address}"));
        ...
        let result = admin
            .list_services(NacosServiceQuery {
                namespace: Some("public".to_string()),
                service_name: None,
                group_name: Some("CHIRON_HORIZON_CURL".to_string()),
                page_no: Some(1),
                page_size: Some(100),
            })
            .await
        ...
        assert_eq!(result.items[0].service_name, "chiron-horizon-curl-e2e");
        assert_eq!(result.items[0].group_name.as_deref(), Some("CHIRON_HORIZON_CURL"));
        server.await.unwrap();
    }
    ...
    async fn rnacos_service_list_uses_catalog_without_a_v3_admin_probe() {
        ...
        let server = tokio::spawn(async move {
            ...
            let params = url.query_pairs().collect::<HashMap<_, _>>();
            assert_eq!(url.path(), "/nacos/v1/ns/catalog/services");
            assert_eq!(params.get("groupNameParam").map(|value| value.as_ref()), Some("CHIRON_HORIZON_RNACOS"));
            write_json_response(
                &mut socket,
                r#"{"count":1,"serviceList":[{"name":"chiron-horizon-demo-api","groupName":"CHIRON_HORIZON_RNACOS"}]}"#,
            )
            .await;
        });
        let mut config = test_admin_config(format!("http://{address}"));
        ...
        let result = admin
            .list_services(NacosServiceQuery {
                namespace: Some("public".to_string()),
                service_name: None,
                group_name: Some("CHIRON_HORIZON_RNACOS".to_string()),
                page_no: Some(1),
                page_size: Some(100),
            })
            .await
            .unwrap();
        assert_eq!(result.items.len(), 1);
        assert_eq!(result.items[0].service_name, "chiron-horizon-demo-api");
        assert_eq!(result.items[0].group_name.as_deref(), Some("CHIRON_HORIZON_RNACOS"));
        server.await.unwrap();
    }
    ...
    async fn rnacos_instance_list_uses_v1_naming_without_catalog_or_v3_probes() {
        ...
        let instances = admin
            .list_instances(NacosInstanceQuery {
                namespace: Some("public".to_string()),
                service_name: "chiron-horizon-demo-api".to_string(),
                group_name: Some("CHIRON_HORIZON_RNACOS".to_string()),
                clusters: None,
            })
            .await
    ...
    async fn rnacos_console_instance_list_includes_disabled_instances() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert_eq!(params.get("groupName").map(|value| value.as_ref()), Some("CHIRON_HORIZON_RNACOS"));
            assert_eq!(params.get("serviceName").map(|value| value.as_ref()), Some("chiron-horizon-demo-api"));
    ...
    async fn rnacos_service_and_instance_crud_use_only_compatible_v1_routes() {
        ...
        let service = NacosServiceUpsert {
            namespace: Some("public".to_string()),
            service_name: "chiron-horizon-rnacos-crud".to_string(),
            group_name: Some("CHIRON_HORIZON_RNACOS".to_string()),
            metadata: Some(serde_json::json!({ "managed-by": "chiron-horizon" })),
    ...
    async fn rnacos_service_detail_embeds_the_group_in_the_service_name() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert_eq!(
                params.get("serviceName").map(|value| value.as_ref()),
                Some("CHIRON_HORIZON_RNACOS@@chiron-horizon-demo-api")
            );
            assert!(!params.contains_key("groupName"));
            write_json_response(
                &mut socket,
                r#"{"namespaceId":"public","groupName":"CHIRON_HORIZON_RNACOS","name":"chiron-horizon-demo-api","metadata":{},"selector":{"type":"none","contextType":"NONE"}}"#,
            )
            .await;
        });
        let mut config = test_admin_config(format!("http://{address}"));
        ...
        let detail = admin
            .get_service(NacosServiceQuery {
                namespace: Some("public".to_string()),
                service_name: Some("chiron-horizon-demo-api".to_string()),
                group_name: Some("CHIRON_HORIZON_RNACOS".to_string()),
                page_no: None,
                page_size: None,
            })
            .await
            .unwrap();
        assert_eq!(detail.service_name, "chiron-horizon-demo-api");
        assert_eq!(detail.group_name.as_deref(), Some("CHIRON_HORIZON_RNACOS"));
        server.await.unwrap();
    }
    ...
    async fn v2_service_list_uses_group_api_for_empty_service() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert_eq!(params.get("groupName").map(|value| value.as_ref()), Some("CHIRON_HORIZON_CURL"));
            assert!(!params.contains_key("groupNameParam"));
            write_json_response(
                &mut socket,
                r#"{"code":0,"message":"success","data":{"count":1,"services":["chiron-horizon-empty-e2e"]}}"#,
            )
            .await;
        });
        let mut config = test_admin_config(format!("http://{address}"));
        ...
        let result = admin
            ...
            .unwrap();
        assert_eq!(result.items.len(), 1);
        assert_eq!(result.items[0].service_name, "chiron-horizon-empty-e2e");
        assert_eq!(result.items[0].group_name.as_deref(), Some("CHIRON_HORIZON_CURL"));
        server.await.unwrap();
    }
    ...
    async fn v2_instance_list_uses_catalog_and_keeps_disabled_instances() {
        ...
        let instances = admin
            .list_instances(NacosInstanceQuery {
                namespace: Some("public".to_string()),
                service_name: "chiron-horizon-ui-crud".to_string(),
                group_name: Some("CHIRON_HORIZON_E2E".to_string()),
                clusters: None,
            })
            .await
        ...
        let form = instance_update_form(
            ...
            NacosInstanceUpdateRequest {
                target: NacosInstanceRef {
                    ...
                    ip: "127.0.0.1".to_string(),
                    port: 8080,
                    group_name: Some("CHIRON_HORIZON_TEST".to_string()),
                    cluster_name: Some("blue".to_string()),
                    ephemeral: Some(false),
                },
