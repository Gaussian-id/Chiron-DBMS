...
/// Access-control changes can affect a different Chiron Horizon connection that targets
...
fn namespace_authorization_unavailable(detail: &str) -> String {
    ...
    format!(
        "NACOS_ERROR[{error_code}]: Chiron Horizon cannot safely determine readable namespaces without per-namespace probes: {detail}"
    )
}
