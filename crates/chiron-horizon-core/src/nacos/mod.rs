...
//!
//! The public API is intentionally owned by chiron-horizon (`NacosAdmin`) instead of
//! exposing SDK/OpenAPI types. The current adapter uses Nacos OpenAPI because
//! the current nacos-sdk-rust releases require Rust edition 2024, while chiron-horizon
...
///
/// This is also the cache key rather than the Chiron Horizon connection ID: one r-nacos
...
impl NacosAdminRegistry {
    ...
    async fn rnacos_console_session(&self, cfg: &NacosAdminConfig) -> RNacosConsoleSessionHandle {
        ...
        // Bound cache retention even when a user removes every matching Chiron Horizon
