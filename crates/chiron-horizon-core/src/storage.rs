...
const STORAGE_DB_FILE_NAME: &str = "chiron-horizon.db";
...
pub struct Storage {
    db: SqliteHandle,
    /// Path to the SQLite database file (`chiron-horizon.db`). Its parent directory is the
    /// application data dir where chiron-horizon-managed state (e.g. `known_hosts`) lives.
    path: PathBuf,
}
...
/// desktop service stores its token in a private file under the Chiron Horizon data dir.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl Storage {
    ...
    /// Multiple `chiron-horizon` processes can end up pointed at the same data directory
        ...
        for attempt in 1..=ATTEMPTS {
            ...
            match result {
                ...
                Ok(mode) => {
                    ...
                    warn!("chiron-horizon.db journal_mode did not switch to WAL (got '{mode}'); concurrent multi-process access may hit 'database is locked' more often");
                    return;
                }
                ...
                Err(error) => {
                    warn!("chiron-horizon.db could not switch journal_mode to WAL after {ATTEMPTS} attempts: {error}; concurrent multi-process access may hit 'database is locked' more often");
                    return;
                }
            }
        }
    ...
    /// Directory containing the SQLite database (`chiron-horizon.db`). SSH host keys are
    /// stored in `<data_dir>/known_hosts` so chiron-horizon never touches `~/.ssh`.
...
fn ensure_mcp_connection_change_allowed_in_tx(
    ...
    if policy.read_only {
        return Err(
            "MCP_READ_ONLY: Chiron Horizon global MCP read-only mode is enabled. Connection changes are blocked."
                .to_string(),
        );
    }
    if let Some(connection_id) = target_connection_id {
        ...
        if !crate::mcp_policy::policy_allows_connection(&policy, group_paths.get(connection_id), connection_id) {
            return Err(format!(
                "CONNECTION_OUT_OF_SCOPE: connection '{connection_id}' is not allowed by the current Chiron Horizon MCP policy"
            ));
        }
    }
    Ok(())
...
mod tests {
    ...
    fn temp_db_path(name: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("chiron-horizon-storage-{name}-{}-{stamp}.db", std::process::id()))
    }
    ...
    async fn open_restricts_the_database_and_its_journals_to_the_owner() {
        ...
        let path = dir.join("chiron-horizon.db");
        ...
        let journal = dir.join("chiron-horizon.db-journal");
        ...
        for name in ["chiron-horizon.db", "chiron-horizon.db-journal", "chiron-horizon.db-wal", "chiron-horizon.db-shm"]
        {
    ...
    async fn open_preserves_group_access_in_a_shared_data_directory() {
        ...
        for name in ["chiron-horizon.db", "chiron-horizon.db-journal"] {
            let Some(mode) = file_mode(&dir.join(name)) else { continue };
    ...
    fn temp_data_dir(name: &str) -> std::path::PathBuf {
        let stamp = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos();
        std::env::temp_dir().join(format!("chiron-horizon-storage-{name}-{}-{stamp}", std::process::id()))
    }
    ...
    async fn concurrent_save_connections_from_two_connections_does_not_lock() {
        // Regression test for issue #6605: multiple `chiron-horizon` processes sharing one
        ...
        // `chiron-horizon.db`. Opening two independent `Storage` instances here exercises
    ...
    async fn create_data_dir_with_connection(name: &str, connection_id: &str, token: &str) -> std::path::PathBuf {
        ...
        let storage = Storage::open(&data_dir.join("chiron-horizon.db")).await.unwrap();
    ...
    async fn import_user_data_db_copies_source_when_target_is_missing() {
        ...
        let storage = Storage::open(&target_dir.join("chiron-horizon.db")).await.unwrap();
    ...
    async fn import_user_data_db_recognizes_settings_and_snippets_as_user_data() {
        ...
        let source_storage = Storage::open(&source_dir.join("chiron-horizon.db")).await.unwrap();
        source_storage
    ...
    async fn import_user_data_db_does_not_overwrite_target_with_settings() {
        ...
        let target_storage = Storage::open(&target_dir.join("chiron-horizon.db")).await.unwrap();
        target_storage
            .save_editor_settings(&serde_json::json!({
    ...
    async fn import_user_data_db_replaces_empty_target_schema() {
        ...
        let _target_storage = Storage::open(&target_dir.join("chiron-horizon.db")).await.unwrap();
    ...
    async fn import_user_data_db_skips_empty_source_schema() {
        ...
        assert!(!target_dir.join("chiron-horizon.db").exists());
    }
    ...
    fn import_user_data_db_skips_invalid_source_file() {
        ...
        std::fs::write(source_dir.join("chiron-horizon.db"), b"not sqlite").unwrap();
    ...
    async fn load_connections_preserves_opaque_null_plugin_secrets() {
        ...
        config.db_type = DatabaseType::Plugin;
        config.driver_profile = Some("plugin".to_string());
        config.plugin_id = Some("id.chiron.horizon.ssh".to_string());
        config.plugin_connection_provider = Some("id.chiron.horizon.ssh.connection".to_string());
    ...
    async fn mcp_connection_mutations_are_atomic_and_recheck_policy() {
        ...
        // Non-MCP callers remain governed by the ordinary Chiron Horizon UI permissions.
    ...
    async fn desktop_settings_preserve_existing_password_hash() {
        ...
        storage
            .save_desktop_settings(&DesktopSettings {
                ...
                saved_sql_sync_dir: None,
                driver_store_dir: Some("/tmp/chiron-horizon-drivers".to_string()),
                plugin_store_dir: Some("/tmp/chiron-horizon-plugins".to_string()),
                agent_store_dir: Some("/tmp/chiron-horizon-agents".to_string()),
