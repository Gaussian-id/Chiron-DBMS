...
fn export_agents_offline_unlocked(
    ...
    for db_type in selected {
        ...
        let filename = match kind {
            AgentOfflineArtifactKind::Jar => format!("chiron-horizon-agent-{db_type}.jar"),
            AgentOfflineArtifactKind::Native => {
                let suffix = if cfg!(windows) { ".exe" } else { "" };
                format!("chiron-horizon-agent-{db_type}-{}{suffix}", AgentManager::current_platform())
            }
        };
    ...
    let staging_dir = tempfile::Builder::new()
        .prefix(".chiron-horizon-agent-export-")
    ...
    for key in required_jres {
        ...
        let filename = format!(
            "chiron-horizon-jre-{}-{}-{}.tar.zst",
            key,
            safe_filename_token(&version),
...
fn archive_managed_jre(am: &AgentManager, key: &str, destination: &Path) -> Result<(), String> {
    ...
    let root_name = PathBuf::from(format!("chiron-horizon-jre-{key}"));
...
fn unavailable_reason_label(reason: AgentOfflineExportUnavailableReason) -> &'static str {
    match reason {
        AgentOfflineExportUnavailableReason::UnmanagedInstall => {
            "the artifact is not tracked as a Chiron Horizon-managed install"
        }
        ...
        AgentOfflineExportUnavailableReason::MissingManagedJre => {
            "the required Chiron Horizon-managed JRE is not installed"
        }
        AgentOfflineExportUnavailableReason::InvalidManagedJre => {
            "the required Chiron Horizon-managed JRE is invalid or unsafe"
        }
    }
}
...
mod tests {
    ...
    async fn exported_package_round_trips_through_existing_importer_and_deduplicates_jre() {
        ...
        assert_eq!(duckdb_artifact.url, "offline://chiron-horizon-agent-duckdb.jar");
        ...
        assert!(jre_artifact.url.starts_with("offline://chiron-horizon-jre-21-21.0.7-"));
    ...
    fn jre_archive_enforces_the_entry_count_limit() {
        ...
        let error = append_jre_tree_with_limits(
            &mut archive,
            &source,
            Path::new("chiron-horizon-jre-test"),
            2,
            MAX_JRE_DEPTH,
            MAX_JRE_UNCOMPRESSED_BYTES,
        )
