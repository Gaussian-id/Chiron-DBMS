...
pub const CHIRON_HORIZON_PLUGIN_EXTENSION: &str = "chiron-horizonp";
...
impl PluginPackageInstaller {
    ...
    pub fn install_file(
        ...
        if package_path.extension().and_then(|extension| extension.to_str()) != Some(CHIRON_HORIZON_PLUGIN_EXTENSION) {
            return Err(format!("Plugin package must use the .{CHIRON_HORIZON_PLUGIN_EXTENSION} extension"));
        }
    ...
    fn install_bytes_locked(
        ...
        let staging = tempfile::Builder::new()
            .prefix(".chiron-horizonp-staging-")
        ...
        if manifest.manifest_version == 0 {
            return Err(".chiron-horizonp packages must use plugin manifest version 1 or newer".to_string());
        }
...
fn extract_package(package: &[u8], destination: &Path) -> Result<HashSet<String>, String> {
    let mut archive = zip::ZipArchive::new(Cursor::new(package))
        .map_err(|error| format!("Invalid .chiron-horizonp archive: {error}"))?;
...
mod tests {
    ...
    fn runtime_install_file_and_signed_packages_share_the_update_guard() {
        ...
        let package_path = root.path().join("update.chiron_horizonp");
    ...
    fn local_installer_loads_official_keys_without_persisting_them() {
        ...
        assert!(installer.trust_store.keys.contains_key("chiron-horizon-store-release-2026"));
        assert!(installer.trust_store.keys.contains_key("chiron-horizon-store-preview-2026"));
    ...
    fn local_installer_rejects_forged_official_signatures() {
        ...
        let package_path = root.path().join("forged.chiron_horizonp");
        std::fs::write(
            &package_path,
            package("1.0.0", Some((&forged_key, "chiron-horizon-store-release-2026")), false),
        )
        .unwrap();
    ...
    fn local_installer_preserves_custom_key_trust() {
        ...
        let installer = PluginPackageInstaller::new(root.path().to_path_buf(), "0.5.67").unwrap();
        let package_path = root.path().join("custom.chiron_horizonp");
    ...
    fn local_installer_rejects_official_key_conflicts() {
        ...
        PluginTrustStore::save_base64_key(root.path(), "chiron-horizon-store-release-2026", &public_key).unwrap();
        ...
        assert!(error.contains("'chiron-horizon-store-release-2026' already exists with a different public key"));
    }
    ...
    fn local_installer_accepts_matching_official_keys() {
        let root = tempfile::tempdir().unwrap();
        let installer = PluginPackageInstaller::new(root.path().to_path_buf(), "0.5.67").unwrap();
        let official_key = installer.trust_store.keys.get("chiron-horizon-store-release-2026").unwrap();
        ...
        assert_eq!(reloaded.trust_store.keys.get("chiron-horizon-store-release-2026"), Some(official_key));
        assert_eq!(PluginTrustStore::list_base64_keys(root.path()).unwrap().len(), 1);
    }
    ...
    fn package(version: &str, signer: Option<(&SigningKey, &str)>, tamper: bool) -> Vec<u8> {
        let manifest = serde_json::json!({
            ...
            "version": version,
            "publisher": "sample",
            "engines": { "chiron-horizon": ">=0.5.0", "host_api": "^1.0" },
            "permissions": ["host.events"],
            "entrypoints": {
                "backend": {
                    "protocol_versions": [1],
