...
impl PluginSidecarSession {
    ...
    async fn initialize(&self) -> Result<PluginHandshake, String> {
        let params = PluginInitializeParams {
            host: PluginHostDescription {
                chiron_horizon_version: &self.app_version,
    ...
    async fn request_user_input(&self, params: serde_json::Value) -> Result<serde_json::Value, PluginHostRequestError> {
        ...
        let Some(receiver) = ssh_prompt::request_ssh_prompt(prompt) else {
            return Err(PluginHostRequestError::new(
                -32001,
                "The Chiron Horizon host cannot ask the user for input right now",
            ));
        };
...
fn spawn_plugin_child(plugin: &InstalledPlugin, app_version: &str, env: &PluginRuntimeEnv) -> Result<Child, String> {
    ...
    command
        ...
        .stderr(Stdio::piped())
        .kill_on_drop(true)
        .env("CHIRON_HORIZON_PLUGIN_ID", &plugin.manifest.id)
        .env("CHIRON_HORIZON_PLUGIN_VERSION", &plugin.manifest.version)
        .env("CHIRON_HORIZON_APP_VERSION", app_version)
        .env("CHIRON_HORIZON_HOST_API_VERSION", SUPPORTED_PLUGIN_HOST_API_VERSION)
        .env("CHIRON_HORIZON_PLUGIN_PROTOCOL_VERSION", SUPPORTED_PLUGIN_PROTOCOL_VERSION.to_string());
