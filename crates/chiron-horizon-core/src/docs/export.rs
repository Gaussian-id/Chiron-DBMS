...
/// `chiron-horizon docs` must work from a plain `cargo install` on a machine with no
...
pub fn to_standalone_html(
    ...
    Ok(format!(
        "<!doctype html>\n<html lang=\"{lang}\">\n<head>\n<meta charset=\"utf-8\">\n<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">\n<title>{title}</title>\n<style>{EXPORT_CSS}</style>\n</head>\n<body>\n<div id=\"app\"></div>\n<script type=\"application/chiron-horizon-snapshot\">{encoded}</script>\n<script>{EXPORT_JS}</script>\n</body>\n</html>\n"
    ))
}
...
mod tests {
    ...
    fn the_payload_round_trips() {
        ...
        let html = to_standalone_html(&snapshot, &annotations, "en").expect("export");
        let start = html.find("application/chiron-horizon-snapshot").expect("payload element");
        let body = &html[start..];
    ...
    fn the_shell_and_stylesheet_reference_no_external_resources() {
        ...
        //    the result.
        let payload_start = html.find("application/chiron-horizon-snapshot").expect("payload element");
