...
pub const REFERENCE_PREFIX: &str = "chiron-horizon-ai-secret:v1:";
...
fn master_key(dir: &Path, create: bool) -> Result<String, String> {
    if let Ok(key) = crate::legacy::var("CHIRON_HORIZON_AI_MASTER_KEY") {
        if key.len() < 32 {
            return Err("CHIRON_HORIZON_AI_MASTER_KEY must contain at least 32 characters".into());
        }
        return Ok(key);
    }
    ...
    let metadata = std::fs::symlink_metadata(&path).map_err(|_| {
        "AI master key missing; restore it or set CHIRON_HORIZON_AI_MASTER_KEY. Existing secrets were not changed."
    })?;
    if !metadata.is_file() {
...
impl Storage {
    ...
    pub(crate) async fn migrate_ai_secrets(&self) -> Result<(), String> {
        ...
        self.with_conn(move |conn| {
            ...
            Ok(())
        }).await.map_err(|e|format!("AI credential migration failed; no configuration rows changed: {e}. Check the original ai-master.key or CHIRON_HORIZON_AI_MASTER_KEY."))
    }
}
