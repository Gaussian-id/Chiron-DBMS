...
/// with "Chiron Horizon metadata pool is busy". Returning the pool's real capacity here
...
impl AppState {
    ...
    pub fn external_driver_runtime_env(&self, driver_id: &str) -> Result<PluginRuntimeEnv, String> {
        ...
        Ok(PluginRuntimeEnv::default().with_var("CHIRON_HORIZON_JAVA_BIN", java.to_string_lossy().to_string()))
    }
    ...
    pub async fn shutdown(&self, deadline: Duration) {
        ...
        if tokio::time::timeout(deadline, shutdown).await.is_err() {
            log::warn!("Timed out shutting down Chiron Horizon runtime resources after {}ms", deadline.as_millis());
        }
    }
...
fn default_plugin_dir() -> PathBuf {
    default_chiron_horizon_dir().join("plugins")
}

pub fn default_agent_dir() -> PathBuf {
    default_chiron_horizon_dir().join("agents")
}

fn default_chiron_horizon_dir() -> PathBuf {
    let home =
        crate::legacy::var(if cfg!(windows) { "USERPROFILE" } else { "HOME" }).unwrap_or_else(|_| ".".to_string());
    PathBuf::from(home).join(".chiron-horizon")
}
...
mod tests {
    ...
    async fn apply_session_credential_injects_saved_password_only_for_no_save_connections() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-session-cred-{}", std::process::id()));
    ...
    async fn apply_session_credential_reads_owner_scoped_credentials_only() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-session-cred-owner-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_dir(storage, dir.join("plugins"));
    ...
    async fn pool_credential_owner_mismatch_prevents_cross_session_pool_reuse() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-pool-owner-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_dir(storage, dir.join("plugins"));
    ...
    fn sqlserver_legacy_url_param_requires_canonicalization_before_agent_driver_selection() {
        let mut config = mysql_config(Some("master"));
        config.db_type = DatabaseType::SqlServer;
        config.url_params = Some("applicationName=chiron-horizon;sqlserverEncryption=disabled".to_string());
    ...
    fn validates_h2_database_base_path_when_mv_db_file_exists() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-h2-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn oceanbase_mysql_setup_queries_follow_query_timeout() {
        let mut config = mysql_config(Some("chiron-horizon"));
    ...
    async fn test_app_state() -> (AppState, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    ...
    async fn app_state_uses_explicit_agent_dir() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-agent-dir-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    ...
    #[tokio::test]
    #[ignore = "requires CHIRON_HORIZON_TEST_MYSQL_URL"]
    async fn live_mysql_health_check_keeps_saturated_pool() {
        let url =
            crate::legacy::var("CHIRON_HORIZON_TEST_MYSQL_URL").expect("CHIRON_HORIZON_TEST_MYSQL_URL is required");
        let (state, dir) = test_app_state().await;
    ...
    async fn jdbc_plugin_env_uses_managed_jre_when_installed() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-jdbc-managed-jre-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_and_agent_dir_and_app_version(
            storage,
        ...
        assert_eq!(env.get("CHIRON_HORIZON_JAVA_BIN"), Some(java.to_string_lossy().as_ref()));
        let _ = std::fs::remove_dir_all(dir);
    }

    #[tokio::test]
    async fn jdbc_plugin_env_keeps_wrapper_fallback_when_managed_jre_is_missing() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-jdbc-missing-jre-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_and_agent_dir_and_app_version(
            storage,
        ...
        assert_eq!(env.get("CHIRON_HORIZON_JAVA_BIN"), None);
        let _ = std::fs::remove_dir_all(dir);
    }

    #[tokio::test]
    async fn jdbc_plugin_env_uses_custom_java_runtime() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-core-jdbc-custom-jre-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
        let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
        let state = AppState::new_with_plugin_and_agent_dir_and_app_version(
            storage,
    ...
    fn kwdb_endpoint_url_keeps_explicit_sslmode() {
        ...
        config.password = "secret".to_string();
        config.port = 26257;
        config.url_params = Some("sslmode=require&application_name=chiron-horizon".to_string());

        assert_eq!(
            connection_url_for_endpoint(&config, &config.host, config.port),
            "postgres://root:secret@127.0.0.1:26257/defaultdb?sslmode=require&application_name=chiron-horizon"
        );
    }
    ...
    fn gaussdb_endpoint_url_keeps_explicit_sslmode() {
        ...
        assert_eq!(
            connection_url_for_endpoint(&config, &config.host, config.port),
            "postgres://gaussdb:secret@127.0.0.1:3306/postgres?sslmode=require&application_name=chiron-horizon"
        );
    }
    ...
    fn gaussdb_endpoint_url_prepends_default_sslmode_to_custom_params() {
        ...
        config.username = "gaussdb".to_string();
        config.password = "secret".to_string();
        config.url_params = Some("application_name=chiron-horizon".to_string());

        assert_eq!(
            connection_url_for_endpoint(&config, &config.host, config.port),
            "postgres://gaussdb:secret@127.0.0.1:3306/postgres?sslmode=prefer&application_name=chiron-horizon"
        );
    }
    ...
    fn session_scoped_pool_keys_are_sanitized_and_detected() {
        ...
        sqlite_memory.host = "/tmp/chiron-horizon-session-test.sqlite".to_string();
        assert_eq!(
...
"#
    ...
    async fn oracle_tns_connection_rejects_transport_layers() {
        let (state, dir) = test_app_state().await;
        let mut config = mysql_config(Some("CHIRON_HORIZON_FAILOVER"));
        config.db_type = DatabaseType::Oracle;
    ...
    async fn live_gaussdb_native_connection_succeeds() {
        let host =
            crate::legacy::var("CHIRON_HORIZON_TEST_GAUSSDB_HOST").expect("CHIRON_HORIZON_TEST_GAUSSDB_HOST not set");
        let port = crate::legacy::var("CHIRON_HORIZON_TEST_GAUSSDB_PORT")
            .expect("CHIRON_HORIZON_TEST_GAUSSDB_PORT not set")
            .parse::<u16>()
            .expect("CHIRON_HORIZON_TEST_GAUSSDB_PORT should be a u16");
        let username =
            crate::legacy::var("CHIRON_HORIZON_TEST_GAUSSDB_USER").expect("CHIRON_HORIZON_TEST_GAUSSDB_USER not set");
        let password = crate::legacy::var("CHIRON_HORIZON_TEST_GAUSSDB_PASSWORD")
            .expect("CHIRON_HORIZON_TEST_GAUSSDB_PASSWORD not set");
        let url_params = crate::legacy::var("CHIRON_HORIZON_TEST_GAUSSDB_URL_PARAMS").ok();
    ...
    async fn live_opengauss_native_connection_succeeds() {
        let host = crate::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_HOST")
            .expect("CHIRON_HORIZON_TEST_OPENGAUSS_HOST not set");
        let port = crate::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_PORT")
            .expect("CHIRON_HORIZON_TEST_OPENGAUSS_PORT not set")
            .parse::<u16>()
            .expect("CHIRON_HORIZON_TEST_OPENGAUSS_PORT should be a u16");
        let username = crate::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_USER")
            .expect("CHIRON_HORIZON_TEST_OPENGAUSS_USER not set");
        let password = crate::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_PASSWORD")
            .expect("CHIRON_HORIZON_TEST_OPENGAUSS_PASSWORD not set");
        let url_params = crate::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_URL_PARAMS").ok();
    ...
    async fn live_kwdb_native_connection_succeeds() {
        let host = crate::legacy::var("CHIRON_HORIZON_TEST_KWDB_HOST").expect("CHIRON_HORIZON_TEST_KWDB_HOST not set");
        let port = crate::legacy::var("CHIRON_HORIZON_TEST_KWDB_PORT")
            .expect("CHIRON_HORIZON_TEST_KWDB_PORT not set")
            .parse::<u16>()
            .expect("CHIRON_HORIZON_TEST_KWDB_PORT should be a u16");
        let username = crate::legacy::var("CHIRON_HORIZON_TEST_KWDB_USER").unwrap_or_else(|_| "root".to_string());
        let password = crate::legacy::var("CHIRON_HORIZON_TEST_KWDB_PASSWORD").unwrap_or_default();
        let database =
            crate::legacy::var("CHIRON_HORIZON_TEST_KWDB_DATABASE").unwrap_or_else(|_| "defaultdb".to_string());
        let url_params =
            crate::legacy::var("CHIRON_HORIZON_TEST_KWDB_URL_PARAMS").unwrap_or_else(|_| "sslmode=disable".to_string());
        ...
        let test_schema = "chiron_horizon_kwdb_live";
