...
pub fn build_query_pagination_execution_plan(
    ...
    // Every page Chiron Horizon generates for this query is derived from the same
...
pub fn build_count_query_sql(options: CountQuerySqlOptions) -> QuerySqlBuildResult {
    ...
    let alias = if options.database_type == Some(DatabaseType::Iris) {
        ...
        "chiron_horizon_count".to_string()
    } else {
        quote_table_identifier(options.database_type, "chiron_horizon_count")
    };
    if options.database_type == Some(DatabaseType::Hive) && starts_with_cte(&statement) {
            ...
            .map(|token| token.start)
        {
            ...
            return ok(format!(
                ...
                derived_table_sql(
                    "SELECT COUNT(*) AS chiron_horizon_total_rows FROM",
                    main_query,
                    &format!("{alias};")
                )
            ));
    ...
    ok(format!(
        "{execution_hint}{}",
        derived_table_sql("SELECT COUNT(*) AS chiron_horizon_total_rows FROM", &wrapped_sql, &format!("{alias};"))
    ))
}
...
fn add_sql_server_offset_fetch(statement: &str, limit: usize, offset: usize) -> Option<String> {
    ...
    // from a derived table used by Chiron Horizon pagination.
    ...
    Some(format!(
        "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER ({row_number_order}) AS [__chiron_horizon_row_num] FROM ({statement_without_order}) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > {offset} AND [__chiron_horizon_row_num] <= {end} ORDER BY [__chiron_horizon_row_num];"
    ))
}
...
const SQLSERVER_RESULT_OFFSET_PREFIX: &str = "/*__chiron_horizon_result_offset=";
...
fn add_sql_server_rowcount_pagination(statement: &str, limit: usize, offset: usize) -> String {
    ...
    // on every SQL Server version supported by Chiron Horizon. The dynamic batch scopes
...
fn add_sql_server_existing_top_pagination(statement: &str, limit: usize, offset: usize) -> String {
    ...
    if offset == 0 {
        ...
        return format!("SELECT TOP ({limit}) * FROM ({derived_statement}) [chiron_horizon_page] {row_number_order};");
    }

    let end = offset + limit;
    let derived_statement = statement_for_sql_suffix(statement);
    format!(
        "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER ({row_number_order}) AS [__chiron_horizon_row_num] FROM ({derived_statement}) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > {offset} AND [__chiron_horizon_row_num] <= {end} ORDER BY [__chiron_horizon_row_num];"
    )
}
...
fn mysql_count_sql(statement: &str) -> Option<String> {
    ...
    let replacement_projection =
        match Parser::parse_sql(&dialect, "SELECT 1 AS chiron_horizon_count_value").ok()?.pop()? {
...
fn mysql_wrapped_count_sql(statement: &str) -> String {
    let alias = quote_table_identifier(Some(DatabaseType::Mysql), "chiron_horizon_count");
    derived_table_sql("SELECT COUNT(*) AS chiron_horizon_total_rows FROM", statement, &format!("{alias};"))
}
...
fn iotdb_tree_count_sql(statement: &str) -> Option<String> {
    ...
    let count_projection =
        match Parser::parse_sql(&dialect, &format!("SELECT COUNT({measurement}) AS chiron_horizon_total_rows"))
            .ok()?
            .pop()?
        {
            Statement::Query(query) => match query.body.as_ref() {
...
fn iris_count_sql(statement: &str) -> Option<String> {
    ...
    let count_projection =
        match Parser::parse_sql(&dialect, "SELECT COUNT(*) AS chiron_horizon_total_rows").ok()?.pop()? {
            Statement::Query(query) => match query.body.as_ref() {
                SetExpr::Select(select) => select.projection.clone(),
                _ => return None,
            },
...
fn sql_server_count_sql(statement: &str) -> Option<String> {
    ...
    if derived_table_projection_safe {
        let alias = quote_table_identifier(Some(DatabaseType::SqlServer), "chiron_horizon_count");
        let wrapped_sql = sql_server_statement_for_derived_table(statement);
        return Some(derived_table_sql(
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM",
            &wrapped_sql,
            &format!("{alias};"),
        ));
...
fn inject_sql_server_top(sql: &str, limit: usize) -> String {
    if sql.len() >= 6 && sql[..6].eq_ignore_ascii_case("SELECT") {
        ...
        format!("SELECT TOP ({limit}) * FROM ({sql}) [chiron_horizon_page]")
    }
}
...
fn add_informix_first_limit(statement: &str, limit: usize, offset: usize) -> String {
    ...
    format!("SELECT {row_limit} * FROM ({statement}) chiron_horizon_page;")
}
...
fn add_iris_top_limit(statement: &str, limit: usize) -> String {
    ...
    if statement.len() >= 6 && statement[..6].eq_ignore_ascii_case("SELECT") {
        ...
        format!("SELECT TOP {limit} * FROM ({statement}) chiron_horizon_page;")
    }
}
...
fn add_fetch_first_limit(statement: &str, limit: usize, offset: usize) -> String {
    if has_top_level_fetch_first(statement) {
        if offset > 0 {
            let alias = quote_table_identifier(None, "chiron_horizon_page");
...
fn add_rownum_limit(statement: &str, limit: usize, offset: usize) -> String {
    ...
    let inner = derived_table_sql(
        "SELECT chiron_horizon_inner.*, ROWNUM AS \"__chiron_horizon_row_num\" FROM",
        statement,
        &format!("chiron_horizon_inner WHERE ROWNUM <= {end}"),
    );
    format!("SELECT * FROM ({inner}) WHERE \"__chiron_horizon_row_num\" > {offset};")
}
...
fn add_outer_standard_limit(
    ...
    let alias = quote_table_identifier(database_type, "chiron_horizon_page");
...
mod tests {
    ...
    /// inline `-- 中文` line comments. Locks in the invariants that keep Chiron Horizon's
    ...
    fn mysql_group_by_with_distinct_if_and_comments_keeps_grouping_in_page_and_count_sql() {
        ...
        assert!(count_sql.starts_with("SELECT COUNT(*) AS chiron_horizon_total_rows FROM ("));
        assert_eq!(
    ...
    fn sqlserver_output_alias_order_keeps_row_number_pagination() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY cxlbh) AS [__chiron_horizon_row_num] FROM (SELECT d.lbbh AS cxlbh FROM T_BASE_WZLB AS d) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 500 AND [__chiron_horizon_row_num] <= 1000 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn paginates_existing_sqlserver_top_clause_on_first_page() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT TOP (100) * FROM (SELECT TOP 1000 * FROM TicketInfo) [chiron_horizon_page] ORDER BY (SELECT NULL);"
        );
    }
    ...
    fn paginates_existing_sqlserver_top_clause_for_later_pages() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS [__chiron_horizon_row_num] FROM (SELECT TOP 1000 * FROM TicketInfo) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 100 AND [__chiron_horizon_row_num] <= 200 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn paginates_existing_sqlserver_top_clause_keeping_user_order_by_desc() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT TOP (100) * FROM (SELECT top 10 [ID], [ProcInstID], [ActInstID], [ActInstDestID], [StartDate], [DestUser], [Status], [Data], [SerialNumber] FROM [dbo].[_WorkList] ORDER BY ID DESC) [chiron_horizon_page] ORDER BY [ID] DESC;"
        );
    }
    ...
    fn paginates_existing_sqlserver_top_clause_later_page_keeping_user_order_by_desc() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY [ID] DESC) AS [__chiron_horizon_row_num] FROM (SELECT top 10 [ID], [ProcInstID] FROM [dbo].[_WorkList] ORDER BY ID DESC) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 100 AND [__chiron_horizon_row_num] <= 200 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn paginates_existing_sqlserver_top_clause_keeping_multi_column_user_order_by() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT TOP (100) * FROM (SELECT top 100 [ID], [StartDate] FROM [dbo].[_WorkList] ORDER BY [StartDate] DESC, ID) [chiron_horizon_page] ORDER BY [StartDate] DESC, [ID];"
        );
    }
    ...
    fn paginates_existing_sqlserver_top_clause_keeping_ordinal_order_by() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT TOP (100) * FROM (SELECT top 100 [ID], [Name] FROM [dbo].[_WorkList] ORDER BY 2 DESC) [chiron_horizon_page] ORDER BY [Name] DESC;"
        );
    }
    ...
    fn paginates_later_sqlserver_top_page_mapping_ordinal_order_to_output_column() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY [Name] DESC) AS [__chiron_horizon_row_num] FROM (SELECT top 100 [ID], [Name] FROM [dbo].[_WorkList] ORDER BY 2 DESC) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 100 AND [__chiron_horizon_row_num] <= 200 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn paginates_sqlserver_top_clause_falling_back_for_out_of_range_ordinal() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY [ID]) AS [__chiron_horizon_row_num] FROM (SELECT top 100 [ID], [Name] FROM [dbo].[_WorkList] ORDER BY 3 DESC) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 100 AND [__chiron_horizon_row_num] <= 200 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn paginates_existing_sqlserver_top_clause_falls_back_when_order_by_not_projected() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT TOP (100) * FROM (SELECT top 100 [ID] FROM [dbo].[_WorkList] ORDER BY [SerialNumber]) [chiron_horizon_page] ORDER BY [ID];"
        );
    }
    ...
    fn sqlserver_join_wildcard_injects_count_projection() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM WZ_CKGL_WZLLDSQ_DETAIL d LEFT JOIN WZ_CKGL_WZLLDSQ_MAIN AS m ON m.ID = d.ParentID;"
        );
    }
    ...
    fn sqlserver_join_wildcard_count_removes_top_level_order_by() {
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some("SELECT COUNT(*) AS chiron_horizon_total_rows FROM AAA a JOIN BBB b ON a.id = b.id;")
        );
    }
    ...
    fn sqlserver_join_wildcard_plan_exposes_count_without_changing_first_page() {
        ...
        assert_eq!(
            ...
            Some(
                "SELECT COUNT(*) AS chiron_horizon_total_rows FROM WZ_CKGL_WZLLDSQ_DETAIL d LEFT JOIN WZ_CKGL_WZLLDSQ_MAIN m ON m.ID = d.ParentID;"
            )
        );
    }
    ...
    fn sqlserver_join_wildcard_uses_bounded_rowcount_pagination() {
        ...
        assert_eq!(
            sql,
            "EXEC sys.sp_executesql N'SET ROWCOUNT 1000; SELECT * FROM WZ_CKGL_WZLLDSQ_DETAIL d LEFT JOIN WZ_CKGL_WZLLDSQ_MAIN AS m ON m.ID = d.ParentID'; /*__chiron_horizon_result_offset=500__*/"
        );
    ...
    fn sqlserver_result_offset_ignores_user_sql_markers() {
        assert_eq!(sqlserver_result_offset("SELECT 1 /*__chiron_horizon_result_offset=500__*/"), 0);
    }
    ...
    fn paginates_sqlserver_top_parenthesized_projection_query_by_first_column() {
        ...
        assert_eq!(
            ...
            "SELECT TOP (100) * FROM (SELECT TOP (500) [id], [order_no], [store_id], [product_id], [customer_name], [quantity], [amount], [order_status], [created_at] FROM [sales].[orders_10k]) [chiron_horizon_page] ORDER BY [id];"
        );
        assert_eq!(
            second_page.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY [id]) AS [__chiron_horizon_row_num] FROM (SELECT TOP (500) [id], [order_no], [store_id], [product_id], [customer_name], [quantity], [amount], [order_status], [created_at] FROM [sales].[orders_10k]) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 100 AND [__chiron_horizon_row_num] <= 200 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn paginates_sqlserver_top_query_with_qualified_first_projection_by_exposed_column() {
        ...
        assert_eq!(
            first_page.sql.unwrap(),
            "SELECT TOP (100) * FROM (select top 1  t1.FSUPPLIERID,  kh.FNAME gysnm   from  GDWORKOUT t1 join  SUPPLIER kh on t1.FSUPPLIERID=kh.FITEMID join GDWORKOUTS t2 on t1.fid=t2.fid) [chiron_horizon_page] ORDER BY [FSUPPLIERID];"
        );
        assert_eq!(
            second_page.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY [FSUPPLIERID]) AS [__chiron_horizon_row_num] FROM (select top 1  t1.FSUPPLIERID,  kh.FNAME gysnm   from  GDWORKOUT t1 join  SUPPLIER kh on t1.FSUPPLIERID=kh.FITEMID join GDWORKOUTS t2 on t1.fid=t2.fid) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 100 AND [__chiron_horizon_row_num] <= 200 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn paginates_sqlserver_top_query_with_dotted_bracket_identifier() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT TOP (100) * FROM (SELECT TOP 1000 [order.id] FROM TicketInfo) [chiron_horizon_page] ORDER BY [order.id];"
        );
    }
    ...
    fn sqlserver_later_page_keeps_derived_table_closing_after_comment_before_order_by() {
        ...
        assert!(generated
            .contains("FROM (SELECT\n*\nFROM\ncode\n-- WHERE\n-- ccode = '1002'\n) chiron_horizon_page_source"));
        assert!(!generated.contains("-- ccode = '1002') chiron_horizon_page_source"));
    }
    ...
    fn sqlserver_later_page_keeps_row_number_window_after_comment_after_order_by() {
        ...
        let generated = result.sql.expect("build SQL Server page SQL");
        assert!(generated.contains("ROW_NUMBER() OVER (ORDER BY\nccode -- sort\n) AS [__chiron_horizon_row_num]"));
        assert!(!generated.contains("-- sort) AS [__chiron_horizon_row_num]"));
    }
...
"#;
    ...
    fn locking_query_plan_keeps_server_pagination_and_count() {
        ...
        assert_eq!(
            plan.count_sql,
            Some("SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT * FROM `test`\nwhere id=1) `chiron_horizon_count`;".to_string())
        );
    ...
    fn locking_query_count_removes_locking_clause() {
        ...
        assert_eq!(
            result.sql,
            Some("SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT * FROM t WHERE deleted = 0) `chiron_horizon_count`;".to_string())
        );
    }
    ...
    fn locking_keywords_inside_postgres_dollar_quote_are_not_rewritten() {
        ...
        assert_eq!(
            ...
            Some("SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT $$FOR UPDATE$$ AS message) \"chiron_horizon_count\";")
        );
    }
    ...
    fn locking_keywords_inside_mysql_hash_comment_are_not_rewritten() {
        ...
        assert_eq!(
            counted.sql.as_deref(),
            Some("SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT * FROM t\n# FOR UPDATE LIMIT 1\n) `chiron_horizon_count`;")
        );
    }
    ...
    fn clickhouse_scalar_with_select_can_be_counted() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (WITH 1 AS min_id SELECT dept, COUNT(*) FROM employees WHERE id >= min_id GROUP BY dept) `chiron_horizon_count`;"
        );
    }
    ...
    fn uses_sqlserver_row_number_pagination_for_later_pages() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_page_source.*, ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS [__chiron_horizon_row_num] FROM (SELECT id FROM users) chiron_horizon_page_source) chiron_horizon_page WHERE [__chiron_horizon_row_num] > 300 AND [__chiron_horizon_row_num] <= 400 ORDER BY [__chiron_horizon_row_num];"
        );
    }
    ...
    fn oceanbase_oracle_pagination_wraps_offset_with_rownum_bounds() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT chiron_horizon_inner.*, ROWNUM AS \"__chiron_horizon_row_num\" FROM (SELECT id FROM users ORDER BY id) chiron_horizon_inner WHERE ROWNUM <= 300) WHERE \"__chiron_horizon_row_num\" > 200;"
        );
    }
    ...
    fn native_mysql_pagination_plan_preserves_exact_issue_directive() {
        ...
        assert_eq!(
            plan.count_sql.as_deref(),
            Some("/*sets:allsets*/SELECT COUNT(*) AS chiron_horizon_total_rows FROM (select @@server_id) `chiron_horizon_count`;")
        );
        assert!(!plan.use_agent_result_session);
    }
    ...
    fn mysql_count_preserves_leading_ampersand_routing_hint() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "/*& tenant:'test' */\nSELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT id FROM users) `chiron_horizon_count`;"
        );
    }
    ...
    fn mysql_count_preserves_directives_at_outermost_start() {
        for prefix in [
            ...
            assert_eq!(
                result.sql.unwrap(),
                format!("{prefix}SELECT COUNT(*) AS chiron_horizon_total_rows FROM (select @@server_id) `chiron_horizon_count`;")
            );
        }
    }
    ...
    fn mysql_pagination_wraps_wide_existing_limit_on_first_page() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT * FROM dy_promotion_item WHERE create_time < '2026-06-01' LIMIT 10000) `chiron_horizon_page` LIMIT 500 OFFSET 0;"
        );
    }
    ...
    fn mysql_pagination_wraps_comma_limit_row_count_on_first_page() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT * FROM users LIMIT 20, 10000) `chiron_horizon_page` LIMIT 500 OFFSET 0;"
        );
    }
    ...
    fn mysql_pagination_wraps_existing_limit_for_later_pages() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT * FROM dy_promotion_item WHERE create_time < '2026-06-01' LIMIT 10000) `chiron_horizon_page` LIMIT 1000 OFFSET 1000;"
        );
    }
    ...
    fn standard_pagination_wraps_existing_limit_for_later_pages() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT id FROM users LIMIT 20) \"chiron_horizon_page\" LIMIT 5 OFFSET 10;"
        );
    }
    ...
    fn builds_count_query() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (WITH cte AS (SELECT 1 AS id) SELECT * FROM cte) `chiron_horizon_count`;"
        );
    }
    ...
    fn hive_count_keeps_cte_outside_derived_table() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "WITH cte AS (SELECT 1 AS id) SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT * FROM cte) `chiron_horizon_count`;"
        );
    }
    ...
    fn hive_count_keeps_from_style_query_inside_derived_table() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "WITH cte AS (SELECT 1 AS id) SELECT COUNT(*) AS chiron_horizon_total_rows FROM (FROM cte SELECT *) `chiron_horizon_count`;"
        );
    }
    ...
    fn mysql_count_rewrites_ambiguous_join_projection() {
        for sql in [
            ...
            assert_eq!(
                result.sql.as_deref(),
                Some(
                    "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT 1 AS chiron_horizon_count_value FROM a JOIN b ON b.a_id = a.id) `chiron_horizon_count`;"
                ),
                "{sql}"
            );
        }
    ...
    fn mysql_count_keeps_unique_projection_wrapper() {
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some(
                "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT a.id AS a_id, b.id AS b_id FROM a JOIN b ON b.a_id = a.id ORDER BY b.id) `chiron_horizon_count`;"
            )
        );
    }
    ...
    fn mysql_count_rewrite_preserves_limit_and_offset() {
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some(
                "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT 1 AS chiron_horizon_count_value FROM a JOIN b ON b.a_id = a.id LIMIT 2 OFFSET 1) `chiron_horizon_count`;"
            )
        );
    }
    ...
    fn mysql_count_keeps_parse_failure_fallback() {
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some(
                "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT a.id AS id, FROM a) `chiron_horizon_count`;"
            )
        );
    }
    ...
    fn non_mysql_count_keeps_ambiguous_projection_wrapper() {
        ...
        for database_type in [DatabaseType::Postgres, DatabaseType::Sqlite, DatabaseType::ClickHouse] {
            ...
            let alias = quote_table_identifier(Some(database_type), "chiron_horizon_count");

            assert_eq!(
                result.sql,
                Some(format!("SELECT COUNT(*) AS chiron_horizon_total_rows FROM ({sql}) {alias};")),
                "{database_type:?}"
            );
        }
    }
    ...
    fn count_query_keeps_wrapper_outside_trailing_line_comment() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT 1 AS id\n-- tail comment\n) `chiron_horizon_count`;"
        );
    }
    ...
    fn count_query_preserves_user_limit() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT * FROM dy_promotion_item WHERE create_time < '2026-06-01' LIMIT 10000) `chiron_horizon_count`;"
        );
    }
    ...
    fn count_query_preserves_user_limit_offset() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT * FROM users WHERE active = 1 LIMIT 100 OFFSET 50) \"chiron_horizon_count\";"
        );
    }
    ...
    fn iotdb_tree_count_rewrites_single_series_without_derived_table() {
        let result = build_count_query_sql(CountQuerySqlOptions {
            original_sql:
                "SELECT WGEN_GnTmpSta1 FROM root.chiron_horizon_time_preview.device WHERE time >= 1 ORDER BY time DESC"
                    .to_string(),
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some("SELECT COUNT(WGEN_GnTmpSta1) AS chiron_horizon_total_rows FROM root.chiron_horizon_time_preview.device WHERE time >= 1;")
        );
    }
    ...
    fn iotdb_pagination_plan_uses_tree_model_count_query() {
        let plan = build_query_pagination_execution_plan(QueryPaginationExecutionPlanOptions {
            sql: "SELECT WGEN_GnTmpSta1 FROM root.chiron_horizon_time_preview.device".to_string(),
            query_base_sql: "SELECT WGEN_GnTmpSta1 FROM root.chiron_horizon_time_preview.device".to_string(),
            database_type: Some(DatabaseType::Iotdb),
            pagination: QueryPagination { limit: 100, offset: 0, session_id: None },
        ...
        assert_eq!(
            plan.count_sql.as_deref(),
            Some(
                "SELECT COUNT(WGEN_GnTmpSta1) AS chiron_horizon_total_rows FROM root.chiron_horizon_time_preview.device;"
            )
        );
    }
    ...
    fn iris_count_query_removes_top_level_order_by() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT id, appointment_time FROM patients WHERE status = ?) chiron_horizon_count;"
        );
    }
    ...
    fn iris_count_query_preserves_nested_order_by_and_parameters() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT * FROM (SELECT TOP ? id FROM visits WHERE status = ? ORDER BY created_at DESC) recent WHERE id > ?) chiron_horizon_count;"
        );
    }
    ...
    fn iris_count_query_without_order_by_is_unchanged() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT id FROM visits WHERE status = ?) chiron_horizon_count;"
        );
    }
    ...
    fn iris_count_query_injects_count_for_legacy_cache_table() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT COUNT(*) AS chiron_horizon_total_rows FROM TATFY WHERE UpdateDate = '2026-07-30' AND UpdateTime > '08:00:00';"
        );
    }
    ...
    fn iris_count_query_injects_count_for_simple_select() {
        ...
        assert_eq!(result.sql.as_deref(), Some("SELECT COUNT(*) AS chiron_horizon_total_rows FROM PA_Adm;"));
    }
    ...
    fn iris_count_query_preserves_simple_select_predicates_without_derived_alias() {
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some("SELECT COUNT(*) AS chiron_horizon_total_rows FROM PA_Adm WHERE status = ?;")
        );
    }
    ...
    fn iris_count_query_keeps_wrapper_for_top_limited_select() {
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some(
                "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT TOP 10 * FROM PA_Adm) chiron_horizon_count;"
            )
        );
    }
    ...
    fn iris_count_query_keeps_derived_wrapper_for_complex_selects() {
        ...
        assert_eq!(
            result.sql.as_deref(),
            Some(
                "SELECT COUNT(*) AS chiron_horizon_total_rows FROM (SELECT a.id, b.id FROM a JOIN b ON b.a_id = a.id) chiron_horizon_count;"
            )
        );
    }
    ...
    fn builds_saphana_sorted_query_without_derived_column_alias_list() {
        let result = build_sorted_query_sql(SortedQuerySqlOptions {
            original_sql: "SELECT ID, NAME, AMOUNT FROM CHIRON_HORIZON_ISSUE_7274_SORT".to_string(),
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT ID, NAME, AMOUNT FROM CHIRON_HORIZON_ISSUE_7274_SORT) t ORDER BY \"NAME\" ASC;"
        );
    }
    ...
    fn doris_distinct_with_existing_limit_first_page_keeps_order_by() {
        ...
        assert_eq!(
            result.sql.unwrap(),
            "SELECT * FROM (SELECT DISTINCT a, b, c FROM t LIMIT 500) `chiron_horizon_page` ORDER BY 1, 2, 3 LIMIT 100 OFFSET 0;"
        );
    }
