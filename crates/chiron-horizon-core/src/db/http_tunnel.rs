...
fn authorized_request(endpoint: &HttpTunnelEndpoint, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
    ...
    match HeaderValue::from_str(&endpoint.token) {
        Ok(token) => builder
            .header("X-Chiron-Horizon-Tunnel-Token", token.clone())
...
fn script_url(
    ...
    url.query_pairs_mut().append_pair("chiron_horizon_action", action).append_pair("chiron_horizon_session", session);
    if let Some((host, port)) = target {
        url.query_pairs_mut()
            .append_pair("chiron_horizon_target_host", host)
            .append_pair("chiron_horizon_target_port", &port.to_string());
    }
    if let Some(connect_timeout_secs) = connect_timeout_secs {
        url.query_pairs_mut().append_pair(
            "chiron_horizon_connect_timeout",
            &effective_connect_timeout_secs(connect_timeout_secs).to_string(),
        );
    }
    if let Some(wait_ms) = wait_ms {
        url.query_pairs_mut().append_pair("chiron_horizon_wait_ms", &wait_ms.to_string());
    }
    Ok(url)
}
...
mod tests {
    ...
    fn script_url_preserves_existing_query_and_appends_action() {
        let url = script_url(
            "https://gateway.example.com/chiron_horizon_tunnel.php?keep=1",
            "open",
            "session-1",
        ...
        assert_eq!(
            url.as_str(),
            "https://gateway.example.com/chiron_horizon_tunnel.php?keep=1&chiron_horizon_action=open&chiron_horizon_session=session-1&chiron_horizon_target_host=mysql.internal&chiron_horizon_target_port=3306&chiron_horizon_connect_timeout=10"
        );
    }
    ...
    impl MockScript {
        async fn start() -> Self {
            ...
            Self { url: format!("http://{addr}/chiron_horizon_tunnel.php"), closed, handle }
        }
    }
    ...
    async fn handle_mock_http_request(stream: TcpStream, buffered: Arc<Mutex<Vec<u8>>>, closed: Arc<Notify>) {
        ...
        if request_line.contains("chiron_horizon_action=write") {
            buffered.lock().await.extend_from_slice(&body);
            write_http_response(&mut stream, "200 OK", b"OK").await;
        } else if request_line.contains("chiron_horizon_action=read") {
            let bytes = {
        ...
        } else if request_line.contains("chiron_horizon_action=close") {
            write_http_response(&mut stream, "200 OK", b"OK").await;
            closed.notify_one();
        } else {
            write_http_response(&mut stream, "200 OK", b"OK").await;
