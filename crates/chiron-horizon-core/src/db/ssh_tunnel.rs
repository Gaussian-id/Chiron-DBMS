...
impl client::Handler for SshClient {
    ...
    async fn check_server_key(
        ...
        // known-hosts stores (system + chiron-horizon-managed). A trusted key is accepted
        // immediately; an unknown key triggers explicit TOFU; a changed key in
        // the chiron-horizon store prompts for replace-or-session-only; a changed key in
        // ~/.ssh/known_hosts is still a hard reject (chiron-horizon never writes that file).
        match self.host_key_verifier.check(&self.host, self.port, server_public_key) {
            ...
            Err(e) => {
                // Changed host key in ~/.ssh/known_hosts => possible MITM. chiron-horizon
...
pub struct TunnelManager {
    ...
    /// chiron-horizon-managed known_hosts path (`<data_dir>/known_hosts`) threaded into
...
mod tests {
    ...
    fn ssh_agent_socket_path_expands_current_user_home() {
        let home = crate::legacy::var("HOME").expect("HOME should be set on Unix test environments");
...
-----END OPENSSH PRIVATE KEY-----"#;
    ...
    async fn unknown_host_without_persist_permission_does_not_leak_password() {
        ...
        // host key can NEVER be persisted. chiron-horizon must fail-closed: abort the
    ...
    async fn reconnect_auth_failure_is_reported_instead_of_a_dead_local_port() {
        ...
        // session. chiron-horizon closes the accepted socket without sending anything,
