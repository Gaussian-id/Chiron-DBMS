...
pub fn classify_command(command: &str) -> RedisCommandSafety {
    ...
    // version, module, or proxy remain high-risk until Chiron Horizon reviews them.
...
mod tests {
    ...
    ///
    /// Set `CHIRON_HORIZON_TEST_REDIS_URL` to a Redis that predates hash-field expiry
    ...
    #[tokio::test]
    #[ignore = "requires a Redis without hash-field expiry via CHIRON_HORIZON_TEST_REDIS_URL"]
    async fn hash_field_update_degrades_on_redis_without_hash_field_ttl() {
        let url = crate::legacy::var("CHIRON_HORIZON_TEST_REDIS_URL").expect("CHIRON_HORIZON_TEST_REDIS_URL");
        let mut con = super::connect(&url, std::time::Duration::from_secs(5)).await.unwrap();

        let key = b"chiron-horizon-7584-regression";
