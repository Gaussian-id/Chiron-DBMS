...
const DYNAMODB_JSON_TYPE_TAG: &str = "$chironHorizonDynamoDb";
...
pub fn connect(config: &ConnectionConfig, host: &str, port: u16) -> Result<DynamoDbClient, String> {
    ...
    let credentials =
        Credentials::new(access_key, secret_key, session_token.map(str::to_string), None, "chiron-horizon");
...
fn parse_dynamodb_statement(source: &str) -> Result<DynamoDbStatement, String> {
    ...
    let operation = match header.to_ascii_uppercase().as_str() {
        "Chiron Horizon DYNAMODB SCAN" | "Chiron Horizon DYNAMODB QUERY / SCAN" => DynamoDbStatementOperation::Read,
        "Chiron Horizon DYNAMODB INSERT ITEM" => DynamoDbStatementOperation::Insert,
        "Chiron Horizon DYNAMODB PUT ITEM" => DynamoDbStatementOperation::Put,
        "Chiron Horizon DYNAMODB DELETE ITEM" => DynamoDbStatementOperation::Delete,
        _ => {
            return Err(
                "Unsupported DynamoDB statement. Use a Chiron Horizon DYNAMODB SCAN, QUERY / SCAN, INSERT ITEM, PUT ITEM, or DELETE ITEM statement."
                    .to_string(),
            )
        }
    };
...
mod tests {
    ...
    fn parses_generated_scan_statement() {
        let statement = parse_dynamodb_statement(
            r#"Chiron Horizon DYNAMODB QUERY / SCAN
table: "orders"
limit: 1000
filter:
{
...
sort:
{
  "created_at": -1
    ...
    fn parses_generated_insert_statement_with_multiline_item() {
        let statement = parse_dynamodb_statement(
            r#"Chiron Horizon DYNAMODB INSERT ITEM
table: "orders"
key:
{
  "tenant_id": "tenant-04",
  ...
  "tags": {
    ...
    fn rejects_write_statement_without_explicit_key() {
        let error = parse_dynamodb_statement(
            r#"Chiron Horizon DYNAMODB PUT ITEM
table: "orders"
item:
{
  "tenant_id": "tenant-04",
  "order_id": "ORD-new"
    ...
    #[tokio::test]
    #[ignore = "requires CHIRON_HORIZON_DYNAMODB_ENDPOINT and an orders table with tenant_id/order_id string keys"]
    async fn live_executes_generated_statements() {
        let endpoint = crate::legacy::var("CHIRON_HORIZON_DYNAMODB_ENDPOINT")
            .expect("CHIRON_HORIZON_DYNAMODB_ENDPOINT is required");
        ...
        let config: ConnectionConfig = serde_json::from_value(serde_json::json!({
            ...
            "username": crate::legacy::var("CHIRON_HORIZON_DYNAMODB_ACCESS_KEY_ID").unwrap_or_else(|_| "dummy".to_string()),
            "password": crate::legacy::var("CHIRON_HORIZON_DYNAMODB_SECRET_ACCESS_KEY").unwrap_or_else(|_| "dummy".to_string()),
            "database": crate::legacy::var("CHIRON_HORIZON_DYNAMODB_REGION").unwrap_or_else(|_| "us-east-1".to_string()),
            "ssl": ssl
        }))
        .unwrap();
        ...
        let insert = format!(
            "Chiron Horizon DYNAMODB INSERT ITEM\ntable: \"orders\"\nkey:\n{}\nitem:\n{}",
        ...
        let query = format!(
            "Chiron Horizon DYNAMODB QUERY / SCAN\ntable: \"orders\"\nlimit: 10\nfilter:\n{}",
            serde_json::to_string_pretty(&key).unwrap()
        );
        ...
        let put = format!(
            "Chiron Horizon DYNAMODB PUT ITEM\ntable: \"orders\"\nkey:\n{}\nitem:\n{}",
            serde_json::to_string_pretty(&key).unwrap(),
        ...
        for cleanup_key in [&rekey_key, &conflict_key] {
            let delete = format!(
                "Chiron Horizon DYNAMODB DELETE ITEM\ntable: \"orders\"\nkey:\n{}",
    ...
    #[tokio::test]
    #[ignore = "requires CHIRON_HORIZON_DYNAMODB_ENDPOINT and permission to create a temporary table"]
    async fn live_reports_partial_projection_metadata() {
        ...
        .unwrap();
        let client = connect(&config, host, config.port).unwrap();
        let table_name = format!("chiron_horizon_projection_{}", uuid::Uuid::new_v4().simple());
    ...
    async fn live_queries_number_and_binary_keys_from_extended_json() {
        ...
        .unwrap();
        let client = connect(&config, host, config.port).unwrap();
        let table_name = format!("chiron_horizon_extended_keys_{}", uuid::Uuid::new_v4().simple());
    ...
    fn preserves_dynamodb_specific_json_types() {
        let value = serde_json::json!({
            "tags": {
                "$chironHorizonDynamoDb": { "version": 1, "type": "stringSet", "value": ["one", "two"] }
            },
            "payload": {
                "$chironHorizonDynamoDb": { "version": 1, "type": "binary", "value": "aGVsbG8=" }
            },
            "large": {
                "$chironHorizonDynamoDb": {
                    "version": 1,
    ...
    fn wraps_integers_that_javascript_cannot_represent_exactly() {
        ...
        assert_eq!(
            value,
            serde_json::json!({
                "$chironHorizonDynamoDb": { "version": 1, "type": "number", "value": "9007199254740993" }
            })
        );
