...
async fn start_transport_layers_internal(
    ...
    for (index, chain) in ssh_chains.iter().enumerate() {
        ...
        if chain.as_ref().is_some_and(|chain| chain.len() > 1) && index != 0 {
            return Err(format!(
                "SSH layer {} uses a multi-hop `~/.ssh/config` ProxyJump chain, which Chiron Horizon only supports as the first transport layer. Add the jump host as its own tunnel layer instead.",
                index + 1
            ));
        }
    }
...
mod tests {
    ...
    fn http_tunnel_must_be_outermost_layer() {
        let layers = vec![
            ssh_layer("ssh-a", "bastion-a", 22),
            http_tunnel_layer("http", "https://chiron-horizon.example.com/chiron_horizon_tunnel.php"),
        ];
