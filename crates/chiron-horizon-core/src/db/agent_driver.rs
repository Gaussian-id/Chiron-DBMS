...
/// `CHIRON_HORIZON_AGENT_ERROR_DATA` marker themselves. Callers must only use it after the
...
const AGENT_JAVA_OPTS_ENV: &str = "CHIRON_HORIZON_AGENT_JAVA_OPTS";
const AGENT_JAVA_TOO_OLD_MESSAGE: &str =
    "Agent requires Java 21, but Chiron Horizon started it with an older Java runtime. Use Chiron Horizon managed JRE 21 or select a Java 21 executable in Driver Manager.";
...
impl AgentDriverClient {
    ...
    /// they speak the Chiron Horizon stdin/stdout JSON-RPC protocol.
...
fn agent_java_args(jar_path: &str) -> Vec<String> {
    agent_java_args_with_extra(jar_path, crate::legacy::var(AGENT_JAVA_OPTS_ENV).ok().as_deref())
}
...
fn agent_java_args_with_extra_args(jar_path: &str, extra_java_args: &[String]) -> Vec<String> {
    agent_java_args_with_extra_opts(jar_path, crate::legacy::var(AGENT_JAVA_OPTS_ENV).ok().as_deref(), extra_java_args)
}
...
mod tests {
    ...
    fn spawn_repairs_missing_native_execute_permission_after_eacces() {
        ...
        let dir = std::env::temp_dir().join(format!("chiron-horizon-agent-permission-{}", uuid::Uuid::new_v4()));
    ...
    fn contract_violation_keeps_identity_when_legacy_api_adds_session_id() {
        ...
        assert_eq!(crate::backend_error::BackendError::from_legacy_string(&legacy).code(), "Chiron Horizon-JDBC-5002");
    }
    ...
    fn legacy_adapter_preserves_strict_identity_when_old_callers_append_context() {
        ...
        assert_eq!(
            crate::backend_error::BackendError::from_legacy_string(&suffixed).code(),
            "Chiron Horizon-JDBC-5002"
        );
    }
    ...
    fn agent_java_args_include_oracle_network_compatibility_flags() {
        let args = agent_java_args("/tmp/chiron-horizon-agent-oracle.jar");
    ...
    fn agent_java_args_open_java_sql_for_legacy_timestamp_serializers() {
        let args = agent_java_args("/tmp/chiron-horizon-agent-dameng.jar");
    ...
    fn agent_java_args_disable_ambient_proxy_settings() {
        let args = agent_java_args("/tmp/chiron-horizon-agent-opengauss.jar");
    ...
    fn agent_java_args_prefer_ipv4_for_informix() {
        let args = agent_java_args("/tmp/chiron-horizon/drivers/informix/agent.jar");
    ...
    fn agent_java_args_do_not_prefer_ipv4_for_other_agents() {
        let args = agent_java_args("/tmp/chiron-horizon/drivers/highgo/agent.jar");
    ...
    fn agent_java_args_open_java_net_for_dameng() {
        let args = agent_java_args("/tmp/chiron-horizon/drivers/dameng/agent.jar");
    ...
    fn agent_java_args_open_jdk_internals_for_ignite() {
        let args = agent_java_args("/tmp/chiron-horizon/drivers/ignite/agent.jar");
    ...
    fn agent_java_args_do_not_open_jdk_internals_for_other_agents() {
        let args = agent_java_args("/tmp/chiron-horizon/drivers/kylin/agent.jar");
    ...
    fn agent_java_args_include_custom_jvm_options_before_jar() {
        let args = agent_java_args_with_extra(
            "/tmp/chiron-horizon/drivers/hive/agent.jar",
    ...
    fn formats_agent_process_error_with_exit_status_and_stderr_tail() {
        ...
        stderr_tail.push_line("\tat com.chiron.horizon.agent.hive.HiveAgent.connect(HiveAgent.kt:21)".to_string());
    ...
    async fn spawn_stateful_test_runtime(prefix: &str) -> (Arc<AgentRuntimeClient>, std::path::PathBuf) {
        let script_path =
            std::env::temp_dir().join(format!("chiron-horizon-agent-{prefix}-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
...
"#,
    ...
    async fn shared_runtime_tolerates_post_startup_driver_stdout_noise() {
        let script_path =
            std::env::temp_dir().join(format!("chiron-horizon-agent-runtime-stdout-noise-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            r#"import json, sys
print(json.dumps({'ready': True}), flush=True)
...
"#,
    ...
    async fn shared_runtime_fails_all_pending_requests_after_stdout_noise_limit() {
        let script_path = std::env::temp_dir()
            .join(format!("chiron-horizon-agent-runtime-stdout-noise-limit-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            format!(
                r#"import json, sys
...
"#,
    ...
    async fn legacy_client_tolerates_post_startup_driver_stdout_noise() {
        let script_path =
            std::env::temp_dir().join(format!("chiron-horizon-agent-legacy-stdout-noise-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            r#"import json, sys
print(json.dumps({'ready': True}), flush=True)
...
"#,
    ...
    async fn multiplexed_runtime_correlates_out_of_order_responses() {
        let script_path =
            std::env::temp_dir().join(format!("chiron-horizon-agent-runtime-test-{}.py", uuid::Uuid::new_v4()));
...
"#,
    ...
    async fn shared_runtime_routes_agents_without_handshake_to_legacy_fallback() {
        let script_path = std::env::temp_dir()
            .join(format!("chiron-horizon-agent-runtime-no-handshake-test-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            r#"import json, sys
print(json.dumps({'ready': True}), flush=True)
...
"#,
    ...
    async fn disconnect_reports_runtime_replacement_without_owning_runtime_routing() {
        let script_path = std::env::temp_dir()
            .join(format!("chiron-horizon-agent-runtime-cleanup-saturation-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            r#"import json, sys
print(json.dumps({'ready': True}), flush=True)
...
"#,
    ...
    async fn canceling_one_runtime_request_keeps_other_requests_alive() {
        let script_path =
            std::env::temp_dir().join(format!("chiron-horizon-agent-cancel-test-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            r#"import json, sys, threading, time
print(json.dumps({'ready': True}), flush=True)
...
"#,
    ...
    async fn runtime_eof_fails_pending_requests_and_marks_runtime_failed() {
        let script_path =
            std::env::temp_dir().join(format!("chiron-horizon-agent-eof-test-{}.py", uuid::Uuid::new_v4()));
        std::fs::write(
            &script_path,
            r#"import json, sys
print(json.dumps({'ready': True}), flush=True)
