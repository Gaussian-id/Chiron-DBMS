...
//!    key change recorded there is detected here too. chiron-horizon never *writes* to it.
//! 2. chiron-horizon's own store at `<data_dir>/known_hosts` (OpenSSH format, managed by
//!    russh's `known_hosts` module) is checked. This is where chiron-horizon-accepted host
...
//! 4. A *changed* key in the chiron-horizon store is [`HostKeyState::Changed`]. The caller
...
//!    in `~/.ssh/known_hosts` stays a hard reject — chiron-horizon does not edit that file.
//!
//! Hardening rules:
...
pub enum HostKeyState {
    /// The key matches a previously trusted entry (system or chiron-horizon store).
    Trusted,
    /// The key is not present in either store (candidate for explicit TOFU).
    Unknown,
    /// The chiron-horizon store has a different key of the same algorithm. The caller
...
///
/// `known_hosts_path` is the chiron-horizon-managed store (typically
...
impl HostKeyVerifier {
    ...
    /// changed key in the chiron-horizon store is [`HostKeyState::Changed`] so the UI can
    ...
    pub fn check(&self, host: &str, port: u16, key: &PublicKey) -> Result<HostKeyState, io::Error> {
        ...
        match check_known_hosts(host, port, key) {
            ...
            // don't fail the whole connection on it; fall through to the chiron-horizon
            // store / TOFU, which still protects against MITM.
            _ => {}
        }

        // 2. chiron-horizon-managed known_hosts.
        match check_known_hosts_path(host, port, key, &self.known_hosts_path) {
            ...
            // Unknown (or an unreadable chiron-horizon store): report as a candidate for TOFU.
            _ => {}
        }
    ...
    /// Records a host key into the chiron-horizon store (TOFU persistence). Called by the
    ...
    /// Drops the previous same-algorithm key for this host in the chiron-horizon store,
