...
mod tests {
    ...
    fn merges_v1_series_and_promotes_tags_to_columns() {
        let first = InfluxSeries {
            name: "chiron_horizon_smoke".to_string(),
    ...
    fn v2_config_uses_token_auth_and_org_urls() {
        ...
        assert_eq!(client.org.as_deref(), Some("Chiron Horizon Org"));
        assert_eq!(
            build_v2_buckets_url(&client, 0).unwrap(),
            "http://localhost:8086/api/v2/buckets?org=Gauss%20Horizon%20Org&limit=100&offset=0"
        );
        assert_eq!(
            build_v2_query_url(&client).unwrap(),
            "http://localhost:8086/api/v2/query?org=Gauss%20Horizon%20Org"
        );
    }
