...
mod tests {
    ...
    fn scanner_location_reuses_the_configured_gateway_origin() {
        assert_eq!(
            scanner_resource_path("http://hbase.internal:8080/chiron-horizon/scanner/123?token=abc").unwrap(),
            "/chiron-horizon/scanner/123?token=abc"
        );
        assert_eq!(scanner_resource_path("/chiron-horizon/scanner/123").unwrap(), "/chiron-horizon/scanner/123");
        assert_eq!(scanner_resource_path("chiron-horizon/scanner/123").unwrap(), "/chiron-horizon/scanner/123");
    }

    #[tokio::test]
    #[ignore = "requires CHIRON_HORIZON_TEST_HBASE_URL and a writable HBase REST gateway"]
    async fn live_hbase_rest_crud_and_scan() {
        let url =
            crate::legacy::var("CHIRON_HORIZON_TEST_HBASE_URL").expect("CHIRON_HORIZON_TEST_HBASE_URL is required");
        let username = crate::legacy::var("CHIRON_HORIZON_TEST_HBASE_USERNAME").ok();
        let password = crate::legacy::var("CHIRON_HORIZON_TEST_HBASE_PASSWORD").ok();
        ...
        let table = format!("chiron_horizon_hbase_live_{suffix}");
