...
fn collect_mapping_columns(
    ...
    // Recursive walks overflow tokio worker stacks on indexes like `chiron_horizon_all_types`.
...
/// Close a previously returned ES cursor. Supports Chiron Horizon-wrapped paged cursors,
/// Chiron Horizon-wrapped SQL cursors, and raw ES SQL cursors.
...
/// form is produced by the Chiron Horizon pagination plan; it must be removed before
...
mod tests {
  ...
  "products": {
    ...
    #[tokio::test]
    #[ignore = "requires CHIRON_HORIZON_TEST_KIBANA_URL pointing to a reachable Kibana instance"]
    async fn live_kibana_proxy_supports_metadata_queries_and_document_writes() {
        let url =
            crate::legacy::var("CHIRON_HORIZON_TEST_KIBANA_URL").expect("CHIRON_HORIZON_TEST_KIBANA_URL is required");
        let username = crate::legacy::var("CHIRON_HORIZON_TEST_KIBANA_USERNAME").ok();
        let password = crate::legacy::var("CHIRON_HORIZON_TEST_KIBANA_PASSWORD").ok();
        let base_path = crate::legacy::var("CHIRON_HORIZON_TEST_KIBANA_BASE_PATH").unwrap_or_default();
        ...
        let index = format!("chiron-horizon-kibana-proxy-{}", uuid::Uuid::new_v4().simple());
