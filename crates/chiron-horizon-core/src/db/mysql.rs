...
/// Chiron Horizon-owned MySQL pool handle. The driver does not expose its configured
...
fn mysql_geometry_to_export_marker(bytes: &[u8]) -> Option<String> {
    ...
    Some(format!("CHIRON_HORIZON_WKB:{srid}:{}", bytes_to_upper_hex(wkb)))
}
...
fn mysql_result_protected_column_indexes(
    ...
    for (index, column) in columns.iter().enumerate() {
        if column
            .name_str()
            .to_ascii_uppercase()
            .starts_with(crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX)
        {
...
/// Extracts an opt-in `catalog=<name>` URL parameter. chiron-horizon strips it from the
/// URL before handing it to mysql_async (see `is_chiron_horizon_handled_mysql_url_param`)
...
fn is_chiron_horizon_handled_mysql_url_param(key: &str) -> bool {
    matches!(
        key.to_ascii_lowercase().as_str(),
        "charset"
            | "catalog"
...
fn mysql_async_url(url: &str) -> Cow<'_, str> {
    ...
    for segment in query.split('&') {
        ...
        if is_chiron_horizon_handled_mysql_url_param(key) {
            changed = true;
            continue;
        }
        if key.eq_ignore_ascii_case("ssl-mode") || key.eq_ignore_ascii_case("sslmode") {
...
async fn execute_result_sets_with_text_protocol_on_conn(
    ...
    // `deprecate_eof` option requested when the pool was built (Chiron Horizon disables
...
fn prefers_text_protocol_query(sql: &str, dialect: MySqlQueryDialect) -> bool {
    // User-entered result-set queries are not parameterized in Chiron Horizon. Text protocol
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires a disposable MySQL reachable via CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_{HOST,PORT,USER,PASSWORD}"]
    async fn live_mysql_progress_read_survives_a_total_duration_beyond_the_timeout() {
        let Ok(host) = crate::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_HOST") else {
            return;
        };
        let port = crate::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_PORT").unwrap_or_else(|_| "3306".to_string());
        let user = crate::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_USER").unwrap_or_else(|_| "root".to_string());
        let password = crate::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_PASSWORD").unwrap_or_default();
        let database =
            crate::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_DATABASE").unwrap_or_else(|_| "test".to_string());
    ...
    #[tokio::test]
    #[ignore = "requires CHIRON_HORIZON_REVIEW_MYSQL_* environment variables"]
    async fn mysql_checkout_fault_injection_preserves_wait_stage_when_real_pool_is_full() {
        let host = crate::legacy::var("CHIRON_HORIZON_REVIEW_MYSQL_HOST")
            .expect("CHIRON_HORIZON_REVIEW_MYSQL_HOST is required");
        let port = crate::legacy::var("CHIRON_HORIZON_REVIEW_MYSQL_PORT")
            .expect("CHIRON_HORIZON_REVIEW_MYSQL_PORT is required");
        let user = crate::legacy::var("CHIRON_HORIZON_REVIEW_MYSQL_USER")
            .expect("CHIRON_HORIZON_REVIEW_MYSQL_USER is required");
        let password = crate::legacy::var("CHIRON_HORIZON_REVIEW_MYSQL_PASSWORD")
            .expect("CHIRON_HORIZON_REVIEW_MYSQL_PASSWORD is required");
        let database = crate::legacy::var("CHIRON_HORIZON_REVIEW_MYSQL_DATABASE")
            .expect("CHIRON_HORIZON_REVIEW_MYSQL_DATABASE is required");
        let pool_options =
            mysql_async::PoolOpts::new().with_constraints(mysql_async::PoolConstraints::new(1, 1).unwrap());
        let options = mysql_async::OptsBuilder::default()
            .ip_or_hostname(host)
            .tcp_port(port.parse().expect("CHIRON_HORIZON_REVIEW_MYSQL_PORT must be a valid port"))
            .user(Some(user))
            .pass(Some(password))
    ...
    fn mysql_geometry_decoder_accepts_unprefixed_wkb() {
        ...
        assert_eq!(
            ...
            Some("CHIRON_HORIZON_WKB:0:0101000000000000000000F03F0000000000000040")
        );
    }
    ...
    fn mysql_geometry_export_marker_strips_internal_srid_prefix() {
        ...
        assert_eq!(
            mysql_geometry_to_export_marker(&raw).as_deref(),
            Some("CHIRON_HORIZON_WKB:4326:0101000000000000000000F03F0000000000000040")
        );
    }
    ...
    fn mysql_xa_recover_queries_are_treated_as_text_result_sets() {
        ...
        for sql in [
            "XA START 'chiron_horizon_xid'",
            "XA BEGIN 'chiron_horizon_xid'",
            "XA END 'chiron_horizon_xid'",
            "XA PREPARE 'chiron_horizon_xid'",
            "XA COMMIT 'chiron_horizon_xid'",
            "XA ROLLBACK 'chiron_horizon_xid'",
        ] {
            assert!(!is_result_set_query(sql, dialect), "{sql}");
        }
    }
    ...
    fn mysql_large_value_preview_skips_bounded_strings_and_server_preview_columns() {
        ...
        let marker_column = Column::new(ColumnType::MYSQL_TYPE_VAR_STRING)
            .with_name(format!("{}B_2", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX).as_bytes())
            .with_character_set(45)
            .with_column_length(64);
        let row = mysql_test_row_with_columns(
            vec![
    ...
    fn mysql_tls_url_strips_client_identity_params_before_driver_parse() {
        ...
        let cert = dir.join(format!("chiron-horizon-mysql-client-cert-{}.pem", std::process::id()));
        let key = dir.join(format!("chiron-horizon-mysql-client-key-{}.pem", std::process::id()));
    ...
    #[test]
    fn mysql_index_metadata_accepts_polarchiron_horizon_expression_error_only_with_matching_message() {
        let unsupported = mysql_async::Error::Server(mysql_async::ServerError {
            code: 4518,
    ...
    async fn live_shardingsphere_proxy_preserves_binary_columns() {
        let url = crate::legacy::var("CHIRON_HORIZON_MYSQL_SHARDING_PROXY_URL")
            .expect("CHIRON_HORIZON_MYSQL_SHARDING_PROXY_URL must point to the live proxy fixture");
        let opts = mysql_async::Opts::from_url(&url).expect("valid MySQL proxy URL");
        let pool = mysql_async::Pool::new(opts);

        let columns = get_columns(&pool, "chiron_horizon_sharding_proxy_test", "binary_samples")
            .await
            .expect("load proxy column metadata");
