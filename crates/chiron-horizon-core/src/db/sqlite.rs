...
mod tests {
    ...
    async fn create_if_missing_rejects_existing_non_sqlite_file() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-not-sqlite-{}.png", uuid::Uuid::new_v4()));
    ...
    async fn create_if_missing_allows_empty_custom_suffix_file() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-empty-sqlite-{}.conf", uuid::Uuid::new_v4()));
    ...
    async fn create_if_missing_allows_sqlite_database_with_custom_suffix() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-custom-sqlite-{}.conf", uuid::Uuid::new_v4()));
        {
    ...
    async fn sqlcipher_key_creates_and_reopens_encrypted_database() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-sqlcipher-{}.db", uuid::Uuid::new_v4()));
        let key = "secret key";
    ...
    async fn sqlcipher_key_opens_legacy_compatible_database() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-sqlcipher-legacy-{}.db", uuid::Uuid::new_v4()));
        let key = "legacy key";
    ...
    async fn encrypted_sqlite_key_opens_and_updates_1024_page_rc4_legacy_database() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-rc4-legacy-{}.db", uuid::Uuid::new_v4()));
        let key = "123456";
        let page_size = 1024;
...

        {
            ...
            execute_query(&reopened, "INSERT INTO t VALUES ('written-by-chiron-horizon')")
                .await
                .expect("update RC4 database");
        }
    ...
    async fn sqlcipher_key_requires_sqlcipher_feature() {
        let err = match connect_path_with_cipher_key_and_extensions(
            "/tmp/chiron-horizon-missing-sqlcipher.db",
            "secret",
            Vec::new(),
        )
        .await
    ...
    async fn attached_database_metadata_uses_attached_schema() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-sqlite-attach-{}.sqlite", uuid::Uuid::new_v4()));
        {
