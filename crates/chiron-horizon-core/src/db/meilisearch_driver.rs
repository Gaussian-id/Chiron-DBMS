...
const STRING_ID_PREFIX: &str = "__chiron_horizon_meilisearch_string_id__";
...
pub async fn update_document(client: &MeilisearchClient, index: &str, id: &str, doc_json: &str) -> Result<u64, String> {
    ...
    let mut document = parse_document_object(doc_json)?;
    // `_id` is only the chiron-horizon browse alias when it matches the target identity; a
    ...
    // Chiron Horizon sends the complete edited document so removed fields must disappear.
...
/// Search hit with chiron-horizon identity and Meilisearch response metadata kept outside
...
mod tests {
    ...
    fn search_identity_never_shadows_a_real_underscore_id_field() {
        // A document may legitimately store its own `_id` field; the chiron-horizon identity
    ...
    fn string_primary_keys_keep_their_string_identity() {
        ...
        let serialized = r#"__chiron_horizon_meilisearch_string_id__"123""#;
    ...
    async fn get_document_fetches_the_canonical_record_by_identity() {
        ...
        let document_json =
            super::get_document(&client, "movies", r#"__chiron_horizon_meilisearch_string_id__"SN-0001""#)
                .await
                .unwrap();
