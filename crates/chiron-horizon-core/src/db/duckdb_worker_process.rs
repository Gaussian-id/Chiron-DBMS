...
pub const DUCKDB_DRIVER_PATH_ENV: &str = "CHIRON_HORIZON_DUCKDB_DRIVER_PATH";
...
fn resolve_duckdb_driver_command() -> Result<(PathBuf, Vec<OsString>), String> {
    if let Some(executable) = crate::legacy::var_os(DUCKDB_DRIVER_PATH_ENV).filter(|value| !value.is_empty()) {
