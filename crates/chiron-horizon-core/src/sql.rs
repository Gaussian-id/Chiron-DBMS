...
mod tests {
    ...
    fn keeps_postgres_e_string_newlines_inside_batch_statement() {
        let sql =
            "SELECT version();\n\nCREATE TABLE chiron_horizon_test (\n    delimiter VARCHAR(32) DEFAULT E'\\n\\n'\n);";
    ...
    fn mysql_split_skips_comment_only_statement() {
        assert!(split_sql_statements_for_database(
            "-- Chiron Horizon SQL preview crash reproducer\n\n;",
            DatabaseType::Mysql
        )
        .is_empty());
    }
...
END;";
    ...
    fn argo_split_handles_full_user_procedure_with_block_comment_and_at_meta() {
        ...
        // is fine and the issue is on the Chiron Horizon.app side.
...
DROP FUNCTION IF EXISTS chiron_horizon_issue_4572_tmp_missing;";
        ...
        assert_eq!(statements[0], "DROP FUNCTION IF EXISTS chiron_horizon_issue_4572_tmp_md5_uuid");
        assert!(statements[1].starts_with("CREATE OR REPLACE FUNCTION chiron_horizon_issue_4572_tmp_md5_uuid"));
        assert!(statements[1].ends_with("END$function$"));
        assert_eq!(statements[2], "DROP FUNCTION IF EXISTS chiron_horizon_issue_4572_tmp_missing");
    }
...
SELECT 1;";
    ...
    fn xugu_split_keeps_create_package_body_together() {
        let sql = "\
CREATE OR REPLACE PACKAGE BODY chiron_horizon_pkg AS
    PROCEDURE ping AS
    BEGIN
        NULL;
    END ping;
END chiron_horizon_pkg;
/
SELECT 1;";
    ...
    fn xugu_split_keeps_routines_without_or_replace_together() {
        let cases = [
            "CREATE PROCEDURE chiron_horizon_proc_without_replace AS BEGIN NULL; END;",
            "CREATE FUNCTION chiron_horizon_func_without_replace RETURN INTEGER AS BEGIN RETURN 1; END;",
            "CREATE TRIGGER chiron_horizon_trigger_without_replace BEFORE INSERT ON chiron_horizon_events FOR EACH ROW BEGIN NULL; END;",
        ];
    ...
    fn xugu_split_keeps_force_package_body_together() {
        let sql = "\
CREATE OR REPLACE FORCE PACKAGE BODY chiron_horizon_pkg AS
    PROCEDURE ping AS
    BEGIN
        NULL;
    END ping;
...
SELECT 1;";
        ...
        assert_eq!(
            split_sql_statements_for_database(
                "CREATE OR REPLACE NOFORCE PACKAGE BODY chiron_horizon_pkg AS\n    PROCEDURE ping AS\n    BEGIN\n        NULL;\n    END ping;\nEND chiron_horizon_pkg;\nSELECT 1;",
                DatabaseType::Xugu
            ),
            vec![
                "CREATE OR REPLACE NOFORCE PACKAGE BODY chiron_horizon_pkg AS\n    PROCEDURE ping AS\n    BEGIN\n        NULL;\n    END ping;\nEND chiron_horizon_pkg;",
        ...
        assert_eq!(
            split_sql_statements_for_database(
                "CREATE PACKAGE BODY chiron_horizon_pkg_without_replace AS\n    PROCEDURE ping AS\n    BEGIN\n        NULL;\n    END ping;\nEND chiron_horizon_pkg_without_replace;\nSELECT 1;",
                DatabaseType::Xugu
            ),
            vec![
                "CREATE PACKAGE BODY chiron_horizon_pkg_without_replace AS\n    PROCEDURE ping AS\n    BEGIN\n        NULL;\n    END ping;\nEND chiron_horizon_pkg_without_replace;",
...
SELECT 1;";
    ...
    fn oracle_like_split_keeps_case_expressions_inside_routines() {
        let function = "CREATE OR REPLACE FUNCTION chiron_horizon_case_expr RETURN NUMBER AS\nBEGIN\n  RETURN CASE WHEN 1 = 1 THEN CASE WHEN 2 = 2 THEN 1 ELSE 2 END ELSE 0 END;\nEND;\nSELECT 1;";
        let procedure = "CREATE OR REPLACE PROCEDURE chiron_horizon_case_statement AS\nBEGIN\n  CASE WHEN 1 = 1 THEN NULL; ELSE NULL; END CASE;\nEND;\nSELECT 1;";

        for database in [DatabaseType::Xugu, DatabaseType::Oracle] {
            assert_eq!(
                split_sql_statements_for_database(function, database),
                vec![
                    "CREATE OR REPLACE FUNCTION chiron_horizon_case_expr RETURN NUMBER AS\nBEGIN\n  RETURN CASE WHEN 1 = 1 THEN CASE WHEN 2 = 2 THEN 1 ELSE 2 END ELSE 0 END;\nEND;",
                    "SELECT 1"
                ]
            );
            assert_eq!(
                ...
                vec![
                    "CREATE OR REPLACE PROCEDURE chiron_horizon_case_statement AS\nBEGIN\n  CASE WHEN 1 = 1 THEN NULL; ELSE NULL; END CASE;\nEND;",
                    "SELECT 1"
                ]
            );
        }
