...
use crate::ai_cli_agent::{
    build_cli_agent_prompt, chiron_horizon_mcp_scope_env, cli_command, parse_cli_jsonl_event, run_cli_jsonl_agent,
...
impl OpenCodeIsolatedCwd {
    fn create() -> Result<Self, String> {
        let path = env::temp_dir().join(format!("chiron-horizon-opencode-{}", uuid::Uuid::new_v4()));
...
fn is_reserved_env_name(name: &str) -> bool {
    ...
    upper.starts_with("CHIRON_HORIZON_MCP_") || OPENCODE_CONTROL_ENV.iter().any(|reserved| upper == *reserved)
}
...
pub fn opencode_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.opencode_cli_env {
        ...
        if is_reserved_env_name(key) {
            return Err(format!(
                "[openCodeEnvReserved] `{key}` is managed by Chiron Horizon for the isolated OpenCode session and cannot be set here."
            ));
        }
...
fn opencode_runtime_config(options: Option<&OpenCodeRunOptions>) -> Value {
    ...
    config.insert("permission".to_string(), json!({ "*": "deny", "chiron_horizon_*": "allow" }));

    if let Some(options) = options {
        let command = options.mcp_server_command.as_ref().cloned().unwrap_or_else(|| CliAgentCommandSpec {
            program: "chiron-horizon-mcp-server".to_string(),
            args: Vec::new(),
        });
        let mut command_parts = vec![command.program];
        command_parts.extend(command.args);
        let environment = chiron_horizon_mcp_scope_env(options)
            .into_iter()
...
fn classify_opencode_run_error(message: &str) -> String {
    if message.starts_with("[openCode") || message.starts_with("[chironHorizonMcpMissing]") {
        ...
        || lower.contains("please sign in")
    {
        ...
        format!("[chironHorizonMcpMissing] {message}")
...
mod tests {
    ...
    fn options() -> OpenCodeRunOptions {
        OpenCodeRunOptions {
            ...
            mcp_server_command: Some(CliAgentCommandSpec {
                program: "/opt/chiron-horizon/chiron-horizon-mcp-server".to_string(),
    ...
    fn live_config() -> AiConfig {
        let mut config = config(
            &crate::legacy::var("CHIRON_HORIZON_LIVE_OPENCODE_MODEL")
                .expect("set CHIRON_HORIZON_LIVE_OPENCODE_MODEL to an authenticated provider/model"),
        );
        config.opencode_cli_path = crate::legacy::var("CHIRON_HORIZON_LIVE_OPENCODE_PATH").ok();
        config
    }
    ...
    fn runtime_config_scopes_mcp_and_denies_other_tools() {
        ...
        assert_eq!(runtime.pointer("/permission/chiron_horizon_*").and_then(Value::as_str), Some("allow"));
        assert_eq!(
            runtime.pointer("/mcp/chiron-horizon/command/0").and_then(Value::as_str),
            Some("/opt/chiron-horizon/chiron-horizon-mcp-server")
        );
        assert_eq!(runtime.pointer("/mcp/chiron-horizon/command/1").and_then(Value::as_str), Some("--stdio"));
        assert_eq!(
            runtime
                .pointer("/mcp/chiron-horizon/environment/CHIRON_HORIZON_MCP_SCOPE_CONNECTION_ID")
                .and_then(Value::as_str),
            Some("conn-1")
        );
        assert_eq!(
            runtime.pointer("/mcp/chiron-horizon/environment/CHIRON_HORIZON_MCP_SCOPE_SCHEMA").and_then(Value::as_str),
            Some("public")
        );
    }
    ...
    fn rejects_reserved_or_invalid_environment_names() {
        ...
        reserved.opencode_cli_env.insert("CHIRON_HORIZON_MCP_ALLOW_WRITES".to_string(), "1".to_string());
        assert!(opencode_cli_env(&reserved).unwrap_err().starts_with("[openCodeEnvReserved]"));
    }
    ...
    fn parses_text_reasoning_tool_error_and_usage_events() {
        ...
        let tool = parse_opencode_jsonl_event(
            r#"{"type":"tool_use","part":{"id":"part-1","callID":"call-1","tool":"chiron_horizon_chiron_horizon_list_connections","state":{"status":"completed","input":{},"output":"ok"}}}"#,
        )
        .unwrap();
        assert_eq!(tool.len(), 2);
        assert!(
            matches!(&tool[0], AgentEvent::ToolCallStart { tool_call_id, tool_name, .. } if tool_call_id == "call-1" && tool_name == "chiron_horizon_chiron_horizon_list_connections")
        );
    ...
    #[tokio::test]
    #[ignore = "requires an installed, authenticated OpenCode CLI and CHIRON_HORIZON_LIVE_OPENCODE_MODEL"]
    ...
    #[tokio::test]
    #[ignore = "requires OpenCode, a running Chiron Horizon MCP bridge, and CHIRON_HORIZON_LIVE_MCP_CONNECTION_NAME/DATABASE"]
    async fn live_scoped_chiron_horizon_mcp_agent_run() {
        ...
        assert!(events.lock().unwrap().iter().any(|event| {
            matches!(event, AgentEvent::ToolCallStart { tool_name, .. } if tool_name.contains("chiron_horizon_list_connections"))
        }));
    }
}
