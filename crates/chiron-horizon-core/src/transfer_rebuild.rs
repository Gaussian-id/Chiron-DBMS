...
/// Marker that identifies a table as a Chiron Horizon transfer backup.
pub const BACKUP_TABLE_MARKER: &str = "__chiron_horizon_bak_";
...
mod tests {
    ...
    fn backup_name_uses_the_full_budget_boundaries() {
        ...
        // The white-label marker is intentionally longer; Oracle retains the
        // largest UTF-8-safe prefix that still leaves room for its hash.
        let oracle = name(DatabaseType::Oracle, LONG_TABLE);
        assert_eq!(oracle.len(), 30);
        assert!(oracle.starts_with("cu"), "unexpected Oracle stem: {oracle}");

        // Short names are left intact.
        let short = name(DatabaseType::Postgres, "orders");
        assert!(short.starts_with("orders__chiron_horizon_bak_"), "unexpected short name: {short}");
    }
    ...
    fn multibyte_names_truncate_on_char_boundaries() {
        let source = "客户订单明细修订历史记录表";
        // PostgreSQL leaves enough room to prove we never split a UTF-8 character.
        let generated = name(DatabaseType::Postgres, source);
        let stem_bytes = max_identifier_bytes(DatabaseType::Postgres) - BACKUP_TABLE_MARKER.len() - BACKUP_HASH_LEN;
        let expected_stem = truncate_on_char_boundary(source, stem_bytes);
        assert!(generated.starts_with(expected_stem), "unexpected CJK stem: {generated}");
        assert!(generated.len() <= max_identifier_bytes(DatabaseType::Postgres));
    }
    ...
    fn failure_messages_point_at_the_retained_backup() {
        let annotated = annotate_error_with_retained_backup(
            ...
            "`shop`.`orders__chiron_horizon_bak_1a2b3c4d`",
        );
        assert!(annotated.starts_with("Failed to insert batch: duplicate key."), "original error must lead");
        assert!(
            annotated.contains("`shop`.`orders__chiron_horizon_bak_1a2b3c4d`"),
            "backup name is missing: {annotated}"
        );
    }
    ...
    fn a_message_that_already_names_the_backup_is_left_alone() {
        ...
        let original = "Transfer completed successfully, but failed to drop backup table \
                        'orders__chiron_horizon_bak_1a2b3c4d': permission denied."
            .to_string();
        assert_eq!(
            annotate_error_with_retained_backup(original.clone(), "orders__chiron_horizon_bak_1a2b3c4d"),
            original
        );
    }
