...
use chiron_horizon_core::plugins::{
    PluginHost, PluginInstallPolicy, PluginPackageInstaller, PluginRegistry, PluginTrustStore,
};
use serde_json::{json, Value};

const PLUGIN_ID: &str = "chiron.horizon.example.hello";
...
async fn run() -> Result<(), String> {
    let package =
        std::env::args().nth(1).map(PathBuf::from).ok_or("Usage: plugin_package_smoke <package.chiron-horizonp>")?;
    let store = tempfile::tempdir().map_err(|error| error.to_string())?;
    let trusted_keys = chiron_horizon_core::legacy::var("CHIRON_HORIZON_PLUGIN_SMOKE_TRUSTED_KEYS_JSON")
        .ok()
    ...
    let provider_icon = installed
        .plugin
        .manifest
        .connection_provider("chiron.horizon.example.hello.connection")?
    ...
    let files = host
        .list_filesystem_entries(
            PLUGIN_ID,
            "chiron.horizon.example.hello.files",
            Some(&config.id),
            Some("hello:/"),
            None,
            Some(20),
