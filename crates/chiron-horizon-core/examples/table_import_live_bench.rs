...
Usage:\n  cargo run -p chiron-horizon-core --example table_import_live_bench --release -- [options]\n\n\
Options:\n\
...
Connection environment:\n\
  CHIRON_HORIZON_BENCH_HOST, CHIRON_HORIZON_BENCH_PORT, CHIRON_HORIZON_BENCH_USER, CHIRON_HORIZON_BENCH_PASSWORD,\n\
  CHIRON_HORIZON_BENCH_DATABASE, CHIRON_HORIZON_BENCH_SCHEMA, CHIRON_HORIZON_BENCH_SSL"
    );
}
...
fn env_required(name: &str) -> Result<String, String> {
    chiron_horizon_core::legacy::var(name).map_err(|_| format!("Missing environment variable: {name}"))
}

fn connection_config(id: &str, database: BenchDatabase) -> Result<ConnectionConfig, String> {
    let database_name = env_required("CHIRON_HORIZON_BENCH_DATABASE")?;
    Ok(ConnectionConfig {
        ...
        host: env_required("CHIRON_HORIZON_BENCH_HOST")?,
        port: chiron_horizon_core::legacy::var("CHIRON_HORIZON_BENCH_PORT")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or_else(|| database.default_port()),
        username: env_required("CHIRON_HORIZON_BENCH_USER")?,
        password: env_required("CHIRON_HORIZON_BENCH_PASSWORD")?,
        ...
        ssl: chiron_horizon_core::legacy::var("CHIRON_HORIZON_BENCH_SSL")
...
async fn execute_sql(
    ...
    chiron_horizon_core::query::execute_sql_statement(state, connection_id, database, sql, Some(schema), None)
        .await
        .map(|_| ())
}
...
async fn run() -> Result<(), String> {
    ...
    let database_name = env_required("CHIRON_HORIZON_BENCH_DATABASE")?;
    let schema = env_required("CHIRON_HORIZON_BENCH_SCHEMA")?;
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let connection_id = format!("table-import-live-bench-{suffix}");
    let table = format!("chiron_horizon_import_bench_{}", &suffix[..12]);
    let temp_dir = tempfile::Builder::new()
        .prefix(&format!("chiron-horizon-table-import-live-bench-{suffix}-"))
        .tempdir()
        .map_err(|error| error.to_string())?;
