...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_POSTGRES_HOST/PORT/USER/PASSWORD/DATABASE pointing at a live db"]
async fn dump_keycloak_fixture() {
    ...
    let options = chiron_horizon_core::docs::CollectOptions {
    ...
    let snapshot_result = chiron_horizon_core::docs::collect_snapshot(
        &state,
        &config,
        &options,
        &|_progress| {},
    ...
    chiron_horizon_core::docs::annotations::apply_annotations(&mut snapshot, &annotations, DatabaseType::Postgres);

    let anchor = snapshot.tables.iter().find(|table| table.name == ANCHOR_TABLE).expect("anchor table collected");
    assert_eq!(
        anchor.note_source,
        chiron_horizon_core::docs::NoteSource::Local,
    ...
    assert!(
        snapshot
            .warnings
            .iter()
            .any(|warning| matches!(warning, chiron_horizon_core::docs::SnapshotWarning::OrphanedNotes { .. })),
    ...
    snapshot.relationships = chiron_horizon_core::docs::build_relationships(&snapshot.tables);
    assert!(!snapshot.relationships.is_empty(), "the allowlist must form a connected foreign-key subgraph");

    let json = serde_json::to_string_pretty(&snapshot).expect("serialize snapshot");

    let out_path = chiron_horizon_core::legacy::var("CHIRON_HORIZON_FIXTURE_OUT")
