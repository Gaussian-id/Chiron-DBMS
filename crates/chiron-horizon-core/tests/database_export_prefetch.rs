...
async fn run_database_export_writes_structure_and_data_for_all_tables() {
    let Some(container) = start_docker_postgres("chiron-horizon-export-prefetch") else {
        return;
    };
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-export-prefetch-{}", uuid::Uuid::new_v4()));
    ...
    let container_replay_file = "/var/lib/postgresql/data/chiron-horizon-issue-6739-replay.sql";
