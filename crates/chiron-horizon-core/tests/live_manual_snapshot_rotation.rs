...
async fn setup(config: ConnectionConfig) -> (Arc<AppState>, std::path::PathBuf, String) {
    let storage_path =
        std::env::temp_dir().join(format!("chiron-horizon-snapshot-rotation-{}.db", uuid::Uuid::new_v4().simple()));
    let state = Arc::new(AppState::new(Storage::open(&storage_path).await.expect("temporary storage")));
    let table_name = format!("chiron_horizon_pr9434_{}", uuid::Uuid::new_v4().simple());
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MANUAL_TXN_MYSQL_* on the remote test server"]
...
async fn live_mysql_write_then_reads_preserve_commit() {
    verify_write_then_read_survives_commit(live_config(
        "CHIRON_HORIZON_LIVE_MANUAL_TXN_MYSQL",
        DatabaseType::Mysql,
        3306,
    ))
    .await;
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MANUAL_TXN_POSTGRES_* on the remote test server"]
async fn live_postgres_write_then_reads_preserve_commit() {
    verify_write_then_read_survives_commit(live_config(
        "CHIRON_HORIZON_LIVE_MANUAL_TXN_POSTGRES",
