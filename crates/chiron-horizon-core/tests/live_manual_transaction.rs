...
use chiron_horizon_core::database_export::{
...
fn live_config(prefix: &str, db_type: DatabaseType, default_port: u16) -> ConnectionConfig {
    let host = chiron_horizon_core::legacy::var(format!("{prefix}_HOST")).expect("live DB host env var");
    let port = chiron_horizon_core::legacy::var(format!("{prefix}_PORT"))
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(default_port);
    let username = chiron_horizon_core::legacy::var(format!("{prefix}_USER")).expect("live DB user env var");
    let password = chiron_horizon_core::legacy::var(format!("{prefix}_PASSWORD")).expect("live DB password env var");
    let database = chiron_horizon_core::legacy::var(format!("{prefix}_DATABASE")).expect("live DB database env var");
    let url_params = chiron_horizon_core::legacy::var(format!("{prefix}_URL_PARAMS")).ok();
...
async fn app_state_with_config(config: ConnectionConfig) -> (Arc<AppState>, std::path::PathBuf) {
    let db_path =
        std::env::temp_dir().join(format!("chiron-horizon-live-manual-txn-{}.db", uuid::Uuid::new_v4().simple()));
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MANUAL_TXN_POSTGRES_* env vars pointing at writable PostgreSQL"]
async fn live_manual_transaction_postgres_preserves_typed_selects_and_empty_metadata() {
    let config = live_config("CHIRON_HORIZON_LIVE_MANUAL_TXN_POSTGRES", DatabaseType::Postgres, 5432);
...
#[test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MANUAL_TXN_POSTGRES_* env vars pointing at writable PostgreSQL with pgvector"]
fn live_postgres_backup_snapshot_streams_wide_pgvector_result() {
    std::thread::Builder::new()
        .name("chiron-horizon-live-pg-wide-vector".to_string())
...
async fn live_postgres_backup_snapshot_streams_wide_pgvector_result_inner() {
    let mut config = live_config("CHIRON_HORIZON_LIVE_MANUAL_TXN_POSTGRES", DatabaseType::Postgres, 5432);
    ...
    let schema = format!("chiron_horizon_7986_{}", &suffix[..12]);
    let setup_sql = [
...
fn live_postgres_backup_snapshot_exports_wide_jsonb_then_next_table() {
    std::thread::Builder::new()
        .name("chiron-horizon-live-pg-wide-backup".to_string())
        .stack_size(16 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_multi_thread()
                .worker_threads(2)
...
async fn live_postgres_backup_snapshot_exports_wide_jsonb_then_next_table_inner() {
    ...
    let (state, db_path) = app_state_with_config(config).await;
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("chiron_horizon_7087_{}", &suffix[..12]);
    let export_path = std::env::temp_dir().join(format!("chiron-horizon-7087-wide-jsonb-{suffix}.sql"));
    ...
    let export_result = export_database_sql_core(&state, &request, |progress| {
        if matches!(progress.status, chiron_horizon_core::database_export::ExportStatus::Done) {
...
fn live_postgres_backup_snapshot_cancel_interrupts_pending_row() {
    std::thread::Builder::new()
        .name("chiron-horizon-live-pg-backup-cancel".to_string())
        .stack_size(16 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_multi_thread()
                .worker_threads(2)
...
async fn live_postgres_backup_snapshot_cancel_interrupts_pending_row_inner() {
    let admin_config = live_config("CHIRON_HORIZON_LIVE_MANUAL_TXN_POSTGRES", DatabaseType::Postgres, 5432);
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("chiron_horizon_7087_cancel_{}", &suffix[..8]);
    let role = format!("chiron-horizon7087_{}", &suffix[..12]);
    let password = format!("chiron-horizon-{suffix}");
    ...
    let export_path = std::env::temp_dir().join(format!("chiron-horizon-7087-cancel-{suffix}.sql"));
    let request = DatabaseExportRequest {
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MANUAL_TXN_MYSQL_* env vars pointing at writable MySQL"]
async fn live_manual_transaction_mysql_streams_with_row_limit() {
    let config = live_config("CHIRON_HORIZON_LIVE_MANUAL_TXN_MYSQL", DatabaseType::Mysql, 3306);
    let database = config.database.clone().expect("database");
    let (state, db_path) = app_state_with_config(config.clone()).await;
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MANUAL_TXN_MYSQL_* env vars pointing at readable MySQL with table t_0001"]
async fn live_mysql_database_backup_refreshes_an_idle_snapshot_before_export() {
    let config = live_config("CHIRON_HORIZON_LIVE_MANUAL_TXN_MYSQL", DatabaseType::Mysql, 3306);
    let database = config.database.clone().expect("database");
    let (state, db_path) = app_state_with_config(config.clone()).await;
    let export_path =
        std::env::temp_dir().join(format!("chiron-horizon-live-backup-{}.sql", uuid::Uuid::new_v4().simple()));
