use chiron_horizon_core::agent_manager::{
...
use chiron_horizon_core::agent_service::{
...
fn test_manager(name: &str) -> AgentManager {
    let dir = std::env::temp_dir().join(format!("chiron-horizon-agent-service-{name}-{}", uuid::Uuid::new_v4()));
    ...
    drivers.insert(
        ...
        DriverInfo {
            ...
            jar: Some(ArtifactInfo {
                url: format!("https://example.com/chiron-horizon-agent-{db_type}.jar"),
                sha256: None,
                size: 42,
                format: None,
            }),
    ...
    drivers.insert(
        ...
        DriverInfo {
            ...
            native: [(
                ...
                ArtifactInfo {
                    url: format!("https://example.com/chiron-horizon-agent-{db_type}"),
                    sha256: None,
                    size: 42,
                    format: None,
                },
...
fn agent_list_marks_installed_driver_update_when_registry_version_differs() {
    ...
    manager
        .save_state(&chiron_horizon_core::agent_manager::AgentState {
            installed_drivers: [(
                "h2".to_string(),
                InstalledDriver {
...
fn local_agent_jar_candidates_include_monorepo_and_legacy_build_output() {
    ...
    assert!(candidates
        .iter()
        .any(|path| path.ends_with("agents/drivers/tdengine/build/libs/chiron-horizon-agent-tdengine.jar")));
    assert!(candidates
        .iter()
        .any(|path| path.ends_with("chiron-horizon-agents/tdengine/build/libs/chiron-horizon-agent-tdengine.jar")));
}

fn test_path(name: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("chiron-horizon-agent-service-{name}-{}", uuid::Uuid::new_v4()))
}
...
async fn local_jar_import_updates_driver_state() {
    ...
    let source = test_path("local-import-source").join("chiron-horizon-agent-h2.jar");
...
async fn local_jar_import_rejects_corrupt_jar() {
    ...
    let source = test_path("local-import-corrupt-source").join("chiron-horizon-agent-h2.jar");
    std::fs::create_dir_all(source.parent().unwrap()).unwrap();
...
async fn local_native_import_installs_current_platform_executable() {
    ...
    let source = test_path("local-native-import-source").join(if cfg!(windows) {
        "chiron-horizon-agent-kingbase-windows.exe"
    } else {
        "chiron-horizon-agent-kingbase"
    });
    std::fs::create_dir_all(source.parent().unwrap()).unwrap();
...
async fn local_native_import_rejects_wrong_platform_binary() {
    ...
    let source = test_path("local-native-import-invalid-source").join("chiron-horizon-agent-kingbase");
    std::fs::create_dir_all(source.parent().unwrap()).unwrap();
    ...
    let registry = serde_json::json!({
        "drivers": {
            "h2": {
                ...
                "jre": DEFAULT_JRE_KEY,
                "jar": { "url": "https://example.com/chiron-horizon-agent-h2.jar", "size": jar.len() }
            }
        }
    });
    zip.start_file("agent-registry.json", options).unwrap();
    std::io::Write::write_all(&mut zip, registry.to_string().as_bytes()).unwrap();
    zip.start_file("drivers/chiron-horizon-agent-h2.jar", options).unwrap();
...
fn write_offline_tar_zstd_java_driver_package(path: &std::path::Path, db_type: &str, version: &str) {
    let jar = test_agent_jar_bytes();
    let filename = format!("chiron-horizon-agent-{db_type}-{version}.jar");
    let registry = serde_json::json!({
        "jres": {},
        "drivers": {
            db_type: {
...
fn write_offline_hybrid_driver_zip(path: &std::path::Path, db_type: &str, version: &str) {
    ...
    let platform = AgentManager::current_platform();
    let extension = if platform.starts_with("windows-") { ".exe" } else { "" };
    let jar_filename = format!("chiron-horizon-agent-{db_type}-{version}.jar");
    let native_filename = format!("chiron-horizon-agent-{db_type}-{version}-{platform}{extension}");
    let registry = serde_json::json!({
        "jres": {},
        "drivers": {
            db_type: {
...
fn write_offline_driver_zip_with_zstd_jre(path: &std::path::Path, db_type: &str, version: &str, jre_version: &str) {
    ...
    let registry = serde_json::json!({
        "jres": {
            DEFAULT_JRE_KEY: {
                ...
                "platforms": {
                    platform: {
                        "url": format!("https://example.com/chiron-horizon-jre-{DEFAULT_JRE_KEY}-{platform}.tar.zst"),
        ...
        "drivers": {
            db_type: {
                ...
                "min_app_version": "0.1.0",
                "jre": DEFAULT_JRE_KEY,
                "jar": { "url": format!("https://example.com/chiron-horizon-agent-{db_type}-{version}.jar"), "size": jar.len() }
            }
        }
    });
    ...
    zip.start_file("agent-registry.json", options).unwrap();
    std::io::Write::write_all(&mut zip, registry.to_string().as_bytes()).unwrap();
    zip.start_file(format!("jre/chiron-horizon-jre-{DEFAULT_JRE_KEY}-{platform}.tar.zst"), options).unwrap();
    std::io::Write::write_all(&mut zip, &jre_archive).unwrap();
    zip.start_file(format!("drivers/chiron-horizon-agent-{db_type}-{version}.jar"), options).unwrap();
    std::io::Write::write_all(&mut zip, &jar).unwrap();
    zip.finish().unwrap();
}
...
fn write_offline_driver_zip_with_jre_and_jar_bytes(
    ...
    std::io::Write::write_all(&mut zip, registry.to_string().as_bytes()).unwrap();
    zip.start_file(
        format!("jre/chiron-horizon-jre-{DEFAULT_JRE_KEY}-{}.tar.gz", AgentManager::current_platform()),
        options,
    )
    .unwrap();
    std::io::Write::write_all(&mut zip, &jre_archive).unwrap();
...
fn test_jre_archive_bytes() -> Vec<u8> {
    ...
    let runtime_root = root.join("chiron-horizon-jre");
    ...
    builder.append_dir_all("chiron-horizon-jre", &runtime_root).unwrap();
