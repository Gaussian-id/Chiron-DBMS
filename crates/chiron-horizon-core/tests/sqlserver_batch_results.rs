use chiron_horizon_core::db::sqlserver;
use chiron_horizon_core::models::connection::DatabaseType;
use chiron_horizon_core::query_result_sql::{build_paginated_query_sql, PaginatedQuerySqlOptions};
use std::time::Duration;

async fn connect_sqlserver() -> sqlserver::SqlServerClient {
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_SQLSERVER_HOST")
        .expect("CHIRON_HORIZON_TEST_SQLSERVER_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_SQLSERVER_PORT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(1433);
    let user =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_SQLSERVER_USER").unwrap_or_else(|_| "sa".to_string());
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_SQLSERVER_PASSWORD")
        .expect("CHIRON_HORIZON_TEST_SQLSERVER_PASSWORD");
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_SQLSERVER_DATABASE")
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_TEST_SQLSERVER_HOST and CHIRON_HORIZON_TEST_SQLSERVER_PASSWORD"]
...
async fn sqlserver_transaction_batch_reports_affected_rows() {
    ...
    sqlserver::execute_simple_batch_with_max_rows(
        &mut client,
        "CREATE TABLE #chiron_horizon_transaction_rows (id INT NOT NULL PRIMARY KEY, value INT NOT NULL); \
         INSERT INTO #chiron_horizon_transaction_rows (id, value) VALUES (1, 0), (2, 0), (3, 0), (4, 0);",
        None,
    )
    .await
    ...
    let plain = sqlserver::execute_query(
        &mut client,
        "UPDATE #chiron_horizon_transaction_rows SET value = value + 1 WHERE id IN (1, 2, 3);",
    )
    .await
    ...
    let persisted = sqlserver::execute_query(
        &mut client,
        "SELECT SUM(value) AS total_value FROM #chiron_horizon_transaction_rows;",
    )
    .await
    ...
    let multiple = sqlserver::execute_query(
        &mut client,
        "BEGIN TRANSACTION; \
         UPDATE #chiron_horizon_transaction_rows SET value = value + 1 WHERE id IN (1, 2); \
         DELETE FROM #chiron_horizon_transaction_rows WHERE id = 4; \
         COMMIT TRANSACTION;",
    )
    .await
...
async fn sqlserver_driver_execute_reports_balanced_transaction_rows() {
    ...
    client
        .simple_query(
            "CREATE TABLE #chiron_horizon_driver_transaction_rows (id INT NOT NULL PRIMARY KEY, value INT NOT NULL); \
             INSERT INTO #chiron_horizon_driver_transaction_rows (id, value) VALUES (1, 0), (2, 0), (3, 0), (4, 0);",
        )
        .await
    ...
    let result = client
        .execute(
            "BEGIN TRANSACTION; \
             UPDATE #chiron_horizon_driver_transaction_rows SET value = value + 1 WHERE id IN (1, 2, 3); \
             COMMIT TRANSACTION;",
            &[],
        )
        .await
...
async fn sqlserver_duplicate_join_columns_page_without_leaking_rowcount() {
    ...
    sqlserver::execute_simple_batch_with_max_rows(
        &mut client,
        "CREATE TABLE #chiron_horizon_page_detail (id INT NOT NULL PRIMARY KEY, parent_id INT NOT NULL); \
         CREATE TABLE #chiron_horizon_page_main (id INT NOT NULL PRIMARY KEY, label NVARCHAR(20) NOT NULL); \
         WITH numbers AS ( \
           SELECT TOP (1200) ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS value \
           FROM sys.all_objects a CROSS JOIN sys.all_objects b \
         ) \
         INSERT INTO #chiron_horizon_page_main (id, label) SELECT value, CONCAT(N'label-', value) FROM numbers; \
         INSERT INTO #chiron_horizon_page_detail (id, parent_id) SELECT id, id FROM #chiron_horizon_page_main;",
        None,
    )
    .await
    ...
    let reported_query =
        "SELECT * FROM #chiron_horizon_page_detail d LEFT JOIN #chiron_horizon_page_main m ON m.id = d.parent_id";
    ...
    let failing_page = build_paginated_query_sql(PaginatedQuerySqlOptions {
        original_sql:
            "SELECT * FROM #chiron_horizon_page_detail d LEFT JOIN #chiron_horizon_missing_page_table m ON m.id = d.parent_id ORDER BY d.id"
                .to_string(),
        database_type: Some(DatabaseType::SqlServer),
        limit: 1,
        offset: 1,
    ...
    assert!(error.to_ascii_lowercase().contains("chiron_horizon_missing_page_table"));
