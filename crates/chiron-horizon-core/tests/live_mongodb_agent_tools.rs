...
use chiron_horizon_core::agent_events::{ToolCall, ToolResult};
use chiron_horizon_core::agent_tools::{execute_tool, AgentSqlPermissions};
...
fn live_mongodb_config(id: &str) -> ConnectionConfig {
    let host =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_HOST").expect("CHIRON_HORIZON_LIVE_MONGODB_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_PORT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(27017);
    let username =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_USER").expect("CHIRON_HORIZON_LIVE_MONGODB_USER");
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_MONGODB_PASSWORD");
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_DATABASE")
        .expect("CHIRON_HORIZON_LIVE_MONGODB_DATABASE");
    let auth_source = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_AUTH_SOURCE")
        .unwrap_or_else(|_| database.clone());
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MONGODB_42_URL pointing at MongoDB 4.2 with enableTestCommands=1"]
