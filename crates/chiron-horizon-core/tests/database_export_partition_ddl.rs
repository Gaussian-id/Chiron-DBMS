...
async fn run_database_export_of_partition_tree_has_no_duplicates_and_replays() {
    let Some(container) = start_docker_postgres("chiron-horizon-export-partition-ddl") else {
        return;
    };
    ...
    psql(
        ...
        "DROP TABLE IF EXISTS events CASCADE;\
         ...
         DROP SERVER IF EXISTS chiron_horizon_export_loopback CASCADE;\
         CREATE EXTENSION IF NOT EXISTS postgres_fdw;\
         CREATE SERVER chiron_horizon_export_loopback FOREIGN DATA WRAPPER postgres_fdw \
           OPTIONS (host 'localhost', dbname 'postgres', port '5432');\
         CREATE USER MAPPING FOR CURRENT_USER SERVER chiron_horizon_export_loopback \
         ...
         CREATE FOREIGN TABLE events_foreign PARTITION OF events FOR VALUES FROM ('2028-01-01') TO ('2029-01-01') \
           SERVER chiron_horizon_export_loopback OPTIONS (schema_name 'public', table_name 'foreign_child_data');\
    ...
    let selected_table_ddl = chiron_horizon_core::schema::get_table_export_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
    ...
    let ordinary_table_ddl = chiron_horizon_core::schema::get_table_export_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
    ...
    let missing_table_error = chiron_horizon_core::schema::get_table_export_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
...
async fn partition_tree_ddl_query_count_does_not_scale_with_partition_count() {
    let Some(container) = start_docker_postgres("chiron-horizon-partition-ddl-query-count") else {
        return;
    };
    ...
    async fn fetch_ddl(state: &AppState, connection_id: &str) -> String {
        chiron_horizon_core::schema::get_table_display_ddl_core(state, connection_id, "postgres", "public", "t", None)
            .await
...
async fn display_ddl_error_distinguishes_wrong_relkind_from_missing_relation() {
    let Some(container) = start_docker_postgres("chiron-horizon-partition-ddl-relkind-error") else {
        return;
    };
    ...
    let view_error =
        chiron_horizon_core::schema::get_table_display_ddl_core(&state, connection_id, "postgres", "public", "v", None)
            .await
    ...
    let missing_error = chiron_horizon_core::schema::get_table_display_ddl_core(
        &state,
        connection_id,
        "postgres",
        "public",
...
async fn cloudberry_native_ddl_still_includes_every_partition() {
    let Some(container) = start_docker_postgres("chiron-horizon-cloudberry-partition-ddl") else {
        return;
    };
    ...
    let pool =
        chiron_horizon_core::db::postgres::connect(&url, std::time::Duration::from_secs(5)).await.expect("connect");

    let ddl = chiron_horizon_core::schema::cloudberry_ddl(&pool, "public", "t", true)
        .await
    ...
    let single_ddl = chiron_horizon_core::schema::cloudberry_ddl(&pool, "public", "t", false)
        .await
...
async fn opentenbase_tree_ddl_gives_every_partition_its_own_distribute_by() {
    let Some(container) = start_docker_postgres_with_server_args(
        "chiron-horizon-opentenbase-partition-ddl",
    ...
    let distributions = chiron_horizon_core::db::opentenbase::table_distribution_for_relations(
        &pool,
    ...
    assert_eq!(
        ...
        Some(&chiron_horizon_core::db::opentenbase::DistributionPolicy::Hash(vec!["region".to_string()]))
    );
    assert_eq!(
        distributions.get(&("public".to_string(), "t_p1".to_string())),
        Some(&chiron_horizon_core::db::opentenbase::DistributionPolicy::Hash(vec!["id".to_string()]))
    );

    let ddl =
        chiron_horizon_core::schema::opentenbase_ddl(&pool, "public", "t", true).await.expect("opentenbase tree ddl");
    assert!(
