...
async fn sqlserver_connect(database: &str) -> chiron_horizon_core::db::sqlserver::SqlServerClient {
    let (host, port, user, password) = sqlserver_env().await;
    chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
...
async fn sqlserver_object_on_table_exists(
    ...
    let result = chiron_horizon_core::db::sqlserver::execute_query(client, &sql).await.expect("query object existence");
    result.rows.first().and_then(|row| row.first()).and_then(|v| v.as_i64()).map(|n| n == 1).unwrap_or(false)
}

#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD pointing at SQL Server"]
async fn live_sqlserver_transfer_rebuild_releases_constraint_and_index_names() {
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_db = format!("chiron_horizon_rebuild_src_{}", &suffix[..12]);
    let target_db = format!("chiron_horizon_rebuild_dst_{}", &suffix[..12]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-sqlserver-rebuild-{suffix}"));
    ...
    let test_result = async {
        ...
        for table in ["departments", "employees"] {
            let pk = chiron_horizon_core::db::sqlserver::execute_query(
                &mut target_client,
                &format!(
        ...
        let count = chiron_horizon_core::db::sqlserver::execute_query(
            &mut target_client,
        ...
        let orphan = chiron_horizon_core::db::sqlserver::execute_query(&mut target_client, "INSERT INTO dbo.employees VALUES (7, 999)").await;
    ...
    let cleanup = chiron_horizon_core::db::sqlserver::execute_batch(
        &mut master,
        &format!(
...
async fn live_sqlserver_transfer_overwrite_handles_existing_identity_target() {
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "chiron_horizon_sqlserver_demo".to_string());
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let connection_id = format!("live-sqlserver-8690-{suffix}");
    let source_schema = format!("chiron_horizon_8690_src_{}", &suffix[..12]);
    let target_schema = format!("chiron_horizon_8690_dst_{}", &suffix[..12]);
    ...
    for statement in [
        ...
        chiron_horizon_core::db::sqlserver::execute_batch(&mut client, &statement)
            .await
            .expect("create issue #8690 fixtures");
    }

    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-sqlserver-8690-{suffix}"));
...
async fn live_sqlserver_keyset_pagination_copies_every_row() {
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_db = format!("chiron_horizon_keyset_src_{}", &suffix[..12]);
    let target_db = format!("chiron_horizon_keyset_dst_{}", &suffix[..12]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-sqlserver-keyset-{suffix}"));
    ...
    let test_result = async {
        ...
        let mut target_client = sqlserver_connect(&target_db).await;
        let rows =
            chiron_horizon_core::db::sqlserver::execute_query(&mut target_client, "SELECT id FROM dbo.big ORDER BY id")
                .await
...
async fn live_sqlserver_progress_read_survives_total_duration_beyond_timeout() {
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_db = format!("chiron_horizon_progress_src_{}", &suffix[..12]);
    let target_db = format!("chiron_horizon_progress_dst_{}", &suffix[..12]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-sqlserver-progress-{suffix}"));
    ...
    let test_result = async {
        ...
        let mut target_client = sqlserver_connect(&target_db).await;
        let count =
            chiron_horizon_core::db::sqlserver::execute_query(&mut target_client, "SELECT COUNT(*) FROM dbo.big")
                .await
...
async fn live_sqlserver_keyset_uniqueidentifier_datetime2_composite_key() {
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_db = format!("chiron_horizon_typed_src_{}", &suffix[..12]);
    let target_db = format!("chiron_horizon_typed_dst_{}", &suffix[..12]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-sqlserver-typed-{suffix}"));
