...
async fn plaintext_migration_preserves_ids_models_and_missing_keys_never_reinitialize() {
    ...
    if chiron_horizon_core::legacy::var_os("CHIRON_HORIZON_AI_MASTER_KEY").is_none() {
        #[cfg(unix)]
        {
...
async fn legacy_reference_migration_rebinds_in_backend_without_changing_selected_model() {
    let dir = tempfile::tempdir().unwrap();
    let storage = Storage::open(&dir.path().join("chiron-horizon.db")).await.unwrap();
...
fn chiron_preview_is_bounded_and_vectors_are_removed_recursively() {
    assert!(chiron_horizon_core::ai_chiron::validate_vector_sources("SEARCH demo NEAR [1,0,0];", "Find similar items")
        .is_err());
    assert!(
        chiron_horizon_core::ai_chiron::validate_vector_sources("SEARCH demo NEAR [1, 0, 0];", "Use [1,0,0]").is_ok()
    );
    assert!(
        chiron_horizon_core::ai_chiron::validate_vector_sources("SEARCH demo NEAR @first;", "Use point first").is_ok()
    );
    assert!(chiron_horizon_core::ai_chiron::validate_vector_sources(
        "SEARCH demo NEAR @unknown;",
        "Find similar items"
    )
    .is_err());
    let rows: Vec<_> =
        (0..30).map(|i| json!({"id":i,"vector":[0.1,0.2],"payload":{"vectors":[1,2],"field":"hello"}})).collect();
    let preview = chiron_horizon_core::ai_chiron::sharing_preview(&json!({"rows":rows}));
    ...
    let huge = chiron_horizon_core::ai_chiron::sharing_preview(&json!({"rows":[{"text":"x".repeat(17000)}]}));
    assert_eq!(huge, "[]");
    assert!(chiron_horizon_core::ai_chiron::proposal(
        "```chironql\nGET c POINTS 'x';\n```\n```chironql\nDROP COLLECTION c;\n```"
    )
    .is_err());
    assert!(chiron_horizon_core::ai_chiron::proposal("Which point ID?").unwrap().is_none());
}

#[test]
fn native_query_commentary_does_not_change_the_proposal() {
    use chiron_horizon_core::ai_chiron::{proposal, proposal_commentary};
    let answer =
