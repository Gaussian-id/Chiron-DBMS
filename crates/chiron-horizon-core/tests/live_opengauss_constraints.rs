...
//!   CHIRON_HORIZON_TEST_OPENGAUSS_HOST=127.0.0.1 \
//!   CHIRON_HORIZON_TEST_OPENGAUSS_PORT=5432 \
//!   CHIRON_HORIZON_TEST_OPENGAUSS_USER=test \
//!   CHIRON_HORIZON_TEST_OPENGAUSS_PASSWORD=secret \
//!   CHIRON_HORIZON_TEST_OPENGAUSS_DATABASE=postgres \
//!   cargo test -p chiron-horizon-core --test live_opengauss_constraints -- --ignored --nocapture

use std::time::Duration;

use chiron_horizon_core::db::postgres;
use chiron_horizon_core::types::ConstraintInfo;
...
fn opengauss_url() -> String {
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_HOST")
        .expect("CHIRON_HORIZON_TEST_OPENGAUSS_HOST not set");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_PORT")
        .expect("CHIRON_HORIZON_TEST_OPENGAUSS_PORT not set");
    let user = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_USER")
        .expect("CHIRON_HORIZON_TEST_OPENGAUSS_USER not set");
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_PASSWORD")
        .expect("CHIRON_HORIZON_TEST_OPENGAUSS_PASSWORD not set");
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_DATABASE")
        .unwrap_or_else(|_| "postgres".to_string());
    let params = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_OPENGAUSS_URL_PARAMS")
...
#[tokio::test]
#[ignore = "requires a writable OpenGauss instance via CHIRON_HORIZON_TEST_OPENGAUSS_* variables"]
async fn opengauss_constraints_report_structural_metadata() {
    ...
    let schema = format!("chiron_horizon_og_constraints_{}", std::process::id());
