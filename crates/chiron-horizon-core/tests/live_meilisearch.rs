use chiron_horizon_core::db::meilisearch_driver::{
...
fn live_client() -> MeilisearchClient {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MEILISEARCH_URL")
        .expect("CHIRON_HORIZON_LIVE_MEILISEARCH_URL");
    let api_key = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MEILISEARCH_API_KEY").ok();
...
fn unique_index() -> String {
    ...
    format!("chiron_horizon_live_{nanos}")
}
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MEILISEARCH_URL and optional CHIRON_HORIZON_LIVE_MEILISEARCH_API_KEY"]
    ...
    let affected = save_document_batch(
        ...
        &[
            MeilisearchDocumentUpdate {
                id: "__chiron_horizon_meilisearch_string_id__\"001\"".to_string(),
