...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_TEST_POSTGRES_GEOMETRY_URL pointing at a PostGIS database such as local-pg/test_geometry"]
async fn live_postgres_geometry_values_render_as_text_in_query_results() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_POSTGRES_GEOMETRY_URL")
        .expect("CHIRON_HORIZON_TEST_POSTGRES_GEOMETRY_URL");
    let pool = chiron_horizon_core::db::postgres::connect(&url, Duration::from_secs(5)).await.expect("connect");
    let result = chiron_horizon_core::db::postgres::execute_query(
        &pool,
