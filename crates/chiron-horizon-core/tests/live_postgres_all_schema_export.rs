...
use chiron_horizon_core::database_export::{export_database_sql_core, DatabaseExportRequest, ExportStatus};
...
fn live_postgres_config(id: &str, database: &str) -> ConnectionConfig {
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_POSTGRES_EXPORT_HOST")
        .expect("CHIRON_HORIZON_LIVE_POSTGRES_EXPORT_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_POSTGRES_EXPORT_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(5432);
    let username = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_POSTGRES_EXPORT_USER")
        .expect("CHIRON_HORIZON_LIVE_POSTGRES_EXPORT_USER");
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_POSTGRES_EXPORT_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_POSTGRES_EXPORT_PASSWORD");
...
async fn live_postgres_all_schema_export_restores_one_sql_file() {
    ...
    let source_database = format!("chiron_horizon_issue8336_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_issue8336_dst_{}", &suffix[..12]);
