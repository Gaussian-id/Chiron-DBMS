...
//
// Run with:
//   CHIRON_HORIZON_TEST_DORIS_HOST=127.0.0.1 CHIRON_HORIZON_TEST_DORIS_PORT=9030 CHIRON_HORIZON_TEST_DORIS_USER=root \
//   CHIRON_HORIZON_TEST_DORIS_PASSWORD= cargo test -p chiron-horizon-core --test live_doris -- --ignored
...
fn live_doris_config(id: &str) -> ConnectionConfig {
    let host =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_DORIS_HOST").expect("CHIRON_HORIZON_TEST_DORIS_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_DORIS_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(9030);
    let username =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_DORIS_USER").expect("CHIRON_HORIZON_TEST_DORIS_USER");
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_DORIS_PASSWORD").unwrap_or_default();
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_TEST_DORIS_HOST/CHIRON_HORIZON_TEST_DORIS_PORT/CHIRON_HORIZON_TEST_DORIS_USER pointing at a writable Apache Doris 2.1.8 cluster"]
async fn doris_qualified_table_metadata_resolves_schema_database_with_comments() {
    ...
    Queryable::query_drop(&mut conn, "CREATE DATABASE IF NOT EXISTS chiron-horizon6590_test")
        .await
        .expect("create Doris test database");
    Queryable::query_drop(
        &mut conn,
        "CREATE TABLE IF NOT EXISTS chiron-horizon6590_test.chiron_horizon_comment_test (\
    ...
    let columns =
        get_columns_core(&state, "doris-6590-live", "", "chiron-horizon6590_test", "chiron_horizon_comment_test")
            .await
