...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_POSTGRES_HOST/PORT/USER/PASSWORD/DATABASE pointing at a writable PostgreSQL database"]
async fn live_postgres_query_result_export_uses_single_streamed_query() {
    ...
    let schema = format!("chiron_horizon_query_export_{}", &suffix[..8]);
    let setup = vec![
...
async fn live_postgres_query_result_xlsx_preserves_temporal_cell_types() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("chiron_horizon_xlsx_temporal_{}", &suffix[..8]);
    let setup = vec![
        format!("CREATE SCHEMA \"{schema}\""),
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_POSTGRES_HOST/PORT/USER/PASSWORD/DATABASE pointing at PostgreSQL 16.3"]
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_POSTGRES_* env vars for temporary-table CSV/XLSX export"]
async fn live_postgres_truncated_batch_result_export_replays_safe_temp_setup() {
    ...
    let t1 = format!("chiron_horizon_temp_t1_{short_suffix}");
    let t2 = format!("chiron_horizon_temp_t2_{short_suffix}");
    let t5 = format!("chiron_horizon_temp_t5_{short_suffix}");
    let setup_sql = vec![
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_POSTGRES_* env vars for a 650,000-row XLSX export"]
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_POSTGRES_* env vars"]
