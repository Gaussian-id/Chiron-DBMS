...
#[tokio::test]
#[ignore = "requires PostgreSQL URLs via CHIRON_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL and CHIRON_HORIZON_LIVE_PG_TRANSFER_TARGET_URL"]
async fn live_postgres_transfer_upserts_generated_always_identity_values() {
    let source_url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL")
        .expect("CHIRON_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL");
    let target_url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_PG_TRANSFER_TARGET_URL")
    ...
    let source_schema = format!("chiron_horizon_src_always_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_always_{}", &suffix[..8]);
    let cleanup_sql = [
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-always-transfer-{suffix}"));
...
#[tokio::test]
#[ignore = "requires source/target PostgreSQL URLs via CHIRON_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL and CHIRON_HORIZON_LIVE_PG_TRANSFER_TARGET_URL"]
async fn live_postgres_structure_only_preserves_table_indexes() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("chiron_horizon_src_structure_only_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_structure_only_{}", &suffix[..8]);
    let cleanup_sql = [
        format!("DROP SCHEMA IF EXISTS \"{}\" CASCADE", source_schema),
        format!("DROP SCHEMA IF EXISTS \"{}\" CASCADE", target_schema),
    ];
    ...
    let dir =
        std::env::temp_dir().join(format!("chiron-horizon-live-structure-only-transfer-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    ...
    state
        ...
        .await;
    request.content = chiron_horizon_core::transfer::TransferContent::StructureOnly;
    transfer_postgres_schema_dependencies(&state, &request, &source_pool_key, &target_pool_key, |_| {}).await.unwrap();
...
async fn live_postgres_transfer_preserves_data_and_schema_objects() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("chiron_horizon_src_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_{}", &suffix[..8]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-transfer-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
...
async fn live_postgres_transfer_skips_create_ddl_for_existing_target_table() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("chiron_horizon_src_existing_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_existing_{}", &suffix[..8]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-existing-transfer-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
...
async fn live_postgres_transfer_creates_selected_sequence_before_referencing_table() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("chiron_horizon_src_sequence_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_sequence_{}", &suffix[..8]);
    let cleanup_sql = [
        format!("DROP SCHEMA IF EXISTS \"{}\" CASCADE", source_schema),
        format!("DROP SCHEMA IF EXISTS \"{}\" CASCADE", target_schema),
    ];
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-sequence-transfer-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
...
async fn live_postgres_transfer_drop_target_rebuilds_structure_and_indexes() {
    ...
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL")
        .expect("CHIRON_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL");
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-drop-rebuild-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
    ...
    assert!(backup_name.starts_with("orders__chiron_horizon_bak_"), "unexpected backup name: {backup_name}");
    ...
    assert!(
        backup_indexes.rows.iter().all(|row| {
            ...
            idx_name.contains("__chiron_horizon_bak_") || idx_name.ends_with("_pkey")
        }),
        "backup indexes should be renamed with __chiron_horizon_bak_ suffix, got: {:?}",
        backup_indexes.rows
    );
...
impl PostgresRebuildFixture {
    async fn new(label: &str, tables: &[&str]) -> Self {
        ...
        let suffix = uuid::Uuid::new_v4().simple().to_string();
        let source_schema = format!("chiron_horizon_{label}_src_{}", &suffix[..8]);
        let target_schema = format!("chiron_horizon_{label}_dst_{}", &suffix[..8]);
...
#[tokio::test]
#[ignore = "requires disposable PostgreSQL URLs via CHIRON_HORIZON_LIVE_PG_TRANSFER_SOURCE_URL and CHIRON_HORIZON_LIVE_PG_TRANSFER_TARGET_URL"]
...
async fn live_postgres_keyset_pagination_copies_every_row() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("chiron_horizon_src_keyset_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_keyset_{}", &suffix[..8]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-pg-keyset-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
...
async fn live_postgres_progress_read_survives_total_duration_beyond_timeout() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("chiron_horizon_src_progress_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_progress_{}", &suffix[..8]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-pg-progress-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
...
async fn live_postgres_keyset_large_batch_copies_every_row() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_schema = format!("chiron_horizon_src_largebatch_{}", &suffix[..8]);
    let target_schema = format!("chiron_horizon_dst_largebatch_{}", &suffix[..8]);
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-pg-largebatch-{suffix}"));
    std::fs::create_dir_all(&dir).unwrap();
    let storage = Storage::open(&dir.join("storage.db")).await.unwrap();
    let state = Arc::new(AppState::new(storage));
