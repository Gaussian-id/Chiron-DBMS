...
fn live_mysql_config(id: &str) -> ConnectionConfig {
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_HOST")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(3306);
    let username = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_USER")
        .unwrap_or_else(|_| "root".to_string());
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_PASSWORD");
...
fn live_cross_version_mysql_config(id: &str, prefix: &str) -> ConnectionConfig {
    let host = chiron_horizon_core::legacy::var(format!("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_HOST"))
        .unwrap_or_else(|_| panic!("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_HOST"));
    let port = chiron_horizon_core::legacy::var(format!("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_PORT"))
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(3306);
    let username = chiron_horizon_core::legacy::var(format!("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_USER"))
        .unwrap_or_else(|_| "root".to_string());
    let password = chiron_horizon_core::legacy::var(format!("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_PASSWORD"))
        .unwrap_or_else(|_| panic!("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_{prefix}_PASSWORD"));
...
#[test]
#[ignore = "requires disposable MySQL 8.0.33 source and 8.0.45 target endpoints via CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_SOURCE_*/TARGET_* variables"]
...
async fn run_live_mysql_cross_version_transfer_completes_on_small_stack() {
    ...
    let source_database = format!("chiron_horizon_stack_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_stack_dst_{}", &suffix[..12]);
    ...
    let task_tmp = std::path::PathBuf::from(
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_TMP_DIR")
            .expect("CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_TMP_DIR"),
    );
...
#[tokio::test]
#[ignore = "requires disposable MySQL 8 endpoints via CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_* variables"]
async fn live_mysql_transfer_keeps_columns_whose_comment_mentions_foreign_key() {
    ...
    let source_database = format!("chiron_horizon_7660_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_7660_dst_{}", &suffix[..12]);
    ...
    let test_result = async {
        ...
        for (_, statement) in &pending_fk_alters {
            chiron_horizon_core::transfer::execute_on_pool_with_max_rows(&state, &target_pool_key, statement, None)
                .await
                .unwrap();
        }
...
#[tokio::test]
#[ignore = "requires disposable MySQL 8 source and 5.7 target endpoints via CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_SOURCE_*/TARGET_* variables"]
async fn live_mysql_transfer_downgrades_unsupported_source_collations() {
    ...
    let source_database = format!("chiron_horizon_6023_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_6023_dst_{}", &suffix[..12]);
    let modern_target_database = format!("chiron_horizon_6023_modern_{}", &suffix[..12]);
    let source_config = live_cross_version_mysql_config(&source_connection_id, "SOURCE");
    let target_config = live_cross_version_mysql_config(&target_connection_id, "TARGET");
    let source_setup_pool = mysql::connect(&mysql_url(&source_config), Duration::from_secs(10)).await.unwrap();
    let target_setup_pool = mysql::connect(&mysql_url(&target_config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_transfer_preserves_spatial_values_and_modes() {
    ...
    let source_database = format!("chiron_horizon_transfer_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_transfer_dst_{}", &suffix[..12]);
...
async fn live_mysql_transfer_structure_overwrite_rejects_incompatible_target_columns() {
    ...
    let source_database = format!("chiron_horizon_transfer_struct_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_transfer_struct_dst_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_transfer_structure_only_rejects_incompatible_target_columns() {
    ...
    let source_database = format!("chiron_horizon_transfer_structonly_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_transfer_structonly_dst_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_transfer_drop_target_parent_child_foreign_key() {
    ...
    let source_database = format!("chiron_horizon_parent_child_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_parent_child_dst_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
...
#[tokio::test]
#[ignore = "requires live MySQL connection via CHIRON_HORIZON_LIVE_MYSQL_TRANSFER_* env vars"]
async fn live_mysql_transfer_drop_target_rebuilds_incompatible_structure() {
    ...
    let source_database = format!("chiron_horizon_drop_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_drop_tgt_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_transfer_drop_target_rejects_external_incoming_fk() {
    ...
    let database = format!("chiron_horizon_ext_fk_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_transfer_drop_target_circular_foreign_keys() {
    ...
    let source_database = format!("chiron_horizon_circ_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_circ_tgt_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_transfer_drop_target_retains_backup_on_failure() {
    ...
    let source_database = format!("chiron_horizon_retain_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_retain_tgt_{}", &suffix[..12]);
    let config = live_mysql_config(&connection_id);
    let setup_pool = mysql::connect(&mysql_url(&config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_keyset_pagination_copies_every_row() {
    ...
    let source_database = format!("chiron_horizon_keyset_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_keyset_dst_{}", &suffix[..12]);
    let source_config = live_mysql_config(&source_connection_id);
    let target_config = live_mysql_config(&target_connection_id);
    let source_setup_pool = mysql::connect(&mysql_url(&source_config), Duration::from_secs(10)).await.unwrap();
    let target_setup_pool = mysql::connect(&mysql_url(&target_config), Duration::from_secs(10)).await.unwrap();
...
async fn live_mysql_progress_read_survives_total_duration_beyond_timeout() {
    ...
    let source_database = format!("chiron_horizon_progress_src_{}", &suffix[..12]);
    let target_database = format!("chiron_horizon_progress_dst_{}", &suffix[..12]);
    let source_config = live_mysql_config(&source_connection_id);
    let target_config = live_mysql_config(&target_connection_id);
    let source_setup_pool = mysql::connect(&mysql_url(&source_config), Duration::from_secs(10)).await.unwrap();
    let target_setup_pool = mysql::connect(&mysql_url(&target_config), Duration::from_secs(10)).await.unwrap();
