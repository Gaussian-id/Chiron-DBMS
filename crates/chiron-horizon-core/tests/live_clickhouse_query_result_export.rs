...
use chiron_horizon_core::table_import::parse_xlsx_file;
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_CLICKHOUSE_* env vars pointing at a ClickHouse HTTP endpoint"]
async fn live_clickhouse_query_result_export_xlsx_streams_random_order_query_once() {
    ...
    let dir = std::env::temp_dir().join(format!("chiron-horizon-live-clickhouse-query-export-{suffix}"));
