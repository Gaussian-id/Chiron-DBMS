...
async fn parses_socks5_proxy_credentials() {
    let options = ClientOptions::parse(
        "mongodb://localhost/?proxyHost=proxy.example.com&proxyUsername=chiron-horizon&proxyPassword=secret",
    )
    .await
    ...
    assert_eq!(proxy.authentication, Some(("chiron-horizon".to_string(), "secret".to_string())));
}
...
async fn leaves_direct_connections_without_a_proxy() {
    let options = ClientOptions::parse("mongodb://localhost:27017/chiron-horizon")
        .await
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MONGODB_SOCKS5_URL routed through a SOCKS5 proxy"]
async fn connects_to_mongodb_through_socks5_proxy() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_SOCKS5_URL")
        .expect("CHIRON_HORIZON_LIVE_MONGODB_SOCKS5_URL");
    ...
    if let Ok(expected) = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MONGODB_EXPECTED_VERSION") {
