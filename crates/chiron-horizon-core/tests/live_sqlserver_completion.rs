...
use chiron_horizon_core::table_structure_sql::{
    build_table_structure_change_sql, ColumnInfo, EditableStructureColumn, TableStructureSqlOptions,
};
use chiron_horizon_core::xlsx_export::{build_xlsx_workbook, XlsxWorksheetData};
...
fn live_sqlserver_config(id: &str, database: &str) -> chiron_horizon_core::models::connection::ConnectionConfig {
    chiron_horizon_core::models::connection::ConnectionConfig {
        ...
        host: chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        port: chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PORT")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(1433),
        username: chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_USER")
            .unwrap_or_else(|_| "sa".to_string()),
        password: chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD")
...
) -> Result<chiron_horizon_core::table_import::TableImportSummary, String> {
...
async fn run_live_sqlserver_generated_insert(
    client: &mut chiron_horizon_core::db::sqlserver::SqlServerClient,
    table: &str,
    file_path: &std::path::Path,
    ...
    if include_identity {
        ...
        chiron_horizon_core::db::sqlserver::execute_batch(client, &sql).await?;
        return Ok(rows_imported);
    }
    let mut rows_imported = 0;
    for batch in batches {
        chiron_horizon_core::db::sqlserver::execute_batch(client, &batch.sql).await?;
...
async fn live_sqlserver_matrix_rows(
    client: &mut chiron_horizon_core::db::sqlserver::SqlServerClient,
    table: &str,
) -> chiron_horizon_core::db::QueryResult {
    chiron_horizon_core::db::sqlserver::execute_query(
        client,
        &format!(
...
async fn live_sqlserver_table_count(
    ...
    let result =
        chiron_horizon_core::db::sqlserver::execute_query(client, &format!("SELECT COUNT_BIG(*) FROM [dbo].[{table}]"))
            .await
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD pointing at a writable SQL Server database"]
async fn live_sqlserver_bulk_matches_generated_insert_type_and_constraint_matrix() {
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let connection_id = format!("live-sqlserver-matrix-{suffix}");
    let bulk_table = format!("chiron_horizon_bulk_matrix_{suffix}");
    let generated_table = format!("chiron_horizon_insert_matrix_{suffix}");
    let bulk_audit = format!("chiron_horizon_bulk_matrix_audit_{suffix}");
    let generated_audit = format!("chiron_horizon_insert_matrix_audit_{suffix}");
    let bulk_trigger = format!("chiron_horizon_bulk_matrix_trigger_{suffix}");
    let generated_trigger = format!("chiron_horizon_insert_matrix_trigger_{suffix}");
    let mut client = chiron_horizon_core::db::sqlserver::connect(
        &chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PORT")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(1433),
        &chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_USER").unwrap_or_else(|_| "sa".to_string()),
        &chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD")
            .expect("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD"),
        Some(&database),
        None,
    ...
    for ddl in [
        ...
        chiron_horizon_core::db::sqlserver::execute_batch(&mut client, &ddl)
            .await
    ...
    let bulk_audit_rows = chiron_horizon_core::db::sqlserver::execute_query(
        &mut client,
        &format!("SELECT [target_id], [code], [default_text] FROM [dbo].[{bulk_audit}] ORDER BY [target_id]"),
    )
    .await
    .expect("query SQL Server bulk audit rows");
    let generated_audit_rows = chiron_horizon_core::db::sqlserver::execute_query(
        &mut client,
    ...
    let _ = chiron_horizon_core::db::sqlserver::execute_batch(&mut client, &cleanup).await;
...
async fn live_sqlserver_bulk_imports_zero_fraction_xlsx_numbers_into_bigint() {
    ...
    let table = format!("chiron_horizon_bulk_xlsx_integer_{suffix}");
    let mut client = chiron_horizon_core::db::sqlserver::connect(
        &chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PORT")
    ...
    .await
    .expect("connect SQL Server");
    chiron_horizon_core::db::sqlserver::execute_batch(
        &mut client,
    ...
    let rows = chiron_horizon_core::db::sqlserver::execute_query(
        &mut client,
    ...
    let _ = chiron_horizon_core::db::sqlserver::execute_batch(
        &mut client,
...
async fn live_sqlserver_table_import_bulk_preserves_target_semantics() {
    ...
    let table = format!("chiron_horizon_bulk_target_{suffix}");
    let audit_table = format!("chiron_horizon_bulk_audit_{suffix}");
    let trigger = format!("chiron_horizon_bulk_trigger_{suffix}");
    let mut setup_client = chiron_horizon_core::db::sqlserver::connect(
        &chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PORT")
            .ok()
    ...
    let audit_count = chiron_horizon_core::db::sqlserver::execute_query(
        &mut setup_client,
    ...
    let _ = chiron_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &cleanup).await;
    state.remove_connection_pools_detached(&connection_id).await;
    let _ = std::fs::remove_dir_all(&dir);
...
async fn live_sqlserver_table_import_bulk_cancels_before_target_write_and_rolls_back_truncate() {
    ...
    let table = format!("chiron_horizon_bulk_rollback_{suffix}");
    let mut setup_client = chiron_horizon_core::db::sqlserver::connect(
        &chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_HOST")
            .unwrap_or_else(|_| "127.0.0.1".to_string()),
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PORT")
    ...
    let rows_after_failed_truncate = chiron_horizon_core::db::sqlserver::execute_query(
        &mut setup_client,
    ...
    .await;
    let rows_after_cancel = chiron_horizon_core::db::sqlserver::execute_query(
        &mut setup_client,
        &format!("SELECT [id], CONVERT(VARCHAR(50), [amount]) FROM [dbo].[{table}]"),
    )
    .await
...
async fn live_sqlserver_column_metadata_marks_non_positional_insert_columns() {
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_HOST")
        .unwrap_or_else(|_| "127.0.0.1".to_string());
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PORT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(1433);
    let user =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_USER").unwrap_or_else(|_| "sa".to_string());
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD");
    let mut client = chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let computed_table = format!("chiron_horizon_insert_computed_{suffix}");
    let temporal_table = format!("chiron_horizon_insert_temporal_{suffix}");
    let history_table = format!("chiron_horizon_insert_history_{suffix}");
    ...
    chiron_horizon_core::db::sqlserver::execute_query(&mut client, &create_computed)
        .await
        .expect("create computed table");
    chiron_horizon_core::db::sqlserver::execute_query(&mut client, &create_temporal)
        .await
    ...
    let computed = chiron_horizon_core::db::sqlserver::get_column_metadata(&mut client, "dbo", &computed_table)
        .await
        .expect("read computed metadata");
    let temporal = chiron_horizon_core::db::sqlserver::get_column_metadata(&mut client, "dbo", &temporal_table)
        .await
...
async fn live_sqlserver_execute_query_creates_schema() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("chiron_horizon_schema_{suffix}");
    ...
    let result = chiron_horizon_core::db::sqlserver::execute_query(&mut client, &create).await;
    let verify_result = chiron_horizon_core::db::sqlserver::execute_query(&mut client, &verify).await;
    let schemas = chiron_horizon_core::db::sqlserver::list_schemas(&mut client).await;
    let _ = chiron_horizon_core::db::sqlserver::execute_query(&mut client, &cleanup).await;
...
async fn live_sqlserver_explicit_transaction_batch_can_rollback() {
    ...
    let table = format!("chiron_horizon_txn_{}", uuid::Uuid::new_v4().simple());
    chiron_horizon_core::db::sqlserver::execute_query(
        &mut client,
    ...
    let execution = chiron_horizon_core::db::sqlserver::execute_batch(
        &mut client,
        &format!("BEGIN TRANSACTION\nUPDATE dbo.[{table}] SET name = N'changed' WHERE id = 50;"),
    )
    .await;
    let transaction_count =
        chiron_horizon_core::db::sqlserver::execute_query(&mut client, "SELECT @@TRANCOUNT AS transaction_count").await;
    let rollback = chiron_horizon_core::db::sqlserver::execute_batch(&mut client, "ROLLBACK TRANSACTION").await;
    let verify = chiron_horizon_core::db::sqlserver::execute_query(
        &mut client,
        &format!("SELECT name FROM dbo.[{table}] WHERE id = 50"),
    )
    .await;
    let cleanup =
        chiron_horizon_core::db::sqlserver::execute_query(&mut client, &format!("DROP TABLE dbo.[{table}]")).await;
...
async fn live_sqlserver_table_structure_default_changes_drop_existing_constraints() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let schema = format!("chiron_horizon_default_{suffix}");
    let table = "products";
    ...
    chiron_horizon_core::db::sqlserver::execute_query(&mut client, &create_schema)
        .await
        .expect("create live test schema");
    chiron_horizon_core::db::sqlserver::execute_query(&mut client, &create_table)
        .await
    ...
    let execution_result = async {
        for statement in &result.statements {
            chiron_horizon_core::db::sqlserver::execute_query(&mut client, statement).await?;
        }
        Ok::<(), String>(())
    }
    .await;
    ...
    let verify_result = chiron_horizon_core::db::sqlserver::execute_query(&mut client, &verify_sql).await;
...
async fn live_sqlserver_stream_first_result_set_exports_cte_query_rows() {
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("chiron_horizon_stream_export_{suffix}");
    let setup = format!(
        "\
        CREATE TABLE [dbo].[{table}] (id INT NOT NULL, name NVARCHAR(64) NULL);\
        INSERT INTO [dbo].[{table}] (id, name) VALUES (2, N'beta'), (1, N'alpha');"
    );
    chiron_horizon_core::db::sqlserver::execute_batch(&mut client, &setup).await.expect("create live test rows");
    ...
    let result = chiron_horizon_core::db::sqlserver::stream_first_result_set(&mut client, &sql, None, None, |item| {
        match item {
            chiron_horizon_core::db::sqlserver::SqlServerStreamItem::Columns { columns: stream_columns, .. } => {
                columns = stream_columns.to_vec();
            }
            chiron_horizon_core::db::sqlserver::SqlServerStreamItem::Row(row) => {
...
async fn live_sqlserver_query_result_export_streams_cte_query_to_csv() {
    ...
    .await
    .expect("connect SQL Server");
    let export_client = chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("chiron_horizon_query_export_{suffix}");
    let setup = format!(
        "\
        CREATE TABLE [dbo].[{table}] (id INT NOT NULL, name NVARCHAR(64) NULL);\
        INSERT INTO [dbo].[{table}] (id, name) VALUES (2, N'beta'), (1, N'alpha');"
    );
    chiron_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &setup).await.expect("create live test rows");
...
async fn live_sqlserver_sql_file_import_executes_go_batches() {
    ...
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD");
    let client = chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("chiron_horizon_sql_file_{suffix}");
    ...
    let rows =
        chiron_horizon_core::db::sqlserver::execute_query(&mut client, &format!("EXEC [dbo].[{procedure}]")).await;
...
async fn live_sqlserver_transfer_table_skips_rowversion_insert_column() {
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let target_database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_TARGET_DATABASE")
        .unwrap_or_else(|_| "tempdb".to_string());
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_HOST")
        .unwrap_or_else(|_| "127.0.0.1".to_string());
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PORT")
    ...
    .await
    .expect("connect SQL Server");
    let mut target_client = chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let source_table = format!("chiron_horizon_rowversion_src_{suffix}");
    ...
    let _ = chiron_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &cleanup_source).await;
    let _ = chiron_horizon_core::db::sqlserver::execute_batch(&mut target_client, &cleanup_target).await;
    ...
    chiron_horizon_core::db::sqlserver::execute_batch(&mut setup_client, &setup)
        .await
    ...
    let request = chiron_horizon_core::transfer::TransferRequest {
        ...
        content: chiron_horizon_core::transfer::TransferContent::default(),
        objects: Vec::new(),
        mode: chiron_horizon_core::transfer::TransferMode::Append,
        target_table_name_case: chiron_horizon_core::transfer::TransferTableNameCase::Upper,
        quote_target_column_names: true,
        ownership_policy: chiron_horizon_core::transfer::TransferOwnershipPolicy::Preserve,
        batch_size: 100,
    };
    let result = chiron_horizon_core::transfer::transfer_table(
        &state,
        &request,
        &source_table,
        0,
    ...
    let verify_result = chiron_horizon_core::db::sqlserver::execute_query(&mut target_client, &verify_sql).await;
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_SQLSERVER_URL or CHIRON_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD pointing at a writable SQL Server database"]
async fn live_sqlserver_completion_assistant_searches_metadata_before_limiting() {
    ...
    chiron_horizon_core::db::sqlserver::execute_batch(&mut client, &setup).await.expect("create live test objects");

    let request = chiron_horizon_core::types::CompletionAssistantRequest {
        connection_id: "live-sqlserver".to_string(),
        database: database.clone(),
        schema: Some(schema.clone()),
        object_kinds: vec![chiron_horizon_core::types::CompletionAssistantObjectKind::Table],
        mask: prefix.clone(),
        ...
        parent_name: None,
        match_mode: Some(chiron_horizon_core::types::CompletionAssistantMatchMode::Prefix),
    };

    let response = chiron_horizon_core::db::sqlserver::completion_assistant_search(&mut client, &request)
        .await
    ...
    let column_response = chiron_horizon_core::db::sqlserver::completion_assistant_search(
        &mut client,
        &chiron_horizon_core::types::CompletionAssistantRequest {
            object_kinds: vec![chiron_horizon_core::types::CompletionAssistantObjectKind::Column],
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_SQLSERVER_HOST/PORT/USER/PASSWORD pointing at a writable SQL Server instance"]
async fn live_sqlserver_cross_database_metadata_and_query() {
    ...
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_SQLSERVER_PASSWORD");
    let default_database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQLSERVER_DATABASE")
        .unwrap_or_else(|_| "master".to_string());
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let database_a = format!("chiron_horizon_completion_a_{}", &suffix[..8]);
    let database_b = format!("chiron_horizon_completion_b_{}", &suffix[..8]);

    let mut master = chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let mut client_a = chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    let mut client_b = chiron_horizon_core::db::sqlserver::connect(
        &host,
        port,
        &user,
        &password,
    ...
    chiron_horizon_core::db::sqlserver::execute_batch(&mut client_a, "CREATE SCHEMA [IN]")
        .await
        .expect("create database A IN schema");
    chiron_horizon_core::db::sqlserver::execute_batch(&mut client_a, "CREATE SCHEMA [OUT]")
        .await
    ...
    chiron_horizon_core::db::sqlserver::execute_batch(&mut client_b, "CREATE SCHEMA [OUT]")
        .await
    ...
    let tables_a = chiron_horizon_core::schema::list_tables_core(
        &state,
        connection_id,
        &database_a,
        "IN",
    ...
    let tables_b = chiron_horizon_core::schema::list_tables_core(
        &state,
        connection_id,
        &database_b,
        "OUT",
    ...
    let columns_a = chiron_horizon_core::schema::get_columns_core(&state, connection_id, &database_a, "OUT", "orders")
        .await
        .expect("load database A columns");
    let columns_b = chiron_horizon_core::schema::get_columns_core(&state, connection_id, &database_b, "OUT", "orders")
        .await
    ...
    let result = chiron_horizon_core::query::execute_sql_statement(
        &state,
        connection_id,
        &default_database,
        &query,
    ...
    for database in [&database_a, &database_b] {
        ...
        chiron_horizon_core::db::sqlserver::execute_batch(&mut master, &cleanup)
            .await
