...
//!   CHIRON_HORIZON_TEST_POSTGRES_URL=postgresql://postgres@127.0.0.1:5432/postgres \
//!     cargo test -p chiron-horizon-core --no-default-features --test live_postgres_constraints -- --ignored --nocapture
...
async fn postgres_constraints_reports_pk_fk_unique_check_and_not_valid() {
    ...
    let schema = format!("chiron_horizon_constraints_{}", std::process::id());
...
async fn postgres_constraints_reports_exclude_constraints() {
    let url =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_TEST_POSTGRES_URL").expect("CHIRON_HORIZON_TEST_POSTGRES_URL");
    let pool = postgres::connect(&url, Duration::from_secs(5)).await.expect("connect postgres");
    let schema = format!("chiron_horizon_constraints_excl_{}", std::process::id());
    let schema_ident = format!("\"{}\"", schema.replace('"', "\"\""));
    let _ = postgres::execute_query(&pool, &format!("DROP SCHEMA IF EXISTS {schema_ident} CASCADE")).await;
