...
use chiron_horizon_core::redis_ops::{
...
/// `CHIRON_HORIZON_LIVE_REDIS_HOST=... CHIRON_HORIZON_LIVE_REDIS_PORT=... CHIRON_HORIZON_LIVE_REDIS_PASSWORD=... \
///  cargo test -p chiron-horizon-core --test live_redis_batch_expiry -- --ignored --nocapture`
async fn live_state(connection_id: &str) -> Arc<AppState> {
    let host =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_REDIS_HOST").expect("CHIRON_HORIZON_LIVE_REDIS_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_REDIS_PORT")
        .expect("CHIRON_HORIZON_LIVE_REDIS_PORT")
        .parse::<u16>()
        .expect("Redis port");
    let username = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_REDIS_USERNAME").unwrap_or_default();
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_REDIS_PASSWORD").unwrap_or_default();
...
fn unique_prefix() -> String {
    ...
    format!("chiron_horizon_8899_{nanos}")
}
...
fn key_raws(prefix: &str) -> Vec<String> {
    ...
    (0..3)
        .map(|index| {
            chiron_horizon_core::db::redis_driver::redis_key_bytes_to_raw(format!("{prefix}:{index}").as_bytes())
        })
        .collect()
}
...
async fn set_keys_ttl_reports_a_concurrently_deleted_key_without_failing_the_batch() {
    ...
    raws.push(chiron_horizon_core::db::redis_driver::redis_key_bytes_to_raw(format!("{prefix}:deleted").as_bytes()));
