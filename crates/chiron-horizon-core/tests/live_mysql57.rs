...
fn live_mysql_sql_file_config(id: &str) -> ConnectionConfig {
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_HOST")
        .expect("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(3306);
    let username = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_USER")
        .expect("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_USER");
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_PASSWORD");
...
#[tokio::test]
#[ignore = "requires the remote Chiron Horizon MySQL 5.7 smoke-test container"]
async fn live_mysql57_text_protocol_select_succeeds() {
    let url =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL57_URL").expect("CHIRON_HORIZON_LIVE_MYSQL57_URL");

    let pool = chiron_horizon_core::db::mysql::connect(&url, std::time::Duration::from_secs(5)).await.unwrap();
    let result = chiron_horizon_core::db::mysql::execute_query_with_max_rows(
        &pool,
...
async fn live_mysql57_checksum_table_returns_native_result_set() {
    ...
    let result = chiron_horizon_core::db::mysql::execute_query_with_max_rows(
        &pool,
        "CHECKSUM TABLE `chiron_horizon_smoke`",
        false,
        Some(10),
        Default::default(),
    )
    ...
    assert!(result.rows[0][0].as_str().is_some_and(|table| table.ends_with(".chiron_horizon_smoke")));
...
async fn live_mysql_stored_procedure_preserves_all_result_sets() {
    let url =
        chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL57_URL").expect("CHIRON_HORIZON_LIVE_MYSQL57_URL");
    let pool = chiron_horizon_core::db::mysql::connect(&url, std::time::Duration::from_secs(5)).await.unwrap();
    let procedure = format!("chiron_horizon_issue_4609_{}", uuid::Uuid::new_v4().simple());
    let mut conn = chiron_horizon_core::db::mysql::get_conn_with_health_check(&pool).await.unwrap();
    ...
    let results = chiron_horizon_core::db::mysql::execute_query_results_on_conn_with_max_rows(
        &mut conn,
...
async fn live_mysql_single_call_public_route_preserves_all_result_sets() {
    ...
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_DATABASE")
        .expect("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_DATABASE");
    let config = live_mysql_sql_file_config(&connection_id);
    let (state, db_path) = app_state_with_config(config).await;
    let procedure = format!("chiron_horizon_issue_5560_{}", &suffix[..8]);
    let empty_procedure = format!("chiron_horizon_issue_5560_empty_{}", &suffix[..8]);
    let slow_procedure = format!("chiron_horizon_issue_5560_slow_{}", &suffix[..8]);
    let table = format!("chiron_horizon_issue_5560_{}", &suffix[..8]);
    let session_id = format!("chiron-horizon-issue-5560-{suffix}");
    let create_sql = format!(
        "CREATE PROCEDURE `{procedure}`() BEGIN \
         SET @chiron_horizon_issue_5560_session = 'kept'; \
    ...
    execute_sql_statement(
        ...
        &connection_id,
        &database,
        &format!("CREATE PROCEDURE `{empty_procedure}`() BEGIN SET @chiron_horizon_issue_5560_empty = 1; END"),
        None,
        None,
    )
    .await
    ...
    let session_result = execute_multi_core_with_options_for_client_typed(
        ...
        &connection_id,
        &database,
        "SELECT @chiron_horizon_issue_5560_session AS session_value",
        None,
        None,
        options.clone(),
    )
    ...
    let sql_error = execute_multi_core_with_options_for_client_typed(
        ...
        &connection_id,
        &database,
        &format!("CALL `chiron_horizon_issue_5560_missing_{}`()", &suffix[..8]),
        None,
        None,
        options.clone(),
    )
...
async fn live_mysql_compatible_limited_text_protocol_query_succeeds() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_COMPAT_URL")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_COMPAT_URL");
    let sql = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_COMPAT_SQL")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_COMPAT_SQL");

    let pool = chiron_horizon_core::db::mysql::connect(&url, std::time::Duration::from_secs(10)).await.unwrap();
    let result =
        chiron_horizon_core::db::mysql::execute_query_with_max_rows(&pool, &sql, false, Some(100), Default::default())
            .await
            .unwrap();
...
async fn live_mysql_query_result_export_xlsx_streams_single_query_without_duplicate_batches() {
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_HOST")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_PORT")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_PORT")
        .parse::<u16>()
        .unwrap();
    let user = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_USER")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_USER");
    let password = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_PASSWORD")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_PASSWORD");
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_DATABASE")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_DATABASE");
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("chiron_horizon_query_export_{}", &suffix[..8]);
...
async fn live_mysql_csv_temporal_export_round_trip_preserves_chiron_horizon_force_text_values() {
    let host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_HOST")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_HOST");
    let port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_PORT")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_PORT")
    ...
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_DATABASE")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_DATABASE");
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("chiron_horizon_issue_8803_{}", &suffix[..8]);
    let target_table = format!("chiron_horizon_issue_8803_target_{}", &suffix[..8]);
    ...
    let parsed = parse_csv_bytes(&csv, 10).expect("Chiron Horizon should parse its own CSV");
    let mappings = parsed
        .columns
        .iter()
...
async fn live_mysql_xlsx_export_can_outlive_query_timeout_while_rows_keep_arriving() {
    ...
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_DATABASE")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_EXPORT_DATABASE");
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let table = format!("chiron_horizon_query_export_timeout_{}", &suffix[..8]);
...
async fn live_mysql_call_procedure_returns_select_result_set() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_PROCEDURE_URL")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_PROCEDURE_URL");

    let pool = chiron_horizon_core::db::mysql::connect(&url, std::time::Duration::from_secs(10)).await.unwrap();
    chiron_horizon_core::db::mysql::execute_query_with_max_rows(
        &pool,
...
async fn live_mysql_splitter_executes_routine_without_delimiter() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_PROCEDURE_URL")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_PROCEDURE_URL");
    let pool = chiron_horizon_core::db::mysql::connect(&url, std::time::Duration::from_secs(10)).await.unwrap();
    let procedure = format!("chiron_horizon_routine_range_{}", uuid::Uuid::new_v4().simple());
...
BEGIN
    SET @chiron_horizon_routine_range_value = CASE WHEN 1 = 1 THEN 2695 ELSE 0 END;
    SELECT @chiron_horizon_routine_range_value AS value;
END;
CALL {procedure}();
DROP PROCEDURE IF EXISTS {procedure};"
    );
    ...
    assert!(statements[1].contains("SELECT @chiron_horizon_routine_range_value AS value;"));
    ...
    .await;
    let cleanup = chiron_horizon_core::db::mysql::execute_query_with_max_rows(
        &pool,
...
async fn live_oceanbase_mysql_setup_applies_query_timeout() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_OCEANBASE_MYSQL_URL")
        .expect("CHIRON_HORIZON_LIVE_OCEANBASE_MYSQL_URL");
    let setup = vec!["SET ob_query_timeout = 30000000".to_string()];

    let pool = chiron_horizon_core::db::mysql::connect_bare_with_pool_limit_and_setup(
        &url,
...
async fn live_mysql_query_cancel_kills_running_sleep() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_CANCEL_URL")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_CANCEL_URL");
    ...
    let query = tokio::spawn(async move {
        chiron_horizon_core::db::mysql::execute_query_on_conn_with_max_rows(
            &mut conn,
            "SELECT SLEEP(30)",
            false,
            Some(10),
    ...
    chiron_horizon_core::db::mysql::kill_query_with_opts(kill_opts, connection_id).await.unwrap();
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_* env vars pointing at a writable MySQL connection"]
async fn live_mysql_conditional_update_timeout_cancels_before_reload() {
    ...
    let table = format!("chiron_horizon_conditional_update_{}", &suffix[..8]);
    let trigger = format!("chiron_horizon_conditional_update_delay_{}", &suffix[..8]);
...
async fn live_mysql_recovers_after_server_idle_disconnect() {
    let url = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_MYSQL_IDLE_URL")
        .expect("CHIRON_HORIZON_LIVE_MYSQL_IDLE_URL");

    let pool = chiron_horizon_core::db::mysql::connect_with_ca_cert_and_pool_limit(
        &url,
        None,
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_* env vars pointing at a writable MySQL connection without a required default database"]
async fn live_mysql_multi_statement_stops_after_first_error() {
    ...
    let database_name = format!("chiron_horizon_issue_2783_{suffix}");
...
async fn live_mysql_pipelined_dml_reports_each_statement_before_the_next_finishes() {
    ...
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_DATABASE")
        .expect("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_DATABASE");
    let (state, db_path) = app_state_with_config(config).await;
    let table_name = format!("chiron_horizon_batch_progress_{}", &suffix[..8]);
...
async fn live_mysql_pipelined_dml_keeps_completed_results_before_error() {
    ...
    let database = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_DATABASE")
        .expect("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_DATABASE");
    let (state, db_path) = app_state_with_config(config).await;
    let table_name = format!("chiron_horizon_batch_error_{}", &suffix[..8]);
...
async fn live_sql_file_import_creates_database_and_switches_context_without_default_database() {
    ...
    let (state, db_path) = app_state_with_config(config.clone()).await;
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    let database_name = format!("chiron_horizon_issue_2356_{suffix}");
    let script = format!(
        r#"
...
async fn live_sql_file_import_preserves_last_insert_id_across_adjacent_inserts() {
    ...
    let config = live_mysql_sql_file_config(&connection_id);
    let (state, db_path) = app_state_with_config(config.clone()).await;
    let database_name = format!("chiron_horizon_issue_7738_{suffix}");
    let script = format!(
        r#"
CREATE DATABASE `{database_name}`;
USE `{database_name}`;
...
async fn live_sql_file_import_preserves_raw_mysql_binary_literal_bytes() {
    ...
    let (state, db_path) = app_state_with_config(config.clone()).await;
    let database_name = chiron_horizon_core::legacy::var("CHIRON_HORIZON_LIVE_SQL_FILE_MYSQL_DATABASE")
        .unwrap_or_else(|_| "chiron_horizon_test".to_string());
