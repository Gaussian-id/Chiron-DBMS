...
async fn annotations_reach_the_generated_dbml() {
  ...
  "tables": {{
    ...
    let notes_path = std::env::temp_dir().join(format!("chiron-horizon-live-docs-notes-{}.json", uuid::Uuid::new_v4()));
    ...
    let annotations_result = chiron_horizon_core::docs::annotations::load_annotations(&notes_path);
    ...
    let dbml = chiron_horizon_core::docs::to_dbml(&snapshot);
    println!("{}", dbml.text);
