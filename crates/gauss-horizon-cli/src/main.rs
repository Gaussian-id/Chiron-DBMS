...
use gauss_horizon_mcp::{
    ...
    GaussHorizonBackend, LocalBackend, WebBackend,
};
use serde::Serialize;
...
async fn run(argv: Vec<String>) -> Result<String, (CliError, bool)> {
    ...
    if flags.args[0] == "doctor" {
        ensure_arg_count(&flags.args, 1, "gauss-horizon doctor").map_err(|error| (error, json_output))?;
    ...
    if flags.args[0] == "capabilities" {
        ensure_arg_count(&flags.args, 1, "gauss-horizon capabilities").map_err(|error| (error, json_output))?;
    ...
    let backend: Arc<dyn GaussHorizonBackend> = if let Ok(base_url) = gauss_horizon_core::legacy::var("GAUSS_HORIZON_WEB_URL") {
        ...
        let db_path = gauss_horizon_mcp::paths::storage_db_path()
            .map_err(|message| (CliError::new("CONNECTION_STORE_ERROR", message), json_output))?;
        Arc::new(
...
async fn run_with_backend(backend: &dyn GaussHorizonBackend, flags: Flags) -> Result<String, CliError> {
    let args = &flags.args;
    if args.first().is_some_and(|arg| arg == "connections") && args.get(1).is_some_and(|arg| arg == "list") {
        ensure_arg_count(args, 2, "gauss-horizon connections list")?;
    ...
    if args.first().is_some_and(|arg| arg == "schema") && args.get(1).is_some_and(|arg| arg == "list") {
        ensure_arg_count(args, 3, "gauss-horizon schema list")?;
    ...
    if args.first().is_some_and(|arg| arg == "schema") && args.get(1).is_some_and(|arg| arg == "describe") {
        ensure_arg_count(args, 4, "gauss-horizon schema describe")?;
        let connection_name = required(args.get(2), "Connection name is required.")?;
    ...
    if args.first().is_some_and(|arg| arg == "open") {
        ensure_arg_count(args, 3, "gauss-horizon open")?;
        ...
        if flags.format == OutputFormat::Csv {
            return Err(CliError::new("INVALID_OPTION", "CSV format is not supported for gauss-horizon open."));
        }
        backend
            .bridge_request(
                "/open-table",
            ...
            .await
            .map_err(|message| CliError::new("GAUSS_HORIZON_NOT_RUNNING", message))?;
        ...
        return Ok(format!("Opened {table} in Gauss Horizon\n"));
    }
    Err(CliError::new("USAGE", usage()))
}

async fn run_query(backend: &dyn GaussHorizonBackend, flags: &Flags) -> Result<String, CliError> {
    let args = &flags.args;
    let default_connection = gauss_horizon_core::legacy::var("GAUSS_HORIZON_CONNECTION").ok().filter(|value| !value.is_empty());
    ...
    let connection = find_connection(backend, connection_name).await?;
    let env_allow_writes = env_flag("GAUSS_HORIZON_MCP_ALLOW_WRITES");
    let env_allow_dangerous = env_flag("GAUSS_HORIZON_MCP_ALLOW_DANGEROUS_SQL");
    ...
    if connection.db_type == DatabaseType::Redis {
        return Err(CliError::new(
            ...
            "Redis connections do not accept SQL through gauss-horizon query. Use an MCP Redis command tool or Gauss Horizon directly.",
        ));
    }
...
async fn run_context(backend: &dyn GaussHorizonBackend, flags: &Flags) -> Result<String, CliError> {
    let args = &flags.args;
    let default_connection = gauss_horizon_core::legacy::var("GAUSS_HORIZON_CONNECTION").ok().filter(|value| !value.is_empty());
    let uses_default = default_connection.is_some() && args.len() == 1;
    ensure_arg_count(args, if uses_default { 1 } else { 2 }, "gauss-horizon context")?;
    if flags.format == OutputFormat::Csv {
        return Err(CliError::new("INVALID_OPTION", "CSV format is not supported for gauss-horizon context."));
    }
    let connection_name = if uses_default {
        default_connection.as_deref().unwrap()
    } else {
...
async fn run_dbml(backend: &dyn GaussHorizonBackend, flags: &Flags) -> Result<String, CliError> {
    let args = &flags.args;
    ...
    if let Some(path) = flags.notes.as_ref() {
        ...
        if let Some(annotations) = gauss_horizon_core::docs::annotations::load_annotations(path)
            .map_err(|error| CliError::new("NOTES_INVALID", error))?
        {
            gauss_horizon_core::docs::annotations::apply_annotations(&mut snapshot, &annotations, connection.db_type);
        }
    }

    let output = gauss_horizon_core::docs::to_dbml(&snapshot);
...
async fn run_docs(backend: &dyn GaussHorizonBackend, flags: &Flags) -> Result<String, CliError> {
    let args = &flags.args;
    let connection_name = required(args.get(1), "Connection name is required.")?;
    let connection = find_connection(backend, connection_name).await?;
    let database = selected_database(&connection, flags.database.as_deref());
    ...
    let mut annotations = gauss_horizon_core::docs::annotations::AnnotationFile {
        format_version: 1,
        project: None,
        groups: Vec::new(),
    ...
    if let Some(path) = flags.notes.as_ref() {
        require_notes_file(path)?;
        if let Some(loaded) = gauss_horizon_core::docs::annotations::load_annotations(path)
            .map_err(|error| CliError::new("NOTES_INVALID", error))?
        {
            gauss_horizon_core::docs::annotations::apply_annotations(&mut snapshot, &loaded, connection.db_type);
            annotations = loaded;
        }
    }
    ...
    let html = gauss_horizon_core::docs::to_standalone_html(&snapshot, &annotations, lang)
...
async fn find_connection(backend: &dyn GaussHorizonBackend, name: &str) -> Result<ConnectionConfig, CliError> {
    backend
        .load_connections()
        .await
        .map_err(store_error)?
...
fn env_flag(name: &str) -> bool {
    gauss_horizon_core::legacy::var(name).ok().is_some_and(|value| matches!(value.trim().to_ascii_lowercase().as_str(), "1" | "true"))
}
    ...
    match format {
        ...
        OutputFormat::Table => {
            let rows = vec![
                ...
                json!({ "mode": "Requires Gauss Horizon Desktop", "types": BRIDGE_REQUIRED_TYPES.join(", ") }),
            ];
...
async fn diagnostics() -> Diagnostics {
    let app_data_dir = gauss_horizon_mcp::paths::app_data_dir().unwrap_or_default();
    let db_path = app_data_dir.join(gauss_horizon_mcp::paths::STORAGE_DB_FILE_NAME);
    ...
    let loaded = if db_path_exists {
        ...
        Err("Gauss Horizon database does not exist.".to_string())
    };
...
fn format_diagnostics(value: &Diagnostics, format: OutputFormat) -> Result<String, CliError> {
    ...
    let rows = vec![
        ...
        json!({ "check": "Gauss Horizon database", "value": if value.db_path_exists { format!("found ({})", value.db_path) } else { format!("missing ({})", value.db_path) } }),
...
fn usage() -> &'static str {
    "Usage:\n  gauss-horizon doctor [--json]\n  gauss-horizon capabilities [--json]\n  gauss-horizon connections list [--json]\n  gauss-horizon schema list <connection> [--schema name] [--json]\n  gauss-horizon schema describe <connection> <table> [--schema name] [--json]\n  gauss-horizon query <connection> <sql> [--file path] [--limit n] [--timeout 10s] [--allow-writes] [--allow-dangerous-sql] [--json]\n  gauss-horizon context <connection> [--schema name] [--tables a,b] [--max-tables n] [--json]\n  gauss-horizon dbml <connection> [--out path] [--notes path] [--schema name] [--database name] [--tables a,b]\n  gauss-horizon docs <connection> [--out path] [--notes path] [--lang code] [--schema name] [--database name] [--tables a,b]\n  gauss-horizon open <connection> <table> [--schema name] [--database name] [--json]"
}
...
mod tests {
    ...
    use gauss_horizon_mcp::{backend::new_connection_config, mongo::MongoCommand};
    ...
    #[async_trait]
    impl GaussHorizonBackend for MongoBackend {
    ...
    #[tokio::test]
    #[ignore = "requires GAUSS_HORIZON_MCP_TEST_MONGO_HOST and GAUSS_HORIZON_MCP_TEST_MONGO_PASSWORD"]
    async fn executes_legacy_mongo_insert_without_desktop_process() {
        let host = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_HOST").expect("MongoDB host");
        let port = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_PORT")
            .unwrap_or_else(|_| "27017".to_string())
            .parse::<u16>()
            .expect("MongoDB port");
        let password = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MCP_TEST_MONGO_PASSWORD").expect("MongoDB password");
        let directory = tempfile::tempdir().expect("temporary data directory");
        let db_path = directory.path().join("gauss-horizon.db");
        ...
        let mut connection = new_connection_config(
            ...
            "root".to_string(),
            password,
            Some("gauss_horizon_mcp_test".to_string()),
            false,
            None,
        )
        .unwrap();
    ...
    fn parses_the_notes_flag() {
        let flags = parse_flags(&args(&["dbml", "local", "--notes", "docs/gauss-horizon-docs.json"])).expect("parse");
        assert_eq!(flags.args, args(&["dbml", "local"]));
        assert_eq!(flags.notes.as_deref(), Some(std::path::Path::new("docs/gauss-horizon-docs.json")));
    }
    ...
    fn dbml_appears_in_the_usage_text() {
        assert!(usage().contains("gauss-horizon dbml <connection>"), "got: {}", usage());
    }
    ...
    fn docs_appears_in_the_usage_text() {
        assert!(usage().contains("gauss-horizon docs <connection>"), "got: {}", usage());
    }
}
