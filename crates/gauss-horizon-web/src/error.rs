...
mod tests {
    ...
    fn legacy_errors_are_exposed_as_the_shared_backend_envelope() {
        ...
        assert_eq!(error.error.code(), "Gauss Horizon-LEGACY-0001");
    ...
    fn structured_agent_text_keeps_catalog_identity_at_http_boundary() {
        let error = AppError::from(
            "Agent RPC error (-1): timed out\nGAUSS_HORIZON_AGENT_ERROR_DATA:{\"category\":\"timeout\",\"stage\":\"execute\"}",
        );
        assert_eq!(error.error.code(), "Gauss Horizon-JDBC-9001");
        assert_eq!(error.error.source(), gauss_horizon_core::backend_error::BackendErrorSource::JdbcAgentLegacy);
    }
    ...
    async fn http_response_preserves_original_legacy_detail() {
        ...
        assert_eq!(payload["code"], "Gauss Horizon-LEGACY-0001");
    ...
    async fn http_response_preserves_original_structured_agent_detail() {
        use gauss_horizon_core::db::agent_driver::{
