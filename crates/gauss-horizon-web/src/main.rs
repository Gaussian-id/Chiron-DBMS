...
use gauss_horizon_mcp::{streamable_http_router, GaussHorizonBackend, HttpAuth, LocalBackend};
use state::WebState;
...
#[derive(OpenApi)]
#[openapi(
    info(title = "Gauss Horizon Data Grid Extractor API", description = "HTTP contract for data-grid clipboard extraction."),
...
fn web_body_limit_bytes() -> usize {
    let value = gauss_horizon_core::legacy::var("GAUSS_HORIZON_MAX_UPLOAD_MB").ok();
...
fn web_agent_dir(data_dir: &std::path::Path) -> std::path::PathBuf {
    web_agent_dir_from_env(data_dir, gauss_horizon_core::legacy::var("GAUSS_HORIZON_AGENT_DIR").ok())
}
...
fn normalize_public_base_path(value: Option<String>) -> String {
    ...
    if trimmed.chars().any(|ch| ch.is_ascii_control() || ch.is_ascii_whitespace() || matches!(ch, ';' | ',')) {
        panic!("GAUSS_HORIZON_PUBLIC_BASE_PATH contains invalid characters");
    }
    if trimmed.is_empty() {
        "/".to_string()
    } else {
...
/// Gauss Horizon Web itself was started.
fn web_mcp_router(web_state: &Arc<WebState>) -> Result<Option<Router>, String> {
    ...
    let allowed_hosts = comma_separated_env("GAUSS_HORIZON_WEB_MCP_ALLOWED_HOSTS");
    if allowed_hosts.is_empty() {
        return Err("GAUSS_HORIZON_WEB_MCP_ALLOWED_HOSTS is required when Gauss Horizon Web MCP is enabled".into());
    }

    let allowed_origins = comma_separated_env("GAUSS_HORIZON_WEB_MCP_ALLOWED_ORIGINS");
...
fn web_mcp_token() -> Result<Option<String>, String> {
    let inline_token = gauss_horizon_core::legacy::var("GAUSS_HORIZON_WEB_MCP_TOKEN").ok();
    let token_file = gauss_horizon_core::legacy::var("GAUSS_HORIZON_WEB_MCP_TOKEN_FILE").ok();
    match (inline_token, token_file) {
        (Some(_), Some(_)) => {
            Err("set only one of GAUSS_HORIZON_WEB_MCP_TOKEN or GAUSS_HORIZON_WEB_MCP_TOKEN_FILE".into())
        }
        (Some(token), None) if !token.trim().is_empty() => Ok(Some(token)),
        (Some(_), None) => Err("GAUSS_HORIZON_WEB_MCP_TOKEN must not be empty".into()),
        (None, Some(path)) => std::fs::read_to_string(&path)
            .map_err(|error| format!("failed to read GAUSS_HORIZON_WEB_MCP_TOKEN_FILE: {error}"))
            .map(|token| token.trim_end_matches(['\r', '\n']).to_owned())
            .and_then(|token| {
                (!token.is_empty()).then_some(token).ok_or_else(|| "GAUSS_HORIZON_WEB_MCP_TOKEN_FILE is empty".into())
            })
            .map(Some),
        (None, None) => Ok(None),
    }
...
fn comma_separated_env(name: &str) -> Vec<String> {
    gauss_horizon_core::legacy::var(name)
        .ok()
        .into_iter()
        .flat_map(|value| {
...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "gauss_horizon_web=info,tower_http=info".parse().unwrap()),
        )
        .init();
    ...
    // Data directory
    let data_dir =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_DATA_DIR").map(std::path::PathBuf::from).unwrap_or_else(|_| {
            let home = gauss_horizon_core::legacy::var("HOME").unwrap_or_else(|_| ".".to_string());
            std::path::PathBuf::from(home).join(".gauss-horizon-web")
        });
    gauss_horizon_core::legacy::migrate_web_profile(&data_dir).expect("Cannot migrate the previous web profile");
    ...
    let password_disabled = gauss_horizon_core::legacy::var("GAUSS_HORIZON_DISABLE_PASSWORD")
        .map(|v| matches!(v.trim().to_lowercase().as_str(), "1" | "true" | "yes" | "on"))
        .unwrap_or(false);

    let password_hash = if password_disabled {
        None
    } else if let Ok(pw) = gauss_horizon_core::legacy::var("GAUSS_HORIZON_PASSWORD") {
    ...
    let public_base_path =
        normalize_public_base_path(gauss_horizon_core::legacy::var("GAUSS_HORIZON_PUBLIC_BASE_PATH").ok());
    ...
    if let Some(mcp_router) = web_mcp_router(&web_state).expect("Invalid Gauss Horizon Web MCP configuration") {
        app = app.merge(mcp_router);
        tracing::info!("Gauss Horizon Web MCP is enabled at /mcp");
    }

    let static_dir = gauss_horizon_core::legacy::var_os("GAUSS_HORIZON_STATIC_DIR").map(std::path::PathBuf::from);
    app = mount_public_base_path(app, &public_base_path, static_dir.as_deref());

    // Bind address
    let port: u16 =
        gauss_horizon_core::legacy::var("GAUSS_HORIZON_PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(4224);
    let addr = SocketAddr::from(([0, 0, 0, 0], port));

    tracing::info!("Gauss Horizon Web server starting on http://{}", addr);
    if public_base_path != "/" {
        tracing::info!("Serving Gauss Horizon Web under context path {}", public_base_path);
    }
    if password_disabled {
        tracing::info!("Password protection is disabled");
    } else if gauss_horizon_core::legacy::var("GAUSS_HORIZON_PASSWORD").is_ok() {
...
mod tests {
    ...
    fn normalize_public_base_path_trims_and_preserves_segments() {
        assert_eq!(normalize_public_base_path(Some("gauss-horizon".to_string())), "/gauss-horizon");
        assert_eq!(normalize_public_base_path(Some("/gauss-horizon/".to_string())), "/gauss-horizon");
        assert_eq!(normalize_public_base_path(Some("/tools/gauss-horizon/?v=1".to_string())), "/tools/gauss-horizon");
    }

    #[test]
    #[should_panic(expected = "GAUSS_HORIZON_PUBLIC_BASE_PATH contains invalid characters")]
    fn normalize_public_base_path_rejects_invalid_characters() {
        normalize_public_base_path(Some("/gauss-horizon admin".to_string()));
    }
    ...
    async fn import_route_reserves_multipart_framing_above_the_file_limit() {
        ...
        let boundary = "gauss-horizon-import-boundary";
        let body = format!(
    ...
    async fn public_base_path_routes_preserve_redirect_query_static_files_and_api() {
        ...
        let static_dir = std::env::temp_dir().join(format!("gauss-horizon-web-public-base-{}", uuid::Uuid::new_v4()));
        ...
        for public_base_path in ["/gauss-horizon", "/xxxx/rsu"] {
    ...
    async fn root_public_base_path_preserves_static_files_and_api() {
        let static_dir = std::env::temp_dir().join(format!("gauss-horizon-web-root-base-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&static_dir).expect("create static directory");
