...
const MCP_REQUEST_HEADER: &str = "x-gauss-horizon-mcp-request";
...
pub async fn resolve_database(
    ...
    Ok(gauss_horizon_core::mcp_policy::resolve_database(database, config.database.as_deref()))
}
...
pub async fn ensure_mongo_pipeline_target(
    ...
    let config = load_connection(state, connection_id).await?;
    let database = gauss_horizon_core::mcp_policy::resolve_database(database, config.database.as_deref());
...
async fn ensure_write_with_risk(
    ...
    if dangerous && !allow_dangerous_sql {
        return Err(AppError::from(format!(
            "SQL_BLOCKED: High-risk operation '{action}' is disabled in Gauss Horizon MCP settings."
        )));
    }
    if config.read_only {
...
pub async fn ensure_sql(
    ...
    if !allow_database_switch
        && gauss_horizon_core::sql_risk::mcp_sql_has_forbidden_database_switch(sql, config.db_type)
    {
        return Err(AppError::from(
    ...
    if !allow_dangerous_sql && gauss_horizon_core::sql_risk::is_dangerous_sql_for_database(sql, config.db_type) {
        return Err(AppError::from(
            "SQL_BLOCKED: High-risk SQL is disabled in Gauss Horizon MCP settings.".to_string(),
        ));
    }
    if config.read_only {
        gauss_horizon_core::query_execution_sql::check_read_only(sql, &config.name, config.db_type)
...
mod tests {
    ...
    use gauss_horizon_core::storage::{McpConnectionPolicy, McpDatabasePolicy, McpDatabaseScope, McpGlobalPolicy};
