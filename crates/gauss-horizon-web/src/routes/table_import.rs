...
mod tests {
    ...
    fn test_upload_path() -> PathBuf {
        std::env::temp_dir().join(format!("gauss-horizon-table-import-test-{}", uuid::Uuid::new_v4()))
    }
    ...
    fn resolves_uploaded_import_source_by_uuid_reference() {
        let data_dir = std::env::temp_dir().join(format!("gauss-horizon-table-import-data-{}", uuid::Uuid::new_v4()));
