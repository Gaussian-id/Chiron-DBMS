...
use axum::Json;
use gauss_horizon_core::docs::{CollectOptions, SchemaSnapshot};
...
pub async fn collect_snapshot(
    ...
    let snapshot = gauss_horizon_core::docs::collect_snapshot(
        &state.app,
        &connection,
        &options,
        &|_progress| {},
...
pub struct DocsApplyRequest {
    ...
    pub annotations: gauss_horizon_core::docs::annotations::AnnotationFile,
}
...
/// this needs no lookup and no new dependency — gauss-horizon-web does NOT depend on
/// gauss-horizon-mcp.
fn notes_path_for(state: &Arc<WebState>, config: &ConnectionConfig) -> std::path::PathBuf {
    gauss_horizon_core::docs::annotations::resolve_notes_path(
        &config.id,
...
) -> Result<Json<Option<gauss_horizon_core::docs::annotations::AnnotationFile>>, AppError> {
    let config = load_connection(&state, &request.connection_id).await?;
    let path = notes_path_for(&state, &config);
    Ok(Json(gauss_horizon_core::docs::annotations::load_annotations(&path).map_err(AppError::from)?))
}
...
pub async fn apply_annotations(
    ...
    gauss_horizon_core::docs::annotations::apply_annotations(&mut applied, &request.annotations, config.db_type);
    Ok(Json(applied))
}
...
pub async fn save_annotations(
    ...
    let config = load_connection(&state, &request.connection_id).await?;
    let path = notes_path_for(&state, &config);
    gauss_horizon_core::docs::annotations::save_annotations(&path, &request.annotations).map_err(AppError::from)?;
    Ok(Json(()))
}
...
pub async fn export_html(Json(request): Json<DocsExportRequest>) -> Result<Json<DocsExportResponse>, AppError> {
    let content = gauss_horizon_core::docs::to_standalone_html(&request.snapshot, &request.annotations, &request.lang)
        .map_err(AppError::from)?;
