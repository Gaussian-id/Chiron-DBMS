...
pub async fn list_data_types(Query(query): Query<DialectDataTypesQuery>) -> Json<Vec<String>> {
    Json(gauss_horizon_core::sql_dialect::dialect_types::list_dialect_type_names(&query.dialect_name))
}
...
mod tests {
    ...
    async fn list_data_types_matches_tauri_command_behavior() {
        gauss_horizon_core::sql_dialect::dialect_loader::register_core_dialects();
        let Json(types) =
