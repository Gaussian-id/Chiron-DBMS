...
use gauss_horizon_core::table_export::{self, ExportStatus, TableExportProgress, TableExportRequest};
...
mod tests {
    ...
    async fn table_export_download_uses_the_requested_web_filename() {
        let dir =
            std::env::temp_dir().join(format!("gauss-horizon-web-table-export-download-{}", uuid::Uuid::new_v4()));
