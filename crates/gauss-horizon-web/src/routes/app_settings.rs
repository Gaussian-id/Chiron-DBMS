...
use gauss_horizon_core::storage::{McpGlobalPolicy, McpGlobalPolicyState};
...
pub async fn load_web_mcp_http_status(State(state): State<Arc<WebState>>) -> Json<WebMcpHttpStatus> {
    let token_source = match (
        gauss_horizon_core::legacy::var_os("GAUSS_HORIZON_WEB_MCP_TOKEN"),
        gauss_horizon_core::legacy::var_os("GAUSS_HORIZON_WEB_MCP_TOKEN_FILE"),
    ) {
    ...
    Json(WebMcpHttpStatus {
        ...
        endpoint_path,
        token_source,
        allowed_hosts: comma_separated_env("GAUSS_HORIZON_WEB_MCP_ALLOWED_HOSTS"),
        allowed_origins: comma_separated_env("GAUSS_HORIZON_WEB_MCP_ALLOWED_ORIGINS"),
    })
}
...
fn decrypt_config_payload(payload: &EncryptedConfigPayload, passphrase: &str) -> Result<String, String> {
    if payload.format != "gauss-horizon-encrypted"
        && !gauss_horizon_core::legacy::is_legacy_encrypted_format(&payload.format)
        || payload.version != 1
    {
...
mod tests {
    ...
    fn exported_browser_payload() -> EncryptedConfigPayload {
        EncryptedConfigPayload {
            format: "gauss-horizon-encrypted".to_string(),
            version: 1,
