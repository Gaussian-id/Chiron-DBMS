...
pub async fn get_driver_runtime_summary(
    ...
    Ok(Json(gauss_horizon_core::driver_runtime::collect_driver_runtime_summary(&state.app).await))
}
...
pub async fn stop_driver_runtime(
    ...
    gauss_horizon_core::driver_runtime::stop_driver_runtime(&state.app, &req.runtime_id)
        .await
...
pub async fn restart_driver_runtime(
    ...
    gauss_horizon_core::driver_runtime::restart_driver_runtime(&state.app, &req.runtime_id)
        .await
        .map_err(AppError::from)?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
...
pub async fn upgrade_all_agents(
    ...
    let result = async {
        ...
        let registry =
            fetch_registry_from_claimed(gauss_horizon_core::DownloadSource::Official, &[cancellation.as_ref()])
                .await
                .map_err(AppError::from)?;
...
pub async fn import_agents_from_zip(
    ...
    while let Some(field) = multipart.next_field().await.map_err(|err| AppError::from(err.to_string()))? {
        ...
        let result = async {
            ...
            .await
            .map_err(AppError::from)?;
            gauss_horizon_core::jdbc::import_offline_jdbc_payload(state.app.plugins.root_dir(), &package_path)
                .map_err(AppError::from)?;
...
async fn ensure_no_agent_update_blockers(
    state: &gauss_horizon_core::connection::AppState,
    db_types: &[String],
) -> Result<(), String> {
...
mod tests {
    ...
    fn offline_package_extension_accepts_zip_and_tar_zstd() {
        assert_eq!(offline_package_extension("gauss-horizon-agents.zip"), Some("zip"));
        assert_eq!(offline_package_extension("gauss-horizon-agent-duckdb.TAR.ZST"), Some("tar.zst"));
        assert_eq!(offline_package_extension("gauss-horizon-agent-duckdb.zst"), None);
    }
}
