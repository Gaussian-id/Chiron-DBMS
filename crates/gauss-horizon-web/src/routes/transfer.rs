...
use gauss_horizon_core::transfer::{self, TransferRequest, TransferStatus};
...
pub async fn start_transfer(
    ...
    if let Some(name) = gauss_horizon_core::query::connection_readonly_name(&state.app, &req.target_connection_id).await
    {
    ...
    tokio::spawn(async move {
        ...
        if matches!(req.content, transfer::TransferContent::StructureOnly)
            && (matches!(source_db_type, gauss_horizon_core::models::connection::DatabaseType::MongoDb)
                || matches!(target_db_type, gauss_horizon_core::models::connection::DatabaseType::MongoDb))
        {
...
) -> Result<Json<gauss_horizon_core::transfer::TransferOwnershipPreview>, AppError> {
    let req = body.request;
    transfer::validate_transfer_request(&req).map_err(AppError::from)?;
...
mod tests {
    ...
    async fn test_web_state() -> (Arc<WebState>, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("gauss-horizon-web-transfer-test-{}", uuid::Uuid::new_v4()));
