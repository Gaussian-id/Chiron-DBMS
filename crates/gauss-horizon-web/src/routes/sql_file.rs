...
use gauss_horizon_core::sql::{SqlFileProgress, SqlFileRequest, SqlFileStatus};
...
async fn sql_file_upload_limit(state: &WebState) -> usize {
    let max_mb = state
        ...
        .await
        .unwrap_or(gauss_horizon_core::sql_file_import::DEFAULT_SQL_FILE_UPLOAD_MAX_MB);
...
) -> Result<Json<Vec<gauss_horizon_core::sql_file_import::SqlFileTable>>, AppError> {
    let path = validated_uploaded_sql_path(&state.data_dir, &body.file_path)?;
    Ok(Json(gauss_horizon_core::sql_file_import::inspect_sql_file_tables(&path).await.map_err(AppError::from)?))
}
...
pub async fn preview_sql_file(
    ...
    if let Some(field) = multipart.next_field().await.map_err(|e| AppError::from(e.to_string()))? {
        ...
        let content = gauss_horizon_core::sql_file_import::read_sql_file_preview(&file_path, 1_000_000)
            .await
            .map_err(AppError::from)?;
        let preview: String = content.chars().take(20_000).collect();
        let bootstrap_analysis = gauss_horizon_core::sql_file_import::mysql_like_sql_file_bootstrap_analysis(&content);
...
mod tests {
    ...
    fn uploaded_sql_paths_are_unique_and_keep_the_extension() {
        let data_dir = std::env::temp_dir().join(format!("gauss-horizon-web-sql-file-test-{}", uuid::Uuid::new_v4()));
    ...
    fn expired_sql_file_uploads_are_removed() {
        let tmp_dir = std::env::temp_dir().join(format!("gauss-horizon-web-sql-file-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp_dir).unwrap();
