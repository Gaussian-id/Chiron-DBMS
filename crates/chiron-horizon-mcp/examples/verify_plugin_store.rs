//! Verifies a real Chiron Horizon app store: plugin discovery + MCP tool bridge.
//!   cargo run -p chiron-horizon-mcp --example verify_plugin_store -- <data-dir> [app-version]
...
async fn main() {
    ...
    let backend =
        LocalBackend::open_with_app_version(&data_dir.join("chiron-horizon.db"), app_version).await.expect("open local backend");
