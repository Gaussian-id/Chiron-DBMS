...
pub struct McpSession {
    /// Opaque handle returned to the MCP client (`chiron_horizon_open_session` result).
    pub id: String,
    ...
    pub async fn open(&self, connection_id: &str, database: &str) -> McpSessionStoreResult<Result<McpSession, String>> {
        ...
        if state.active.len() + state.closing.len() >= MAX_SESSIONS {
            return McpSessionStoreResult::new(
                Err(format!(
                    "Too many open MCP sessions (max {MAX_SESSIONS}). Close unused sessions with chiron_horizon_close_session."
                )),
                expired,
            );
        }
