...
use crate::{
    ...
    ChironHorizonBackend, ChironHorizonMcpServer, McpScope,
};
...
fn build_streamable_http_router(
    ...
    let service: StreamableHttpService<ChironHorizonMcpServer, LocalSessionManager> = StreamableHttpService::new(
        move || Ok(ChironHorizonMcpServer::with_runtime_options(server_backend.clone(), scope.clone(), web_mode)),
        Default::default(),
        rmcp_config,
    );
...
/// session receives a fresh `ChironHorizonMcpServer`, while the database backend remains
...
/// as healthy (for example, Chiron Horizon Desktop settings UI).
pub async fn serve_streamable_http_on_listener(
    ...
    eprintln!("Chiron Horizon MCP Streamable HTTP listening on http://{}{}", config.bind_addr, config.path);
