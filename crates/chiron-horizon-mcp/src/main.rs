...
use chiron_horizon_mcp::{
    http::serve_streamable_http, with_legacy_discovery_fallback, ChironHorizonBackend, ChironHorizonMcpServer,
    LocalBackend, McpTransport, RuntimeConfig, WebBackend,
};
use rmcp::ServiceExt;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let backend: Arc<dyn ChironHorizonBackend> =
        if let Ok(base_url) = chiron_horizon_core::legacy::var("CHIRON_HORIZON_WEB_URL") {
            ...
            let db_path = chiron_horizon_mcp::paths::storage_db_path().map_err(std::io::Error::other)?;
    ...
    match runtime.transport {
        McpTransport::Stdio => {
            ...
            let service = ChironHorizonMcpServer::new(backend).serve(transport).await?;
