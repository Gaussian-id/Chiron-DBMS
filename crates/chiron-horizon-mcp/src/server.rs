...
use crate::backend::{
    format_query_result, new_connection_config, parse_database_type, ChironHorizonBackend, ConnectionSummary,
};
...
pub struct ConnectionSelector {
    #[schemars(description = "Unique ID of the Chiron Horizon connection")]
    #[schemars(extend("type" = "string"))]
    pub connection_id: Option<String>,
    #[schemars(description = "Name of the Chiron Horizon connection")]
    #[schemars(extend("type" = "string"))]
...
pub struct GetRoutineSourceRequest {
    #[serde(flatten)]
    pub selector: ConnectionSelector,
    #[schemars(description = "Routine name (as returned by chiron_horizon_list_routines)")]
    pub name: String,
...
pub struct ExecuteQueryRequest {
    ...
    pub sql: String,
    #[schemars(
        description = "Session ID from chiron_horizon_open_session. When set, the query runs on the session's pinned connection, preserving USE/SET and other session state across calls."
    )]
    #[schemars(extend("type" = "string"))]
...
pub struct CloseSessionRequest {
    #[schemars(description = "Session ID returned by chiron_horizon_open_session")]
...
pub struct RemoveConnectionRequest {
    #[schemars(description = "Name of the Chiron Horizon connection when connection_id is not provided")]
    #[schemars(extend("type" = "string"))]
    pub connection_name: Option<String>,
    #[schemars(extend("type" = "string"))]
    pub connection_id: Option<String>,
...
pub struct ExecuteBatchQueryRequest {
    ...
    pub sql: String,
    #[schemars(
        description = "Session ID from chiron_horizon_open_session. When set, every statement runs on the session's pinned connection, preserving USE/SET, temporary tables and other session state across statements in the script and across calls."
    )]
    #[schemars(extend("type" = "string"))]
    pub session_id: Option<String>,
    #[schemars(
...
#[derive(Clone)]
pub struct ChironHorizonMcpServer {
    backend: Arc<dyn ChironHorizonBackend>,
    scope: McpScope,
...
struct ResolvedConnection {
    connection: chiron_horizon_core::models::connection::ConnectionConfig,
...
impl McpScope {
    pub fn from_env() -> Self {
        let mut connection_ids = scoped_connection_ids(
            chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_SCOPE_CONNECTION_IDS").ok().as_deref(),
        );
        if connection_ids.is_empty() {
            if let Some(connection_id) = non_empty_env("CHIRON_HORIZON_MCP_SCOPE_CONNECTION_ID") {
                connection_ids.push(connection_id);
            }
        }
        Self {
            connection_ids,
            connection_name: non_empty_env("CHIRON_HORIZON_MCP_SCOPE_CONNECTION_NAME"),
            database: non_empty_env("CHIRON_HORIZON_MCP_SCOPE_DATABASE"),
            schema: non_empty_env("CHIRON_HORIZON_MCP_SCOPE_SCHEMA"),
        }
    }
    ...
    fn matches(&self, connection: &chiron_horizon_core::models::connection::ConnectionConfig) -> bool {
...
impl ChironHorizonMcpServer {
    pub fn new(backend: Arc<dyn ChironHorizonBackend>) -> Self {
        Self::with_runtime_options(
            backend,
            McpScope::from_env(),
            chiron_horizon_core::legacy::var_os("CHIRON_HORIZON_WEB_URL").is_some(),
        )
    }

    pub fn with_runtime_options(backend: Arc<dyn ChironHorizonBackend>, scope: McpScope, web_mode: bool) -> Self {
        ...
        if scope.enabled() {
            tool_router.disable_route("chiron_horizon_add_connection");
            tool_router.disable_route("chiron_horizon_duplicate_connection");
            tool_router.disable_route("chiron_horizon_remove_connection");
        }
        // Desktop UI bridge operations are intentionally unavailable remotely and in scoped AI sessions.
        if web_mode || scope.enabled() {
            tool_router.disable_route("chiron_horizon_open_table");
            tool_router.disable_route("chiron_horizon_execute_and_show");
        }
        #[cfg(not(feature = "mq-admin"))]
        {
            tool_router.disable_route("chiron_horizon_send_message");
            tool_router.disable_route("chiron_horizon_peek_messages");
        }
...
impl ChironHorizonMcpServer {
    #[tool(
        name = "chiron_horizon_list_connections",
        description = "List database connections configured in Chiron Horizon. Returns connection IDs, names, group paths, database types, endpoints, and selected databases."
    )]
    async fn list_connections(
        &self,
        Parameters(ListConnectionsRequest {}): Parameters<ListConnectionsRequest>,
    ) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_list_connections").await {
            return error;
        }
        match self.load_scoped_connections().await {
            Ok(connections) if connections.is_empty() => text("No connections configured in Chiron Horizon."),
            Ok(connections) => {
    ...
    #[tool(
        name = "chiron_horizon_list_databases",
        description = "List database names available through a Chiron Horizon connection. When the connection has a database allowlist, returns only the databases allowed by Chiron Horizon MCP settings; use this before listing tables if the connection has no default database."
    )]
    async fn list_databases(&self, Parameters(request): Parameters<ListDatabasesRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_list_databases").await {
            return error;
        }
    ...
    #[tool(name = "chiron_horizon_list_tables", description = "List tables and views for a database connection")]
    async fn list_tables(&self, Parameters(request): Parameters<ListTablesRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_list_tables").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
    ...
    #[tool(name = "chiron_horizon_describe_table", description = "Get column definitions for a table")]
    async fn describe_table(&self, Parameters(request): Parameters<DescribeTableRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_describe_table").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
    ...
    #[tool(
        name = "chiron_horizon_list_routines",
        description = "List stored procedures and functions for a database schema"
    )]
    async fn list_routines(&self, Parameters(request): Parameters<ListRoutinesRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_list_routines").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
    ...
    #[tool(
        name = "chiron_horizon_get_routine_source",
        description = "Get the full source text of a stored procedure or function"
    )]
    async fn get_routine_source(&self, Parameters(request): Parameters<GetRoutineSourceRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_get_routine_source").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
    ...
    #[tool(
        name = "chiron_horizon_execute_query",
    ...
    async fn execute_query(&self, Parameters(request): Parameters<ExecuteQueryRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_execute_query").await {
            return error;
        }
        ...
        if connection.db_type == chiron_horizon_core::models::connection::DatabaseType::Redis {
            return tool_error(
                "REDIS_COMMAND_REQUIRED",
                "Redis connections do not accept SQL through chiron_horizon_execute_query. Use chiron_horizon_execute_redis_command.",
            );
        }
                ...
                match session {
                    ...
                    None => {
                        return tool_error(
                            "SESSION_NOT_FOUND",
                            format!(
                                "Session \"{session_id}\" not found or expired. Open a new one with chiron_horizon_open_session."
                            ),
                        );
                    }
                }
    ...
    async fn execute_batch_request(&self, request: ExecuteBatchQueryRequest) -> CallToolResult {
        ...
        if matches!(connection.db_type, DatabaseType::Redis | DatabaseType::MongoDb) {
            return tool_error(
                "CHIRON_HORIZON_BATCH_UNSUPPORTED",
        ...
        if transactional
            && chiron_horizon_core::query::batch_transaction_ddl_is_unrollbackable(
        ...
        let mut options = chiron_horizon_core::query::QueryExecutionOptions {
        ...
        match execution {
            ...
            Err(error) => backend_tool_error("CHIRON_HORIZON_BATCH_EXECUTION_ERROR", error),
        }
    }

    #[tool(
        name = "chiron_horizon_open_session",
        description = "Open a stateful query session pinned to a single backend connection. Returns a session ID for chiron_horizon_execute_query: USE, SET CATALOG, session variables and temporary tables persist across calls within the session. Close with chiron_horizon_close_session when done; idle sessions expire after 30 minutes."
    )]
    async fn open_session(&self, Parameters(request): Parameters<OpenSessionRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_open_session").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
        ...
        match session {
            Ok(session) => text(format!(
                "Session opened.\nsession_id: {}\nconnection: {} (id: {})\ndatabase: {}\n\nPass session_id to chiron_horizon_execute_query to run every query on the same pinned connection. Close with chiron_horizon_close_session when done.",
    ...
    #[tool(
        name = "chiron_horizon_close_session",
    ...
    async fn close_session(&self, Parameters(request): Parameters<CloseSessionRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_close_session").await {
            return error;
        }
    ...
    #[tool(
        name = "chiron_horizon_execute_redis_command",
        description = "Execute a Redis command on a Redis connection"
    )]
    async fn execute_redis_command(
        &self,
        Parameters(request): Parameters<ExecuteRedisCommandRequest>,
    ) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_execute_redis_command").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
        ...
        if safety == RedisCommandSafety::Blocked && !permissions.allow_dangerous {
            return tool_error(
                ...
                format!(
                    "Dangerous Redis command \"{}\" is disabled in Chiron Horizon MCP settings.",
        ...
        if safety != RedisCommandSafety::Allowed && !permissions.allow_writes {
            return tool_error(
                "REDIS_COMMAND_BLOCKED",
                "MCP Redis command execution is read-only in Chiron Horizon MCP settings.",
            );
        }
    ...
    #[tool(
        name = "chiron_horizon_peek_messages",
    ...
    async fn peek_messages(&self, Parameters(request): Parameters<PeekMessagesRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_peek_messages").await {
            return error;
        }
        #[cfg(not(feature = "mq-admin"))]
        {
            let _ = request;
            tool_error("MQ_UNSUPPORTED", "Message queue support is not compiled into this Chiron Horizon MCP build.")
        }
        #[cfg(feature = "mq-admin")]
        {
            use chiron_horizon_core::mq::{
    ...
    #[tool(
        name = "chiron_horizon_send_message",
    ...
    async fn send_message(&self, Parameters(request): Parameters<SendMessageRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_send_message").await {
            return error;
        }
        #[cfg(not(feature = "mq-admin"))]
        {
            ...
            return tool_error(
                "MQ_UNSUPPORTED",
                "Message queue support is not compiled into this Chiron Horizon MCP build.",
            );
        }
        ...
        #[cfg(feature = "mq-admin")]
        {
            ...
            let connection = &resolved.connection;
            let mq_config = match chiron_horizon_core::mq::config::MqAdminConfig::from_connection(connection) {
                Ok(config) => config,
            ...
            if chiron_horizon_core::production_safety::is_production_database(
                connection,
                connection.database.as_deref().unwrap_or(""),
            ) {
                return tool_error("PRODUCTION_WRITE_BLOCKED", "MCP cannot send messages to a production database.");
            }
            let request = chiron_horizon_core::mq::SendMessageRequest {
    ...
    #[tool(
        name = "chiron_horizon_get_schema_context",
        description = "Get compact table and column context for writing SQL"
    )]
    async fn get_schema_context(&self, Parameters(request): Parameters<SchemaContextRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_get_schema_context").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
    ...
    #[tool(name = "chiron_horizon_add_connection", description = "Add a new database connection to Chiron Horizon")]
    async fn add_connection(&self, Parameters(request): Parameters<AddConnectionRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_add_connection").await {
            return error;
        }
        ...
        if policy.read_only {
            return tool_error(
                "MCP_READ_ONLY",
                "Chiron Horizon global MCP read-only mode is enabled. Connection management is not allowed.",
            );
        }
    ...
    #[tool(
        name = "chiron_horizon_duplicate_connection",
        description = "Duplicate a Chiron Horizon connection with its complete settings, credentials, tunnels, and sidebar group"
    )]
    async fn duplicate_connection(
        &self,
        Parameters(request): Parameters<DuplicateConnectionRequest>,
    ) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_duplicate_connection").await {
            return error;
        }
        let policy = match self.load_policy().await {
            Ok(policy) => policy,
    ...
    #[tool(name = "chiron_horizon_remove_connection", description = "Remove a database connection from Chiron Horizon")]
    async fn remove_connection(&self, Parameters(request): Parameters<RemoveConnectionRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_remove_connection").await {
            return error;
        }
        let policy = match self.load_policy().await {
            Ok(policy) => policy,
        ...
        if !policy_allows_connection(&policy, group_paths.get(&target.id), &target) {
            return tool_error(
                ...
                format!("Connection \"{}\" is not allowed by Chiron Horizon MCP settings.", target.id),
            );
        }
    ...
    #[tool(
        name = "chiron_horizon_open_table",
        description = "Open a table in Chiron Horizon desktop app. Requires Chiron Horizon to be running."
    )]
    async fn open_table(&self, Parameters(request): Parameters<OpenTableRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_open_table").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
        ...
        match self
            ...
            .await
        {
            Ok(()) => text(format!("Opened {} in Chiron Horizon", request.table)),
            Err(error) => backend_tool_error("CHIRON_HORIZON_NOT_RUNNING", error),
        }
    }

    #[tool(
        name = "chiron_horizon_execute_and_show",
        description = "Execute a SQL query in Chiron Horizon desktop app UI and show results there. Requires Chiron Horizon to be running."
    )]
    async fn execute_and_show(&self, Parameters(request): Parameters<ExecuteAndShowRequest>) -> CallToolResult {
        if let Err(error) = self.ensure_tool_allowed("chiron_horizon_execute_and_show").await {
            return error;
        }
        let resolved = match self.resolve_connection(&request.selector).await {
            Ok(resolved) => resolved,
        ...
        if connection.db_type == DatabaseType::Redis {
            return tool_error(
                "REDIS_COMMAND_REQUIRED",
                "Use chiron_horizon_execute_redis_command for Redis connections.",
            );
        }
        let database = match self.resolve_database(request.database, &resolved) {
            Ok(database) => database,
        ...
        match self
            ...
            .await
        {
            Ok(()) => text("Query sent to Chiron Horizon"),
            Err(error) => backend_tool_error("CHIRON_HORIZON_NOT_RUNNING", error),
        }
    }
}
...
impl ChironHorizonMcpServer {
    async fn list_databases_for_resolved(&self, resolved: &ResolvedConnection) -> CallToolResult {
        match &resolved.database_scope {
            DatabaseScope::None => tool_error(
                ...
                "This connection is configured with no database access in Chiron Horizon MCP settings.",
            ),
    ...
    async fn load_scoped_connections(
        &self,
    ) -> Result<Vec<chiron_horizon_core::models::connection::ConnectionConfig>, String> {
    ...
    async fn load_group_paths_for_policy(
        &self,
        policy: &McpGlobalPolicy,
    ) -> Result<HashMap<String, chiron_horizon_core::mcp_policy::McpConnectionGroupPath>, String> {
    ...
    async fn ensure_tool_allowed(&self, tool_name: &str) -> Result<(), CallToolResult> {
        ...
        if policy_allows_tool(&policy, tool_name) {
            ...
            Err(tool_error(
                "TOOL_OUT_OF_SCOPE",
                format!("Tool \"{tool_name}\" is not allowed by Chiron Horizon MCP settings."),
            ))
        }
    }
    ...
    async fn resolve_connection(&self, selector: &ConnectionSelector) -> Result<ResolvedConnection, CallToolResult> {
        ...
        if let Some(id) = selector.connection_id.as_deref().map(str::trim).filter(|id| !id.is_empty()) {
            ...
            if self.scope.connection_scope_enabled() && !self.scope.matches(&connection) {
                return Err(tool_error(
                    "CONNECTION_OUT_OF_SCOPE",
                    format!("Connection \"{id}\" is outside this Chiron Horizon AI session scope."),
                ));
            }
            if !policy_allows_connection(&policy, group_paths.get(&connection.id), &connection) {
                return Err(tool_error(
                    "CONNECTION_OUT_OF_SCOPE",
                    format!("Connection \"{id}\" is not allowed by Chiron Horizon MCP settings."),
                ));
            }
        ...
        if self.scope.connection_scope_enabled() {
            let connection = connections
                ...
                .ok_or_else(|| tool_error("CONNECTION_NOT_FOUND", "Scoped Chiron Horizon connection was not found."))?;
            if let Some(name) = selector.connection_name.as_deref().map(str::trim).filter(|name| !name.is_empty()) {
                if name != connection.name && name != connection.id {
                    return Err(tool_error(
                        "CONNECTION_OUT_OF_SCOPE",
                        format!("Connection \"{name}\" is outside this Chiron Horizon AI session scope."),
                    ));
                }
            }
            if !policy_allows_connection(&policy, group_paths.get(&connection.id), &connection) {
                return Err(tool_error(
                    "CONNECTION_OUT_OF_SCOPE",
                    "The Chiron Horizon AI session scope is outside the global MCP connection allowlist.",
                ));
            }
        ...
        match allowed.as_slice() {
            ...
            [] => Err(tool_error(
                "CONNECTION_OUT_OF_SCOPE",
                format!("Connection \"{name}\" is not allowed by Chiron Horizon MCP settings."),
            )),
...
impl ServerHandler for ChironHorizonMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("chiron-horizon", env!("CARGO_PKG_VERSION")))
            .with_instructions("Use Chiron Horizon connections to inspect schemas and query databases safely.")
    }
...
fn sql_requires_batch_execution(sql: &str, database_type: DatabaseType) -> bool {
    ...
    chiron_horizon_core::sql::sql_execution_plan_for_database(sql, database_type).statements.len() > 1
}

/// Maximum rows returned per statement in a `chiron_horizon_execute_batch` call, matching
/// the `chiron_horizon_execute_query` "at most 100 rows" contract so batch responses stay
/// bounded.
...
fn agent_result(result: chiron_horizon_core::agent_events::ToolResult) -> CallToolResult {
    if result.is_error {
        backend_tool_error("CHIRON_HORIZON_TOOL_ERROR", result.content.trim_start_matches("Error: "))
    } else {
        text(result.content)
    }
}
...
fn policy_allows_connection(
    policy: &McpGlobalPolicy,
    group_path: Option<&chiron_horizon_core::mcp_policy::McpConnectionGroupPath>,
    connection: &chiron_horizon_core::models::connection::ConnectionConfig,
) -> bool {
    chiron_horizon_core::mcp_policy::policy_allows_connection(policy, group_path, &connection.id)
}
...
fn ensure_database_in_scope(scope: &DatabaseScope, database: &str) -> Result<(), CallToolResult> {
    match scope {
        ...
        DatabaseScope::Selected(_) | DatabaseScope::None => Err(tool_error(
            "DATABASE_OUT_OF_SCOPE",
            format!("Database \"{database}\" is not allowed by Chiron Horizon MCP settings for this connection."),
        )),
    }
}
...
fn ensure_sql_database_scope(
    ...
    if sql_references_disallowed_database(sql, &connection.db_type, active_database, allowed_databases) {
        return Err(tool_error(
            "DATABASE_OUT_OF_SCOPE",
            "SQL references a database that is not allowed by Chiron Horizon MCP settings for this connection.",
        ));
    }
    Ok(())
}
...
fn ensure_sql_database_execution_scope(
    ...
    chiron_horizon_core::mcp_policy::ensure_sql_database_execution_scope(policy, connection, active_database, sql)
...
fn mcp_permissions(
    connection: &chiron_horizon_core::models::connection::ConnectionConfig,
    policy: &McpGlobalPolicy,
) -> chiron_horizon_core::agent_tools::AgentSqlPermissions {
    chiron_horizon_core::agent_tools::AgentSqlPermissions {
...
/// Read the CHIRON_HORIZON_MCP_CONFIRMED_WRITE_SQL env var (set by the CLI agent when the
...
fn mcp_confirmed_write_sql_from_env() -> Option<String> {
    normalize_confirmed_write_sql(chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_CONFIRMED_WRITE_SQL").ok())
}
...
fn confirmed_batch_sql_block_reason(
    sql: &str,
    db_type: chiron_horizon_core::models::connection::DatabaseType,
    ...
    let is_write = match classify_sql_risk_for_database(sql, db_type) {
        Ok(risk) => risk != chiron_horizon_core::sql_risk::SqlRisk::ReadOnly,
        Err(_) => true,
    };
    if !is_write {
        return None;
...
fn validate_sql_policy(
...
) -> Result<chiron_horizon_core::agent_tools::AgentSqlPermissions, CallToolResult> {
    if !allow_database_switch && mcp_sql_has_forbidden_database_switch(sql, connection.db_type) {
        return Err(tool_error(
            "SQL_BLOCKED",
            "MCP does not allow USE or persistent database switching outside a session. Open one with chiron_horizon_open_session to run stateful queries.",
        ));
    }
    let risk =
    ...
    if high_risk && !policy.allow_dangerous_sql {
        return Err(tool_error("SQL_BLOCKED", "High-risk SQL is disabled in Chiron Horizon MCP settings."));
    }
...
fn validate_mongo_command_with_groups(
    ...
    if matches!(command, MongoCommand::RunCommand { .. }) {
        return Err(tool_error(
            "SQL_BLOCKED",
            "MongoDB runCommand is not available through MCP; review and execute it manually in Chiron Horizon.",
        ));
    }
        ...
        mongo::validate_safety(&command, permissions.allow_writes, permissions.allow_dangerous, production_database)
    {
        return Err(match error {
            MongoSafetyError::WritesDisabled => tool_error(
                ...
                "MCP MongoDB execution is read-only for this database in Chiron Horizon MCP settings.",
            ),
            MongoSafetyError::EmptyFilter => tool_error(
                "SQL_BLOCKED",
                "MongoDB update/delete commands must include a non-empty filter unless high-risk operations are enabled in Chiron Horizon MCP settings.",
            ),
            MongoSafetyError::Dangerous => {
                tool_error("SQL_BLOCKED", "Dangerous MongoDB command is disabled in Chiron Horizon MCP settings.")
            }
...
fn mongo_aggregate_target_databases(pipeline: &str, active_database: &str) -> Result<Vec<String>, CallToolResult> {
    chiron_horizon_core::mcp_policy::mongo_pipeline_output_databases(pipeline, active_database)
        .map_err(|error| tool_error("QUERY_ERROR", error.strip_prefix("QUERY_ERROR: ").unwrap_or(&error)))
}

fn non_empty_env(name: &str) -> Option<String> {
    chiron_horizon_core::legacy::var(name).ok().map(|value| value.trim().to_string()).filter(|value| !value.is_empty())
}

#[cfg(feature = "mq-admin")]
fn format_peek_messages_result(result: chiron_horizon_core::mq::PeekMessagesResult, count: usize) -> String {
...
fn format_send_message_result(
    system_kind: &chiron_horizon_core::mq::MqSystemKind,
    result: &chiron_horizon_core::mq::SendMessageResponse,
) -> String {
...
fn ambiguous_connections(
    name: &str,
    connections: &[chiron_horizon_core::models::connection::ConnectionConfig],
) -> String {
...
fn format_routines(routines: &[chiron_horizon_core::db::ObjectInfo]) -> String {
    let rows = routines
        .iter()
        .map(|routine| {
            vec![
...
fn format_columns(columns: &[chiron_horizon_core::db::ColumnInfo]) -> String {
    let rows = columns
        .iter()
        .map(|column| {
            vec![
...
fn redis_database(connection: &chiron_horizon_core::models::connection::ConnectionConfig) -> Option<u32> {
...
fn format_schema_context(
    ...
    database: &str,
    schema: &str,
    tables: &[(chiron_horizon_core::db::TableInfo, Vec<chiron_horizon_core::db::ColumnInfo>)],
    truncated: bool,
) -> String {
...
mod tests {
    ...
    #[async_trait]
    impl ChironHorizonBackend for FakeBackend {
        ...
        async fn execute_agent_tool(
            ...
            _permissions: chiron_horizon_core::agent_tools::AgentSqlPermissions,
        ) -> chiron_horizon_core::agent_events::ToolResult {
        ...
        async fn peek_messages(
            ...
            count: u32,
            options: chiron_horizon_core::mq::PeekMessagesOptions,
        ) -> Result<chiron_horizon_core::mq::PeekMessagesResult, String> {
            self.recorded_arguments.lock().unwrap().push((
                "peek_messages".into(),
                json!({"connectionId": connection.id, "topic": topic, "count": count, "options": options}),
            ));
            Ok(chiron_horizon_core::mq::PeekMessagesResult {
                messages: vec![chiron_horizon_core::mq::PeekedMessage {
        ...
        async fn execute_batch(
            ...
            schema: Option<&str>,
            sql: &str,
            options: chiron_horizon_core::query::QueryExecutionOptions,
            ...
            Ok(vec![crate::backend::BatchStatementResult {
                result: chiron_horizon_core::db::QueryResult {
                    columns: vec![],
                    column_types: vec![],
    ...
    async fn peek_messages_reads_with_read_only_policy_and_forwards_positions() {
        ...
        let server = ChironHorizonMcpServer::with_runtime_options(backend.clone(), McpScope::default(), false);
    ...
    async fn peek_messages_respects_session_scope_and_rejects_other_queues() {
        ...
        for (conn, scope, expected) in [
            ...
            let server = ChironHorizonMcpServer::with_runtime_options(backend.clone(), scope, false);
            let result = server
                .peek_messages(Parameters(
                    serde_json::from_value(json!({"connection_id":"kafka", "topic":"events"})).unwrap(),
                ))
    ...
    fn peek_messages_output_preserves_metadata_and_bounds_whole_messages() {
        use chiron_horizon_core::mq::{PeekMessagesResult, PeekedMessage};
    ...
    fn server_registers_list_connections_tool() {
        let server =
            ChironHorizonMcpServer::with_runtime_options(Arc::new(FakeBackend::default()), McpScope::default(), false);
        ...
        #[cfg(feature = "mq-admin")]
        assert!(names.contains(&"chiron_horizon_peek_messages"));
        #[cfg(not(feature = "mq-admin"))]
        assert!(!names.contains(&"chiron_horizon_peek_messages"));
        assert!(names.contains(&"chiron_horizon_list_connections"));
        assert!(names.contains(&"chiron_horizon_list_databases"));
        assert!(names.contains(&"chiron_horizon_list_tables"));
        assert!(names.contains(&"chiron_horizon_describe_table"));
        assert!(names.contains(&"chiron_horizon_list_routines"));
        assert!(names.contains(&"chiron_horizon_get_routine_source"));
        assert!(names.contains(&"chiron_horizon_execute_query"));
        assert!(names.contains(&"chiron_horizon_execute_batch"));
        assert!(names.contains(&"chiron_horizon_add_connection"));
        assert!(names.contains(&"chiron_horizon_duplicate_connection"));
        assert!(names.contains(&"chiron_horizon_remove_connection"));
        assert!(names.contains(&"chiron_horizon_execute_redis_command"));
        assert!(names.contains(&"chiron_horizon_get_schema_context"));
        assert!(names.contains(&"chiron_horizon_open_table"));
        assert!(names.contains(&"chiron_horizon_execute_and_show"));
        assert!(names.contains(&"chiron_horizon_open_session"));
        assert!(names.contains(&"chiron_horizon_close_session"));
        #[cfg(feature = "mq-admin")]
        assert!(names.contains(&"chiron_horizon_send_message"));
    }
    ...
    async fn tools_list_hides_tools_the_policy_disallows() {
        let policy = McpGlobalPolicy {
            allowed_tool_names: Some(vec![
                "chiron_horizon_list_connections".to_string(),
                "chiron_horizon_execute_query".to_string(),
            ]),
            ..Default::default()
        };
        let server = ChironHorizonMcpServer::with_runtime_options(
        ...
        assert_eq!(names, vec!["chiron_horizon_execute_query", "chiron_horizon_list_connections"]);
    }
    ...
    fn schema_context_tables_schema_is_gemini_compatible() {
        ...
        let tool = server
            ...
            .list_all()
            .into_iter()
            .find(|tool| tool.name == "chiron_horizon_get_schema_context")
    ...
    fn connection_selector_schema_uses_optional_strings() {
        ...
        for tool_name in ["chiron_horizon_execute_query", "chiron_horizon_list_tables", "chiron_horizon_open_session"] {
    ...
    fn optional_fields_never_publish_nullable_union_types() {
        ...
        let mut checks: Vec<(&str, &[&str])> = vec![
            ("chiron_horizon_list_tables", &["database", "schema"]),
            ("chiron_horizon_describe_table", &["database", "schema"]),
            ("chiron_horizon_list_routines", &["database", "schema", "routine_type"]),
            ("chiron_horizon_get_routine_source", &["database", "schema", "signature"]),
            ("chiron_horizon_execute_query", &["database", "session_id", "cell_char_offset", "cell_char_limit"]),
            ("chiron_horizon_execute_batch", &["database", "session_id", "continue_on_error", "use_transaction"]),
            ("chiron_horizon_open_session", &["database"]),
            ("chiron_horizon_open_table", &["database", "schema"]),
            ("chiron_horizon_execute_and_show", &["database"]),
            ("chiron_horizon_add_connection", &["port", "database", "driver_profile"]),
            ("chiron_horizon_remove_connection", &["connection_id", "connection_name"]),
            ("chiron_horizon_execute_redis_command", &["db"]),
            ("chiron_horizon_get_schema_context", &["database", "schema", "max_tables"]),
        ];
        #[cfg(feature = "mq-admin")]
        checks.push((
            "chiron_horizon_send_message",
            &["key", "payload_text", "partition", "exchange", "routing_key", "namespace"],
        ));

        #[cfg(feature = "mq-admin")]
        checks.push(("chiron_horizon_peek_messages", &["count", "partition", "offset"]));
    ...
    fn execute_query_selector_preserves_serde_inputs() {
        ...
        let nested = serde_json::from_str::<ExecuteQueryRequest>(
            r#"{"connection_name":{"tool":"chiron_horizon_chiron_horizon_execute_query","error":"Invalid input"},"sql":"SELECT 1"}"#,
        )
        .unwrap_err();
    ...
    fn format_send_message_result_is_concise_and_includes_delivery_metadata() {
        let result = chiron_horizon_core::mq::SendMessageResponse {
            topic: "orders".to_string(),
            partition: 2,
            offset: 41,
            timestamp: Some("1700000000000".to_string()),
        };
        let output = format_send_message_result(&chiron_horizon_core::mq::MqSystemKind::Kafka, &result);
    ...
    fn scoped_server_hides_mutating_and_desktop_tools() {
        ...
        assert!(!names.iter().any(|name| name == "chiron_horizon_add_connection"));
        assert!(!names.iter().any(|name| name == "chiron_horizon_duplicate_connection"));
        assert!(!names.iter().any(|name| name == "chiron_horizon_remove_connection"));
        assert!(!names.iter().any(|name| name == "chiron_horizon_open_table"));
        assert!(!names.iter().any(|name| name == "chiron_horizon_execute_and_show"));
        assert!(names.iter().any(|name| name == "chiron_horizon_execute_batch"));
        assert!(names.iter().any(|name| name == "chiron_horizon_list_routines"));
        assert!(names.iter().any(|name| name == "chiron_horizon_get_routine_source"));
        assert!(names.iter().any(|name| name == "chiron_horizon_open_session"));
        assert!(names.iter().any(|name| name == "chiron_horizon_close_session"));
        #[cfg(feature = "mq-admin")]
        assert!(names.iter().any(|name| name == "chiron_horizon_send_message"));
    }
    ...
    fn database_execution_policy_overrides_connection_and_global_defaults_and_blocks_cross_database_sql() {
        ...
        let policy = McpGlobalPolicy {
            connection_policies: vec![chiron_horizon_core::storage::McpConnectionPolicy {
                ...
                database_policies: vec![
                    chiron_horizon_core::storage::McpDatabasePolicy {
    ...
    fn redis_database_scope_fails_closed_and_cannot_be_overridden() {
        ...
        let scoped = ChironHorizonMcpServer::with_runtime_options(
        ...
        let invalid = ChironHorizonMcpServer::with_runtime_options(
            Arc::new(FakeBackend { connections: vec![redis.clone()], ..Default::default() }),
            McpScope { database: Some("analytics".to_string()), ..Default::default() },
            false,
        );
    ...
    fn agent_results_preserve_stable_backend_policy_errors() {
        let result = agent_result(chiron_horizon_core::agent_events::ToolResult {
            tool_call_id: "test".to_string(),
    ...
    impl EnvGuard {
        fn set(key: &'static str, value: &str) -> Self {
            let original = chiron_horizon_core::legacy::var(key).ok();
    ...
    fn confirmed_sql_binding_cannot_elevate_central_policy() {
        ...
        let _guard = EnvGuard::set("CHIRON_HORIZON_MCP_CONFIRMED_WRITE_SQL", "CREATE TABLE metrics (id INT)");
        ...
        let _guard = EnvGuard::remove("CHIRON_HORIZON_MCP_CONFIRMED_WRITE_SQL");
        ...
        let _guard = EnvGuard::set("CHIRON_HORIZON_MCP_CONFIRMED_WRITE_SQL", "INSERT INTO metrics (id) VALUES (1)");
        let permissions =
    ...
    fn use_statements_require_a_session() {
        ...
        assert!(result_text(&blocked).contains("chiron_horizon_open_session"));
    ...
    async fn execute_batch_blocks_redis_and_mongo_connections() {
        for (id, db_type) in [("redis", "redis"), ("mongo", "mongodb")] {
            ...
            let result = server
                ...
                .await;
            assert!(result_text(&result).contains("CHIRON_HORIZON_BATCH_UNSUPPORTED"));
        }
    }
    ...
    async fn execute_batch_rejects_mysql_family_ddl_with_transaction() {
        ...
        let goldendb_server =
            ChironHorizonMcpServer::with_runtime_options(goldendb_backend.clone(), McpScope::default(), false);
        ...
        let postgres_server =
            ChironHorizonMcpServer::with_runtime_options(postgres_backend.clone(), McpScope::default(), false);
    ...
    async fn execute_batch_enforces_confirmed_write_sql_binding() {
        ...
        // the handler with CHIRON_HORIZON_MCP_CONFIRMED_WRITE_SQL) because the env var is
        // process-global and would race with `confirmed_sql_binding_cannot_elevate_central_policy`.
        let postgres_db_type = chiron_horizon_core::models::connection::DatabaseType::Postgres;
