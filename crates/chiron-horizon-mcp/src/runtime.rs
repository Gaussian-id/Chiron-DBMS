...
impl RuntimeConfig {
    /// Parses Chiron Horizon MCP's small runtime surface without adding a command-line dependency.
    ...
    pub fn from_environment_and_args() -> Result<Self, String> {
        let mut transport = chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_TRANSPORT").ok();
        let mut host = chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_HTTP_HOST").ok();
        let mut port = chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_HTTP_PORT").ok();
        let mut path = chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_HTTP_PATH").ok();
        let mut allow_remote = env_flag("CHIRON_HORIZON_MCP_HTTP_ALLOW_REMOTE")?;
            ...
            match argument.as_str() {
                ...
                _ => {
                    if let Some(value) = argument.strip_prefix("--transport=") {
                        ...
                        return Err(format!("unknown Chiron Horizon MCP argument: {argument}"));
                    }
                }
            }
        }
        ...
        let ip =
            host.parse::<IpAddr>().map_err(|_| "CHIRON_HORIZON_MCP_HTTP_HOST must be an IP address".to_string())?;
        let port = port
            .unwrap_or_else(|| DEFAULT_HTTP_PORT.to_string())
            .parse::<u16>()
            .map_err(|_| "CHIRON_HORIZON_MCP_HTTP_PORT must be a valid TCP port".to_string())?;
        ...
        if !(is_loopback || (allow_remote && explicit_allow_remote)) {
            return Err(
                "non-loopback HTTP binding requires both CHIRON_HORIZON_MCP_HTTP_ALLOW_REMOTE=1 and --http-allow-remote".into()
            );
        }

        let allowed_hosts = comma_separated_env("CHIRON_HORIZON_MCP_HTTP_ALLOWED_HOSTS");
        let allowed_origins = comma_separated_env("CHIRON_HORIZON_MCP_HTTP_ALLOWED_ORIGINS");
        if !is_loopback && (allowed_hosts.is_empty() || allowed_origins.is_empty()) {
            return Err(
                "non-loopback HTTP binding requires CHIRON_HORIZON_MCP_HTTP_ALLOWED_HOSTS and CHIRON_HORIZON_MCP_HTTP_ALLOWED_ORIGINS".into(),
            );
        }
...
fn parse_transport(value: &str) -> Result<McpTransport, String> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err("CHIRON_HORIZON_MCP_TRANSPORT must be stdio or streamable-http".into()),
    }
}
...
fn env_flag(name: &str) -> Result<bool, String> {
    match chiron_horizon_core::legacy::var(name) {
...
fn http_token_from_environment() -> Result<String, String> {
    let inline_token = chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_HTTP_TOKEN").ok();
    let token_file = chiron_horizon_core::legacy::var("CHIRON_HORIZON_MCP_HTTP_TOKEN_FILE").ok();
    match (inline_token, token_file) {
        (Some(_), Some(_)) => {
            Err("set only one of CHIRON_HORIZON_MCP_HTTP_TOKEN or CHIRON_HORIZON_MCP_HTTP_TOKEN_FILE".into())
        }
        (Some(token), None) if !token.trim().is_empty() => Ok(token),
        (None, Some(path)) => fs::read_to_string(&path)
            .map_err(|error| format!("failed to read CHIRON_HORIZON_MCP_HTTP_TOKEN_FILE: {error}"))
            .map(|token| token.trim_end_matches(['\r', '\n']).to_owned())
            .and_then(|token| {
                (!token.is_empty()).then_some(token).ok_or_else(|| "CHIRON_HORIZON_MCP_HTTP_TOKEN_FILE is empty".into())
            }),
        _ => Err("Streamable HTTP requires CHIRON_HORIZON_MCP_HTTP_TOKEN or CHIRON_HORIZON_MCP_HTTP_TOKEN_FILE".into()),
    }
}

fn validate_path(path: &str) -> Result<(), String> {
    if path == "/" || !path.starts_with('/') || path.ends_with('/') || path.contains('?') || path.contains('#') {
        return Err("CHIRON_HORIZON_MCP_HTTP_PATH must be an absolute path such as /mcp".into());
    }
    Ok(())
}
