...
pub async fn authorize_request(State(auth): State<HttpAuth>, request: Request, next: Next) -> Response {
    ...
    if let Some(origin) = request.headers().get(header::ORIGIN) {
        let origin = match origin.to_str() {
            Ok(origin) => origin,
            Err(_) => {
                log::warn!(target: "chiron_horizon_mcp::audit", "MCP HTTP request rejected: method={method} path={path} reason=invalid-origin");
                return forbidden();
            }
        };
        if !auth.origin_is_allowed(origin) {
            log::warn!(target: "chiron_horizon_mcp::audit", "MCP HTTP request rejected: method={method} path={path} origin={origin} reason=origin-not-allowed");
            return forbidden();
        }
    }

    let Some(token) = bearer_token(request.headers().get(header::AUTHORIZATION)) else {
        log::warn!(target: "chiron_horizon_mcp::audit", "MCP HTTP request rejected: method={method} path={path} reason=missing-bearer-token");
        return unauthorized();
    };
    if !auth.token_matches(token) {
        log::warn!(target: "chiron_horizon_mcp::audit", "MCP HTTP request rejected: method={method} path={path} reason=invalid-bearer-token");
        return unauthorized();
    }

    let response = next.run(request).await;
    log::info!(target: "chiron_horizon_mcp::audit", "MCP HTTP request authenticated: method={method} path={path} status={}", response.status());
    response
}
