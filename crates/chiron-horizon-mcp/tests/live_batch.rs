//! Live regression for `chiron_horizon_execute_batch` transaction semantics against real
...
//!
//! ```text
//! CHIRON_HORIZON_LIVE_MYSQL_HOST / CHIRON_HORIZON_LIVE_MYSQL_PORT / CHIRON_HORIZON_LIVE_MYSQL_USER
//! CHIRON_HORIZON_LIVE_MYSQL_PASSWORD / CHIRON_HORIZON_LIVE_MYSQL_DATABASE
//! CHIRON_HORIZON_LIVE_PG_HOST / CHIRON_HORIZON_LIVE_PG_PORT / CHIRON_HORIZON_LIVE_PG_USER
//! CHIRON_HORIZON_LIVE_PG_PASSWORD / CHIRON_HORIZON_LIVE_PG_DATABASE
//! ```
//!
//! Run with:
//! ```text
//! CHIRON_HORIZON_LIVE_MYSQL_*=... CHIRON_HORIZON_LIVE_PG_*=... cargo test -p chiron-horizon-mcp --test live_batch -- --ignored --nocapture
//! ```
//!
//! What this proves:
...
use chiron_horizon_core::{models::connection::ConnectionConfig, storage::McpGlobalPolicy, storage::Storage};
use chiron_horizon_mcp::{ChironHorizonMcpServer, LocalBackend, McpScope};
use rmcp::{model::CallToolRequestParams, ServiceExt};
use serde_json::{json, Map};

fn required_env(name: &str) -> String {
    chiron_horizon_core::legacy::var(name).unwrap_or_else(|_| panic!("set {name} to run the live batch tests"))
}

/// Build the MySQL connection config and database name from `CHIRON_HORIZON_LIVE_MYSQL_*`.
/// Each engine is independent so a test can run with only its own env set.
fn mysql_env() -> (ConnectionConfig, String) {
    build_env("CHIRON_HORIZON_LIVE_MYSQL", "live-mysql", "mysql")
}

/// Build the PostgreSQL connection config and database name from `CHIRON_HORIZON_LIVE_PG_*`.
fn postgres_env() -> (ConnectionConfig, String) {
    build_env("CHIRON_HORIZON_LIVE_PG", "live-postgres", "postgres")
}
...
/// Boot a real `ChironHorizonMcpServer` over a live LocalBackend seeded from a temp
...
macro_rules! boot_live_server {
    ($connections:expr, $policy:expr) => {{
        ...
        let storage = Storage::open(&dir.path().join("chiron-horizon.db")).await.expect("open storage");
        ...
        drop(storage);
        let backend =
            Arc::new(LocalBackend::open(&dir.path().join("chiron-horizon.db")).await.expect("open LocalBackend"));
...
/// Call `chiron_horizon_execute_query` on the live server; returns `(is_error, text)`.
macro_rules! query {
    ($client:expr, $connection_id:expr, $database:expr, $sql:expr) => {{
        let result = $client
            .peer()
            .call_tool(
                CallToolRequestParams::new("chiron_horizon_execute_query").with_arguments(arguments(json!({
            ...
            .await
            .expect("call chiron_horizon_execute_query");
        let text = result
            .content
            .iter()
...
/// Call `chiron_horizon_execute_batch` on the live server; returns `(is_error, text)`.
macro_rules! batch {
    ($client:expr, $connection_id:expr, $database:expr, $sql:expr, $use_transaction:expr) => {{
        let result = $client
            .peer()
            .call_tool(
                CallToolRequestParams::new("chiron_horizon_execute_batch").with_arguments(arguments(json!({
                    "connection_id": $connection_id,
                    "database": $database,
                    "sql": $sql,
            ...
            .await
            .expect("call chiron_horizon_execute_batch");
        let text = result
            .content
            .iter()
            .filter_map(|block| block.as_text().map(|t| t.text.clone()))
...
#[tokio::test]
#[ignore = "requires CHIRON_HORIZON_LIVE_MYSQL_* and CHIRON_HORIZON_LIVE_PG_* env pointing at reachable MySQL 5.7 / PostgreSQL"]
async fn mysql_batch_rejects_ddl_transaction_and_rolls_back_failed_dml() {
    ...
    assert!(text.contains("CHIRON_HORIZON_BATCH_EXECUTION_ERROR"), "expected execution error, got: {text}");
...
async fn postgres_transaction_batch_respects_mcp_query_timeout() {
    ...
    assert!(
        text.contains("CHIRON_HORIZON_BATCH_EXECUTION_ERROR") || text.contains("Query timed out"),
