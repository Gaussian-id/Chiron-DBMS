//! Full installation-pipeline test: installs the real ssh-sftp `.chiron-horizonp`
...
//!
//! Set CHIRON_HORIZON_SSH_SFTP_PACKAGE to the packaged artifact; the test skips itself
//! otherwise.

use chiron_horizon_core::plugins::{PluginInstallPolicy, PluginPackageInstaller, PluginSignatureStatus};
use chiron_horizon_mcp::LocalBackend;

#[tokio::test]
async fn ssh_sftp_package_installs_and_serves_mcp_tools() {
    let Ok(package) = std::env::var("CHIRON_HORIZON_SSH_SFTP_PACKAGE") else {
        eprintln!("skipping: CHIRON_HORIZON_SSH_SFTP_PACKAGE is not set");
        return;
    };
    ...
    // 1. Install the .chiron-horizonp exactly like the Chiron Horizon desktop app would.
    let installer = PluginPackageInstaller::new(plugins_root.clone(), "0.6.0").expect("installer");
    let result = installer.install_file(&package, PluginInstallPolicy::LocalDevelopment).expect("install .chiron-horizonp");
    let response = result.response();
    assert_eq!(response.plugin.manifest.id, "chiron.horizon.ssh-sftp");
    ...
    let storage = chiron_horizon_core::storage::Storage::open(&database_path).await.unwrap();
    drop(storage);
    ...
    let plugin =
        installed.iter().find(|plugin| plugin.manifest.id == "chiron.horizon.ssh-sftp").expect("installed plugin listed");
    ...
    let ssh = providers
        .iter()
        .find(|provider| provider["pluginId"] == "chiron.horizon.ssh-sftp")
    ...
    let listed = backend
        .call_plugin_tool("chiron.horizon.ssh-sftp", "ssh_list_known_hosts", None, &serde_json::json!({}))
        .await
