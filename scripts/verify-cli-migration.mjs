...
if (!rustBinary || !existsSync(rustBinary)) {
  throw new Error("Set GAUSS_HORIZON_CLI_RUST_BIN to a release gauss-horizon binary before running this test.");
}
if (!legacyBinary || !existsSync(legacyBinary)) {
  throw new Error("Set GAUSS_HORIZON_CLI_LEGACY_BIN to the legacy TypeScript gauss-horizon binary before running this test.");
}
if (!connection) {
  throw new Error("Set GAUSS_HORIZON_CLI_TEST_CONNECTION to a safe test connection name before running this test.");
}
...
assert.match(run(rustBinary, ["--help"]).stdout, /gauss-horizon query/);
assert.match(run(legacyBinary, ["--help"]).stdout, /gauss-horizon query/);
...
compareJsonCommand("query", ["query", connection, "select 1 as gauss_horizon_cli_migration_check", "--json"], (legacy, rust) => {
...
const temporaryDirectory = mkdtempSync(join(tmpdir(), "gauss-horizon-cli-migration-"));
const sqlFile = join(temporaryDirectory, "query.sql");
writeFileSync(sqlFile, "select 1 as gauss_horizon_cli_file_check", "utf8");
try {
...
const contextJson = jsonOutput(run(rustBinary, ["context", "--tables", table, "--json"], { GAUSS_HORIZON_CONNECTION: connection }));
...
console.log("PASS GAUSS_HORIZON_CONNECTION context");

const defaultConnectionQuery = jsonOutput(run(rustBinary, ["query", "select 1 as default_connection", "--json"], { GAUSS_HORIZON_CONNECTION: connection }));
...
console.log("PASS GAUSS_HORIZON_CONNECTION query");
...
const missingDefault = run(rustBinary, ["query", "select 1", "--json"], { GAUSS_HORIZON_CONNECTION: null });
...
const temporaryConflictDirectory = mkdtempSync(join(tmpdir(), "gauss-horizon-cli-migration-conflict-"));
