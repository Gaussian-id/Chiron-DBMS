...
function fixture(database, version) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'gauss-horizon-db-env-'));
...
test('Bash completion preserves all Make targets and completes database selectors', { skip: !bashAvailable }, () => {
  ...
  const completionScript = join(DEFAULT_RECIPES_ROOT, 'completion', 'gauss-horizon-make.bash');
  const result = spawnSync(
    ...
    [
      ...
      '--norc',
      '-c',
      'source "$COMPLETION_SCRIPT"; COMP_WORDS=(make ""); COMP_CWORD=1; _gauss_horizon_make; printf "TARGET:%s\\n" "${COMPREPLY[@]}"; COMP_WORDS=(make db DB=); COMP_CWORD=2; _gauss_horizon_make; printf "SELECTOR:%s\\n" "${COMPREPLY[@]}"',
    ],
...
test('Bash completion delegates to the previous completer outside the repository', { skip: !bashAvailable }, () => {
  ...
  const result = spawnSync(
    ...
    [
      ...
      '--norc',
      '-c',
      '_original_make_completion() { COMPREPLY=(ORIGINAL); }; complete -F _original_make_completion make; source "$COMPLETION_SCRIPT"; cd "$TMPDIR"; COMP_WORDS=(make db DB=); COMP_CWORD=2; _gauss_horizon_make; printf "%s\\n" "${COMPREPLY[@]}"',
    ],
...
test('Zsh completion delegates ordinary Make completion and handles DB selectors', { skip: !zshAvailable }, () => {
  const repoRoot = join(DEFAULT_RECIPES_ROOT, '..', '..');
  const completionScript = join(DEFAULT_RECIPES_ROOT, 'completion', '_gauss-horizon-make.zsh');
  const result = spawnSync(
    'zsh',
    [
      '-f',
      '-c',
      'autoload -Uz compinit && compinit -D; _original_make_completion() { print ORIGINAL; }; compdef _original_make_completion make; source "$COMPLETION_SCRIPT"; words=(make ""); CURRENT=2; _gauss_horizon_make; _describe() { local values_name="$2"; print -l -- "${(@P)values_name}"; }; compset() { return 0; }; words=(make db DB=); CURRENT=3; _gauss_horizon_make',
    ],
    { cwd: repoRoot, env: { ...process.env, COMPLETION_SCRIPT: completionScript }, encoding: 'utf8' },
  );
  assert.equal(result.status, 0, result.stderr);
...
test('generates Gauss Horizon deep links from effective recipe connection values', () => {
  const recipe = discoverRecipes().find((item) => recipeSelector(item) === 'postgresql@17.4');
  assert.ok(recipe);

  const link = gaussHorizonConnectionDeepLink(recipe, { DB_PORT: '15433', DB_PASSWORD: 'p@ss & word' });
  assert.ok(link);
  ...
  assert.equal(params.get('database'), 'gauss-horizon');
});

test('prints Gauss Horizon deep links for compatible connection types', () => {
  const repoRoot = join(DEFAULT_RECIPES_ROOT, '..', '..');
  ...
  assert.match(postgres.stdout, /Gauss Horizon connection link: "gauss-horizon":\/\/connection\/new\?type=postgres&/);
  ...
  assert.match(elasticsearch.stdout, /Gauss Horizon connection link: "gauss-horizon":\/\/connection\/new\?type=elasticsearch&/);
  ...
  assert.match(consul.stdout, /Gauss Horizon connection link: "gauss-horizon":\/\/connection\/new\?type=consul&/);
});
...
test('generates canonical service deep-link types from recipes', () => {
  ...
  for (const [selector, type] of Object.entries(expected)) {
    ...
    const params = new URL(gaussHorizonConnectionDeepLink(recipe)).searchParams;
...
test('derives the required gauss-horizon-prefixed container name', () => {
  assert.equal(expectedContainerName({ database: 'mysql', displayVersion: '5.7' }), 'gauss-horizon-mysql-5.7');
  assert.equal(expectedContainerName({ database: 'PostgreSQL', displayVersion: '17.4' }), 'gauss-horizon-postgresql-17.4');
});
...
test('rejects a recipe whose Compose container name is not standardized', () => {
  ...
  writeFileSync(composePath, readFileSync(composePath, 'utf8').replace('container_name: gauss-horizon-mysql-8.4', 'container_name: custom-mysql'));
  assert.match(validateRecipe(recipe).join('; '), /container_name must be gauss-horizon-mysql-8.4/);
});
...
test('requires the shared default password and database name', () => {
  ...
  assert.match(validateRecipe(recipe).join('; '), /connection.database must be gauss-horizon/);
});
