...
const DEFAULTS = {
  ...
  container: "gauss_horizon_bench_redis_key_search",
...
Options:
  ...
  --container=gauss_horizon_bench_redis_key_search Redis Docker container name
  --port=16151                              Host port for Redis
  --key-count=1000000                       Number of generated keys
  --pattern=user:*                          SCAN MATCH pattern
  --scan-count=10000                        Gauss Horizon SCAN COUNT value
  --max-iterations=15                       Gauss Horizon server-side SCAN iterations per API call
  --typed=false                             Skip Gauss Horizon includeTypes=true comparison
  --json                                    Print JSON only

Before running this benchmark, start Gauss Horizon Web separately, for example:
  GAUSS_HORIZON_DATA_DIR=/tmp/gauss-horizon-bench GAUSS_HORIZON_PORT=4224 GAUSS_HORIZON_DISABLE_PASSWORD=1 cargo run -p gauss-horizon-web
`);
}
...
async function measureGaussHorizonScan(options, connectionId, includeTypes) {
  let cursor = 0;
  let calls = 0;
  let keys = 0;
  let payloadBytes = 0;
  ...
  return {
    label: includeTypes ? "Gauss Horizon scan-keys-batch includeTypes=true" : "Gauss Horizon scan-keys-batch includeTypes=false",
    keys,
    calls,
    payloadBytes,
    elapsedMs: performance.now() - startedAt,
...
function printReport(result) {
  ...
  console.log(`- Gauss Horizon scan count: ${result.config.scanCount}`);
  console.log(`- Gauss Horizon max iterations: ${result.config.maxIterations}`);
...
async function main() {
  ...
  if (!options.json) {
    console.log("Connecting Gauss Horizon Web API...");
  }
  const connectionId = await ensureGaussHorizonConnection(options);

  const measurements = [];
  measurements.push(await measureRedisCliScan(options));
  measurements.push(await measureGaussHorizonScan(options, connectionId, false));
  if (options.includeTyped) {
    measurements.push(await measureGaussHorizonScan(options, connectionId, true));
  }
