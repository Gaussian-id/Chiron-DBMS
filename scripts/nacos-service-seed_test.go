...
func TestServiceOwnedBySeedSupportsWrappedAndStringMetadata(t *testing.T) {
	for name, body := range map[string]string{
		"wrapped object":   `{"code":0,"data":{"metadata":{"source":"chiron-horizon-nacos-service-seed"}}}`,
		"top-level string": `{"metadata":"{\"source\":\"chiron-horizon-nacos-service-seed\"}"}`,
	} {
		t.Run(name, func(t *testing.T) {
...
func TestSeedRefusesToReplaceUnownedServiceUnlessForced(t *testing.T) {
	test := func(force bool) ([]string, error) {
		...
		opts := options{
			namespace: "public", group: "DEFAULT_GROUP", cluster: "DEFAULT", prefix: "chiron-horizon-demo-service",
...
func TestCleanupRefusesToDeleteUnownedServiceUnlessForced(t *testing.T) {
	test := func(force bool) ([]string, error) {
		...
		opts := options{namespace: "public", group: "DEFAULT_GROUP", cluster: "DEFAULT", prefix: "chiron-horizon-demo-service", serviceCount: 1, instancesPerSvc: 1, forceExisting: force}
...
func TestCleanupUsesActualInstanceIdentities(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && r.URL.Path == "/v3/admin/ns/service" {
			_, _ = w.Write([]byte(`{"code":0,"data":{"metadata":{"source":"chiron-horizon-nacos-service-seed"}}}`))
			return
		}
	...
	opts := options{namespace: "public", group: "DEFAULT_GROUP", cluster: "DEFAULT", prefix: "chiron-horizon-demo-service", serviceCount: 1, instancesPerSvc: 3}
...
func TestV2CleanupUsesPaginatedCatalogAndIncludesDisabledInstances(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/ns/service":
			_, _ = w.Write([]byte(`{"metadata":{"source":"chiron-horizon-nacos-service-seed"}}`))
	...
	opts := options{namespace: "public", group: "DEFAULT_GROUP", cluster: "manual", prefix: "chiron-horizon-demo-service", serviceCount: 1, instancesPerSvc: 2}
	if err := target.cleanup(context.Background(), opts); err != nil {
		t.Fatal(err)
	}
	if len(requests) != 6 {
	...
	for index, pageNo := range []string{"1", "2"} {
		...
		if catalog.path != "/v1/ns/catalog/instances" || catalog.query.Get("serviceName") != "DEFAULT_GROUP@@chiron-horizon-demo-service-01" || catalog.query.Get("groupName") != "" || catalog.query.Get("clusterName") != "manual" || catalog.query.Get("pageNo") != pageNo || catalog.query.Get("pageSize") != "100" {
...
func TestServeStartsHTTPServerForEveryGeneratedPort(t *testing.T) {
	opts := options{
		prefix: "chiron-horizon-demo-service", serviceCount: 1, instancesPerSvc: 1,
	...
	if payload["service"] != "chiron-horizon-demo-service-01" || payload["status"] != "healthy" {
...
func TestValidateOptionsRejectsServeDuringCleanup(t *testing.T) {
	err := validateOptions(options{
		target: "v2", action: "cleanup", username: "nacos", password: "123456", namespace: "public", group: "DEFAULT_GROUP", cluster: "DEFAULT", prefix: "chiron-horizon-demo-service",
