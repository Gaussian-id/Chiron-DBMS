...
const DEFAULT_DATABASE = 'chiron-horizon';
...
const CHIRON_HORIZON_DEEP_LINK_TYPES = {
...
function printCompletionSetup() {
  console.log('Bash:       source deploy/database/completion/chiron-horizon-make.bash');
  console.log('Zsh:        autoload -Uz compinit && compinit && source deploy/database/completion/_chiron-horizon-make.zsh');
  console.log('PowerShell: . .\\deploy\\database\\completion\\ChironHorizon.Make.ps1');
}
...
export function expectedContainerName(recipe) {
  return `chiron-horizon-${recipe.database}-${recipe.displayVersion}`.toLowerCase().replace(/[^a-z0-9_.-]+/g, '-');
}
...
function composeArgs(recipe, ...args) {
  const project = (process.env.DB_PROJECT || `chiron-horizon-${basename(REPO_ROOT)}-${recipe.database}-${recipe.displayVersion}`).toLowerCase().replace(/[^a-z0-9_-]+/g, '-');
...
export function chironHorizonConnectionDeepLink(recipe, environment = process.env) {
  const type = recipe.deepLinkType || CHIRON_HORIZON_DEEP_LINK_TYPES[recipe.database];
  if (!type) return null;
...
function printConnection(recipe) {
  ...
  const deepLink = chironHorizonConnectionDeepLink(recipe);
  if (deepLink) console.log(`Chiron Horizon connection link: ${deepLink}`);
  else console.log(`Chiron Horizon connection link: unavailable (Chiron Horizon has no compatible ${recipe.name} connection type)`);
