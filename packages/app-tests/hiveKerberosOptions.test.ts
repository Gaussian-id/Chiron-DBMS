...
test("adds Hive Kerberos URL and JVM options while preserving unrelated params", () => {
  const result = applyHiveKerberosSubmitConfig({
    ...
    jaasConfigPath: "C:\\gauss-horizon\\hive-jaas.conf",
  ...
  assert.deepEqual(result.agentJavaOptions, ["-Djava.security.krb5.conf=C:\\ProgramData\\MIT\\Kerberos5\\krb5.ini", "-Djava.security.auth.login.config=C:\\gauss-horizon\\hive-jaas.conf", "-Djavax.security.auth.useSubjectCredsOnly=false", "-Dsun.security.krb5.debug=true"]);
});
