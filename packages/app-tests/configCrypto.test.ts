...
test("encrypts and decrypts config round-trip", async () => {
  ...
  assert.equal(encrypted.format, "chiron-horizon-encrypted");
...
test("detects encrypted config format", () => {
  assert.equal(isEncryptedConfig({ format: "chiron-horizon-encrypted", version: 1, salt: "a", iv: "b", data: "c" }), true);
  assert.equal(isEncryptedConfig({ format: "chiron-horizon-config", version: 1, connections: [] }), false);
