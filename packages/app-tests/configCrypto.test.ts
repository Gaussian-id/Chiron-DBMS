...
test("encrypts and decrypts config round-trip", async () => {
  ...
  assert.equal(encrypted.format, "gauss-horizon-encrypted");
...
test("detects encrypted config format", () => {
  assert.equal(isEncryptedConfig({ format: "gauss-horizon-encrypted", version: 1, salt: "a", iv: "b", data: "c" }), true);
  assert.equal(isEncryptedConfig({ format: "gauss-horizon-config", version: 1, connections: [] }), false);
