...
test("uses ROWID as Oracle editable key when a table has no primary key", () => {
  assert.deepEqual(editablePrimaryKeys("oracle", [column("ID"), column("CITY")], "TABLE"), [CHIRON_HORIZON_ROWID_COLUMN]);
  assert.deepEqual(editablePrimaryKeys("oceanbase-oracle", [column("ID"), column("CITY")], "TABLE"), [CHIRON_HORIZON_ROWID_COLUMN]);
});
...
test("uses table-specific TDengine editable keys", () => {
  ...
  assert.deepEqual(editablePrimaryKeys("tdengine", columns, "STABLE"), [CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN, "ts", "seq"]);
  assert.deepEqual(editablePrimaryKeys("tdengine", columns, "TABLE"), ["ts", "seq"]);
  assert.equal(hasCompleteTdengineRowIdentity("tdengine", [CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN, "ts", "seq"], ["tbname", "ts", "seq", "current"]), true);
  assert.equal(hasCompleteTdengineRowIdentity("tdengine", [CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN, "ts", "seq"], ["tbname", "ts", "current"]), false);
});
...
test("allows existing row edits according to database-specific key requirements", () => {
  ...
  assert.equal(canEditExistingTableRows("tdengine", undefined, [CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN, "ts"]), true);
  assert.equal(canEditExistingTableRows("postgres", undefined), true);
});
...
test("uses elementId as Neo4j editable key when labels have no primary key", () => {
  assert.deepEqual(editablePrimaryKeys("neo4j", [column("name"), column("role")]), [CHIRON_HORIZON_NEO4J_ELEMENT_ID_COLUMN]);
});

test("keeps TDengine existing row identity and tag columns read-only", () => {
  assert.equal(isTdengineExistingRowReadonlyColumn("tdengine", CHIRON_HORIZON_TDENGINE_TBNAME_COLUMN, [column("ts", true)]), true);
...
test("detects the synthetic Oracle ROWID key case", () => {
  assert.equal(usesSyntheticRowIdKey("oracle", [CHIRON_HORIZON_ROWID_COLUMN]), true);
  assert.equal(usesSyntheticRowIdKey("oracle", [CHIRON_HORIZON_ROWID_COLUMN.toLowerCase()]), true);
  assert.equal(usesSyntheticRowIdKey("oceanbase-oracle", [CHIRON_HORIZON_ROWID_COLUMN]), true);
  assert.equal(usesSyntheticRowIdKey("oracle", [CHIRON_HORIZON_ROWID_COLUMN], "VIEW"), false);
  assert.equal(usesSyntheticRowIdKey("oracle", [CHIRON_HORIZON_ROWID_COLUMN], "MATERIALIZED_VIEW"), false);
  assert.equal(usesSyntheticRowIdKey("postgres", [CHIRON_HORIZON_ROWID_COLUMN]), false);
  assert.equal(usesSyntheticRowIdKey("oracle", ["ID"]), false);
  assert.equal(usesSyntheticRowIdKey("neo4j", [CHIRON_HORIZON_NEO4J_ELEMENT_ID_COLUMN]), true);
});

test("hides only the synthetic Oracle ROWID grid column", () => {
  assert.equal(isHiddenGridColumn("oracle", CHIRON_HORIZON_ROWID_COLUMN, [CHIRON_HORIZON_ROWID_COLUMN]), false);
  assert.equal(isHiddenGridColumn("oracle", CHIRON_HORIZON_ROWID_COLUMN, [CHIRON_HORIZON_ROWID_COLUMN], "TABLE"), true);
  assert.equal(isHiddenGridColumn("oceanbase-oracle", CHIRON_HORIZON_ROWID_COLUMN, [CHIRON_HORIZON_ROWID_COLUMN], "TABLE"), true);
  assert.equal(isHiddenGridColumn("oracle", CHIRON_HORIZON_ROWID_COLUMN, [CHIRON_HORIZON_ROWID_COLUMN], "VIEW"), false);
  assert.equal(isHiddenGridColumn("oracle", "ROWID", [CHIRON_HORIZON_ROWID_COLUMN]), false);
  assert.equal(isHiddenGridColumn("mysql", CHIRON_HORIZON_ROWID_COLUMN, [CHIRON_HORIZON_ROWID_COLUMN]), false);
  assert.equal(isHiddenGridColumn("neo4j", CHIRON_HORIZON_NEO4J_ELEMENT_ID_COLUMN, [CHIRON_HORIZON_NEO4J_ELEMENT_ID_COLUMN]), true);
});
