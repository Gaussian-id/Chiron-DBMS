...
test("parses gauss-horizon connection deep link query fields", () => {
  const draft = parseConnectionDeepLink("gauss-horizon://connection/new?type=postgres&host=db.internal&port=15432&user=app&database=orders&name=Orders");
...
test("parses encoded database URL with password", () => {
  const draft = parseConnectionDeepLink("gauss-horizon://connection/new?url=postgres%3A%2F%2Fapp%3Asecret%40db.internal%3A5432%2Forders%3Fsslmode%3Drequire");
...
test("preserves explicit SQL Server default port from nested URLs", () => {
  ...
  const draft = parseConnectionDeepLink(`"gauss-horizon"://connection/new?url=${nested}`);
...
test("marks top-level SQL Server ports explicit for one-time links", () => {
  const draft = parseConnectionDeepLink("gauss-horizon://connection/new?type=sqlserver&host=db\\instance&port=1433&one_time=1");
...
test("top-level deep link name overrides nested database URL name", () => {
  ...
  const draft = parseConnectionDeepLink(`"gauss-horizon"://connection/new?url=${nested}&name=Top+Level`);

  assert.equal(draft?.name, "Top Level");
});

test("allows password query field to override database URL password", () => {
  const draft = parseConnectionDeepLink("gauss-horizon://connection/new?url=postgres%3A%2F%2Fapp%3Asecret%40db.internal%3A5432%2Forders&password=override");

  assert.equal(draft?.password, "override");
});

test("parses boolean control parameters consistently", () => {
  const draft = parseConnectionDeepLink("gauss-horizon://connection/new?type=mysql&ssl=1&one_time=yes");

  assert.equal(draft?.ssl, true);
  assert.equal(draft?.oneTime, true);
});

test("ignores unsupported gauss-horizon deep link targets", () => {
  assert.equal(parseConnectionDeepLink("gauss-horizon://query/open?sql=select%201"), null);
  assert.equal(parseConnectionDeepLink("gauss-horizon://connections/new?type=postgres"), null);
});
