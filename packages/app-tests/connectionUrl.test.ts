...
test("parses H2 split JDBC URLs as file connections", () => {
  const source = "jdbc:h2:split:28:C:/chiron-horizon-test/h2/sample-db;AUTO_SERVER=TRUE";
  const parsed = parseConnectionUrl(source);

  assert.equal(parsed.dbType, "h2");
  assert.equal(parsed.driverProfile, "h2");
  assert.equal(parsed.driverLabel, "H2");
  assert.equal(parsed.host, "C:/chiron-horizon-test/h2/sample-db");
  assert.equal(parsed.port, 0);
  assert.equal(parsed.username, "sa");
  assert.equal(parsed.password, "");
...
test("keeps typed H2 credentials when JDBC URL does not include them", () => {
  const parsed = parseConnectionUrl("jdbc:h2:split:28:C:/chiron-horizon-test/h2/sample-db;AUTO_SERVER=TRUE");
...
test("uses H2 JDBC URL credentials when they are included", () => {
  const parsed = parseConnectionUrl("jdbc:h2:split:28:C:/chiron-horizon-test/h2/sample-db;USER=url-user;PASSWORD=url-secret;AUTO_SERVER=TRUE");
  const applied = applyParsedConnectionUrl({ name: "", db_type: "h2", username: "typed-user", password: "typed-secret" } as any, parsed);
...
test("rebuilds H2 split JDBC URLs with an edited file path", () => {
  assert.equal(h2FileJdbcUrlWithPath("jdbc:h2:split:28:C:/chiron-horizon-test/h2/sample-db;AUTO_SERVER=TRUE", "D:/chiron-horizon/new-sample.mv.db"), "jdbc:h2:split:28:D:/chiron-horizon/new-sample;AUTO_SERVER=TRUE");
});
...
test("parses Easysearch URLs and keeps the selected HTTPS profile", () => {
  const dedicated = parseConnectionUrl("easysearch://chiron_horizon_test:secret@search.example.com:9200");
  ...
  assert.equal(dedicated.username, "chiron_horizon_test");
