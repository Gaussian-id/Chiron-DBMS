...
test("keeps Oracle queries with a NUM column or alias editable when result labels disagree", () => {
  ...
  assert.deepEqual(sourceColumnsForResult(byAlias, ["ID", "AMOUNT", "__chiron_horizon_row_num"], undefined, "oracle"), ["id", "amount", undefined]);
  assert.deepEqual(sourceColumnsForResult(byAlias, ["ID", "AMOUNT", "chiron_horizon_rn"], undefined, "oracle"), ["id", "amount", undefined]);
  assert.equal(allEditableColumnsWriteable(byAlias, ["ID", "AMOUNT", "__chiron_horizon_row_num"], undefined, "oracle"), true);
});

test("tolerates trailing pagination labels for non-Oracle engines", () => {
  const analysis = analyzeEditableQuery("select id, amount as num from users");
  assert.ok(analysis);
  assert.deepEqual(sourceColumnsForResult(analysis, ["id", "num", "__chiron_horizon_row_num"], undefined, "mysql"), ["id", "amount", undefined]);
  assert.equal(allEditableColumnsWriteable(analysis, ["id", "num", "__chiron_horizon_row_num"], undefined, "mysql"), true);
  assert.deepEqual(sourceColumnsForResult(analysis, ["id", "num", "chiron_horizon_rn"], undefined, "mysql"), ["id", "amount", undefined]);
  assert.equal(allEditableColumnsWriteable(analysis, ["id", "num", "chiron_horizon_rn"], undefined, "mysql"), true);
