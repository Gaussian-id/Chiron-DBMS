...
/** Mirrors the shapes crates/chiron-horizon-core/src/db/sqlserver.rs:523-542 actually produces. */
...
test("#6189: a later SQL Server execution does not inherit the previously focused result index", async () => {
  ...
  connectionStore.addEphemeralConnection({ ...sqlServerConn("conn-1"), database: "chiron-horizon" });
  const tabId = store.createTab("conn-1", "chiron-horizon", "Query");
