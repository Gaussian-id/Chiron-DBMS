...
test("omits relationship paths when relationshipPaths entry is missing", () => {
  ...
  assert.doesNotMatch(svg, /id="gauss-horizon-diagram-arrow"/);
});
...
test("builds safe SVG file names from the active diagram context", () => {
  assert.equal(diagramSvgFileName("prod/main", "billing db", "engineering"), "gauss-horizon-prod-main-billing-db-engineering-er.svg");
  assert.equal(diagramSvgFileName("", "", "table"), "gauss-horizon-diagram-table-structure.svg");
});
