...
it("grants only exists and readTextFile command scopes for the exact UI tuning file", () => {
  ...
  expect(scopes).toEqual([
    { identifier: "fs:allow-exists", allow: [{ path: "$HOME/.chiron-horizon/ui-tuning.json" }] },
    { identifier: "fs:allow-read-text-file", allow: [{ path: "$HOME/.chiron-horizon/ui-tuning.json" }] },
  ]);
