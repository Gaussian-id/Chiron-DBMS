...
test("Oracle JDBC uses schema filtering for visible object selection", () => {
  ...
  assert.deepEqual(filterSchemaNamesForConnection(["ANONYMOUS", "CHIRON_HORIZON_TEST", "SYS", "SYSTEM"], connection, ""), ["CHIRON_HORIZON_TEST"]);
});
...
test("Oracle keeps an existing DIP user visible", () => {
  assert.deepEqual(filterSchemaNamesForConnection(["CHIRON_HORIZON_TEST", "DIP", "SYSTEM"], config({ db_type: "oracle", database: "XE" }), "XE"), ["CHIRON_HORIZON_TEST", "DIP"]);
});
