...
test("keeps JSON-like strings as raw text in Redis member details", () => {
  const detail = formatRedisMemberDetail('{"id":1,"name":"Ada","tags":["gauss-horizon","redis"]}');

  assert.equal(detail.format, "text");
  assert.equal(detail.rawText, '{"id":1,"name":"Ada","tags":["gauss-horizon","redis"]}');
  assert.equal(detail.text, '{"id":1,"name":"Ada","tags":["gauss-horizon","redis"]}');
});
