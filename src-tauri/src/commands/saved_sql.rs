...
use chiron_horizon_core::saved_sql::{SavedSqlFile, SavedSqlFolder, SavedSqlLibrary};
...
const SYNC_MANIFEST_FILE: &str = ".chiron-horizon-sql-library-sync.json";
...
fn sync_saved_sql_directory_blocking(target_dir: &Path, entries: &[SavedSqlSyncEntry]) -> Result<(), String> {
    let sync_root = target_dir.join("chiron-horizon-sql-library");
...
fn verify_previous_sync_files<'a>(
    ...
    for file in files {
        ...
        if !sha256_hex(&contents).eq_ignore_ascii_case(&file.sha256) {
            return Err(sync_conflict(&file.path, "managed file was edited outside Chiron Horizon"));
        }
    }
    Ok(())
}
...
fn write_sync_file(file_path: &Path, contents: &[u8], replace: bool) -> Result<(), String> {
    ...
    let temporary_path = parent.join(format!(".chiron-horizon-sql-sync.{}.tmp", uuid::Uuid::new_v4()));
...
fn open_path(path: &Path) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    let mut command = {
        let mut command = chiron_horizon_core::process::new_std_command("open");
        command.arg(path);
        command
    };
    ...
    #[cfg(target_os = "windows")]
    let mut command = {
        let mut command = chiron_horizon_core::process::new_std_command("explorer");
        command.arg(path);
        command
    };
    ...
    let mut command = {
        let mut command = chiron_horizon_core::process::new_std_command("xdg-open");
        command.arg(path);
        command
    };
...
mod tests {
    ...
    impl TestDirectory {
        fn new(name: &str) -> Self {
            let task_tmp = chiron_horizon_core::legacy::var_os("CHIRON_HORIZON_TEST_TMP_DIR")
                .map(PathBuf::from)
                .unwrap_or_else(std::env::temp_dir);
            let path = task_tmp.join(format!("chiron-horizon-saved-sql-{name}-{}", uuid::Uuid::new_v4()));
    ...
    fn sync_root(target: &TestDirectory) -> PathBuf {
        target.0.join("chiron-horizon-sql-library")
    }
    ...
    fn rejects_external_edits_before_overwriting_managed_files() {
        ...
        let managed_file = target.0.join("chiron-horizon-sql-library/reports/daily.sql");
        std::fs::write(&managed_file, "SELECT 'edited outside Chiron Horizon';").unwrap();
        ...
        assert_eq!(std::fs::read_to_string(managed_file).unwrap(), "SELECT 'edited outside Chiron Horizon';");
    }

    #[test]
    fn unchanged_external_edit_is_left_untouched_until_chiron_horizon_needs_the_path() {
        ...
        let error = sync_saved_sql_directory_blocking(&target.0, &[entry("SELECT 2;")]).unwrap_err();
        assert!(error.contains("managed file was edited outside Chiron Horizon"), "unexpected error: {error}");
        assert_eq!(std::fs::read_to_string(managed_file).unwrap(), "SELECT 'edited outside Chiron Horizon';");
    }
    ...
    fn failed_managed_file_publish_keeps_destination_and_cleans_temporary_file() {
        ...
        let temporary_files = std::fs::read_dir(&target.0)
            ...
            .filter(|entry| {
                ...
                name.starts_with(".chiron-horizon-sql-sync.") && name.ends_with(".tmp")
            })
            .count();
        assert_eq!(temporary_files, 0);
    }
