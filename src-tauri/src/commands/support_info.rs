...
pub(crate) fn format_support_info_for_clipboard() -> String {
    ...
    [
        format!("Gauss Horizon Version: {}", normalize_app_version(&info.app_version)),
...
fn command_first_line(command: &str, args: &[&str]) -> Option<String> {
    let mut process = gauss_horizon_core::process::new_std_command(command);
    process.args(args);
...
mod tests {
    ...
    fn formats_native_about_support_info_compactly() {
        ...
        assert!(!text.contains("Gauss Horizon Version:"));
