use chiron_horizon_core::text_export::{format_html, format_json, format_markdown, QueryResultTextExportData};
...
mod tests {
    ...
    fn writes_json_without_utf8_bom() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-json-export-{}.json", uuid::Uuid::new_v4()));
