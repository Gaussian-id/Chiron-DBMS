...
mod tests {
    ...
    fn scan_sql_files_skips_pruned_metadata_directories() {
        let root = std::env::temp_dir().join(format!("chiron-horizon-sql-folder-scan-{}", uuid::Uuid::new_v4()));
    ...
    fn scan_sql_files_accepts_user_regular_expressions() {
        let root = std::env::temp_dir().join(format!("chiron-horizon-file-folder-scan-{}", uuid::Uuid::new_v4()));
    ...
    async fn renames_and_deletes_filtered_text_files_within_the_opened_root() {
        let root = std::env::temp_dir().join(format!("chiron-horizon-text-file-manage-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
    ...
    async fn manages_sql_files_within_the_opened_root() {
        let root = std::env::temp_dir().join(format!("chiron-horizon-sql-file-manage-{}", uuid::Uuid::new_v4()));
        let nested = root.join("queries");
        std::fs::create_dir_all(&nested).unwrap();
    ...
    async fn keeps_the_file_tree_path_representation_for_managed_files() {
        let root = std::env::temp_dir().join(format!("chiron-horizon-sql-file-path-{}", uuid::Uuid::new_v4()));
        let nested = root.join("queries");
    ...
    async fn renames_files_when_only_the_case_changes() {
        let root = std::env::temp_dir().join(format!("chiron-horizon-sql-file-case-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
