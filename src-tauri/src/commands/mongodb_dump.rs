...
use chiron_horizon_core::mongodb_dump::{
    ...
    tokio::task::spawn_blocking(move || {
        runtime.block_on(async move {
            let result = mongodb_dump::dump_mongodb_database(
                ...
                |id| {
                    let id = id.to_string();
                    Box::pin(async move { chiron_horizon_core::transfer::is_cancelled(&id).await })
                },
                |progress| {
            ...
            .await;
            chiron_horizon_core::transfer::clear_cancelled(&request.task_id).await;
            result
        })
    })
    .await
...
pub async fn cancel_mongodb_database_dump(task_id: String) -> bool {
    chiron_horizon_core::transfer::set_cancelled(&task_id).await;
    true
}
