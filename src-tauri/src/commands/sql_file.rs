...
pub async fn preview_sql_file(file_path: String) -> Result<SqlFilePreview, String> {
        ...
        .is_some_and(|extension| extension.eq_ignore_ascii_case("zip"))
    {
        ...
        let (package, extracted_paths) = tokio::task::spawn_blocking({
            ...
            move || {
                let package =
                    chiron_horizon_core::sql_file_zip_package::extract_sql_file_zip_package(&path, &extraction_dir)?;
                let paths =
                    chiron_horizon_core::sql_file_zip_package::extracted_sql_zip_paths(&extraction_dir, &package)
                        .into_iter()
