...
pub use gauss_horizon_core::update::UpdateInfo;
use semver::Version;
...
pub async fn check_for_updates(
    ...
    source: Option<gauss_horizon_core::DownloadSource>,
) -> Result<UpdateInfo, String> {
    gauss_horizon_core::distribution::ensure_enabled()?;
    let locale = locale.unwrap_or_else(|| "zh-CN".to_string());
    let release = gauss_horizon_core::update::fetch_latest_release(&locale, source.unwrap_or_default()).await?;
    let current_version = env!("CARGO_PKG_VERSION");
    let mut info = gauss_horizon_core::update::build_update_info(release, current_version);
...
#[tauri::command]
pub async fn fetch_changelog(lang: Option<String>) -> Result<gauss_horizon_core::changelog::ChangelogData, String> {
    let lang = lang.unwrap_or_else(|| "en".to_string());
    gauss_horizon_core::changelog::fetch_changelog(&lang).await
}

#[tauri::command]
pub async fn get_system_proxy_url() -> Option<String> {
    tauri::async_runtime::spawn_blocking(gauss_horizon_core::update::system_proxy_url).await.ok().flatten()
}
...
pub fn get_downloaded_update(
    ...
    match restore_cached(&app) {
        ...
        Ok(None) => Ok(None),
        Err(error) => {
            eprintln!("[Gauss Horizon updater] discarded invalid cache: {error}");
            Ok(None)
        }
    }
}
...
async fn download_update_inner(
    ...
    println!("[Gauss Horizon updater] checking from {} endpoints: {}", source.label(), endpoint_urls.join(", "));
    ...
    if let Some(proxy_url) = gauss_horizon_core::update::system_proxy_url() {
    ...
    println!("[Gauss Horizon updater] candidates for installer download: {:?}", candidates);
    ...
    for candidate_url in candidates {
        ...
        println!("[Gauss Horizon updater] downloading installer update from {candidate_url}");
        ...
        match download_result {
            ...
            Err(error) => {
                ...
                println!("[Gauss Horizon updater] installer candidate failed ({candidate_url}): {error}");
...
async fn download_portable_update_inner(
    ...
    for candidate in candidates {
        ...
        println!("[Gauss Horizon updater] downloading portable update from {}", candidate.archive_url);
        let result = async {
        ...
        match result {
            ...
            Err(error) => {
                ...
                println!("[Gauss Horizon updater] portable update candidate failed: {error}");
...
mod tests {
    ...
    fn rewrites_github_asset_url_to_cnb() {
        let download_url = UpdateDownloadSource::Cnb
            .rewrite_download_url("https://github.com/Gaussian-id/Gauss-Horizon/releases/download/v0.5.39/GAUSS_HORIZON_0.5.39_aarch64.dmg")
            .unwrap()
            .unwrap();
        assert_eq!(
            download_url,
            "https://distribution-disabled.invalid/-/releases/download/v0.5.39/GAUSS_HORIZON_0.5.39_aarch64.dmg"
        );
    }
    ...
    fn builds_signed_official_portable_asset_candidates() {
        ...
        assert_eq!(
            ...
            format!("{R2_LATEST_RELEASE_DOWNLOAD_PREFIX}GAUSS_HORIZON_0.5.64_x64-portable.zip")
        );
        assert_eq!(
            candidates[1].archive_url,
            format!("{GITHUB_RELEASE_DOWNLOAD_PREFIX}v0.5.64/GAUSS_HORIZON_0.5.64_x64-portable.zip")
        );
    ...
    fn builds_cnb_portable_asset_candidate_with_r2_fallback() {
        ...
        assert_eq!(
            candidates[0].archive_url,
            format!("{CNB_RELEASE_DOWNLOAD_PREFIX}v0.5.64/GAUSS_HORIZON_0.5.64_arm64-portable.zip")
        );
        assert_eq!(
            candidates[1].archive_url,
            format!("{R2_LATEST_RELEASE_DOWNLOAD_PREFIX}GAUSS_HORIZON_0.5.64_arm64-portable.zip")
        );
    }
    ...
    fn builds_installer_asset_candidates_for_cnb_source() {
        let candidates = UpdateDownloadSource::Cnb.installer_asset_candidates(
            "https://github.com/Gaussian-id/Gauss-Horizon/releases/download/v0.5.64/GAUSS_HORIZON_0.5.64_aarch64.dmg",
            Some("0.5.64"),
        );
        assert_eq!(candidates.len(), 3);
        assert_eq!(
            candidates[0],
            "https://distribution-disabled.invalid/-/releases/download/v0.5.64/GAUSS_HORIZON_0.5.64_aarch64.dmg"
        );
        assert_eq!(candidates[1], format!("{R2_LATEST_RELEASE_DOWNLOAD_PREFIX}GAUSS_HORIZON_0.5.64_aarch64.dmg"));
        assert_eq!(
            candidates[2],
            "https://github.com/Gaussian-id/Gauss-Horizon/releases/download/v0.5.64/GAUSS_HORIZON_0.5.64_aarch64.dmg"
        );
    ...
    fn builds_installer_asset_candidates_for_official_source() {
        ...
        assert_eq!(candidates.len(), 2);
        assert_eq!(candidates[0], format!("{R2_LATEST_RELEASE_DOWNLOAD_PREFIX}GAUSS_HORIZON_0.5.64_aarch64.dmg"));
        assert_eq!(
            candidates[1],
            "https://github.com/Gaussian-id/Gauss-Horizon/releases/download/v0.5.64/GAUSS_HORIZON_0.5.64_aarch64.dmg"
        );
