...
use chiron_horizon_core::db::sqlite::path_has_sqlite_header;
use chiron_horizon_core::path_utils::expand_tilde;
...
pub fn reveal_in_file_manager(path: &Path) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        chiron_horizon_core::process::new_std_command("open")
            .arg("-R")
            .arg(path)
            .spawn()
            .map(|_| ())
    ...
    #[cfg(all(not(target_os = "macos"), not(target_os = "windows")))]
    {
        ...
        chiron_horizon_core::process::new_std_command("xdg-open")
            .arg(&target)
            .spawn()
            .map(|_| ())
...
/// leading `~` which is expanded via `chiron_horizon_core::path_utils::expand_tilde`.
#[tauri::command]
...
fn validate_database_backup_file(raw: &str, allowed_roots: &[PathBuf]) -> Result<Option<PathBuf>, String> {
    ...
    if !file_name.starts_with("chiron-horizon-backup__") {
...
mod tests {
    ...
    fn nonexistent_absolute_path_is_rejected() {
        ...
        let probe = if cfg!(windows) {
            "C:/__chiron_horizon_definitely_missing__/foo.sqlite".to_string()
        } else {
            "/__chiron_horizon_definitely_missing__/foo.sqlite".to_string()
        };
    ...
    fn database_backup_file_requires_absolute_sql_path() {
        ...
        let valid = if cfg!(windows) {
            "C:/tmp/chiron-horizon-backup__nightly.SQL"
        } else {
            "/tmp/chiron-horizon-backup__nightly.SQL"
        };
        assert!(validate_database_backup_file(valid, &[]).is_ok());
        let valid_gzip = if cfg!(windows) {
            "C:/tmp/chiron-horizon-backup__nightly.SQL.GZ"
        } else {
            "/tmp/chiron-horizon-backup__nightly.SQL.GZ"
        };
    ...
    async fn disconnected_root_does_not_abort_batch_cleanup() {
        let scratch = std::env::temp_dir().join(format!("chiron-horizon-backup-mixed-{}", uuid::Uuid::new_v4()));
    ...
    async fn custom_backup_outside_allowed_root_is_rejected() {
        let scratch = std::env::temp_dir().join(format!("chiron-horizon-backup-root-test-{}", uuid::Uuid::new_v4()));
    ...
    fn sqlite_header_is_detected() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-sqlite-header-{}.conf", uuid::Uuid::new_v4()));
    ...
    fn tilde_is_expanded_when_home_set() {
        ...
        // passes).
        let home =
            chiron_horizon_core::legacy::var("HOME").or_else(|_| chiron_horizon_core::legacy::var("USERPROFILE"));
        if let Ok(h) = home {
