...
/// resolved here (portable mode / GAUSS_HORIZON_DATA_DIR override) — the frontend only
...
mod tests {
    ...
    impl TempDir {
        fn new(label: &str) -> Self {
            let path = std::env::temp_dir().join(format!("gauss-horizon-tauri-{label}-{}", uuid::Uuid::new_v4()));
    ...
    fn clear_guard_rejects_unexpected_file_names() {
        ...
        assert!(!is_background_image_path(Path::new("/data/gauss-horizon.db")));
    }
}
