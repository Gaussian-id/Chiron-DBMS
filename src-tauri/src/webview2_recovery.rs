...
//!   environment is gone; a plain reload is ineffective. Chiron Horizon restarts the
...
//!   renderer may recover on its own, so Chiron Horizon does **not** reload on the first
...
//!   Chiron Horizon escalate to a controlled restart rather than looping forever.
//! - A successful reload resets the consecutive-failure counter.
//!
//! ## Session-task cancellation on reload
//!
//! Before reloading, Chiron Horizon cancels the tasks whose frontend consumer lives in
...
/// Consecutive `window.reload()` execution errors tolerated before Chiron Horizon
